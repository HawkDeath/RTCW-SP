...
bspbrush_t *HL_CreateBrushes_r(bspbrush_t *brush, int nodenum) {
  ...
  planenum =
      FindFloatPlane(hl_dplanes[planenum].normal, hl_dplanes[planenum].dist);
    ...
    for (sn = 0; sn < brush->numsides; sn++) {
      ...
      if (bestfacenum >= 0) {
        ...
        map_texinfo[texinfonum].value =
            0; // HL_ and HL texinfos don't have a value
        // the mip texture
        miptexlump = (hl_dmiptexlump_t *)hl_dtexdata;
        miptex =
            (hl_miptex_t *)((byte *)miptexlump +
                            miptexlump->dataofs[hl_texinfo[texinfonum].miptex]);
...
void HL_FixContentsTextures(bspbrush_t *brushlist) {
  ...
  for (brush = brushlist; brush; brush = brush->next)
  {
          ...
          for (i = 0; i < brush->numsides; i++)
          {
                  if (HL_TextureContents(map_texinfo[texinfonum].texture) !=
  brush->side)
                  {
                          Error("brush contents %d with wrong contents textures
  %s\n", brush->side, HL_TextureContents(map_texinfo[texinfonum].texture)); }
...
// Changes Globals:		-
//===========================================================================
void HL_ResetMapLoading(void) {} // end of the function HL_ResetMapLoading
//===========================================================================
//
// Parameter:				-
// Returns:					-
