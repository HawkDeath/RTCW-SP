...
void Sin_AllocMaxBSP(void) {
  ...
  sin_dleafbrushes = (unsigned short *)GetClearedMemory(
      SIN_MAX_MAP_LEAFBRUSHES * sizeof(unsigned short));
  ...
  // light info
  sin_numlightinfo = 0;
  sin_lightinfo = (sin_lightvalue_t *)GetClearedMemory(
      SIN_MAX_MAP_LIGHTINFO * sizeof(sin_lightvalue_t));
...
void Sin_AddLump(int lumpnum, void *data, int len, int size, int maxsize) {
  ...
  if (len > maxsize) {
    Error("Sin_WriteBSPFile: exceeded max size for lump %d size %d > maxsize "
          "%d\n",
          lumpnum, len, maxsize);
  }
