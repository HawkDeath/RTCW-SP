...
void HL_AllocMaxBSP(void) {
  ...
  hl_texdatasize = 0;
  hl_dtexdata =
      (byte *)GetMemory(HL_MAX_MAP_MIPTEX * sizeof(byte)); // (dmiptexlump_t)
  ...
  // clip nodes
  hl_numclipnodes = 0;
  hl_dclipnodes = (hl_dclipnode_t *)GetMemory(HL_MAX_MAP_CLIPNODES *
  ...
  hl_dmarksurfaces = (unsigned short *)GetMemory(HL_MAX_MAP_MARKSURFACES *
...
void HL_LoadBSPFile(char *filename, int offset, int length) {
  ...
  hl_nummarksurfaces = HL_CopyLump(HL_LUMP_MARKSURFACES, hl_dmarksurfaces,
                                   sizeof(hl_dmarksurfaces[0]));
  hl_numsurfedges =
  ...
  hl_dmarksurfaces_checksum = FastChecksum(
      hl_dmarksurfaces, hl_nummarksurfaces * sizeof(hl_dmarksurfaces[0]));
...
int ArrayUsage(char *szItem, int items, int maxitems, int itemsize) {
  ...
  qprintf("%-12s  %7i/%-7i  %7i/%-7i  (%4.1f%%)", szItem, items, maxitems,
...
int GlobUsage(char *szItem, int itemstorage, int maxstorage) {
  ...
  qprintf("%-12s     [variable]    %7i/%-7i  (%4.1f%%)", szItem, itemstorage,
...
void HL_PrintBSPFileSizes(void) {
  // int	numtextures = hl_texdatasize ?
  // ((hl_dmiptexlump_t*)hl_dtexdata)->nummiptex : 0; // TTimo: unused
  int totalmemory = 0;
  ...
  totalmemory += ArrayUsage("models", hl_nummodels, ENTRIES(hl_dmodels),
                            ENTRYSIZE(hl_dmodels));
  totalmemory += ArrayUsage("planes", hl_numplanes, ENTRIES(hl_dplanes),
                            ENTRYSIZE(hl_dplanes));
  totalmemory += ArrayUsage("vertexes", hl_numvertexes, ENTRIES(hl_dvertexes),
                            ENTRYSIZE(hl_dvertexes));
  totalmemory += ArrayUsage("nodes", hl_numnodes, ENTRIES(hl_dnodes),
                            ENTRYSIZE(hl_dnodes));
  totalmemory += ArrayUsage("texinfos", hl_numtexinfo, ENTRIES(hl_texinfo),
                            ENTRYSIZE(hl_texinfo));
  totalmemory += ArrayUsage("faces", hl_numfaces, ENTRIES(hl_dfaces),
                            ENTRYSIZE(hl_dfaces));
  totalmemory += ArrayUsage("clipnodes", hl_numclipnodes,
                            ENTRIES(hl_dclipnodes), ENTRYSIZE(hl_dclipnodes));
  totalmemory += ArrayUsage("leaves", hl_numleafs, ENTRIES(hl_dleafs),
                            ENTRYSIZE(hl_dleafs));
  totalmemory +=
      ArrayUsage("marksurfaces", hl_nummarksurfaces, ENTRIES(hl_dmarksurfaces),
                 ENTRYSIZE(hl_dmarksurfaces));
  totalmemory += ArrayUsage("surfedges", hl_numsurfedges,
                            ENTRIES(hl_dsurfedges), ENTRYSIZE(hl_dsurfedges));
  totalmemory += ArrayUsage("edges", hl_numedges, ENTRIES(hl_dedges),
                            ENTRYSIZE(hl_dedges));
