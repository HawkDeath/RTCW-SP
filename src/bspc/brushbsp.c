...
// Changes Globals:		-
//===========================================================================
int BrushOutOfBounds(bspbrush_t *brush, vec3_t mins, vec3_t maxs,
                     float epsilon) {
  int i, j, n;
  winding_t *w;
  side_t *side;
...
// Changes Globals:		-
//===========================================================================
int TestBrushToPlanenum(bspbrush_t *brush, int planenum, int *numsplits,
                        qboolean *hintsplit, int *epsilonbrush) {
  int i, j, num;
  plane_t *plane;
  int s = 0;
  winding_t *w;
  ...
  for (pass = 0; pass < numpasses; pass++) {
    for (brush = brushes; brush; brush = brush->next) {
      ...
      for (i = 0; i < brush->numsides; i++) {
        ...
        //					continue;	// never use a bevel as a
        //spliter
        if (!side->winding) {
        ...
        //					continue;	// skip surfaces are
        //never chosen 				if (!(side->flags & SFL_VISIBLE) && (pass < 2)) 					continue;
        //// only check visible faces on first pass
...
// Changes Globals:		-
//===========================================================================
void SplitBrushList(bspbrush_t *brushes, node_t *node, bspbrush_t **front,
                    bspbrush_t **back) {
