...
void Q3_DPlanes2MapPlanes(void) {
  ...
  for (i = 0; i < q3_numplanes; i++) {
    dplanes2mapplanes[i] =
        FindFloatPlane(q3_dplanes[i].normal, q3_dplanes[i].dist);
  } // end for
...
void Q3_BSPBrushToMapBrush(q3_dbrush_t *bspbrush, entity_t *mapent) {
  ...
  for (n = 0; n < bspbrush->numSides; n++) {
    ...
    if (bspbrushside->shaderNum < 0) {
      ...
      if (cfg.rs_allowladders &&
          strstr(q3_dshaders[bspbrushside->shaderNum].shader,
                 "common/ladder")) {
    ...
    if ((side->surf & SURF_NODRAW) && (side->surf & SURF_NOIMPACT))
    {
            ...
            Log_Print("probably found hint brush in a BSP without hints being
    used\n"); } //end if*/
  ...
  if (!strcmp("func_explosive",
              ValueForKey(&entities[b->entitynum], "classname")) ||
      !strcmp("func_invisible_user",
              ValueForKey(&entities[b->entitynum], "classname")) ||
      !strcmp("script_mover",
              ValueForKey(&entities[b->entitynum], "classname")) ||
      !strcmp("func_static",
  ...
  //		if (num_entities == 1)
  //		{
  //			Error ("Entity %i, Brush %i: origin brushes not allowed in
  //world" 				, b->entitynum, b->brushnum); 			return;
  //		}
  //
...
void AAS_CreateCurveBrushes(void) {
  ...
  for (i = 0; i < q3_numDrawSurfaces; i++) {
    ...
    for (j = 0; j < pc->numFacets; j++) {
      ...
      //
      planenum = FindFloatPlane(pc->planes[facet->surfacePlane].plane,
                                pc->planes[facet->surfacePlane].plane[3]);
      //
      ...
      if (create_aas) {
        ...
        side->planenum = FindFloatPlane(mapplanes[planenum ^ 1].normal,
                                        mapplanes[planenum ^ 1].dist + 1);
        side->flags |= SFL_TEXTURED | SFL_VISIBLE;
      } // end else
      side->contents = CONTENTS_SOLID;
