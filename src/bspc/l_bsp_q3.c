...
// Changes Globals:		-
//===========================================================================
void Q3_PlaneFromPoints(vec3_t p0, vec3_t p1, vec3_t p2, vec3_t normal,
                        float *dist) {
  vec3_t t1, t2;
  ...
  for (i = 0; i < q3_numbrushes; i++) {
    ...
    for (j = 0; j < brush->numSides; j++) {
      ...
      for (k = 0; k < q3_numDrawSurfaces; k++) {
        ...
        // the surface plane and the brush side plane should be pretty much the
        // same
  ...
  for (i = 0; i < q3_numShaders; i++) {
    ...
    q3_dshaders[i].contentFlags &=
        (CONTENTS_SOLID | CONTENTS_WINDOW | CONTENTS_LAVA | CONTENTS_SLIME |
         CONTENTS_WATER | CONTENTS_MIST | CONTENTS_AREAPORTAL |
         CONTENTS_PLAYERCLIP | CONTENTS_MONSTERCLIP | CONTENTS_ORIGIN |
         CONTENTS_DETAIL);
  }
...
void Q3_LoadBSPFile(struct quakefile_s *qf) {
  ...
  q3_numplanes = Q3_CopyLump(header, Q3_LUMP_PLANES, (void *)&q3_dplanes,
                             sizeof(q3_dplane_t));
  q3_numleafs = Q3_CopyLump(header, Q3_LUMP_LEAFS, (void *)&q3_dleafs,
                            sizeof(q3_dleaf_t));
  q3_numnodes = Q3_CopyLump(header, Q3_LUMP_NODES, (void *)&q3_dnodes,
                            sizeof(q3_dnode_t));
  q3_numleafsurfaces =
  ...
  q3_numDrawVerts = Q3_CopyLump(header, Q3_LUMP_DRAWVERTS,
                                (void *)&q3_drawVerts, sizeof(q3_drawVert_t));
  q3_numDrawSurfaces =
...
=============
*/
void Q3_AddLump(FILE *bspfile, q3_dheader_t *header, int lumpnum, void *data,
                int len) {
  q3_lump_t *lump;
