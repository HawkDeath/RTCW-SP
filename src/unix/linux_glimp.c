  ...
  default:
    if (XLookupRet == 0) {
      ...
      *key = *(unsigned char *)buf;
      ...
      // if ctrl is pressed, the keys are not between 'A' and 'Z', for instance
      // ctrl-z == 26 ^Z ^C etc. see show_bug.cgi?id=19
...
static void
signal_handler(int sig) { // bk010104 - replace this... (NOTE TTimo huh?)
  if (signalcaught) {
...
void GLimp_FrontEndSleep(void) { sem_wait(&renderCompletedEvent); }
...
void IN_Init(void) {
  ...
  joy_threshold =
      Cvar_Get("joy_threshold", "0.15", CVAR_ARCHIVE); // FIXME: in_joythreshold
...
void IN_Shutdown(void) { mouse_avail = qfalse; }
