...
void Sys_Exit(int ex) {
  ...
  if (exit_cmdline[0] != '\0') {
    // temporize, it seems if you spawn too fast before GL is released, or if
    // you exit too fast after the fork some kernel can panic (my 2.4.17 does)
    sleep(1);
...
void *Sys_LoadDll_Wrapped(const char *name, int (**entryPoint)(int, ...),
                          int (*systemcalls)(int, ...))
#else
  ...
  if (!libHandle) {
    ...
    if (!libHandle) {
      ...
      if (!libHandle) {
        ...
        if (strlen(gamedir) &&
            Q_stricmp(gamedir, BASEGAME)) { // begin BASEGAME != fs_game section
