...
GLXContext (*qglXCreateContext)(Display *dpy, XVisualInfo *vis,
                                GLXContext shareList, Bool direct);
void (*qglXDestroyContext)(Display *dpy, GLXContext ctx);
Bool (*qglXMakeCurrent)(Display *dpy, GLXDrawable drawable, GLXContext ctx);
void (*qglXCopyContext)(Display *dpy, GLXContext src, GLXContext dst,
                        GLuint mask);
...
void(APIENTRY *qglMap2d)(GLenum target, GLdouble u1, GLdouble u2, GLint ustride,
...
static void(APIENTRY *dllGetTexGeniv)(GLenum coord, GLenum pname,
                                      GLint *params);
static void(APIENTRY *dllGetTexImage)(GLenum target, GLint level, GLenum format,
                                      GLenum type, GLvoid *pixels);
...
static void(APIENTRY *dllVertexPointer)(GLint size, GLenum type, GLsizei stride,
                                        const GLvoid *pointer);
  ...
  if ((glw_state.OpenGLLib = dlopen(dllname, RTLD_LAZY | RTLD_GLOBAL)) == 0) {
    ...
    if (getuid() == saved_euid) {
      ...
      if ((glw_state.OpenGLLib = dlopen(fn, RTLD_LAZY)) == 0) {
        ri.Printf(
            PRINT_ALL,
            "QGL_Init: Can't load %s from /etc/ld.so.conf or current dir: %s\n",
            dllname, dlerror());
        return qfalse;
      }
    } else {
