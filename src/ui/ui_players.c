...
tryagain:
  ...
  if (pi->weaponModel == 0) {
    //		if( weaponNum == WP_MACHINEGUN ) {	//----(SA)	removing old weapon
    //references
    if (weaponNum == WP_MP40) {
      weaponNum = WP_NONE;
      goto tryagain;
    }
    //		weaponNum = WP_MACHINEGUN;	//----(SA)	removing old weapon
    //references
    weaponNum = WP_MP40;
    goto tryagain;
  }
...
===============
*/
void WM_getWeaponAnim(const char **torso_anim,
                      const char **legs_anim); // NERVE - SMF
...
void UI_DrawPlayer(float x, float y, float w, float h, playerInfo_t *pi,
  ...
  if (pi->pendingWeapon != -1 && dp_realtime > pi->weaponTimer) {
    ...
    if (pi->currentWeapon != pi->weapon) {
      trap_S_StartLocalSound(trap_S_RegisterSound("sound/weapons/change.wav"),
                             CHAN_LOCAL);
    }
  }
...
static qboolean AnimParseAnimConfig(playerInfo_t *animModelInfo,
  ...
  if (token && token[0]) {
    if (animModelInfo->version < 2 || !Q_stricmp(token, "HEADFRAMES")) {
      ...
      if (i != MAX_HEAD_ANIMS) {
        //				BG_AnimParseError( "Incorrect number of head
        //frames" );
        return qfalse;
      }
    }
