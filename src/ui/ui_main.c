...
static const char *netSources[] = {"Local", "Mplayer", "Internet", "Favorites"};
...
static const char *teamArenaGameTypes[] = {
    ...
    "1FCTF", "OVERLOAD",   "HARVESTER", "TEAMTOURNAMENT"};
...
static const char *teamArenaGameNames[] = {
    ...
    "Overload",        "Harvester",        "Team Tournament",
};

static int const numTeamArenaGameNames =
    sizeof(teamArenaGameNames) / sizeof(const char *);

static const int numServerFilters =
    sizeof(serverFilters) / sizeof(serverFilter_t);

static const char *sortKeys[] = {"Server Name", "Map Name", "Open Player Spots",
                                 "Game Type", "Ping Time"};
...
// TTimo: unused
// static char quake3worldMessage[] = "Visit www.quake3world.com - News,
// Community, Events, Files";
...
void Text_PaintCenter(float x, float y, int font, float scale, vec4_t color,
...
void Text_PaintChar(float x, float y, float width, float height, int font,
...
void Text_Paint(float x, float y, int font, float scale, vec4_t color,
      ...
      if (Q_IsColorString(s)) {
        ...
        if (style == ITEM_TEXTSTYLE_SHADOWED ||
          ...
          Text_PaintChar(x + ofs, y - yadj + ofs, glyph->imageWidth,
                         glyph->imageHeight, font, useScale, glyph->s, glyph->t,
                         glyph->s2, glyph->t2, glyph->glyph);
          trap_R_SetColor(newColor);
          colorBlack[3] = 1.0;
        }
        Text_PaintChar(x, y - yadj, glyph->imageWidth, glyph->imageHeight, font,
...
void Text_PaintWithCursor(float x, float y, int font, float scale, vec4_t color,
  ...
  if (text) {
      ...
      if (Q_IsColorString(s)) {
        ...
        if (count == cursorPos &&
          ...
          Text_PaintChar(x, y - yadj, glyph2->imageWidth, glyph2->imageHeight,
                         font, useScale, glyph2->s, glyph2->t, glyph2->s2,
    ...
    if (cursorPos == len && !((uiInfo.uiDC.realTime / BLINK_DIVISOR) & 1)) {
      yadj = useScale * glyph2->top;
      Text_PaintChar(x, y - yadj, glyph2->imageWidth, glyph2->imageHeight, font,
...
// TTimo: unused
/*
static void Text_Paint_Limit(float *maxX, float x, float y, int font, float
scale, vec4_t color, const char* text, float adjust, int limit) { int len,
count; vec4_t newColor; glyphInfo_t *glyph; if (text) { const unsigned char *s =
text; float max = *maxX; float useScale;
                ...
                while (s && *s && count < len) {
                        ...
                        if ( Q_IsColorString( s ) ) {
                                memcpy( newColor,
g_color_table[ColorIndex(*(s+1))], sizeof( newColor ) ); newColor[3] = color[3];
                                trap_R_SetColor( newColor );
                                s += 2;
                                continue;
                        } else {
...
void _UI_Refresh(int realtime) {
  ...
  if (Menu_Count() > 0) {
    ...
    if (mymenu != UIMENU_BRIEFING) {
      UI_DrawHandlePic(uiInfo.uiDC.cursorx - 16, uiInfo.uiDC.cursory - 16, 32,
                       32, uiInfo.uiDC.Assets.cursor);
    }
  }
  ...
  while (1) {
    ...
    if (Q_stricmp(token.string, "handwritingFont") == 0) {
      ...
      trap_R_RegisterFont(tempStr, pointSize,
                          &uiInfo.uiDC.Assets.handwritingFont);
      continue;
    }
...
static void UI_LoadTranslationStrings(void) {
  ...
  len = trap_FS_FOpenFile(filename, &f, FS_READ);
  if (len <= 0) {
    //		CG_Printf( S_COLOR_RED "WARNING: string translation file (strings.txt
...
static void UI_DrawHandicap(rectDef_t *rect, int font, float scale,
  ...
  Text_Paint(rect->x, rect->y, font, scale, color, handicapValues[i], 0, 0,
             textStyle);
}
...
==============
*/
static void UI_DrawSavegameName(rectDef_t *rect, int font, float scale,
                                vec4_t color, int textStyle) {
  Text_PaintCenter(rect->x, rect->y, font, scale, color, ui_savegameName.string,
                   textStyle);
}
...
==============
*/
static void UI_DrawClanName(rectDef_t *rect, int font, float scale,
                            vec4_t color, int textStyle) {
  Text_Paint(rect->x, rect->y, font, scale, color,
...
static void UI_DrawGameType(rectDef_t *rect, int font, float scale,
                            vec4_t color, int textStyle) {
  Text_Paint(rect->x, rect->y, font, scale, color,
             uiInfo.gameTypes[ui_gameType.integer].gameType, 0, 0, textStyle);
}

static void UI_DrawNetGameType(rectDef_t *rect, int font, float scale,
                               vec4_t color, int textStyle) {
...
static void UI_DrawJoinGameType(rectDef_t *rect, int font, float scale,
                                vec4_t color, int textStyle) {
...
static int UI_SavegameIndexFromName(const char *name) {
  ...
  if (name && *name) {
    for (i = 0; i < uiInfo.savegameCount; i++) {
      //			if (Q_stricmp(name,
      //uiInfo.savegameList[i].savegameName) == 0) {
      if (Q_stricmp(
              name,
              uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[i]]
                  .savegameName) == 0) {
        return i;
      }
    }
...
static void UI_DrawSaveGameShot(rectDef_t *rect, float scale, vec4_t color) {
  ...
  if (!strlen(ui_savegameName.string) || ui_savegameName.string[0] == '0') {
    ...
    if (uiInfo.savegameList[i].sshotImage == -1) {
      uiInfo.savegameList[i].sshotImage =
          trap_R_RegisterShaderNoMip(uiInfo.savegameList[i].imageName);
    }
...
uiInfo.savegameList[uiInfo.savegameIndex].sshotImage); trap_R_SetColor(NULL);
}
*/
...
static void UI_DrawPregameCinematic(rectDef_t *rect, float scale,
                                    vec4_t color) {
  if (uiInfo.previewMovie > -2) {
    uiInfo.previewMovie =
        trap_CIN_PlayCinematic(va("%s.roq", "assault"), 0, 0, 0, 0,
                               (CIN_loop | CIN_silent | CIN_system));
...
static void UI_DrawSkill(rectDef_t *rect, int font, float scale, vec4_t color,
                         int textStyle) {
  int i;
  ...
  Text_Paint(rect->x, rect->y, font, scale, color, skillLevels[i - 1], 0, 0,
             textStyle);
}

static void UI_DrawTeamName(rectDef_t *rect, int font, float scale,
                            vec4_t color, qboolean blue, int textStyle) {
  int i;
  i = UI_TeamIndexFromName(
      UI_Cvar_VariableString((blue) ? "ui_blueTeam" : "ui_redTeam"));
  if (i >= 0 && i < uiInfo.teamCount) {
    Text_Paint(
        rect->x, rect->y, font, scale, color,
        va("%s: %s", (blue) ? "Blue" : "Red", uiInfo.teamList[i].teamName), 0,
        0, textStyle);
  }
}

static void UI_DrawTeamMember(rectDef_t *rect, int font, float scale,
                              vec4_t color, qboolean blue, int num,
                              int textStyle) {
#ifdef MISSIONPACK
  // 0 - None
  // 1 - Human
...
orientation is 'horizontal' vert		- 4
        nohudalpha	- 8		// don't adjust bar's alpha value by the
cg_hudalpha value bg			- 16	// background contrast box (bg
set with bgColor of 'NULL' means use default bg color (1,1,1,0.25) spacing
- 32	// some bars use different sorts of spacing when drawing both an inner
and outer box
...
void UI_FilledBar(float x, float y, float w, float h, float *startColor,
  ...
  if ((flags & BAR_BG)) {
    ...
    UI_FillRect(x, y, w, h, backgroundcolor);
...
static void UI_DrawLoadStatus(rectDef_t *rect, vec4_t color, int align) {
  ...
  if (align != HUD_HORIZONTAL) {
    flags |= 4; // BAR_VERT
                //		flags|=1;	// BAR_LEFT (left, when vertical means grow
                //'up')
  }
  ...
  if (expectedHunk > 0) {
    ...
    if (percentDone >
        0.97) { // never actually show 100%, since we are not in the game yet
      percentDone = 0.97;
    }

    UI_FilledBar(rect->x, rect->y, rect->w, rect->h, color, NULL, NULL,
                 percentDone,
                 flags); // flags (BAR_CENTER|BAR_VERT|BAR_LERP_COLOR)
  } else {
    //		Text_Paint( rect->x, rect->y, UI_FONT_DEFAULT, 0.2f, color, "Please
    //Wait...", 0, 0, 0);
    Text_Paint(rect->x, rect->y, UI_FONT_DEFAULT, 0.2f, color,
               DC->getTranslatedString("pleasewait"), 0, 0, 0);
  }
}
//----(SA)	end
...
static void UI_DrawMapTimeToBeat(rectDef_t *rect, int font, float scale,
                                 vec4_t color, int textStyle) {
...
static void UI_DrawNetSource(rectDef_t *rect, int font, float scale,
                             vec4_t color, int textStyle) {
...
static void UI_DrawNetFilter(rectDef_t *rect, int font, float scale,
                             vec4_t color, int textStyle) {
...
static void UI_DrawTier(rectDef_t *rect, int font, float scale, vec4_t color,
                        int textStyle) {
  int i;
...
static void UI_DrawTierMapName(rectDef_t *rect, int font, float scale,
                               vec4_t color, int textStyle) {
  int i, j;
  i = trap_Cvar_VariableValue("ui_currentTier");
  if (i < 0 || i >= uiInfo.tierCount) {
...
static void UI_DrawTierGameType(rectDef_t *rect, int font, float scale,
                                vec4_t color, int textStyle) {
  int i, j;
  i = trap_Cvar_VariableValue("ui_currentTier");
  if (i < 0 || i >= uiInfo.tierCount) {
...
static void UI_DrawAllMapsSelection(rectDef_t *rect, int font, float scale,
                                    vec4_t color, int textStyle, qboolean net) {
#ifdef MISSIONPACK
  int map = (net) ? ui_currentNetMap.integer : ui_currentMap.integer;
  if (map >= 0 && map < uiInfo.mapCount) {
    Text_Paint(rect->x, rect->y, font, scale, color,
...
static void UI_DrawOpponentName(rectDef_t *rect, int font, float scale,
                                vec4_t color, int textStyle) {
  Text_Paint(rect->x, rect->y, font, scale, color,
...
static int UI_OwnerDrawWidth(int ownerDraw, int font, float scale) {
  ...
  case UI_KEYBINDSTATUS:
    if (Display_KeyBindPending()) {
      //			s = "Waiting for new key... Press ESCAPE to
      //cancel";
      s = DC->getTranslatedString("keywait");
    } else {
      //			s = "Press ENTER or CLICK to change, Press
      //BACKSPACE to clear";
...
static void UI_DrawBotName(rectDef_t *rect, int font, float scale, vec4_t color,
                           int textStyle) {
#ifdef MISSIONPACK
...
static void UI_DrawBotSkill(rectDef_t *rect, int font, float scale,
                            vec4_t color, int textStyle) {
...
static void UI_DrawRedBlue(rectDef_t *rect, int font, float scale, vec4_t color,
                           int textStyle) {
  Text_Paint(rect->x, rect->y, font, scale, color,
...
static void UI_DrawCrosshair(rectDef_t *rect, float scale, vec4_t color) {
  ...
  UI_DrawHandlePic(rect->x, rect->y - rect->h, rect->w, rect->h,
                   uiInfo.uiDC.Assets.crosshairShader[ch]);
  trap_R_SetColor(NULL);
}
...
static void UI_DrawSelectedPlayer(rectDef_t *rect, int font, float scale,
                                  vec4_t color, int textStyle) {
...
static void UI_DrawServerRefreshDate(rectDef_t *rect, int font, float scale,
                                     vec4_t color, int textStyle) {
#ifdef MISSIONPACK
  if (uiInfo.serverStatus.refreshActive) {
    ...
    Text_Paint(rect->x, rect->y, font, scale, newColor,
...
static void UI_DrawServerMOTD(rectDef_t *rect, int font, float scale,
                              vec4_t color) {
#ifdef MISSIONPACK
  if (uiInfo.serverStatus.motdLen) {
    ...
    Text_Paint_Limit(&maxX, uiInfo.serverStatus.motdPaintX,
                     rect->y + rect->h - 3, font, scale, color,
                     &uiInfo.serverStatus.motd[uiInfo.serverStatus.motdOffset],
                     0, 0);
    if (uiInfo.serverStatus.motdPaintX2 >= 0) {
...
static void UI_DrawKeyBindStatus(rectDef_t *rect, int font, float scale,
                                 vec4_t color, int textStyle) {
  // int ofs = 0; // TTimo: unused
  if (Display_KeyBindPending()) {
    //		Text_Paint(rect->x, rect->y, font, scale, color, "Waiting for new
    //key... Press ESCAPE to cancel", 0, 0, textStyle);
    Text_Paint(rect->x, rect->y, font, scale, color,
               DC->getTranslatedString("keywait"), 0, 0, textStyle);
  } else {
    //		Text_Paint(rect->x, rect->y, font, scale, color, "Press ENTER or CLICK
    //to change, Press BACKSPACE to clear", 0, 0, textStyle);
    Text_Paint(rect->x, rect->y, font, scale, color,
               DC->getTranslatedString("keychange"), 0, 0, textStyle);
  }
}

static void UI_DrawGLInfo(rectDef_t *rect, int font, float scale, vec4_t color,
                          int textStyle) {
  char *eptr;
  char buff[4096];
  const char *lines[64];
  ...
  //	i = 0;
  //	while (i < numLines) {
  //		Text_Paint(rect->x + 2, y, font, scale, color, lines[i++], 0, 20,
  //textStyle); 		if (i < numLines) { 			Text_Paint(rect->x + rect->w / 2, y, font,
  //scale, color, lines[i++], 0, 20, textStyle);
  //		}
  //		y += 10;
  //		if (y > rect->y + rect->h - 11) {
  //			break;
...
// NERVE - SMF
static void UI_DrawLimboChat(rectDef_t *rect, int font, float scale,
                             vec4_t color, int textStyle) {
  int i, count;
  char buf[140];
  float x, y, w, h;
...
static void UI_OwnerDraw(float x, float y, float w, float h, float text_x,
                         ...
                         int align, float special, int font, float scale,
  ...
  case UI_BLUETEAM4:
  case UI_BLUETEAM5:
    UI_DrawTeamMember(&rect, font, scale, color, qtrue,
  ...
  case UI_REDTEAM4:
  case UI_REDTEAM5:
    UI_DrawTeamMember(&rect, font, scale, color, qfalse,
...
static int QDECL UI_SavegamesQsortCompare(const void *arg1, const void *arg2) {
  ...
  if (uiInfo.savegameStatus.sortKey == SORT_SAVENAME) {
    ...
    // (SA) better way to do this?  (i was adding up seconds, but that seems
    // slower than a bunch of comparisons)
    i = sg->tm.tm_year;
    j = sg2->tm.tm_year;
    if (i < j) {
      ret = -1;
...
void UI_SavegameSort(int column, qboolean force) {
  ...
  if (uiInfo.savegameCount) {
    qsort(&uiInfo.savegameStatus.displaySavegames[0], uiInfo.savegameCount,
          sizeof(int), UI_SavegamesQsortCompare);
...
void UI_ParseSavegame(int index) {
  ...
  trap_FS_FOpenFile(va("save/%s.svg", uiInfo.savegameList[index].savegameFile),
                    &f, FS_READ);
  if (!f) {
    return;
  }
  ...
  if (ver < 9) { // don't try anything for really old savegames
    ...
    uiInfo.savegameList[index].savegameInfoText =
        "Gametime: (unknown)\nHealth: (unknown)\n(old savegame)";
    ...
    uiInfo.savegameList[index].time =
        String_Alloc(va("(old savegame ver: %d)", ver));
    return;
  }
  ...
  if (ver > 14) {
    ...
    trap_FS_Read(&tm->tm_yday, sizeof(tm->tm_yday),
                 f); // days since jan1 (0-365)
    trap_FS_Read(&tm->tm_isdst, sizeof(tm->tm_isdst), f);
    uiInfo.savegameList[index].time = String_Alloc(
        va("%s %i, %i   %02i:%02i", monthStr[tm->tm_mon], tm->tm_mday,
           1900 + tm->tm_year, tm->tm_hour, tm->tm_min));
  } else {
    memset(&uiInfo.savegameList[index].tm, 0, sizeof(qtime_t));
    uiInfo.savegameList[index].time =
        String_Alloc(va("(old save ver: %d)", ver));
  }
...
static void UI_LoadSavegames(char *dir) {
  ...
  if (dir) {
    uiInfo.savegameCount =
        trap_FS_GetFileList(va("save/%s", dir), "svg", sglist, 4096);
  } else {
  ...
  if (uiInfo.savegameCount) {
    ...
    for (i = 0; i < uiInfo.savegameCount; i++) {
      ...
      if (!Q_stricmp(
              sgname,
              "current.svg")) { // ignore some savegames that have special uses
                                // and shouldn't be loaded by the user directly
        i--;
      ...
      if (dir) {
        uiInfo.savegameList[i].savegameFile =
            String_Alloc(va("%s/%s", dir, sgname));
      } else {
      ...
      //			qsort(
      //&uiInfo.savegameStatus.displaySavegames[0], uiInfo.savegameCount,
      //sizeof(int), UI_SavegamesQsortCompare);
      ...
      if (uiInfo.savegameList[i].episode != -1) {
        uiInfo.savegameList[i].sshotImage =
            trap_R_RegisterShaderNoMip(va("levelshots/episodeshots/e%d.tga",
                                          uiInfo.savegameList[i].episode + 1));
      } else {
        uiInfo.savegameList[i].sshotImage =
...
static playerType_t playerTypes[] = {
    ...
    {"player_window_engineer",
     PT_KNIFE | PT_PISTOL | PT_LIGHTONLY | PT_EXPLOSIVES | PT_GRENADES},
...
static weaponType_t weaponTypes[] =
    {{0, "NULL", 0, "none", 0, "none", "", ""},
     ...
     {WP_COLT, "1911 pistol", PT_PISTOL, "mp_weapon", 0,
      "ui/assets/weapon_colt1911.tga", "firing_pistolB_1", "stand_pistolB"},
     {WP_LUGER, "Luger pistol", PT_PISTOL, "mp_weapon", 1,
      "ui/assets/weapon_luger.tga", "firing_pistolB_1", "stand_pistolB"},
     //	{ 0,		"Medkit",		PT_MEDKIT,	"mp_item2",		2, "ui/assets/item_medkit.tga",
     //"firing_pistolB_1",		"stand_pistolB" },

     {WP_MP40, "MP40 submachinegun", PT_LIGHTONLY | PT_RIFLE, "mp_weapon", 3,
      "ui/assets/weapon_mp40.tga", "relaxed_idle_2h_1", "relaxed_idle_2h_1"},
     {WP_THOMPSON, "Thompson submachinegun", PT_LIGHTONLY | PT_RIFLE,
      "mp_weapon", 4, "ui/assets/weapon_thompson.tga", "relaxed_idle_2h_1",
      "relaxed_idle_2h_1"},
     {WP_STEN, "Sten submachinegun", PT_LIGHTONLY | PT_RIFLE, "mp_weapon", 5,
      "ui/assets/weapon_sten.tga", "relaxed_idle_2h_1", "relaxed_idle_2h_1"},

     {WP_MAUSER, "Mauser sniper rifle", PT_RIFLE, "mp_weapon", 6,
      "ui/assets/weapon_mauser.tga", "stand_rifle", "stand_rifle"},
     {WP_GARAND, "Garand rifle", PT_RIFLE, "mp_weapon", 7,
      "ui/assets/weapon_mauser.tga", "stand_rifle", "stand_rifle"},
     {WP_PANZERFAUST, "Panzerfaust", PT_RIFLE, "mp_weapon", 8,
      "ui/assets/weapon_panzerfaust.tga", "stand_machinegun",
      "stand_machinegun"},
     {WP_VENOM, "Minigun", PT_RIFLE, "mp_weapon", 9,
      "ui/assets/weapon_venom.tga", "stand_machinegun", "stand_machinegun"},
     {WP_FLAMETHROWER, "Flamethrower", PT_RIFLE, "mp_weapon", 10,
      "ui/assets/weapon_flamethrower.tga", "stand_machinegun",
      "stand_machinegun"},

     {WP_GRENADE_PINEAPPLE, "Pineapple grenade", PT_GRENADES, "mp_item1", 11,
      "ui/assets/weapon_grenade.tga", "firing_pistolB_1", "stand_pistolB"},
     {WP_GRENADE_LAUNCHER, "Stick grenade", PT_GRENADES, "mp_item1", 12,
      "ui/assets/weapon_grenade_ger.tga", "firing_pistolB_1", "stand_pistolB"},

     {WP_DYNAMITE, "Explosives", PT_EXPLOSIVES, "mp_item2", 13,
      "ui/assets/weapon_dynamite.tga", "firing_pistolB_1", "stand_pistolB"},
...
static void WM_ChangePlayerType() {
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; itemTypes[j].name; j++) {
      ...
      if (itemdef2) {
        itemdef2->window.background =
            DC->registerShaderNoMip("ui/assets/item_none.tga");
      }
    }
    ...
    for (j = 0; itemTypes[j].name; j++) {
      ...
      if (itemdef2 && (playerTypes[i].items & itemTypes[j].flags)) {
        itemdef2->window.background =
            DC->registerShaderNoMip(itemTypes[j].shader);
      }
    }
  }
}
...
void WM_PickItem(int selectionType, int itemIndex) {
  ...
  if (selectionType == WM_SELECT_TEAM) {
    ...
    if (itemIndex == WM_START_SELECT) {
      ...
      trap_Cvar_Set(weaponTypes[itemIndex].cvar,
                    va("%i", weaponTypes[itemIndex].value));
    }
    ...
    if (itemIndex == WM_START_SELECT) {
      ...
      if (itemdef) {
        itemdef->window.background =
            DC->registerShaderNoMip(weaponTypes[itemIndex].name);
      }
...
static void UI_Update(const char *name) {
  ...
  if (Q_stricmp(name, "ui_SetName") == 0) {
    ...
    case 1: // normal
      ...
      trap_Cvar_Set(
          "r_texturemode",
          "GL_LINEAR_MIPMAP_NEAREST"); //----(SA)	modified so wolf never
                                       //sets trilinear automatically
    ...
    if (val == 0) {
      ...
      UI_LoadSavegames(
          "autosave"); // get from default directory 'main/save/autosave/*.svg'
    }
  }
  //----(SA)	end
}
...
static void UI_RunMenuScript(char **args) {
  ...
  if (String_Parse(args, &name)) {
    //#ifdef MISSIONPACK			// NERVE - SMF - enabled for
    //multiplayer
    if (Q_stricmp(name, "StartServer") == 0) {
      ...
      // SP was like...
      //			trap_Cmd_ExecuteText( EXEC_APPEND, "exec
      ...
      trap_Cvar_Set("g_playerstart",
                    "1"); // set cvar which will trigger "playerstart" in script
      Menus_CloseAll();
      //----(SA)	end
      ...
      if (trap_Cvar_VariableValue("g_gametype") >= GT_TEAM) {
        ...
        // NERVE - SMF - no bots in wolf multiplayer
        //				trap_Cmd_ExecuteText( EXEC_APPEND, va("addbot
      ...
      // NERVE - SMF
      //----(SA)	// start other .exe
    } else if (Q_stricmp(name, "startSingleplayer") ==
               0) { // so it doesn't barf if it gets a mp menu
...
static void UI_BuildServerDisplayList(qboolean force) {
  ...
  for (i = 0; i < count; i++) {
    ...
    if (ping > 0 || ui_netSource.integer == AS_FAVORITES) {
      ...
      /*			// NERVE - SMF - comment out for now, not
      recognizing "gametype" properly if
      (uiInfo.joinGameTypes[ui_joinGameType.integer].gtEnum != -1) { game =
      atoi(Info_ValueForKey(info, "gametype")); if (game !=
  ...
  *handle = -1;
  if (feederID == FEEDER_HEADS) {
    ...
    if (index >= 0 && index < uiInfo.serverStatus.numDisplayServers) {
      int ping, game;
      if (lastServerColumn != column ||
          lastServerTime > uiInfo.uiDC.realTime + 5000) {
    ...
    if (index >= 0 && index < uiInfo.savegameCount) {
      ...
      if (lastSaveColumn != column) {
        //				trap_LAN_GetServerInfo(ui_netSource.integer,
        //uiInfo.serverStatus.displayServers[index], info, MAX_STRING_CHARS);
      ...
      case SORT_SAVENAME:
        return uiInfo
            .savegameList[uiInfo.savegameStatus.displaySavegames[index]]
            .savegameName;
        break;
      case SORT_SAVETIME:
        return uiInfo
...
static qhandle_t UI_FeederItemImage(float feederID, int index) {
  if (feederID == FEEDER_HEADS) {
    ...
    if (index >= 0 && index < uiInfo.savegameCount) {
      if (uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]]
              .sshotImage == -1) {
        uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]]
            .sshotImage = trap_R_RegisterShaderNoMip(va(
            "levelshots/%s.tga",
            uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]]
                .mapName));
      }
      return uiInfo.savegameList[uiInfo.savegameStatus.displaySavegames[index]]
          .sshotImage;
    }
  }
...
static qboolean Team_Parse(char **p) {
        ...
        while ( 1 ) {
                ...
                if (token[0] == '{') {
                        // seven tokens per line, team name and icon, and 5 team
member names if (!String_Parse(p, &uiInfo.teamList[uiInfo.teamCount].teamName)
...
&tempStr)) { return qfalse;
                        ...
                        uiInfo.characterList[uiInfo.characterCount].headImage =
-1; uiInfo.characterList[uiInfo.characterCount].imageName =
...
{ uiInfo.characterList[uiInfo.characterCount].female = qtrue; } else {
                                uiInfo.characterList[uiInfo.characterCount].female
= qfalse;
                        }
...
static qboolean Alias_Parse(char **p) {
        ...
        while ( 1 ) {
                ...
                if (token[0] == '{') {
                        // three tokens per line, character name, bot alias, and
preferred action a - all purpose, d - defense, o - offense if (!String_Parse(p,
...
static qboolean MapList_Parse(char **p) {
  ...
  while (1) {
    ...
    if (token[0] == '{') {
      ...
      uiInfo.mapList[uiInfo.mapCount].levelShot = trap_R_RegisterShaderNoMip(va(
          "levelshots/%s_small", uiInfo.mapList[uiInfo.mapCount].mapLoadName));
      ...
      if (Q_stricmpn(skinname, "icon_", 5) == 0 &&
        ...
        if (Q_stricmp(skinname, "icon_default") == 0) {
          Com_sprintf(uiInfo.q3HeadNames[uiInfo.q3HeadCount],
                      sizeof(uiInfo.q3HeadNames[uiInfo.q3HeadCount]), dirptr);
        } else {
          Com_sprintf(uiInfo.q3HeadNames[uiInfo.q3HeadCount],
                      sizeof(uiInfo.q3HeadNames[uiInfo.q3HeadCount]), "%s/%s",
                      dirptr, skinname + 5);
        }
...
void _UI_SetActiveMenu(uiMenuCommand_t menu) {
  ...
  if (Menu_Count() > 0) {
    ...
    if (menu == UIMENU_BRIEFING &&
        menutype == menu) { // don't let briefing be set multiple times
      return;
    }
    ...
    case UIMENU_NEED_CD:
      ...
      //			UI_ConfirmMenu( "Insert the CD", NULL,
      //NeedCDAction );
      return;
    ...
    case UIMENU_BAD_CD_KEY:
      ...
      //			UI_ConfirmMenu( "Bad CD Key", NULL,
      //NeedCDKeyAction );
      return;
...
static void UI_DisplayDownloadInfo(const char *downloadName, float centerPoint,
  ...
  Text_PaintCenter(centerPoint, yStart + 112, font, scale, colorWhite, dlText,
                   0);
  Text_PaintCenter(centerPoint, yStart + 144, font, scale, colorWhite, etaText,
                   0);
  Text_PaintCenter(centerPoint, yStart + 208, font, scale, colorWhite, xferText,
                   0);
  ...
  Text_Paint(centerPoint, yStart + 244, font, 0.6f, colorWhite, s, 0, 0,
             ITEM_TEXTSTYLE_SHADOWEDMORE);
  ...
  if (downloadCount < 4096 || !downloadTime) {
    ...
    Text_PaintCenter(leftWidth, 192, font, 0.6f, colorWhite,
    ...
    if (downloadSize && xferRate) {
      ...
      Text_PaintCenter(leftWidth, 160, font, 0.6f, colorWhite, "estimating", 0);
      if (downloadSize) {
        Text_PaintCenter(leftWidth, 160, font, 0.6f, colorWhite,
                         va("(%s of %s copied)", dlSizeBuf, totalSizeBuf), 0);
      } else {
        Text_PaintCenter(leftWidth, 160, font, 0.6f, colorWhite,
...
void UI_DrawConnectScreen(qboolean overlay) {
  ...
  if (trap_GetConfigString(CS_SERVERINFO, info, sizeof(info))) {
    Text_PaintCenter(centerPoint, yStart, UI_FONT_DEFAULT, scale, colorWhite,
                     va("Loading %s", Info_ValueForKey(info, "mapname")), 0);
  }

  if (!Q_stricmp(cstate.servername, "localhost")) {
    //		Text_PaintCenter(centerPoint, yStart + 48, UI_FONT_DEFAULT, scale,
    //colorWhite, va("Get Psyched!"), ITEM_TEXTSTYLE_SHADOWEDMORE);
  } else {
    strcpy(text, va("Connecting to %s", cstate.servername));
    Text_PaintCenter(centerPoint, yStart + 48, UI_FONT_DEFAULT, scale,
                     colorWhite, text, ITEM_TEXTSTYLE_SHADOWEDMORE);
  }
  ...
  Text_PaintCenter(centerPoint, 600, UI_FONT_DEFAULT, scale, colorWhite,
                   Info_ValueForKey(cstate.updateInfoString, "motd"), 0);
  // print any server info (server full, bad version, etc)
  if (cstate.connState < CA_CONNECTED) {
    Text_PaintCenter(centerPoint, yStart + 176, UI_FONT_DEFAULT, scale,
  ...
  case CA_CONNECTED: {
    ...
    trap_Cvar_VariableStringBuffer("cl_downloadName", downloadName,
                                   sizeof(downloadName));
    if (*downloadName) {
      UI_DisplayDownloadInfo(downloadName, centerPoint, yStart, UI_FONT_DEFAULT,
                             scale);
      return;
    }
  }
  ...
  if (Q_stricmp(cstate.servername, "localhost")) {
    Text_PaintCenter(centerPoint, yStart + 80, UI_FONT_DEFAULT, scale,
                     colorWhite, s, 0);
  }
...
cvarTable_t cvarTable[] = {
    ...
    {&ui_useSuggestedWeapons, "cg_useSuggestedWeapons", "1",
     CVAR_ARCHIVE},                                         //----(SA)	added
