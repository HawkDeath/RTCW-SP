...
typedef struct {
  ...
  const char
      *onROQDone; //----(SA)	added.  callback for roqs played from menus
...
typedef struct {
  ...
  void (*drawText)(float x, float y, int font, float scale, vec4_t color,
...
void UI_RoQDone(
    void); // ui callback.  roq is done, allow menu to handle if it wants
