...
translateString_t translateStrings[] = {
    ...
    {"keychange"},     //	"Press ENTER or CLICK to change, Press BACKSPACE to
                       //clear"
...
void Script_Clipboard(itemDef_t *item, char **args) {
  char curscript[64];
  DC->getCVarString("cg_clipboardName", curscript,
                    sizeof(curscript)); // grab the string the client set
...
void Script_NotebookShowpage(itemDef_t *item, char **args) {
  ...
  if (Int_Parse(args, &inc)) {
    ...
    DC->setCVar("ui_notebookCurrentPage",
                va("%d", curpage)); // store new current page
  }
}
...
commandDef_t commandList[] = {
    ...
    {"addlistitem", &Script_AddListItem} // NERVE - SMF - special command to add
                                         // text items to list box
};
...
qboolean Item_TextField_HandleKey(itemDef_t *item, int key) {
  ...
  if (item->cvar) {
    ...
    if (key & K_CHAR_FLAG) {
      ...
      // 'valid filename' text entry.  so users can't try to save games like
      // '!**$er' and stuff (from the menu)
    ...
    if (key == K_TAB || key == K_DOWNARROW || key == K_KP_DOWNARROW) {
      ...
      if (newItem && (newItem->type == ITEM_TYPE_EDITFIELD ||
                      ...
                      newItem->type == ITEM_TYPE_VALIDFILEFIELD)) {
        g_editItem = newItem;
      }
    }
...
void Menus_Activate(menuDef_t *menu) {
  ...
  if (menu->soundName && *menu->soundName) {
    //		DC->stopBackgroundTrack();					// you don't want to do this since
    //it will reset s_rawend
...
void Menu_HandleKey(menuDef_t *menu, int key, qboolean down) {
  ...
  if (g_editingField && down) {
    if (!Item_TextField_HandleKey(g_editItem, key)) {
      ...
      //		} else if (key == K_TAB || key == K_UPARROW || key ==
      //K_DOWNARROW) {
    } else {
      return;
    }
  ...
  if (down && !(menu->window.flags & WINDOW_POPUP) &&
    ...
    // TTimo: gcc: suggest parentheses around && within ||
    // initial line:
    // if (!inHandleKey && key == K_MOUSE1 || key == K_MOUSE2 || key ==
    // K_MOUSE3) { changed behaviour
    if (!inHandleKey &&
...
void Item_SetTextExtents(itemDef_t *item, int *width, int *height,
  ...
  *height = item->textRect.h;
  ...
  if (*width == 0 || (item->type == ITEM_TYPE_OWNERDRAW &&
    ...
    int originalWidth =
        DC->textWidth(item->text, item->font, item->textscale, 0);

    if (item->type == ITEM_TYPE_OWNERDRAW &&
        (item->textalignment == ITEM_ALIGN_CENTER ||
         item->textalignment == ITEM_ALIGN_RIGHT)) {
      originalWidth += DC->ownerDrawWidth(item->window.ownerDraw, item->font,
                                          item->textscale);
    } else if ((item->type == ITEM_TYPE_EDITFIELD ||
                item->type == ITEM_TYPE_VALIDFILEFIELD) &&
  ...
  while (p) {
    ...
    if ((newLine && textWidth > item->window.rect.w) || *p == '\n' ||
      ...
      if (len) {
        ...
        //
        buff[newLine] = '\0';
        DC->drawText(item->textRect.x, item->textRect.y, item->font,
                     item->textscale, color, buff, 0, 0, item->textStyle);
      }
      if (*p == '\0') {
        break;
      }
...
void Item_Text_Wrapped_Paint(itemDef_t *item) {
  ...
  while (p && *p) {
    ...
    buff[p - start] = '\0';
    DC->drawText(x, y, item->font, item->textscale, color, buff, 0, 0,
                 item->textStyle);
    y += height + 5;
    start += p - start + 1;
    p = strchr(p + 1, '\r');
  }
  DC->drawText(x, y, item->font, item->textscale, color, start, 0, 0,
               item->textStyle);
}
...
void Item_Text_Paint(itemDef_t *item) {
  ...
  if (item->textSavegameInfo) {
    DC->getCVarString("ui_savegameInfo", infostring,
                      sizeof(infostring)); // grab the string the client set
  ...
  */
  ...
  //item->window.foreColor, textPtr, adjust); 		Text_Paint(item->textRect.x,
  //item->textRect.y-1, item->textscale, item->window.foreColor, textPtr,
  //adjust); 		Text_Paint(item->textRect.x+1, item->textRect.y-1, item->textscale,
  //item->window.foreColor, textPtr, adjust); 		Text_Paint(item->textRect.x-1,
  //item->textRect.y, item->textscale, item->window.foreColor, textPtr, adjust);
  //		Text_Paint(item->textRect.x+1, item->textRect.y, item->textscale,
  //item->window.foreColor, textPtr, adjust); 		Text_Paint(item->textRect.x-1,
  //item->textRect.y+1, item->textscale, item->window.foreColor, textPtr,
  //adjust); 		Text_Paint(item->textRect.x, item->textRect.y+1, item->textscale,
  //item->window.foreColor, textPtr, adjust); 		Text_Paint(item->textRect.x+1,
  //item->textRect.y+1, item->textscale, item->window.foreColor, textPtr,
  //adjust);
  //		*/
  ...
  DC->drawText(item->textRect.x, item->textRect.y, item->font, item->textscale,
               color, textPtr, 0, 0, item->textStyle);
}

// float			trap_Cvar_VariableValue( const char *var_name );
// void			trap_Cvar_VariableStringBuffer( const char *var_name, char
// *buffer, int bufsize );
...
void Item_TextField_Paint(itemDef_t *item) {
  ...
  if (item->window.flags & WINDOW_HASFOCUS && g_editingField) {
    ...
    DC->drawTextWithCursor(item->textRect.x + item->textRect.w + offset,
                           ...
                           newColor, buff + editPtr->paintOffset,
    ...
    DC->drawText(item->textRect.x + item->textRect.w + offset, item->textRect.y,
                 item->font, item->textscale, newColor,
...
void Item_YesNo_Paint(itemDef_t *item) {
  ...
  if (item->text) {
    ...
    DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y,
                 item->font, item->textscale, newColor,
                 (value != 0) ? yes_str : no_str, 0, 0, item->textStyle);
  } else {
    DC->drawText(item->textRect.x, item->textRect.y, item->font,
                 item->textscale, newColor, (value != 0) ? yes_str : no_str, 0,
                 0, item->textStyle);
  }
}
...
void Item_Multi_Paint(itemDef_t *item) {
  ...
  if (item->text) {
    Item_Text_Paint(item);
    DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y,
                 item->font, item->textscale, newColor, text, 0, 0,
                 item->textStyle);
  } else {
    DC->drawText(item->textRect.x, item->textRect.y, item->font,
...
void Item_Bind_Paint(itemDef_t *item) {
  ...
  if (item->text) {
    ...
    DC->drawText(item->textRect.x + item->textRect.w + 8, item->textRect.y,
                 item->font, item->textscale, newColor, g_nameBind1, 0,
                 maxChars, item->textStyle);
  } else {
    DC->drawText(item->textRect.x, item->textRect.y, item->font,
                 item->textscale, newColor, (value != 0) ? "FIXME" : "FIXME", 0,
                 maxChars, item->textStyle);
  }
}
...
qboolean Item_Bind_HandleKey(itemDef_t *item, int key, qboolean down) {
  ...
  if (Rect_ContainsPoint(&item->window.rect, DC->cursorx, DC->cursory) &&
    ...
    case K_BACKSPACE:
      ...
      if (id != -1) {
        key = -1; // null out the key, but let it pass down so it can get
                  // 'unbound' if it just returns here, the old bindings don't
                  // get cleared out. so if user has both 'r' and 'g' bound to
                  // '+attack' and they only want the 'r', they click to bind
                  // the key, <backsp> to kill the bindings (which appears to
                  // the user to work) then they click to bind and hit 'r'.  now
                  // the menu looks right to them, but if you drop the menu and
                  // come back, the 'g' is magically re-bound since it didn't
                  // get bound to "" on the <backsp>.  <phew> does this seem
                  // reasonable to anybody reading this? (SA)
      ...
      for (i = listPtr->startPos; i < count; i++) {
        ...
        if (listPtr->numColumns > 0) {
          ...
          for (j = 0; j < listPtr->numColumns; j++) {
            ...
            if (optionalImage >= 0) {
              ...
              DC->drawText(x + 4 + listPtr->columnInfo[j].pos,
                           y + listPtr->elementHeight, item->font,
          ...
          if (optionalImage >= 0) {
            ...
            DC->drawText(x + 4, y + listPtr->elementHeight, item->font,
  ...
  if (DC->ownerDrawItem) {
    ...
    if (item->text) {
      ...
      if (item->text[0]) {
        ...
        DC->ownerDrawItem(
            item->textRect.x + item->textRect.w + 8, item->window.rect.y,
            item->window.rect.w, item->window.rect.h, 0, item->textaligny,
            item->window.ownerDraw, item->window.ownerDrawFlags,
            item->alignment, item->special, item->font, item->textscale, color,
            item->window.background, item->textStyle);
      } else {
        DC->ownerDrawItem(
            item->textRect.x + item->textRect.w, item->window.rect.y,
            item->window.rect.w, item->window.rect.h, 0, item->textaligny,
            item->window.ownerDraw, item->window.ownerDrawFlags,
            item->alignment, item->special, item->font, item->textscale, color,
            item->window.background, item->textStyle);
