...
// Ridah
static qboolean R_LoadMDC(model_t *mod, int lod, void *buffer,
                          const char *mod_name);
// done.
...
qhandle_t RE_RegisterModel(const char *name) {
  ...
  for (lod = MD3_MAX_LODS - 1; lod >= 0; lod--) {
    ...
    if (ident == MD3_IDENT) {
      ...
      if (r_compressModels->integer && r_exportCompressedModels->integer &&
          mod->mdc[lod]) {
        // save it out
...
=================
*/
qboolean R_MDC_EncodeXyzCompressed(const vec3_t vec, const vec3_t normal,
  ...
  for (i = 0; i < 3; i++) {
    ...
    retval.ofsVec +=
        (((int)fabs((vec[i] + MDC_DIST_SCALE * 0.5) * (1.0 / MDC_DIST_SCALE) +
                    MDC_MAX_OFS))
         << (i * MDC_BITS_PER_AXIS));
  }
...
=================
*/
static qboolean R_MDC_GetXyzCompressed(md3Header_t *md3, md3XyzNormal_t *newXyz,
                                       vec3_t oldPos, mdcXyzCompressed_t *out,
                                       qboolean verify) {
  vec3_t newPos, vec;
  int i;
...
=================
*/
static qboolean R_MDC_CompressSurfaceFrame(md3Header_t *md3, md3Surface_t *surf,
                                           int frame, int lastBaseFrame,
                                           mdcXyzCompressed_t *out) {
  int i, j;
...
=================
*/
static qboolean R_MDC_CanCompressSurfaceFrame(md3Header_t *md3,
                                              md3Surface_t *surf, int frame,
                                              int lastBaseFrame) {
  int i, j;
  md3XyzNormal_t *xyz, *baseXyz;
...
static qboolean R_MDC_ConvertMD3(model_t *mod, int lod, const char *mod_name) {
  ...
  // we need to treat the entire model as a single surface, if we compress some
  // surfaces, and not others, we may get tearing between surfaces
  ...
  for (f = 1; f < md3->numFrames; f++) {
    ...
    for (i = 0; i < md3->numSurfaces; i++) {
      ...
      if (!foundBase && !R_MDC_CanCompressSurfaceFrame(
                            md3, surf, f, baseFrames[numBaseFrames - 1])) {
  ...
  mdcHeader.ofsTagNames =
      mdcHeader.ofsFrames + mdcHeader.numFrames * sizeof(md3Frame_t);
  mdcHeader.ofsTags =
      mdcHeader.ofsTagNames + mdcHeader.numTags * sizeof(mdcTagName_t);
  mdcHeader.ofsSurfaces = mdcHeader.ofsTags + mdcHeader.numTags *
                                                  mdcHeader.numFrames *
                                                  sizeof(mdcTag_t);
  ...
  for (f = 0; f < md3->numSurfaces; f++) {
    mdcHeader.ofsEnd +=
        sizeof(mdcSurface_t) + surf->numShaders * sizeof(md3Shader_t) +
        surf->numTriangles * sizeof(md3Triangle_t) +
        surf->numVerts * sizeof(md3St_t) +
        surf->numVerts * numBaseFrames * sizeof(md3XyzNormal_t) +
        surf->numVerts * (md3->numFrames - numBaseFrames) *
            sizeof(mdcXyzCompressed_t) +
  ...
  *mdc = mdcHeader;
  // frames
  memcpy((md3Frame_t *)((byte *)mdc + mdc->ofsFrames),
         (md3Frame_t *)((byte *)md3 + md3->ofsFrames),
         mdcHeader.numFrames * sizeof(md3Frame_t));
  // tag names
  for (j = 0; j < md3->numTags; j++) {
    memcpy((mdcTagName_t *)((byte *)mdc + mdc->ofsTagNames) + j,
           ((md3Tag_t *)((byte *)md3 + md3->ofsTags) + j)->name,
           sizeof(mdcTagName_t));
  }
  // tags
  ...
  for (j = 0; j < md3->numSurfaces; j++) {
    ...
    cSurf->ofsShaders =
        cSurf->ofsTriangles + cSurf->numTriangles * sizeof(md3Triangle_t);
    ...
    cSurf->ofsXyzCompressed = cSurf->ofsXyzNormals + cSurf->numVerts *
                                                         numBaseFrames *
                                                         sizeof(md3XyzNormal_t);
    cSurf->ofsFrameBaseFrames =
        cSurf->ofsXyzCompressed + cSurf->numVerts *
                                      (mdc->numFrames - numBaseFrames) *
                                      sizeof(mdcXyzCompressed_t);
    cSurf->ofsFrameCompFrames =
        cSurf->ofsFrameBaseFrames + mdc->numFrames * sizeof(short);
    cSurf->ofsEnd = cSurf->ofsFrameCompFrames + mdc->numFrames * sizeof(short);

    // triangles
    memcpy((byte *)cSurf + cSurf->ofsTriangles,
           (byte *)surf + surf->ofsTriangles,
           cSurf->numTriangles * sizeof(md3Triangle_t));
    // shaders
    memcpy((byte *)cSurf + cSurf->ofsShaders, (byte *)surf + surf->ofsShaders,
           cSurf->numShaders * sizeof(md3Shader_t));
    // st
    memcpy((byte *)cSurf + cSurf->ofsSt, (byte *)surf + surf->ofsSt,
           cSurf->numVerts * sizeof(md3St_t));
    ...
    for (f = 0, i = 0, c = 0; f < md3->numFrames; f++) {
      if (i < numBaseFrames && f == baseFrames[i]) {
        ...
        memcpy((byte *)cSurf + cSurf->ofsXyzNormals +
                   (sizeof(md3XyzNormal_t) * cSurf->numVerts * i),
               (byte *)surf + surf->ofsXyzNormals +
                   (sizeof(md3XyzNormal_t) * cSurf->numVerts * f),
        ...
        if (!R_MDC_CompressSurfaceFrame(
                ...
                (mdcXyzCompressed_t *)((byte *)cSurf + cSurf->ofsXyzCompressed +
                                       sizeof(mdcXyzCompressed_t) *
                                           cSurf->numVerts * c))) {
          ri.Error(ERR_DROP,
                   "R_MDC_ConvertMD3: tried to compress an unsuitable frame\n");
        }
        frameCompFrames[f] = c;
        frameBaseFrames[f] = i - 1;
        c++;
...
*/
void RE_BeginRegistration(renderconfig_t *renderContextOut) {
  ri.Hunk_Clear(); // (SA) MEM NOTE: not in missionpack

  R_Init();
  *renderContextOut = renderConfig;
...
================
*/
static int R_GetTag(byte *mod, int frame, const char *tagName,
...
================
*/
static int R_GetMDCTag(byte *mod, int frame, const char *tagName,
                       int startTagIndex, mdcTag_t **outTag) {
  mdcTag_t *tag;
...
int R_LerpTag(orientation_t *tag, const refEntity_t *refent,
              const char *tagNameIn, int startIndex) {
  md3Tag_t *start, *end;
  ...
  */
  ...
  if (model->type == MOD_MESH) {
    // old MD3 style
    retval = R_GetTag((byte *)model->md3[0], startFrame, tagName, startIndex,
                      &start);
    retval =
    ...
    retval = R_GetMDCTag((byte *)model->mdc[0], startFrame, tagName, startIndex,
                         &cstart);
    retval = R_GetMDCTag((byte *)model->mdc[0], endFrame, tagName, startIndex,
                         &cend);
...
void *R_Hunk_Begin(void) {
  ...
  // anything NOTE: in SP we currently default to r_cache 0, meaning this code
  // is not used at all
  //   I am merging the MP way of doing things though, just in case (since we
  //   have r_cache 1 on linux MP)
  if (!membase) {
