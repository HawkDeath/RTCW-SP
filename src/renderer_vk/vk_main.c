...
void R_Fog(glfog_t *curfog) {
 ...
 //   curfog->mode = GL_LINEAR;
  ...
  //    qglFogf(GL_FOG_END,
  //            curfog->end +
  //                1000); // snooper ends GL fog out further.  this works fine
  //                       // with our maps, but could be 'funky' with later maps
  //  }
...
void R_SetFog(int fogvar, int var1, int var2, float r, float g, float b,
  ...
  if (glfogsettings[FOG_CURRENT].registered) {
    ...
    //		memcpy(&glfogsettings[FOG_LAST], &glfogsettings[FOG_MAP],
    //sizeof(glfog_t));
...
void R_SetFrameFog(void) {
  ...
  if (glfogsettings[FOG_TARGET].finishTime &&
    ...
    if (glfogsettings[FOG_LAST].mode == /*GL_EXP*/0 &&
        glfogsettings[FOG_TARGET].mode == /*GL_LINEAR*/1) {
    ...
    else if (glfogsettings[FOG_LAST].mode == /*GL_LINEAR*/1 &&
             glfogsettings[FOG_TARGET].mode == /*GL_EXP*/0) {
      memcpy(&glfogsettings[FOG_CURRENT], &glfogsettings[FOG_TARGET],
             sizeof(glfog_t));
      glfogsettings[FOG_TARGET].finishTime = 0;
    }
  ...
  if (glfogsettings[FOG_CURRENT].mode == /*GL_LINEAR*/1) {
    if (glfogsettings[FOG_CURRENT].end < tr.viewParms.zFar) {
      tr.viewParms.zFar = glfogsettings[FOG_CURRENT].end;
    }
    if (backEnd.refdef.rdflags & RDF_SNOOPERVIEW) {
      tr.viewParms.zFar +=
          1000; // zfar out slightly further for snooper.  this works fine with
                // our maps, but could be 'funky' with later maps
    }
  }
  //	else
...
void R_SetupFrustum(void) {
  ...
  VectorMA(tr.viewParms.frustum[0].normal, xc, tr.viewParms.or.axis[1],
           tr.viewParms.frustum[0].normal);

  VectorScale(tr.viewParms.or.axis[0], xs, tr.viewParms.frustum[1].normal);
  VectorMA(tr.viewParms.frustum[1].normal, -xc, tr.viewParms.or.axis[1],
  ...
  VectorMA(tr.viewParms.frustum[2].normal, xc, tr.viewParms.or.axis[2],
           tr.viewParms.frustum[2].normal);

  VectorScale(tr.viewParms.or.axis[0], xs, tr.viewParms.frustum[3].normal);
  VectorMA(tr.viewParms.frustum[3].normal, -xc, tr.viewParms.or.axis[2],
           tr.viewParms.frustum[3].normal);

  for (i = 0; i < 4; i++) {
    tr.viewParms.frustum[i].type = PLANE_NON_AXIAL;
    tr.viewParms.frustum[i].dist =
        DotProduct(tr.viewParms.or.origin, tr.viewParms.frustum[i].normal);
...
static qboolean SurfIsOffscreen(const drawSurf_t *drawSurf,
  ...
  if (renderConfig
          .smpActive) { // FIXME!  we can't do RB_BeginSurface/RB_EndSurface
                            // stuff with smp!
    return qfalse;
  }
...
void R_AddDrawSurf(surfaceType_t *surface, shader_t *shader, int fogIndex,
  ...
  tr.refdef.drawSurfs[index].sort =
      ...
      (atiTess << QSORT_ATI_TESS_SHIFT) | tr.shiftedEntityNum |
