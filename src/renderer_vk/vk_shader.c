...
static qboolean ParseStage(shaderStage_t *stage, char **text) {
  ...
  while (1) {
    ...
    if (!Q_stricmp(token, "map")) {
      ...
      //----(SA) end
        ...
        stage->bundle[0].image[0] =
            R_FindImageFileExt(token, !shader.noMipMaps, !shader.noPicMip,
                               shader.characterMip, /*GL_REPEAT*/1);
        //----(SA)	end
    ...
    //
      ...
      stage->bundle[0].image[0] =
          R_FindImageFileExt(token, !shader.noMipMaps, !shader.noPicMip,
                             shader.characterMip, /*GL_CLAMP*/0);
      if (!stage->bundle[0].image[0]) {
        ri.Printf(
            PRINT_WARNING,
            "WARNING: R_FindImageFile could not find '%s' in shader '%s'\n",
      ...
      while (1) {
        ...
        if (num < MAX_IMAGE_ANIMATIONS) {
          stage->bundle[0].image[num] =
              R_FindImageFileExt(token, !shader.noMipMaps, !shader.noPicMip,
                                 shader.characterMip, /*GL_REPEAT*/0);
  ...
  if (strcmp(token, "-")) {
    for (i = 0; i < 6; i++) {
      ...
      shader.sky.outerbox[i] =
          R_FindImageFile((char *)pathname, qtrue, qtrue, /*GL_CLAMP*/0);
...
infoParm_t infoParms[] = {
    ...
    //	{"slime",		1,	0,	CONTENTS_SLIME },		// mildly
    //damaging
...
static qboolean ParseShader(char **text) {
  ...
  while (1) {
    ...
    // skyfogvars <(r,g,b)> <dist>
      ...
      if (atof(token) > 1) {
        ri.Printf(PRINT_WARNING, "WARNING: last value for skyfogvars is "
                                 "'density' which needs to be 0.0-1.0\n");
        continue;
      }
    ...
    //----(SA)	added
      ...
      if (atof(token) > 0) {
        tr.lightGridMulAmbient = atof(token);
                   ...
                   token,
                   "lightgridmuldir")) { // directional multiplier for lightgrid
      token = COM_ParseExt(text, qfalse);
      if (!token[0]) {
        ri.Printf(
            PRINT_WARNING,
    ...
    //----(SA)	end
      ...
      if (fogvar == 0) { // '0' specifies "use the map values for everything
        ...
        Com_sprintf(fogString, sizeof(fogString), "0 %d 1.1 %f %f %f 200",
                    (int)fogvar, watercolor[0], watercolor[1], watercolor[2]);
        //				R_SetFog(FOG_WATER, 0, fogvar, watercolor[0],
        //watercolor[1], watercolor[2], 1.1);
      } else { // density "exp" fog
        Com_sprintf(fogString, sizeof(fogString), "0 5 %f %f %f %f 200", fogvar,
                    watercolor[0], watercolor[1], watercolor[2]);
        //				R_SetFog(FOG_WATER, 0, 5, watercolor[0],
        //watercolor[1], watercolor[2], fogvar);
      }
    ...
    // fogvars
      ...
      //			R_SetFog(FOG_MAP, 0, fogFar, fogColor[0],
      //fogColor[1], fogColor[2], fogDensity);
      ri.Cvar_Set("r_mapFogColor", va("0 %d %f %f %f %f 0", fogFar, fogDensity,
                                      fogColor[0], fogColor[1], fogColor[2]));
      //			R_SetFog(FOG_CMD_SWITCHFOG, FOG_MAP, 50, 0, 0,
      //0, 0);
...
static collapse_t collapse[] = {
    {0, GLS_DSTBLEND_SRC_COLOR | GLS_SRCBLEND_ZERO, /*GL_MODULATE*/0, 0},

    {0, GLS_DSTBLEND_ZERO | GLS_SRCBLEND_DST_COLOR, /*GL_MODULATE*/0, 0},

    {GLS_DSTBLEND_ZERO | GLS_SRCBLEND_DST_COLOR,
     GLS_DSTBLEND_ZERO | GLS_SRCBLEND_DST_COLOR, /*GL_MODULATE*/0,
     GLS_DSTBLEND_ZERO | GLS_SRCBLEND_DST_COLOR},
    ...
    {GLS_DSTBLEND_ZERO | GLS_SRCBLEND_DST_COLOR,
     GLS_DSTBLEND_SRC_COLOR | GLS_SRCBLEND_ZERO, /*GL_MODULATE*/0,
     GLS_DSTBLEND_ZERO | GLS_SRCBLEND_DST_COLOR},
    ...
    {0, GLS_DSTBLEND_ONE | GLS_SRCBLEND_ONE, /*GL_ADD*/0, 0},

    {GLS_DSTBLEND_ONE | GLS_SRCBLEND_ONE, GLS_DSTBLEND_ONE | GLS_SRCBLEND_ONE,
     /*GL_ADD*/0, GLS_DSTBLEND_ONE | GLS_SRCBLEND_ONE},
#if 0
...
shader_t *R_FindShader(const char *name, int lightmapIndex,
  ...
  for (sh = hashTable[hash]; sh; sh = sh->next) {
    ...
    */
    if (((sh->lightmapIndex == lightmapIndex) ||
         (sh->lightmapIndex < 0 && lightmapIndex >= 0)) &&
        !Q_stricmp(sh->name, strippedName)) {
      // match found
      return sh;
    }
  ...
  image = R_FindImageFile(fileName, mipRawImage, mipRawImage,
                          mipRawImage ? /*GL_REPEAT*/0 : /*GL_CLAMP*/1);
  if (!image) {
...
void R_ShaderList_f(void) {
  ...
  for (i = 0; i < tr.numShaders; i++) {
    ...
    if (shader->multitextureEnv == /*GL_ADD*/0) {
      ri.Printf(PRINT_ALL, "MT(a) ");
    } else if (shader->multitextureEnv == /*GL_MODULATE*/1) {
      ri.Printf(PRINT_ALL, "MT(m) ");
    } else if (shader->multitextureEnv == /*GL_DECAL*/2) {
...
static void CreateExternalShaders(void) {
  //	tr.projectionShadowShader = R_FindShader( "projectionShadow",
  //LIGHTMAP_NONE, qtrue );
