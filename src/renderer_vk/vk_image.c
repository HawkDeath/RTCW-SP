...
static float R_RMSE(byte *in, int width, int height) {
  ...
  for (i = 0; i < height; i++, in += row) {
    for (j = 0; j < width; j++, out += 4, in += 8) {
      ...
      rtemp = ((fabs(out - in[0]) + fabs(out - in[4]) +
                fabs(out - in[row + 0]) + fabs(out - in[row + 4])));
      rtemp = rtemp * rtemp;
      rmse += rtemp;
      out = (in[1] + in[5] + in[row + 1] + in[row + 5]) >> 2;
      rtemp = ((fabs(out - in[1]) + fabs(out - in[5]) +
                fabs(out - in[row + 1]) + fabs(out - in[row + 5])));
      rtemp = rtemp * rtemp;
      rmse += rtemp;
      out = (in[2] + in[6] + in[row + 2] + in[row + 6]) >> 2;
      rtemp = ((fabs(out - in[2]) + fabs(out - in[6]) +
                fabs(out - in[row + 2]) + fabs(out - in[row + 6])));
      rtemp = rtemp * rtemp;
      rmse += rtemp;
      out = (in[3] + in[7] + in[row + 3] + in[row + 7]) >> 2;
      rtemp = ((fabs(out - in[3]) + fabs(out - in[7]) +
                fabs(out - in[row + 3]) + fabs(out - in[row + 7])));
      rtemp = rtemp * rtemp;
      rmse += rtemp;
    }
  }
...
static void Upload32(unsigned *data, int width, int height, qboolean mipmap,
                     ...
                     qboolean lightMap, int *format, int *pUploadWidth,
                     int *pUploadHeight, qboolean noCompress) {
  int samples;
  ...
  if (r_rmse->value) {
    while (R_RMSE((byte *)data, width, height) < r_rmse->value) {
      ...
      ResampleTexture(data, width, height, resampledBuffer, width >> 1,
                      height >> 1);
  ...
  if (r_lowMemTextureSize->integer &&
      (scaled_width > r_lowMemTextureSize->integer ||
       scaled_height > r_lowMemTextureSize->integer) &&
      rmse < r_lowMemTextureThreshold->value) {
    int scale;
    ...
    ri.Printf(PRINT_ALL,
              "r_lowMemTextureSize forcing reduction from %i x %i to %i x %i\n",
...
================
*/
image_t *R_CreateImageExt(const char *name, const byte *pic, int width,
                          int height, qboolean mipmap, qboolean allowPicmip,
                          qboolean characterMip, int glWrapClampMode) {
  image_t *image;
...
image_t *R_CreateImage(const char *name, const byte *pic, int width, int height,
                       ...
                       int glWrapClampMode) {
  return R_CreateImageExt(name, pic, width, height, mipmap, allowPicmip, qfalse,
                          glWrapClampMode);
}
...
==============
*/
static void LoadPCX(const char *filename, byte **pic, byte **palette,
...
==============
*/
static void LoadPCX32(const char *filename, byte **pic, int *width,
                      int *height) {
  byte *palette;
  byte *pic8;
  int i, c, p;
...
static void LoadJPG(const char *filename, unsigned char **pic, int *width,
  ...
  out = R_GetImageBuffer(cinfo.output_width * cinfo.output_height *
...
image_t *R_FindImageFileExt(const char *name, qboolean mipmap,
                            qboolean allowPicmip, qboolean characterMIP,
                            int glWrapClampMode) {
  image_t *image;
  int width, height;
  byte *pic;
  ...
  if (r_cacheGathering->integer) {
    ri.Cmd_ExecuteText(EXEC_NOW,
                       va("cache_usedfile image %s %i %i %i %i\n", name, mipmap,
                          allowPicmip, characterMIP, glWrapClampMode));
  }
    ...
    if (!Q_stricmp(name, image->imgName)) {
      ...
      if (strcmp(name, "*white")) {
        ...
        if (image->characterMIP != characterMIP) {
          ri.Printf(PRINT_DEVELOPER,
                    "WARNING: reused image %s with mixed characterMIP parm\n",
                    name);
        }
...
// developer
  ...
  image = R_CreateImageExt((char *)name, pic, width, height, mipmap,
                           allowPicmip, characterMIP, glWrapClampMode);
  // ri.Free( pic );
  return image;
}
...
static void R_CreateDlightImage(void) {
  ...
  tr.dlightImage = R_CreateImage("*dlight", (byte *)data, DLIGHT_SIZE,
                                 DLIGHT_SIZE, qfalse, qfalse, /*GL_CLAMP*/0);
}
...
static void R_CreateFogImage(void) {
  ...
  // what we want.
  tr.fogImage = R_CreateImage("*fog", (byte *)data, FOG_S, FOG_T, qfalse,
                              qfalse, /*GL_CLAMP*/0);
...
static void R_CreateDefaultImage(void) {
  ...
  for (x = 0; x < DEFAULT_SIZE; x++) {
    data[0][x][0] = data[0][x][1] = data[0][x][2] =
        0; //----(SA) to make the default grid noticable but not blinding
    data[0][x][3] = 255;

    data[x][0][0] = data[x][0][1] = data[x][0][2] =
        0; //----(SA) to make the default grid noticable but not blinding
    data[x][0][3] = 255;
  ...
  tr.defaultImage = R_CreateImage("*default", (byte *)data, DEFAULT_SIZE,
                                  DEFAULT_SIZE, qtrue, qfalse, /*GL_REPEAT*/0);
}
...
void R_CreateBuiltinImages(void) {
  ...
  tr.whiteImage =
      R_CreateImage("*white", (byte *)data, 8, 8, qfalse, qfalse, /*GL_REPEAT*/0);
...
//----(SA) added so client can see what model or scale for the model was
//specified in a skin
/*
==============
RE_GetSkinModel
...
qboolean RE_GetSkinModel(qhandle_t skinid, const char *type, char *name) {
  ...
  if (!Q_stricmp(type, "playerscale")) { // client is requesting scale from the
                                         // skin rather than a model
    Com_sprintf(name, MAX_QPATH, "%.2f %.2f %.2f", bar->scale[0], bar->scale[1],
                bar->scale[2]);
    return qtrue;
  }

  for (i = 0; i < bar->numModels; i++) {
    if (!Q_stricmp(bar->models[i]->type,
                   type)) { // (SA) whoops, should've been this way
...
==============
*/
qhandle_t RE_GetShaderFromModel(qhandle_t modelid, int surfnum,
                                int withlightmap) {
  model_t *model;
  bmodel_t *bmodel;
  msurface_t *surf;
  ...
  if (model) {
    ...
    if (bmodel && bmodel->firstSurface) {
      if (surfnum >=
          bmodel
              ->numSurfaces) { // if it's out of range, return the first surface
        surfnum = 0;
      }
...
qhandle_t RE_RegisterSkin(const char *name) {
  ...
  //----(SA)	allow this for the (current) special case of the loper's upper
  //body 			(it's upper body has no surfaces, only tags)
  if (skin->numSurfaces == 0) {
...
qboolean R_CropImage(char *name, byte **pic, int border, int *width,
                     int *height, int lastBox[2]) {
...
#define FADE_BORDER_RANGE ((*width) / 40)
  ...
  for (row = 0; row < rows; row++) {
    ...
    for (col = 0; col < cols; col++, inpixel += 4) {
      if (filterColors) {
        ...
        // if this pixel is near the edge, then fade it out (just do a
        // brightness fade)
        if ((int *)inpixel) {
          // calc the fade scale
  ...
  */
  ...
  // for some reason this causes memory drop, not worth investigating (dev
  // command only)
  // ri.Free( *pic );
...
===============
*/
void R_CropAndNumberImagesInDirectory(char *dir, char *ext, int maxWidth,
                                      int maxHeight, int withAlpha) {
#ifdef CROPIMAGES_ENABLED
  char **fileList;
  int numFiles, j;
#ifndef RESIZE
  ...
  for (j = 0; j < numFiles; j++) {
    ...
    for (i = 2; (1 << i) <= maxWidth; i++) {
      if ((width < (1 << i)) && (width > (1 << (i - 1)))) {
        ...
        ResampleTexture((unsigned int *)pic, width, height,
                        (unsigned int *)temppic, newWidth, newHeight);
    ...
    ResampleTexture((unsigned int *)pic, width, height, (unsigned int *)temppic,
                    newWidth, newHeight);
    memcpy(pic, temppic, sizeof(unsigned int) * newWidth * newHeight);
    free(temppic);
    width = newWidth;
    ...
    *pch = '\0';
    ...
    lastNumber -= c * 10;
    d = lastNumber;
    Com_sprintf(outfilename, sizeof(outfilename), "%s/spr%i%i%i.tga", filename,
                b, c, d);
...
void R_CropImages_f(void) {
  ...
  if (ri.Cmd_Argc() < 5) {
    ri.Printf(PRINT_ALL,
              "syntax: cropimages <dir> <extension> <maxWidth> <maxHeight> "
              "<alpha 0/1>\neg: 'cropimages sprites/fire1 .tga 64 64 0'\n");
    return;
  }
  R_CropAndNumberImagesInDirectory(ri.Cmd_Argv(1), ri.Cmd_Argv(2),
                                   atoi(ri.Cmd_Argv(3)), atoi(ri.Cmd_Argv(4)),
                                   atoi(ri.Cmd_Argv(5)));
#else
...
qboolean R_TouchImage(image_t *inImage) {
  ...
  if (inImage == tr.dlightImage || inImage == tr.whiteImage ||
      ...
      inImage->imgName[0] ==
          '*') { // can't use lightmaps since they might have the same name, but
                 // different maps will have different actual lightmap pixels
    return qfalse;
  }
