...
static void R_LoadLightmaps(lump_t *l) {
  ...
  for (i = 0; i < tr.numLightmaps; i++) {
    ...
    if (r_lightmap->integer == 2) { // color code by intensity as development
                                    // tool	(FIXME: check range)
    ...
    tr.lightmaps[i] = R_CreateImage(va("*lightmap%d", i), image, LIGHTMAP_SIZE,
                                    LIGHTMAP_SIZE, qfalse, qfalse, /*GL_CLAMP*/0);
  }
...
static shader_t *ShaderForShaderNum(int shaderNum, int lightmapNum) {
  ...
  //if (r_vertexLight->integer || vkConfig.hardwareType == GLHW_PERMEDIA2) {
...
static void R_LoadSurfaces(lump_t *surfs, lump_t *verts, lump_t *indexLump) {
  ...
  ri.Printf(PRINT_ALL,
            ...
            numMeshes, numTriSurfs, numFlares);
}
...
void R_FindLightGridBounds(vec3_t mins, vec3_t maxs) {
  ...
  //
  //			surfmin[0] = -dplanes[ dbrushsides[ db->firstSide + 0 ].planeNum
  //].dist - 1; 			surfmin[1] = -dplanes[ dbrushsides[ db->firstSide + 2 ].planeNum
  //].dist - 1; 			surfmin[2] = -dplanes[ dbrushsides[ db->firstSide + 4 ].planeNum
  //].dist - 1; 			surfmax[0] = dplanes[ dbrushsides[ db->firstSide + 1 ].planeNum
  //].dist + 1; 			surfmax[1] = dplanes[ dbrushsides[ db->firstSide + 3 ].planeNum
  //].dist + 1; 			surfmax[2] = dplanes[ dbrushsides[ db->firstSide + 5 ].planeNum
  //].dist + 1; 			AddPointToBounds (surfmin, mins, maxs); 			AddPointToBounds
  //(surfmax, mins, maxs);
  //		}
  //	}
...
void RE_LoadWorldMap(const char *name) {
  ...
  // invalidate fogs (likely to be re-initialized to new values by the current
  // map)
  // TODO:(SA)this is sort of silly.  I'm going to do a general cleanup on fog
  // stuff
  //			now that I can see how it's been used.  (functionality can
  //narrow since 			it's not used as much as it's designed for.)
