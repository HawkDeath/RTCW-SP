...
void R_PerformanceCounters(void) {
  ...
  //----(SA)	this is unnecessary since it will always show 2048.  I moved
  //this to where it is accurate for the world 	else if (r_speeds->integer == 5 )
  //	{
...
void RE_BeginFrame(stereoFrame_t stereoFrame) {
  ...
  if (r_measureOverdraw->integer) {
    if (renderConfig.stencilBits < 4) {
      ...
      ri.Printf(PRINT_ALL, "Warning: stencil shadows and overdraw measurement "
  ...
  if (renderConfig.stereoEnabled) {
    if (stereoFrame == STEREO_LEFT) {
      ...
      cmd->buffer = (int)/*GL_BACK_RIGHT*/1;
    } else {
      ri.Error(ERR_FATAL,
    ...
    if (!Q_stricmp(r_drawBuffer->string, "GL_FRONT")) {
      cmd->buffer = (int)/*GL_FRONT*/1;
    } else {
      cmd->buffer = (int)/*GL_BACK*/0;
    }
  }
}
