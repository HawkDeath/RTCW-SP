  ...
  while (1) {
    ...
    if (nodenum < 0) {
      ...
      if (-nodenum > (*aasworld).numareasettings) {
       ...
       // AAS_PointAreaNum(start)); if can't enter the area because it hasn't got
       // the right presence type
...
//
// Parameter:				face		: face to test if the point is in
// it
//								pnormal	: normal of the
//plane to use for the face 								point		: point to test if inside face
//boundaries
// Returns:					qtrue if the point is within the face
// boundaries Changes Globals:		-
//===========================================================================
...
#define AAS_BoxOnPlaneSide(absmins, absmaxs, p)                                \
  (((p)->type < 3)                                                             \
       ...
       : (AAS_BoxOnPlaneSide2((absmins), (absmaxs),                            \
