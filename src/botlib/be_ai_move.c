...
// Ridah, disabled this to prevent wierd navigational behaviour (mostly by
// Zombie, since it's so slow)
//#define AVOIDREACH
...
int BotGetReachabilityToGoal(vec3_t origin, int areanum, int entnum,
  ...
  for (reachnum = AAS_NextAreaReachability(areanum, 0); reachnum;
    ...
    t = AAS_AreaTravelTimeToGoalAreaCheckLoop(
        reach.areanum, reach.end, goal->areanum, travelflags, areanum);
    ...
    // if (AAS_AreaTravelTimeToGoalArea(areanum, reach.start, goal->areanum,
    // travelflags) + reach.traveltime < t) 	continue;

    // add the travel time towards the area
    // Ridah, not sure why this was disabled, but it causes looped links in the
    // route-cache RF, update.. seems to work better like this....
    t +=
        reach.traveltime; // + AAS_AreaTravelTime(areanum, origin, reach.start);
    ...
    //		if (reach.areanum != goal->areanum && AAS_IsEntityInArea( entnum,
    //goal->entitynum, reach.areanum )) { 			t += 50;
    //		}
  ...
  for (dist = 8; dist <= 100; dist += 8) {
    ...
    if (!trace.startsolid) {
      ...
      if (trace.endpos[2] < startz - sv_maxstep - 8) {
        ...
        //break; if a gap is found slow down botimport.Print(PRT_MESSAGE, "gap
        // at %f\n", dist);
        return dist;
      } // end if
...
void BotMoveToGoal(bot_moveresult_t *result, int movestate, bot_goal_t *goal,
  ...
  if (ms->moveflags & (MFL_ONGROUND | MFL_SWIMMING | MFL_AGAINSTLADDER)) {
    ...
    // Ridah, best calc it each frame, especially for Zombie, which moves so
    // slow that we can't afford to take a long route
    //		reachnum = 0;
    // done.
    if (reachnum) {
      AAS_ReachabilityFromNum(reachnum, &reach);
  ...
  if (reachnum > 0 &&
      !(result->flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_SWIMVIEW))) {
    vec3_t dir;
    int ftraveltime, freachnum, straveltime, ltraveltime;

    AAS_ReachabilityFromNum(reachnum, &reach);
    if (reach.areanum != goal->areanum) {
      if (AAS_AreaRouteToGoalArea(reach.areanum, reach.end, goal->areanum,
                                  travelflags, &straveltime, &freachnum)) {
        ltraveltime = 999999;
        while (AAS_AreaRouteToGoalArea(reach.areanum, reach.end, goal->areanum,
                                       travelflags, &ftraveltime, &freachnum)) {
