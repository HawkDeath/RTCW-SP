...
void AAS_JumpReachRunStart(aas_reachability_t *reach, vec3_t runstart) {
  ...
  if (move.stopevent &
      (SE_ENTERLAVA | SE_HITGROUNDDAMAGE)) { //----(SA)	modified since slime is
                                             //no longer deadly
...
//
// Parameter:				origin			: origin to start
// with
//								presencetype	: presence
//type to start with 								velocity			: velocity to start with 								cmdmove
      ...
      if (!gaptrace.startsolid) {
        ...
        if (gaptrace.endpos[2] < org[2] - aassettings.sv_maxstep - 1) {
          if (!(AAS_PointContents(end) &
                (CONTENTS_WATER |
                 CONTENTS_SLIME))) { //----(SA)	modified since slime is no
                                     //longer deadly
                                     //					if (!(AAS_PointContents(end) &
                                     //CONTENTS_WATER))
...
//								start		: start
//position of jump 								end		: end position of jump 								*speed	: returned speed
//for jump
// Returns:					qfalse if too high or too far from start to
// end Changes Globals:		-
//===========================================================================
