...
void AAS_SwapAASData(void) {
  ...
  for (i = 0; i < (*aasworld).numbboxes; i++) {
    ...
    for (j = 0; j < 3; j++) {
      (*aasworld).bboxes[i].mins[j] =
          LittleFloat((*aasworld).bboxes[i].mins[j]);
      (*aasworld).bboxes[i].maxs[j] =
          LittleFloat((*aasworld).bboxes[i].maxs[j]);
    } // end for
  }   // end for
    // vertexes
  ...
  for (i = 0; i < (*aasworld).numareasettings; i++) {
    ...
    (*aasworld).areasettings[i].groundsteepness =
        LittleLong((*aasworld).areasettings[i].groundsteepness);
  } // end for
    // area reachability
...
// Changes Globals:		-
//===========================================================================
char *AAS_LoadAASLump(fileHandle_t fp, int offset, int length,
                      int *lastoffset) {
  char *buf;
  //
  if (!length) {
