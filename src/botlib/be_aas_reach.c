      ...
      if (!trace.startsolid && trace.fraction >= 1.0) {
        ...
        if (AAS_PointAreaNum(trace.endpos) == area2num) {
          ...
          lreach->traveltime =
              ...
              fabs(ground_bestdist) * 50 / aassettings.sv_gravity;
...
//
// Parameter:			beststart1		start of the range of points on
// edge v1-v2
//						beststart2		end of the range of
//points  on edge v1-v2 						bestend1		start of the range of points on
//edge v3-v4 						bestend2		end of the range of points  on edge v3-v4
//						bestdist		best distance so
//far
// Returns:				-
// Changes Globals:		-
//===========================================================================
...
int AAS_Reachability_Jump(int area1num, int area2num) {
  ...
  if (bestdist > 4 && bestdist < maxjumpdistance) {
    ...
    //area2num, bestdist); if the fall would damage the bot
    //
...
void AAS_Reachability_FuncBobbing(void) {
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    //
    Log_Write("funcbob model %d, start = {%1.1f, %1.1f, %1.1f} end = {%1.1f, "
              "%1.1f, %1.1f}\n",
...
#ifndef BSPC
//					AAS_DrawArrow(lreach->start, lreach->end,
//LINECOLOR_BLUE, LINECOLOR_YELLOW); 					AAS_PermanentLine(lreach->start,
//lreach->end, 1);
#endif
...
int AAS_Reachability_Grapple(int area1num, int area2num) {
  ...
  for (i = 0; i < area2->numfaces; i++) {
    ...
    if ((*aasworld).areasettings[areanum].contents &
        AREACONTENTS_LAVA) { //----(SA)	modified since slime is no longer deadly
      //		if ((*aasworld).areasettings[areanum].contents &
...
void AAS_SetWeaponJumpAreaFlags(void) {
  ...
  for (ent = AAS_NextBSPEntity(0); ent; ent = AAS_NextBSPEntity(ent)) {
    ...
    if (!strcmp(classname, "item_armor_body") ||
        ...
        !strcmp(classname, "weapon_bfg") || !strcmp(classname, "item_quad") ||
      ...
      for (j = 0; j < area->numfaces; j++) {
        ...
        for (l = 0; l < face2->numedges; l++) {
          ...
          if (abs(edge1num) == abs(edge2num)) {
            ...
            lreach->traveltime =
                STARTWALKOFFLEDGE_TIME +
                fabs(mid[2] - trace.endpos[2]) * 50 / aassettings.sv_gravity;
...
// 0%
// TRAVEL_ROCKETJUMP			100%	(currently limited towards areas
