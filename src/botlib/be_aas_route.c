...
#define GROUNDSTEEPNESS_TIMESCALE                                              \
  20 // this is the maximum scale, 1 being the usual for a flat ground
...
// easily
void *AAS_RoutingGetMemory(int size) { return GetClearedMemory(size); }

void AAS_RoutingFreeMemory(void *ptr) { FreeMemory(ptr); }
// done.
...
__inline int AAS_ClusterAreaNum(int cluster, int areanum) {
  ...
  if (areacluster > 0) {
    ...
    cluster &&
                                    (*aasworld).portals[-areacluster].backcluster
    != cluster)
                    {
...
void AAS_InitRoutingUpdate(void) {
  ...
  /*
          ...
          for (i = 0; i < (*aasworld).numclusters; i++)
          {
                  if ((*aasworld).clusters[i].numreachabilityareas >
     maxreachabilityareas)
                  {
                          maxreachabilityareas =
     (*aasworld).clusters[i].numreachabilityareas; } //end if } //end for
...
void AAS_WriteRouteCache(void) {
  ...
  for (i = 0; i < (*aasworld).numareas; i++) {
    ...
    size =
        AAS_CompressVis((*aasworld).decompressedvis, (*aasworld).numareas, buf);
    botimport.FS_Write(&size, sizeof(int), fp);
...
int AAS_ReadRouteCache(void) {
  ...
  // GJD: route cache data MUST be written on a PC because I've not altered the
  // writing code.
  ...
  if (1 != LittleLong(1)) {
    for (i = 0; i < (*aasworld).numareas; i++) {
      (*aasworld).areawaypoints[i][0] =
          LittleFloat((*aasworld).areawaypoints[i][0]);
      (*aasworld).areawaypoints[i][1] =
          LittleFloat((*aasworld).areawaypoints[i][1]);
      (*aasworld).areawaypoints[i][2] =
          LittleFloat((*aasworld).areawaypoints[i][2]);
    }
  }
  //
  botimport.FS_FCloseFile(fp);
...
void AAS_UpdateAreaRoutingCache(aas_routingcache_t *areacache) {
  ...
  curupdate->areatraveltimes =
      (*aasworld).areatraveltimes[areacache->areanum][0];
...
// Changes Globals:		-
//===========================================================================
int AAS_AreaTravelTimeToGoalAreaCheckLoop(int areanum, vec3_t origin,
...
void AAS_CreateVisibility(void) {
  ...
  (*aasworld).areavisibility =
      (byte **)GetClearedMemory(numAreas * sizeof(byte *));
  (*aasworld).decompressedvis =
      (byte *)GetClearedMemory(numAreas * sizeof(byte));
  (*aasworld).areawaypoints =
  ...
  for (i = 1; i < numAreas; i++) {
    ...
    for (j = 1; j < numAreas; j++) {
      ...
      if (!AAS_inPVS((*aasworld).areawaypoints[i],
                     (*aasworld).areawaypoints[j])) {
        continue;
      }
...
#define MAX_HIDEAREA_LOOPS 3000
  ...
  startVisible = botimport.AICast_VisibleFromPos(enemyorigin, enemynum, origin,
                                                 srcnum, qfalse);
  //
  ...
  while (updateliststart) {
    ...
    reach =
        &(*aasworld).reachability
             [(*aasworld).areasettings[curupdate->areanum].firstreachablearea];
    //
          ...
          if (!AAS_AreaVisible(
            ...
            if (((*aasworld).visCache[nextareanum] == 2) ||
                (!(*aasworld).visCache[nextareanum] &&
                 !botimport.AICast_VisibleFromPos(
...
// Changes Globals:		-
//===========================================================================
int AAS_FindAttackSpotWithinRange(int srcnum, int rangenum, int enemynum,
        ...
        if (AAS_AreaVisible(enemyarea,
          ...
          if (((*aasworld).visCache[nextareanum] == 2) ||
              (!(*aasworld).visCache[nextareanum] &&
               ...
               botimport.AICast_CheckAttackAtPos(
...
// Changes Globals:		-
//===========================================================================
qboolean AAS_GetRouteFirstVisPos(vec3_t srcpos, vec3_t destpos, int travelflags,
                                 vec3_t retpos) {
