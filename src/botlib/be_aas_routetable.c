...
// Changes Globals:		-
//===========================================================================
void AAS_RT_AddParentLink(aas_area_childlocaldata_t *child, int parentindex,
                          int childindex) {
...
void AAS_RT_WriteRouteTable() {
  ...
  botimport.FS_Write((*aasworld).routetable->parentChildren,
                     (*aasworld).routetable->numParentChildren *
  ...
  botimport.FS_Write((*aasworld).routetable->visibleParents,
                     (*aasworld).routetable->numVisibleParents *
                         sizeof(unsigned short int),
                     fp);
  ...
  botimport.FS_Write((*aasworld).routetable->parentLinks,
                     (*aasworld).routetable->numParentLinks *
                         sizeof(aas_rt_parent_link_t),
                     fp);
...
int BotGetReachabilityToGoal(vec3_t origin, int areanum, int entnum,
                             ...
                             int *avoidreachtries, bot_goal_t *goal,
                             int travelflags, int movetravelflags);
...
qboolean AAS_RT_GetHidePos(vec3_t srcpos, int srcnum, int srcarea,
  ...
  hideareanum = AAS_NearestHideArea(srcnum, srcpos, srcarea, destnum, destpos,
                                    destarea, tfl);
  if (!hideareanum) {
...
#define MAX_HIDE_TRAVELTIME 1000 // 10 seconds is a very long way away
  ...
  */
  ...
  destParent =
      &rt->parents[rt->parentLinks[destChild->startParentLinks].parent];
  //
  ...
  bestTravelTime =
      MAX_HIDE_TRAVELTIME; // ignore any routes longer than 10 seconds away
  // set the destVec
  VectorSubtract(destpos, srcpos, destVec);
  destTravelDist = VectorNormalize(destVec);
  //
  // randomize the direction we traverse the list, so the hiding spot isn't
  // always the same
  if (rand() % 2) {
    dir = 1; // forward
  } else {
  ...
  startVisible =
      botimport.AICast_VisibleFromPos(destpos, destnum, srcpos, srcnum, qfalse);
  //
  // set the firstreach to prevent having to do an array and pointer lookup for
  // each destination
  firstreach = (*aasworld).areasettings[srcarea].firstreachablearea;
  //
  // just check random parent areas, traversing the route until we find an area
  // that can't be seen from the dest area
  for (visparents_count = 0, total_parents_checked = 0;
       visparents_count < MAX_CHECK_VISPARENTS &&
       total_parents_checked < rt->numParents;
       total_parents_checked++) {
    ...
    //
    visparents_count++;
    // they might be visible, check to see if the path to the area, takes us
    // towards the enemy we are trying to hide from
    {
...
#define GETHIDE_MAX_CHECK_PATHS 15
      ...
      for (j = 0; j < GETHIDE_MAX_CHECK_PATHS; j++) {
        ...
        elapsedTravelTime =
            AAS_AreaTravelTimeToGoalArea(pathArea, curPos, reach->areanum, tfl);
        // add the travel to the center of the area
        elapsedTravelTime +=
            AAS_AreaTravelTime(reach->areanum, reach->end,
                               (*aasworld).areas[reach->areanum].center);
        ...
        // if we are very close, don't get any closer under any circumstances
        {
          ...
          float dist;
          //
          VectorSubtract(destpos,
                         reachability[firstreach + route->reachable_index].end,
                         vec);
          ...
          //
          // check the directions to make sure we're not trying to run through
          // them
          if (j > 0) {
        ...
        //
        // if this area isn't in the visible list for the enemy's area, it's a
        // good hiding spot
        if (!(travChild = AAS_RT_GetChild(pathArea))) {
          invalidRoute = qtrue;
          break;
        }
        if (!destVisLookup[rt->parentLinks[travChild->startParentLinks]
                               .parent]) {
          // success ?
          if (!botimport.AICast_VisibleFromPos(
                  destpos, destnum, (*aasworld).areas[pathArea].center, srcnum,
                  qfalse)) {
            // SUCESS !!
            travParent =
                &rt->parents[rt->parentLinks[travChild->startParentLinks]
                                 .parent];
            break;
          }
        } else {
          ...
          // their view
          if (!startVisible) { // botimport.AICast_VisibleFromPos( destpos,
                               // destnum, reachability[firstreach +
    ...
    //		if (botimport.AICast_VisibleFromPos( destpos, destnum,
    //(*aasworld).areas[travParent->areanum].center, srcnum, qfalse )) {
    //			continue;
    //		}
    //
