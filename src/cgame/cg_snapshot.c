...
static void CG_ResetEntity(centity_t *cent) {
  ...
  // RF, disabled this, since we clear events out now in g_main.c, this is
  // redundant, and actually causes "double-door-sound" syndrome if a door is
  // triggered and heard, and then comes into view before it has timed out in
  // g_main.c, therefore playing the sound again, since it thinks it hasn't
  // processed this event yet.
...
void CG_SetInitialSnapshot(snapshot_t *snap) {
  ...
  trap_SendClientCommand(
      "fogswitch 0"); // clear it out so the set below will take
...
static snapshot_t *CG_ReadNextSnapshot(void) {
  ...
  while (cgs.processedSnapshotNum < cg.latestSnapshotNum) {
    ...
    if (r) {
      ...
      // RF, if we have no weapon selected, and this snapshots says we have a
      // weapon, then switch to that
