...
static void CG_UseItem(centity_t *cent) {
  ...
  if (es->number == cg.snap->ps.clientNum) {
    if (!itemNum) {
      CG_CenterPrint("noitem", SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25),
      ...
      if (item) {
        ...
        case HI_BOOK3:
          break;
        case HI_WINE:
          CG_CenterPrint("drankwine", SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25),
                         SMALLCHAR_WIDTH);
          break;
        default:
          CG_CenterPrint(va("Use %s", cgs.itemPrintNames[item - bg_itemlist]),
...
static void CG_ItemPickup(int itemNum) {
  ...
  if (bg_itemlist[itemNum].giType == IT_WEAPON) {
    ...
    if (cg_autoswitch.integer &&
      ...
      if (weapon != WP_SNIPERRIFLE && weapon != WP_SNOOPERSCOPE &&
          weapon != WP_FG42SCOPE) { //----(SA)	modified
        ...
        // 1 - "Always Switch" - always switch to new weap (Q3A default)
          ...
          // 2 - "If New" - switch to weap if it's not already in the player's
          // inventory (Wolf default) 4 - either 2 or 3 5 - both 2 and 3

          // FIXME:	this works fine for predicted pickups (when you walk over
          // the weapon), but not for
          //			manual pickups (activate item)
          if (cg_autoswitch.integer == 2 || cg_autoswitch.integer == 4 ||
              cg_autoswitch.integer == 5) {
            if (!COM_BitCheck(cg.snap->ps.weapons, weapon)) {
              selectIt = qtrue;
            }
          } // end 2/4/5

          // 3 - "If Better" - switch to weap if it's in a bank greater than the
          // current weap 4 - either 2 or 3 5 - both 2 and 3
              ...
              if (CG_WeaponIndex(weapon, &wpbank_pickup, NULL)) {
                if (wpbank_pickup > wpbank_cur) {
                  if (cg_autoswitch.integer == 5) { // 'new /and/ better'
                    if (!selectIt) {
                      selectIt =
                          qfalse; // if it isn't selected because it's new, then
                                  // this isn't "both" new /and/ better
                    }
                  } else {
                    selectIt = qtrue;
                  }
  ...
  if (bg_itemlist[itemNum].giType == IT_HOLDABLE) {
    cg.holdableSelectTime =
        cg.time;                // show holdables when a new one is picked up
...
#define STUNNED_ANIM BOTH_PAIN8
  ...
  if (cent->currentState.aiChar && cgs.gametype == GT_SINGLE_PLAYER) {
    ...
    if (VectorLength(cent->currentState.origin2) > 1) {
      ...
      // previous frame
      for (tagIndex = 0, bestDist = 0, bestTag = -1; tagAnims[tagIndex].tag;
           tagIndex++) {
        if (oldPainAnim >= 0 && tagAnims[tagIndex].anim == oldPainAnim) {
          continue;
        }
        // grab the tag with this name
        if (CG_GetOriginForTag(cent,
                               (refEntity_t *)(((byte *)&cent->pe) +
                                               tagAnims[tagIndex].refEntOfs),
                               tagAnims[tagIndex].tag, 0, tagOrg, NULL) >= 0) {
    ...
    // adjust the animation speed
    {
      ...
      cent->pe.animSpeed =
          (anim->frameLerp * anim->numFrames) / (float)cent->pe.painDuration;
    }
...
void CG_Explodef(vec3_t origin, vec3_t dir, int mass, int type, qhandle_t sound,
                 int forceLowGrav, qhandle_t shader, int parent,
                 qboolean damage);
...
void CG_Explodef(vec3_t origin, vec3_t dir, int mass, int type, qhandle_t sound,
  ...
  if (type == 5 && damage) {
    ...
    VectorSubtract(origin, org,
                   vec); // whoops, that way wouldn't work for cameras...
    vec[2] = 0;
    if (VectorLength(vec) > 800) {
      return;
    }
    mass =
        (int)(2.0 * mass * (1.0 - (0.6 + 0.4 * (VectorLength(vec) / 800.0))));
  }
  ...
  if (type == 0) { // cap wood even more since it's often grouped, and the small
                   // splinters can add up
    if (pieces[0] > 10) {
      pieces[0] = 10;
    }
    if (pieces[1] > 10) {
  ...
  for (i = 0; i < POSSIBLE_PIECES; i++) {
    ...
    for (howmany = 0; howmany < pieces[i]; howmany++) {
      ...
      case 0: // "wood"
        ...
        //					else
        //						scale =
        //cg_forceModel.value; 					else goto pass;
      ...
      case 5: // "rock"
        ...
        if (i == 5) {
          hmodel =
              cgs.media
                  .debRock[2]; // temporarily use the next smallest rock piece
        } else if (i == 4) {
          ...
          hmodel =
              cgs.media.debBlock[1]; // temporarily use the small block pieces
        } else {
          hmodel =
              cgs.media.debBlock[0]; // temporarily use the small block pieces
        }
        if (i <= 2) {
      ...
      // RF, debris rocks last longer in the boss map (is thee a better way of
      // doing this at this late stage?)
      ...
      default:
        ...
        le->bounceFactor =
            0.2f; // RF, rubble in end is like rubber, also ID requestsed this
        break;
      }
...
==============
CG_Effect
        Quake ed -> target_effect (0 .5 .8) (-6 -6 -6) (6 6 6) TNT explode smoke
debris gore lowgrav
==============
*/
void CG_Effect(centity_t *cent, vec3_t origin, vec3_t dir) {
  ...
  if (cent->currentState.eventParm & 1) { // fire
    ...
    // RF, sprVel is used without being set, so to be sure, I'm going to clear
    // out the vector
    VectorClear(sprVel);
    ...
    CG_ImpactMark(cgs.media.burnMarkShader, origin, dir, random() * 360, 1, 1,
                  1, 1, qfalse, 64, qfalse, -1);
  }
  ...
  if (cent->currentState.eventParm & 2) { // explode
    ...
    CG_ImpactMark(cgs.media.burnMarkShader, origin, dir, random() * 360, 1, 1,
                  1, 1, qfalse, 64, qfalse, 0xffffffff);
  }
  ...
  if (cent->currentState.eventParm & 8) { // rubble
    ...
    if (s && strlen(s) > 0) {
      sh = trap_R_RegisterShader(
          va("textures/%s", s)); // FIXME: don't do this here.  only for testing
    }
...
void CG_SpawnSpirit(centity_t *cent) {
  ...
  if (cg_entities[le->ownerNum].currentState.otherEntityNum2 ==
...
void CG_EntityEvent(centity_t *cent, vec3_t position) {
  ...
  case EV_FOOTSTEP:
    ...
    if (cg_footsteps.integer) {
      if (cent->currentState.aiChar == AICHAR_ELITEGUARD) {
        ...
        trap_S_StartSound(
            NULL, es->number, CHAN_BODY,
            cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_STEP][footstepcnt]);
        ...
        trap_S_StartSound(NULL, es->number, CHAN_BODY,
                          cgs.media.footsteps[FOOTSTEP_HEINRICH][footstepcnt]);
        CG_StartShakeCamera(0.08, 500, es->pos.trBase, 800);
      } else if (cent->currentState.aiChar == AICHAR_HELGA) {
        CG_SoundPlayIndexedScript(cgs.media.footsteps[FOOTSTEP_BEAST][0], NULL,
                                  es->number);
      } else {
        trap_S_StartSound(
            NULL, es->number, CHAN_BODY,
  ...
  case EV_FOOTSTEP_METAL:
    ...
    if (cg_footsteps.integer) {
      if (cent->currentState.aiChar == AICHAR_ELITEGUARD) {
        ...
        trap_S_StartSound(
            NULL, es->number, CHAN_BODY,
            cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_METAL][footstepcnt]);
        CG_StartShakeCamera(0.05, 400, es->pos.trBase, 512);
      } else if (cent->currentState.aiChar == AICHAR_SUPERSOLDIER ||
                 cent->currentState.aiChar == AICHAR_HEINRICH) {
        trap_S_StartSound(NULL, es->number, CHAN_BODY,
                          cgs.media.footsteps[FOOTSTEP_SUPERSOLDIER_METAL][0]);
        CG_StartShakeCamera(0.08, 500, es->pos.trBase, 800);
      } else if (cent->currentState.aiChar == AICHAR_HELGA) {
        CG_SoundPlayIndexedScript(cgs.media.footsteps[FOOTSTEP_BEAST][0], NULL,
                                  es->number);
  ...
  case EV_FOOTSTEP_WOOD:
    ...
    if (cg_footsteps.integer) {
      if (cent->currentState.aiChar == AICHAR_ELITEGUARD) {
        ...
        trap_S_StartSound(
            NULL, es->number, CHAN_BODY,
            cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_WOOD][footstepcnt]);
        CG_StartShakeCamera(0.05, 400, es->pos.trBase, 512);
      } else if (cent->currentState.aiChar == AICHAR_SUPERSOLDIER ||
                 cent->currentState.aiChar == AICHAR_HEINRICH) {
        trap_S_StartSound(
  ...
  case EV_FOOTSTEP_GRASS:
    ...
    if (cg_footsteps.integer) {
      ...
      if (cent->currentState.aiChar == AICHAR_PROTOSOLDIER) {
        trap_S_StartSound(
            NULL, es->number, CHAN_BODY,
            cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_GRASS][footstepcnt]);
      } else if (cent->currentState.aiChar == AICHAR_SUPERSOLDIER ||
                 cent->currentState.aiChar == AICHAR_HEINRICH) {
        trap_S_StartSound(
            NULL, es->number, CHAN_BODY,
  ...
  case EV_FOOTSTEP_GRAVEL:
    ...
    if (cg_footsteps.integer) {
      if (cent->currentState.aiChar == AICHAR_ELITEGUARD) {
        ...
        trap_S_StartSound(
            NULL, es->number, CHAN_BODY,
            cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_GRAVEL][footstepcnt]);
      } else if (cent->currentState.aiChar == AICHAR_SUPERSOLDIER ||
                 cent->currentState.aiChar == AICHAR_HEINRICH) {
        trap_S_StartSound(
            NULL, es->number, CHAN_BODY,
  ...
  case EV_FOOTSTEP_CARPET:
    ...
    if (cg_footsteps.integer) {
      trap_S_StartSound(NULL, es->number, CHAN_BODY,
                        cgs.media.footsteps[FOOTSTEP_CARPET][footstepcnt]);
    }
    break;
  ...
  case EV_FALL_DMG_10:
    ...
    // use normal pain sound trap_S_StartSound( NULL, es->number, CHAN_VOICE,
  ...
  case EV_FALL_DMG_25:
    ...
    trap_S_StartSound(NULL, es->number, CHAN_AUTO,
                      CG_CustomSound(es->number, "*fall2.wav"));
  ...
  case EV_WATER_UNDER:
    ...
    //----(SA)	the related issues of load/savegames, death underwater, etc. are
    //not handled at all.
    //----(SA)	the actual problem, of course, is doing underwater stuff when
    //the water is very turbulant and you can't simply
    //----(SA)	do things based on the players head being above/below the water
    //brushes top surface. (since the waves can potentially be /way/ above/below
    //that)

    // DHM - Nerve :: causes problems in multiplayer...
    if (cgs.gametype == GT_SINGLE_PLAYER &&
        clientNum == cg.predictedPlayerState.clientNum) {
      //			trap_R_SetFog(FOG_WATER, 0, 400, .1, .1, .1,
      //111); 			trap_R_SetFog(FOG_CMD_SWITCHFOG, FOG_WATER, 200, 0, 0, 0, 0);
      char buff[64];
  ...
  case EV_WATER_CLEAR:
    ...
    if (cgs.gametype == GT_SINGLE_PLAYER &&
        clientNum == cg.predictedPlayerState.clientNum) {
      //			trap_R_SetFog(FOG_CMD_SWITCHFOG, FOG_MAP,
      //400,0,0,0,0);
      char buff[64];
    ...
    DEBUGNAME("EV_ITEM_PICKUP");
    {
      ...
      if (event == EV_ITEM_PICKUP) { // not quiet
        ...
        if (item->giType == IT_POWERUP || item->giType == IT_TEAM) {
          trap_S_StartSound(NULL, es->number, CHAN_AUTO,
                            trap_S_RegisterSound("sound/items/n_health.wav"));
        } else {
          trap_S_StartSound(NULL, es->number, CHAN_AUTO,
  ...
  case EV_WEAP_OVERHEAT:
    ...
    if (es->number == cg.snap->ps.clientNum) {
      if ((cg.predictedPlayerState.weapAnim & ~ANIM_TOGGLEBIT) ==
          WEAP_ATTACK1) { // if attacking, go idle while overheating
  ...
  case EV_FILL_CLIP:
    ...
    if (cg_weapons[es->weapon].reloadSound) {
      ...
      // hope this does not cause trouble.  changed it to chan_weapon so i could
      // kill the sound if the guy dies while reloading can re-work if this
      // causes trouble
      //			trap_S_StartSound (NULL, es->number, CHAN_AUTO,
  ...
  case EV_GRENADE_SUICIDE:
    ...
    CG_MissileHitWall(
        WP_GRENADE_LAUNCHER, 0, position, dir,
  ...
  case EV_NOFIRE_UNDERWATER:
    ...
    if (es->number ==
        cg.snap->ps.clientNum) { // reset client-side weapon animation
      cg.predictedPlayerState.weapAnim =
          ((cg.predictedPlayerState.weapAnim & ANIM_TOGGLEBIT) ^
           ANIM_TOGGLEBIT) |
          WEAP_IDLE1;
  ...
  case EV_GRENADE_BOUNCE:
    ...
    if (es->weapon == WP_DYNAMITE) {
      ...
      if (flags & SURF_WOOD) { // SURF_WOOD
        if (rand() & 1) {
          ...
          trap_S_StartSound(NULL, es->number, CHAN_AUTO,
                 ...
                 (SURF_METAL | SURF_ROOF |
                  SURF_GLASS)) { //	SURF_METAL | SURF_ROOF | SURF_GLASS
        if (rand() & 1) {
          trap_S_StartSound(NULL, es->number, CHAN_AUTO,
      ...
      } else if (flags & (SURF_GRASS | SURF_GRAVEL | SURF_SNOW |
                          SURF_CARPET)) { // SURF_GRASS | SURF_GRAVEL |
                                          // SURF_SNOW | SURF_CARPET
        if (rand() & 1) {
          trap_S_StartSound(NULL, es->number, CHAN_AUTO,
  ...
  case EV_POPUP:
    ...
    trap_Cvar_Set("cg_clipboardName",
                  s); // store new current page name for the ui to pick up
    trap_UI_Popup(s);
    break;
  ...
  case EV_GIVEPAGE: {
    ...
    trap_Cvar_Set(
        "cg_notebookpages",
        va("%d",
           havepages)); // store new current page name for the ui to pick up
  ...
  case EV_DEATH3:
    ...
    //		trap_S_StartSoundEx(NULL, cent->currentState.number, CHAN_WEAPON, 0,
  ...
  case EV_ENTDEATH:
    ...
    switch (es->eventParm) {
      //			case ET_SPOTLIGHT:
      //				CG_Explodef(cent->lerpOrigin,
      //normalized_direction, 50, 1, cgs.media.sfx_bullet_glasshit[0], 1, 0,
      //cent->currentState.number, qfalse); 				break;

    case ET_ALARMBOX:
      // all this crap shouldn't be in here, but we don't have a generic
      // entry_point into explosions that's reasonable.  This will be an early
      // thing to get fixed in future work
      {
        int i, j;
        vec3_t sprVel, sprOrg;
        ...
        for (i = 0; i < 5; i++) {
          ...
          sprVel[2] += rand() % 10;
          CG_ParticleExplosion("blacksmokeanimb", sprOrg, sprVel,
                               3500 + rand() % 250, 4, 50 + rand() % 20);
        }

        CG_AddDebris(cent->lerpOrigin, dir,
                     80,              // speed
                     1000,            // duration
                     3 + rand() % 2); // count
      }
  ...
  case EV_POWERUP_BATTLESUIT:
    ...
    trap_S_StartSound(NULL, es->number, CHAN_ITEM,
                      trap_S_RegisterSound("sound/items/protect3.wav"));
    break;
  case EV_POWERUP_REGEN:
    ...
    trap_S_StartSound(NULL, es->number, CHAN_ITEM,
                      trap_S_RegisterSound("sound/items/regen.wav"));
    break;

  case EV_LOSE_HAT:
    DEBUGNAME("EV_LOSE_HAT");
    ByteToDir(es->eventParm, dir);
    // (SA) okay, some events not getting through, so I'm still testing.  Works
    // except for that tho.
  ...
  case EV_GIB_PLAYER:
    ...
    if (es->aiChar == AICHAR_ZOMBIE) {
      trap_S_StartSound(es->pos.trBase, es->number, CHAN_VOICE,
  ...
  case EV_SMOKE:
    ...
    if (cent->currentState.density == 3) { // cannon
      ...
      CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader,
                                         cent->currentState.origin, tv(0, 0, 1),
                                         8, 1000, 8, 20, 20, 0.25f);
  ...
  case EV_FLAMETHROWER_EFFECT: {
    ...
    //{ 				CG_FireFlameChunks( cent, cent->currentState.origin, cent->lerpAngles,
    //0.1, qfalse, 1 ); 				CG_FireFlameChunks( cent, cent->currentState.origin,
    //cent->currentState.apos.trBase, 0.1, qfalse, 1 );
    //			}
    //			else
    //				CG_FireFlameChunks( cent, cent->currentState.origin,
    //cent->lerpAngles, 0.6, 2, 1 );
    CG_FireFlameChunks(cent, cent->currentState.origin,
                       cent->currentState.apos.trBase, 0.6, 2, 1);
  ...
  case EV_EMITTER: {
    ...
    le->startTime = cg.time;
    le->endTime =
        le->startTime + cent->currentState.time; // 'time' stores lifetime
  ...
  case EV_OILPARTICLES:
    CG_Particle_OilParticle(cgs.media.oilParticle, cent->currentState.origin,
                            cent->currentState.origin2, cent->currentState.time,
  ...
  case EV_SPARKS: {
    ...
    for (i = 0; i < numsparks; i++) {
      ...
      if (event == EV_SPARKS_ELECTRIC) {
        ...
        CG_ParticleSparks(cent->currentState.origin, cent->currentState.angles,
                          duration, x, y, speed);
      }
    }
  ...
  case EV_GUNSPARKS: {
    ...
    CG_AddBulletParticles(cent->currentState.origin, cent->currentState.angles,
                          speed, 800, numsparks, 1.0f);
...
void CG_CheckEvents( centity_t *cent ) {
        ...
        // calculate the position at exactly the frame time
        BG_EvaluateTrajectory( &cent->currentState.pos, cg.snap->serverTime,
cent->lerpOrigin ); CG_SetEntitySoundPosition( cent );

        // check for event-only entities
        if ( cent->currentState.eType > ET_EVENTS ) {
                if ( !cent->previousEvent ) {
                        cent->previousEvent = 1;
                        cent->currentState.event = cent->currentState.eType -
ET_EVENTS; CG_EntityEvent( cent, cent->lerpOrigin );
                }
        } else {
                // check for events riding with another entity
                if ( cent->currentState.event != cent->previousEvent ) {
...
only add them once if (cent->currentState.eventSequence <
cent->previousEventSequence) { cent->previousEventSequence -= (1 << 8);	//
eventSequence is sent as an 8-bit through network stream
        }
        if (cent->currentState.eventSequence - cent->previousEventSequence >
MAX_EVENTS) { cent->previousEventSequence = cent->currentState.eventSequence -
MAX_EVENTS;
        }
        for ( i = cent->previousEventSequence ; i !=
cent->currentState.eventSequence; i++ ) { event = cent->currentState.events[ i &
(MAX_EVENTS-1) ];

                cent->currentState.event = event;
                cent->currentState.eventParm = cent->currentState.eventParms[ i
& (MAX_EVENTS-1) ]; CG_EntityEvent( cent, cent->lerpOrigin );
        }
        cent->previousEventSequence = cent->currentState.eventSequence;

        // set the event back so we don't think it's changed next frame (unless
it really has) cent->currentState.event = cent->previousEvent;
}
*/
