...
static void CG_DrawPlayerArmorValue(rectDef_t *rect, int font, float scale,
  ...
  if (shader) {
    ...
    CG_Text_Paint(rect->x + (rect->w - value) / 2, rect->y + rect->h, font,
...
1} }; { 1, 0.69f, 0, 1.0f } ,		// normal { 1.0f, 0.2f, 0.2f, 1.0f },
// low health {0.5f, 0.5f, 0.5f, 1},			// weapon firing { 1, 1,
1, 1 } };			// health > 100
*/
...
==============
*/
static void CG_DrawPlayerWeaponIcon(rectDef_t *rect, qboolean drawHighlighted,
                                    int align) {
  int size;
...
static void CG_DrawCursorhint(rectDef_t *rect) {
  ...
  case HINT_CHAIR:
    ...
    if (cg.predictedPlayerState.weapon &&
        !(WEAPS_ONE_HANDED &
          (1 << (cg.predictedPlayerState.weapon)))) { // (SA) this was backwards
  ...
  case HINT_NOEXIT_FAR:
    redbar =
        qtrue; // draw the status bar in red to show that you can't exit yet
  case HINT_EXIT_FAR:
    break;
  ...
  if (cg.cursorHintValue &&
      (!(cg.time - cg.cursorHintTime))) { // don't fade bar out w/ hint icon
    if (redbar) {
...
==============
*/
static void CG_DrawPlayerAmmoValue(rectDef_t *rect, int font, float scale,
                                   vec4_t color, qhandle_t shader,
                                   int textStyle, int type) {
  char num[16];
  int value, value2 = 0;
  centity_t *cent;
  playerState_t *ps;
  ...
  case WP_TESLA:
  case WP_FLAMETHROWER:
    if (type == 0) { // don't draw reserve value, just clip (since these weapons
                     // have all their ammo in the clip)
      return;
    }
    break;
  ...
  if (value > -1) {
    if (shader) {
      ...
      if (value2 || (special && type == 1)) {
        ...
        value = CG_Text_Width(num, font, scale, 0);
        CG_Text_Paint(-30 + rect->x + (rect->w - value) / 2, rect->y + rect->h,
...
static void CG_DrawSelectedPlayerHealth(rectDef_t *rect, int font, float scale,
                                        vec4_t color, qhandle_t shader,
                                        int textStyle) {
  clientInfo_t *ci;
  int value;
...
static void CG_DrawSelectedPlayerArmor(rectDef_t *rect, int font, float scale,
                                       vec4_t color, qhandle_t shader,
                                       int textStyle) {
  clientInfo_t *ci;
  int value;
...
static void CG_DrawSelectedPlayerName(rectDef_t *rect, int font, float scale,
  ...
  ci = cgs.clientinfo + sortedTeamPlayers[CG_GetSelectedPlayer()];
  if (ci) {
    CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color, ci->name, 0,
                  0, textStyle);
  }
}

static void CG_DrawSelectedPlayerLocation(rectDef_t *rect, int font,
  ...
  if (ci) {
    ...
    CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color, p, 0, 0,
                  textStyle);
  }
}

static void CG_DrawPlayerLocation(rectDef_t *rect, int font, float scale,
  ...
  if (ci) {
    if (cg_weapons[ci->curWeapon].weaponIcon[1]) {
      CG_DrawPic(
          ...
          cg_weapons[ci->curWeapon]
              .weaponIcon[1]); // (SA) using the 'selected' version of the icon
    } else {
...
static void CG_DrawPlayerScore(rectDef_t *rect, int font, float scale,
...
static void CG_DrawHoldableItem(rectDef_t *rect, int font, float scale,
                                qboolean draw2D) {
  int value;
  gitem_t *item;
  ...
  if (value) {
    ...
    if (cg.holdableSelect == HI_WINE) {
      ...
      //cg_items[ value ].icons[2-(value-1)] );
      CG_DrawPic(rect->x, rect->y, rect->w, rect->h,
                 cg_items[item - bg_itemlist].icons[2 - (value - 1)]);
    } else {
      //			CG_DrawPic( rect->x, rect->y, rect->w, rect->h,
      //cg_items[ value ].icons[0] );
      CG_DrawPic(rect->x, rect->y, rect->w, rect->h,
                 cg_items[item - bg_itemlist].icons[0]);
    }
  }
}
...
void flubfoo() {
  ...
  if (value) {
    ...
    if (cg.holdableSelect == HI_WINE) {
      ...
      //			CG_DrawPic( 100, (SCREEN_HEIGHT-ICON_SIZE)-8,
      //ICON_SIZE/2, ICON_SIZE, cg_items[item - bg_itemlist].icons[2-(value-1)]
      //);
      CG_DrawPic(606, 366, 24, 48,
                 cg_items[item - bg_itemlist].icons[2 - (value - 1)]);

    } else {
      //			CG_DrawPic( 100, (SCREEN_HEIGHT-ICON_SIZE)-8,
      //ICON_SIZE/2, ICON_SIZE, cg_items[item - bg_itemlist].icons[0] );
...
static void CG_DrawPlayerItem(rectDef_t *rect, int font, float scale,
                              qboolean draw2D) {
  int value;
...
static void CG_DrawPlayerHealth(rectDef_t *rect, int font, float scale,
                                vec4_t color, qhandle_t shader, int textStyle) {
  playerState_t *ps;
  int value;
  char num[16];
...
static void CG_DrawRedScore(rectDef_t *rect, int font, float scale,
                            vec4_t color, qhandle_t shader, int textStyle) {
  int value;
  char num[16];
  ...
  value = CG_Text_Width(num, font, scale, 0);
  CG_Text_Paint(rect->x + rect->w - value, rect->y + rect->h, font, scale,
                color, num, 0, 0, textStyle);
}

static void CG_DrawBlueScore(rectDef_t *rect, int font, float scale,
                             vec4_t color, qhandle_t shader, int textStyle) {
  int value;
  char num[16];
...
static void CG_DrawRedName(rectDef_t *rect, int font, float scale, vec4_t color,
                           int textStyle) {
#ifdef MISSIONPACK
  CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color,
                cg_redTeamName.string, 0, 0, textStyle);
#endif // #ifdef MISSIONPACK
}

static void CG_DrawBlueName(rectDef_t *rect, int font, float scale,
                            vec4_t color, int textStyle) {
#ifdef MISSIONPACK
  CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color,
...
static void CG_DrawBlueFlagName(rectDef_t *rect, int font, float scale,
                                vec4_t color, int textStyle) {
#ifdef MISSIONPACK
  int i;
...
static void CG_DrawRedFlagName(rectDef_t *rect, int font, float scale,
                               vec4_t color, int textStyle) {
#ifdef MISSIONPACK
  int i;
  for (i = 0; i < cgs.maxclients; i++) {
...
static void CG_HarvesterSkulls(rectDef_t *rect, int font, float scale,
                               vec4_t color, qboolean force2D, int textStyle) {
#ifdef MISSIONPACK
  char num[16];
  vec3_t origin, angles;
  qhandle_t handle;
...
static void CG_DrawAreaHoldable(rectDef_t *rect, int align, float spacing,
                                int font, float scale, vec4_t color) {}

static void CG_DrawAreaWeapons(rectDef_t *rect, int align, float spacing,
                               int font, float scale, vec4_t color) {}

static void CG_DrawAreaPowerUp(rectDef_t *rect, int align, float spacing,
                               int font, float scale, vec4_t color) {
  char num[16];
  ...
  for (i = 0; i < active; i++) {
    ...
    if (item) {
      ...
      CG_Text_Paint(r2.x + (r2.w * .75) + 3, r2.y + r2.h, font, scale, color,
                    num, 0, 0, 0);
      *inc += r2.w + spacing;
    }
  }
...
float CG_GetValue(int ownerDraw, int type) {
  ...
  case CG_PLAYER_AMMOCLIP_VALUE:
    if (cent->currentState.weapon) {
      if (type == RANGETYPE_RELATIVE) {
        return (float)ps
                   ->ammoclip[BG_FindClipForWeapon(cent->currentState.weapon)] /
               (float)ammoTable[cent->currentState.weapon].maxclip;
      } else {
...
qboolean CG_OwnerDrawVisible(int flags) {
  ...
  if (flags & CG_SHOW_NOT_V_CLEAR) {
    ...
    if (!cg.zoomedBinoc && !(cg.weaponSelect == WP_SNIPERRIFLE) &&
  ...
  if (flags & (CG_SHOW_NOT_V_BINOC | CG_SHOW_NOT_V_SNIPER |
               CG_SHOW_NOT_V_SNOOPER | CG_SHOW_NOT_V_FGSCOPE)) {
...
static void CG_DrawAreaSystemChat(rectDef_t *rect, int font, float scale,
                                  vec4_t color, qhandle_t shader) {
  CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color, systemChat, 0,
                0, 0);
}

static void CG_DrawAreaTeamChat(rectDef_t *rect, int font, float scale,
                                vec4_t color, qhandle_t shader) {
  CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color, teamChat1, 0, 0,
                0);
}

static void CG_DrawAreaChat(rectDef_t *rect, int font, float scale,
                            vec4_t color, qhandle_t shader) {
  CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color, teamChat2, 0, 0,
                0);
}
...
static void CG_DrawKiller(rectDef_t *rect, int font, float scale, vec4_t color,
  ...
  if (cg.killerName[0]) {
    ...
    CG_Text_Paint(x - CG_Text_Width(CG_GetKillerText(), font, scale, 0) / 2,
                  rect->y + rect->h, font, scale, color, CG_GetKillerText(), 0,
                  0, textStyle);
  }
}

static void CG_DrawCapFragLimit(rectDef_t *rect, int font, float scale,
                                vec4_t color, qhandle_t shader, int textStyle) {
  int limit = (cgs.gametype >= GT_CTF) ? cgs.capturelimit : cgs.fraglimit;
  CG_Text_Paint(rect->x, rect->y, font, scale, color, va("%2i", limit), 0, 0,
                textStyle);
}

static void CG_Draw1stPlace(rectDef_t *rect, int font, float scale,
                            vec4_t color, qhandle_t shader, int textStyle) {
  if (cgs.scores1 != SCORE_NOT_PRESENT) {
    CG_Text_Paint(rect->x, rect->y, font, scale, color, va("%2i", cgs.scores1),
                  0, 0, textStyle);
  }
}

static void CG_Draw2ndPlace(rectDef_t *rect, int font, float scale,
                            vec4_t color, qhandle_t shader, int textStyle) {
  if (cgs.scores2 != SCORE_NOT_PRESENT) {
    CG_Text_Paint(rect->x, rect->y, font, scale, color, va("%2i", cgs.scores2),
                  0, 0, textStyle);
  }
}
...
static void CG_DrawGameStatus(rectDef_t *rect, int font, float scale,
                              vec4_t color, qhandle_t shader, int textStyle) {
  CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color,
...
static void CG_DrawGameType(rectDef_t *rect, int font, float scale,
                            vec4_t color, qhandle_t shader, int textStyle) {
  CG_Text_Paint(rect->x, rect->y + rect->h, font, scale, color,
                CG_GameTypeString(), 0, 0, textStyle);
}

static void CG_Text_Paint_Limit(float *maxX, float x, float y, int font,
                                float scale, vec4_t color, const char *text,
...
void CG_DrawNewTeamInfo(rectDef_t *rect, float text_x, float text_y, int font,
...
// weapon used is not that useful, use the space for task
      ...
      CG_Text_Paint_Limit(&maxx, xx, y + text_y, UI_FONT_DEFAULT, scale, color,
                          ci->name, 0, 0);
...
void CG_DrawTeamSpectators(rectDef_t *rect, int font, float scale, vec4_t color,
                           qhandle_t shader) {
  if (cg.spectatorLen) {
    float maxX;
    ...
    if (cg.time > cg.spectatorTime) {
      ...
      if (cg.spectatorPaintX <= rect->x + 2) {
        if (cg.spectatorOffset < cg.spectatorLen) {
          cg.spectatorPaintX +=
              CG_Text_Width(&cg.spectatorList[cg.spectatorOffset], font, scale,
                            1) -
              1;
          cg.spectatorOffset++;
        } else {
...
void CG_DrawMedal(int ownerDraw, rectDef_t *rect, int font, float scale,
                  vec4_t color, qhandle_t shader) {
  ...
  if (text) {
    ...
    CG_Text_Paint(rect->x + (rect->w - value) / 2, rect->y + rect->h + 10, font,
...
void CG_DrawWeapStability(rectDef_t *rect, vec4_t color, int align) {
  ...
  if (cg_drawSpreadScale.integer == 1 &&
      !(cg.weaponSelect == WP_SNOOPERSCOPE ||
        cg.weaponSelect == WP_SNIPERRIFLE || cg.weaponSelect == WP_FG42SCOPE)) {
...
void CG_DrawWeapHeat(rectDef_t *rect, int align) {
  ...
  flags |= 1;  // BAR_LEFT			- this is hardcoded now, but will
               // be decided by the menu script
...
static void CG_DrawFatigue(rectDef_t *rect, vec4_t color, int align) {
  ...
  vec4_t colorBonus = {
      1, 1, 0, 0.45f}; // yellow (a little more solid for the 'bonus' stamina)
  ...
  CG_FilledBar(rect->x, rect->y, rect->w, rect->h, color, NULL, NULL,
               (float)cg.snap->ps.sprintTime / SPRINTTIME, flags);

  // fill in the left side of the bar with the counter for the nofatigue powerup
  if (cg.snap->ps.powerups[PW_NOFATIGUE]) {
    CG_FilledBar(rect->x, rect->y, rect->w / 2, rect->h, colorBonus, NULL, NULL,
                 cg.snap->ps.powerups[PW_NOFATIGUE] / BONUSTIME, flags);
  }
  ...
  if (cgs.gametype != GT_SINGLE_PLAYER) {
    ...
    CG_FilledBar(rect->x - rect->w, rect->y, rect->w / 2, rect->h, color, NULL,
                 NULL, barFrac, flags);
  }
  // jpw
}
...
void CG_OwnerDraw(float x, float y, float w, float h, float text_x,
                  ...
                  float special, int font, float scale, vec4_t color,
                  qhandle_t shader, int textStyle) {
  rectDef_t rect;
...
*/
void CG_ShowTeamMenu() { Menus_OpenByName("teamMenu"); }
