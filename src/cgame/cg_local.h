 ...
 * desc:	The entire cgame module is unloaded and reloaded on each level
 ...
 the client side.
 *			If you absolutely need something stored, it can either be
 kept
 *			by the server in the server stored userinfos, or stashed
 in a cvar.
...
typedef struct {
  ...
  animation_t
      *cgAnim; // pointer to the root of the animation array to use (*animation
               // above points at the current sequence)	//----(SA)	added
} lerpFrame_t;
...
typedef struct {
  ...
  int lastTime; // last time we were processed/ If the time goes backwards,
                // reset.
  ...
  // !!FIXME: these effects will be restarted by a *_restart command, can we
  // save this data somehow?
  qboolean cueZombieSpirit;  // if this is qfalse, and the zombie effect flag is
                             // set, then we need to start a new attack
  int zombieSpiritStartTime; // time the effect was started, so we can fade
                             // things in
  ...
  // !!FIXME: these effects will be restarted by a *_restart command, can we
  // save this data somehow?
  qboolean cueZombieDeath;  // if this is qfalse, and the zombie effect flag is
                            // set, then we need to start a new attack
  int zombieDeathStartTime; // time the effect was started, so we can fade
                            // things in
  ...
  vec3_t teslaOffsetDirs[MAX_TESLA_BOLTS]; // bending direction from center or
                                           // direct beam
...
typedef enum {
  ...
  ,
  LEF_NOTOUCHPARENT =
      0x0010 // (SA) when tracing to eval trajectory, ignore parent cent
  ,
...
typedef enum {
  ...
  ACC_MAX // this is bound by network limits, must change network stream to
          // increase this (SA) No, really?  that's not true is it?  isn't this
          // client-side only?
} accType_t;
...
typedef struct {
  ...
  qhandle_t accSkins[ACC_MAX]; // FIXME: put the #define for number of accessory
                               // models somewhere. (SA)

  //----(SA)	additional parts for specialized characters (the loper's
  //spinning trunk for example)
  qhandle_t partModels[9]; // [0-7] are optionally called in scripts, [8] is
...
typedef struct weaponInfo_s {
  ...
  pose_t position; // wolf locations (high, low, knife, pistol, shoulder, throw)
                   // defines are WPOS_HIGH, WPOS_LOW, WPOS_KNIFE, WPOS_PISTOL,
                   // WPOS_SHOULDER, WPOS_THROW
  ...
  sfxHandle_t
      spindownSound; //----(SA)	added // sound called if the above is running
...
typedef struct {
  ...
  int weaponSelect;
  int holdableSelect; // (SA) which holdable item is currently held
                      // ("selected").  When the client is ready to use it, send
                      // "use item <holdableSelect>"
  ...
  int exitStatsTime;
  int exitStatsFade;
  // just a copy of what's on the server, updated by configstring.  better way
  // to communicate/store this I'm sure
  int playTimeH;
  int playTimeM;
  int playTimeS;
  int attempts;
  ...
  int lastFiredWeapon;
  int lastWeapSelInBank[MAX_WEAP_BANKS]; // remember which weapon was last
                                         // selected in a bank for 'weaponbank'
                                         // commands //----(SA)	added
...
typedef struct {
  ...
  //	qhandle_t	medicReviveShader;	//----(SA)	commented out from
  //MP
  ...
  qhandle_t binocShaderSimpleQ; // same as above, but quartered.  (trying to
                                // save texture space)
  ...
  qhandle_t protoArmor[9 * 3];     // 9 parts, 3 sections each	(nodam, dam1,
                                   // dam2)
...
typedef struct {
  ...
  // screen fading
  //----(SA)	modified just in name so global searching is easier to narrow
  //down (added 'scrF')
...
extern vmCvar_t
    cg_notebookpages; // bitflags for the currently accessable pages.  if they
                      // wanna cheat, let 'em.  Most won't, or will wait 'til
...
void CG_FilledBar(float x, float y, float w, float h, const float *startColorIn,
...
void CG_Fade(int r, int g, int b, int a, int time,
             int duration); //----(SA)	modified
...
void CG_Bullet(vec3_t origin, int sourceEntityNum, vec3_t normal,
               qboolean flesh, int fleshEntityNum, qboolean wolfkick,
               int otherEntNum2);
...
void CG_ParticleDirtBulletDebris_Core(
    vec3_t org, vec3_t vel, int duration, float width, float height,
    float alpha, char *shadername); // NERVE - SMF // JPW addtnl params
...
void CG_ParticleImpactSmokePuffExtended(
    qhandle_t pshader, vec3_t origin, vec3_t dir, int radius, int lifetime,
    int vel, int acc, int maxroll,
    float alpha); // (SA) so I can add more parameters without screwing up the
                  // one that's there
...
int CG_AddTrailJunc(int headJuncIndex, qhandle_t shader, int spawnTime,
                    ...
                    vec3_t colorStart, vec3_t colorEnd, float sRatio,
                    float animSpeed);
int CG_AddSparkJunc(int headJuncIndex, qhandle_t shader, vec3_t pos,
                    int trailLife, float alphaStart, float alphaEnd,
                    float startWidth, float endWidth);
int CG_AddSmokeJunc(int headJuncIndex, qhandle_t shader, vec3_t pos,
                    int trailLife, float alpha, float startWidth,
                    float endWidth);
int CG_AddFireJunc(int headJuncIndex, qhandle_t shader, vec3_t pos,
                   int trailLife, float alpha, float startWidth,
                   float endWidth);
...
// cg_effects.c
//
int CG_GetOriginForTag(centity_t *cent, refEntity_t *parent, char *tagName,
                       int startIndex, vec3_t org, vec3_t axis[3]);
...
#define SL_LOCKTRACETORANGE                                                    \
  0x010 // only trace out as far as the specified range (rather than to max spot
        // range)
#define SL_NOFLARE                                                             \
...
void trap_S_AddLoopingSound(int entityNum, const vec3_t origin,
                            const vec3_t velocity, sfxHandle_t sfx, int volume);
void trap_S_AddRangedLoopingSound(int entityNum, const vec3_t origin,
...
void trap_S_Respatialize(int entityNum, const vec3_t origin, vec3_t axis[3],
                         int inwater);
sfxHandle_t
trap_S_RegisterSound(const char *sample); // returns buzz if not found
void trap_S_StartBackgroundTrack(const char *intro, const char *loop,
                                 int fadeupTime); // empty name stops music
void trap_S_StopBackgroundTrack(void);
void trap_S_FadeBackgroundTrack(float targetvol, int time,
                                int sound); //----(SA)	added
void trap_S_StartStreamingSound(const char *intro, const char *loop, int entnum,
...
void trap_RB_ZombieFXAddNewHit(int entityNum, const vec3_t hitPos,
                               const vec3_t hitDir);
// done.
...
void trap_SetUserCmdValue(int stateValue, int holdValue, float sensitivityScale,
                          int cld); // NERVE - SMF - added cld
...
qboolean trap_GetModelInfo(int clientNum, char *modelName,
                           animModelInfo_t **modelInfo);