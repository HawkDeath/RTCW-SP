...
void CG_BloodTrail(localEntity_t *le) {
  ...
  for (; t <= t2; t += step) {
    ...
    if (cent && cent->currentState.aiChar == AICHAR_ZOMBIE) {
      CG_Particle_Bleed(cgs.media.smokePuffShader, newOrigin, vec3_origin, 1,
                        500 + rand() % 200);
    } else {
      // Ridah, blood trail using trail code (should be faster since we don't
      // have to spawn as many)
      le->headJuncIndex =
          CG_AddTrailJunc(le->headJuncIndex, cgs.media.bloodTrailShader, t,
                          STYPE_STRETCH, newOrigin, 180,
                          1.0, // start alpha
                          0.0, // end alpha
  ...
  if (le->leMarkType == LEMT_BLOOD) {
    ...
    if (!(lastBloodMark > cg.time || lastBloodMark > cg.time - 100)) {
      ...
      CG_ImpactMark(cgs.media.bloodDotShaders[rand() % 5], trace->endpos,
                    ...
                    radius, qfalse, cg_bloodTime.integer * 1000);
...
void CG_FragmentBounceSound(localEntity_t *le, trace_t *trace) {
  if (le->leBounceSoundType == LEBS_BLOOD) {
    ...
    // half the hits will make thunk sounds	(this is just to start since we
    // don't even have the sound yet... (SA))
    if (rand() & 1) {
      int r = rand() & 3;
      sfxHandle_t s;
...
void CG_ReflectVelocity(localEntity_t *le, trace_t *trace) {
  ...
  if (le->leMarkType == LEMT_BLOOD && trace->startsolid) {
    ...
    //			if(le->leType == LE_DEBRIS && trace->entityNum < (MAX_ENTITIES -
    //1))
...
void CG_AddEmitter(localEntity_t *le) {
  ...
  switch (le->headJuncIndex) {
    //		CG_ParticleImpactSmokePuff (cgs.media.oilParticle,
    //le->pos.trBase); 		CG_Particle_Bleed(cgs.media.smokePuffShader,
    //le->pos.trBase, dir, 0, 200);

  case 1: // oil
    VectorScale(le->angles.trBase, le->radius, dir);
    CG_Particle_OilParticle(cgs.media.oilParticle, le->pos.trBase, dir, 10000,
                            le->ownerNum);
    break;
  case 2: // water
    VectorScale(le->angles.trBase, le->radius, dir);
  ...
  case 3: // steam
    ...
    //le->pos.trBase, 8, 1000, 8, 20, 20, 0.25f);
    CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader,
                                       le->pos.trBase, le->angles.trBase, 8,
                                       1000, 8, le->radius, 20, 0.25f);
    break;
  case 4: // wine
    VectorScale(le->angles.trBase, le->radius, dir);
    CG_Particle_OilParticle(cgs.media.oilParticle, le->pos.trBase, dir, 10000,
  ...
  case 5: // smoke
    nextTime = 100;
    CG_ParticleImpactSmokePuffExtended(cgs.media.smokeParticleShader,
                                       le->pos.trBase, dir, 8, 1000, 8, 20, 20,
                                       0.25f);
    break;
  case 6: // electrical
    nextTime = 100;
...
void CG_AddFragment(localEntity_t *le) {
  ...
  if (le->onFireStart &&
    ...
    trap_S_AddLoopingSound(-1, le->refEntity.origin, vec3_origin,
                           cgs.media.flameCrackSound, (int)(5.0 * flameAlpha));
  }
  ...
  if (hasFlame) {
    ...
    if (VectorLength(flameDir) == 0) {
      ...
      trap_S_AddLoopingSound(0, newOrigin, vec3_origin, cgs.media.flameSound,
                             (int)(0.3 * 255.0 * flameAlpha));
    } else {
      VectorNormalize(flameDir);
      // play a flame blow sound when moving
      trap_S_AddLoopingSound(0, newOrigin, vec3_origin,
  ...
  if ((le->leFlags & LEF_NOTOUCHPARENT) && le->ownerNum) {
    CG_Trace(&trace, le->refEntity.origin, NULL, NULL, newOrigin, le->ownerNum,
             contents);
  } else {
  ...
  if (le->leFlags & LEF_PLAYER_DAMAGE) {
    ...
    if (newOrigin[2] < le->refEntity.origin[2]) {
      ...
      dmax[2] +=
          le->refEntity.origin[2] - newOrigin[2]; // add falling distance to box
      ...
      if (i == 3) {
        trap_S_StartSound(cg.snap->ps.origin, cg.snap->ps.clientNum, CHAN_VOICE,
  ...
  if (le->breakCount) {
    ...
    for (i = 0; i <= le->breakCount; i++) {
      ...
      memcpy(&(nle->leType), &(le->leType),
             sizeof(localEntity_t) - 2 * sizeof(localEntity_t *));
      if (nle->breakCount-- < 2) {
      ...
      // randomize vel a bit
      VectorMA(nle->pos.trDelta, VectorLength(nle->pos.trDelta) * 0.3,
               bytedirs[rand() % NUMVERTEXNORMALS], nle->pos.trDelta);
    }
    // we're done
    CG_FreeLocalEntity(le);
    return;
  ...
  while (1) {
    ...
    */
    ...
    le->headJuncIndex = CG_AddSparkJunc(
        le->headJuncIndex, le->refEntity.customShader, le->refEntity.origin,
        200,
    ...
    //		if ( trap_CM_PointContents( le->refEntity.origin, 0 ) &
...
void CG_AddFuseSparkElements(localEntity_t *le) {
  ...
  while (time < cg.time) {
    ...
    le->headJuncIndex = CG_AddTrailJunc(
        le->headJuncIndex, cgs.media.sparkParticleShader, time, STYPE_STRETCH,
        le->refEntity.origin,
...
void CG_AddBloodElements(localEntity_t *le) {
  ...
  while (1) {
    ...
    // add a trail
    le->headJuncIndex =
        CG_AddSparkJunc(le->headJuncIndex, cgs.media.bloodTrailShader,
                        le->refEntity.origin, 200,
                        1.0 - lifeFrac, // start alpha
...
#define SPIRIT_FLASH_FADEOUT 2000
  ...
  if (cg.time < le->startTime + SPIRIT_FLASH_FADEIN) {
    alpha = (float)(cg.time - le->startTime) / (float)SPIRIT_FLASH_FADEIN;
             ...
             le->startTime + SPIRIT_FLASH_FADEIN + SPIRIT_FLASH_DURATION) {
    alpha = 1.0;
  } else if (cg.time < le->startTime + SPIRIT_FLASH_FADEIN +
                           SPIRIT_FLASH_DURATION + SPIRIT_FLASH_FADEOUT) {
    alpha = 1.0 - (float)(cg.time - (le->startTime + SPIRIT_FLASH_FADEIN +
                                     SPIRIT_FLASH_DURATION)) /
                      (float)SPIRIT_FLASH_FADEOUT;
  } else {
    return;
  }
...
void CG_AddClientCritter(localEntity_t *le) {
  ...
  while (time <= cg.time) {
    if (time > le->refEntity.fadeStartTime) {
      alpha = (float)(time - le->refEntity.fadeStartTime) /
              (float)(le->refEntity.fadeEndTime - le->refEntity.fadeStartTime);
      if (alpha < 0) {
        alpha = 0;
      } else if (alpha > 1) {
        alpha = 1;
    ...
    if (trace.startsolid || (trace.fraction < 1.0)) {
      ...
      if (!(le->leType == LE_HELGA_SPIRIT &&
            (le->refEntity.hModel == cgs.media.ssSpiritSkullModel))) {
        // kill it
        CG_FreeLocalEntity(le);
        return;
      } else {
    ...
    if (le->leType == LE_ZOMBIE_SPIRIT) {
      le->headJuncIndex =
          CG_AddTrailJunc(le->headJuncIndex, cgs.media.zombieSpiritTrailShader,
                          time, STYPE_STRETCH, le->refEntity.origin,
                          (int)le->effectWidth, // trail life
                          0.3 * alpha, 0.0, le->radius, 0,
                          0, // TJFL_FIXDISTORT,
    ...
    if (le->leType == LE_HELGA_SPIRIT &&
        le->refEntity.hModel != cgs.media.ssSpiritSkullModel) {
      le->headJuncIndex =
          CG_AddTrailJunc(le->headJuncIndex, cgs.media.helgaSpiritTrailShader,
                          time, STYPE_STRETCH, le->refEntity.origin,
                          (int)le->effectWidth, // trail life
                          0.3 * alpha, 0.0, le->radius, 0,
                          0, // TJFL_FIXDISTORT,
    ...
    if (alpha > 0.5 && (le->lastSpiritDmgTime < time - 100) && enemyDist < 24) {
      ...
      // if dead, ignore
      if (!(le->ownerNum != cg.snap->ps.clientNum
                ? cg_entities[le->ownerNum].currentState.eFlags & EF_DEAD
                : cg.snap->ps.pm_type == PM_DEAD)) {
        // inflict the damage!
        CG_ClientDamage(cg_entities[le->ownerNum].currentState.otherEntityNum2,
                        le->ownerNum, CLDMG_SPIRIT);
        ...
        if (le->leType == LE_HELGA_SPIRIT) {
          ...
          fb->endTime = fb->startTime + SPIRIT_FLASH_FADEIN +
                        SPIRIT_FLASH_DURATION + SPIRIT_FLASH_FADEOUT;
          // gasp!
          CG_SoundPlayIndexedScript(
              cgs.media.helgaGaspSound, NULL,
              cg_entities[le->ownerNum].currentState.otherEntityNum2);
        }
      }
    }
    ...
    trap_CM_BoxTrace(&trace, le->refEntity.origin, enemyPos, NULL, NULL, 0,
                     MASK_SOLID);
    if (trace.fraction < 1.0) {
      BG_EvaluateTrajectory(&le->pos, time + 1000, newOrigin);

      // if we would go passed the enemy, don't bother
      if (VectorDistance(le->refEntity.origin, enemyPos) >
          VectorDistance(le->refEntity.origin, newOrigin)) {

        trap_CM_BoxTrace(&trace, le->refEntity.origin, newOrigin, NULL, NULL, 0,
                         MASK_SOLID);
        ...
        if (trace.fraction < 1.0) {
          ...
          if (DotProduct(v, v2) > 0.7) {
            ...
            backup = *le;
            le->bounceFactor = (1.0 - trace.fraction) * 10.0 * (float)step /
                               1000.0; // tracking and avoidance factor
            // reflect the velocity on the trace plane
            VectorMA(le->pos.trDelta, le->bounceFactor * oldSpeed,
                     trace.plane.normal, le->pos.trDelta);
            if (VectorLength(le->pos.trDelta) < 1) {
              CG_FreeLocalEntity(le);
              return;
            }
    ...
    if (le->leType == LE_ZOMBIE_SPIRIT) {
      ...
      VectorMA(le->refEntity.origin, -10, le->refEntity.axis[2],
               le->refEntity.origin);
    }
...
#define BAT_ANIM_FRAMETIME 30
    ...
    le->refEntity.backlerp = 1.0 - ((float)(cg.time % BAT_ANIM_FRAMETIME) /
                                    (float)BAT_ANIM_FRAMETIME);
  }
  ...
  if (le->loopingSound) {
    if (cg.time > le->refEntity.fadeStartTime) {
      trap_S_AddLoopingSound(
          0, le->refEntity.origin, vec3_origin, le->loopingSound,
          255 - (int)(255.0 * (float)(cg.time - le->refEntity.fadeStartTime) /
                      (float)(le->refEntity.fadeEndTime -
                              le->refEntity.fadeStartTime)));
    } else {
      trap_S_AddLoopingSound(0, le->refEntity.origin, vec3_origin,
                             le->loopingSound,
                             255 - (int)(255.0 * (1.0 - alpha)));
    }
  }
  /*
          if (le->leType == LE_HELGA_SPIRIT) {
  ...
  for (t = le->lastTrailTime + step; t < cg.time; t += step) {
    ...
    if (le->effectFlags & 1) {
      le->headJuncIndex2 = CG_AddSmokeJunc(
          le->headJuncIndex2, cgs.media.smokeTrailShader, le->refEntity.origin,
  ...
  if (le->light) {
    ...
    */
    ...
    trap_R_AddLightToScene(re.origin, le->light, light * le->lightColor[0],
                           light * le->lightColor[1], light * le->lightColor[2],
                           0);
    // done.
  }
}
