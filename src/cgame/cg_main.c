...
vmCvar_t cg_notebookpages; // bitflags for the currently accessable pages.  if
                           // they wanna cheat, let 'em.  Most won't, or will
                           // wait 'til they actually play it.
...
cvarTable_t cvarTable[] = {
    ...
    {&cg_hintFadeTime, "cg_hintFadeTime", "500",
    ...
    {&cg_fov, "cg_fov", "90",
     CVAR_ARCHIVE | CVAR_CHEAT}, // JPW NERVE added cheat protect	NOTE: there
                                 // is already a dmflag (DF_FIXED_FOV) to allow
                                 // server control of this cheat
    ...
    {&cg_crosshairAlpha, "cg_crosshairAlpha", "0.5",
     CVAR_ARCHIVE}, //----(SA)	added
    ...
    {&cg_useSuggestedWeapons, "cg_useSuggestedWeapons", "1",
     CVAR_ARCHIVE}, //----(SA)	added
                    //----(SA)	end
    ...
    {&cg_synchronousClients, "g_syncronousClients", "0",
     0}, // communicated by systeminfo
    ...
    {&cg_gameSkill, "g_gameskill", "2",
     0}, // communicated by systeminfo	// (SA) new default '2' (was '1')

    {&cg_reloading, "g_reloading", "0", 0}, //----(SA)	added

    // JPW NERVE
    {&cg_medicChargeTime, "g_medicChargeTime", "10000",
     0}, // communicated by systeminfo
    {&cg_LTChargeTime, "g_LTChargeTime", "30000",
     0}, // communicated by systeminfo
    {&cg_engineerChargeTime, "g_engineerChargeTime", "30000",
     0}, // communicated by systeminfo
    {&cg_soldierChargeTime, "g_soldierChargeTime", "20000",
     0}, // communicated by systeminfo
    {&cg_bluelimbotime, "g_bluelimbotime", "30000",
     0}, // communicated by systeminfo
    {&cg_redlimbotime, "g_redlimbotime", "30000",
     0}, // communicated by systeminfo
    // jpw

    {&cg_notebook, "cl_notebook", "0", CVAR_ROM},
    {&cg_notebookpages, "cg_notebookpages", "0", CVAR_ROM},
    //	{ &cg_youGotMail, "cg_youGotMail", "0", CVAR_ROM},	// used to display
    //notebook new-info icon
    {&cg_youGotMail, "cg_youGotMail", "0",
     0}, // used to display notebook new-info icon
...
// this is the only thing that sets a cap on # items.  would like it to be
// adaptable. (rather than 256 max items with pickup name fixed at 32 chars)
...
static void CG_LoadPickupNames(void) {
  ...
  if (len <= 0) {
    CG_Printf(
        S_COLOR_RED
        "WARNING: pickup name file (pickupnames.txt not found in main/text)\n");
    return;
  }
  if (len > MAX_BUFFER) {
  ...
  for (i = 0; i < bg_numItems; i++) {
    ...
    if (!Q_stricmp(token, "---")) { // no name.  use hardcoded value
      if (bg_itemlist[i].pickup_name && strlen(bg_itemlist[i].pickup_name)) {
        Com_sprintf(cgs.itemPrintNames[i], MAX_QPATH,
                    bg_itemlist[i].pickup_name);
      } else {
...
static void CG_LoadTranslationStrings(void) {
  ...
  len = trap_FS_FOpenFile(filename, &f, FS_READ);
  if (len <= 0) {
    CG_Printf(S_COLOR_RED "WARNING: string translation file (strings.txt not "
                          "found in main/text)\n");
    return;
  }
  if (len > MAX_BUFFER) {
...
static void CG_RegisterSounds(void) {
  ...
  cgs.media.noFireUnderwater = trap_S_RegisterSound(
      "sound/weapons/underwaterfire.wav"); //----(SA)	added

  cgs.media.snipersound =
      trap_S_RegisterSound("sound/weapons/mauser/mauserf1.wav");
  cgs.media.tracerSound =
      trap_S_RegisterSound("sound/weapons/machinegun/buletby1.wav");
  ...
  cgs.media.useNothingSound =
      trap_S_RegisterSound("sound/items/use_nothing.wav");
  ...
  cgs.media.grenadebounce[GRENBOUNCE_DEFAULT][0] =
      trap_S_RegisterSound("sound/weapons/grenade/hgrenb1a.wav");
  cgs.media.grenadebounce[GRENBOUNCE_DEFAULT][1] =
      trap_S_RegisterSound("sound/weapons/grenade/hgrenb2a.wav");
  cgs.media.grenadebounce[GRENBOUNCE_DIRT][0] =
      trap_S_RegisterSound("sound/weapons/grenade/hg_dirt1a.wav");
  cgs.media.grenadebounce[GRENBOUNCE_DIRT][1] =
      trap_S_RegisterSound("sound/weapons/grenade/hg_dirt2a.wav");
  cgs.media.grenadebounce[GRENBOUNCE_WOOD][0] =
      trap_S_RegisterSound("sound/weapons/grenade/hg_wood1a.wav");
  cgs.media.grenadebounce[GRENBOUNCE_WOOD][1] =
      trap_S_RegisterSound("sound/weapons/grenade/hg_wood2a.wav");
  cgs.media.grenadebounce[GRENBOUNCE_METAL][0] =
      trap_S_RegisterSound("sound/weapons/grenade/hg_metal1a.wav");
  cgs.media.grenadebounce[GRENBOUNCE_METAL][1] =
      trap_S_RegisterSound("sound/weapons/grenade/hg_metal2a.wav");

  cgs.media.dynamitebounce1 =
      trap_S_RegisterSound("sound/weapons/dynamite/dynamite_bounce.wav");

  cgs.media.fbarrelexp1 =
      trap_S_RegisterSound("sound/weapons/flamebarrel/fbarrela.wav");
  cgs.media.fbarrelexp2 =
      trap_S_RegisterSound("sound/weapons/flamebarrel/fbarrelb.wav");

  cgs.media.fkickwall = trap_S_RegisterSound("sound/weapons/melee/fstatck.wav");
  cgs.media.fkickflesh =
  ...
  cgs.media.underWaterSound =
      trap_S_RegisterSound("sound/world/underwater03.wav");
  ...
  for (i = 1; i < MAX_SOUNDS; i++) {
    ...
    if (!strstr(soundName, ".wav")) {
      cgs.gameSounds[i] = CG_SoundScriptPrecache(
          soundName); //----(SA)	shouldn't this be okay?  The cs index is
                      //reserved anyway, so it can't hurt, right?
  ...
  cgs.media.grenadePulseSound4 =
      trap_S_RegisterSound("sound/weapons/grenade/grenpulse4.wav");
  cgs.media.grenadePulseSound3 =
      trap_S_RegisterSound("sound/weapons/grenade/grenpulse3.wav");
  cgs.media.grenadePulseSound2 =
      trap_S_RegisterSound("sound/weapons/grenade/grenpulse2.wav");
  cgs.media.grenadePulseSound1 =
      trap_S_RegisterSound("sound/weapons/grenade/grenpulse1.wav");
  //----(SA)	end
  ...
  cgs.media.flameSound =
      trap_S_RegisterSound("sound/weapons/flamethrower/fl_fire.wav");
  cgs.media.flameBlowSound =
      trap_S_RegisterSound("sound/weapons/flamethrower/fl_blow.wav");
  cgs.media.flameStartSound =
      trap_S_RegisterSound("sound/weapons/flamethrower/fl_start.wav");
  cgs.media.flameStreamSound =
      trap_S_RegisterSound("sound/weapons/flamethrower/fl_stream.wav");
  cgs.media.flameCrackSound =
      trap_S_RegisterSound("sound/world/firecrack1.wav");
  cgs.media.boneBounceSound = trap_S_RegisterSound(
      "sound/world/boardbreak.wav"); // TODO: need a real sound for this

  cgs.media.lightningSounds[0] =
      trap_S_RegisterSound("sound/world/electzap1.wav");
  cgs.media.lightningSounds[1] =
      trap_S_RegisterSound("sound/world/electzap2.wav");
  cgs.media.lightningSounds[2] =
      trap_S_RegisterSound("sound/world/electzap3.wav");
  ...
  cgs.media.bulletHitFleshMetalScript =
      CG_SoundScriptPrecache("bulletHitFleshMetal");

  cgs.media.teslaZapScript = CG_SoundScriptPrecache("teslaZap");
  cgs.media.teslaLoopSound =
      trap_S_RegisterSound("sound/weapons/tesla/loop.wav");

  cgs.media.batsFlyingLoopSound =
      trap_S_RegisterSound("sound/world/bats_flying.wav");
  ...
  cgs.media.wineSound = trap_S_RegisterSound(
      "sound/pickup/holdable/use_wine.wav"); //----(SA)	modified
  cgs.media.bookSound = trap_S_RegisterSound(
      "sound/pickup/holdable/use_book.wav"); //----(SA)	added
  cgs.media.staminaSound = trap_S_RegisterSound(
      "sound/pickup/holdable/use_stamina.wav"); //----(SA)	added
  cgs.media.quadSound = trap_S_RegisterSound("sound/items/damage3.wav");
  cgs.media.sfx_ric1 =
      trap_S_RegisterSound("sound/weapons/machinegun/ric1.wav");
  cgs.media.sfx_ric2 =
      trap_S_RegisterSound("sound/weapons/machinegun/ric2.wav");
  cgs.media.sfx_ric3 =
      trap_S_RegisterSound("sound/weapons/machinegun/ric3.wav");
  //	cgs.media.sfx_railg = trap_S_RegisterSound
  //("sound/weapons/railgun/railgf1a.wav");
  cgs.media.sfx_rockexp =
      trap_S_RegisterSound("sound/weapons/rocket/rocklx1a.wav");
  cgs.media.sfx_dynamiteexp =
      trap_S_RegisterSound("sound/weapons/dynamite/dynamite_exp.wav");
  cgs.media.sfx_dynamiteexpDist = trap_S_RegisterSound(
      "sound/weapons/dynamite/dynamite_exp_dist.wav"); //----(SA)	added

  cgs.media.sfx_spearhit =
      trap_S_RegisterSound("sound/weapons/speargun/spearhit.wav");

  cgs.media.sfx_knifehit[0] = trap_S_RegisterSound(
      "sound/weapons/knife/knife_hit1.wav"); // hitting player
  cgs.media.sfx_knifehit[1] =
      trap_S_RegisterSound("sound/weapons/knife/knife_hit2.wav");
  cgs.media.sfx_knifehit[2] =
      trap_S_RegisterSound("sound/weapons/knife/knife_hit3.wav");
  cgs.media.sfx_knifehit[3] =
      trap_S_RegisterSound("sound/weapons/knife/knife_hit4.wav");

  cgs.media.sfx_knifehit[4] = trap_S_RegisterSound(
      "sound/weapons/knife/knife_hitwall1.wav"); // hitting wall

  cgs.media.sfx_bullet_metalhit[0] =
      trap_S_RegisterSound("sound/weapons/bullethit_metal1.wav");
  cgs.media.sfx_bullet_metalhit[1] =
      trap_S_RegisterSound("sound/weapons/bullethit_metal2.wav");
  cgs.media.sfx_bullet_metalhit[2] =
      trap_S_RegisterSound("sound/weapons/bullethit_metal3.wav");

  cgs.media.sfx_bullet_woodhit[0] =
      trap_S_RegisterSound("sound/weapons/bullethit_wood1.wav");
  cgs.media.sfx_bullet_woodhit[1] =
      trap_S_RegisterSound("sound/weapons/bullethit_wood2.wav");
  cgs.media.sfx_bullet_woodhit[2] =
      trap_S_RegisterSound("sound/weapons/bullethit_wood3.wav");

  cgs.media.sfx_bullet_roofhit[0] =
      trap_S_RegisterSound("sound/weapons/bullethit_roof1.wav");
  cgs.media.sfx_bullet_roofhit[1] =
      trap_S_RegisterSound("sound/weapons/bullethit_roof2.wav");
  cgs.media.sfx_bullet_roofhit[2] =
      trap_S_RegisterSound("sound/weapons/bullethit_roof3.wav");

  cgs.media.sfx_bullet_ceramichit[0] =
      trap_S_RegisterSound("sound/weapons/bullethit_ceramic1.wav");
  cgs.media.sfx_bullet_ceramichit[1] =
      trap_S_RegisterSound("sound/weapons/bullethit_ceramic2.wav");
  cgs.media.sfx_bullet_ceramichit[2] =
      trap_S_RegisterSound("sound/weapons/bullethit_ceramic3.wav");

  cgs.media.sfx_bullet_glasshit[0] =
      trap_S_RegisterSound("sound/weapons/bullethit_glass1.wav");
  cgs.media.sfx_bullet_glasshit[1] =
      trap_S_RegisterSound("sound/weapons/bullethit_glass2.wav");
  cgs.media.sfx_bullet_glasshit[2] =
      trap_S_RegisterSound("sound/weapons/bullethit_glass3.wav");
...
static void CG_RegisterGraphics(void) {
  ...
  //	cgs.media.reticleShaderSimple = trap_R_RegisterShader(
  //"gfx/misc/reticlesimple" );		// TODO: remove
  cgs.media.reticleShaderSimpleQ =
      trap_R_RegisterShader("gfx/misc/reticlesimple_quarter");

  cgs.media.snooperShaderSimple =
      trap_R_RegisterShader("gfx/misc/snoopersimple");

  //	cgs.media.binocShaderSimple = trap_R_RegisterShader(
  //"gfx/misc/binocsimple" );			// TODO: remove
  cgs.media.binocShaderSimpleQ =
      trap_R_RegisterShader("gfx/misc/binocsimple_quarter"); //----(SA)	added
  ...
  cgs.media.hintShaders[HINT_ACTIVATE] =
      trap_R_RegisterShader("gfx/2d/usableHint");
  cgs.media.hintShaders[HINT_NOACTIVATE] =
      trap_R_RegisterShader("gfx/2d/notUsableHint");
  cgs.media.hintShaders[HINT_DOOR] = trap_R_RegisterShader("gfx/2d/doorHint");
  cgs.media.hintShaders[HINT_DOOR_ROTATING] =
      trap_R_RegisterShader("gfx/2d/doorRotateHint");
  cgs.media.hintShaders[HINT_DOOR_LOCKED] =
      trap_R_RegisterShader("gfx/2d/doorLockHint");
  cgs.media.hintShaders[HINT_DOOR_ROTATING_LOCKED] =
      trap_R_RegisterShader("gfx/2d/doorRotateLockHint");
  cgs.media.hintShaders[HINT_MG42] = trap_R_RegisterShader("gfx/2d/mg42Hint");
  cgs.media.hintShaders[HINT_BREAKABLE] =
      trap_R_RegisterShader("gfx/2d/breakableHint");
  ...
  cgs.media.hintShaders[HINT_HEALTH] =
      trap_R_RegisterShader("gfx/2d/healthHint");
  cgs.media.hintShaders[HINT_TREASURE] =
      trap_R_RegisterShader("gfx/2d/treasureHint");
  cgs.media.hintShaders[HINT_KNIFE] = trap_R_RegisterShader("gfx/2d/knifeHint");
  cgs.media.hintShaders[HINT_LADDER] =
      trap_R_RegisterShader("gfx/2d/ladderHint");
  cgs.media.hintShaders[HINT_BUTTON] =
      trap_R_RegisterShader("gfx/2d/buttonHint");
  cgs.media.hintShaders[HINT_WATER] = trap_R_RegisterShader("gfx/2d/waterHint");
  cgs.media.hintShaders[HINT_CAUTION] =
      trap_R_RegisterShader("gfx/2d/cautionHint");
  cgs.media.hintShaders[HINT_DANGER] =
      trap_R_RegisterShader("gfx/2d/dangerHint");
  cgs.media.hintShaders[HINT_SECRET] =
      trap_R_RegisterShader("gfx/2d/secretHint");
  cgs.media.hintShaders[HINT_QUESTION] =
      trap_R_RegisterShader("gfx/2d/questionHint");
  cgs.media.hintShaders[HINT_EXCLAMATION] =
      trap_R_RegisterShader("gfx/2d/exclamationHint");
  cgs.media.hintShaders[HINT_CLIPBOARD] =
      trap_R_RegisterShader("gfx/2d/clipboardHint");
  cgs.media.hintShaders[HINT_WEAPON] =
      trap_R_RegisterShader("gfx/2d/weaponHint");
  ...
  cgs.media.hintShaders[HINT_POWERUP] =
      trap_R_RegisterShader("gfx/2d/powerupHint");
  cgs.media.hintShaders[HINT_HOLDABLE] =
      trap_R_RegisterShader("gfx/2d/holdableHint");
  cgs.media.hintShaders[HINT_INVENTORY] =
      trap_R_RegisterShader("gfx/2d/inventoryHint");
  ...
  //	cgs.media.hintShaders[HINT_PLYR_FRIEND]			=
  //trap_R_RegisterShader( "gfx/2d/hintPlrFriend" );
  //	cgs.media.hintShaders[HINT_PLYR_NEUTRAL]		= trap_R_RegisterShader(
  //"gfx/2d/hintPlrNeutral" );
  //	cgs.media.hintShaders[HINT_PLYR_ENEMY]			=
  //trap_R_RegisterShader( "gfx/2d/hintPlrEnemy" );
  //	cgs.media.hintShaders[HINT_PLYR_UNKNOWN]		= trap_R_RegisterShader(
  //"gfx/2d/hintPlrUnknown" );

  //	cgs.media.hintShaders[HINT_BUILD]				=
  //trap_R_RegisterShader( "gfx/2d/buildHint" );		// DHM - Nerve

  cgs.media.youGotMailShader =
      trap_R_RegisterShader("gfx/2d/yougotmail"); //----(SA)	added
  cgs.media.youGotObjectiveShader =
      trap_R_RegisterShader("gfx/2d/yougotobjective"); //----(SA)	added

  //----(SA)	end

  for (i = 0; i < NUM_CROSSHAIRS; i++) {
    cgs.media.crosshairShader[i] =
        trap_R_RegisterShaderNoMip(va("gfx/2d/crosshair%c", 'a' + i));
  }

  cgs.media.crosshairFriendly =
      trap_R_RegisterShader("gfx/2d/friendlycross"); //----(SA)	added
  ...
  //trap_R_RegisterShader("powerups/battleWeapon" ); 	cgs.media.invisShader =
  //trap_R_RegisterShader("powerups/invisibility" ); 	cgs.media.regenShader =
  //trap_R_RegisterShader("powerups/regen" ); 	cgs.media.hastePuffShader =
  //trap_R_RegisterShader("hasteSmokePuff" );

  // DHM - Nerve :: Allow flags again, will change later to more appropriate
  // models
  if (cgs.gametype == GT_CTF || cgs.gametype == GT_WOLF ||
  ...
  //	cgs.media.debWoodl = trap_R_RegisterModel(
  //"models/mapobjects/debris/woodxl.md3" ); 	cgs.media.debWoodm =
  //trap_R_RegisterModel( "models/mapobjects/debris/woodm.md3" );
  //	cgs.media.debWoods = trap_R_RegisterModel(
  //"models/mapobjects/debris/woodsm.md3" );
  ...
  //	cgs.media.medicReviveShader = trap_R_RegisterShader(
  //"sprites/medic_revive" );	//----(SA)	commented out from MP
  cgs.media.balloonShader = trap_R_RegisterShader("sprites/balloon3");

  for (i = 0; i < MAX_AISTATES; i++) {
    cgs.media.aiStateShaders[i] =
        trap_R_RegisterShader(va("sprites/aistate%i", i + 1));
  }
  ...
  // cgs.media.spearModel = trap_R_RegisterModel("models/weaphits/spear.md3");
  // //----(SA)

  // cgs.media.bulletFlashModel =
  // trap_R_RegisterModel("models/weaphits/bullet.md3"); cgs.media.ringFlashModel
  ...
  //"models/misc/telep.md3" ); 	cgs.media.teleportEffectShader =
  //trap_R_RegisterShader( "teleportEffect" );

  //	cgs.media.batModel = trap_R_RegisterModel(
  //"models/mapobjects/bat/bat.md3" );

  //	cgs.media.medalImpressive = trap_R_RegisterShaderNoMip(
  //"medal_impressive" ); 	cgs.media.medalExcellent = trap_R_RegisterShaderNoMip(
  //"medal_excellent" ); 	cgs.media.medalGauntlet = trap_R_RegisterShaderNoMip(
  //"medal_gauntlet" );
  ...
  // cgs.media.lightningBoltShaderGreen = trap_R_RegisterShader(
  // "lightningBoltGreen" );	//----(SA)	alternate lightning color
  ...
  // cgs.media.flamethrowerFuelStream = trap_R_RegisterShader(
  // "flamethrowerFuelStream" ); cgs.media.flamethrowerFuelShader =
  // trap_R_RegisterShader( "flamethrowerFuel" );
  ...
  // models
  cgs.media.spotLightBaseModel =
      trap_R_RegisterModel("models/mapobjects/light/searchlight1_b.md3");
  cgs.media.spotLightLightModel =
      trap_R_RegisterModel("models/mapobjects/light/searchlight1_l.md3");
  cgs.media.spotLightLightModelBroke =
      trap_R_RegisterModel("models/mapobjects/light/searchlight_l_broke.md3");
  ...
  cgs.media.teslaDamageEffectShader =
      trap_R_RegisterShader("teslaDamageEffect");
  cgs.media.teslaAltDamageEffectShader =
      trap_R_RegisterShader("teslaAltDamageEffect");
  cgs.media.viewTeslaDamageEffectShader =
      trap_R_RegisterShader("viewTeslaDamageEffect");
  cgs.media.viewTeslaAltDamageEffectShader =
      trap_R_RegisterShader("viewTeslaAltDamageEffect");
  // done.

  cgs.media.railCoreShader =
      trap_R_RegisterShader("railCore"); // (SA) for debugging server traces

  cgs.media.thirdPersonBinocModel = trap_R_RegisterModel(
      "models/powerups/holdable/binocs_thirdperson.md3"); //----(SA)	added
  cgs.media.cigModel = trap_R_RegisterModel(
      "models/players/infantryss/acc/cig.md3"); //----(SA)	added

  // RF, not used anymore
  // cgs.media.targetEffectExplosionShader	= trap_R_RegisterShader(
  // "targetEffectExplode" );
  // cgs.media.rocketExplosionShader			= trap_R_RegisterShader(
  // "rocketExplosion" ); cgs.media.grenadeExplosionShader		=
  // trap_R_RegisterShader( "grenadeExplosion" );

  // zombie shot
  // cgs.media.zombieLoogie = trap_R_RegisterModel(
  // "models/mapobjects/bodyparts/zom_loog.md3" );
  ...
  // cgs.media.mg42muzzleflashgg = trap_R_RegisterModel
  // ("models/weapons2/machinegun/mg42_flash_gg.md3" );

  cgs.media.planemuzzleflash =
      trap_R_RegisterModel("models/mapobjects/vehicles/gunflare.md3");
  ...
  CG_LoadTranslateStrings(); //----(SA)	added.  for localization, read on-screen
                             //print names from text file
  ...
  ///////////
  // code is almost complete for doing this correctly.  will remove when that is
  // complete.
  ...
  cgs.media.bulletMarkShaderMetal =
      trap_R_RegisterShader("gfx/damage/metal_mrk");
  cgs.media.bulletMarkShaderWood = trap_R_RegisterShader("gfx/damage/wood_mrk");
  cgs.media.bulletMarkShaderCeramic =
      trap_R_RegisterShader("gfx/damage/ceramic_mrk");
  cgs.media.bulletMarkShaderGlass =
      trap_R_RegisterShader("gfx/damage/glass_mrk");
...
void CG_QueueMusic(void) {
  ...
  //				it doesn't have to go through startbackgroundtrack() (which
  //is stupid)
...
void CG_Text_PaintWithCursor(float x, float y, int font, float scale,
                             vec4_t color, const char *text, int cursorPos,
...
==============
CG_translateString
        presumably if this gets used more extensively, it'll be modified to a
hash table
==============
*/
  ...
  for (i = 0; i < numStrings; i++) {
    ...
    if (!strcmp(str, translateStrings[i].name)) {
      if (translateStrings[i].localname &&
          strlen(translateStrings[i].localname)) {
...
void CG_Init(int serverMessageNum, int serverCommandSequence) {
  ...
  // too late...
  //	trap_S_StartBackgroundTrack( "sound/music/fla_mp03.wav",
  //"sound/music/fla_mp03.wav", 1 );
