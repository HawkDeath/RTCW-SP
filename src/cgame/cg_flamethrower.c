...
typedef struct flameChunk_s {
  ...
  qboolean inuse;
  qboolean
      dead; // set when a chunk is effectively inactive, but waiting to be freed
...
typedef struct {
  //	float fireVolume;	// not needed, since we add individual loop sources
  //for the flame, so it gets spacialized
  float blowVolume;
  float streamVolume;
} flameSoundStatus_t;
...
void CG_FlameGetMuzzlePoint(vec3_t org, vec3_t fwd, vec3_t right, vec3_t up,
                            vec3_t outPos);
...
#define GET_FLAME_SIZE_SPEED(x)                                                \
  (((float)x / FLAME_LIFETIME) / 0.6) // x is the current sizeMax
...
===============
*/
void CG_FlameLerpVec(const vec3_t oldV, const vec3_t newV, float backLerp,
                     vec3_t outV) {
...
void CG_FlameAdjustSpeed(flameChunk_t *f, float change) {
  ...
  /*
          f->sizeMax = FLAME_START_MAX_SIZE + ((1.0 - (f->velSpeed /
     FLAME_START_SPEED)) * (FLAME_MAX_SIZE - FLAME_START_MAX_SIZE));
...
void CG_FireFlameChunks(centity_t *cent, vec3_t origin, vec3_t angles,
                        float speedScale, qboolean firing, int flags) {
...
#define MAX_CHUNKS_PER_CALL 30
  ...
  if (CG_PointContents(origin, cent->currentState.number) &
  ...
  if ((centInfo->lastClientFrame == cent->currentState.frame) &&
    ...
    timeInc = 1000.0 * (firing ? 1.0 : 0.5) *
              (FLAME_CHUNK_DIST /**(1.0+crandom()*0.1)*/ /
               (FLAME_START_SPEED * speedScale));
    ...
    while (t <= cg.time) {
      ...
      f->sizeStart = f->size =
          FLAME_START_SIZE * speedScale; // * (firing ? 1.0 : 0.2);
      f->sizeMax = (0.3 + 0.7 * speedScale) *
      ...
      */
      ...
      if (!firing) {
        ...
        //				f->blueLife =
        //(int)(0.3*(1.0/speedScale)*(float)FLAME_BLUE_LIFE);
      } else {
        f->gravity = 0;
    ...
    // if (firing) {
    //	trap_S_StartSound( org, cent->currentState.number, CHAN_AUTO,
    ...
    f->timeEnd =
        cg.time +
        FLAME_LIFETIME *
            (1.0 / (0.5 + 0.5 * speedScale)); // * (firing ? 1.0 : 0.2);
    ...
    while (f) {
      ...
      if (f->lastFriction < cg.time - 50) {
        ...
        if (dot >= 0.99) {
          dot -= 0.99;
          dot *= (1.0 / (1.0 - 0.99));
          CG_FlameAdjustSpeed(f, /*(0.3+0.7*f->speedScale) **/ 0.5 * frametime *
...
flameChunk_t *CG_SpawnFlameChunk(flameChunk_t *headFlameChunk) {
  ...
  f->nextFlameChunk = headFlameChunk; // if headJunc is NULL, then we'll just be
                                      // the end of the list
  ...
  if (cg_drawRewards.integer > 1 &&
      numFlameChunksInuse > cg_drawRewards.integer) {
...
void CG_MoveFlameChunk(flameChunk_t *f, int t) {
  ...
  if (f->velSpeed > 1 && f->lastFrictionTake < t - 50) {
    CG_FlameAdjustSpeed(f, /*(0.3+0.7*f->speedScale) **/ -(
                               (float)(t - f->lastFrictionTake) / 1000.0) *
  ...
  /*
          ...
          if (f->size < f->sizeMax) {
                  //f->sizeMax = FLAME_START_MAX_SIZE + ((1.0 - (f->velSpeed /
     FLAME_START_SPEED)) * (FLAME_MAX_SIZE - FLAME_START_MAX_SIZE)); if ((t -
     f->timeStart) < f->blueLife) f->sizeRate =
     GET_FLAME_BLUE_SIZE_SPEED(FLAME_START_MAX_SIZE);	// use a constant so the
     blue flame doesn't distort else f->sizeRate =
     GET_FLAME_SIZE_SPEED(f->sizeMax);
...
void CG_FlameDamage(int owner, vec3_t org, float radius) {
  ...
  if ((centFlameInfo[cg.snap->ps.clientNum].lastDmgCheck < cg.time - 100) &&
      (Distance(org, cg.snap->ps.origin) <
       radius * (cg.snap->ps.clientNum == owner ? 0.1 : 1.0))) {
    ...
    for (cent = cg_entities, i = 0; i < cgs.maxclients; cent++, i++) {
      if (cent->currentState.aiChar &&
          cent->currentState.aiChar !=
              AICHAR_VENOM && // Venom has no flame damage
...
void CG_AddFlameSpriteToScene(flameChunk_t *f, float lifeFrac, float alpha) {
  ...
  if (sdist < f->size * 0.6) {
    // clip the sprite to the viewport, avoiding rendering off-screen pixels
    // which is the main cause of slow-downs
    ...
    // find the distances from the sprite origin to the projection along the
    // refdef axis
    ...
    fovRadius[0] = tan(DEG2RAD((rollAngleClamped % 2 == 0 ? cg.refdef.fov_x
                                                          : cg.refdef.fov_x) *
                               0.52)) *
                   sdist;
    fovRadius[1] = tan(DEG2RAD((rollAngleClamped % 2 == 0 ? cg.refdef.fov_x
                                                          : cg.refdef.fov_x) *
                               0.52)) *
                   sdist;
...
#define FLAME_SOUND_RANGE 1024.0
  ...
  if ((cg_entities[fHead->ownerCent].currentState.eFlags &
       EF_FIRING) /*(centFlameInfo[fHead->ownerCent].lastClientFrame ==
                     cg_entities[fHead->ownerCent].lastWeaponClientFrame)*/
  ...
  // if (cg_entities[fHead->ownerCent].currentState.aiChar == AICHAR_ZOMBIE &&
  // !centFlameInfo[fHead->ownerCent].lastFiring) { 	return;
  //}
  ...
  while (f) {
    ...
    vdist = Distance(cg.refdef.vieworg,
    ...
    if (!f->ignitionOnly && f->velSpeed < 1) {
      ...
      /*
                      } else if (cg_entities[f->ownerCent].currentState.aiChar
         == AICHAR_ZOMBIE) {
                              ...
                              if (isClientFlame && !fHead->ignitionOnly) {
                                      ...
                                      if (f->lifeFrac < 0.3) {
                                              ...
                                              // if this is backwards from the
         last chunk, then skip it if (!f->ignitionOnly && fNext && f != fHead &&
         lastBlueChunk) { VectorSubtract( f->org, lastBlueChunk->org, v ); if
         (VectorNormalize( v ) < f->size/4 ) skip = qtrue; else if (DotProduct(
         v, f->velDir ) < 0.7) skip = qtrue;
                                              }

                                              if (!skip) {
                                                      lastBlueChunk = f;
                                                      alpha = 1.0;	// new
         nozzle sprite VectorScale( whiteColor, alpha, c ); shader =
         cgs.media.funnelFireShader[(cg.time/10)%NUM_FUNNEL_SPRITES];
                                                      blueTrailHead =
         CG_AddTrailJunc(	blueTrailHead, shader, cg.time, STYPE_STRETCH,
                                                                                                                              f->org,
                                                                                                                              1,
                                                                                                                              alpha, alpha,
      ...
      */
      ...
      // stream sound
      if (!f->ignitionOnly &&
          (cg_entities[f->ownerCent].currentState.number >= MAX_CLIENTS ||
           cg_entities[f->ownerCent].currentState.weapon == WP_FLAMETHROWER)) {
      ...
      if (!skip) {
        ...
        if (f->blueLife > lived * (f->ignitionOnly ? 3.0 : 3.0)) {
          ...
          // if (cg_entities[f->ownerCent].currentState.aiChar == AICHAR_ZOMBIE)
          //	shader = cgs.media.funnelFireShader[(cg.time/50 + (cg.time/50 >>
          //1))%NUM_FUNNEL_SPRITES]; else
          shader = nozzleShaders[(cg.time / 50 + (cg.time / 50 >> 1)) %
                                 NUM_NOZZLE_SPRITES];

          blueTrailHead = CG_AddTrailJunc(
              blueTrailHead, shader, cg.time, STYPE_STRETCH, f->org, 1, alpha,
              alpha,
              f->size *
        ...
        if (!f->ignitionOnly) { // && isClientFlame) {
          ...
          if (!fskip) {
            ...
            fuelTrailHead = CG_AddTrailJunc(
                fuelTrailHead, cgs.media.flamethrowerFireStream, cg.time,
...
#define FLAME_SPRITE_START_BLUE_SCALE 0.2
    ...
    if (!f->ignitionOnly &&
      ...
      while (fNext && !droppedTrail) {
        if ((Distance(f->org, fNext->org) <
             ((0.2 + 0.8 * f->lifeFrac) * f->size *
              (isClientFlame ? 0.2 : 0.1))) &&
      ...
      if (!skip && VectorLengthSquared(lastDrawPos)) {
        ...
        if ((vDist / f->size) * fDist *
                (0.1 + 0.9 * (1.0 - fabs(DotProduct(v1, v2)))) <
            (2.0 * (f->size / 30.0 < 1.0 ? f->size / 30.0 : 1.0))) {
          skip = qtrue;
        }
      }
...
#ifdef FLAMETHROW_LIGHTS
      // drop a light?
      if (/*f->size > FLAME_START_MAX_SIZE/1.5 &&*/ (
          !lastLightFlameChunk ||
          Distance(lastLightPos, f->org) > lastLightSize * 0.4)) {
        lightSize = f->size * 10;
        if (lightSize > 250) {
          lightSize = 250;
        }
        ...
        trap_R_AddLightToScene(f->org, lightSize, 1.0 * lightAlpha,
                               0.7 * lightAlpha, 0.3 * lightAlpha, 0);
  ...
  if (fHead->ignitionOnly) {
    ...
    // 0.603922*alpha, 0.207843*alpha, 2/*isClientFlame * (fHead->ownerCent ==
...
=============
*/
void CG_GenerateShaders(char *filename, char *shaderName, char *dir,
                        int numFrames, char *srcBlend, char *dstBlend,
                        char *extras, qboolean compressedVersionAvailable,
                        qboolean nomipmap) {
  fileHandle_t f;
...
void CG_InitFlameChunks(void) {
  ...
  CG_GenerateShaders("scripts/expblue.shader", "expblue", "expblue", 25,
                     "GL_ONE", "GL_ONE_MINUS_SRC_COLOR", "", qfalse, qfalse);
  ...
  while (f) {
    if (!f->dead) {
      if (cg.time > f->timeEnd) {
        ...
        if ((f->sizeTime - f->timeStart < f->blueLife) !=
            (cg.time - f->timeStart < f->blueLife)) {
        ...
        //
        f->lifeFrac = (float)(f->baseOrgTime - f->timeStart) /
...
#define MIN_BLOW_VOLUME 30
  ...
  while (f) {
    ...
    if (centFlameInfo[f->ownerCent].lastSoundUpdate != cg.time) {
      ...
      if (centFlameStatus[f->ownerCent].blowVolume * 255.0 > MIN_BLOW_VOLUME) {
        trap_S_AddLoopingSound(
            f->ownerCent, f->org, vec3_origin, cgs.media.flameBlowSound,
            (int)(255.0 * centFlameStatus[f->ownerCent].blowVolume));
      } else {
        trap_S_AddLoopingSound(f->ownerCent, f->org, vec3_origin,
                               cgs.media.flameBlowSound, MIN_BLOW_VOLUME);
      }

      if (centFlameStatus[f->ownerCent].streamVolume) {
        if (cg_entities[f->ownerCent].currentState.aiChar != AICHAR_ZOMBIE) {
          trap_S_AddLoopingSound(
              f->ownerCent, f->org, vec3_origin, cgs.media.flameStreamSound,
              (int)(255.0 /**centFlameStatus[f->ownerCent].streamVolume*/));
        } else {
          trap_S_AddLoopingSound(
              f->ownerCent, f->org, vec3_origin, cgs.media.flameCrackSound,
              (int)(255.0 * centFlameStatus[f->ownerCent].streamVolume));
        }
      }
    ...
    for (trav = f; trav; trav = trav->nextFlameChunk) {
      ...
      if (trav->blueLife + 100 < (cg.time - trav->timeStart)) {
        // if (!lastSoundFlameChunk || Distance( lastSoundFlameChunk->org,
        // trav->org ) > lastSoundFlameChunk->size) {
        trap_S_AddLoopingSound(
            trav->ownerCent, trav->org, vec3_origin, cgs.media.flameSound,
            (int)(255.0 * (0.2 * (trav->size / FLAME_MAX_SIZE))));
  ...
  for (cent = cg_entities, i = 0; i < cgs.maxclients; cent++, i++) {
    if (centFlameInfo[i].lastDmgCheck > centFlameInfo[i].lastDmgUpdate &&
        centFlameInfo[i].lastDmgUpdate <
            cg.time - 50) { // JPW NERVE (cgs.gametype == GT_SINGLE_PLAYER ? 50
                            // : 50)) -- sean changed clientdamage so this isn't
                            // a saturation issue any longer
      if ((cg.snap->ps.pm_flags & PMF_LIMBO) ||
          (cg.snap->ps.persistant[PERS_TEAM] == TEAM_SPECTATOR)) { // JPW NERVE
        return; // JPW NERVE don't do flame damage to guys in limbo or
                // spectator, they drop out of the game
      }
