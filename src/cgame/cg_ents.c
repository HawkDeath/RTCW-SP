...
void CG_PositionEntityOnTag(refEntity_t *entity, const refEntity_t *parent,
  ...
  // Ridah, not sure why this was here.. causes jittery torso animation, since
  // the torso might have different frame/oldFrame
...
void CG_PositionRotatedEntityOnTag(refEntity_t *entity,
                                   const refEntity_t *parent, char *tagName) {
  int i;
  orientation_t lerped;
  vec3_t tempAxis[3];
...
void CG_LoseArmor(centity_t *cent, int index) {
  ...
  char *heinrichTags[] = {"tag_chest",    "tag_calfleft",  "tag_armleft",
                          ...
                          "tag_legright", "tag_calfleft",  "tag_calfright"};
  ...
  tagIndex = CG_GetOriginForTag(cent, &cent->pe.torsoRefEnt, tags[index], 0,
                                origin, NULL);
...
#define FLY_JUMP 300
...

  {
    ...
    // (SA) FIXME: origin of md3 is offset from center.  need to center the
    ...
    // want to add a trail, but I need to find out what all these parameters are
    // first

    le->headJuncIndex = CG_AddTrailJunc(
        le->headJuncIndex, cgs.media.zombieSpiritTrailShader,
        //											time,
        le->lastTrailTime + 50, STYPE_STRETCH, le->refEntity.origin,
        //											(int)le->effectWidth,
        //// trail life
        1000, 0.3, 0.0,
        //											le->radius,
...
void CG_AttachedPartChange(centity_t *cent) {
  ...
  diff = (unsigned int)cent->nextState.dmgFlags ^
         (unsigned int)cent->currentState.dmgFlags;

  if (aiCharNum == AICHAR_PROTOSOLDIER || aiCharNum == AICHAR_SUPERSOLDIER ||
    ...
    for (i = 0; i < numParts;
         i++) { // FIXME: 7 is how many parts he currently has, this should
                // probably share a define with the server, or be stored in the
                // cent (cent->currentState.dmgParts)
      if (diff & (1 << i)) {
        CG_LoseArmor(cent, i);
      }
    }
...
void CG_AddLightstyle(centity_t *cent) {
  ...
  lightval = (lightval * (1000.0f / 24.0f)) -
             200.0f; // they want 'm' as the "middle" value as 300
  ...
  trap_R_AddLightToScene(cent->lerpOrigin, lightval, (float)r / 255.0f,
                         (float)g / 255.0f, (float)b / 255.0f,
                         0); // overdraw forced to 0 for now
}
...
static void CG_EntityEffects(centity_t *cent) {
  ...
  if (cent->currentState.loopSound) {
    //----(SA)	hmm, the above (CG_SetEntitySoundPosition()) sets
    //s_entityPosition[entityNum] with a valid 			location, but the looping sound
    //for a bmodel will never get it since that sound is 			started with the
    //lerpOriging right here. \/ \/	How do looping sounds ever work for
    //bmodels? 			Or have they always been broken and we just never used them?

    if (cent->currentState.eType == ET_SPEAKER) {
      /*if(cent->currentState.density == 1) {	// NO_PVS
              trap_S_AddRealLoopingSound( cent->currentState.number,
      cent->lerpOrigin, vec3_origin, cgs.gameSounds[
      cent->currentState.loopSound ] );
      }
      else*/
      if (cent->currentState.dmgFlags) { // range is set
        trap_S_AddRangedLoopingSound(
      ...
      if (cgs.gameSoundTypes[cent->currentState.loopSound] == 1) { // old style
        ...
        int soundIndex;
        soundIndex =
            soundScripts[cgs.gameSounds[cent->currentState.loopSound] - 1]
                .soundList->sfxHandle;
        trap_S_AddLoopingSound(cent->currentState.number, origin, vec3_origin,
                               soundIndex, 255);
      }
  ...
  if (cent->currentState.constantLight) {
    ...
    if (cent->dl_stylestring[0] != 0) { // it's probably a dlight
      ...
      trap_R_AddLightToScene(cent->lerpOrigin, i, (float)r / 255.0f,
                             (float)g / 255.0f, (float)b / 255.0f, 0);
    }
  }
  ...
  if (CG_EntOnFire(cent)) {
    ...
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           vec3_origin, cgs.media.flameBlowSound,
                           (int)(255.0 * (1.0 - fabs(cent->fireRiseDir[2]))));
    // play a burning sound when not moving
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           vec3_origin, cgs.media.flameSound,
...
static void CG_General(centity_t *cent) {
  ...
  if (cent->currentState.eType == ET_GAMEMODEL) {
    ...
    //cent->currentState.angles2[2]) != 1) { 			ent.reFlags |=
    //REFLAG_SCALEDSPHERECULL;
    //
...
CG_DrawHoldableSelect
  ...
  This, of course, will all change when we've got a hud, but for now it makes
the holdable items usable and not bad looking
==============
*/
void CG_DrawHoldableSelect(void) {
  ...
  for (i = 1; i <= HI_BOOK3; i++) {
    ...
    // draw icon
    if (i == HI_WINE) {
      // wine icons have three stages since each bottle has three uses (as
      // opposed to others so far where there's only 1 use)
      int wine = amount;
      if (wine > 3) {
        wine = 3;
      }
      CG_DrawPic(x, y, 32, 32,
                 cg_items[item - bg_itemlist].icons[2 - (wine - 1)]);
    } else {
  ...
  if (cg.holdableSelect) {
    ...
    if (item) {
      ...
      if (name) {
        ...
        //				w = CG_DrawStrlen( name ) *
        //BIGCHAR_WIDTH;
        w = CG_DrawStrlen(name) * 10;
        x = (SCREEN_WIDTH - w) / 2;
        //				CG_DrawBigStringColor(x, y - 22, name,
        //color);
        CG_DrawStringExt2(x, y + 74, name, color, qfalse, qtrue, 10, 10, 0);
        //				CG_Text_Paint(x, y + 74, 2, 0.3f, color, name,
        //0, 0, 6); // ITEM_TEXTSTYLE_SHADOWEDMORE
      }
    }
  }
...
qboolean CG_PlayerSeesItem(playerState_t *ps, entityState_t *item, int atTime,
                           int itemType) {
  ...
  //	eorigin[2] += 8;						// and subtract the item's
  //offset (that is used to place it on the ground)
...
static void CG_Item(centity_t *cent) {
      ...
      if (es->eFlags & EF_SPINNING) {
        if (es->groundEntityNum == -1 ||
            !es->groundEntityNum) { // (SA) spinning with a stand will spin the
                                    // stand and the attached weap (only when in
                                    // the air)
  ...
  if (es->modelindex2) { // modelindex2 was specified for the ent, meaning it
                         ...
                         // one in the itemlist) try to load it first, and if it
                         // fails, default to the itemlist model
    ent.hModel = cgs.gameModels[es->modelindex2];
  } else {
    if (item->giType == IT_WEAPON &&
        cg_items[es->modelindex]
            .models[2]) { // check if there's a specific model for weapon pickup
                          // placement
      ent.hModel = cg_items[es->modelindex].models[2];
    } else if (item->giType == IT_HEALTH || item->giType == IT_AMMO ||
               item->giType == IT_POWERUP) {
      if (es->density <
          (1 << 9)) { // (10 bits of data transmission for density)
        ent.hModel = cg_items[es->modelindex]
                         .models[es->density]; // multi-state powerups store
                                               // their state in 'density'
  ...
  //			Can't do it when item is registered since it wouldn't know about
  //replacement model
    ...
    if (CG_PlayerSeesItem(&cg.predictedPlayerState, es, cg.time,
      ...
      if (item->giType == IT_TREASURE) {
        trap_R_AddCoronaToScene(cent->highlightOrigin, 1, 0.85, 0.5, 2,
                                ...
                                1); //----(SA)	add corona to treasure
      }
    } else {
      if (item->giType == IT_TREASURE) {
        trap_R_AddCoronaToScene(cent->highlightOrigin, 1, 0.85, 0.5, 2,
                                cent->currentState.number,
                                0); //----(SA)	"empty corona" for proper fades
      }
    }
...
static void CG_Missile(centity_t *cent) {
  ...
  if (weapon->missileDlight) {
    trap_R_AddLightToScene(
        ...
        weapon->missileDlightColor[1], weapon->missileDlightColor[2], 0);
  }
  ...
  if (weapon->missileSound) {
    ...
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           velocity, weapon->missileSound, 255);
  }
...
static void CG_Bat(centity_t *cent) {
  ...
  // CG_Printf("remove me!!");
  // cgs.media.zombieSpiritLoopSound = trap_S_RegisterSound(
  // "sound/zombie/attack/spirit_loop.wav" ); cgs.media.ssSpiritSkullModel =
  ...
  // emit a sound
  trap_S_AddLoopingSound(0, refent.origin, vec3_origin,
                         cgs.media.zombieSpiritLoopSound, 255);
}
...
static animation_t footlockerAnims[] = {
        {"",	0,	1,	1,	1000/5,		1000/5 },	// (main
idle)
        {"",	0,	5,	5,	1000/5,		1000/5 },	// (lock
rattle)
        {"",	5,	6,	0,	1000/5,		1000/5
}	// (break open)
};
...
extern void CG_RunLerpFrame(clientInfo_t *ci, lerpFrame_t *lf, int newAnimation,
                            float speedScale);

/////
//////----(SA)	okay, a whole bunch of changes in here.  LMK if this breaks
///anything.  I will fix.
/////			I'm trying to test any potential breaks, but stuff could
///slip by.
/////
...
==============
*/
static int CG_NewAnim(centity_t *cent, lerpFrame_t *lf, animation_t *anim,
                      int initialSequence) {

  lf->frameTime = lf->oldFrameTime = cg.time;

  lf->cgAnim =
      anim; // set the pointer to the base of the animation array for this ent
  // TODO: handle out of range?
  lf->animation = &anim[initialSequence];

  CG_SetAnim(cent, lf, initialSequence);

  lf->frame = lf->oldFrame = lf->animation->firstFrame;
...
==============
*/
static void CG_RunAnim(centity_t *cent, int *frame, int *oldframe,
                       float *backlerp) {

  // transition to new anim if requested (rather than letting it get done in
  // cg_runlerpframe() since that expects a player ent)
  if ((cent->lerpFrame.animationNumber != cent->currentState.frame) ||
  ...
  if (!cent->lerpFrame.oldFrameTime) {
    ...
    if (cgs.gametype == GT_WOLF) {
      ...
      case 1:
        ...
        //				lf->animation = &multi_flagpoleAnims[
...
static void CG_Corona(centity_t *cent) {
  ...
  if (!behind && !toofar) {
    CG_Trace(&tr, cg.refdef.vieworg, NULL, NULL, cent->lerpOrigin, -1,
             MASK_SOLID |
    ...
    trap_R_AddCoronaToScene(cent->lerpOrigin, (float)r / 255.0f,
                            ...
                            cent->currentState.number, flags);
  }
}
...
==============
*/
extern void CG_Explodef(vec3_t origin, vec3_t dir, int mass, int type,
                        qhandle_t sound, int forceLowGrav, qhandle_t shader,
                        int parent, qboolean damage);
  ...
  if (cent->currentState.eType == ET_TESLA_EF) {
    ...
    for (i = 0; i < MAX_TESLA_BOLTS; i++) {
      if (cent->boltTimes[i] < cg.time) {
        ...
        VectorMA(cent->currentState.origin2, rnd, cent->boltLocs[i],
                 cent->boltLocs[i]);

        cent->boltTimes[i] =
            cg.time +
            rand() % cent->currentState
                         .time2; // hold this position for ~1 second
                                 // ('stickytime' value is stored in .time2)

        // cut the bolt short if it collides w/ something
        CG_Trace(&trace, cent->currentState.origin, NULL, NULL,
                 cent->boltLocs[i], -1, MASK_SOLID | CONTENTS_BODY);

        if (trace.fraction < 1) {
          // take damage
          if (trace.entityNum != ENTITYNUM_WORLD) {
            //						CG_ClientDamage(trace.entityNum,
            //cent->currentState.number, CLDMG_TESLA);
            //						cg_entities[trace.entityNum].pe.teslaDamagedTime =
            //cg.time;
          }
          ...
          RotatePointAroundVector(stickPoint, trace.plane.normal, perpvec,
                                  crandom() * 360);

          // scale it so it won't move too far with bolts that have long
          // boltTimer's
          movePerUpdate = 1.0f / (float)(cent->boltTimes[i] - cg.time);

          // move a max of 64 away from the 'original' target location
          VectorScale(stickPoint, movePerUpdate * trace.fraction * 64.0f,
                      cent->boltCrawlDirs[i]);
    ...
    for (i = 0; i < MAX_TESLA_BOLTS; i++) {
      ...
      if (cent->boltCrawlDirs[0] || cent->boltCrawlDirs[1] ||
          cent->boltCrawlDirs[2]) {
        VectorMA(cent->boltLocs[i], cent->boltTimes[i] - cg.time,
                 cent->boltCrawlDirs[i], perpvec);
      } else {
    ...
    if (cent->currentState.dl_intensity) {
      ...
      if ((cg.time / 50) % (randomness + (cg.time % randomness)) == 0) {
        ...
        g = (dli >> 8) & 255;
        b = (dli >> 16) & 255;
        trap_R_AddLightToScene(cent->currentState.origin,
                               cent->currentState.time, (float)r / 255.0f,
                               (float)g / 255.0f, (float)b / 255.0f, 0);
      }
    }
    ...
    if (cent->currentState.frame == 1) { // dead, don't move
      time =
          cent->currentState
              .time2; // 'time2' set when it died, so you have a good position
      ...
      if (!cg_paused.integer) { // don't add while paused
        if (!(rand() % 3)) {
          CG_ParticleImpactSmokePuffExtended(
              cgs.media.smokeParticleShader, cent->currentState.origin,
              tv(0, 0, 1), 8, 1000, 8, 20, 20, 0.25f);
        }
      }
    }
    ...
    if (cent->currentState.frame == 1) { // dead
      if ((cg.time - cent->currentState.time2) <
          100) { // it just died, throw some glass
        CG_Explodef(cent->lerpOrigin, normalized_direction, 50, 1,
                    cgs.media.sfx_bullet_glasshit[0], 1, 0,
    ...
    // messed up tag attachment rotation (forward for a tag is not forward in
    // the world)
    ...
    //);
    CG_FireFlameChunks(cent, cent->currentState.pos.trBase, forward, 1.0, qtrue,
                       1);
  }
}
...
static void CG_Explosive(centity_t *cent) {
  ...
  if (cent->currentState.modelindex2) { // there's a 'model2'
    ...
    //		if(cent->currentState.effect3Time) {			// if there's at least 1
    //frame specified, this is animated

    // first time initialize
    if (!cent->lerpFrame.oldFrameTime) {
      // note:	effect3Time and density will be invalidated when this explodes.
      //			they get re-used for explosion parameters. seems
      //like this 			could be re-arranged a bit better in the future. 			(this is why
      //the above check for currentstate.effect3Time is commented out)
...
static void CG_Mover(centity_t *cent) {
  ...
  if (cent->currentState.density == 7 || cent->currentState.density == 8) {
    ...
    if (cent->currentState.density == 8) {
      ...
      trap_R_AddLightToScene(flash.origin, 200 + (rand() & 31), 1.0, 0.6, 0.23,
                             0);
  ...
  // (SA) I'd like to do this as an entity flag (EF_SPARKING or something) and
  // have it randomly pick a vert on the model and have a few sparks at that
  // point, jumping away from the core (geometric center or lerporigin) of the
  // model.  Then we could use it generally for sparking
  // ents/characters/character gibs/etc (since we've got lots of electric based
  // characters)
...
void CG_FlamethrowerProp(centity_t *cent) {
  ...
  //cent->lerpAngles, 0.1, qfalse, flags ); 		CG_FireFlameChunks( cent,
  //cent->currentState.origin, cent->currentState.apos.trBase, 0.1, qfalse,
  //flags );
  //	}
  //	else
  //		CG_FireFlameChunks( cent, cent->currentState.origin,
  //cent->lerpAngles, 0.6, 2, flags );

  CG_FireFlameChunks(cent, cent->currentState.origin,
                     cent->currentState.apos.trBase, 0.6, 2, flags);
...
static void CG_AddEntityToTag(centity_t *cent) {
  ...
  sParent = NULL;
  centParent = NULL;
  for (i = CS_TAGCONNECTS + 1; i < CS_TAGCONNECTS + MAX_TAGCONNECTS;
       i++) { // NOTE: +1 since G_FindConfigStringIndex() starts at index 1
              // rather than 0 (not sure why)
  ...
  if (!sParent) {
    ...
    CG_Error("CG_EntityTagConnected: unable to find configstring to perform "
             "connection");
  }
  ...
  // if there was a higher ranking parent not added to the scene, then don't add
  // us
  if (centParent->processedFrame != cg.clientFrame) {
    return;
  }
  ...
  //	CG_PositionRotatedEntityOnTag( &ent, &centParent->refEnt,
  //centParent->refEnt.hModel, token );

  VectorCopy(ent.origin, cent->lerpOrigin);
  // we need to add the child's angles to the tag angles
  // if (!cent->currentState.density) {	// this entity should rotate with it's
  // parent, but can turn around using it's own angles
...
void CG_AddPacketEntities(void) {
  ...
  // RF, count the number of players in the scene, so we can force low LOD's for
  // dead bodies
  ...
  for (num = 0; num < cg.snap->numEntities; num++) {
    ...
    if (!(cent->currentState.eFlags & EF_TAGCONNECT)) {
      ...
      //			if (cent->currentState.number < MAX_CLIENTS) {
      //				if ((clcount > 2) &&
      //(cent->currentState.eFlags & EF_DEAD)) { 					cent->pe.forceLOD = qtrue;
      //				}
      //			}
      //----(SA)	end
      CG_AddCEntity(cent);
