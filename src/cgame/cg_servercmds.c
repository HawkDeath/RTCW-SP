...
void CG_ParseServerinfo(void) {
  ...
  trap_Cvar_Set("g_medicChargeTime",
                Info_ValueForKey(info, "g_medicChargeTime"));
  trap_Cvar_Set("g_engineerChargeTime",
                Info_ValueForKey(info, "g_engineerChargeTime"));
  trap_Cvar_Set("g_soldierChargeTime",
                Info_ValueForKey(info, "g_soldierChargeTime"));
  ...
  //sizeof(cgs.blueTeam) ); 	trap_Cvar_Set("g_blueTeam", cgs.blueTeam);
}
...
static void CG_ConfigStringModified(void) {
  ...
  //----(SA)	have not reached this code yet so I don't know if I really need
  //this here
  else if (num >= CS_DLIGHTS && num < CS_DLIGHTS + MAX_DLIGHTS) {
    CG_Printf(
        ">>>>>>>>>>>got configstring for dlight: %d\ntell Sherman!!!!!!!!!!",
        num - CS_DLIGHTS);
    //----(SA)
...
void CG_SendMoveSpeed(animation_t *animList, int numAnims, char *modelName) {
  ...
  for (i = 0, anim = animList; i < numAnims; i++, anim++) {
    ...
    Q_strcat(text, sizeof(text),
             va(" %s %i %.1f", anim->name, anim->moveSpeed, anim->stepGap));
  }
...
static void CG_MapRestart(void) {
  ...
  memset(&cg.lastWeapSelInBank[0], 0,
         MAX_WEAP_BANKS * sizeof(int)); // clear weapon bank selections
  ...
  // RF, clear out animScriptData so we recalc everything and get new pointers
  // from server
...
void CG_RequestMoveSpeed(const char *modelname) {
  ...
  // send it
  CG_SendMoveSpeed(modelInfo->animations, modelInfo->numAnimations,
                   (char *)modelname);
}
...
static void CG_ServerCommand(void) {
  ...
  if (!strcmp(cmd, "dp")) { // dynamite print (what a hack :(
    ...
    CG_CenterPrint(va("%s %d %s", CG_translateString("dynamitetimer"),
                      atoi(CG_Argv(1)), CG_translateString("seconds")),
                   SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25), SMALLCHAR_WIDTH);
    return;
  }

  if (!strcmp(cmd, "cp")) {
    CG_CenterPrint(CG_Argv(1), SCREEN_HEIGHT - (SCREEN_HEIGHT * 0.25),
                   SMALLCHAR_WIDTH);
    return;
  }
  ...
  if (!strcmp(cmd, "rockandroll")) {   // map loaded, game is ready to begin.
    ...
    trap_Cvar_Set("cg_norender",
                  "1"); // don't render the world until the player clicks in and
                        // the 'playerstart' func has been called (g_main in
