...
void CG_BuildSolidList(void) {
  ...
  for (i = 0; i < snap->numEntities; i++) {
    ...
    if (cent->nextState.solid == SOLID_BMODEL &&
        (cent->nextState.eFlags & EF_NONSOLID_BMODEL)) {
      continue;
    }
...
static void CG_TouchTriggerPrediction(void) {
  ...
  for (i = 0; i < cg_numTriggerEntities; i++) {
    ...
    if (ent->eType == ET_TELEPORT_TRIGGER) {
      ...
      // if we are already flying along the bounce direction, don't play sound
      // again
      ...
      if (s < 500) {
        ...
        BG_AddPredictableEventToPlayerstate(EV_JUMP_PAD, 0,
...
void CG_PredictPlayerState(void) {
  ...
  if (cg_nopredict.integer || cg_synchronousClients.integer ||
      (cg.snap->ps.eFlags &
       EF_MG42_ACTIVE)) { // RF, somewhat of a hack, but just disable prediction
                          // if on MG42, since it's just not very prediction
                          // friendly
  ...
  // this could be done as suggested in qshared.h ~line 991, but right now I
  ...
  // show_bug.cgi?id=416
  // FIXME TTimo this causing a double weapon reload sound if you hit reload at
  // the same time you run out of ammo
  // cg.predictedPlayerState.weaponstate				=
  // oldPlayerState.weaponstate;	// RF, added this, since they can become
  // unsynched on loadgame, leaving incorrect anims
