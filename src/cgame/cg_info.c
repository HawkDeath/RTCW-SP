...
void CG_LoadingItem(int itemNum) {
  ...
  //----(SA)	Max Kaufman request that we don't show any pacifier stuff for
  //items
  return;
  //----(SA)	end
...
void CG_LoadingClient(int clientNum) {
  ...
  if (cgs.gametype == GT_SINGLE_PLAYER &&
      clientNum > 0) { // for now only show the player's icon in SP games
    return;
  }
...
static statsItem_t statsItems[] = {{"end_time",
                                    168,
                                    214,
...
#define MAX_STATS_VARS 64
  ...
  for (i = 0, y = 0, v = 0; statsItems[i].label; i++) {
    ...
    //		UI_DrawProportionalString( statsItems[i].labelX, y,
    //statsItems[i].label, 			statsItems[i].labelFlags, *statsItems[i].labelColor
    //);
    ...
    if (statsItems[i].numVars) {
      ...
      case 2:
        formatStr =
            va(statsItems[i].format, vars[varIndex], vars[varIndex + 1]);
        break;
      case 3:
      ...
      case 4:
        formatStr = va(statsItems[i].format, vars[varIndex], vars[varIndex + 1],
                       vars[varIndex + 2], vars[varIndex + 3]);
        break;
      }

      //			UI_DrawProportionalString(
      //statsItems[i].formatX, y, formatStr, 				statsItems[i].formatFlags,
...
#define MAX_STATS_VARS 64
  ...
  color2[3] = color[3];
  //	CG_Text_Paint(280, 120, 2, 0.25f, color2, va("%s",
  //CG_translateString("end_title")), 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE);
  //----(SA)	scale change per MK
  CG_Text_Paint(270, 120, 2, 0.313f, color2,
                va("%s", CG_translateString("end_title")), 0, 0,
                ITEM_TEXTSTYLE_SHADOWEDMORE);

  color2[0] = color2[1] = color2[2] = 1;
  if (cg.cursorHintIcon == HINT_NOEXIT) {
    // "exit not available"
    //		CG_Text_Paint(250, 320, 2, 0.3f, color2, va("%s",
    //CG_translateString("end_noexit")), 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE);
    //----(SA)	scale change per MK
    CG_Text_Paint(260, 320, 2, 0.225f, color2,
                  va("%s", CG_translateString("end_noexit")), 0, 0,
                  ITEM_TEXTSTYLE_SHADOWEDMORE);
  } else {
    // "forward to proceed"
    //		CG_Text_Paint(230, 320, 2, 0.3f, color2, va("%s",
    //CG_translateString("end_exit")), 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE);
    //----(SA)	scale change per MK
    CG_Text_Paint(250, 320, 2, 0.225f, color2,
                  va("%s", CG_translateString("end_exit")), 0, 0,
                  ITEM_TEXTSTYLE_SHADOWEDMORE);
  }
  ...
  for (i = 0, y = 0, v = 0; statsItems[i].label; i++) {
    ...
    //		statsItems[i].labelColor[3] = statsItems[i].formatColor[3] = color[3];
    //// set proper alpha
    ...
    if (statsItems[i].numVars) {
      ...
      CG_Text_Paint(statsItems[i].formatX, y, 2, 0.3, statsItems[i].formatColor,
                    formatStr, 0, 0, statsItems[i].formatFlags);
      //			UI_DrawProportionalString(
      //statsItems[i].formatX, y, formatStr, statsItems[i].formatFlags,
      //*statsItems[i].formatColor );
    }
    ...
    if (i == 1) { // 'objectives'
      if (vars[varIndex] <
          vars[varIndex + 1]) { // missing objectives, draw in red
        color2[0] = 1;
        color2[1] = color2[2] = 0;
      }
    }
    ...
    if (i == 3) { // 'treasure'
      if (vars[varIndex] < vars[varIndex + 1] ||
          !vars[varIndex + 1]) { // missing treasure, only draw in white (gold
                                 // when you got em all)  (unless there's no
                                 // gold available, then 0/0 shows white)
    ...
    //		CG_Text_Paint(statsItems[i].labelX, y, 2, 0.3,
    //statsItems[i].labelColor, va("%s:",
    //CG_translateString(statsItems[i].label)), 0, 0, statsItems[i].labelFlags);
    CG_Text_Paint(statsItems[i].labelX, y, 2, 0.3, color2,
                  va("%s:", CG_translateString(statsItems[i].label)), 0, 0,
...
void CG_DrawInformation(void) {
  ...
  char buf[1024];
  static int lastDraw =
      0; // Ridah, so we don't draw the screen more often than we need to
  int ms;
  ...
  //	UI_DrawProportionalString( 320, 16, va( "Loading %s", Info_ValueForKey(
  //info, "mapname" ) ), UI_SMALLFONT|UI_CENTER|UI_DROPSHADOW, colorWhite );
  ...
  if (strlen(cg_missionStats.string) > 1 && cg_missionStats.string[0] == 's') {
    ...
    if (expectedHunk > 0) {
      percentDone = (float)(cg_hunkUsed.integer + cg_soundAdjust.integer) /
                    (float)(expectedHunk);
      if (percentDone >
          0.97) { // never actually show 100%, since we are not in the game yet
        percentDone = 0.97;
      }
      CG_HorizontalPercentBar(xy[0] + 10, xy[1] + wh[1] - 10, wh[0] - 20, 10,
                              percentDone);
    } else {
      UI_DrawProportionalString(320, xy[1] + wh[1] - 10, "please wait",
                                UI_CENTER | UI_EXSMALLFONT | UI_DROPSHADOW,
                                color);
    }
  ...
  // Ridah, in single player, cheats disabled, don't show unnecessary
  // information
  if (cgs.gametype == GT_SINGLE_PLAYER) {
