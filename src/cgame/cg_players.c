...
char *cg_customSoundNames[MAX_CUSTOM_SOUNDS] = {
    ...
    "*fall2.wav",    "*taunt.wav",    "*exert1.wav",   "*exert2.wav",
    "*exert3.wav",
};
...
void CG_CalcMoveSpeeds(clientInfo_t *ci) {
  ...
  for (i = 0, anim = ci->modelInfo->animations;
       i < ci->modelInfo->numAnimations; i++, anim++) {
    ...
    for (k = 0; k < 2; k++) {
      if (trap_R_LerpTag(&o[k], &refent, tags[k], 0) < 0) {
        CG_Error("CG_CalcMoveSpeeds: unable to find tag %s, cannot calculate "
                 "movespeed",
                 tags[k]);
      }
    }
    ...
    if (anim->moveSpeed < 0) { // use the auto calculations
      anim->moveSpeed =
          (int)((totalSpeed / numSpeed) * 1000.0 / anim->frameLerp);
    }
    // set the stepGap
    if (!numSteps) {
      numSteps = 2;
    ...
    anim->stepGap =
        (0.5 * ((float)anim->moveSpeed * (float)anim->duration / 1000.0));
    anim->stepGap /=
        (numSteps / 2); // in case there are more than 2 steps in animation
    if (isStrafe) {
  ...
  if (cgs.localServer) {
    CG_SendMoveSpeed(ci->modelInfo->animations, ci->modelInfo->numAnimations,
                     ci->modelInfo->modelname);
  }
}
...
#if 0 // RF, this entire function not used anymore, since we now grab all this
...
==============
*/
static qboolean CG_RegisterClientHeadSkin(clientInfo_t *ci,
                                          const char *modelName,
                                          const char *hSkinName) {
  char filename[MAX_QPATH];
...
==============
*/
static qboolean CG_RegisterAcc(clientInfo_t *ci, const char *modelName,
                               const char *skinName, int *model, int *skin) {
  ...
  if (!strstr(skinName,
              ".md3")) { // try to find a skin in the acc folder that matches
    *skin = trap_R_RegisterSkin(va("%s/%s.skin", modelName, skinName));

    if (*skin) {
      if (trap_R_GetSkinModel(*skin, "md3_part", &namefromskin[0])) {
        Com_sprintf(filename, sizeof(filename), "%s/acc/%s", modelName,
                    namefromskin);
        // NOTE: FIXME: this will currently only work with accessories in the
        // <modelName>/acc directory.
        //				It will have to strip the directory off the
        //end and then use the remaining 				path in order to work for arbitrary
        //sub-directories.
        //				Com_sprintf( filename, sizeof( filename ),
        //"%s/%s/%s.md3", modelName, <accpath>, namefromskin );

      } else {
        Com_sprintf(filename, sizeof(filename), "%s/%s.md3", modelName,
                    skinName);
      }
    } else {
...
qboolean CG_CheckForExistingModelInfo(clientInfo_t *ci, char *modelName,
  ...
  for (i = 0; i < MAX_ANIMSCRIPT_MODELS; i++) {
    ...
    if (trav && trav->modelname[0]) {
      ...
      if (trap_GetModelInfo(ci->clientNum, modelName,
                            &cgs.animScriptData.modelInfo[i])) {
      ...
      // huh!?
      CG_Error("CG_CheckForExistingModelInfo: unable to optain modelInfo from "
               "server");
    }
  }
...
static qboolean CG_RegisterClientModelname(clientInfo_t *ci,
  ...
  if (trap_R_GetSkinModel(ci->legsSkin, "md3_part", &namefromskin[0])) {
    Com_sprintf(filename, sizeof(filename), "models/players/%s/%s", modelName,
                namefromskin);
  ...
  //----(SA)	testing
  {
    ...
    if (badscale) {
      ci->playermodelScale[0] = ci->playermodelScale[1] =
  ...
  if (trap_R_GetSkinModel(ci->legsSkin, "md3_beltr", &namefromskin[0])) {
    CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                   &ci->accModels[ACC_BELT_LEFT], &ci->accSkins[ACC_BELT_LEFT]);
  }
  if (trap_R_GetSkinModel(ci->legsSkin, "md3_beltl", &namefromskin[0])) {
    CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                   &ci->accModels[ACC_BELT_RIGHT],
                   &ci->accSkins[ACC_BELT_RIGHT]);
  }
  if (trap_R_GetSkinModel(ci->torsoSkin, "md3_belt", &namefromskin[0])) {
    CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                   &ci->accModels[ACC_BELT], &ci->accSkins[ACC_BELT]);
  }
  if (trap_R_GetSkinModel(ci->torsoSkin, "md3_back", &namefromskin[0])) {
    CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                   &ci->accModels[ACC_BACK], &ci->accSkins[ACC_BACK]);
  }
  if (trap_R_GetSkinModel(ci->torsoSkin, "md3_weapon", &namefromskin[0])) {
    CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                   &ci->accModels[ACC_WEAPON], &ci->accSkins[ACC_WEAPON]);
  }
  if (trap_R_GetSkinModel(ci->torsoSkin, "md3_weapon2", &namefromskin[0])) {
    CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                   &ci->accModels[ACC_WEAPON2], &ci->accSkins[ACC_WEAPON2]);
  }
  //----(SA)	added
  // try anim script parts
  for (i = 0; i < 8; i++) {
    if (trap_R_GetSkinModel(ci->torsoSkin, va("md3_animscript%d", i),
                            &namefromskin[0])) {
      CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                     &ci->partModels[ACC_WEAPON], &ci->partSkins[ACC_WEAPON]);
    }
  }
  //----(SA)	end
  ...
  if (!CG_CheckForExistingModelInfo(ci, (char *)modelName, &ci->modelInfo)) {
    ...
    */
    ...
    if (!Q_strcasecmp((char *)modelName, "zombie")) {
      cgs.media.zombieSpiritWallShader =
          trap_R_RegisterShader("zombieDeathWindTrail");
      cgs.media.zombieSpiritTrailShader =
          trap_R_RegisterShader("zombieSpiritTrail");
      cgs.media.zombieSpiritSkullShader =
          trap_R_RegisterShader("zombieSpiritSkull");
      // cgs.media.zombieDeathDustShader = trap_R_RegisterShader(
      // "zombieDeathDust" ); cgs.media.zombieBodyFadeShader =
      // trap_R_RegisterShader( "zombieBodyFade" );
      // cgs.media.zombieHeadFadeShader = trap_R_RegisterShader(
      // "zombieHeadFade" );

      cgs.media.skeletonSkinShader = trap_R_RegisterShader("skeletonSkin");
      cgs.media.skeletonLegsModel =
          trap_R_RegisterModel("models/players/skel/lower.md3");
      cgs.media.skeletonLegsSkin =
          trap_R_RegisterSkin("models/players/skel/lower_default.skin");
      cgs.media.skeletonTorsoModel =
          trap_R_RegisterModel("models/players/skel/upper.md3");
      cgs.media.skeletonTorsoSkin =
          trap_R_RegisterSkin("models/players/skel/upper_default.skin");
      cgs.media.skeletonHeadModel =
          trap_R_RegisterModel("models/players/skel/head.md3");
      cgs.media.skeletonHeadSkin =
          trap_R_RegisterSkin("models/players/skel/head_default.skin");

      cgs.media.zombieSpiritSound =
          trap_S_RegisterSound("sound/zombie/attack/spirit_start.wav");
      cgs.media.zombieSpiritLoopSound =
          trap_S_RegisterSound("sound/zombie/attack/spirit_loop.wav");
      cgs.media.zombieDeathSound =
          trap_S_RegisterSound("sound/world/ceramicbreak.wav"); // Zombie Gib

      cgs.media.spiritSkullModel =
          trap_R_RegisterModel("models/mapobjects/skull/skul2t.md3");

      CG_RegisterWeapon(WP_GAUNTLET);
    } else if (!Q_strcasecmp((char *)modelName, "beast")) {
      cgs.media.helgaSpiritSkullShader =
          trap_R_RegisterShader("helgaSpiritGhost");
      cgs.media.helgaSpiritTrailShader =
          trap_R_RegisterShader("helgaSpiritTrail");
      cgs.media.helgaGhostModel =
          trap_R_RegisterModel("models/players/beast/ghost.md3");
      cgs.media.helgaSpiritLoopSound =
          trap_S_RegisterSound("sound/beast/tortured_souls_loop.wav");
      cgs.media.helgaSpiritSound =
          CG_SoundScriptPrecache("helgaSpiritStartSound");
      cgs.media.helgaGaspSound = CG_SoundScriptPrecache("helgaSpiritGasp");
    } else if (!Q_strcasecmp((char *)modelName, "loper")) {
      // cgs.media.loperGroundChargeShader = trap_R_RegisterShader(
      // "loperGroundCharge" );
    } else if (!Q_strcasecmp((char *)modelName, "protosoldier")) {
      cgs.media.protoArmorBreak =
          CG_SoundScriptPrecache("Protosoldier_loseArmor");

      cgs.media.protoArmor[0] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_chest.md3");
      cgs.media.protoArmor[1] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_lftcalf.md3");
      cgs.media.protoArmor[2] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_lftforarm.md3");
      cgs.media.protoArmor[3] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_lftshoulder.md3");
      cgs.media.protoArmor[4] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_lftthigh.md3");
      cgs.media.protoArmor[5] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_rtcalf.md3");
      cgs.media.protoArmor[6] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_rtforarm.md3");
      cgs.media.protoArmor[7] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_rtshoulder.md3");
      cgs.media.protoArmor[8] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/nodam_rtthigh.md3");

      cgs.media.protoArmor[9] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_chest1.md3");
      cgs.media.protoArmor[10] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_lftcalf1.md3");
      cgs.media.protoArmor[11] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_lftforarm1.md3");
      cgs.media.protoArmor[12] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_lftshoulder1.md3");
      cgs.media.protoArmor[13] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_lftthigh1.md3");
      cgs.media.protoArmor[14] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_rtcalf1.md3");
      cgs.media.protoArmor[15] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_rtforarm1.md3");
      cgs.media.protoArmor[16] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_rtshoulder1.md3");
      cgs.media.protoArmor[17] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_rtthigh1.md3");

      cgs.media.protoArmor[18] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_chest2.md3");
      cgs.media.protoArmor[19] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_lftcalf2.md3");
      cgs.media.protoArmor[20] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_lftforarm2.md3");
      cgs.media.protoArmor[21] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_lftshoulder2.md3");
      cgs.media.protoArmor[22] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_lftthigh2.md3");
      cgs.media.protoArmor[23] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_rtcalf2.md3");
      cgs.media.protoArmor[24] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_rtforarm2.md3");
      cgs.media.protoArmor[25] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_rtshoulder2.md3");
      cgs.media.protoArmor[26] = trap_R_RegisterModel(
          "models/players/protosoldier/armor/dam_rtthigh2.md3");
    } else if (!Q_strcasecmp((char *)modelName, "supersoldier")) {

      cgs.media.superArmorBreak =
          CG_SoundScriptPrecache("Supersoldier_loseArmor");

      cgs.media.superArmor[0] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_chest.md3");
      cgs.media.superArmor[1] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_lftcalf.md3");
      cgs.media.superArmor[2] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_lftforarm.md3");
      cgs.media.superArmor[3] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_lftshoulder.md3");
      cgs.media.superArmor[4] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_lftthigh.md3");
      cgs.media.superArmor[5] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_rtcalf.md3");
      cgs.media.superArmor[6] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_rtforarm.md3");
      cgs.media.superArmor[7] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_rtshoulder.md3");
      cgs.media.superArmor[8] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_rtthigh.md3");

      cgs.media.superArmor[9] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_lftfoot.md3");
      cgs.media.superArmor[10] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_rtfoot.md3");
      cgs.media.superArmor[11] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_lftuparm.md3");
      cgs.media.superArmor[12] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_rtuparm.md3");
      cgs.media.superArmor[13] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_waist.md3");
      cgs.media.superArmor[14] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_lftknee.md3");
      cgs.media.superArmor[15] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/nodam_rtknee.md3");

      cgs.media.superArmor[16] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_chest1.md3");
      cgs.media.superArmor[17] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftcalf1.md3");
      cgs.media.superArmor[18] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftforarm1.md3");
      cgs.media.superArmor[19] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftshoulder1.md3");
      cgs.media.superArmor[20] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftthigh1.md3");
      cgs.media.superArmor[21] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtcalf1.md3");
      cgs.media.superArmor[22] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtforarm1.md3");
      cgs.media.superArmor[23] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtshoulder1.md3");
      cgs.media.superArmor[24] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtthigh1.md3");

      cgs.media.superArmor[25] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftfoot1.md3");
      cgs.media.superArmor[26] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtfoot1.md3");
      cgs.media.superArmor[27] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftuparm1.md3");
      cgs.media.superArmor[28] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtuparm1.md3");
      cgs.media.superArmor[29] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_waist1.md3");

      cgs.media.superArmor[30] = 0;
      cgs.media.superArmor[31] = 0;

      cgs.media.superArmor[32] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_chest2.md3");
      cgs.media.superArmor[33] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftcalf2.md3");
      cgs.media.superArmor[34] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftforarm2.md3");
      cgs.media.superArmor[35] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftshoulder2.md3");
      cgs.media.superArmor[36] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftthigh2.md3");
      cgs.media.superArmor[37] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtcalf2.md3");
      cgs.media.superArmor[38] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtforarm2.md3");
      cgs.media.superArmor[39] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtshoulder2.md3");
      cgs.media.superArmor[30] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtthigh2.md3");

      cgs.media.superArmor[31] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftfoot2.md3");
      cgs.media.superArmor[32] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtfoot2.md3");
      cgs.media.superArmor[33] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_lftuparm2.md3");
      cgs.media.superArmor[44] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_rtuparm2.md3");
      cgs.media.superArmor[45] = trap_R_RegisterModel(
          "models/players/supersoldier/armor/dam_waist2.md3");
      ...
      */
      ...
      cgs.media.superArmor[0] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_chest.md3");
      cgs.media.superArmor[1] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_lftcalf.md3");
      cgs.media.superArmor[2] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_lftforarm.md3");
      cgs.media.superArmor[3] = trap_R_RegisterModel(
          "models/players/dark/armor/nodam_lftshoulder.md3");
      cgs.media.superArmor[4] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_lftthigh.md3");
      cgs.media.superArmor[5] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_rtcalf.md3");
      cgs.media.superArmor[6] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_rtforarm.md3");
      cgs.media.superArmor[7] = trap_R_RegisterModel(
          "models/players/dark/armor/nodam_rtshoulder.md3");
      cgs.media.superArmor[8] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_rtthigh.md3");

      cgs.media.superArmor[9] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_lftfoot.md3");
      cgs.media.superArmor[10] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_rtfoot.md3");
      cgs.media.superArmor[11] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_lftuparm.md3");
      cgs.media.superArmor[12] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_rtuparm.md3");
      cgs.media.superArmor[13] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_waist.md3");
      cgs.media.superArmor[14] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_lftknee.md3");
      cgs.media.superArmor[15] =
          trap_R_RegisterModel("models/players/dark/armor/nodam_rtknee.md3");

      cgs.media.superArmor[16] =
          trap_R_RegisterModel("models/players/dark/armor/dam_chest1.md3");
      cgs.media.superArmor[17] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftcalf1.md3");
      cgs.media.superArmor[18] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftforarm1.md3");
      cgs.media.superArmor[19] = trap_R_RegisterModel(
          "models/players/dark/armor/dam_lftshoulder1.md3");
      cgs.media.superArmor[20] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftthigh1.md3");
      cgs.media.superArmor[21] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtcalf1.md3");
      cgs.media.superArmor[22] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtforarm1.md3");
      cgs.media.superArmor[23] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtshoulder1.md3");
      cgs.media.superArmor[24] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtthigh1.md3");

      cgs.media.superArmor[25] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftfoot1.md3");
      cgs.media.superArmor[26] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtfoot1.md3");
      cgs.media.superArmor[27] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftuparm1.md3");
      cgs.media.superArmor[28] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtuparm1.md3");
      cgs.media.superArmor[29] =
          trap_R_RegisterModel("models/players/dark/armor/dam_waist1.md3");

      cgs.media.superArmor[30] = 0;
      cgs.media.superArmor[31] = 0;

      cgs.media.superArmor[32] =
          trap_R_RegisterModel("models/players/dark/armor/dam_chest2.md3");
      cgs.media.superArmor[33] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftcalf2.md3");
      cgs.media.superArmor[34] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftforarm2.md3");
      cgs.media.superArmor[35] = trap_R_RegisterModel(
          "models/players/dark/armor/dam_lftshoulder2.md3");
      cgs.media.superArmor[36] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftthigh2.md3");
      cgs.media.superArmor[37] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtcalf2.md3");
      cgs.media.superArmor[38] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtforarm2.md3");
      cgs.media.superArmor[39] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtshoulder2.md3");
      cgs.media.superArmor[30] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtthigh2.md3");

      cgs.media.superArmor[31] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftfoot2.md3");
      cgs.media.superArmor[32] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtfoot2.md3");
      cgs.media.superArmor[33] =
          trap_R_RegisterModel("models/players/dark/armor/dam_lftuparm2.md3");
      cgs.media.superArmor[44] =
          trap_R_RegisterModel("models/players/dark/armor/dam_rtuparm2.md3");
      cgs.media.superArmor[45] =
          trap_R_RegisterModel("models/players/dark/armor/dam_waist2.md3");
      ...
      cgs.media.heinrichArmorBreak =
          CG_SoundScriptPrecache("Heinrich_loseArmor");

      // RF, these are also used but supersoldier "spirits" in end map
      cgs.media.zombieSpiritLoopSound =
          trap_S_RegisterSound("sound/zombie/attack/spirit_loop.wav");
      cgs.media.ssSpiritSkullModel =
          trap_R_RegisterModel("models/players/supersoldier/ssghost.md3");
      ...
      cgs.media.debrisHitSound =
          trap_S_RegisterSound("sound/world/debris_hit.wav");

      cgs.media.heinrichArmor[0] =
          trap_R_RegisterModel("models/players/heinrich/armor/nodam_chest.md3");
      cgs.media.heinrichArmor[1] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftcalf.md3");
      cgs.media.heinrichArmor[2] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftforarm.md3");
      cgs.media.heinrichArmor[3] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftshoulder.md3");
      cgs.media.heinrichArmor[4] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftthigh.md3");
      cgs.media.heinrichArmor[5] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtcalf.md3");
      cgs.media.heinrichArmor[6] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtforarm.md3");
      cgs.media.heinrichArmor[7] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtshoulder.md3");
      cgs.media.heinrichArmor[8] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtthigh.md3");

      cgs.media.heinrichArmor[9] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftfoot.md3");
      cgs.media.heinrichArmor[10] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtfoot.md3");
      cgs.media.heinrichArmor[11] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftuparm.md3");
      cgs.media.heinrichArmor[12] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtuparm.md3");
      cgs.media.heinrichArmor[13] =
          trap_R_RegisterModel("models/players/heinrich/armor/nodam_waist.md3");
      cgs.media.heinrichArmor[14] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftknee.md3");
      cgs.media.heinrichArmor[15] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtknee.md3");

      cgs.media.heinrichArmor[16] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftelbow.md3");
      cgs.media.heinrichArmor[17] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtelbow.md3");
      cgs.media.heinrichArmor[18] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lfthip.md3");
      cgs.media.heinrichArmor[19] =
          trap_R_RegisterModel("models/players/heinrich/armor/nodam_rthip.md3");
      cgs.media.heinrichArmor[20] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_lftshin.md3");
      cgs.media.heinrichArmor[21] = trap_R_RegisterModel(
          "models/players/heinrich/armor/nodam_rtshin.md3");

      cgs.media.heinrichArmor[22] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_chest1.md3");
      cgs.media.heinrichArmor[23] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftcalf1.md3");
      cgs.media.heinrichArmor[24] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftforarm1.md3");
      cgs.media.heinrichArmor[25] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftshoulder1.md3");
      cgs.media.heinrichArmor[26] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftthigh1.md3");
      cgs.media.heinrichArmor[27] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_rtcalf1.md3");
      cgs.media.heinrichArmor[28] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtforarm1.md3");
      cgs.media.heinrichArmor[29] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtshoulder1.md3");
      cgs.media.heinrichArmor[30] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtthigh1.md3");

      cgs.media.heinrichArmor[31] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftfoot1.md3");
      cgs.media.heinrichArmor[32] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_rtfoot1.md3");
      cgs.media.heinrichArmor[33] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftuparm1.md3");
      cgs.media.heinrichArmor[34] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtuparm1.md3");
      cgs.media.heinrichArmor[35] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_waist1.md3");
      cgs.media.heinrichArmor[36] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftknee1.md3");
      cgs.media.heinrichArmor[37] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_rtknee1.md3");

      cgs.media.heinrichArmor[38] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftelbow1.md3");
      cgs.media.heinrichArmor[39] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtelbow1.md3");
      cgs.media.heinrichArmor[40] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_lfthip1.md3");
      cgs.media.heinrichArmor[41] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_rthip1.md3");
      cgs.media.heinrichArmor[42] = 0;
      cgs.media.heinrichArmor[43] = 0;

      cgs.media.heinrichArmor[44] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_chest2.md3");
      cgs.media.heinrichArmor[45] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftcalf2.md3");
      cgs.media.heinrichArmor[46] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftforarm2.md3");
      cgs.media.heinrichArmor[47] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftshoulder2.md3");
      cgs.media.heinrichArmor[48] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftthigh2.md3");
      cgs.media.heinrichArmor[49] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_rtcalf2.md3");
      cgs.media.heinrichArmor[50] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtforarm2.md3");
      cgs.media.heinrichArmor[51] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtshoulder2.md3");
      cgs.media.heinrichArmor[52] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtthigh2.md3");

      cgs.media.heinrichArmor[43] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftfoot2.md3");
      cgs.media.heinrichArmor[54] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_rtfoot2.md3");
      cgs.media.heinrichArmor[55] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftuparm2.md3");
      cgs.media.heinrichArmor[56] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtuparm2.md3");
      cgs.media.heinrichArmor[57] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_waist2.md3");
      cgs.media.heinrichArmor[58] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftknee2.md3");
      cgs.media.heinrichArmor[59] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_rtknee2.md3");

      cgs.media.heinrichArmor[60] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_lftelbow2.md3");
      cgs.media.heinrichArmor[61] = trap_R_RegisterModel(
          "models/players/heinrich/armor/dam_rtelbow2.md3");
      cgs.media.heinrichArmor[62] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_lfthip2.md3");
      cgs.media.heinrichArmor[63] =
          trap_R_RegisterModel("models/players/heinrich/armor/dam_rthip2.md3");
    ...
    // FIXME: this should be moved over to per model scripts or animation
    // scripting
    if (!Q_strcasecmp((char *)modelName, "eliteguard")) {
      ...
      for (i = 0; i < 4; i++) {
        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/eliteguard/step%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_ELITE_STEP][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/eliteguard/clank%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_ELITE_METAL][i] =
            trap_S_RegisterSound(name);

        //				Com_sprintf (name, sizeof(name),
        //"sound/player/footsteps/eliteguard/roof%i.wav", i+1);
        //				cgs.media.footsteps[FOOTSTEP_ELITE_ROOF][i] =
        //trap_S_RegisterSound (name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/eliteguard/wood%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_ELITE_WOOD][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/eliteguard/gravel%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_ELITE_GRAVEL][i] =
            trap_S_RegisterSound(name);
      }
    } else if (!Q_strcasecmp((char *)modelName, "protosoldier")) {
      // ProtoSoldier
      for (i = 0; i < 4; i++) {
        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/protosoldier/step%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_STEP][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/protosoldier/clank%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_METAL][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/protosoldier/grass%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_GRASS][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/protosoldier/gravel%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_GRAVEL][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/protosoldier/wood%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_PROTOSOLDIER_WOOD][i] =
            trap_S_RegisterSound(name);
      }
    } else if (!Q_strcasecmp((char *)modelName, "supersoldier")) {
      ...
      for (i = 0; i < 4; i++) {
        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/supersoldier/step%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_SUPERSOLDIER_STEP][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/supersoldier/clank%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_SUPERSOLDIER_METAL][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/supersoldier/grass%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_SUPERSOLDIER_GRASS][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/supersoldier/gravel%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_SUPERSOLDIER_GRAVEL][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/supersoldier/wood%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_SUPERSOLDIER_WOOD][i] =
            trap_S_RegisterSound(name);
      }
    }
    // Heinrich special
      ...
      for (i = 0; i < 4; i++) {
        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/heinrich/step%i.wav", i + 1);
      ...
      for (i = 0; i < 4; i++) {
        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/loper/clank%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_LOPER_METAL][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/loper/step%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_LOPER_STEP][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/loper/wood%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_LOPER_WOOD][i] =
            trap_S_RegisterSound(name);
      }
      ...
      for (i = 0; i < 4; i++) {
        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/zombie/gravel%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_ZOMBIE_GRAVEL][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/zombie/step%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_ZOMBIE_STEP][i] =
            trap_S_RegisterSound(name);

        Com_sprintf(name, sizeof(name),
                    "sound/player/footsteps/zombie/wood%i.wav", i + 1);
        cgs.media.footsteps[FOOTSTEP_ZOMBIE_WOOD][i] =
            trap_S_RegisterSound(name);
      }
    } else if (!Q_strcasecmp((char *)modelName, "beast")) {
      // Helga Boss
      cgs.media.footsteps[FOOTSTEP_BEAST][0] =
          CG_SoundScriptPrecache("beastStep"); // just precache the sound script
    }
  }
  ...
  if (!Q_strcasecmp((char *)modelName, "loper")) {
    ci->partModels[8] =
        trap_R_RegisterModel(va("models/players/%s/spinner.md3", modelName));
...
==============
*/
static qboolean CG_RegisterClientHeadname(clientInfo_t *ci,
                                          const char *modelName,
                                          const char *hSkinName) {
  char namefromskin[MAX_QPATH];
  char filename[MAX_QPATH];
  ...
  if (trap_R_GetSkinModel(ci->headSkin, "md3_hat", &namefromskin[0])) {
    CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                   &ci->accModels[ACC_HAT], &ci->accSkins[ACC_HAT]);
  }

  for (i = 0; i < ACC_NUM_MOUTH - 1; i++) {
    if (trap_R_GetSkinModel(ci->headSkin, va("md3_hat%d", 2 + i),
                            &namefromskin[0])) {
      CG_RegisterAcc(ci, va("models/players/%s", modelName), namefromskin,
                     &ci->accModels[ACC_MOUTH2 + i],
                     &ci->accSkins[ACC_MOUTH2 + i]);
    }
  }
...
void CG_LoadClientInfo(clientInfo_t *ci) {
  ...
  // load the head first (since if the head loads but there is a problem with
  // something in the lower body, you will want to default the model back to a
  // default and want the head to match)
  //

  if (!CG_RegisterClientHeadname(ci, ci->modelName, ci->hSkinName)) {
    if (cg_buildScript.integer) {
      CG_Error("CG_RegisterClientHeadname( %s, %s ) failed.  setting default",
               ci->modelName, ci->hSkinName);
    }
    ...
    if (!CG_RegisterClientHeadname(ci, ci->modelName, "default")) {
      headfail = 1;
      if (cg_buildScript.integer) {
        CG_Error("head model/skin (%s/default) failed to register",
                 ci->modelName); //----(SA)
      }
    }
  }
  ...
  if (headfail ||
      !CG_RegisterClientModelname(ci, ci->modelName, ci->skinName)) {
    if (cg_buildScript.integer) {
      CG_Error("CG_RegisterClientModelname( %s, %s ) failed", ci->modelName,
               ci->skinName);
    }
    ...
    if (cgs.gametype >= GT_TEAM) {
      ...
      // try to keep the model but default the skin (so you can tell bad guys
      // from good)
      if (!CG_RegisterClientModelname(ci, ci->modelName, "default")) {
        CG_Error("DEFAULT_MODEL (%s/default) failed to register",
                 ci->modelName);
      }
    } else {
      // go totally default
      ...
      if (!CG_RegisterClientHeadname(ci, DEFAULT_MODEL, "default")) {
        CG_Error("model/ DEFAULT_HEAD / skin (%s/default) failed to register",
                 DEFAULT_HEAD);
      }
    }
  }
...
static void CG_CopyClientInfoModel(clientInfo_t *from, clientInfo_t *to) {
  ...
  cgs.animScriptData.clientModels[to->clientNum] =
      cgs.animScriptData.clientModels[from->clientNum];
}
...
static qboolean CG_ScanForExistingClientInfo(clientInfo_t *ci) {
  ...
  for (i = 0; i < cgs.maxclients; i++) {
    ...
    //----(SA) added checks for same head.  FIXME: soon this will be more
    //efficient than just looking for a complete character (body/head) match.
...
void CG_NewClientInfo(int clientNum) {
  ...
  // (SA) note to Ryan: The problem I see with having the model set for
  // cg_forceModel in the game (g_forcemodel)
  //		is that it was initally there for a performance/fairness thing so
  //you can connect to a 		server and not use other players goofy models or
  //whatever.  We should still have some simple 		client-side thing for defaulting
  //all models to one particular player model or something.  (did that make
  //sense?)
  ...
  /* RF, disabled this, not needed anymore
          if ( cg_forceModel.integer )
          {
                  ...
                  trap_Cvar_VariableStringBuffer( "head", modelStr, sizeof(
     modelStr ) ); Q_strncpyz( newInfo.hSkinName, modelStr, sizeof(
     newInfo.hSkinName ) );
          }
          else {
  */
  ...
  /* RF, disabled this, not needed anymore
          if ( cg_forceModel.integer ) {
                  ...
                  trap_Cvar_VariableStringBuffer( "model", modelStr, sizeof(
     modelStr ) ); if ( ( skin = strchr( modelStr, '/' ) ) == NULL) { skin =
     "default"; } else { *skin++ = 0;
                  }
  ...
  */
  ...
  if (!CG_ScanForExistingClientInfo(&newInfo)) {
    ...
    // RF, disabled this, we can't have this happening in Wolf. If there is not
    // enough memory, then we have a leak or havent allocated enough hunk
    forceDefer = qfalse;

    //		forceDefer = trap_MemoryRemaining() < 4000000;

    // if we are defering loads, just have it pick the first valid
    //		if ( forceDefer || ( cg_deferPlayers.integer &&
    ...
    if (forceDefer) {
      ...
      if (ci->infoValid && (cgs.gametype < GT_TEAM ||
...
static void CG_SetLerpFrameAnimation(clientInfo_t *ci, lerpFrame_t *lf,
  ...
  if (cg_debugAnim.integer == 1) { // DHM - Nerve :: extra debug info
    CG_Printf("Anim: %i, %s\n", newAnimation,
              ci->modelInfo->animations[newAnimation].name);
  }
}
...
void CG_RunLerpFrame(clientInfo_t *ci, lerpFrame_t *lf, int newAnimation,
  ...
  if (ci && (newAnimation != lf->animationNumber ||
             !lf->animation)) { //----(SA)	modified
...
===============
*/
void CG_SetLerpFrameAnimationRate(centity_t *cent, clientInfo_t *ci,
  ...
  if (!(anim->flags & ANIMFL_FIRINGANIM) || (lf != &cent->pe.torso)) {
    if ((lf == &cent->pe.legs) && (CG_IsCrouchingAnim(ci, newAnimation) !=
                                   CG_IsCrouchingAnim(ci, oldAnimNum))) {
      if (anim->moveSpeed ||
          (anim->movetype &
           ((1 << ANIM_MT_TURNLEFT) |
            (1
...
===============
*/
void CG_RunLerpFrameRate(clientInfo_t *ci, lerpFrame_t *lf, int newAnimation,
                         centity_t *cent, int recursion) {
  int f;
  animation_t *anim, *oldAnim;
    ...
    if (cg.latestSnapshotTime != lf->oldFrameSnapshotTime && cg.nextSnap) {
      if (cent->currentState.number == cg.snap->ps.clientNum) {
        ...
        if (cg.snap != cg.nextSnap) {
          moveSpeed =
              Distance(cent->currentState.pos.trBase,
                       cent->nextState.pos.trBase) /
              ((float)(cg.nextSnap->serverTime - cg.snap->serverTime) / 1000.0);
        } else {
          moveSpeed = 0;
        }
      }
                ...
                lf->oldFrame >=
                    anim->firstFrame +
                        anim->numFrames)) { // Ridah, added this so walking
                                            ...
                                            // 0, which can happen in the middle
                                            // of a walking anim, which looks
                                            // wierd
      lf->frameTime = lf->animationTime;    // initial lerp
      ...
      if (anim->moveSpeed || lf->frameTime + 1000 < cg.time) {
        ...
        */
        while (lf->frameTime < cg.time) {
          lf->frameTime +=
              (int)((float)anim->frameLerp * (1.0 / lf->animSpeedScale));
          f++;
        ...
        while (lf->frameTime < cg.time) {
          ...
          f++; // f is allowed to go over anim->numFrame, since it gets adjusted
               // later
        }
      }
#endif
...
===============
*/
void CG_ClearLerpFrameRate(clientInfo_t *ci, lerpFrame_t *lf,
                           int animationNumber, centity_t *cent) {
  lf->frameTime = lf->oldFrameTime = cg.time;
...
static void CG_PlayerAnimation(centity_t *cent, int *legsOld, int *legs,
  ...
  if (!(cent->currentState.eFlags & (EF_DEAD | EF_NO_TURN_ANIM)) &&
      cent->pe.legs.yawing) {
    // CG_Printf("turn: %i\n", cg.time );
    tempIndex = BG_GetAnimScriptAnimation(clientNum, cent->currentState.aiState,
                                          (cent->pe.legs.yawing == SWING_RIGHT
                                               ? ANIM_MT_TURNRIGHT
                                               : ANIM_MT_TURNLEFT));
    if (tempIndex > -1) {
      animIndex = tempIndex;
...
static void CG_AddPainTwitch(centity_t *cent, vec3_t torsoAngles) {
  ...
  if (cent->currentState.aiChar) {
    if (cent->pe.painAnimTorso >= 0) {
      ...
      if (cent->pe.torso.frame != cent->pe.torso.oldFrame ||
          cent->pe.torso.frame != anim->firstFrame + anim->numFrames - 1) {
...
#define FADEOUT_RATIO (1.0 - FADEIN_RATIO)
    ...
    if (f < FADEIN_RATIO) {
      ...
      torsoAngles[ROLL] += (0.5 * direction * (1.0 - (f - FADEIN_RATIO)) *
                            (1.0 / FADEOUT_RATIO));
      torsoAngles[PITCH] -= (fabs(direction) * (1.0 - (f - FADEIN_RATIO)) *
                             (1.0 / FADEOUT_RATIO));
      torsoAngles[YAW] +=
          (direction * (1.0 - (f - FADEIN_RATIO)) * (1.0 / FADEOUT_RATIO));
    }
  } else { // fast, Q3 style
...
================
*/
static void CG_IdleHeadMovement(centity_t *cent, const vec3_t torsoAngles,
                                vec3_t headAngles) {
  const float angleSpeedMax = 60;
  const float angleSpeedAccel = 40;
  // const float diffThreshold = 50;	// don't yaw from torso beyond this in
  // any axis
  ...
  if (cent->pe.headLookStopTime < cg.time &&
      !VectorCompare(cent->pe.headLookIdeal, cent->pe.headLookOffset)) {
    // slow down as we get closer
    if (fabs(cent->pe.headLookOffset[YAW] - cent->pe.headLookIdeal[YAW]) <
        angleSpeedMax * 1.2) {
      cent->pe.headLookSpeedMax =
          angleSpeedMax * (0.1 + 0.9 * (fabs(cent->pe.headLookOffset[YAW] -
                                             cent->pe.headLookIdeal[YAW]) /
                                        (angleSpeedMax * 1.2)));
    }
    ...
    for (i = 0; i < 3; i++) {
      if (cent->pe.headLookOffset[i] < cent->pe.headLookIdeal[i]) {
        cent->pe.headLookOffset[i] +=
            cent->pe.headLookSpeed * 0.001 * cg.frametime;
        if (cent->pe.headLookOffset[i] > cent->pe.headLookIdeal[i]) {
    ...
    if (VectorCompare(cent->pe.headLookIdeal, cent->pe.headLookOffset)) {
      cent->pe.headLookStopTime =
          cg.time + (int)(stopTime * (0.5 + 0.5 * random()));
    }
    // randomly choose a new destination before reaching ideal angles
    if (cent->pe.headLookStopTime % 2 &&
        rand() % (cg.time - cent->pe.headLookStopTime) > 700) {
  ...
  /*
          ...
          for (i=0; i<3; i++) {
                  if (fabs(cent->pe.headLookOffset[i] + headAngles[i] -
     torsoAngles[i]) > diffThreshold) { if (cent->pe.headLookOffset[i] +
     headAngles[i] - torsoAngles[i] > 0) cent->pe.headLookOffset[i] =
     diffThreshold - (headAngles[i] - torsoAngles[i]); else
                                  cent->pe.headLookOffset[i] = -diffThreshold -
     (headAngles[i] - torsoAngles[i]);
                  }
          }
  */
...
static void CG_PlayerAngles(centity_t *cent, vec3_t legs[3], vec3_t torso[3],
  ...
  } else if (BG_GetConditionValue(cent->currentState.number, ANIM_COND_FIRING,
                                  qtrue)) {
  ...
  if (cent->currentState.eFlags & EF_DEAD) {
    ...
    if (cent->currentState.eFlags & EF_NOSWINGANGLES) {
      legsAngles[YAW] = torsoAngles[YAW] =
          headAngles[YAW]; // always face firing direction
      clampTolerance = 40;
    } else if (!(cent->currentState.eFlags & EF_FIRING) &&
               !(cent->currentState.eFlags & EF_RECENTLY_FIRING)) {
      torsoAngles[YAW] =
    ...
    if (!cent->pe.torso.yawing) {
      CG_SwingAngles(torsoAngles[YAW], 30, clampTolerance,
                     0.5 * cg_swingSpeed.value, &cent->pe.torso.yawAngle,
                     &cent->pe.torso.yawing);
    } else if (!(cent->currentState.eFlags & EF_FIRING) &&
               !(cent->currentState.eFlags & EF_RECENTLY_FIRING)) {
      // not firing
      CG_SwingAngles(torsoAngles[YAW], 0, clampTolerance,
                     0.5 * cg_swingSpeed.value, &cent->pe.torso.yawAngle,
                     &cent->pe.torso.yawing);
    } else { // firing
      CG_SwingAngles(torsoAngles[YAW], 0, clampTolerance,
                     4.0 * cg_swingSpeed.value, &cent->pe.torso.yawAngle,
                     &cent->pe.torso.yawing);
    }
    ...
    if (BG_GetConditionValue(ci->clientNum, ANIM_COND_MOVETYPE, qfalse) &
        (1 << ANIM_MT_IDLE)) {
      if (cent->pe.legs.yawing) {
        CG_SwingAngles(legsAngles[YAW], 0, clampTolerance,
                       0.75 * cg_swingSpeed.value, &cent->pe.legs.yawAngle,
                       &cent->pe.legs.yawing);
      } else {
        cent->pe.legs.yawing = qfalse; // set it if they really need to swing
        CG_SwingAngles(legsAngles[YAW], 50, clampTolerance,
                       0.75 * cg_swingSpeed.value, &cent->pe.legs.yawAngle,
                       &cent->pe.legs.yawing);
      }
    } else
      ...
      CG_SwingAngles(legsAngles[YAW], 50, clampTolerance, cg_swingSpeed.value,
                     &cent->pe.legs.yawAngle, &cent->pe.legs.yawing);
    }
...
static void CG_PlayerPowerups(centity_t *cent) {
  ...
  if (cent->pe.teslaDamagedTime > cg.time - 400) {
    trap_R_AddLightToScene(cent->lerpOrigin, 128 + 128 * sin(cg.time * cg.time),
                           0.2, 0.6, 1, 0);
  }

  // RF, AI don't use these effects, they are generally added manually by the
  // game
  if (cent->currentState.aiChar) {
    return;
  }
  ...
  //vec3_origin, cgs.media.flightSound, 255 );
  //	}
...
static void CG_PlayerSprites(centity_t *cent) {
  ...
  if (cg_showAIState.integer && cent->currentState.aiChar) {
    CG_PlayerFloatSprite(
        cent, cgs.media.aiStateShaders[cent->currentState.aiState], 48);
    return;
  }
  ...
  if (cgs.gametype == GT_WOLF && (cent->currentState.eFlags & EF_DEAD) &&
    ...
    ////----(SA)	commented out from MP
    return;
  }

  // DHM - Nerve :: not using, gives away position if chatting to coordinate
  // attack
...
#define SHADOW_MAX_DIST 512.0
  shadowPart_t shadowParts[] = {{"tag_footleft", 10, 4, 0.5, 0},
  ...
  *shadowPlane = trace.endpos[2] + 1;
  ...
  if (CG_GetOriginForTag(cent, &cent->pe.legsRefEnt, "tag_torso", 0, origin,
                         axis)) {
  ...
  if (dist < SHADOW_MAX_DIST) { // show more detail
    ...
    for (tagIndex = 0; shadowParts[tagIndex].tagname; tagIndex++) {
      ...
      for (subIndex = 0;
           (subIndex = CG_GetOriginForTag(cent, &cent->pe.legsRefEnt,
        ...
        alpha = 1.0 - ((origin[2] - (*shadowPlane + ZOFS)) /
                       shadowParts[tagIndex].maxdist);
        if (alpha < 0) {
          continue;
        }
        ...
        CG_ImpactMark(shadowParts[tagIndex].shader, origin, trace.plane.normal,
                      angles[YAW] /*cent->pe.legs.yawAngle*/, alpha, alpha,
                      alpha, 1, qfalse, shadowParts[tagIndex].size * scale,
                      qtrue, -1);
      }
    }
  }
...
==============
*/
void CG_SpawnZombieSpirit(vec3_t origin, vec3_t vel, int trailHead,
                          int ownerNum, refEntity_t *oldrefent, int trailLife,
                          int idealWidth) {
  localEntity_t *le;
  refEntity_t *re;
  ...
  *re = *oldrefent;
  ...
  le->startTime =
      cg.time -
      5000; // set it back so we dont fade it in like the "end" spirits
...
void CG_AddZombieSpiritEffect(centity_t *cent) {
  ...
  if (cent->currentState.eFlags & EF_MONSTER_EFFECT) {
    ...
    if (!cent->pe.cueZombieSpirit) {
      ...
      for (i = 0; i < MAX_ZOMBIE_SPIRITS; i++) {
        ...
        cent->pe.zombieSpiritRotationTimes[i] =
            minRotationTime + (random() * (maxRotationTime - minRotationTime));
        cent->pe.zombieSpiritRadiusCycleTimes[i] =
            minRadiusCycleTime +
            (random() * (maxRadiusCycleTime - minRadiusCycleTime));
    ...
    if (cent->currentState.eFlags & EF_MONSTER_EFFECT3 ||
        cent->currentState.effect1Time < cent->currentState.effect3Time) {
      return;
    }
  ...
  for (t = cent->pe.lastZombieSpirit + step; t <= cg.time; t += step) {
    ...
    for (i = 0; i < MAX_ZOMBIE_SPIRITS; i++) {
      ...
      fadeRatio = (float)(cg.time - cent->pe.zombieSpiritStartTimes[i]) /
                  (float)fadeInTime;
      if (fadeRatio < 0.0) {
        fadeRatio = 0.0;
      }
      ...
      if (cent->pe.cueZombieSpirit) {
        ...
        radius = (minDist +
                  sin(M_PI * 2 *
                      (float)((float)((int)(t + ((float)(radiusCycleTime * i) /
                                                 MAX_ZOMBIE_SPIRITS)) %
                                      radiusCycleTime) /
                              (float)radiusCycleTime)) *
                      (maxDist - minDist));

        // get the position
        v[0] = (0.5 + 0.5 * fadeRatio) *
               sin(M_PI * 2 *
                   (float)((float)((int)(t + ((float)(rotationTime * i) /
                                              MAX_ZOMBIE_SPIRITS)) %
                                   rotationTime) /
                           (float)rotationTime)) *
               radius;
        v[1] = (0.5 + 0.5 * fadeRatio) *
               cos(M_PI * 2 *
                   (float)((float)((int)(t + ((float)(rotationTime * i) /
                                              MAX_ZOMBIE_SPIRITS)) %
                                   rotationTime) /
                           (float)rotationTime)) *
               radius;
        v[2] = 12 + 36 * (0.5 + 0.5 * fadeRatio) *
                        sin(M_PI * 2 *
                            (float)((float)((int)(t + ((float)(zCycleTime * i) /
                                                       MAX_ZOMBIE_SPIRITS)) %
                                            zCycleTime) /
                                    (float)zCycleTime));
        ...
        radius =
            pow(1.0 - fadeRatio, 2) * fadeDist +
            (1.0 - pow(1.0 - fadeRatio, 2)) *
                (minDist +
                 sin(M_PI * 2 *
                     (float)((float)((int)(t + ((float)(radiusCycleTime * i) /
                                                MAX_ZOMBIE_SPIRITS)) %
      ...
      VectorSubtract(p[i], cent->pe.zombieSpiritPos[i],
                     cent->pe.zombieSpiritDir[i]);
      cent->pe.zombieSpiritSpeed[i] =
          1000.0 / step * VectorNormalize(cent->pe.zombieSpiritDir[i]);
      VectorCopy(p[i], cent->pe.zombieSpiritPos[i]);

      cent->pe.zombieSpiritTrailHead[i] = CG_AddTrailJunc(
          cent->pe.zombieSpiritTrailHead[i], cgs.media.zombieSpiritTrailShader,
          t, STYPE_STRETCH, p[i], trailLife * 2, alpha, 0.0,
          (0.5 + 0.5 * fadeRatio) * idealWidth, 0, TJFL_NOCULL, colorWhite,
          colorWhite, 1.0, 1);
    }
  ...
  for (i = 0; i < MAX_ZOMBIE_SPIRITS; i++) {
    ...
    // add the sound
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           vec3_origin, cgs.media.zombieSpiritLoopSound,
                           fadeRatio);
    //
    // if this spirit is in a good position to be released and head to the
    // enemy, then release it
    if (fadeRatio == 1.0 &&
        (lastSpiritRelease > cg.time || (lastSpiritRelease < cg.time - 2000))) {
      VectorSubtract(cent->currentState.origin2, refent.origin, v);
      VectorNormalize(v);
      if (DotProduct(cent->pe.zombieSpiritDir[i], v) > 0.6 ||
          (cent->currentState.eFlags & EF_DEAD)) {
        // check for sinking into geometry
        trap_CM_BoxTrace(&trace, refent.origin, refent.origin, NULL, NULL, 0,
                         MASK_SOLID);
        ...
        if (!trace.startsolid) {
          ...
          VectorScale(cent->pe.zombieSpiritDir[i],
                      cent->pe.zombieSpiritSpeed[i], v);
          CG_SpawnZombieSpirit(
              refent.origin, v, cent->pe.zombieSpiritTrailHead[i],
              cent->currentState.number, &refent, trailLife, idealWidth);
  ...
  if (cg.time > cent->pe.nextZombieSpiritSound &&
      cent->pe.cueZombieSpirit) { //&& (cg.time < cent->pe.zombieSpiritStartTime
                                  //+ sndDuration)) {
    // spawn a new sound
    trap_S_StartSound(cent->lerpOrigin, -1, CHAN_AUTO,
                      cgs.media.zombieSpiritSound);
    cent->pe.nextZombieSpiritSound =
        cg.time + sndIntervalMin +
        (int)((float)(sndIntervalMax - sndIntervalMin) * random());
  }

  // add a negative light around us
  fadeRatio =
      (float)(cg.time - cent->pe.zombieSpiritStartTime) / (float)fadeInTime;
  if (fadeRatio < 0.0) {
    fadeRatio = 0.0;
  }
  if (fadeRatio > 1.0) {
  ...
  fadeRatio *= 0.7;
  trap_R_AddLightToScene(cent->lerpOrigin, 300.0, 1.0 * fadeRatio,
                         1.0 * fadeRatio, 1.0 * fadeRatio, 10);
}
...
void CG_SpawnZombieBat(centity_t *cent, refEntity_t *oldrefent) {
  ...
  *re = *oldrefent;
  ...
  le->endTime =
      le->startTime + 6000; // TODO: some kind of effect when the bat "burns up"
...
void CG_AddZombieFlameEffect(centity_t *cent) {
  // const float	flameRadius		= ZOMBIE_FLAME_RADIUS*2; // TTimo:
  // unused
  ...
  */
  ...
  if ((cent->currentState.eFlags & EF_MONSTER_EFFECT3) &&
      (cg.time > cent->currentState.effect3Time)) {
  ...
  if (cent->pe.cueZombieSpirit) {
    ...
    alpha =
        ((float)(cent->pe.zombieSpiritEndTime - cg.time) / (float)fadeOutTime);
    fadeRatio = alpha;
    if (alpha < 0.0) {
      cent->pe.zombieSpiritEndTime = 0; // stop the effect
  ...
  if (fadeRatio >= 1.0) {
    ...
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           vec3_origin, cgs.media.flameSound, 50);
  }
}
...
#define LOPER_MAX_POINT_TESTS_PERFRAME 20
  ...
  CG_GetOriginForTag(cent, &cent->pe.legsRefEnt, "tag_spinner", 0, tagPos,
                     NULL);
  ...
  trap_R_AddLightToScene(
      tagPos, LOPER_LIGHTNING_NORMAL_DIST * (2.5 + (1.0 + sin(cg.time)) / 4.0),
      c[0], c[1], c[2], 1);
  ...
  for (i = 0; i < numPoints; i++) {
    ...
    if ((cent->currentState.eFlags & EF_MONSTER_EFFECT) &&
        ((!cent->pe.lightningTimes[i]) ||
         ...
         (VectorDistance(cent->lerpOrigin, cent->pe.lightningPoints[i]) >
          maxDist))) {
      // attacking the player
      VectorSet(testPos, 12 * crandom(), 12 * crandom(), 32 * crandom());
      VectorAdd(testPos, cg.snap->ps.origin, testPos);
      cent->pe.lightningTimes[i] =
          cg.time - rand() % (LOPER_LIGHTNING_POINT_TIMEOUT / 2);
      VectorCopy(testPos, cent->pe.lightningPoints[i]);
      // play a zap sound
      if (cent->pe.lightningSoundTime < cg.time - 100) {
        trap_S_StartSound(
            testPos, ENTITYNUM_WORLD, CHAN_AUTO,
            cgs.media.lightningZap /*cgs.media.lightningSounds[rand()%3]*/);
        cent->pe.lightningSoundTime = cg.time;
      }
    } else if ((!cent->pe.lightningTimes[i]) ||
               (cent->pe.lightningTimes[i] > cg.time) ||
               (cent->pe.lightningTimes[i] <
                cg.time - LOPER_LIGHTNING_POINT_TIMEOUT) ||
               (VectorDistance(cent->lerpOrigin, cent->pe.lightningPoints[i]) >
                maxDist)) {
      ...
      for (j = 0; j < LOPER_MAX_POINT_TESTS; j++) {
        VectorSet(testPos, maxDist * crandom(), maxDist * crandom(),
                  maxDist * (crandom() - 0.5));
        VectorAdd(testPos, cent->lerpOrigin, testPos);
        // try a trace to find a world collision
        CG_Trace(&tr, cent->lerpOrigin, NULL, NULL, testPos,
        ...
        if (tr.fraction < 1 && tr.entityNum == ENTITYNUM_WORLD) {
          ...
          // play a zap sound
          if (cent->pe.lightningSoundTime < cg.time - 100) {
            // HACK, move ths sound away from the viewpos, to simulate lower
            // volume
            VectorSubtract(testPos, cg.refdef.vieworg, v);
            VectorMA(cg.refdef.vieworg, 3.0, v, v);
            trap_S_StartSound(v, ENTITYNUM_WORLD, CHAN_AUTO,
                              cgs.media.lightningSounds[rand() % 3]);
            cent->pe.lightningSoundTime = cg.time;
          }
          break;
        }
    ...
    if (VectorDistance(cent->lerpOrigin, cent->pe.lightningPoints[i]) <=
        maxDist) {
      CG_DynamicLightningBolt(
          cgs.media.lightningBoltShader, tagPos, cent->pe.lightningPoints[i], 1,
          25 + 12.0 * random(),
          (cent->currentState.eFlags & EF_MONSTER_EFFECT) == 0, 1.0, 0,
          i * i * 2);
    }
  }
}
...
#define LOPER_GROUNDCHARGE_RADIUS 150
  ...
  if (!(cent->currentState.eFlags & EF_MONSTER_EFFECT3)) {
    if (!cent->pe.loperGroundValidTime) {
      ...
      duration = LOPER_GROUNDCHARGE_FADEOUT -
                 (cg.time - cent->pe.loperGroundValidTime);
      if (duration <= 0) {
      ...
      lightAlpha =
          (float)(duration + LOPER_GROUNDCHARGE_DURATION) /
          (float)(LOPER_GROUNDCHARGE_FADEOUT + LOPER_GROUNDCHARGE_DURATION);
      if (lightAlpha < 0) {
        lightAlpha = 0;
      }
    }
  ...
  trap_R_AddLightToScene(
      cent->lerpOrigin,
      LOPER_GROUNDCHARGE_RADIUS *
          (3.0 +
           2.0 *
               (1.0 + sin(0.001 * ((cg.time) %
                                   (1000 * (2 + cent->currentState.number))))) /
               2.0),
      c[0], c[1], c[2], 1);
  // trap_R_AddLightToScene( cent->lerpOrigin, LOPER_GROUNDCHARGE_RADIUS*(2.0
  // + 1.0*(1.0+cos(0.001343*((cg.time)%(1000*(2+cent->currentState.number)))))/2.0),
  // c[0], c[1], c[2], 0 );
  ...
  if (cent->pe.loperLastGroundChargeTime <
      cg.time - LOPER_GROUNDCHARGE_INTERVAL) {
    // org
    ...
    // draw the mark
    // CG_ImpactMark( cgs.media.loperGroundChargeShader, org, up, random()*360,
    // c[0], c[1], c[2], alpha, qtrue,
    // (0.5+0.5*random())*LOPER_GROUNDCHARGE_RADIUS, qfalse,
    // LOPER_GROUNDCHARGE_DURATION );
    cent->pe.loperLastGroundChargeTime = cg.time;
    // make a new sound
    VectorSet(org, org[0] + crandom() * 256, org[1] + crandom() * 256,
              org[2] + crandom() * 256);
...
==============
*/
void CG_SpawnHelgaSpirit(vec3_t origin, vec3_t vel, int trailHead, int ownerNum,
                         refEntity_t *oldrefent, int trailLife,
                         int idealWidth) {
  localEntity_t *le;
  refEntity_t *re;
...
void CG_AddHelgaSpiritEffect(centity_t *cent) {
  ...
  for (i = 0; i < MAX_ZOMBIE_SPIRITS; i++) {
    ...
    */                                                                                                                                                                                                                           //
		// add the sound
    trap_S_AddLoopingSound(-1, cent->lerpOrigin, vec3_origin,
                           cgs.media.helgaSpiritLoopSound, fadeRatio);
    //
    // if this spirit is in a good position to be released and head to the
    // enemy, then release it
    if (fadeRatio == 1.0 &&
        (lastSpiritRelease > cg.time || (lastSpiritRelease < cg.time - 1000))) {
      VectorSubtract(cent->currentState.origin2, refent.origin, v);
      VectorNormalize(v);
      if (DotProduct(cent->pe.zombieSpiritDir[i], v) > 0.4 ||
          (cent->currentState.eFlags & EF_DEAD)) {
        // check for sinking into geometry
        trap_CM_BoxTrace(&trace, refent.origin, refent.origin, NULL, NULL, 0,
                         MASK_SOLID);
  ...
  if (cg.time > cent->pe.nextZombieSpiritSound &&
    ...
    // trap_S_StartSound( cent->lerpOrigin, -1, CHAN_AUTO,
    // cgs.media.helgaSpiritSound );
    CG_SoundPlayIndexedScript(cgs.media.helgaSpiritSound, NULL,
  ...
  fadeRatio *= 0.7;
  trap_R_AddLightToScene(cent->lerpOrigin, 500.0, 1.0 * fadeRatio,
                         1.0 * fadeRatio, 1.0 * fadeRatio, 10);
}
...
void CG_AddRefEntityWithPowerups(refEntity_t *ent, int powerups, int team,
  ...
  // RF, if in camera mode, force a full lod, since we are in a controlled
  // environment
  if (cg.cameraMode) {
...
*/
...
#endif
    /*
            } else if (es->eFlags & EF_MONSTER_EFFECT2 && es->aiChar ==
       AICHAR_ZOMBIE && cent->currentState.effect2Time < cg.time) {	//
       Ridah, Zombie death effect const int fadeRiseTime = 4000;

                    if (cent->pe.zombieDeathFadeStart < cg.time && ent->hModel
       == cent->pe.legsRefEnt.hModel) {
                            ...
                            // torso
                            parentEnt = *ent;
                            CG_PositionEntityOnTag( ent, &parentEnt,
       parentEnt.hModel, "tag_torso", NULL ); ent->hModel =
       cgs.media.skeletonTorsoModel; ent->customSkin =
       cgs.media.skeletonTorsoSkin; trap_R_AddRefEntityToScene( ent );

                            // head
                            parentEnt = *ent;
                            CG_PositionEntityOnTag( ent, &parentEnt,
       parentEnt.hModel, "tag_head", NULL ); ent->hModel =
       cgs.media.skeletonHeadModel; ent->customSkin =
       cgs.media.skeletonHeadSkin; trap_R_AddRefEntityToScene( ent );
                    ...
                    if (cent->pe.zombieDeathFadeEnd + fadeRiseTime > cg.time) {
                            // slowly fade the zombie "skin" out, revealing the
       skeleton underneath

                            //VectorSubtract( ent->origin, cg.snap->ps.origin,
       ent->fireRiseDir );
                            ...
                            if (cent->pe.zombieDeathFadeEnd > cg.time) {
                                    ...
                                    // the zombie has hard-edged alpha blending
       on it's body texture by default
                                    // so we need to override that for smooth
       fading if (ent->hModel == cent->pe.legsRefEnt.hModel || ent->hModel ==
       cent->pe.torsoRefEnt.hModel) { ent->customShader =
       cgs.media.zombieBodyFadeShader; } else if (ent->hModel ==
       cent->pe.headRefEnt.hModel) { ent->customShader =
       cgs.media.zombieHeadFadeShader;
                                    }
                                    // fade the alpha from 0 -> 255 as the time
       goes, which will fade from front to back if
       (cent->pe.zombieDeathFadeStart > cg.time) { ent->shaderRGBA[3] = 128; }
       else { ent->shaderRGBA[3] = 128 - (unsigned
       char)(128.0*pow(((float)(cg.time - cent->pe.zombieDeathFadeStart) /
       (float)(cent->pe.zombieDeathFadeEnd - cent->pe.zombieDeathFadeStart)),
       2));
                                    }
                                    ent->shaderTime = 1.0;
  ...
  if (!es->time2 && alpha < 1.0 &&
      (cent->currentState.aiChar == AICHAR_ZOMBIE) &&
      IS_FLAMING_ZOMBIE(
          cent->currentState) /*&& !(cent->currentState.eFlags & EF_DEAD)*/) {
    onFire = qtrue;
    // set the alpha
    alpha = 1.0;
  }
  ...
  if (onFire) {
    ...
    */
    ...
    if (ent->hModel == cent->pe.legsRefEnt.hModel) {
      trap_S_AddLoopingSound(es->number, ent->origin, vec3_origin,
                             cgs.media.flameCrackSound, (int)(40.0 * alpha));
    }
  }
  ...
  if (cg_entities[es->number].pe.teslaDamagedTime > cg.time - 400) {
    ...
    alpha = (400.0 -
             (float)(cg.time - cg_entities[es->number].pe.teslaDamagedTime)) /
            400.0;
...
static float CG_SpinnerSpinAngle(centity_t *cent) {
  ...
  if (cent->currentState.eFlags &
      EF_DEAD) { // don't spin for dead loper (TODO: spindown, or blow off parts
...
static void CG_AddFireLight(centity_t *cent) {
  ...
  /* OPTIMIZATION, TOO MANY DLIGHTS WHEN FIRE IS AROUND
          ...
          if (CG_EntOnFire(&cg_entities[cent->currentState.number])) {
                  ...
                  trap_R_AddLightToScene( cent->lerpOrigin, 128 +
     128*alpha, 1.000000*alpha, 0.603922*alpha, 0.207843*alpha, 0 );
          }
  */
}
...
void CG_AnimPlayerConditions(centity_t *cent) {
  ...
  es = &cent->currentState;
  // DHM-Nerve
  // ci = &cgs.clientinfo[es->number];			// es->number is not always a
  // valid client num
  ...
  /* TODO???
  if (es->lean > 0) {
          BG_UpdateConditionValue( es->clientNum, ANIM_COND_LEANING,
  LEANING_RIGHT, qtrue ); } else if (es->lean < 0) { BG_UpdateConditionValue(
  es->clientNum, ANIM_COND_LEANING, LEANING_LEFT, qtrue ); } else {
          BG_UpdateConditionValue( es->clientNum, ANIM_COND_LEANING,
  LEANING_UNUSED, qtrue );
  }
  */
  ...
  //ci->modelInfo->animations[legsAnim].movetype, qfalse );
  //}
  // RF, changed this since we dont need to be careful about bandwidth anymore,
  // and this method is much more accurate
  if (cent->currentState.animMovetype) {
    BG_UpdateConditionValue(es->clientNum, ANIM_COND_MOVETYPE,
                            cent->currentState.animMovetype, qtrue);
  }
}
...
void CG_DeadSink(centity_t *cent) {
  ...
  cent->lerpOrigin[2] -=
      DEAD_SINK_DEPTH *
      ((float)(cg.time - cent->currentState.effect3Time) / DEAD_SINK_DURATION);
}
...
void CG_Player(centity_t *cent) {
  ...
  //----(SA)	added
  // setting the legs axis should pass the scale down through the other
  // parts/tags/etc. and will need to be 'undone' for the weapon
  ...
  if (ci->playermodelScale[0] != 0) { // player scaled, adjust for the (-24)
                                      // offset of player legs origin to ground
  ...
  //----(SA) check for ladder and if you're on it, don't allow torso model
  //rotation (so the body climbs aligned with the ladder)
  //----(SA)	also taking care of the Loper's interesting heirarchy (his upper
  //body is effectively the same as a weapon_hand.md3.  it keeps things
  //connected, but has no geometry)
  ...
  if (!ci->isSkeletal) {
    ...
    // skeletal models combine the legs and torso, so we must build a compiled
    // refEntity_t
...
#define NUM_EMOTIONS 2 // 0 neutral, 1 happy, 2 angry
    ...
    talk_frame =
        (int)floor((HEAD_EMOTION_SUBTYPES - 1) * (gumsflappin / 256.0));

    // add the current frame to the total, so when it comes to pick a new frame,
    // we choose the average of those we missed over the last frame
    ...
    if ((cent->pe.head.frameTime > cg.time + 30) &&
        cent->pe.head.animationTime &&
        !((cent->pe.head.frame - ci->modelInfo->headAnims[0].firstFrame) %
          HEAD_EMOTION_SUBTYPES) &&
        !((cent->pe.head.oldFrame - ci->modelInfo->headAnims[0].firstFrame) %
          HEAD_EMOTION_SUBTYPES)) {
    ...
    if (cent->pe.head.frameTime < cg.time) { // set the new frame
      ...
      if (cent->pe.head.animationTime) {
        ...
        talk_frame = (int)((float)cent->pe.head.animationTime /
                           (float)cent->pe.head.animationNumber);
        // subtype = rand()%HEAD_EMOTION_SUBTYPES;
        emotion = rand() % NUM_EMOTIONS; // this helps animate the mouth more
                                         // realistically while talking
        ...
        case AICHAR_LOPER:
          ...
          break;
        default:
          // randomly set it back to 0 amplitude to simulate certain synonyms
          // pronounced with a closed mouth
          closed =
              ((rand() % 5) == 0) && (talk_frame < (HEAD_EMOTION_SUBTYPES - 1));
        }
        ...
        cent->pe.head.frame =
            emotion * HEAD_EMOTION_SUBTYPES + talk_frame; // ci->modelInfo->headAnims[emotion*HEAD_EMOTION_SUBTYPES].firstFrame
                                                          // + talk_frame;
        cent->pe.head.frameTime =
            cg.time + 80 +
            rand() % 40; // interpolate for smoother animation vs latency

        // CG_Printf("%i head: frame %i, oldframe %i, nextframetime %i\n",
        // cg.time, cent->pe.head.frame, cent->pe.head.oldFrame,
        // cent->pe.head.frameTime );
...
#else
        while ((emotion = rand() % NUM_EMOTIONS) == 1)
          ; // don't use happy emotion
        if ((cent->pe.head.frame - ci->modelInfo->headAnims[0].firstFrame) %
            HEAD_EMOTION_SUBTYPES) {
          cent->pe.head.frame =
              ci->modelInfo->headAnims[emotion * HEAD_EMOTION_SUBTYPES]
                  .firstFrame;
          ...
          cent->pe.head.frame =
              ci->modelInfo->headAnims[emotion * HEAD_EMOTION_SUBTYPES]
                  .firstFrame;
          cent->pe.head.frameTime =
              cent->pe.head.frameTime + 1000 + rand() % 2000;
    ...
    head.backlerp =
        1.0 - (float)(cg.time - cent->pe.head.oldFrameTime) /
                  (cent->pe.head.frameTime - cent->pe.head.oldFrameTime);
  ...
  //----(SA)	modified
    ...
    char *protoTags[] = {"tag_chest",    "tag_calfleft", "tag_armleft",
                         "tag_back",     "tag_legleft",  "tag_calfright",
                         "tag_armright", "tag_back",     "tag_legright"};

    char *ssTags[] = {"tag_chest",    "tag_calfleft",  "tag_armleft",
                      "tag_back",     "tag_legleft",   "tag_calfright",
                      "tag_armright", "tag_back",      "tag_legright",

                      "tag_footleft", "tag_footright", "tag_sholeft",
                      "tag_shoright", "tag_torso",     "tag_calfleft",
                      "tag_calfright"};

    char *heinrichTags[] = {"tag_chest",    "tag_calfleft",  "tag_armleft",
                            "tag_back",     "tag_legleft",   "tag_calfright",
                            "tag_armright", "tag_back",      "tag_legright",

                            "tag_footleft", "tag_footright", "tag_sholeft",
                            "tag_shoright", "tag_torso",     "tag_legleft",
                            "tag_legright",

                            "tag_sholeft",  "tag_shoright",  "tag_legleft",
                            "tag_legright", "tag_legleft",   "tag_legright"};

    // TTimo: init
    int totalparts = 0, dynamicparts = 0, protoParts = 9, superParts = 16,
        heinrichParts = 22;
    char **tags = NULL;
    ...
    for (i = 0; i < totalparts; i++) {
      if ((i >= dynamicparts) ||
          (!(cent->currentState.dmgFlags &
             (1 << i)))) { // ones beyond 16 just draw the good part
        acc.hModel = models[i];
      } else {
...
*/
    ...
    /*
                    for(i=0;i<16;i++) {
                            ...
                            CG_PositionEntityOnTag( &acc, &torso, tags[i], 0,
       NULL); CG_AddRefEntityWithPowerups( &acc, cent->currentState.powerups,
       ci->team, &cent->currentState, cent->fireRiseDir );
                    }
    */
    //
  }
...
//#if 1
  ...
  //	if(cent->currentState.number != cg.predictedPlayerState.clientNum)
  {
    ...
    CG_Spotlight(cent, color, morg, viewDir, 12, 512, 2, 5,
                 SL_TRACEWORLDONLY | SL_NOCORE | SL_LOCKUV); // SL_NOTRACE
                                                             //		color[0] = 1;
                                                             //		color[1] = 1;
                                                             //		color[2] = 1;
    color[3] = 0.1f;
    CG_Spotlight(cent, color, morg, viewDir, 12, 512, 1.5, 2,
                 SL_TRACEWORLDONLY | SL_NOCORE | SL_NODLIGHT |
                     SL_LOCKUV); // SL_NOTRACE
  }
#endif
...
extern void CG_ClearWeapLerpFrame(clientInfo_t *ci, lerpFrame_t *lf,
                                  int animationNumber);
...
void CG_ResetPlayerEntity(centity_t *cent) {
  ...
  if (!(cent->currentState.eFlags & EF_DEAD)) {
    CG_ClearLerpFrameRate(&cgs.clientinfo[cent->currentState.clientNum],
                          &cent->pe.legs, cent->currentState.legsAnim, cent);
    ...
    cent->pe.legs.yawAngle =
        cent->nextState.apos.trBase[YAW]; // cent->rawAngles[YAW];
    ...
    cent->pe.torso.pitchAngle =
        cent->nextState.apos.trBase[PITCH]; // cent->rawAngles[PITCH];
...
void CG_GetBleedOrigin(vec3_t head_origin, vec3_t torso_origin,
                       vec3_t legs_origin, int fleshEntityNum) {
  clientInfo_t *ci;
  refEntity_t legs;
  refEntity_t torso;
  refEntity_t head;
  ...
  legs.hModel = ci->legsModel;
  //		VectorCopy( cg.snap->entities[fleshEntityNum - 1].pos.trBase,
  //legs.origin );
  VectorCopy(cent->lerpOrigin, legs.origin);
