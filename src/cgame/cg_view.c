...
static void CG_CalcVrect(void) {
  ...
  if (cg_letterbox.integer) {
    ysize = lbheight;
    //		if(letterbox_frac != 0) {
    //			letterbox_frac -= 0.01f;	// (SA) TODO: make non fps
    //dependant 			if(letterbox_frac < 0) 				letterbox_frac = 0; 			ysize += (lbdiff *
    //letterbox_frac);
    //		}
    //	} else {
    //		if(letterbox_frac != 1) {
    //			letterbox_frac += 0.01f;	// (SA) TODO: make non fps
    //dependant 			if(letterbox_frac > 1) 				letterbox_frac = 1; 			ysize = lbheight +
...
#define STEP 20
      ...
      if (cg.kickAVel[i] || cg.kickAngles[i]) {
        ...
        if (!cg.kickAngles[i] ||
          ...
          if (!cg.kickAngles[i] && frametime) {
            ...
            cg.kickAVel[i] = 0; // force Avel to return us to center rather than
                                // keep going outside range
          }
  ...
  //----(SA)	commented out since it doesn't appear to be used, and it spams
  //the console when in "developer 1" 	trap_Cvar_Set( "cg_recoilPitch", va("%f",
  //cg.recoilPitchAngle) );
}
...
static void CG_OffsetFirstPersonView(void) {
  ...
  if (cg.predictedPlayerState.leanf != 0) {
    ...
    VectorMA(cg.refdef.vieworg, cg.predictedPlayerState.leanf, right,
             cg.refdef.vieworg);
  }
...
static int CG_CalcFov(void) {
  ...
  if (cgs.gametype == GT_WOLF) {
    if (cg.snap->ps.pm_flags & PMF_FOLLOW &&
        cg.snap->ps.weapon == WP_SNIPERRIFLE) {
  ...
  if (!cg.zoomedBinoc) {
    ...
    if (cg.zoomval) {
      if (cg.snap->ps.weapon == WP_SNOOPERSCOPE) {
        cg.zoomSensitivity =
            0.3f * (cg.zoomval / 90.f); // NERVE - SMF - changed to get less
                                        // sensitive as you zoom in;
      }
      //				cg.zoomSensitivity = 0.2;
      else {
        cg.zoomSensitivity =
            0.6 * (cg.zoomval / 90.f); // NERVE - SMF - changed to get less
                                       // sensitive as you zoom in
      }
...
static void CG_UnderwaterSounds(void) {
  ...
  //vec3_origin, cgs.media.underWaterSound, 255 );
  trap_S_AddLoopingSound(cg.snap->ps.clientNum, cg.snap->ps.origin, vec3_origin,
                         cgs.media.underWaterSound, 255 & (1 << 8));
}
...
static void CG_DamageBlendBlob(void) {
  ...
  if (cg.v_dmg_time > cg.time) {
          redFlash = fabs(cg.v_dmg_pitch * ((cg.v_dmg_time - cg.time) /
  DAMAGE_TIME));
...
static int CG_CalcViewValues(void) {
  ...
  if (cg.cameraMode) {
    ...
    if (trap_getCameraInfo(CAM_PRIMARY, cg.time, &origin, &angles, &fov)) {
      ...
      // RF, had to disable, sometimes a loadgame to a camera in the same
      // position can cause the game to not know where the camera is, therefore
      // snapshots dont show the correct entities
  ...
  if (ps->viewlocked) {
    ...
    */
    BG_EvaluateTrajectory(&cg_entities[ps->viewlocked_entNum].currentState.apos,
                          cg.time, cg.refdefViewAngles);
  ...
  if (cg.renderingThirdPerson) {
    ...
    if (ps->viewlocked == 4) {
      ...
      VectorMA(cg_entities[ps->viewlocked_entNum].currentState.pos.trBase, 16,
               fwd, cg.refdef.vieworg);
      ...
      AngleVectors(cg.refdefViewAngles, fwd, NULL, NULL);
      VectorMA(cg_entities[ps->viewlocked_entNum].currentState.pos.trBase, -34,
               fwd, cg.refdef.vieworg);
...
void CG_DrawSkyBoxPortal(void) {
  ...
  if (cg_skybox.integer) {
    ...
    token = COM_ParseExt(&cstr, qfalse);
    if (!token || !token[0]) {
      CG_Error("CG_DrawSkyBoxPortal: error parsing skybox configstring.  No "
               "fog state\n");
    } else {
      vec4_t fogColor;
      int fogStart, fogEnd;
      ...
      if (atoi(token)) { // this camera has fog
        ...
        if (1) {
          ...
          trap_R_SetFog(FOG_PORTALVIEW, fogStart, fogEnd, fogColor[0],
                        fogColor[1], fogColor[2], 1.1);
          foginited = qtrue;
        }
      } else {
        if (!foginited) {
...
/*
        ...
        if( !cg.weaponSelect ) {
                if (cg_loadWeaponSelect.integer > 0) {
                        ...
                        trap_Cvar_Set( "cg_loadWeaponSelect", "0" );	// turn
   it off
                }
        }
*/
  ...
  if (cg.weaponSelect == WP_FG42SCOPE || cg.weaponSelect == WP_SNOOPERSCOPE ||
      cg.weaponSelect == WP_SNIPERRIFLE) {
    float spd;
  ...
  if (!cg.hyperspace) {
    CG_AddFlameChunks();
    CG_AddTrails(); // this must come last, so the trails dropped this frame get
                    // drawn
  }
  // done.
