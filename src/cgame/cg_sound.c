...
int CG_SoundScriptPrecache(const char *name) {
  ...
  while (sound) {
    if (!Q_strcasecmp(s, sound->name)) {
      ...
      } else /*if (cg_buildScript.integer)*/ { // RF, 11/6/01 enabled this
                                               // permanently so that streaming
                                               // sounds get touched within file
                                               // system on startup
        while (scriptSound) {
          // just open the file so it gets copied to the build dir
          fileHandle_t f;
          trap_FS_FOpenFile(scriptSound->filename, &f, FS_READ);
          // read a few bytes so the operating system does a better job of
          // caching it for us
...
==============
*/
void CG_SoundPickOldestRandomSound(soundScript_t *sound, vec3_t org,
                                   int entnum) {
  ...
  if (oldestSound) {
    ...
    if (!sound->streaming) {
      ...
      trap_S_StartStreamingSound(oldestSound->filename,
                                 sound->looping ? oldestSound->filename : NULL,
                                 entnum, sound->channel, sound->attenuation);
    }
    ...
    if (sound->shakeScale) {
      ...
      //
      // start the shaker
      CG_StartShakeCamera(sound->shakeScale, sound->shakeDuration, eOrg,
                          sound->shakeRadius);
    }
  } else {
...
qboolean CG_SoundPlaySoundScript(const char *name, vec3_t org, int entnum) {
  ...
  // CG_Printf( S_COLOR_RED "CG_SoundPlaySoundScript: cannot find sound script
  // '%s'\n", name );
  return qfalse;
}
...
static void CG_SoundLoadSoundFiles(void) {
  ...
  if (len <= 0) {
    CG_Printf(S_COLOR_RED "WARNING: no sound files found (filelist.txt not "
