...
//----(SA) added to make it easier to raise/lower our statsubar by only changing
//one thing
...
void CG_Text_Paint(float x, float y, int font, float scale, vec4_t color,
...
void CG_DrawFlagModel(float x, float y, float w, float h, int team) {
  ...
  CG_Draw3DModel(x, y, w, h,
                 team == TEAM_RED ? cgs.media.redFlagModel
                                  : cgs.media.blueFlagModel,
                 0, origin, angles);
}
...
static void CG_DrawStatusBarKeys() {
  ...
  // (SA) just don't draw this stuff for now.  It's got fog issues I need to
  // clean up
  ...
  for (i = 1; i < KEY_NUM_KEYS; i++) {
    ...
    if (cg.snap->ps.stats[STAT_KEYS] & (1 << gi->giTag)) {
      y += ICON_SIZE + 5;
      CG_DrawKeyModel(itemnum, 640 - (1.5 * ICON_SIZE), y, ICON_SIZE, ICON_SIZE,
                      cg.time + fadeColor[0] * 1000);
    }
  }
}
#endif // #ifdef OLDWOLFUI
...
static void CG_DrawStatusBar(void) {
  ...
  /*
          if ( cent->currentState.weapon && cg_weapons[
  cent->currentState.weapon ].ammoModel ) { origin[0] = 70; origin[1] = 0;
                  origin[2] = 0;
                  angles[YAW] = 90 + 20 * sin( cg.time / 1000.0 );;
  //----(SA) Wolf statusbar change
                  CG_Draw3DModel( CHAR_WIDTH*3 + TEXT_ICON_SPACE,
  STATUSBARHEIGHT -20, ICON_SIZE, ICON_SIZE, cg_weapons[
  cent->currentState.weapon ].ammoModel, 0, origin, angles );
  //----(SA) end
          }
  */
  ...
  //----(SA) Wolf statusbar change
  //		CG_Draw3DModel( 370 + CHAR_WIDTH*3 + TEXT_ICON_SPACE,
  STATUSBARHEIGHT -20, ICON_SIZE, ICON_SIZE,
  ...
  */
  ...
  if (cent->currentState.weapon) {
    ...
    if (value > -1) {
      if ((cg.predictedPlayerState.weaponstate == WEAPON_FIRING ||
           cg.predictedPlayerState.weaponstate == WEAPON_FIRINGALT) &&
      ...
      if (cg.predictedPlayerState.weapon != WP_KNIFE) {
        if (cgs.dmflags & DF_NO_WEAPRELOAD) {
          CG_DrawBigString2((580 - 23 + 35) + wideOffset, STATUSBARHEIGHT,
                            va("%d.", value), cg_hudAlpha.value);
        } else if (value) {
          CG_DrawBigString2((580 - 23 + 35) + wideOffset, STATUSBARHEIGHT,
                            va("%d/%d", inclip, value), cg_hudAlpha.value);
        } else {
          CG_DrawBigString2((580 - 23 + 35) + wideOffset, STATUSBARHEIGHT,
                            va("%d", inclip), cg_hudAlpha.value);
        }
      }

      icon = cg_weapons[cg.predictedPlayerState.weapon].weaponIcon[0];
      if (icon) {
        CG_DrawPic(((530 + 68) - halfScale) + wideOffset,
                   (446 - 10) - halfScale, (38 + scale) - wideOffset,
                   38 + scale, icon);
      }
      ...
      if (!cg_draw3dIcons.integer) {
        ...
        if (icon) {
          CG_DrawPic(CHAR_WIDTH * 3 + TEXT_ICON_SPACE, STATUSBARHEIGHT,
  ...
  if (value > 0) {
    ...
    trap_R_SetColor(NULL);
    //----(SA) Wolf statusbar change
    //		CG_DrawPic( 370 + CHAR_WIDTH*3 + TEXT_ICON_SPACE, STATUSBARHEIGHT,
    ...
    if (ci->infoValid && ci->team == cg.snap->ps.persistant[PERS_TEAM]) {
      ...
      for (j = 0; j < PW_NUM_POWERUPS; j++) {
        if (ci->powerups & (1 << j)) {
          ...
          if (item != NULL) { // JPW NERVE added for invulnerability powerup at
                              // beginning of map
            CG_DrawPic(xx, y, TINYCHAR_WIDTH, TINYCHAR_HEIGHT,
...
static float CG_DrawPowerups(float y) {
  ...
  float size;
  float f;
  static float colors[2][4] = {{0.2, 1.0, 0.2, 1.0}, {1.0, 0.2, 0.2, 1.0}};
...
#define CHATLOC_X 0
  ...
  if (cgs.teamLastChatPos != cgs.teamChatPos) {
    ...
    for (i = cgs.teamChatPos - 1; i >= cgs.teamLastChatPos; i--) {
      ...
      //i)*SMALLCHAR_HEIGHT, 				cgs.teamChatMsgs[i % TEAMCHAT_HEIGHT], 1.0F );
    }
  }
}
...
static void CG_DrawPickupItem(void) {
  ...
  if (value) {
    ...
    if (fadeColor) {
      ...
      //			CG_DrawPic( 8, 380, ICON_SIZE, ICON_SIZE,
      //cg_items[ value ].icon );
      //----(SA) end

      //----(SA)	so we don't pick up all sorts of items and have it print
      //"0 <itemname>"
      if (bg_itemlist[value].giType == IT_AMMO ||
          bg_itemlist[value].giType == IT_HEALTH ||
          bg_itemlist[value].giType == IT_POWERUP) {
        if (bg_itemlist[value].world_model[2]) { // this is a multi-stage item
          // FIXME: print the correct amount for multi-stage
          Com_sprintf(pickupText, sizeof(pickupText), "%s",
                      cgs.itemPrintNames[value]);
        } else {
          if (bg_itemlist[value].gameskillnumber[cg_gameSkill.integer] > 1) {
            Com_sprintf(
                pickupText, sizeof(pickupText), "%i  %s",
                bg_itemlist[value].gameskillnumber[cg_gameSkill.integer],
                cgs.itemPrintNames[value]);
          } else {
            Com_sprintf(pickupText, sizeof(pickupText), "%s",
                        cgs.itemPrintNames[value]);
      ...
      color[3] = fadeColor[0];
      CG_DrawStringExt2(ICON_SIZE + 16, 398, pickupText, color, qfalse, qtrue,
                        10, 10, 0);
      //			CG_Text_Paint(ICON_SIZE + 16, 398, 2, 0.3f,
      //color, pickupText, 0, 0, ITEM_TEXTSTYLE_SHADOWEDMORE);
...
/*
                if(cg.holdableSelect == HI_WINE) {
                        if(value > 3)
                                value = 3;	// 3 stages to icon, just draw
   full if beyond 'full'

                        CG_DrawPic( 598 + 16, 366, 16, 32, cg_items[item -
   bg_itemlist].icons[2-(value-1)] ); CG_DrawPic( (598 + 16)-4, 366-4, 24, 40,
   cgs.media.selectShader );

                } else {
                        CG_DrawPic( 598, 366, 32, 32, cg_items[item -
   bg_itemlist].icons[0] ); CG_DrawPic( 598-4, 366-4, 40, 40,
   cgs.media.selectShader );
                }
*/
...
void CG_CenterPrint(const char *str, int y, int charWidth) {
  ...
  Q_strncpyz(cg.centerPrint, CG_translateString((char *)str),
...
static void CG_DrawCenterString(void) {
  ...
  while (1) {
    ...
    //		CG_Text_Paint(x, y, 2, 0.3f, color, linebuffer, 0, 0,
...
static void CG_DrawWeapReticle(void) {
  ...
  if (weap == WP_SNIPERRIFLE) {
    ...
    CG_FillRect(310, 120, 20, 1,
                color); //					-----
    CG_FillRect(300, 160, 40, 1,
                color); //				-------------
    CG_FillRect(310, 200, 20, 1,
                color); //					-----

    CG_FillRect(140, 239, 360, 1, color); // horiz ---------------------------

    CG_FillRect(310, 280, 20, 1,
                color); //					-----
    CG_FillRect(300, 320, 40, 1,
                color); //				-------------
    CG_FillRect(310, 360, 20, 1,
                color); //					-----
...
static void CG_DrawCrosshair(void) {
  ...
  friendInSights = (qboolean)(cg.snap->ps.serverCursorHint ==
                              HINT_PLYR_FRIEND); //----(SA)	added
  ...
  case WP_KNIFE:
    ...
    if (cg.snap->ps.serverCursorHint >= HINT_EXIT &&
        cg.snap->ps.serverCursorHint <= HINT_NOEXIT_FAR) {
      return;
    }
  ...
  case WP_FG42SCOPE:
    ...
    // JPW NERVE -- don't let players run with rifles -- speed 80 == crouch, 128
    // == walk, 256 == run
  ...
  // mauser only gets crosshair if you don't have the scope (I don't like this,
  // but it's a test)
  ...
  if (!cg_drawCrosshair.integer) { //----(SA)	moved down so it doesn't keep
...
static void CG_ScanForCrosshairEntity(void) {
  ...
  VectorMA(start, 4096, cg.refdef.viewaxis[0],
...
void CG_CheckForCursorHints(void) {
  ...
  if (cg.snap->ps.serverCursorHint !=
      HINT_NONE) { // let the client remember what was last looked at (for
                   // fading out)
  ...
  // (SA) (8/14/01) removed all the client-side stuff.  don't think it's really
  // necessary anymore
}
...
static void CG_DrawCrosshairNames(void) {
  ...
  //	if ( cgs.clientinfo[ cg.crosshairClientNum ].team != cgs.clientinfo[
  //cg.clientNum ].team ) 		VectorSet( teamColor, 0.7608, 0.1250, 0.0859 );
  //// LIGHT-RED 	else
...
static void CG_DrawVote(void) {
  ...
  s = va("VOTE(%i):%s yes(F1):%i no(F2):%i", sec, cgs.voteString, cgs.voteYes,
         cgs.voteNo);
...
static qboolean CG_DrawFollow(void) {
  ...
  if (cg.snap->ps.pm_flags & PMF_LIMBO) {
    //		CG_Printf("following %s\n",cgs.clientinfo[ cg.snap->ps.clientNum
    //].name);
    color[1] = 0;
    color[2] = 0;
    if (cgs.clientinfo[cg.snap->ps.clientNum].team == TEAM_RED) {
      sprintf(deploytime, "Deploying in %d seconds",
              (int)((float)(cg_redlimbotime.integer -
                            (cg.time % cg_redlimbotime.integer)) *
                    0.001f));
    } else {
      sprintf(deploytime, "Deploying in %d seconds",
              (int)((float)(cg_bluelimbotime.integer -
                            (cg.time % cg_bluelimbotime.integer)) *
                    0.001f));
    }

    x = 0.5 * (640 - BIGCHAR_WIDTH *
                         strlen(deploytime)); // CG_DrawStrlen( deploytime ) );
    CG_DrawStringExt(x, 24, deploytime, color, qtrue, qtrue, BIGCHAR_WIDTH,
                     BIGCHAR_HEIGHT, 0);
    sprintf(deploytime, "(Following %s)",
            cgs.clientinfo[cg.snap->ps.clientNum].name);
    x = 0.5 * (640 - BIGCHAR_WIDTH *
                         strlen(deploytime)); // CG_DrawStrlen( deploytime ) );
    CG_DrawStringExt(x, 48, deploytime, color, qtrue, qtrue, BIGCHAR_WIDTH,
                     BIGCHAR_HEIGHT, 0);
    ...
    if (elapsed < 500 && elapsed > 0) {
      if (cgs.scrFadeAlphaCurrent > cgs.scrFadeAlpha) {
        cgs.scrFadeAlphaCurrent -=
            ((float)elapsed / (float)cgs.scrFadeDuration);
        if (cgs.scrFadeAlphaCurrent < cgs.scrFadeAlpha) {
          cgs.scrFadeAlphaCurrent = cgs.scrFadeAlpha;
        }
      } else {
...
static void CG_DrawFlashZoomTransition(void) {
  ...
  if (frac < fadeTime) {
    ...
    if (cg.weaponSelect == WP_SNOOPERSCOPE) {
      ...
      //); 			Vector4Set( color, 1, 0.5, 1, 1.0f - frac ); 			Vector4Set( color, 0.5f,
      //0.3f, 0.5f, 1.0f - frac );
...
static void CG_DrawFlashFire(void) {
  ...
  if (cg.cameraMode) { // don't draw flames on camera screen.  will still do
                       // damage though, so not a potential cheat
    return;
  }
  ...
  if (alpha > 0) {
    ...
    trap_S_AddLoopingSound(cg.snap->ps.clientNum, cg.snap->ps.origin,
                           vec3_origin, cgs.media.flameSound,
                           (int)(255.0 * alpha));
    trap_S_AddLoopingSound(cg.snap->ps.clientNum, cg.snap->ps.origin,
                           vec3_origin, cgs.media.flameCrackSound,
                           (int)(255.0 * alpha));
  } else {
    cg.v_noFireTime = cg.time;
  }
...
static void CG_DrawFlashLightning(void) {
  ...
  alpha =
      1.0 - (float)(cg.time - cent->pe.teslaDamagedTime) / LIGHTNING_FLASH_TIME;
  if (alpha > 0) {
    if (alpha >= 1.0) {
      alpha = 1.0;
    }
...
==============
*/
static void CG_DrawFlashBlendBehindHUD(void) { CG_DrawFlashZoomTransition(); }
...
void CG_StartShakeCamera(float p, int duration, vec3_t src, float radius) {
  ...
  for (i = 0; i < MAX_CAMERA_SHAKE; i++) {
    if (cg.cameraShake[i].time > cg.time ||
        cg.cameraShake[i].time + cg.cameraShake[i].length <= cg.time) {
      break;
    }
  }
...
void CG_CalcShakeCamera() {
  ...
  for (i = 0; i < MAX_CAMERA_SHAKE; i++) {
    if (cg.cameraShake[i].time <= cg.time &&
      ...
      // fade with distance
      val =
          cg.cameraShake[i].scale * (1.0f - (dist / cg.cameraShake[i].radius));
      // fade with time
      x = 1.0f -
          ((cg.time - cg.cameraShake[i].time) / cg.cameraShake[i].length);
      val *= x;
...
void CG_DrawActive(stereoFrame_t stereoView) {
  ...
  // clear around the rendered view if sized down
  CG_TileClear(); //----(SA)	moved to 2d section to avoid 2d/3d fog-state
                  //problems
