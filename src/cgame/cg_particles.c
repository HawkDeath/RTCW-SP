...
qboolean CG_ParticleLODCheck(void) {
  ...
  if (!(rand() %
        (cg_particleLOD.integer))) { // let particle lod thin out particles
    return qtrue;
  }
  ...
  if (p->type == P_WEATHER || p->type == P_WEATHER_TURBULENT ||
    ...
    if (p->type != P_WEATHER_FLURRY) {
      if (p->type == P_BUBBLE || p->type == P_BUBBLE_TURBULENT) {
        if (org[2] > p->end) {
          p->time = cg.time;
          VectorCopy(org, p->org); // Ridah, fixes rare snow flakes that flicker
                                   // on the ground
    ...
    // dot product removal  (gets you the dist^2, which you needed anyway, also
    // dot lets you adjust lod when zooming)
    if (1) {
      vec3_t dir;
      float dot, distSqrd;
    ...
    //		if ( p->type == P_SMOKE_IMPACT && Distance( cg.snap->ps.origin, org )
    //> 1024) { 			return;
    //		}
...
void CG_ParticleBubble(qhandle_t pshader, vec3_t origin, vec3_t origin2,
...
void CG_ParticleExplosion(char *animStr, vec3_t origin, vec3_t vel,
  ...
  if (animStr < (char *)10) {
    CG_Error("CG_ParticleExplosion: animStr is probably an index rather than a "
             "string");
  }
...
// Rafael Shrapnel
void CG_AddParticleShrapnel(localEntity_t *le) { return; }
// done.
...
void CG_ParticleImpactSmokePuffExtended(qhandle_t pshader, vec3_t origin,
                                        vec3_t dir, int radius, int lifetime,
...
void CG_ParticleImpactSmokePuff(qhandle_t pshader, vec3_t origin) {
  CG_ParticleImpactSmokePuffExtended(pshader, origin, tv(0, 0, 1), 8, 500, 20,
                                     20, 30, 0.25f);
}
...
void CG_Particle_OilParticle(qhandle_t pshader, vec3_t origin, vec3_t dir,
                             int ptime,
