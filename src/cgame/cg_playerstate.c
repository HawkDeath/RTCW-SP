...
void CG_Respawn(void) {
  ...
  cg.predictedPlayerState.weapAnimTimer =
      0; // allow other animations to happen right away
  cg.predictedPlayerState.weaponstate =
      WEAPON_RAISING; // hmm, set this?  what to?

  // display weapons available
  cg.weaponSelectTime = cg.time;

  cg.holdableSelectTime = 0; //----(SA) reset holdable timer
  cg.centerPrintTime = 0;    //----(SA)	reset centerprint counter so previous
  ...
  trap_Cvar_Set("cg_notebookpages",
                "3"); // (SA) TEMP: clear notebook pages on spawn (cept for page
                      // 1&2)  this is temporary
  trap_Cvar_Set("ui_notebookCurrentPage",
                "0"); // (SA) TEMP: clear notebook pages on spawn (cept for page
                      // 1)  this is temporary
