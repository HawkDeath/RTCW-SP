...
#define RF_THIRD_PERSON                                                        \
  2 // don't draw through eyes, only mirrors (player bodies, chat sprites)
#define RF_FIRST_PERSON                                                        \
  4 // only draw through eyes (view weapon, damage blood blob)
...
#define RF_HILIGHT                                                             \
  (1 << 8) // more than RF_MINLIGHT.  For when an object is "Highlighted"
...
#define RDF_DRAWSKYBOX                                                         \
  16 // the above marks a scene as being a 'portal sky'.  this flag says to draw
     // it or not
...
#define REFLAG_ZOMBIEFX2                                                       \
  4 // special post-tesselation processing for zombie skin
...
#define REFLAG_SCALEDSPHERECULL                                                \
  64 // on LOD switch, align the model to the player's camera
...
typedef enum {
  ...
  FOG_SKY,        //	1	fog values to apply to the sky when using density fog for
                  //the world (non-distance clipping fog) (only used
  ...
  //		The result of these for a given frame is copied to the scene.glFog
  //when the scene is rendered

  // the following are fogs applied to the main world scene
  FOG_MAP,     //	4	use fog parameter specified using the "fogvars" in the sky
               //shader
