...
#define BLOOD_SPURT_COUNT 4
  ...
  if (cg_reloading.integer) {
    return; // to dangerous, since we call playerangles() in here, which calls
            // the animation system, which might not be setup yet
  }
...
void CG_LaunchGib(centity_t *cent, vec3_t origin, vec3_t angles,
  ...
  case AICHAR_HELGA:
    le->endTime = le->startTime +
                  999000; // stay around for long enough to see the player off
  default:
  ...
  if (cent && CG_EntOnFire(cent)) {
    ...
    le->onFireEnd = re->fadeEndTime + 1000;
             ...
             IS_FLAMING_ZOMBIE(cent->currentState)) {
...
#define TORSOTAGSSTART 5 // where the 'split' happens in the above table
  ...
  for (gibIndex = 0, count = 0, foundtag = qtrue;
       foundtag && gibIndex < MAX_GIB_MODELS && gibTags[gibIndex]; gibIndex++) {
    ...
    // (SA) split this into torso and legs, since the tags are only attached to
    ...
    for (tagIndex = 0;
      ...
      if ((cent->currentState.aiChar == AICHAR_ZOMBIE) ||
        ...
        if ((cent->currentState.aiChar == AICHAR_HELGA) ||
          ...
          velocity[2] = GIB_JUMP + crandom() * GIB_VELOCITY;
          // additional gibs
          CG_LaunchGib(cent, origin, angles, velocity, ci->gibModels[gibIndex],
                       size, 1);
          CG_LaunchGib(cent, origin, angles, velocity, ci->gibModels[gibIndex],
                       size, 1);
        } else {
          CG_LaunchGib(cent, origin, angles, velocity, ci->gibModels[gibIndex],
                       size, 1 + (int)(2.0 * (size - 0.4)));
        }
      } else {
        CG_LaunchGib(cent, origin, angles, velocity, ci->gibModels[gibIndex],
                     1.0, 0);
          ...
          if (newjunction[j] == qtrue) {
            ...
            if ((cent->currentState.aiChar == AICHAR_ZOMBIE) ||
              ...
              if ((cent->currentState.aiChar == AICHAR_HELGA) ||
                ...
                velocity[1] = dir[1] * (0.5 + random()) * GIB_VELOCITY * 0.3;
                velocity[2] =
                    GIB_JUMP + dir[2] * (0.5 + random()) * GIB_VELOCITY * 0.5;
                VectorMA(velocity, GIB_VELOCITY, gdir, velocity);
              }
              ...
              if ((cent->currentState.aiChar == AICHAR_HELGA) ||
                ...
                CG_LaunchGib(cent, origin, angles, velocity,
                             ci->gibModels[rand() % 4], size, 1);
              } else {
                CG_LaunchGib(cent, origin, angles, velocity,
                             ci->gibModels[rand() % 4], size,
                             1 + (int)(2.0 * (size - 0.4)));
              }
            }
          }
  ...
  if (!count) {
    ...
    velocity[2] = GIB_JUMP + crandom() * GIB_VELOCITY;
    if (rand() & 1) {
      CG_LaunchGib(cent, origin, vec3_origin, velocity, cgs.media.gibSkull, 1.0,
                   0);
    } else {
      CG_LaunchGib(cent, origin, vec3_origin, velocity, cgs.media.gibBrain, 1.0,
                   0);
    }
    ...
    velocity[1] = crandom() * GIB_VELOCITY;
    velocity[2] = GIB_JUMP + crandom() * GIB_VELOCITY;
    CG_LaunchGib(cent, origin, vec3_origin, velocity, cgs.media.gibAbdomen, 1.0,
                 0);
    ...
    velocity[1] = crandom() * GIB_VELOCITY;
    velocity[2] = GIB_JUMP + crandom() * GIB_VELOCITY;
    CG_LaunchGib(cent, origin, vec3_origin, velocity, cgs.media.gibChest, 1.0,
                 0);
    ...
    velocity[1] = crandom() * GIB_VELOCITY;
    velocity[2] = GIB_JUMP + crandom() * GIB_VELOCITY;
    CG_LaunchGib(cent, origin, vec3_origin, velocity, cgs.media.gibFist, 1.0,
                 0);
    ...
    velocity[1] = crandom() * GIB_VELOCITY;
    velocity[2] = GIB_JUMP + crandom() * GIB_VELOCITY;
    CG_LaunchGib(cent, origin, vec3_origin, velocity, cgs.media.gibFoot, 1.0,
                 0);
    ...
    velocity[1] = crandom() * GIB_VELOCITY;
    velocity[2] = GIB_JUMP + crandom() * GIB_VELOCITY;
    CG_LaunchGib(cent, origin, vec3_origin, velocity, cgs.media.gibForearm, 1.0,
                 0);
    ...
    velocity[1] = crandom() * GIB_VELOCITY;
    velocity[2] = GIB_JUMP + crandom() * GIB_VELOCITY;
    CG_LaunchGib(cent, origin, vec3_origin, velocity, cgs.media.gibIntestine,
                 1.0, 0);
...
float lt_random(int thisrandseed, int t) {
  return (float)randtable[abs((thisrandseed + t +
...
float lt_crandom(int thisrandseed, int t) {
  return (
      (2.0 * ((float)randtable[abs((thisrandseed + t +
                                    (cg.time / LT_MS) * (cg.time / LT_MS))) %
                               NUMRANDTABLE] /
              (float)LT_RANDMAX)) -
      1.0);
...
#define VIEW_SCALE_DIST 128
  ...
  VectorCopy(
      pend,
      end); // need this so recursive calls don't override stacked endpoints
  ...
  for (i = 0; i < numBolts; i++) {
    ...
    //
    curJunc = CG_AddTrailJunc(0, shader, cg.time, STYPE_LIGHTNING, pos,
                              trailLife, 1, 1, thisWidth, thisWidth,
                              TJFL_NOCULL, c, c, stScale, 20.0);
    while (distLeft > 0) { // create this bolt
      ...
      // thisWidth *= alpha;
      if ((viewDist = VectorDistance(pos, cg.refdef.vieworg)) <
          VIEW_SCALE_DIST) {
        thisWidth *= 0.5 + (0.5 * (viewDist / VIEW_SCALE_DIST));
        if (thisWidth < 4.0 && thisWidth < maxWidth) {
          thisWidth = 4.0;
      ...
      curJunc = CG_AddTrailJunc(curJunc, shader, cg.time, STYPE_LIGHTNING, pos,
                                trailLife, 1, 1, thisWidth, thisWidth,
                                TJFL_NOCULL, c, c, stScale, 20.0);

      // fork from here?
      if (thisWidth < 4 && distLeft > 10 && recursion < 3 && forks < 3 &&
          lt_random(randseed, 383 + i + forks) < FORK_CHANCE) {
        vec3_t fend;
        forks++;
        VectorSet(fend, distLeft * 0.3 * lt_crandom(randseed, 56 + i + forks),
        ...
        VectorMA(pos, 0.2 + 0.7 * lt_random(randseed, 6 + i + forks), fend,
                 fend);

        // if (recursion > 0 && recursion < 2) {
        //	CG_DynamicLightningBolt( cgs.media.lightningBoltShader, pos,
        //fend, 1, maxWidth, qtrue, alpha, recursion, randseed ); 	return;
        //// divert bolt rather than split } else {
        CG_DynamicLightningBolt(shader, pos, fend, 1, maxWidth, qtrue, alpha,
                                recursion + 1, randseed + 765);
        //}
      }
...
void CG_ProjectedSpotLight(vec3_t start, vec3_t dir) {
  ...
  CG_ImpactMark(cgs.media.spotLightShader, tr.endpos, proj, 0, alpha, alpha,
                alpha, 1.0, qfalse, radius, qtrue, -2);
}
...
pointing at the camera
        SL_NOIMPACT			- don't draw the impact mark on hit
surfaces SL_LOCKUV			- lock the texture coordinates at the
'true' length of the requested beam.
...
void CG_Spotlight(centity_t *cent, float *color, vec3_t realstart,
                  vec3_t lightDir, int segs, float range, int startWidth,
  ...
  VectorSubtract(endCenter, endvec,
                 coreverts[3].xyz); // get a vector of the radius out at the end
                                    // for the core to use
...
#define CORESCALE 0.6f
  ...
  if (!(flags & SL_NOIMPACT)) {
    if (hitDist) {
      ...
      VectorMA(tr.endpos, -0.5f * radius, lightDir,
               impactPos); // back away a little from the hit

      CG_ImpactMark(cgs.media.spotLightShader, impactPos, proj, 0, colorNorm[0],
                    colorNorm[1], colorNorm[2], 0.3f, qfalse, radius, qtrue,
                    -1);
      //			CG_ImpactMark( cgs.media.spotLightShader,
      //tr.endpos, proj, 0, colorNorm[0], colorNorm[1], colorNorm[2], 1.0f,
      //qfalse, radius, qtrue, -1 );
    }
  }
  ...
  if (!(flags & SL_NODLIGHT)) {
    vec3_t dlightLoc;
    //		VectorMA(tr.endpos, -60, lightDir, dlightLoc);	// back away from the
    //hit
    VectorMA(tr.endpos, 0, lightDir, dlightLoc); // back away from the hit
    //		trap_R_AddLightToScene(dlightLoc, 200, colorNorm[0], colorNorm[1],
    //colorNorm[2], 0);	// ,REF_JUNIOR_DLIGHT);
    //		trap_R_AddLightToScene(dlightLoc, radius*2, colorNorm[0],
    //colorNorm[1], colorNorm[2], 0);	// ,REF_JUNIOR_DLIGHT);
    trap_R_AddLightToScene(dlightLoc, radius * 2, 0.3, 0.3, 0.3,
                           0); // ,REF_JUNIOR_DLIGHT);
  }
...
#define FLAREANGLE 35
  ...
  if (!(flags & SL_NOFLARE)) {
    ...
    if (deg <= FLAREANGLE) { // start flare a bit before the camera gets inside
                             // the cylinder
      lightInEyes = qtrue;
    ...
    if (lightInEyes) {
      ...
      trap_R_AddCoronaToScene(start, colorNorm[0], colorNorm[1], colorNorm[2],
                              coronasize, cent->currentState.number,
                              3); // 1&2 ('visible' & 'spotlightflare')
    } else {
      // even though it's off, still need to add it, but turned off so it can
      // fade in/out properly
      trap_R_AddCoronaToScene(start, colorNorm[0], colorNorm[1], colorNorm[2],
                              0, cent->currentState.number,
                              2); // 0&2 ('not visible' & 'spotlightflare')
    }
  }
}
