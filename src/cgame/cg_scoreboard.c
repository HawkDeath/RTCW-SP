...
=================
*/
static void CG_DrawClientScore(int x, int y, score_t *score, float *color,
                               float fade) {
  char string[1024];
  vec3_t headAngles;
  clientInfo_t *ci;
  ...
  CG_DrawHead(x - ICON_SIZE, y - (ICON_SIZE - BIGCHAR_HEIGHT) / 2, ICON_SIZE,
              ICON_SIZE, score->client, headAngles);

  if (ci->powerups & (1 << PW_REDFLAG)) {
    CG_DrawFlagModel(x - ICON_SIZE - ICON_SIZE / 2,
                     y - (ICON_SIZE - BIGCHAR_HEIGHT) / 2, ICON_SIZE, ICON_SIZE,
                     TEAM_RED);
  ...
  if (score->ping == -1) {
    ...
    Com_sprintf(string, sizeof(string), "SPECT %4i %4i %s", score->ping,
                score->time, ci->name);
  } else {
...
static void WM_DrawClientScore(int x, int y, score_t *score, float *color,
                               float fade) {
  float tempx;
  vec4_t hcolor;
  clientInfo_t *ci;
...
static int WM_TeamScoreboard(int x, int y, team_t team, float fade) {
  ...
  CG_FillRect(tempx, y, INFO_PLAYER_WIDTH - INFO_BORDER, INFO_TEAM_HEIGHT,
              hcolor);
  if (team == TEAM_RED) {
  ...
  CG_FillRect(tempx, y, INFO_SCORE_WIDTH - INFO_BORDER, INFO_TEAM_HEIGHT,
              hcolor);
  tempx += INFO_SCORE_WIDTH;

  CG_FillRect(tempx, y, INFO_LATENCY_WIDTH - INFO_BORDER, INFO_TEAM_HEIGHT,
              hcolor);
  tempx += INFO_LATENCY_WIDTH;
...
int WM_DrawObjectives(int x, int y, int width, float fade) {
  ...
  if (buf && atoi(buf)) {
    ...
    for (i = 0; i < num; i++) {
      ...
      if (status == 0) {
        CG_DrawPic(x + width / 2 - strwidth / 2 - 16 - 24, y, 24, 16,
                   trap_R_RegisterShaderNoMip("ui/assets/ger_flag.tga"));
        CG_DrawPic(x + width / 2 + strwidth / 2 - 12 + 4, y, 24, 16,
                   trap_R_RegisterShaderNoMip("ui/assets/ger_flag.tga"));
      } else if (status == 1) {
        CG_DrawPic(x + width / 2 - strwidth / 2 - 16 - 24, y, 24, 16,
                   trap_R_RegisterShaderNoMip("ui/assets/usa_flag.tga"));
        CG_DrawPic(x + width / 2 + strwidth / 2 - 12 + 4, y, 24, 16,
                   trap_R_RegisterShaderNoMip("ui/assets/usa_flag.tga"));
      }
...
int WM_ScoreboardOverlay(int x, int y, float fade) {
  ...
  // JPW NERVE -- mission time & reinforce time
    ...
    s = va("Mission time:   %2.0f:%i%i", (float)mins, tens,
  ...
  // jpw
  //	CG_DrawSmallString( x - 12 + 5, y, "Wolfenstein Multiplayer", fade ); //
  //old one
...
qboolean CG_DrawScoreboard(void) {
  ...
  if (cgs.gametype != GT_SINGLE_PLAYER &&
      cgs.gametype != GT_WOLF) { // NERVE - SMF - added wolf multiplayer check
    if (cg.snap->ps.persistant[PERS_TEAM] != TEAM_SPECTATOR) {
