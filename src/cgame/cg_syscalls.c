...
void trap_Error(const char *fmt) { syscall(CG_ERROR, fmt); }
...
void trap_S_AddLoopingSound(int entityNum, const vec3_t origin,
                            ...
                            int volume) {
  syscall(CG_S_ADDLOOPINGSOUND, entityNum, origin, velocity, 1250, sfx,
          volume); // volume was previously removed from CG_S_ADDLOOPINGSOUND.
                   // I added 'range'
}
...
void trap_S_AddRangedLoopingSound(int entityNum, const vec3_t origin,
                                  const vec3_t velocity, sfxHandle_t sfx,
                                  int range) {
  syscall(CG_S_ADDLOOPINGSOUND, entityNum, origin, velocity, range, sfx,
          255); // RF, assume full volume, since thats how it worked before
}
//----(SA)	end
...
void trap_S_AddRealLoopingSound(int entityNum, const vec3_t origin,
                                const vec3_t velocity, sfxHandle_t sfx) {
  // not in use
  //	syscall( CG_S_ADDREALLOOPINGSOUND, entityNum, origin, velocity, 1250,
  //sfx, 255 );	//----(SA)	modified
}
...
void trap_R_AddLightToScene(const vec3_t org, float intensity, float r, float g,
  ...
  syscall(CG_R_ADDLIGHTTOSCENE, org, PASSFLOAT(intensity), PASSFLOAT(r),
          PASSFLOAT(g), PASSFLOAT(b), overdraw);
}
...
void trap_GetRenderconfig(renderconfig_t *renderconfig) {
  syscall(CG_GETRENDERCONFIG, renderconfig);
}
...
void trap_SetUserCmdValue(int stateValue, int holdableValue,
                          float sensitivityScale,
                          int cld) { //----(SA)	// NERVE - SMF - added cld
  syscall(CG_SETUSERCMDVALUE, stateValue, holdableValue,
          PASSFLOAT(sensitivityScale), cld);
}
...
//----(SA)	added
void trap_stopCamera(int camNum) { syscall(CG_STOPCAMERA, camNum); }
//----(SA)	end
...
void trap_UI_Popup(const char *arg0) { syscall(CG_INGAME_POPUP, arg0); }

// NERVE - SMF
void trap_UI_LimboChat(const char *arg0) { syscall(CG_LIMBOCHAT, arg0); }
