...
static int maxWeapBanks = MAX_WEAP_BANKS,
           maxWeapsInBank = MAX_WEAPS_IN_BANK; // JPW NERVE
...
// JPW NERVE -- in mutiplayer, characters get knife/special on button 1, pistols
// on 2, 2-handed on 3
int weapBanksMultiPlayer[MAX_WEAP_BANKS_MP][MAX_WEAPS_IN_BANK_MP] = {
    ...
    {WP_MP40, WP_THOMPSON, WP_STEN, WP_MAUSER, WP_GARAND, WP_PANZERFAUST,
     WP_VENOM, WP_FLAMETHROWER},
    {
        WP_GRENADE_LAUNCHER,
...
static void CG_MachineGunEjectBrass(centity_t *cent) {
  ...
  if (cg.snap->ps.persistant[PERS_HWEAPON_USE]) {
    ...
    offset[1] = -4;
    offset[2] = 0;
  } else if (cg.predictedPlayerState.weapon == WP_MP40 ||
             cg.predictedPlayerState.weapon == WP_THOMPSON) {
...
void CG_PyroSmokeTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  if (grounddir ==
      99) { // pick a wind direction -- cheap trick because it can be different
    grounddir = crandom(); // on different clients, but it's all smoke and
                           // mirrors anyway
  }
  step = 30;
  es = &ent->currentState;
  ...
  */
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    //			CG_ParticleExplosion( "expblue", lastPos, vec3_origin, 100 +
    //(int)(rnd*400), 4, 4 );	// fire "flare"
...
static void CG_GrenadeTrail(centity_t *ent, const weaponInfo_t *wi) {
  ...
  for (; t <= ent->trailTime; t += step) {
    ...
    ent->headJuncIndex =
        CG_AddSmokeJunc(ent->headJuncIndex, cgs.media.smokeTrailShader, origin,
                        //												1500,
                        //0.3, 10, 50 );
                        1000, 0.3, 2, 20);
    ent->lastTrailTime = cg.time;
    ...
    if (newfmt) {
      ...
      if (atoi(token)) {
        wi->weapAnimations[i].moveSpeed |=
            (1 << W_MAX_PARTS); // set the bit one higher than can be set by the
                                // barrel bits
      }
      token = COM_Parse(
...
void CG_RegisterWeapon(int weaponNum) {
  ...
  weaponInfo->weaponModel[W_TP_MODEL] =
      trap_R_RegisterModel(item->world_model[W_TP_MODEL]);
  weaponInfo->weaponModel[W_FP_MODEL] =
      trap_R_RegisterModel(item->world_model[W_FP_MODEL]);
  weaponInfo->weaponModel[W_SKTP_MODEL] =
      trap_R_RegisterModel(item->world_model[W_SKTP_MODEL]);
  ...
  if (!weaponInfo->weaponModel[W_TP_MODEL]) {
    // left commented out since we have level-loading optimization issues to
    // still resolve.
    //	ie. every weapon and it's associated effects/parts/sounds etc. are
    //loaded for every level.
    // This was turned off when we started (the "only load what the level calls
    // for" thing) because when DM does a "give all" and fires, he doesn't want
    // to wait for everything to load.  So perhaps a "cacheallweaps" or
    // something.
//		CG_Printf( "Couldn't register weapon model %i (unable to load view
//model)", weaponNum );
// RF, I need to be able to run the game, I dont have the silencer weapon (19)
#ifndef _DEBUG
//		CG_Error( "Couldn't register weapon model %i (unable to load view
//model)", weaponNum );
#endif
    return;
  }
  ...
  if (item->world_model[W_FP_MODEL]) {
    ...
    if (!CG_ParseWeaponConfig(va("%sweapon.cfg", path), weaponInfo)) {
      CG_Error("Couldn't register weapon %i (%s) (failed to parse weapon.cfg)",
               weaponNum, path);
    }
  }
  //----(SA)	end
  ...
  weaponInfo->weaponIcon[1] = trap_R_RegisterShader(
      va("%s_select", item->icon)); // get the 'selected' icon as well
  ...
  for (ammo = bg_itemlist + 1; ammo->classname; ammo++) {
    if ((ammo->giType == IT_AMMO &&
         ammo->giTag == BG_FindAmmoForWeapon(weaponNum))) {
      break;
    }
  }
  ...
  if (item->world_model[W_FP_MODEL]) {
    strcpy(comppath,
           item->world_model[W_FP_MODEL]); // first try the fp view weap
  } else if (item->world_model[W_TP_MODEL]) {
    strcpy(
        comppath,
        item->world_model[W_TP_MODEL]); // not there, use the standard view hand
  }
  if ((!comppath ||
       !cg_drawFPGun.integer) && // then if it didn't find the 1st person one or
                                 // you are set to not use one
      item->world_model[W_TP_MODEL]) {
    strcpy(comppath,
           item->world_model[W_TP_MODEL]); // use the standard view hand
  }
  ...
  case WP_AKIMBO: //----(SA)	added
    ...
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/colt/coltf1.wav");
    weaponInfo->flashEchoSound[0] = trap_S_RegisterSound(
        "sound/weapons/mp40/mp40e1.wav"); // use same as mp40
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;

    // unique
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/colt/colt_reload2.wav");
    break;
  ...
  case WP_COLT:
    ...
    weaponInfo->flashEchoSound[0] = trap_S_RegisterSound(
        "sound/weapons/mp40/mp40e1.wav"); // use same as mp40
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/colt/colt_reload.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;

  case WP_KNIFE:
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/knife/knife_slash1.wav");
    weaponInfo->flashSound[1] =
        trap_S_RegisterSound("sound/weapons/knife/knife_slash2.wav");
    break;

  case WP_LUGER:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);

    weaponInfo->switchSound[0] = trap_S_RegisterSound(
        "sound/weapons/luger/silencerremove.wav"); //----(SA)	added

    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/luger/lugerf1.wav");
    weaponInfo->flashEchoSound[0] = trap_S_RegisterSound(
        "sound/weapons/mp40/mp40e1.wav"); // use same as mp40
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/luger/luger_reload.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_SILENCER: // luger mod
    ...
    weaponInfo->switchSound[0] = trap_S_RegisterSound(
        "sound/weapons/luger/silencerattatch.wav"); //----(SA)	added

    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/luger/silencerf1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/luger/luger_reload.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_MAUSER:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/mauser/mauserf1.wav");
    weaponInfo->flashEchoSound[0] =
        trap_S_RegisterSound("sound/weapons/mauser/mausere1.wav");
    weaponInfo->lastShotSound[0] =
        trap_S_RegisterSound("sound/weapons/mauser/mauserf1_last.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/mauser/mauser_reload.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  case WP_SNIPERRIFLE:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/mauser/sniperf1.wav");
    weaponInfo->flashEchoSound[0] =
        trap_S_RegisterSound("sound/weapons/mauser/mausere1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/mauser/sniper_reload.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_GARAND:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/garand/garandf1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/garand/garand_reload.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  case WP_SNOOPERSCOPE:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/garand/snooperf1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/garand/snooper_reload.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_THOMPSON:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/thompson/thompson.wav");
    weaponInfo->flashEchoSound[0] = trap_S_RegisterSound(
        "sound/weapons/mp40/mp40e1.wav"); // use same as mp40
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/thompson/thompson_reload.wav");
    weaponInfo->overheatSound =
        trap_S_RegisterSound("sound/weapons/thompson/thompson_overheat.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_MP40:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/mp40/mp40f1.wav");
    weaponInfo->flashEchoSound[0] =
        trap_S_RegisterSound("sound/weapons/mp40/mp40e1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/mp40/mp40_reload.wav");
    weaponInfo->overheatSound =
        trap_S_RegisterSound("sound/weapons/mp40/mp40_overheat.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_STEN:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/sten/stenf1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/sten/sten_reload.wav");
    weaponInfo->overheatSound =
        trap_S_RegisterSound("sound/weapons/sten/sten_overheat.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_FG42SCOPE:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/fg42/fg42f1.wav");
    weaponInfo->flashEchoSound[0] =
        trap_S_RegisterSound("sound/weapons/fg42/fg42e1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/fg42/fg42_reload.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_PANZERFAUST:
    ...
    weaponInfo->missileSound =
        trap_S_RegisterSound("sound/weapons/rocket/rockfly.wav");
    ...
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/rocket/rocklf1a.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/rocket/rocklf_reload.wav");
    cgs.media.rocketExplosionShader = trap_R_RegisterShader("rocketExplosion");
    break;

  case WP_MORTAR:
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/mortar/mortarf1.wav");
  ...
  case WP_GRENADE_SMOKE:
    weaponInfo->missileModel =
        trap_R_RegisterModel("models/weapons2/grenade/pineapple.md3");
  ...
  case WP_GRENADE_PINEAPPLE:
    ...
    MAKERGB(weaponInfo->flashDlightColor, 1, 0.7, 0.5);
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/grenade/grenlf1a.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/grenade/grenlf_reload.wav");
  ...
  case WP_DYNAMITE:
    ...
    //"sound/weapons/grenade/grenlf1a.wav" ); 		weaponInfo->reloadSound =
    //trap_S_RegisterSound( "sound/weapons/grenade/grenlf_reload.wav" );
    cgs.media.grenadeExplosionShader =
        trap_R_RegisterShader("grenadeExplosion");
    break;
  ...
  case WP_VENOM:
    MAKERGB(weaponInfo->flashDlightColor, 1.0, 0.6, 0.23);
    weaponInfo->spinupSound = trap_S_RegisterSound(
        "sound/weapons/venom/venomsu1.wav"); //----(SA)	added
    weaponInfo->spindownSound = trap_S_RegisterSound(
        "sound/weapons/venom/venomsd1.wav"); //----(SA)	added
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/venom/venomf1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/venom/venom_reload.wav");
    weaponInfo->overheatSound =
        trap_S_RegisterSound("sound/weapons/venom/venom_overheat.wav");
    weaponInfo->ejectBrassFunc = CG_MachineGunEjectBrass;
    break;
  ...
  case WP_TESLA:
    ...
    weaponInfo->flashSound[0] =
        trap_S_RegisterSound("sound/weapons/tesla/teslaf1.wav");
    weaponInfo->reloadSound =
        trap_S_RegisterSound("sound/weapons/tesla/tesla_reload.wav");
    weaponInfo->overheatSound =
        trap_S_RegisterSound("sound/weapons/tesla/tesla_overheat.wav");
    break;
...
void CG_RegisterItemVisuals(int itemNum) {
  ...
  wolfkickModel =
      trap_R_RegisterModel("models/weapons2/foot/v_wolfoot_10f.md3");
  ...
  propellerModel =
      trap_R_RegisterModel("models/mapobjects/vehicles/m109_prop.md3");
  ...
  if (cg_gameType.integer != GT_WOLF) {
    ...
    CG_RegisterWeapon(
        WP_GRENADE_SMOKE); // register WP_CLASS_SPECIAL visuals here
    CG_RegisterWeapon(WP_MEDIC_HEAL);
    maxWeapBanks = MAX_WEAP_BANKS_MP;
    maxWeapsInBank = MAX_WEAPS_IN_BANK_MP;
  }
  // if player runs out of SMG ammunition, it shouldn't *also* deplete pistol
  // ammunition.  If you change this, change g_spawn.c as well
  ...
  //		if ( item->world_model[W_FP_MODEL] ) {
  //			itemInfo->models[W_FP_MODEL] = trap_R_RegisterModel(
  //item->world_model[W_FP_MODEL] );
  //		}
  //	}
}
...
===============
*/
static void CG_SetWeapLerpFrameAnimation(weaponInfo_t *wi, lerpFrame_t *lf,
                                         int newAnimation) {
  animation_t *anim;
...
===============
*/
void CG_ClearWeapLerpFrame(weaponInfo_t *wi, lerpFrame_t *lf,
                           int animationNumber) {
...
===============
*/
static void CG_RunWeapLerpFrame(clientInfo_t *ci, weaponInfo_t *wi,
  ...
  // RF, if the animation number is the same, but we are using a different
  // weapon, we need to reset the lf->animation
  // else if ( memcmp( &wi->weapAnimations[newAnimation&~ANIM_TOGGLEBIT],
  // lf->animation, sizeof(*lf->animation) ) ) { 	CG_ClearWeapLerpFrame(wi, lf,
  //newAnimation );	// clear when switching to raise (since it should be out
  //of view anyway)
  //}
...
static void CG_WeaponAnimation(playerState_t *ps, weaponInfo_t *weapon,
                               int *weapOld, int *weap, float *weapBackLerp) {
...
static void CG_FlamethrowerFlame(centity_t *cent, vec3_t origin) {
  ...
  if (cent->currentState.number == cg.snap->ps.clientNum) {
    if (cg.snap->ps.weapon != WP_FLAMETHROWER ||
  ...
  //		CG_FireFlameChunks( cent, origin, cent->lerpAngles, 650.0 /
  //FLAMETHROWER_RANGE, qtrue, 0 );	// fixed length for AI 	else
...
static float	CG_MachinegunSpinAngle( centity_t *cent ) {
        ...
        if ( cent->pe.barrelSpinning ) {
                ...
                speed = 0.5 * ( SPIN_SPEED + (float)( COAST_TIME - delta ) /
COAST_TIME ); angle = cent->pe.barrelAngle + delta * speed;
        }

        if ( cent->pe.barrelSpinning == !(cent->currentState.eFlags & EF_FIRING)
) { cent->pe.barrelTime = cg.time; cent->pe.barrelAngle = AngleMod( angle );
                cent->pe.barrelSpinning = !!(cent->currentState.eFlags &
EF_FIRING);
        }
...
static float CG_TeslaSpinAngle(centity_t *cent) {
  ...
  if (cent->pe.barrelSpinning) {
    ...
    speed = 0.5 * (TESLA_SPINSPEED +
                   (float)(TESLA_COASTTIME - delta) / TESLA_COASTTIME);
...
#define VENOM_DELTATIME                                                        \
  (VENOM_LOADTIME / 10) // as there are 10 shots to be loaded
...
static void CG_AddWeaponWithPowerups(refEntity_t *gun, int powerups,
  ...
  //			alpha = 2.0 * (float)(FIRE_FLASH_TIME - (cg.time -
  cg.snap->ps.onFireStart))/FIRE_FLASH_TIME; alpha = 1; if (alpha > 0) { if
  (alpha >= 1.0) { alpha = 1.0;
                                  }
                                  gun->shaderRGBA[3] = (unsigned
  char)(255.0*alpha);
                                  // calc the fireRiseDir from the velocity
                                  VectorNegate( cg.snap->ps.velocity,
  gun->fireRiseDir ); VectorNormalize( gun->fireRiseDir ); gun->fireRiseDir[2]
  += 1; if (VectorNormalize( gun->fireRiseDir ) < 1) { VectorClear(
  gun->fireRiseDir ); gun->fireRiseDir[2] = 1;
                                  }
                                  // now move towards the newDir
                                  adjust = 5.0*(0.001*cg.frametime);
                                  VectorMA( cg.v_fireRiseDir, adjust,
  gun->fireRiseDir, cg.v_fireRiseDir ); if (VectorNormalize( cg.v_fireRiseDir )
  <= 0.1) { VectorCopy( gun->fireRiseDir, cg.v_fireRiseDir );
                                  }
...
CG_PlayerTeslaCoilFire
  ...
  TODO: this needs to be fixed for multiplay. entities being hurt need to be
sent by server to all clients, so they draw the correct effects.
==============
*/
void CG_PlayerTeslaCoilFire(centity_t *cent, vec3_t flashorigin) {

#define TESLA_LIGHTNING_POINT_TIMEOUT 3000
#define TESLA_LIGHTNING_MAX_DIST                                               \
  (cent->currentState.aiChar == AICHAR_SUPERSOLDIER                            \
       ? TESLA_SUPERSOLDIER_RANGE                                              \
       : TESLA_RANGE) // use these to perhaps vary the distance according to
                      // aiming
...
#define TESLA_MAX_POINT_TESTS_PERFRAME 20
  ...
  // first, build a list of visible enemies that can be hurt by this tesla, then
  // filter by distance
  if (!cent->pe.teslaDamageApplyTime ||
      cent->pe.teslaDamageApplyTime < cg.time - 200) {
    numEnemies = 0;
    ...
    if ((cent != &cg_entities[cg.snap->ps.clientNum]) &&
      ...
      CG_Trace(&tr, traceOrg, NULL, NULL, cg.snap->ps.origin,
               cg.snap->ps.clientNum, MASK_SHOT & ~(CONTENTS_BODY));
      if (tr.fraction == 1 || tr.entityNum == cg.snap->ps.clientNum) {
        ...
        CG_Trace(&tr, tagPos, NULL, NULL, vec, cg.snap->ps.clientNum,
    ...
    if (cgs.localServer && cgs.gametype == GT_SINGLE_PLAYER) {
      ...
      for (ctrav = cg_entities, i = 0; i < cgs.maxclients && numEnemies < 16;
           ctrav++, i++) {
        ...
        */                                                                                                                              //
        if (ctrav->currentState.aiChar && (ctrav != cent) &&
          ...
          if (DotProduct(viewDir, vec) > 0.8) {
            CG_Trace(&tr, traceOrg, NULL, NULL, ctrav->lerpOrigin,
                     ctrav->currentState.number, MASK_SHOT & ~CONTENTS_BODY);
            if (tr.fraction == 1 ||
                tr.entityNum == ctrav->currentState.number) {
    ...
    for (i = 0; i < MAX_TESLA_BOLTS; i++) {
      if (numSorted &&
          i / numSorted < 1 /*(MAX_TESLA_BOLTS/3)*/) { // bolts per enemy
        j = i % numSorted;
        cent->pe.teslaEnemy[i] = visEnemiesSorted[j];
        // apply damage
        CG_ClientDamage(visEnemiesSorted[j], cent->currentState.number,
                        CLDMG_TESLA);
        // show the effect
  ...
  for (i = 0; i < numPoints; i++) {
    ...
    if (cent->pe.teslaEnemy[i] >= 0) {
      ...
      VectorSet(testPos, 6 * crandom(), 6 * crandom(), 20 * crandom() - 8);
      // VectorClear( testPos );
      if (cent->pe.teslaEnemy[i] != cg.snap->ps.clientNum) {
        VectorAdd(testPos, cg_entities[cent->pe.teslaEnemy[i]].lerpOrigin,
                  testPos);
      } else {
        VectorAdd(testPos, cg.snap->ps.origin, testPos);
      }
      cent->pe.teslaEndPointTimes[i] =
          cg.time; // - rand()%(TESLA_LIGHTNING_POINT_TIMEOUT/2);
      VectorCopy(testPos, cent->pe.teslaEndPoints[i]);
    } else if ((!cent->pe.teslaEndPointTimes[i]) ||
               (cent->pe.teslaEndPointTimes[i] > cg.time) ||
               (cent->pe.teslaEndPointTimes[i] <
                cg.time - TESLA_LIGHTNING_POINT_TIMEOUT) ||
      ...
      for (j = 0; j < TESLA_MAX_POINT_TESTS; j++) {
        ...
        CG_Trace(&tr, tagPos, NULL, NULL, testPos, cent->currentState.number,
                 MASK_SHOT & ~CONTENTS_BODY);
        if (tr.fraction < 1 && tr.entityNum == ENTITYNUM_WORLD &&
            !(tr.surfaceFlags & (SURF_NOIMPACT | SURF_SKY))) {
          // found a valid spot!
          cent->pe.teslaEndPointTimes[i] =
              cg.time - rand() % (TESLA_LIGHTNING_POINT_TIMEOUT / 2);
      ...
      VectorNormalize(vec);
      VectorInverse(vec);
      CG_ImpactMark(cgs.media.lightningHitWallShader,
                    cent->pe.teslaEndPoints[i], vec, random() * 360, 0.2, 0.2,
                    0.2, 1.0, qtrue, 4, qfalse, 300);
    }
    //
    ...
    if (VectorDistance(tagPos, cent->pe.teslaEndPoints[i]) <= maxDist) {
      CG_DynamicLightningBolt(cgs.media.lightningBoltShader, tagPos,
                              cent->pe.teslaEndPoints[i],
                              1 + ((cg.time % ((i + 2) * (i + 3))) + i) % 2,
                              20 + (float)(i % 3) * 5 + 6.0 * random(),
                              (cent->pe.teslaEnemy[i] < 0), 1.0, 0, i * i * 3);

      // play a zap sound
      if (cent->pe.lightningSoundTime < cg.time - 200) {
        CG_SoundPlayIndexedScript(cgs.media.teslaZapScript,
                                  cent->pe.teslaEndPoints[i], ENTITYNUM_WORLD);
        CG_SoundPlayIndexedScript(cgs.media.teslaZapScript, cent->lerpOrigin,
                                  ENTITYNUM_WORLD);
        // trap_S_StartSound( cent->pe.teslaEndPoints[i], ENTITYNUM_WORLD,
        // CHAN_AUTO, cgs.media.lightningSounds[rand()%3] );
  ...
  if (cg.time % 3) { // break it up a bit
    ...
    trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                           vec3_origin, cgs.media.teslaLoopSound, 255);
  }
...
==============
*/
void CG_AddProtoWeapons(refEntity_t *parent, playerState_t *ps,
                        centity_t *cent) {
  refEntity_t gun;
...
qboolean CG_MonsterUsingWeapon(centity_t *cent, int aiChar, int weaponNum) {
  return (cent->currentState.aiChar == aiChar) &&
...
void CG_AddPlayerWeapon(refEntity_t *parent, playerState_t *ps,
  ...
  if (weaponNum == WP_SNOOPERSCOPE || weaponNum == WP_SNIPERRIFLE ||
      weaponNum == WP_FG42SCOPE || weapSelect == WP_SNOOPERSCOPE ||
      weapSelect == WP_SNIPERRIFLE || weapSelect == WP_FG42SCOPE) {
  ...
  if (isPlayer) {
    akimboFire = BG_AkimboFireSequence(
        weaponNum, cg.predictedPlayerState.ammoclip[WP_AKIMBO],
        cg.predictedPlayerState.ammoclip[WP_COLT]);
  } else if (ps) {
    akimboFire = BG_AkimboFireSequence(weaponNum, ps->ammoclip[WP_AKIMBO],
                                       ps->ammoclip[WP_AKIMBO]);
  }
  ...
  if (ps) {
    ...
    // skeletal guys use a different third person weapon (for different tag
    // business)
    if (cgs.clientinfo[cent->currentState.clientNum].isSkeletal &&
        weapon->weaponModel[W_SKTP_MODEL]) {
  ...
  if (weaponNum ==
      WP_GAUNTLET) { // (SA) this is the 'knife'.  no model yet, so we can give
                     // it to the zombie and have him visually 'unarmed'
    if (debuggingweapon) {
  ...
  if (!ps) {
    ...
    if ((cent->currentState.eFlags & EF_FIRING) && weapon->firingSound) {
      ...
      // down
      trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                             vec3_origin, weapon->firingSound, 255);
      cent->pe.lightningFiring = qtrue;
    } else if (weapon->readySound) {
      trap_S_AddLoopingSound(cent->currentState.number, cent->lerpOrigin,
                             vec3_origin, weapon->readySound, 255);
    }
  }
  ...
  playerScaled = (qboolean)(
      cgs.clientinfo[cent->currentState.clientNum].playermodelScale[0] != 0);
  if (!ps && playerScaled) { // don't "un-scale" weap up in 1st person
    for (i = 0; i < 3;
         i++) { // scale weapon back up so it doesn't pick up the adjusted scale
                // of the character models. this will affect any parts attached
                // to the gun as well (barrel/bolt/flash/brass/etc.)
      VectorScale(gun.axis[i],
                  1.0 / (cgs.clientinfo[cent->currentState.clientNum]
                             .playermodelScale[i]),
                  gun.axis[i]);
    }
  }

  // characters that draw their own special weapon model will not draw the
  // standard ones
  ...
  if (ps) {
    ...
    for (i = W_PART_1; i < W_MAX_PARTS; i++) {
      ...
      if (weaponNum == WP_VENOM) {
        ...
        // (SA) not right now.  at the moment, just spin the belt when firing,
        // no swapout
        else if (i == W_PART_3) {
          if ((cent->pe.weap.animationNumber & ~ANIM_TOGGLEBIT) ==
              WEAP_ATTACK1) {
            //						barrel.hModel =
            //weapon->wpPartModels[W_FP_MODEL_SWAP][i];
            barrel.hModel = weapon->wpPartModels[W_FP_MODEL][i];
            angles[ROLL] = -CG_VenomSpinAngle(cent);
      ...
      if (barrel.hModel) {
        if (i == W_PART_1) {
          ...
          if (spunpart) {
            CG_PositionRotatedEntityOnTag(&barrel, parent,
                                          va("tag_barrel%d", i + 1));
          } else {
            CG_PositionEntityOnTag(&barrel, parent, va("tag_barrel%d", i + 1),
                                   0, NULL);
          }
        }
...
#define BARREL_SMOKE_TIME 1000
        ...
        if (!cg_paused.integer) { // don't add while paused
          ...
          if (cg.time - cent->overheatTime < 3000) {
            if (!(rand() % 3)) {
              ...
              if (weaponNum ==
                  WP_VENOM) { // silly thing that makes the smoke off the venom
                              // swirlier since it's spinning real fast
                CG_ParticleImpactSmokePuffExtended(
                    cgs.media.smokeParticleShader, flash.origin, tv(0, 0, 1), 8,
                    1000, 8, 20, 70, alpha);
              } else {
                CG_ParticleImpactSmokePuffExtended(
                    cgs.media.smokeParticleShader, flash.origin, tv(0, 0, 1), 8,
  ...
  if (flash.hModel) {
    if (weaponNum != WP_FLAMETHROWER &&
        weaponNum != WP_TESLA) { // Ridah, hide the flash also for now
    ...
    if (firing) {
      ...
      if (weapon->flashDlightColor[0] || weapon->flashDlightColor[1] ||
        ...
        trap_R_AddLightToScene(
            flash.origin, 200 + (rand() & 31), weapon->flashDlightColor[0],
...
void CG_AddPlayerFoot(refEntity_t *parent, playerState_t *ps, centity_t *cent) {
  ...
  if (cg_gun_x.value) {
    VectorMA(wolfkick.origin, cg_gun_x.value, cg.refdef.viewaxis[0],
             wolfkick.origin);
  }
  if (cg_gun_y.value) {
    VectorMA(wolfkick.origin, cg_gun_y.value, cg.refdef.viewaxis[1],
             wolfkick.origin);
  }
  if (cg_gun_z.value) {
    VectorMA(wolfkick.origin, cg_gun_z.value, cg.refdef.viewaxis[2],
             wolfkick.origin);
  }
  //----(SA)	end
...
void CG_AddViewWeapon(playerState_t *ps) {
  ...
  if (ps->weapon > WP_NONE) {
    ...
    VectorMA(hand.origin, (gunoff[2] + fovOffset), cg.refdef.viewaxis[2],
             hand.origin);
  ...
  for (i = 0; i < maxWeapsInBank; i++) {
    ...
    if (drawweap && (bits[0] & (1 << drawweap))) {
      ...
      if (wideweap) {
        // weapon icon
        if (realweap == curweap) {
          CG_DrawPic(x, y, WP_ICON_X_WIDE, WP_ICON_Y,
                     cg_weapons[drawweap].weaponIcon[1]);
        } else {
          CG_DrawPic(x, y, WP_ICON_X_WIDE, WP_ICON_Y,
                     cg_weapons[drawweap].weaponIcon[0]);
        }
        ...
        // weapon icon
        if (realweap == curweap) {
          CG_DrawPic(x, y, WP_ICON_X, WP_ICON_Y,
                     cg_weapons[drawweap].weaponIcon[1]);
        } else {
          CG_DrawPic(x, y, WP_ICON_X, WP_ICON_Y,
                     cg_weapons[drawweap].weaponIcon[0]);
    ...
    // clear drawweap if getaltweap() returns the same weap as passed in. (no
    // secondary available)
    // JPW NERVE
    if (cg_gameType.integer == GT_WOLF) {
    ...
    if (drawweap && (bits[0] & (1 << drawweap))) {
      ...
      if (realweap == cg.weaponSelect) {
        CG_DrawPic(x, y, WP_ICON_SEC_X, WP_ICON_SEC_Y,
                   cg_weapons[drawweap].weaponIcon[1]);
      } else {
        CG_DrawPic(x, y, WP_ICON_SEC_X, WP_ICON_SEC_Y,
                   cg_weapons[drawweap].weaponIcon[0]);
...
static qboolean CG_WeaponSelectable(int i) {
  ...
  // allow switch out of scope for weapons where you fired the last shot while
  // scoped and we left you in that view to see the result of the shot
...
(maxWeapsInBank), this will safely loop around
==============
*/
...
will loop around to the top (maxWeapsInBank-1) and start down from there looking
for a valid weapon position
==============
*/
...
cycle,
        return that	(colt returns thompson for example) rather than the
lowest weapon
==============
*/
...
cycle,
        return that	(thompson returns colt for example) rather than the
highest weapon
==============
*/
...
void CG_WeaponSuggest(int weap) {
  ...
  cg.lastWeapSelInBank[bank] =
      weap; // make this weap first priority in that bank
}
...
void CG_AltWeapon_f(void) {
  ...
  if (CG_WeaponSelectable(num)) { // new weapon is valid
    ...
    //		(until you remove it)
    // TODO: will need to make sure the table gets initialized properly on
    // restart/death/whatever.
    //		 I still think I'm going to make the weapon banks stored in the
    //config, so this will 		just be a matter of resetting the banks to what's in
    //the config.
    switch (original) {
    case WP_LUGER:
      if (cg.snap->ps.eFlags &
          EF_MELEE_ACTIVE) { // if you're holding a chair, you can't screw on
                             // the silencer
        return;
      }
    ...
    case WP_SILENCER:
      if (cg.snap->ps.eFlags &
          EF_MELEE_ACTIVE) { // if you're holding a chair, you can't remove the
                             // silencer
        return;
      }
  ...
  if (1) {
    for (i = 0; i < maxWeapsInBank; i++) {
      ...
      CG_WeaponIndex(num, NULL,
                     &newcycle); // get cycle of new weapon.  if it's lower than
                                 // the original, then it cycled around
...
void CG_NextWeapon_f(void) {
  ...
  //				present, but would discard the reload.  So the when you
  //switched 				back you'd have to start the reload over.  This seems bad, however
  //				the delay for the current reload is already in effect, so
  //you'd lose 				the reload time twice.  (the first pause for the current weapon
  //reload, 				and the pause when you have to reload again 'cause you canceled this
  //one)
...
void CG_PrevWeapon_f(void) {
  ...
  /*
    int			bank = 0, cycle = 0, newbank = 0, newcycle = 0;
    qboolean	prevbank = qfalse;	// need to switch to the next bank of
    weapons?
  */
...
void CG_Weapon_f(void) {
  ...
  CG_WeaponIndex(curweap, &bank, &cycle); // get bank/cycle of current weapon
  banked = CG_WeaponIndex(num, &newbank,
  ...
  if (bank != newbank) {
    cycle = newcycle -
            1; //	drop down one from the requested weap's cycle so it will
  }
...
void CG_MortarEFX(centity_t *cent) {
  ...
  if (cent->currentState.density & 2) {
    // light
    trap_R_AddLightToScene(cent->currentState.origin, 200 + (rand() & 31), 1.0,
                           1.0, 1.0, 0);
  ...
  if (c > 0) {
    ...
    if (firesound[c]) {
      ...
      if (fireEchosound && fireEchosound[c]) { // check for echo
        ...
        if (gdist > 512 && gdist < 4096) { // temp dist.  TODO: use numbers that
                                           // are weapon specific
          // use gorg as the new sound origin
          VectorMA(cg.refdef.vieworg, 64, norm, gorg); // sound-on-a-stick
          trap_S_StartSoundEx(gorg, ent->number, CHAN_WEAPON, fireEchosound[c],
                              SND_NOCUT);
        }
      }
    }
...
=================
*/
void CG_AddSparks(vec3_t origin, vec3_t dir, int speed, int duration, int count,
                  float randScale) {
  localEntity_t *le;
  refEntity_t *re;
  vec3_t velocity;
...
=================
*/
void CG_AddBulletParticles(vec3_t origin, vec3_t dir, int speed, int duration,
...
void CG_AddDirtBulletParticles(vec3_t origin, vec3_t dir, int speed,
  ...
  CG_ParticleDirtBulletDebris_Core(
      ...
      shadername); // 600 + rand()%300 ); // keep central one
  for (i = 0; i < count; i++) {
    ...
    CG_ParticleDirtBulletDebris_Core(
        pos, velocity,
        duration + (rand() % (duration >> 1)), // dur * 0.5, but int
...
void CG_AddDebris(vec3_t origin, vec3_t dir, int speed, int duration,
  ...
  for (i = 0; i < count; i++) {
    ...
    VectorSet(unitvel, dir[0] + crandom() * 0.9, dir[1] + crandom() * 0.9,
              fabs(dir[2]) > 0.5 ? dir[2] * (0.2 + 0.8 * random())
                                 : random() * 0.6);
...
==============
*/
void CG_WaterRipple(qhandle_t shader, vec3_t loc, vec3_t dir, int size,
                    int lifetime) {
  localEntity_t *le;
  refEntity_t *re;
...
void CG_MissileHitWall(
  ...
  switch (weapon) {
  case WP_KNIFE:
    sfx = cgs.media.sfx_knifehit[4]; // different values for different types
  ...
  case WP_VENOM:
    ...
    if (weapon == WP_FG42SCOPE || weapon == WP_SNIPERRIFLE ||
        weapon == WP_SNOOPERSCOPE ||
        (Distance(cg.refdef.vieworg, origin) < 384)) {
      ...
      if (clientNum) {
        ...
        alphaFade = qtrue; // max made the bullet mark alpha (he'll make
                           // everything in the game out of 1024 textures, all
                           // with alpha blend funcs yet...)
        ...
        if (surfFlags & SURF_METAL) {
          ...
          start);
                                                          ...
                                                          VectorCopy
          (muzzleTrace, dir); VectorNegate (dir, dir);

                                                          G_SetOrigin (sfx,
          start); G_SetAngle (sfx, dir);

                                                          G_AddEvent( sfx,
          EV_SHARD, DirToByte( dir ));

                                                          sfx->think =
          G_FreeEntity; sfx->nextthink = level.time + 1000;
  ...
  case WP_GRENADE_PINEAPPLE:
    ...
    /*
                    r = 2 + rand()%3;
                    for (i=0; i<3; i++) {
                            for (j=0;j<3;j++) sprOrg[j] = origin[j] + 14*dir[j]
       + 14*crandom(); CG_ParticleExplosion( 3, sprOrg, sprVel, 800+rand()%250,
       9, 60+rand()%200 );
                    }
    */
  ...
  case WP_PANZERFAUST:
    ...
    if (cg_gameType.integer == GT_SINGLE_PLAYER) {
      ...
      // NOTE: these must all have the same duration, so that we are less likely
      // to use a wider range of images per scene
      r = 2 + rand() % 3;
      for (i = 0; i < 4; i++) {
        if (weapon == VERYBIGEXPLOSION) {
          for (j = 0; j < 3; j++)
            sprOrg[j] = origin[j] + 32 * dir[j] + 32 * crandom();
          CG_ParticleExplosion("explode1", sprOrg, sprVel, 1200, 40,
                               160 + rand() % 120);
        } else if (i < 2) {
          for (j = 0; j < 3; j++)
            sprOrg[j] = origin[j] + 24 * dir[j] + 16 * crandom();
          CG_ParticleExplosion("explode1", sprOrg, sprVel, 1400, 15,
                               40 + rand() % 30);
        }
      }
    ...
    // JPW NERVE -- multiplayer explosions over the top due to large damage
    // radiusesesizes
    else {
...
void CG_MissileHitWallSmall(int weapon, int clientNum, vec3_t origin,
  ...
  // CG_ImpactMark( mark, origin, dir, random()*360, 1,1,1,1, alphaFade, radius,
  // qfalse, 60000 );
  CG_ImpactMark(mark, origin, dir, random() * 360, 1, 1, 1, 1, alphaFade,
                radius, qfalse, 0xffffffff);
...
void CG_MissileHitPlayer(centity_t *cent, int weapon, vec3_t origin, vec3_t dir,
  ...
  case WP_KNIFE:
    ...
    if (cgs.media.sfx_knifehit[i]) {
      trap_S_StartSound(origin, cent->currentState.number, CHAN_WEAPON,
                        cgs.media.sfx_knifehit[i]);
    }
  ...
  case WP_GRENADE_LAUNCHER:
  case WP_PANZERFAUST:
    // this shake is /on top/ of the shake from the impact (done in
    // CG_MissileHitWall)
...
hit splashes (FIXME: random seed isn't synced anymore)
  ...
  (SA)	right now this is random like a shotgun.  I want to make it more
                organized so that the pattern is more of a circle (with some
...
void CG_VenomFire(entityState_t *es, qboolean fullmode) {
  ...
  if (cgs.glconfig.hardwareType != GLHW_RAGEPRO) {
    ...
    if (!(contents & CONTENTS_WATER)) {
      VectorSet(up, 0, 0, 32);
      if (fullmode) {
        CG_SmokePuff(v, up, 24, 1, 1, 1, 0.33, 1200, cg.time, 0, 0,
                     cgs.media.shotgunSmokePuffShader); // LEF_PUFF_DONT_SCALE
      }
      //----(SA)	for the time being don't do the single shot smoke as
      //it's position is funky 			else 				CG_SmokePuff( v, up, 4, 1, 1, 1, 0.33, 700,
      //cg.time, 0, cgs.media.shotgunSmokePuffShader );
    }
  }
  if (fullmode) {
...
void CG_SpawnTracer(int sourceEnt, vec3_t pstart, vec3_t pend) {
  ...
  if (sourceEnt < cgs.maxclients) {
    ...
    if (cg_entities[sourceEnt].currentState.eFlags &
        EF_MG42_ACTIVE) { // mounted
      start[2] -= 32;     // (SA) hack to get the tracer down below the barrel
                          // FIXME: do properly
    } else {
...
static qboolean CG_CalcMuzzlePoint(int entityNum, vec3_t muzzle) {
  ...
  //	if ( anim == LEGS_WALKCR || anim == LEGS_IDLECR || anim  ==
  //LEGS_IDLE_ALT ) { 		muzzle[2] += CROUCH_VIEWHEIGHT; 	} else {
...
void CG_Bullet(vec3_t end, int sourceEntityNum, vec3_t normal, qboolean flesh,
               int fleshEntityNum, qboolean wolfkick, int otherEntNum2) {
  trace_t trace;
  ...
  if (sourceEntityNum >= 0 && cg_tracerChance.value > 0) {
    if (CG_CalcMuzzlePoint(sourceEntityNum, start)) {
      ...
      // bubble trail from air into water
        ...
        //				CG_Printf( "Dist: %f\n",
        //Distance(cg.refdef.vieworg, end) ); 				CG_AddDirtBulletParticles( end,
        //dir, 							190,	// speed 							900,	// duration 							5,	// count 							0.5, 80, 16,
        //0.125, "water_splash" );	// rand scale
  ...
  if (flesh) {
    ...
    if (fleshEntityNum == cg.snap->ps.clientNum) {
      ...
      // (SA) TODO: for metal guys, make metal a flag rather than an aitype
      // check?
      if (aiType == AICHAR_PROTOSOLDIER || aiType == AICHAR_SUPERSOLDIER) {
        CG_SoundPlayIndexedScript(cgs.media.bulletHitFleshMetalScript, NULL,
                                  fleshEntityNum);
      } else {
      ...
      VectorSubtract(cg_entities[fleshEntityNum].lerpOrigin, cg.snap->ps.origin,
                     origin);
      VectorMA(cg.snap->ps.origin, 3, origin, origin);
      if (aiType == AICHAR_PROTOSOLDIER || aiType == AICHAR_SUPERSOLDIER) {
        CG_SoundPlayIndexedScript(cgs.media.bulletHitFleshMetalScript, origin,
                                  ENTITYNUM_WORLD);
      } else {
        CG_SoundPlayIndexedScript(cgs.media.bulletHitFleshScript, origin,
                                  ENTITYNUM_WORLD);
      }
    }
    ...
    if (cent->currentState.aiChar == AICHAR_ZOMBIE) {
            ...
            // lower
            trap_RB_ZombieFXAddNewHit( cent->currentState.number | (1<<30), end,
    dir ); return;
    }
    */
    ...
    if (!fromweap || cg_entities[sourceEntityNum].currentState.eFlags &
                         EF_MG42_ACTIVE) { // mounted
      fromweap = WP_MP40;
    }
    ...
    // moved this up to (what seems like) the proper area.  trails above. didn't
    // always have valid start/end positions here.
    ...
    if (CG_CalcMuzzlePoint(sourceEntityNum, start) ||
      ...
      VectorNormalize(dir);
      VectorMA(end, -4, dir,
               start2); // back off a little so it doesn't start in solid
      VectorMA(end, 64, dir, dir);
      trap_CM_BoxTrace(&trace, start2, dir, NULL, NULL, 0, MASK_SHOT);

      if ((trace.surfaceFlags & SURF_METAL) || !(rand() % 10) ||
          (otherEntNum2 != ENTITYNUM_NONE)) {
        // JPW NERVE compute new spark direction from normal & dir (rotate -dir
        // 180 degrees about normal)

        // (SA) NOTE: isn't this done by the server and sent along in the
        // 'normal' that's passed into this routine? (1107 g_weapon.c)

        VectorScale(dir, -1.0f, tmp);
        RotatePointAroundVector(tmp, normal, tmp, 180.0f);
        CG_MissileHitWall(
            fromweap, 0, end, tmp,
            trace.surfaceFlags); // sparks	//	(SA) modified to send
                                 // missilehitwall surface parameters
                                 // jpw
        //				CG_MissileHitWall( fromweap, 0, end, normal,
        //trace.surfaceFlags );	// sparks	//	(SA) modified to send
        //missilehitwall surface parameters
      }
      if (!(sourceContentType & CONTENTS_WATER) &&
          (destContentType &
           (CONTENTS_WATER | CONTENTS_SLIME |
            CONTENTS_LAVA))) { // only when shooting /into/ water
        trap_CM_BoxTrace(&trace, start, end, NULL, NULL, 0, MASK_WATER);
        //				CG_Trace(&trace, start, NULL, NULL, end, -1,
        //MASK_WATER);
        //				if (!(trace.surfaceFlags & SURF_NOMARKS)) {
        //// check to see if the surface should draw splashes
        CG_MissileHitWall(fromweap, 2, trace.endpos, trace.plane.normal,
                          trace.surfaceFlags);
        //				}
      } else {
        CG_MissileHitWall(
            fromweap, 1, end, normal,
            trace.surfaceFlags); // smoke puff	//	(SA) modified to send
                                 // missilehitwall surface parameters
...
void CG_ClientDamage(int entnum, int enemynum, int id) {
  ...
  // NERVE - SMF - clientDamage commands are now sent through usercmds for
  // multiplayer
