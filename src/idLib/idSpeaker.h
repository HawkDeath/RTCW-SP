...
/*
 * idSpeaker is the container class for all sound stuff.  It has static members
 * that allow you to start, stop, and so forth, and private members that will
 * not be exposed to the public API.  This class will start a thread that plays
 * independently of the game API.
 *
...
public:
  ...
  bool activate();
  bool loop(const char *intro, const char *filename, int channel, bool looping,
            bool in3D, bool useNotification);
  ...
  void setEntityPosition(const int entityNum, const float x, const float y,
                         const float z) {
    if (entityNum >= 0 && entityNum < MAX_GENTITIES) {
  ...
  void setEntityVelocity(const int entityNum, const float x, const float y,
                         const float z) {
    if (entityNum >= 0 && entityNum < MAX_GENTITIES) {
  ...
  idVec3 &getListenerPosition() { return listener; }
  ...
  void bind(const char *name, idAudioBuffer *nis) { hashTable.Set(name, nis); }
  void unbind(const char *name) { hashTable.Remove(name); }
