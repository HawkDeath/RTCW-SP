...
dword WINAPI NotificationProc(LPVOID lpParameter) {
  ...
  while (!bDone) {
    dwResult =
        MsgWaitForMultipleObjects(MAX_STREAMING_SOUNDS, g_hNotificationEvent,
                                  false, INFINITE, QS_ALLEVENTS);
    dwResult -= WAIT_OBJECT_0;

    if (dwResult < MAX_STREAMING_SOUNDS) {
      EnterCriticalSection(&g_crit);
      if (g_pStreamingSound[dwResult] &&
          g_pStreamingSound[dwResult]->IsSoundPlaying()) {
...
idSpeaker::idSpeaker() {
  ...
  g_hNotifyThread = CreateThread(NULL, 0, NotificationProc, g_wv.hWnd, 0,
                                 &g_dwNotifyThreadID);
...
bool idSpeaker::loop(const char *intro, const char *filename, int channel,
                     bool looping, bool in3d, bool useNotification) {
  HRESULT hr;
  idWavefile waveFile;
  dword dwNotifySize;
  int flags;
  ...
  if (SUCCEEDED(hr = waveFile.Open(intro, NULL, WAVEFILE_READ))) {
    if (waveFile.GetSize() != 0 && waveFile.m_pwfx != NULL) {
      ...
      // piece and when a notification arrives then it fills the circular
      // streaming buffer with new wav data over the sound data which was just
      // played

      // Determine the g_dwNotifySize.  It should be an integer multiple of
      // nBlockAlign
      ...
      // that we are notified as the sound buffer plays.  Note, that using this
      // flag may limit the amount of hardware acceleration that can occur.
      if (SUCCEEDED(hr = hw->CreateStreaming(
                        &g_pStreamingSound[channel], intro, filename, cflags,
...
void idSpeaker::process() {
  ...
  pDSListener->SetOrientation(listenerFront.x, listenerFront.y, listenerFront.z,
                              listenerTop.x, listenerTop.y, listenerTop.z,
                              DS3D_DEFERRED);
...
int idSpeaker::getAudioHardwareChannel() { return 0; }
