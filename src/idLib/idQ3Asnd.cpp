...
void S_Init(void) { idSpeak = new idSpeaker(); }

void S_Shutdown(void) { delete idSpeak; }
...
void S_StartSoundEx(vec3_t origin, int entnum, int entchannel, sfxHandle_t sfx,
  ...
  if (entnum >= 0) {
    for (i = 1; i < MAX_STREAMING_SOUNDS; i++) { // track 0 is music/cinematics
      ...
      // the same channel
      if ((entchannel != CHAN_AUTO) && (g_pStreamingSound[i]->entnum[0] >= 0) &&
          (g_pStreamingSound[i]->entchannel[0] == entchannel) &&
          (g_pStreamingSound[i]->entnum[0] == entnum)) {
...
void S_StopBackgroundTrack(void) { g_pStreamingSound[0]->Stop(0); }
...
void S_StartStreamingSound(const char *intro, const char *loop, int entnum,
  ...
  if (entnum >= 0) {
    for (i = 1; i < MAX_STREAMING_SOUNDS; i++) { // track 0 is music/cinematics
      ...
      // check to see if this character currently has another sound streaming on
      // the same channel
      if ((channel != CHAN_AUTO) && (g_pStreamingSound[i]->entnum[0] >= 0) &&
          (g_pStreamingSound[i]->entchannel[0] == channel) &&
          (g_pStreamingSound[i]->entnum[0] == entnum)) {
        // found a match, override this channel
        idSpeak->stopLoop(i);
  ...
  if (idSpeak->loop(intro, loop, ss, looping, in3D, false)) {
    if (g_pStreamingSound[ss] != NULL) {
      ...
      g_pStreamingSound[ss]->SetPosition(
          0, pos.x, pos.y, pos.z); // streaming sound is always channel 0
      g_pStreamingSound[ss]->Play(0, 0, looping ? DSBPLAY_LOOPING : 0);
      g_pStreamingSound[ss]->SetRange(0, idSpeak->getMinDistance(),
                                      idSpeak->getMaxDistance() * 4);
    }
  }
}
...
void S_AddLoopingSound(int entityNum, const vec3_t origin,
  ...
  idSpeak->setEntityPosition(entityNum, origin[0] * QUAKE_TO_DS3D,
                             origin[1] * QUAKE_TO_DS3D,
                             origin[2] * QUAKE_TO_DS3D);
  idSpeak->setEntityVelocity(entityNum,
                             velocity[0] * QUAKE_TO_DS3D * (1.0f / 20.0f),
                             velocity[1] * QUAKE_TO_DS3D * (1.0f / 20.0f),
                             velocity[2] * QUAKE_TO_DS3D * (1.0f / 20.0f));
  ...
  loopSounds[numLoopSounds].velocity.x =
      velocity[0] * QUAKE_TO_DS3D * (1.0f / 20.0f);
  loopSounds[numLoopSounds].velocity.y =
      velocity[1] * QUAKE_TO_DS3D * (1.0f / 20.0f);
  loopSounds[numLoopSounds].velocity.z =
      velocity[2] * QUAKE_TO_DS3D * (1.0f / 20.0f);
...
void S_Respatialize(int entityNum, const vec3_t origin, vec3_t axis[3],
  ...
  idSpeak->setListenerPosition(origin[0] * QUAKE_TO_DS3D,
                               origin[1] * QUAKE_TO_DS3D,
                               origin[2] * QUAKE_TO_DS3D);
    ...
    if (loopSounds[i].sfx) {
      ...
      for (j = 0; j < oldNumLoopSounds; j++) {
        if (indexList[j].sfx == iab) {
          ...
          iab->SetPosition(index, loopSounds[i].origin.x,
                           loopSounds[i].origin.y, loopSounds[i].origin.z);
          iab->SetVelocity(index, loopSounds[i].velocity.x,
                           loopSounds[i].velocity.y, loopSounds[i].velocity.z);
...
sfxHandle_t S_RegisterSound(const char *sample) {
  ...
  if ((iab = idSpeak->find(sample))) {
    ...
    hr = idSpeak->hw->Create(&iab, sample, DSBCAPS_CTRL3D | DSBCAPS_LOCHARDWARE,
                             DS3DALG_DEFAULT);
    if (hr != E_FAIL) {
      // Get the 3D buffer from the secondary buffer
      iab->Get3DBufferInterfaces(idSpeak->getMinDistance(),
                                 idSpeak->getMaxDistance());
      if (!initEAX) {
        initEAX = true;
