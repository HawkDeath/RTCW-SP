...
/*
 * idSound is the class that actually stores a sound.  It also has a limited
 * public API that allows you to signal a sound from the game code or an entity.
 * We need to use signals since the sound code runs in it's own thread and the
 * entity class doesn't need to become thread safe just for the sound.
 *
 * to register a sound use from any thread, you can specify as many flags as you
 * like:
 *
 * handle = idSound::register( "coolfile.wav", idSound::LOOPING,
 * idSound::DOPPLER );
 *
 ...
 * renderer so they can be, for the most part, updated without bothering the
 * game thread.
 *
 */
