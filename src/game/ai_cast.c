...
Some modifications to the botlib and botai are to be expected, the extent of
those changes is currently unknown.
...
// string versions of the attributes used for per-level, per-character
// definitions
char *castAttributeStrings[] = {
    ...
    "ATTACK_SKILL",    // max = 1.0	(ability to attack and do other things, like
                       // retreat)
    "REACTION_TIME",   // max = 1.0	(upon seeing enemy, wait this long
                       // before reaction)
    ...
    "TACTICAL",        // max = 1.0	(ability to use strategy to their advantage,
                // also behaviour whilst hunting enemy, more likely to creep
                // around)
...
gentity_t *AICast_AddCastToGame(gentity_t *ent, char *castname, char *model,
  ...
  //----(SA) ClientConnect requires a third 'isbot' parameter.  setting to
  //qfalse and noting
...
void AICast_SetAASIndex(cast_state_t *cs) {
  if (aiDefaults[cs->aiCharacter].bboxType == BBOX_SMALL) {
    ...
    Com_Error(ERR_DROP, "AICast_SetAASIndex: unsupported bounds size (%i)",
              aiDefaults[cs->aiCharacter].bboxType);
  }
...
gentity_t *AICast_CreateCharacter(gentity_t *ent, float *attributes,
  ...
  newent =
      AICast_AddCastToGame(ent, castname, model, head, sex, color, handicap);
  ...
  for (j = 0; j < 3; j++) {
    cs->viewangles[j] = AngleMod(
        newent->s.angles[j] - SHORT2ANGLE(newent->client->ps.delta_angles[j]));
  }
  ...
  cs->aiFlags |=
      AIFL_CORPSESIGHTING; // this is on by default for all characters, disabled
                           // if they have a "friendlysightcorpse" script event
  ...
  // clear weapons/ammo
  client->ps.weapon = 0;
  memcpy(client->ps.weapons, weaponInfo->startingWeapons,
         sizeof(weaponInfo->startingWeapons));
  memcpy(client->ps.ammo, weaponInfo->startingAmmo, sizeof(client->ps.ammo));
  //
  // starting health
  if (ent->health) {
    newent->health = client->ps.stats[STAT_HEALTH] =
        client->ps.stats[STAT_MAX_HEALTH] = ent->health;
  } else {
    newent->health = client->ps.stats[STAT_HEALTH] =
        client->ps.stats[STAT_MAX_HEALTH] = cs->attributes[STARTING_HEALTH];
  }
  //
  ...
  //----(SA)	make sure all clips are loaded so we don't hear everyone loading
  //up 			(we don't want to do this inside AICast_UpdateBattleInventory(), only on
  //spawn or giveweapon)
...
AICast_Init
  ...
  called at each level start, before the world and it's entities have been
spawned
============
*/
...
void AICast_Init(void) {
  ...
  // (aicast_thinktime / sv_fps) * aicast_maxthink = number of cast's to think
  // between each aicast frame so.. (100 / 20) * 6 = 30
  //
  // so if the level has more than 30 AI cast's, they could start to bunch up,
  // resulting in slower thinks
  ...
  aicast_skillscale =
      (float)trap_Cvar_VariableIntegerValue("g_gameSkill") / (float)GSKILL_MAX;
  ...
  /* RF, this is useless, since the AAS hasnt been loaded yet
     ...
     spawning of entities i = 0; trap_AAS_SetCurrentWorld(0); while
     (!trap_AAS_Initialized() && (i++ < 10)) { trap_BotLibStartFrame((float)
     level.time / 1000);
          }
  */
}
...
void AIChar_AIScript_AlertEntity(gentity_t *ent) {
  ...
  if (numTouch) {
    for (i = 0; i < numTouch; i++) {
      // RF, note we should only check against clients since zombies need to
      // spawn inside func_explosive (so they dont clip into view after it
      // explodes)
      if (g_entities[touch[i]].client &&
          g_entities[touch[i]].r.contents == CONTENTS_BODY) {
  ...
  // RF, has to disable this so I could test some maps which have erroneously
  // placed alertentity calls
...
void AICast_DelayedSpawnCast(gentity_t *ent, int castType) {
  ...
  // we have to wait a bit before spawning it, otherwise the server will just
  // delete it, since it's treated like a client
  ent->think = AIChar_spawn;
  ent->nextthink =
      level.time +
      FRAMETIME * 4; // have to wait more than 3 frames, since the server runs 3
                     // frames before it clears all clients

  // we don't really want to start this character right away, but if we don't
  // spawn the client now, if the game gets saved after the character spawns in,
  // when it gets re-loaded, the client won't get spawned properly.
  if (ent->spawnflags & 1) { // TriggerSpawn
    ent->AIScript_AlertEntity = AIChar_AIScript_AlertEntity;
    ent->aiInactive = qtrue;
  }

  // RF, had to move this down since some dev maps don't properly spawn the guys
  // in, so we get a crash when transitioning between levels after they all
  // spawn at once (overloading the client/server command buffers)
  ent->nextthink +=
      FRAMETIME * ((numSpawningCast + 1) /
                   3); // space them out a bit so we don't overflow the client
...
void AICast_CastScriptThink(void) {
  ...
  for (i = 0, ent = g_entities, cs = caststates; i < level.maxclients;
       i++, ent++, cs++) {
    if (!ent->inuse) {
      continue;
    }
...
void AICast_CheckLoadGame(void) {
  ...
  if (strlen(loading) > 0 && atoi(loading) != 0) {
    ...
    if (ready) {
      ...
      // RF, disabled, since the pregame menu turns this off after the button is
      // pressed, this isn't required here RF, spawn a thinker that will enable
      // rendering after the client has had time to process the entities and
      // setup the display
      // trap_Cvar_Set( "cg_norender", "0" );
      // ent = G_Spawn();
      // ent->nextthink = level.time + 200;
      ...
      // wait for the clients to return from faded screen
      //			trap_SetConfigstring( CS_SCREENFADE, va("0 %i
      //1500", level.time + 500) ); 			trap_SetConfigstring( CS_SCREENFADE, va("0
      //%i 750", level.time + 500) );
      // (SA) send a command that will be interpreted for both the screenfade
      // and any other effects (music cues, pregame menu, etc)
