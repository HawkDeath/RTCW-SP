...
void Team_FragBonuses(gentity_t *targ, gentity_t *inflictor,
  ...
  if (targ->client->pers.teamState.lasthurtcarrier &&
    ...
    PrintMsg(NULL,
             "%s" S_COLOR_WHITE
             " defends %s's flag carrier against an agressive enemy\n",
             attacker->client->pers.netname, TeamName(team));
    return;
  }
  ...
  if (carrier && carrier != attacker) {
    ...
    if (VectorLength(v1) < CTF_ATTACKER_PROTECT_RADIUS ||
        ...
        CanDamage(carrier, targ->s.origin) ||
...
void Team_ReturnFlagSound(gentity_t *ent, int team) {
  ...
  te->s.eventParm =
      G_SoundIndex(team == TEAM_RED ? "sound/teamplay/flagret_red.wav"
...
int Team_TouchOurFlag(gentity_t *ent, gentity_t *other, int team) {
  ...
  te = G_TempEntity(ent->s.pos.trBase, EV_GLOBAL_SOUND);
  te->s.eventParm =
      G_SoundIndex(our_flag == PW_REDFLAG ? "sound/teamplay/flagcap_red.wav"
  ...
  for (i = 0; i < g_maxclients.integer; i++) {
    ...
    if (player->client->sess.sessionTeam != cl->sess.sessionTeam) {
      ...
      if (player->client->pers.teamState.lastreturnedflag +
        ...
        PrintMsg(NULL,
                 "%s" S_COLOR_WHITE
                 " gets an assist for returning the %s flag!\n",
      ...
      if (player->client->pers.teamState.lastfraggedcarrier +
        ...
        PrintMsg(NULL,
                 "%s" S_COLOR_WHITE
                 " gets an assist for fragging the %s flag carrier!\n",
...
int Pickup_Team(gentity_t *ent, gentity_t *other) {
  ...
  // JPW NERVE -- set flag model in carrying entity if multiplayer and flagmodel
  // is set
...
gentity_t *SelectRandomTeamSpawnPoint(int teamstate, team_t team) {
  ...
  // JPW NERVE
  if ((g_gametype.integer != GT_WOLF) || (!level.numspawntargets) ||
      initialSpawn) { // no spawn targets or not wolf MP, do it the old way
    selection = rand() % count;
    return spots[selection];
  } else {
    // FIXME: temporarily select target as farthest point from first team
    // spawnpoint we'll want to replace this with the target coords pulled from
    // the UI target selection
    j = 0;
    for (j = 0; j < count; j++) {
      if (spots[j]->spawnflags &
          1) { // only use spawnpoint if it's a permanent one
...
void TeamplayInfoMessage(gentity_t *ent) {
  ...
  for (i = 0, cnt = 0; i < level.numConnectedClients && cnt < TEAM_MAXOVERLAY;
       i++) {
...
void SP_team_WOLF_objective(gentity_t *ent) {
  ...
  G_SpawnString("description", "WARNING: No objective description set",
                &objectivename);
  ...
  G_Printf("OBJECTIVE %d: %s (total %s) x=%f %f %f\n", numobjectives,
           objectivename, numspawntargets, test[0], test[1], test[2]);
}
// jpw
