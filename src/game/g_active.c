...
void P_DamageFeedback(gentity_t *player) {
  ...
  if ((level.time > player->pain_debounce_time) &&
      !(player->flags & FL_GODMODE) && !(player->r.svFlags & SVF_CASTAI)) {
    player->pain_debounce_time = level.time + 700;
    G_AddEvent(player, EV_PAIN, player->health);
  }

  client->ps.damageEvent++; // Ridah, always increment this since we do multiple
                            // view damage anims
...
void P_WorldEffects(gentity_t *ent) {
  ...
  //ent->client->ps.powerups[PW_BREATHER], level.time -
  //ent->client->ps.powerups[PW_INVULNERABLE],
  //ent->client->ps.powerups[PW_NOFATIGUE]);
  ...
  if (waterlevel == 3) {
    ...
    //----(SA)	both these will end up being by virtue of having the 'breather'
    //powerup
    if (ent->client->ps.aiChar ==
        AICHAR_FROGMAN) { // let frogmen breathe forever
      ent->client->airOutTime = level.time + 10000;
    }
    ...
    if (ent->client->airOutTime < level.time) {
      ...
      if (0) {
        ...
        //(level.time - ent->client->airOutTime); 				ent->client->airOutTime =
      ...
      if (envirosuit) {
        ...
        if (ent->watertype & CONTENTS_LAVA) {
          ...
          G_Sound(ent, G_SoundIndex(
                           "sound/world/hurt_me.wav")); // sound gets cached by
                                                        // target_speaker...
                                                        // last minute hack.
        }
        ...
        //NULL, 						10*waterlevel, 0, MOD_SLIME); 	end			}
      }
    }
  }
  ...
  if (ent->s.onFireEnd > level.time &&
      (AICast_AllowFlameDamage(ent->s.number))) {
    gentity_t *attacker;
    ...
    if (ent->health > 0) {
      ...
      if (g_gametype.integer == GT_SINGLE_PLAYER) { // JPW NERVE
        if (ent->r.svFlags & SVF_CASTAI) {
          G_Damage(ent, attacker, attacker, NULL, NULL, 2, DAMAGE_NO_KNOCKBACK,
                   MOD_FLAMETHROWER);
        } else if ((ent->s.onFireEnd - level.time) > FIRE_FLASH_TIME / 2 &&
                   rand() % 5000 < (ent->s.onFireEnd -
                                    level.time)) { // as it fades out, also fade
                                                   // out damage rate
          G_Damage(ent, attacker, attacker, NULL, NULL, 1, DAMAGE_NO_KNOCKBACK,
                   MOD_FLAMETHROWER);
        }
      }      // JPW NERVE
      else { // JPW NERVE multiplayer flamethrower
        if (level.time < ent->s.onFireEnd) { // flamethrower does total 80 pts
      ...
      // jpw
    } else if (ent->s.onFireEnd >
               level.time + 4000) {         // dead, so sto pthe flames soon
...
void ClientIntermissionThink(gclient_t *client) {
  ...
  if ((client->buttons & (BUTTON_ATTACK | BUTTON_USE_HOLDABLE) &
       (client->oldbuttons ^ client->buttons)) ||
      (client->wbuttons & WBUTTON_ATTACK2 &
       (client->oldwbuttons ^ client->wbuttons))) {
...
void ClientEvents(gentity_t *ent, int oldEventSequence) {
  ...
  for (i = oldEventSequence; i < client->ps.eventSequence; i++) {
    ...
    case EV_FALL_DMG_50:
      ...
      //----(SA)	FIXME: TODO:  hmm, going through here adding
      //surfaceparms it seems that the value for ent->client->ps.pm_time was
      //weird.  (1000 for all but dmg_25 which has 250?)
      ...
      // (SA) right now just do a short range event on all surfaces until
      // surface-specific stuff is working
      AICast_AudibleEvent(ent->s.number, ent->s.pos.trBase,
                          g_footstepAudibleRange.value);

    case EV_FALL_SHORT:

      //				 if(eventParm &
      //(SURF_GRAVEL|SURF_GLASS))
      //fallSoundMul *= 0.5f; 			else if(eventParm & SURF_ROOF)
      //fallSoundMul *= 0.75f; 			else if(eventParm &
      //SURF_NOSTEPS|SURF_GRASS|SURF_CARPET|SURF_SNOW)	fallSoundMul *= 0.0f;
      //// no sound event
      //
      //			if(fallSoundMul)
      //				AICast_AudibleEvent( ent->s.number,
      //ent->s.pos.trBase, fallSoundMul * g_footstepAudibleRange.value);
      break;
      //----(SA)	end
    ...
    case EV_USE_ITEM7:  // ( HI_STAMINA )	restores fatigue bar and sets
                        // "nofatigue" for a time period
...
void ClientDamage(gentity_t *clent, int entnum, int enemynum,
                  int id); // NERVE - SMF
...
// JPW NERVE do some time-based muzzle flip -- this never gets touched in single
// player (see g_weapon.c) #define RIFLE_SHAKE_TIME 150 // JPW NERVE this one
// goes with the commented out old damped "realistic" behavior below
#define RIFLE_SHAKE_TIME 300 // per Id request, longer recoil time
  if (client->sniperRifleFiredTime) {
    if (level.time - client->sniperRifleFiredTime > RIFLE_SHAKE_TIME) {
      ...
      // JPW old damped behavior -- feels more like a real rifle (modeled on
      // Remington 700 7.62x51mm w/ 14x scope)
      /*
                              muzzlebounce[PITCH] -= 2*cos(1.0-(level.time -
         client->sniperRifleFiredTime)*3/RIFLE_SHAKE_TIME); muzzlebounce[YAW] +=
         client->sniperRifleMuzzleYaw*cos(1.0-(level.time -
         client->sniperRifleFiredTime)*3/RIFLE_SHAKE_TIME); muzzlebounce[PITCH]
         -= random()*(1.0f-(level.time -
         client->sniperRifleFiredTime)/RIFLE_SHAKE_TIME); muzzlebounce[YAW] +=
         crandom()*(1.0f-(level.time -
         client->sniperRifleFiredTime)/RIFLE_SHAKE_TIME);
      */

      // JPW per Id request, longer recoil time

      // MP method \/
      /*			muzzlebounce[PITCH] -= 2*cos(2.5*(level.time -
         client->sniperRifleFiredTime)/RIFLE_SHAKE_TIME); muzzlebounce[YAW] +=
         0.5*client->sniperRifleMuzzleYaw*cos(1.0-(level.time -
         client->sniperRifleFiredTime)*3/RIFLE_SHAKE_TIME); muzzlebounce[PITCH]
         -= 0.25*random()*(1.0f-(level.time -
         client->sniperRifleFiredTime)/RIFLE_SHAKE_TIME); muzzlebounce[YAW] +=
         0.5*crandom()*(1.0f-(level.time -
         client->sniperRifleFiredTime)/RIFLE_SHAKE_TIME);
      */

      // NOTE: ----------------- SP uses this method
      muzzlebounce[PITCH] -=
          0.25 * client->sniperRifleMuzzlePitch *
          cos(2.5 * (level.time - client->sniperRifleFiredTime) /
              RIFLE_SHAKE_TIME);
      muzzlebounce[YAW] +=
          0.2 * client->sniperRifleMuzzleYaw *
          cos(1.0 - (level.time - client->sniperRifleFiredTime) * 3 /
                        RIFLE_SHAKE_TIME);
      muzzlebounce[PITCH] -=
          0.25 * client->sniperRifleMuzzlePitch * random() *
          (1.0f -
           (level.time - client->sniperRifleFiredTime) / RIFLE_SHAKE_TIME);
      muzzlebounce[YAW] += 0.2 * crandom() *
                           (1.0f - (level.time - client->sniperRifleFiredTime) /
                                       RIFLE_SHAKE_TIME);
  ...
  /*
          if (g_gametype.integer != GT_SINGLE_PLAYER) {
                  if (client->ps.classWeaponTime < 1.0f) { // FIXME check based
     on character class client->ps.classWeaponPercent += 0.001;
                  }
  ...
  */
  ...
  if (!saveGamePending && (g_gametype.integer == GT_SINGLE_PLAYER) &&
  ...
  if (((saveGamePending || !ent->aiCharacter) && g_reloading.integer &&
    ...
    if (client->cameraPortal ||
        (g_reloading.integer & (RELOAD_NEXTMAP_WAITING | RELOAD_ENDGAME))) {
  ...
  BG_UpdateConditionValue(ent->s.number, ANIM_COND_PARACHUTE,
                          (ent->flags & FL_PARACHUTE) != 0, qfalse);
  ...
  if (ent->aiCharacter && AICast_NoReload(ent->s.number)) {
    pm.noWeapClips = qtrue; // ensure AI characters don't use clips if they're
                            // not supposed to.
  }
  // Ridah
  ...
  if (monsterslick && !(ent->flags & FL_NO_MONSTERSLICK)) {
    {
      ...
      // NE
      if ((monsterslick & SURF_MONSLICK_N) &&
      ...
      if (!bogus) {
        // when an actor dies test if tag_head is in a contents_solid and stop
        // slide
        {
          orientation_t or ;
          trace_t tr;
        ...
        */
             ...
             !(ent->flags & FL_NO_HEADCHECK)) { // move bodies away from walls
    ...
    if (VectorLength(pm.ps->velocity) < 100 &&
        trap_InPVS(pm.ps->origin, g_entities[0].s.pos.trBase)) {
      ...
      if (trap_GetTag(ent->s.number, "tag_head", & or)) {
        ...
        if (or.origin[2] < src[2]) {
          or.origin[2] = src[2]; // dont let the head sink into the ground (even
                                 // if it is visually)
        }
        trap_Trace(&tr, src, vec3_origin, vec3_origin, or.origin, ent->s.number,
                   MASK_SOLID);
  ...
  if (ent->client->ps.groundEntityNum >= 0 &&
  ...
  if (ent->client->ps.groundEntityNum >= MAX_CLIENTS &&
    ...
    if (groundEnt->isProp && groundEnt->takedamage && !groundEnt->nopickup) {
      ...
      if (groundEnt->s.groundEntityNum >= MAX_CLIENTS &&
          groundEnt->s.groundEntityNum < ENTITYNUM_WORLD) {
        if (!Q_stricmp(g_entities[groundEnt->s.groundEntityNum].classname,
                       "script_mover")) {
          // break it
  ...
  ent->s.animMovetype =
      BG_GetConditionValue(ent->s.number, ANIM_COND_MOVETYPE, qtrue);
  ...
  if (ent->client->ps.persistant[PERS_WOLFKICK] && !ent->melee &&
      !(ent->active)) {
    if (wolfkicktimer < level.time) {
      ent->client->ps.persistant[PERS_WOLFKICK] = 0;
    } else {
      ent->client->ps.persistant[PERS_WOLFKICK] =
          (int)(22 - ((wolfkicktimer - level.time) / 50));
  ...
  if (!(ent->r.svFlags & SVF_CASTAI)) {
    ...
    if (client->ps.stats[STAT_HEALTH] <= 0) {
      ...
      if (level.time > client->respawnTime) {
        ...
        //				  but in multiplayer, require button press
        //before respawn
        if (g_gametype.integer == GT_SINGLE_PLAYER) {
          respawn(ent);
        }
...
void SpectatorClientEndFrame(gentity_t *ent) {
  ...
  if ((ent->client->sess.spectatorState == SPECTATOR_FOLLOW) ||
      (ent->client->ps.pm_flags & PMF_LIMBO)) { // JPW NERVE for limbo
    int clientNum;

    // JPW NERVE check if it's time to respawn limbo players back into game
    //		G_Printf("rlt=%d blt=%d
    //diff=%d\n",g_redlimbotime.integer,g_bluelimbotime.integer, level.time -
    //level.redReinforceTime);
    ...
    /* old way, based on first guy to die
       ...
       g_redlimbotime.integer - (level.time - level.redReinforceTime); if
       (ent->client->ps.pm_time < 0) { do_respawn = 1; level.redReinforceTime =
       level.time+6000; // JPW NERVE 5 sec min delay
                                    }
                            }
                            if (ent->client->sess.sessionTeam == TEAM_BLUE) {
                                    ent->client->ps.pm_time =
       g_bluelimbotime.integer - (level.time - level.blueReinforceTime); if
       (ent->client->ps.pm_time < 0) { do_respawn = 1; level.blueReinforceTime =
       level.time+6000; // JPW NERVE 5 sec min delay
                                    }
                            }
                    }
    ...
    */
    ...
    if (clientNum >= 0) {
      ...
      if (cl->pers.connected == CON_CONNECTED &&
        ...
        //EF_TEAMVOTED)) | (ent->client->ps.eFlags & (EF_VOTED | EF_TEAMVOTED));
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    ...
    case WP_TESLA: // fear the tesla
      AICast_CheckDangerousEntity(
          ent, DANGER_CLIENTAIM, TESLA_RANGE + 150, 0.5, 0.6,
          (ent->client->buttons & BUTTON_ATTACK ? qtrue : qfalse));
      break;
    case WP_MONSTER_ATTACK1:
      if (ent->aiCharacter == AICHAR_ZOMBIE) {
        AICast_CheckDangerousEntity(ent, DANGER_CLIENTAIM | DANGER_FLAMES,
                                    FLAMETHROWER_RANGE + 150, 0.5, 0.8, qtrue);
      }
      break;
    ...
    case WP_MONSTER_ATTACK3:
      if (ent->aiCharacter == AICHAR_LOPER) {
        AICast_CheckDangerousEntity(ent, 0, LOPER_GROUND_RANGE + 100, 0.5, 0.8,
                                    qtrue);
      }
      break;
    case WP_FLAMETHROWER: // fear the flamethrower
      AICast_CheckDangerousEntity(
          ent, DANGER_CLIENTAIM | DANGER_FLAMES, FLAMETHROWER_RANGE + 150, 0.5,
          0.8, (ent->client->buttons & BUTTON_ATTACK ? qtrue : qfalse));
      break;
    case WP_VENOM: // avoid the venom
      AICast_CheckDangerousEntity(
          ent, DANGER_CLIENTAIM, 4000, 0.5, 0.8,
          (ent->client->buttons & BUTTON_ATTACK ? qtrue : qfalse));
      break;
    case WP_PANZERFAUST: // just predict a rocket attack, rather than waiting
                         // for them to fire, then avoiding
      AICast_CheckDangerousEntity(
          ent, DANGER_CLIENTAIM, 1000, 0.5, 0.8,
          (ent->client->buttons & BUTTON_ATTACK ? qtrue : qfalse));
      break;
    case WP_GRENADE_PINEAPPLE:
    case WP_GRENADE_LAUNCHER: // if they are wearing down a grenade fuse, we
                              // should be very afraid
      if (ent->client->ps.grenadeTimeLeft &&
          ent->client->ps.grenadeTimeLeft < 3000) {
        AICast_CheckDangerousEntity(ent, DANGER_CLIENTAIM, 1000, 0.5, 0.9,
                                    qtrue);
      }
      break;
    }
