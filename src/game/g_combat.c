...
void TossClientItems(gentity_t *self) {
  ...
  G_ThrowChair(self, forward,
               qtrue); // drop chair if you're holding one  //----(SA)	added
  ...
  if (1) {
    ...
    if (self->client->ps.weaponstate == WEAPON_DROPPING ||
        self->client->ps.weaponstate == WEAPON_DROPPING_TORELOAD) {
  ...
  if (weapon > WP_NONE && weapon < WP_MONSTER_ATTACK1 &&
      self->client->ps.ammo[BG_FindAmmoForWeapon(weapon)]) {
  ...
  if (g_gametype.integer ==
      GT_SINGLE_PLAYER) { // dropped items stay forever in SP
    if (drop) {
      drop->nextthink = 0;
    }
  }
  ...
  if (g_gametype.integer !=
      GT_TEAM) { // drop all the powerups if not in teamplay
    angle = 45;
...
void body_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (self->aiCharacter == AICHAR_HEINRICH ||
      self->aiCharacter == AICHAR_HELGA ||
...
char *modNames[] = {
    ...
    "MOD_RAILGUN", "MOD_LIGHTNING", "MOD_BFG", "MOD_BFG_SPLASH", "MOD_KNIFE",
    "MOD_KNIFE2", "MOD_KNIFE_STEALTH", "MOD_LUGER", "MOD_COLT", "MOD_MP40",
    "MOD_THOMPSON", "MOD_STEN", "MOD_MAUSER", "MOD_SNIPERRIFLE", "MOD_GARAND",
    "MOD_SNOOPERSCOPE",
    "MOD_SILENCER", //----(SA)
    "MOD_AKIMBO",   //----(SA)
    "MOD_BAR",      //----(SA)
    "MOD_FG42", "MOD_FG42SCOPE", "MOD_PANZERFAUST", "MOD_ROCKET_LAUNCHER",
    "MOD_GRENADE_LAUNCHER", "MOD_VENOM", "MOD_VENOM_FULL", "MOD_FLAMETHROWER",
    "MOD_TESLA", "MOD_SPEARGUN", "MOD_SPEARGUN_CO2", "MOD_GRENADE_PINEAPPLE",
    "MOD_CROSS", "MOD_MORTAR", "MOD_MORTAR_SPLASH", "MOD_KICKED", "MOD_GRABBER",
    "MOD_DYNAMITE", "MOD_DYNAMITE_SPLASH",
    ...
    "MOD_GRAPPLE", "MOD_EXPLOSIVE", "MOD_POISONGAS", "MOD_ZOMBIESPIT",
    "MOD_ZOMBIESPIT_SPLASH", "MOD_ZOMBIESPIRIT", "MOD_ZOMBIESPIRIT_SPLASH",
    "MOD_LOPER_LEAP", "MOD_LOPER_GROUND", "MOD_LOPER_HIT",
    // JPW NERVE
    "MOD_LT_ARTILLERY", "MOD_LT_AIRSTRIKE",
...
void player_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (g_gametype.integer ==
      GT_SINGLE_PLAYER) { // only drop here in single player; in multiplayer,
                          // drop @ limbo
  ...
  if (g_gametype.integer != GT_SINGLE_PLAYER) {
    ...
    if (item) {
      ...
      flag->s.modelindex2 =
          self->s.otherEntityNum2; // JPW NERVE FIXME set
  ...
  self->s.powerups = 0;
  // JPW NERVE -- only corpse in SP; in MP, need CONTENTS_BODY so medic can
  // operate
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    ...
    trap_SetConfigstring(CS_MUSIC_QUEUE,
                         ""); // clear queue so it'll be quiet after hit
...
qboolean IsHeadShotWeapon(int mod, gentity_t *targ, gentity_t *attacker) {
  // distance rejection
  if (DistanceSquared(targ->r.currentOrigin, attacker->r.currentOrigin) >
      (g_headshotMaxDist.integer * g_headshotMaxDist.integer)) {
    return qfalse;
  }
...
==============
*/
qboolean IsHeadShot(gentity_t *targ, gentity_t *attacker, vec3_t dir,
  ...
  if (head_shot_weapon) {
    ...
    if ((targ->r.svFlags & SVF_CASTAI) &&
        trap_GetTag(targ->s.number, "tag_head", & or)) {
      VectorCopy(or.origin, head->r.currentOrigin);
      VectorMA(head->r.currentOrigin, 6, or.axis[2],
               head->r.currentOrigin); // tag is at base of neck
    } else if (targ->client->ps.pm_flags &
               PMF_DUCKED) { // closer fake offset for 'head' box when crouching
      head->r.currentOrigin[2] +=
          targ->client->ps.crouchViewHeight +
          8; // JPW NERVE 16 is kludge to get head height to match up
    }
    // else if(targ->client->ps.legsAnim == LEGS_IDLE && targ->aiCharacter ==
    // AICHAR_SOLDIER)	// standing with legs bent (about a head shorter than
    // other leg poses) 	head->r.currentOrigin[2] += targ->client->ps.viewheight;
    else {
      head->r.currentOrigin[2] +=
          targ->client->ps.viewheight; // JPW NERVE pulled this	// 6 is fudged
                                       // "head height" value
    }
    ...
    VectorSet(head->r.mins, -6, -6,
              -6); // JPW NERVE changed this z from -12 to -6 for crouching,
                   // also removed standing offset
...
void G_ArmorDamage(gentity_t *targ) {
  ...
  if (numParts > dmgbits) {
    numParts = dmgbits; // lock this down so it doesn't overwrite any bits that
                        // it shouldn't.  TODO: fix this
  }
  // determined here (on server) by location of hit and existing armor, you're
  // updating here so the client knows which pieces are still in place, and by
  // difference with previous state, which pieces to play an effect where the
  // part is blown off. Need to do it here so we have info on where the hit
  // registered (head, torso, legs or if we go with more detail; arm, leg,
  // chest, codpiece, etc)

  // ... Ick, just discovered that the refined hit detection ("hit nearest to
  // which tag") is clientside...

  // For now, I'll randomly pick a part that hasn't been cleared.  This might
  // end up looking okay, and we won't need the refined hits.
  //	however, we still have control on the server-side of which parts come
  //off, regardless of what shceme is used.

  brokeparts = (int)((1 - ((float)(targ->health) /
                           (float)(targ->client->ps.stats[STAT_MAX_HEALTH]))) *
                     numParts);

  // RF, remove flame protection after enough parts gone
  if (AICast_NoFlameDamage(targ->s.number) &&
      ((float)brokeparts / (float)numParts >= 5.0 / 6.0)) { // figure from DM
    AICast_SetFlameDamage(targ->s.number, qfalse);
  }

  if (brokeparts &&
      ((targ->s.dmgFlags & ((1 << numParts) - 1)) !=
       (1 << numParts) - 1)) { // there are still parts left to clear
    ...
    if (brokeparts - curbroke >= 1 && curbroke < numParts) {
      for (i = 0; i < (brokeparts - curbroke); i++) {
        ...
        if (!((targ->s.dmgFlags & ((1 << numParts) - 1)) !=
              (1 << numParts) - 1)) { // no parts are available any more
          break;
        }

        // FIXME: lose the 'while' loop.  Still should be safe though, since the
        // check above verifies that it will eventually find a valid part
...
void G_Damage(gentity_t *targ, gentity_t *inflictor, gentity_t *attacker,
  ...
  //----(SA)	added
  if (g_gametype.integer == GT_SINGLE_PLAYER && !targ->aiCharacter &&
      targ->client && targ->client->cameraPortal) {
    // get out of damage in sp if in cutscene.
    return;
  }
  //----(SA)	end

  //	if (reloading || saveGamePending) {	// map transition is happening,
  //don't do anything
  ...
  // This is used by AI to determine how long it has been since their enemy was
  // injured

  if (attacker) { // (SA) whoops, for falling damage there's no attacker
    if (targ->client && attacker->client && !(targ->r.svFlags & SVF_CASTAI) &&
  ...
  if ((g_gametype.integer == GT_SINGLE_PLAYER) &&
      !(targ->r.svFlags & SVF_CASTAI)) { // the player
    switch (mod) {
    case MOD_GRENADE:
  ...
  if (targ->s.eType == ET_MOVER && !(targ->aiName) && !(targ->isProp) &&
      !targ->scriptName) {
  ...
  /*
          ...
          if ( attacker->client && attacker != targ ) {
                  damage = damage * attacker->client->ps.stats[STAT_MAX_HEALTH]
     / 100;
          }
  */
  // jpw
  ...
  if ((attacker->r.svFlags & SVF_CASTAI) && (targ->r.svFlags & SVF_CASTAI)) {
    if (!AICast_AIDamageOK(AICast_GetCastState(targ->s.number),
                           AICast_GetCastState(attacker->s.number))) {
      return;
    }
  ...
  //		knockback = 200;
  if (knockback >
      60) { // /way/ lessened for SP.  keeps dynamite-jumping potential down
    knockback = 60;
  }
  ...
  if (knockback && targ->client) {
    ...
    if (targ == attacker &&
        !(mod != MOD_ROCKET && mod != MOD_ROCKET_SPLASH && mod != MOD_GRENADE &&
          mod != MOD_GRENADE_SPLASH && mod != MOD_DYNAMITE)) {
  ...
  if (!(dflags & DAMAGE_NO_PROTECTION)) {
    ...
    // check for invulnerability // (SA) moved from below so
    // DAMAGE_NO_PROTECTION will still work
  ...
  if (g_gametype.integer ==
      GT_SINGLE_PLAYER) { // JPW NERVE -- removed from multiplayer -- plays
                          // havoc with pfaust & demolition balancing

    qboolean dynamite =
        (qboolean)(mod == MOD_DYNAMITE || mod == MOD_DYNAMITE_SPLASH);
  ...
  if (IsHeadShot(targ, attacker, dir, point, mod)) {
    ...
    if (g_gametype.integer != GT_SINGLE_PLAYER) {
      ...
      if (!attacker->aiCharacter) {
        ...
        // (SA) except pistols.
        // first pistol head shot does normal 2x damage and flings hat, second
        // gets kill
        //			if((mod != MOD_LUGER && mod != MOD_COLT ) ||
        //(targ->client->ps.eFlags & EF_HEADSHOT))	{	// (SA) DM
        //requests removing double shot pistol head shots (3/19)

        // (SA) removed BG for DM.

        if (!(dflags &
              DAMAGE_PASSTHRU)) { // ignore headshot 2x damage and
                                  // snooper-instant-death if the bullet passed
                                  // through something.  just do reg damage.
          switch (targ->aiCharacter) {
          case AICHAR_BLACKGUARD:
            if (!(targ->client->ps.eFlags &
                  EF_HEADSHOT)) { // only obliterate him after he's lost his
                                  // helmet
              break;
            }
          case AICHAR_SOLDIER:
    ...
    if (!(dflags &
      ...
      if ((mod == MOD_SNOOPERSCOPE || mod == MOD_GARAND) &&
  ...
  if (take) {
    ...
    if (targ->client) {
      if (mod != MOD_VENOM && attacker == inflictor &&
          targ->health <= GIB_HEALTH) {
        if (targ->aiCharacter !=
            AICHAR_ZOMBIE) { // zombie needs to be able to gib so we can kill
                             // him (although he doesn't actually GIB, he just
                             // dies)
    ...
    if (targ->health <= 0) {
      ...
      if (targ->s.number >= MAX_CLIENTS &&
          targ->health <= 0) { // might have revived itself in death function
...
qboolean G_RadiusDamage(vec3_t origin, gentity_t *attacker, float damage,
  ...
  for (e = 0; e < numListedEntities; e++) {
    ...
    /* JPW NERVE -- we can put this back if we need to, but it kinna sucks for
       human-sized bboxes
    ...
    */
    ...
    // JPW NERVE -- different radiusdmg behavior for MP -- big explosions should
    // do less damage (over less distance) through failed traces
    ...
    // JPW NERVE --  MP weapons should do 1/8 damage through walls over 1/8th
    // distance
    else {
      if (g_gametype.integer != GT_SINGLE_PLAYER) {
        ...
        if (tr.fraction < 1.0) {
          ...
          if (dist < radius * 0.2f) { // closer than 1/4 dist
            ...
            VectorSubtract(ent->r.currentOrigin, origin, dir);
            dir[2] += 24;
            G_Damage(ent, NULL, attacker, dir, origin, (int)(points * 0.1f),
                     DAMAGE_RADIUS, mod);
          }
        }
      }
