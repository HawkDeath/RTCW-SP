...
void trap_Endgame(void) { syscall(G_ENDGAME); }
...
qboolean trap_AAS_RT_GetHidePos(vec3_t srcpos, int srcnum, int srcarea,
                                ...
                                vec3_t returnPos) {
  return syscall(BOTLIB_AAS_RT_GETHIDEPOS, srcpos, srcnum, srcarea, destpos,
...
int trap_AAS_FindAttackSpotWithinRange(int srcnum, int rangenum, int enemynum,
                                       ...
                                       float *outpos) {
  return syscall(BOTLIB_AAS_FINDATTACKSPOTWITHINRANGE, srcnum, rangenum,
                 enemynum, PASSFLOAT(rangedist), travelflags, outpos);
}

qboolean trap_AAS_GetRouteFirstVisPos(vec3_t srcpos, vec3_t destpos,
                                      int travelflags, vec3_t retpos) {
  return syscall(BOTLIB_AAS_GETROUTEFIRSTVISPOS, srcpos, destpos, travelflags,
                 retpos);
}
...
void trap_EA_Reload(int client) { syscall(BOTLIB_EA_RELOAD, client); }
