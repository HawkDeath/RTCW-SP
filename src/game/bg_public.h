 ...
 * name:		bg_public.h
 *
 * desc:		definitions shared by both the server game and client game
 * modules
 *
 */
...
//#define	ITEM_RADIUS			15		// item sizes are
//needed for client side pickup detection
#define ITEM_RADIUS                                                            \
...
// JPW NERVE -- make flamethrower range variable with single/multiplayer --
// actual routine is in bg_pmove.c
...
#define CS_INTERMISSION                                                        \
  14 // when 1, fraglimit/timelimit has been hit and intermission will start in
     // a second or two
...
#define CS_SCREENFADE                                                          \
  28 // Ridah, used to tell clients to fade their screen to black/normal
#define CS_FOGVARS                                                             \
  29 //----(SA)	(hoping 29-31 are available) used for saving the current
     //state/settings of the fog
...
typedef enum {
  ...
  WEAPON_DROPPING,
  WEAPON_DROPPING_TORELOAD, //----(SA)	added.  will reload upon completion of
                            //weapon switch
  WEAPON_READYING,          // getting from 'ready' to 'firing'
  WEAPON_RELAXING, // weapon is ready, but since not firing, it's on it's way to
                   // a "relaxed" stance
  WEAPON_VENOM_REST,
  WEAPON_FIRING,
  WEAPON_FIRINGALT,
  WEAPON_WAITING,  //----(SA)	added.  player allowed to switch/reload, but not
                   //fire
...
#define PMF_TIME_LOAD                                                          \
  32768 // hold for this time after a load game, and prevent large thinks

#define PMF_ALL_TIMES                                                          \
  (PMF_TIME_WATERJUMP | PMF_TIME_LAND | PMF_TIME_KNOCKBACK | PMF_TIME_LOAD)
...
#define PC_MEDIC_CHARGETIME                                                    \
  30000 // FIXME just for testing, this will change to server cvars for each
        // class
// jpw
...
typedef enum {
  ...
  // Rafael - mg42		// (SA) I don't understand these here.  can someone
  // explain?
  PERS_HWEAPON_USE,
  // Rafael wolfkick
  PERS_WOLFKICK
...
#define EF_MONSTER_EFFECT                                                      \
  0x00000008 // draw an aiChar dependant effect for this character
...
#define EF_BOUNCE_HEAVY                                                        \
  0x00000200 // more realistic bounce.  not as rubbery as above (currently for
             // c4)
#define EF_SPINNING                                                            \
  0x00000400 // (SA) added for level editor control of spinning pickup items
#define EF_BREATH                                                              \
  EF_SPINNING // Characters will not have EF_SPINNING set, hijack for drawing
              // character breath

#define EF_MELEE_ACTIVE                                                        \
  0x00000800 // (SA) added for client knowledge of melee items held (chair/etc.)
#define EF_TALK 0x00001000       // draw a talk balloon
#define EF_CONNECTION 0x00002000 // draw a connection trouble sprite
#define EF_MONSTER_EFFECT2                                                     \
  0x00004000 // show the secondary special effect for this character
#define EF_HEADSHOT 0x00008000 // last hit to player was head shot
#define EF_MONSTER_EFFECT3                                                     \
  0x00010000 // show the third special effect for this character
#define EF_HEADLOOK 0x00020000 // make the head look around
#define EF_STAND_IDLE2                                                         \
  0x00040000 // when standing, play idle2 instead of the default
...
//#define EF_MOVER_BLOCKED	0x00100000		// mover was blocked dont
//lerp on the client
#define EF_NO_TURN_ANIM 0x00100000 // dont play turning anims in the cgame
#define EF_FORCED_ANGLES                                                       \
  0x00200000 // enforce all body parts to use these angles

#define EF_ZOOMING 0x00400000 // client is zooming
#define EF_NOSWINGANGLES                                                       \
  0x00800000 // try and keep all parts facing same direction

#define EF_DUMMY_PMOVE 0x01000000
#define EF_VOTED 0x02000000  // already cast a vote
#define EF_BOUNCE 0x04000000 // for missiles
#define EF_CIG                                                                 \
  EF_BOUNCE // players should never use bounce, hijack for cigarette
...
#define EF_DEATH_FRAME                                                         \
  EF_MOVER_ANIMATE // hijack to stick death at last frame after vid_restart
...
typedef enum {
  ...
  //			I'll change to 'INV' as the item becomes 'permanent' and not a
  //test item.
  KEY_NONE,
  //	KEY_1,		// skull
  //	KEY_2,		// chalice
  ...
  KEY_UNLOCKED_ENT,    // ent is unlocked (will be replaced by KEY_NONE after
                       // checks for all manners of being locked)
  KEY_LOCKED_TARGET,   // ent is locked by virtue of being the target of another
                       // ent
...
typedef enum {
  ...
  // weapon keys only go 1-0, so put the alternates above that (since selection
  // will be a double click on the german weapon key)
  ...
  WP_GARAND,   // 13	equivalent american weapon to german mauser
               //	WP_BAR,					// 16	equivalent american weapon to german
               //fg42
  WP_GRENADE_PINEAPPLE, // 14
  //	WP_ROCKET_LAUNCHER,		// 18	equivalent american weapon to german
  //panzerfaust
  ...
  WP_CLASS_SPECIAL, // 21	// class-specific multiplayer weapon (airstrike,
                    // engineer, or medpack)
  // (SA) go ahead and take the 'freezeray' spot.  it ain't happenin
  //      (I checked for instances of WP_CLASS_SPECIAL and I don't think this'll
  //      cause you a problem.  however, if it does, move it where you need to.
  //      ) (SA)
  // jpw
  //	WP_CROSS,				// 29
  WP_DYNAMITE, // 22
  ...
  WP_NUM_WEAPONS // 32   NOTE: this cannot be larger than 64 for AI/player
                 // weapons!
...
typedef enum {
  ...
  EV_ITEM_PICKUP_QUIET, // (SA) same, but don't play the default pickup sound as
                        // it was specified in the ent
...
typedef enum {
  ...
  IT_CLIPBOARD, // 'clipboard' used as a general term for 'popup' items where
                // you pick up the item and it pauses and opens a menu
  IT_TEAM
} itemType_t;
...
typedef struct gitem_s {
  ...
  int giAmmoIndex; // type of weapon ammo this uses.  (ex. WP_MP40 and WP_LUGER
                   ...
                   // giAmmoIndex)
  int giClipIndex; // which clip this weapon uses.  this allows the sniper rifle
                   // to use the same clip as the garand, etc.
...
// qboolean BG_AkimboFireSequence	( playerState_t *ps );	//----(SA)
// added
...
#define MASK_SHOT                                                              \
  (CONTENTS_SOLID | CONTENTS_BODY | CONTENTS_CORPSE | CONTENTS_CLIPSHOT)
...
typedef struct {
  ...
  animScript_t scriptCannedAnims[MAX_AISTATES]
                                [NUM_ANIM_MOVETYPES]; // played randomly
  animScript_t scriptStateChange[MAX_AISTATES]
                                [MAX_AISTATES]; // state change events
  animScript_t
...
qboolean BG_AnimParseAnimConfig(animModelInfo_t *animModelInfo,
...
int BG_AnimScriptStateChange(playerState_t *ps, aistateEnum_t newState,
...
int BG_GetAnimScriptAnimation(int client, aistateEnum_t state,
...
void BG_UpdateConditionValueStrings(int client, char *conditionStr,
                                    char *valueStr);
