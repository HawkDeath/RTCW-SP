...
// FIXME: go through here and convert all weapon/character parameters to
// #define's and move them to a seperate header file for easy modification
...
qboolean AICast_StateChange(cast_state_t *cs, aistateEnum_t newaistate) {
  ...
  // check scripting to see if this event should be ignored (no anim or
  // handling)
  cs->aiFlags &= ~AIFL_DENYACTION;
  AICast_ScriptEvent(cs, "statechange",
                     va("%s %s", animStateStr[oldstate].string,
                        animStateStr[newaistate].string));
  ...
  if (!(cs->aiFlags & AIFL_DENYACTION)) {
    ...
    if (newaistate == AISTATE_COMBAT && cs->scriptCallIndex == scriptIndex &&
        !(cs->vislist[cs->enemyNum].flags &
          AIVIS_SIGHT_SCRIPT_CALLED)) { // no script was found, so default back
                                        // to enemysight
      AICast_ScriptEvent(cs, "enemysight", g_entities[cs->enemyNum].aiName);
      cs->vislist[cs->enemyNum].flags |= AIVIS_SIGHT_SCRIPT_CALLED;
      if (!(cs->aiFlags & AIFL_DENYACTION)) {
        G_AddEvent(
            ent, EV_GENERAL_SOUND,
            G_SoundIndex(
                aiDefaults[ent->aiCharacter].soundScripts[SIGHTSOUNDSCRIPT]));
      }
...
AICast_ScanForEnemies
  ...
  returns the number of enemies visible, filling the "enemies" list before
exiting
...
int AICast_ScanForEnemies(cast_state_t *cs, int *enemies) {
  ...
  if (cs->castScriptStatus.scriptAttackEnt >= 0) {
    if (g_entities[cs->castScriptStatus.scriptAttackEnt].health <= 0) {
      ...
      // if we are not in combat mode, then an enemy should trigger a state
      // change straight to combat mode
      if (cs->aiState < AISTATE_COMBAT) {
        AICast_StateChange(cs,
                           AISTATE_COMBAT); // just go straight to combat mode
      }
      ...
      if (i != cs->entityNum) {
        if (AICast_EntityVisible(cs, i, qfalse)) {
          ...
          if ((g_entities[i].health > 0) &&
              AICast_HostileEnemy(cs, i)) { // visible and a baddy!
            enemies[enemyCount] = i;
            enemyCount++;
            queryCount = 0;
            friendlyAlertCount = 0;
          } else if (!enemyCount && (g_entities[i].health > 0) &&
                     AICast_QueryEnemy(cs, i) &&
                     (cs->vislist[i].flags & AIVIS_PROCESS_SIGHTING)) {
            enemies[queryCount] = i;
            queryCount++;
            friendlyAlertCount = 0;
          } else if (!queryCount && !enemyCount &&
                     (cs->vislist[i].flags & AIVIS_INSPECT)) {
  ...
  if (!enemyCount) { // nothing worth doing anything about
    ...
    if (cs->audibleEventTime && cs->audibleEventTime < level.time &&
        cs->audibleEventTime > level.time - 2000) {
      return -4;
    }
    // look for bullet impacts that have occured recently
    if (cs->bulletImpactTime && cs->bulletImpactTime < level.time &&
        cs->bulletImpactTime > level.time - 1000) {
      return -3;
    }
    return 0;
  }
  ...
  for (i = 0; i < enemyCount; i++) {
    distances[i] =
        Distance(cs->bs->origin, g_entities[enemies[i]].client->ps.origin);
    if (!distances[i]) {
      G_Printf("WARNING: zero distance between enemies:\n%s at %s, %s at %s\n",
               g_entities[cs->entityNum].aiName, vtos(cs->bs->origin),
               g_entities[enemies[i]].aiName,
               vtos(g_entities[enemies[i]].client->ps.origin));
  ...
  // if we are not in combat mode, then an enemy should trigger a state change
  // straight to combat mode
  if (!queryCount && !friendlyAlertCount && enemyCount &&
  ...
  // if we are in relaxed state, and we see a query enemy, then go into query
  // mode
  if (queryCount) {
    if (cs->aiState == AISTATE_RELAXED) {
      ...
      return 0; // scripting obviously doesn't want us to progress from relaxed
                // just yet
    }
  ...
  if (friendlyAlertCount) {
    ...
    if (g_entities[enemies[0]].health <= 0) {
      AICast_ForceScriptEvent(cs, "inspectbodystart",
                              g_entities[enemies[0]].aiName);
      if (cs->aiFlags & AIFL_DENYACTION) {
        // ignore this friendly
        cs->vislist[enemies[0]].flags |=
            AIVIS_INSPECTED; // they have been notified
        cs->vislist[enemies[0]].flags &=
            ~AIVIS_INSPECT; // they have been notified
        return 0;
      }
    }
    //
    if (cs->aiState < AISTATE_COMBAT) {
      // go into alert mode, and return this entity so we can inspect it or
      // something, but let dynamic AI sort out what it wants to do
...
==================
*/
qboolean AICast_EntityVisible(cast_state_t *cs, int enemynum,
                              qboolean directview) {
  ...
  if (cs->bs && enemynum == cs->enemyNum) {
    ...
    dist = VectorDistance(g_entities[cs->entityNum].client->ps.origin,
                          cs->vislist[enemynum].visible_pos);
  }
  if (dist < 384) {
  ...
  if (vis->notvisible_timestamp < (level.time - reactionTime)) {
    // make sure we've seen them since we've last not seen them (since the
    // visibility checking is spread amongst server frames)
    if (vis->notvisible_timestamp < last_visible) {
      return qtrue;
    }
  }

  // we can't directly see them, but if they've just left our sight, pretend we
  // can see them for another second or so
...
AICast_QueryEnemy
  ...
  returns qtrue if the entity can become hostile (they hurt us or we recognize
them)
==================
*/
qboolean AICast_QueryEnemy(cast_state_t *cs, int enemynum) {
  ...
  if (g_entities[cs->entityNum].aiTeam != g_entities[enemynum].aiTeam) {
    if (g_entities[cs->entityNum].aiTeam == AITEAM_MONSTER ||
        g_entities[enemynum].aiTeam == AITEAM_MONSTER) {
      // monsters hate all non-monsters and vice-versa
      return qtrue;
    } else {
      // neutral's can only possibly become hostile if they hurt us, otherwise
      // they are assumed to be harmless
      if (g_entities[cs->entityNum].aiTeam == AITEAM_NEUTRAL ||
          g_entities[enemynum].aiTeam == AITEAM_NEUTRAL) {
...
float AICast_WeaponRange(cast_state_t *cs, int weaponnum) {
  ...
  case WP_MONSTER_ATTACK1:
    ...
    case AICHAR_WARZOMBIE:
      return 80; // make it larger so we can start swinging early, and move in
                 // while swinging
...
==================
*/
qboolean AICast_CheckAttack_real(cast_state_t *cs, int enemy,
  ...
  if (!ent->waterlevel && ent->client->ps.groundEntityNum == ENTITYNUM_NONE &&
      !ent->active) {
  ...
  if (!allowHitWorld && cs->vislist[enemy].real_visible_timestamp !=
                            cs->vislist[enemy].real_update_timestamp) {
    return qfalse;
  }
  //
  ...
  if (!ent->active) {
    ...
    CalcMuzzlePoint(&g_entities[cs->entityNum], weapnum, forward, right, up,
                    start);
    ...
    if (traceDist > dist) {
      ...
      dist -=
          enemyEnt->r.maxs[0]; // subtract distance to edge of bounding box edge
      if (traceDist < dist) {
        return qfalse;
      }
    }
  ...
  for (i = 0; i <= fuzzyCount; i++) {
    ...
    // fuzzy end point
    if (i > 0) {
      VectorMA(end, enemyEnt->r.maxs[0] * 0.9 * (float)((i % 2) * 2 - 1), right,
               end);
      halfHeight = (enemyEnt->r.maxs[2] - enemyEnt->r.mins[2]) / 2.0;
      end[2] =
          (enemyEnt->r.currentOrigin[2] + enemyEnt->r.mins[2]) + halfHeight;
      VectorMA(end,
               halfHeight * 0.9 *
                   (((float)((i - 1) - ((i - 1) % 2)) / 2 - 1.0)),
               up, end);
    }
    ...
    if (trace.fraction == 1.0) {
      if (!trace.startsolid) {
        return qtrue; // not sure why, but this fixes blackguards in chateau
                      // shooting through glass ceiling
      }
      // return qfalse;
      continue;
    }
    ...
    if (trace.entityNum != enemy) {
      ...
      if (g_entities[trace.entityNum].takedamage &&
          g_entities[trace.entityNum].health > 0 &&
...
==================
*/
qboolean AICast_CheckAttackAtPos(int entnum, int enemy, vec3_t pos,
                                 qboolean ducking, qboolean allowHitWorld) {
  gentity_t *ent;
  vec3_t savepos;
  int saveview;
  qboolean rval;
...
AICast_CheckAttack
  ...
  optimization, uses the cache to avoid possible duplicate calls with same world
paramaters
==================
*/
qboolean AICast_CheckAttack(cast_state_t *cs, int enemy,
                            qboolean allowHitWorld) {
  if (cs->bs) {
    if ((cs->checkAttackCache.time == level.time) &&
      ...
      cs->checkAttackCache.weapon = cs->weaponNum;
      return (cs->checkAttackCache.result =
                  AICast_CheckAttack_real(cs, enemy, allowHitWorld));
    }
  } else {
...
void AICast_UpdateBattleInventory(cast_state_t *cs, int enemy) {
  ...
  for (i = 0; i < WP_NUM_WEAPONS; i++) {
    if ((i >= WP_MONSTER_ATTACK1 && i <= WP_MONSTER_ATTACK3) ||
        (g_entities[cs->bs->entitynum]
             .client->ps.ammo[BG_FindAmmoForWeapon(i)] > 800)) {
      // g_entities[cs->bs->entitynum].client->ps.ammo[ BG_FindAmmoForWeapon(i)]
      // = 999;
...
qboolean AICast_GotEnoughAmmoForWeapon(cast_state_t *cs, int weapon) {
  ...
  case WP_GAUNTLET:
    return qtrue;
  default:
    return (qboolean)((clip >= ammoTable[weapon].uses) ||
                      (ammo >= ammoTable[weapon].uses)); //----(SA)
  }
}
...
AICast_WeaponUsable
  ...
  This can be used to add a delay between firing for special attacks, or make
certain that certain weapons are only selected within a certain range or under
certain conditions.

  NOTE: that monster_attack2 will always override monster_attack1 if both are
usable
==============
*/
qboolean AICast_WeaponUsable(cast_state_t *cs, int weaponNum) {
  ...
  switch (weaponNum) {
    // don't attempt to lob a grenade more than this often, since we will abort
    // a grenade throw if it's not safe, we shouldn't keep switching back too
    // quickly
  case WP_DYNAMITE:
  case WP_GRENADE_LAUNCHER:
  case WP_GRENADE_PINEAPPLE:
    ...
    if (cs->weaponFireTimes[weaponNum] < level.time - delay) {
      ...
      hitclient = AICast_SafeMissileFire(
          grenade, grenade->nextthink - level.time, cs->enemyNum,
          g_entities[cs->enemyNum].s.pos.trBase, cs->entityNum, NULL);
  ...
  case WP_MONSTER_ATTACK2:
    ...
    case AICHAR_HELGA:
      ...
      break;
    case AICHAR_LOPER: // loper leap attack
      if (cs->bs->areanum && VectorLength(cs->bs->velocity) >
                                 1) { // if we are in a valid area, and are
                                      // persuing, then leave a delay
        ...
        if (cs->enemyNum >= 0) {
          ...
          mins[0] = 0;
          trap_Trace(&trace, g_entities[cs->entityNum].client->ps.origin, mins,
                     cs->bs->cur_ps.maxs,
                     g_entities[cs->enemyNum].client->ps.origin, cs->entityNum,
  ...
  //
  return (!cs->weaponFireTimes[weaponNum] ||
          (cs->weaponFireTimes[weaponNum] < level.time - delay));
}
...
void AICast_ChooseWeapon(cast_state_t *cs, qboolean battleFunc) {
  ...
  if (cs->enemyNum >= 0) {
    enemyDist =
        VectorDistance(g_entities[cs->enemyNum].s.pos.trBase, cs->bs->origin);
  ...
  if (cs->bs->cur_ps.weaponstate == WEAPON_RAISING ||
      cs->bs->cur_ps.weaponstate ==
          WEAPON_RAISING_TORELOAD || //----(SA)	added
      cs->bs->cur_ps.weaponstate == WEAPON_DROPPING ||
      cs->bs->cur_ps.weaponstate ==
          WEAPON_DROPPING_TORELOAD) { //----(SA)	added
    return;
  }

  // disabled this, makes grenade guy keep trying to throw a grenade he doesn't
  // have
  ...
  if (cs->weaponNum &&
    ...
    if (cs->weaponNum == WP_GRENADE_LAUNCHER ||
  ...
  for (i = 0; i < WP_NUM_WEAPONS; i++) {
    if (i == WP_GRENADE_LAUNCHER || i == WP_GRENADE_PINEAPPLE) {
      continue; // never choose grenades at will, only when going into grenade
                // flush mode
    }
    ...
    if (COM_BitCheck(cs->bs->cur_ps.weapons, i)) {
      ...
      //
      if ((!inRange && thisInRange) ||
          ((!inRange || thisInRange) && (wantScale >= bestWantScale))) {
        cs->weaponNum = i;
        bestWantScale = wantScale;
        if (thisInRange) { // we have found a weapon inside attackable range,
                           // don't override with one outside range
          inRange = qtrue;
        }
      }
    }
  ...
  if (!gotOne && (cs->weaponNum < WP_MONSTER_ATTACK1 ||
                  cs->weaponNum > WP_MONSTER_ATTACK3)) {
    if (g_cheats.integer &&
        (!cs->bs->cur_ps.weapons[0] && !cs->bs->cur_ps.weapons[1])) {
...
AICast_Aggression
  ...
  Check all possible reasons why we shouldn't attack, returning a value from 1.0
(fully willing) to 0.0 (please don't hurt me).
==================
*/
float AICast_Aggression(cast_state_t *cs) {
  ...
  if (bs->cur_ps.stats[STAT_HEALTH] < 50) {
    scale -= (1.0 - cs->attributes[AGGRESSION]) *
             (1.0 - ((float)bs->cur_ps.stats[STAT_HEALTH] / 50.0));
  }
  ...
  painTime = 15000 - (int)(10000.0 * cs->attributes[AGGRESSION] *
                           cs->attributes[AGGRESSION]);
  if (cs->lastPain + painTime > level.time) {
    scale -= 3 * (1.0 - cs->attributes[AGGRESSION]) *
             ((float)(cs->lastPain + painTime - level.time) / (float)painTime);
  }

  // if we just rolled, stay out of view if we jumped behind cover
  painTime = 10000 - (int)(10000.0 * cs->attributes[AGGRESSION] *
                           cs->attributes[AGGRESSION]);
  if (cs->battleRollTime + painTime > level.time) {
    scale -=
        2 * (1.0 - cs->attributes[AGGRESSION]) *
        ((float)(cs->battleRollTime + painTime - level.time) / (float)painTime);
  }
  ...
  if (cs->entityNum >= 0 &&
      ((level.time + 2000 * g_entities[cs->entityNum].aiTeam) %
       (4000 + 500 * g_entities[cs->entityNum].aiTeam)) > 4000) {
    if (cs->vislist[cs->entityNum].visible_timestamp > level.time - 10000) {
      scale +=
          0.3 *
          (float)(level.time - cs->vislist[cs->entityNum].visible_timestamp) /
          10000.0;
    }
  }
...
int AICast_WantsToTakeCover(cast_state_t *cs, qboolean attacking) {
  ...
  //
  // Dodge enemy aim?
  if (cs->attributes[AGGRESSION] < 1.0 && attacking && (cs->enemyNum >= 0) &&
    ...
    AngleVectors(g_entities[cs->enemyNum].client->ps.viewangles, aim, NULL,
                 NULL);
    VectorSubtract(cs->bs->origin, g_entities[cs->enemyNum].r.currentOrigin,
                   enemyVec);
...
bot_moveresult_t AICast_CombatMove(cast_state_t *cs, int tfl) {
  ...
  if (cs->combatGoalTime > level.time) {
    if (VectorLength(cs->combatGoalOrigin) > 1) {
      ...
      // if we are there, and the enemy can see us, but we cant hit them, abort
      // immediately
    ...
    if (((cs->attributes[CAMPER] < random()) &&
      ...
      if ((cs->attributes[TACTICAL] > 0.3 + random() * 0.5) &&
          trap_AAS_RT_GetHidePos(
              cs->bs->origin, cs->bs->entitynum, cs->bs->areanum,
              cs->vislist[cs->enemyNum].visible_pos, cs->enemyNum,
              BotPointAreaNum(cs->vislist[cs->enemyNum].visible_pos),
              cs->combatGoalOrigin)) {
        cs->combatGoalTime =
            level.time + 10000; // give us plenty of time to get there
        // cs->combatSpotAttackCount = cs->startAttackCount + 3;	// don't
        // keep moving around to different positions on our own
...
void AICast_WeaponSway(cast_state_t *cs, vec3_t ofs) {
  ...
  case WP_FLAMETHROWER:
    ofs[PITCH] = (3.0 + 4.0 * sin(((float)level.time / 320.0))) *
                 sin(((float)level.time / 500.0));
    ofs[YAW] = (6.0 + 8.0 * sin(((float)level.time / 250.0))) *
               sin(((float)level.time / 400.0));
    ofs[ROLL] = 0;
    break;
  case WP_VENOM:
    ofs[PITCH] = 2 * (float)cos((level.time / 200));
    ofs[YAW] =
        10 * (float)sin((level.time / 150)) * (float)sin((level.time / 100));
    ofs[ROLL] = 0;
    break;
  }
}
...
qboolean AICast_AimAtEnemy(cast_state_t *cs) {
  ...
  if (cs->aiCharacter == AICHAR_STIMSOLDIER2 &&
  ...
  if (vis->visible_timestamp < vis->lastcheck_timestamp) {
    ...
    if (cs->weaponNum == WP_GRENADE_LAUNCHER ||
        cs->weaponNum == WP_GRENADE_PINEAPPLE) {
      aicast_predictmove_t move;
      AICast_PredictMovement(AICast_GetCastState(cs->enemyNum), 1, 1.0, &move,
                             &g_entities[cs->enemyNum].client->pers.cmd, -1);
  ...
  if (cs->weaponNum == WP_GRENADE_LAUNCHER ||
    ...
    if (Distance(start, bestorigin) < 180) {
      bestorigin[2] =
          enemyOrg[2] + g_entities[cs->enemyNum].r.mins[2] + crandom() * 20;
  ...
  if (cs->weaponNum == WP_PANZERFAUST) {
    VectorMA(bestorigin, aim_skill * aim_skill * (dist / 900),
             g_entities[cs->enemyNum].client->ps.velocity, bestorigin);
    // if they are close, aim down at their feet
    if (dist < 512) {
      bestorigin[2] -=
          (VectorLength(g_entities[cs->enemyNum].client->ps.velocity) / 500.0) *
          (1.0 - dist / 2048) *
          (bestorigin[2] - (g_entities[cs->enemyNum].client->ps.origin[2] +
                            g_entities[cs->enemyNum].client->ps.mins[2]));
    }
  }
  // if the enemy is moving, they are harder to hit
  if (dist > 256) {
    VectorMA(bestorigin,
             (0.3 + 0.7 * (1 - aim_accuracy)) * 0.4 *
                 sin((float)level.time /
                     (500.0 + (100.0 * ((cs->entityNum + 3) % 4)))),
             g_entities[cs->enemyNum].client->ps.velocity, bestorigin);
  }
  // if we are good at aiming, we should aim ahead of where they are now
  // since by the time we have rotated to that direction, some time will have
  // passed
  if (aim_skill > 0.2) {
    VectorMA(bestorigin, aim_skill * 0.2,
             g_entities[cs->enemyNum].client->ps.velocity, bestorigin);
  }
  // get aim direction
...
qboolean AICast_RandomTriggerRelease(cast_state_t *cs) {
  ...
  switch (cs->aiCharacter) {
  case AICHAR_BLACKGUARD: // this is here since his "ready" frame is different
                          // to his firing frame, so it looks wierd to keep
...
void AICast_ProcessAttack(cast_state_t *cs) {
  ...
  // never fire grenades from within here (needs special AIFunc_GrenadeFlush()
  // code)
  if (cs->weaponNum == WP_GRENADE_LAUNCHER ||
      cs->weaponNum == WP_GRENADE_PINEAPPLE) {
    return;
  ...
  //
  bs = cs->bs;
  // check for not firing while moving flag, if present, abort attack if any
  ...
  if (!AICast_CanMoveWhileFiringWeapon(cs->weaponNum)) {
    ...
    // if we're trying to move somewhere, don't let us shoot, until we've
    // arrived
    ...
    if ((cs->castScriptStatus.scriptNoMoveTime < level.time) &&
        ((bi.actionflags & ACTION_MOVEFORWARD) ||
         ...
         (bi.actionflags & ACTION_MOVERIGHT) || (bi.speed))) {
      return;
    }
  }
  //
  // if not a "walk forward" AI, then aim at the enemy regardless of whether we
  // can attack them or not
  ...
  if (!AICast_CheckAttack(cs, cs->enemyNum, qfalse)) {
    ...
    if (cs->attributes[ATTACK_CROUCH] &&
      ...
      if (!AICast_CheckAttackAtPos(cs->entityNum, cs->enemyNum, cs->bs->origin,
                                   qfalse, qfalse)) {
        cs->attackcrouch_time = level.time + 2000;
      } else {
        cs->attackcrouch_time =
            0; // we can attack them if we stand, so go for it
      }
    }
    return;
  }
  //
  // if we are a "walk forward" AI, then aim at the enemy only if we can attack
  // them
  if (cs->aiFlags & AIFL_WALKFORWARD) {
    if (!AICast_AimAtEnemy(cs)) {
      return;
  ...
  if (AICast_RandomTriggerRelease(cs) &&
      cs->triggerReleaseTime < (level.time - 500)) {
    if (rand() % 5 == 0) {
...
qboolean AICast_GetTakeCoverPos(cast_state_t *cs, int enemyNum, vec3_t enemyPos,
  ...
  //
  // can we just crouch?
  if ((cs->attackcrouch_time < level.time) && (enemyNum < aicast_maxclients) &&
      AICast_CheckAttackAtPos(cs->entityNum, enemyNum, cs->bs->origin, qfalse,
                              qfalse) &&
      !AICast_CheckAttackAtPos(cs->entityNum, enemyNum, cs->bs->origin, qtrue,
                               qfalse)) {
    ...
    visible =
        AICast_VisibleFromPos(g_entities[enemyNum].r.currentOrigin, enemyNum,
                              cs->bs->origin, cs->entityNum, qfalse);
  ...
  if (cs->bs->areanum &&
      trap_AAS_RT_GetHidePos(cs->bs->origin, cs->bs->entitynum, cs->bs->areanum,
                             enemyPos, enemyNum, BotPointAreaNum(enemyPos),
                             returnPos)) {
    return qtrue;
  }
...
float AICast_GetWeaponSoundRange(int weapon) {
  // NOTE: made this a case, that way changing the ordering of weapons won't
  // cause problems, as it would with an array lookup
...
qboolean AICast_StopAndAttack(cast_state_t *cs) {
  ...
  switch (cs->weaponNum) {
    ...
    // if they are using Venom, and are too far away to be effective, then keep
    // chasing
  case WP_VENOM:
    if (dist > 300) {
      return qfalse;
  ...
  case WP_PANZERFAUST:
    // if we haven't injured them in a while, advance
    if ((cs->aiCharacter == AICHAR_PROTOSOLDIER ||
         cs->aiCharacter == AICHAR_SUPERSOLDIER) &&
        cs->enemyNum >= 0) {
      if (dist > 300) {
        return qfalse;
      }
...
#define AICAST_ACC_SCALE 0.4
  ...
  if (AICAST_VARIABLE_ACC_ENABLED) {
    if (cs->enemyNum >= 0) {
      if (cs->vislist[cs->enemyNum].real_notvisible_timestamp <
          level.time - AICAST_ACC_VISTIME) {
        acc += 0.5 * AICAST_ACC_SCALE;
      } else {
        acc += AICAST_ACC_SCALE *
               ((float)(-0.5 * AICAST_ACC_VISTIME + level.time -
                        cs->vislist[cs->enemyNum].real_notvisible_timestamp) /
                (float)(AICAST_ACC_VISTIME));
      }
...
AICast_SafeMissileFire
  ...
  checks to see if firing the missile will be successful, neutral, or dangerous
to us or a friendly
==============
*/
int AICast_SafeMissileFire(gentity_t *ent, int duration, int enemyNum,
  ...
  if (!rval &&
      Distance(g_entities[ent->r.ownerNum].r.currentOrigin, org) >
          Distance(g_entities[ent->r.ownerNum].r.currentOrigin, enemyPos)) {
...
=============
*/
void AICast_CheckDangerousEntity(gentity_t *ent, int dangerFlags,
                                 float dangerDist, float tacticalLevel,
                                 float aggressionLevel, qboolean hurtFriendly) {
  vec3_t org, fwd, vec;
  cast_state_t *cs, *dcs;
  gentity_t *trav;
  int i, endTime;
  ...
  if (dangerFlags & DANGER_MISSILE) {
    ...
    if (!(endTime =
              G_PredictMissile(ent, ent->nextthink - level.time, org, qtrue))) {
  ...
  for (trav = g_entities, cs = AICast_GetCastState(0), i = 0;
       i < level.numPlayingClients; cs++, trav++) {
    ...
    if (!hurtFriendly && ent->s.number < MAX_CLIENTS &&
        AICast_SameTeam(cs, ent->s.number)) {
    ...
    if (cs->aiState >= AISTATE_COMBAT &&
        cs->attributes[AGGRESSION] >
            aggressionLevel) { // we are too aggressive to worry about being
                               // hurt by this
      continue;
    }
    // if they are below alert mode, and the danger is not in FOV, then ignore
    // it
    if (cs->aiState < AISTATE_ALERT) {
      vec3_t ang, dir;
    ...
    if (ent->client &&
        (!cs->vislist[ent->s.number].visible_timestamp ||
         (cs->vislist[ent->s.number].visible_timestamp < level.time - 3000))) {
      //	(!dcs->vislist[trav->s.number].visible_timestamp ||
      //(dcs->vislist[trav->s.number].visible_timestamp < level.time - 3000))))
    ...
    if (cs->dangerEntityValidTime < level.time + 50) {
      ...
      if (dist < dangerDist) {
        ...
        cs->dangerDist =
            dangerDist * 1.5; // when we hide from it, get a good distance away
...
void AICast_ProcessBullet(gentity_t *attacker, vec3_t start, vec3_t end) {
  ...
  // RF, AI should hear this pass by them really closely, or hitting a wall
  // close by
  for (cs = caststates, tent = g_entities, i = 0; i < level.maxclients;
       i++, tent++, cs++) {
    if (!tent->inuse) {
      continue;
    }
    ...
    if (cs->aiState >= AISTATE_COMBAT) { // RF add	// already fighting, not
                                         // interested in bullet impacts
      continue;
    }
    ...
    // are they within radius of the bullet path to hear it travel through the
    // air?
    ...
    if (Distance(vProj, tent->client->ps.origin) >
        0.5 * cs->attributes[INNER_DETECTION_RADIUS]) {
      continue;
    }
  heard:
    ...
    //
    cs->bulletImpactTime =
        level.time + 100 + rand() % 200; // random reaction delay;
...
void AICast_AudibleEvent(int srcnum, vec3_t pos, float range) {
  ...
  for (ent = g_entities, cs = caststates, i = 0; i < level.maxclients;
       i++, cs++, ent++) {
    if (!cs->bs) {
      continue;
    }
    ...
    if (scs) {
      if ((srcnum < level.maxclients) && scs->aiState < AISTATE_COMBAT &&
          !AICast_QueryEnemy(cs, srcnum)) {
        continue;
      }
    }
    ...
    if (localDist >
        adjustedRange * adjustedRange) { // fast out if already outside range
      continue;
    }
    ...
    if (g_debugAudibleEvents.integer) {
      G_Printf("AICast_AudibleEvent heard: %s \"%s\" (dist:%0.0f s:%0.2f "
               "pvss:%0.2f)\n",
               ent->classname, ent->aiName, (sqrt(localDist)),
               cs->attributes[HEARING_SCALE],
               cs->attributes[HEARING_SCALE_NOT_PVS]);
    }

    cs->audibleEventTime =
        level.time + 200 + rand() % 300; // random reaction delay
