...
AIFunc_ZombieFlameAttack()
  ...
  NOTE: this actually uses the EFFECT3 slot for client-side effects (others are
taken)
============
*/
...
char *AIFunc_ZombieFlameAttack(cast_state_t *cs) {
  ...
  /*	disabled, keep going so they cant come back for the easy kill
     ...
     cs->vislist[cs->enemyNum].real_update_timestamp) { ent->s.onFireEnd =
     level.time + 1500; ent->client->ps.torsoTimer = 0;
                  ent->client->ps.legsTimer = 0;
                  return AIFunc_DefaultStart( cs );
          }
  */
  // if outside range, move closer
  if (VectorDistance(cs->bs->origin, cs->vislist[cs->enemyNum].visible_pos) >
      ZOMBIE_FLAME_RADIUS) {
    ent->s.onFireEnd = level.time + 1500;
    ent->client->ps.torsoTimer = 0;
    ent->client->ps.legsTimer = 0;
  ...
  // once an attack has started, only abort once the player leaves our view, or
  // time runs out
...
char *AIFunc_ZombieFlameAttackStart(cast_state_t *cs) {
  ...
  BG_PlayAnimName(&ent->client->ps, "both_attack1", ANIM_BP_BOTH, qtrue, qfalse,
                  qtrue);
...
AIFunc_ZombieAttack2()
  ...
  Character draws the light from surrounding walls (expanding negative light)
and builds up to the release of a flying translucent skull with trail effect
(and beady eyes).

  Spirit should track it's enemy slightly, inflicting lots of damage, removing
sprint bar, and effecting sight temporarily.

  Speed of spirit is effected by skill level, higher skill = faster speed

  Spirits inflicting AI soldiers should kill instantly, removing all flesh from
the soldier's face (draw skull under head model, then fade head model away over
a short period).
============
*/
...
char *AIFunc_ZombieAttack2(cast_state_t *cs) {
  ...
  // if we can't see them anymore, abort immediately
  if (cs->vislist[cs->enemyNum].real_visible_timestamp !=
      cs->vislist[cs->enemyNum].real_update_timestamp) {
    return AIFunc_DefaultStart(cs);
  }
  //
  ...
  if (cs->thinkFuncChangeTime < level.time - ZOMBIE_SPIRIT_BUILDUP_TIME) {
    ...
    if (level.time > cs->thinkFuncChangeTime + ZOMBIE_SPIRIT_BUILDUP_TIME +
                         ZOMBIE_SPIRIT_FADEOUT_TIME) {
      return AIFunc_DefaultStart(cs);
    }
  } else {
    ...
    // TODO
    // ent->client->ps.torsoTimer = 300;	// leave enough time to cancel if we
    // stop coming in here, but stay in the anim if we come back next thing
...
char *AIFunc_ZombieAttack2Start(cast_state_t *cs) {
  ...
  if (lastZombieSpiritAttack <= level.time &&
      lastZombieSpiritAttack > level.time - 1000) {
    return NULL;
  }
  lastZombieSpiritAttack = level.time;
  //
  ...
  //	( ( ent->client->ps.torsoAnim & ANIM_TOGGLEBIT ) ^ ANIM_TOGGLEBIT ) |
  //BOTH_SALUTE;	// FIXME: need a specific anim for this
  //
...
int zombieHitDamage[5] = {16, 16, 16, 12, 20};

#define NUM_ZOMBIE_ANIMS 5
int zombieHitTimes[NUM_ZOMBIE_ANIMS][3] = {
    // up to three hits per attack
    {ANIMLENGTH(11, 20), -1},
    {ANIMLENGTH(9, 20), -1},
    {ANIMLENGTH(9, 20), -1},
...
char *AIFunc_ZombieMelee(cast_state_t *cs) {
  ...
  if (ecs) {
    ...
    anim = (ent->client->ps.torsoAnim & ~ANIM_TOGGLEBIT) -
           BG_AnimationIndexForString("attack1", cs->entityNum);
    ...
    if (zombieHitTimes[anim][cs->animHitCount] >= 0 && cs->animHitCount < 3) {
      ...
      if (!cs->animHitCount) {
        ...
        hitDelay = zombieHitTimes[anim][cs->animHitCount] -
                   zombieHitTimes[anim][cs->animHitCount - 1];
      }

      // check for inflicting damage
      if (level.time - cs->weaponFireTimes[cs->weaponNum] > hitDelay) {
        // do melee damage
        if ((tr = CheckMeleeAttack(
                 ent, AICast_WeaponRange(cs, cs->weaponNum) + 4.0, qfalse)) &&
            (tr->entityNum == cs->enemyNum)) {
          G_Damage(&g_entities[tr->entityNum], ent, ent, vec3_origin,
                   tr->endpos, zombieHitDamage[anim], 0, MOD_GAUNTLET);
          G_AddEvent(
              ent, EV_GENERAL_SOUND,
              G_SoundIndex(
                  aiDefaults[ent->aiCharacter].soundScripts[STAYSOUNDSCRIPT]));
        } else {
          G_AddEvent(ent, EV_GENERAL_SOUND,
                     G_SoundIndex(aiDefaults[ent->aiCharacter]
                                      .soundScripts[FOLLOWSOUNDSCRIPT]));
        }
    ...
    if (!ent->client->ps.legsTimer ||
        zombieHitTimes[anim][cs->animHitCount] < 0 || cs->animHitCount >= 3) {
      // if they are outside range, move forward
      AICast_PredictMovement(ecs, 2, 0.5, &move,
      ...
      if (/*anim != 4 &&*/ (enemyDist > 16)) { // we can get closer
        if (ent->client->ps.legsTimer) {
          ...
          if (cs->castScriptStatus.scriptNoMoveTime <
              level.time + 200) { // dont move until the legs are done lerping
                                  // out of attack anim
...
char *AIFunc_ZombieMeleeStart(cast_state_t *cs) {
  ...
  // play an anim
  BG_UpdateConditionValue(cs->entityNum, ANIM_COND_WEAPON, cs->weaponNum,
                          qtrue);
...
int loperHitTimes[NUM_LOPERMELEE_ANIMS] = {(ANIMLENGTH(3, 15)),
                                           (ANIMLENGTH(6, 15))};
...
char *AIFunc_LoperAttack1(cast_state_t *cs) {
  ...
  if (cs->thinkFuncChangeTime < level.time - loperHitTimes[anim]) {
    ...
    if ((tr = CheckMeleeAttack(&g_entities[cs->entityNum], LOPER_MELEE_RANGE,
                               qfalse))) {
      ...
      if (anim == 0) {
        G_AddEvent(ent, EV_GENERAL_SOUND,
                   G_SoundIndex(aiDefaults[ent->aiCharacter]
                                    .soundScripts[ORDERSDENYSOUNDSCRIPT]));
      } else {
        G_AddEvent(
            ent, EV_GENERAL_SOUND,
            G_SoundIndex(
                aiDefaults[ent->aiCharacter].soundScripts[MISC1SOUNDSCRIPT]));
      }
    }
    cs->weaponFireTimes[WP_MONSTER_ATTACK1] = level.time;
  }
...
char *AIFunc_LoperAttack1Start(cast_state_t *cs) {
  ...
  if (rand() % 2) {
    G_AddEvent(
        ent, EV_GENERAL_SOUND,
        G_SoundIndex(
            aiDefaults[ent->aiCharacter].soundScripts[FOLLOWSOUNDSCRIPT]));
    BG_PlayAnimName(&ent->client->ps, "legs_extra", ANIM_BP_LEGS, qtrue, qfalse,
                    qtrue);
  } else {
    G_AddEvent(ent, EV_GENERAL_SOUND,
               G_SoundIndex(
                   aiDefaults[ent->aiCharacter].soundScripts[STAYSOUNDSCRIPT]));
    BG_PlayAnimName(&ent->client->ps, "legs_extra2", ANIM_BP_LEGS, qtrue,
                    qfalse, qtrue);
  }
  //
...
#define LOPER_LEAP_DELAY                                                       \
  100 // note, this needs to be something, or they'll think they're done before
      // they even started
...
char *AIFunc_LoperAttack2(cast_state_t *cs) {
  ...
  if ((cs->enemyNum >= 0) &&
      (cs->weaponFireTimes[WP_MONSTER_ATTACK2] < level.time - 50) &&
    ...
    if (cs->thinkFuncChangeTime < level.time - LOPER_LEAP_DELAY) {
      ...
      if (VectorDistance(cs->bs->origin,
        ...
        ent->client->ps.eFlags |= EF_MONSTER_EFFECT;
        // do the damage
        G_Damage(&g_entities[cs->enemyNum], ent, ent, vec3_origin,
                 cs->bs->origin, LOPER_LEAP_DAMAGE, 0, MOD_LOPER_LEAP);
  ...
  if (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE) {
    ...
    VectorScale(vec,
                LOPER_LEAP_VELOCITY_START +
                    changeTime *
                        (LOPER_LEAP_VELOCITY_END - LOPER_LEAP_VELOCITY_START),
                vec);
  ...
  if (onGround || (cs->aiFlags & AIFL_LAND_ANIM_PLAYED)) {
    // if we just started the attack recently, we probably haven't had a chance
    // to get airborne yet
    if (cs->thinkFuncChangeTime < level.time - LOPER_LEAP_DELAY) {
      // loper is back on ground, wait for animation to play out
      if (!(cs->aiFlags & AIFL_LAND_ANIM_PLAYED)) {
        BG_PlayAnimName(&ent->client->ps, "legs_extra4", ANIM_BP_LEGS, qtrue,
                        qfalse, qtrue);
        //
      ...
      // keep moving slightly in our facing direction to simulate landing
      // momentum
      AngleVectors(cs->viewangles, vec, NULL, NULL);
      trap_EA_Move(
          cs->entityNum, vec,
          ((float)ent->client->ps.legsTimer / (float)LOPER_LAND_DURATION) *
              (float)LOPER_LEAP_LAND_MOMENTUM);
      return NULL;
    }
  }
...
char *AIFunc_LoperAttack2Start(cast_state_t *cs) {
  ...
  VectorSubtract(g_entities[cs->enemyNum].client->ps.origin, cs->bs->origin,
                 vec);
  VectorNormalize(vec);
  ...
  BG_PlayAnimName(&ent->client->ps, "legs_extra3", ANIM_BP_LEGS, qtrue, qfalse,
                  qtrue);
  ent->client->ps.legsTimer = 500; // stay on this until landing
...
#define LOPER_GROUND_DURATION                                                  \
  (LOPER_GROUND_FRAME_COUNT * (1000 / LOPER_GROUND_FPS))
...
char *AIFunc_LoperAttack3(cast_state_t *cs) {
  ...
  if (cs->thinkFuncChangeTime < level.time - 900) {
    ...
    // draw the client-side lightning effect
    ent->client->ps.eFlags |= EF_MONSTER_EFFECT3;
    // ent->s.effect3Time = level.time + 500;//cs->thinkFuncChangeTime +
    // LOPER_GROUND_DELAY - 200;
    //
    // are we waiting to inflict damage?
    if (cs->weaponFireTimes[WP_MONSTER_ATTACK3] < level.time - 100) {
      // check for damage
      hitClient = G_RadiusDamage(cs->bs->origin, ent, LOPER_GROUND_DAMAGE,
                                 LOPER_GROUND_RANGE, ent, MOD_LOPER_GROUND);
      //
    ...
    //
    if (!hitClient && (ent->client->ps.legsTimer > 400) &&
        cs->thinkFuncChangeTime <
            (level.time - 1000)) {      // we're done with this attack
...
char *AIFunc_LoperAttack3Start(cast_state_t *cs) {
  ...
  AICast_AimAtEnemy(cs);
  // play the animation
  BG_PlayAnimName(&ent->client->ps, "legs_extra5", ANIM_BP_LEGS, qtrue, qfalse,
                  qtrue);
  //
...
#define STIMSOLDIER_FLYJUMP_DURATION                                           \
  (STIMSOLDIER_FLYJUMP_FRAME_COUNT * (1000 / STIMSOLDIER_FLYJUMP_FPS))
...
#define STIMSOLDIER_FLYLAND_DURATION                                           \
  (STIMSOLDIER_FLYLAND_FRAME_COUNT * (1000 / STIMSOLDIER_FLYLAND_FPS))
...
char *AIFunc_StimSoldierAttack1(cast_state_t *cs) {
    ...
    if (ent->s.groundEntityNum != ENTITYNUM_NONE) {
      ...
      if (!(cs->aiFlags & AIFL_LAND_ANIM_PLAYED)) {
        ent->client->ps.legsAnim =
            ((ent->client->ps.legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) |
            STIMSOLDIER_FLYLAND_ANIM;
        ent->client->ps.legsTimer =
            STIMSOLDIER_FLYLAND_DURATION; // stay down until attack is finished
        //
  ...
  if (cs->thinkFuncChangeTime < (level.time - STIMSOLDIER_STARTJUMP_DELAY)) {
    ...
    ent->client->ps.eFlags |=
        EF_MONSTER_EFFECT; // client-side stim engine effect
    if (ent->s.effect1Time !=
        (cs->thinkFuncChangeTime + STIMSOLDIER_STARTJUMP_DELAY)) {
      ent->s.effect1Time =
          (cs->thinkFuncChangeTime + STIMSOLDIER_STARTJUMP_DELAY);
      // start the hovering animation
      ent->client->ps.legsAnim =
          ((ent->client->ps.legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) |
          STIMSOLDIER_FLYHOVER_ANIM;
    }
    // give us some upwards velocity?
    if (cs->thinkFuncChangeTime >
        level.time - STIMSOLDIER_FLYJUMP_DURATION * 0.9) {
...
char *AIFunc_StimSoldierAttack1Start(cast_state_t *cs) {
  ...
  pos[2] += 128;
  trap_Trace(&tr, cs->bs->origin, cs->bs->cur_ps.mins, cs->bs->cur_ps.maxs, pos,
  ...
  if (COM_BitCheck(cs->bs->cur_ps.weapons, WP_PANZERFAUST)) {
    ...
    G_Error("stim soldier tried special jump attack without a tesla or rocket "
            "launcher\n");
  }
  if (!AICast_CheckAttackAtPos(cs->entityNum, cs->enemyNum, pos, qfalse,
                               qfalse)) {
  ...
  ent->client->ps.legsAnim =
      ((ent->client->ps.legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) |
      STIMSOLDIER_FLYJUMP_ANIM;
  ent->client->ps.legsTimer =
      STIMSOLDIER_FLYJUMP_DELAY; // stay down until attack is finished
  //
  cs->aiFlags &= ~AIFL_LAND_ANIM_PLAYED;
  // play the buildup sound
  // TODO
...
char *AIFunc_StimSoldierAttack2(cast_state_t *cs) { return NULL; }
  ...
  if (!(cs->aiFlags & AIFL_MISCFLAG1)) {
    ...
    if (cs->thinkFuncChangeTime < level.time - BLACKGUARD_KICK_DELAY) {
      ...
      if (tr && (tr->entityNum == cs->enemyNum)) {
        ...
        G_Damage(&g_entities[tr->entityNum], ent, ent, fwd, tr->endpos,
                 BLACKGUARD_KICK_DAMAGE, 0, MOD_GAUNTLET);
        // throw them in direction of impact
        fwd[2] = 0.5;
        VectorMA(g_entities[cs->enemyNum].client->ps.velocity, 300, fwd,
...
==============
*/
char *AIFunc_RejectAttack1(cast_state_t *cs) { return NULL; }
...
int warriorHitDamage[5] = {16, 16, 16, 12, 20};

#define NUM_WARRIOR_ANIMS 5
int warriorHitTimes[NUM_WARRIOR_ANIMS][3] = {
    // up to three hits per attack
    {ANIMLENGTH(10, 20), -1}, {ANIMLENGTH(15, 20), -1},
    {ANIMLENGTH(18, 20), -1}, {ANIMLENGTH(15, 20), -1},
    {ANIMLENGTH(14, 20), -1},
};
  ...
  if (ecs) {
    ...
    if (warriorHitTimes[anim][cs->animHitCount] >= 0 && cs->animHitCount < 3) {
      ...
      if (!cs->animHitCount) {
        ...
        hitDelay = warriorHitTimes[anim][cs->animHitCount] -
                   warriorHitTimes[anim][cs->animHitCount - 1];
      }

      // check for inflicting damage
      if (level.time - cs->weaponFireTimes[cs->weaponNum] > hitDelay) {
        // do melee damage
        if ((tr = CheckMeleeAttack(ent, 44, qfalse)) &&
            (tr->entityNum == cs->enemyNum)) {
          G_Damage(&g_entities[tr->entityNum], ent, ent, vec3_origin,
                   tr->endpos, warriorHitDamage[anim], 0, MOD_GAUNTLET);
          G_AddEvent(
              ent, EV_GENERAL_SOUND,
              G_SoundIndex(
                  aiDefaults[ent->aiCharacter].soundScripts[STAYSOUNDSCRIPT]));
...
char *AIFunc_WarriorZombieSightStart(cast_state_t *cs) {
  ...
  // BG_PlayAnimName( &ent->client->ps, "first_sight", ANIM_BP_BOTH, qtrue,
  // qfalse, qtrue );
...
char *AIFunc_WarriorZombieDefense(cast_state_t *cs) {
  ...
  if (cs->thinkFuncChangeTime < level.time - 1500) {
    ...
    if (enemy->client->ps.weapon < WP_LUGER ||
        enemy->client->ps.weapon > WP_CLASS_SPECIAL) {
      ent->flags &= ~FL_DEFENSE_GUARD;
      ent->client->ps.torsoTimer = 0;
      ent->client->ps.legsTimer = 0;
      return NULL;
...
char *AIFunc_WarriorZombieDefenseStart(cast_state_t *cs) {
  ...
  if (lastWarriorDefense <= level.time &&
      lastWarriorDefense > level.time - 3000) {
