...
==========
*/
void AICast_DBG_InitAIFuncs(void) { numaifuncs = 0; }
...
void AICast_DBG_ListAIFuncs(cast_state_t *cs, int numprint) {
  ...
  if (aicast_debug.integer != 2 ||
    ...
    AICast_Printf(AICAST_PRT_DEBUG,
                  S_COLOR_RED
                  "AICast_ProcessAIFunctions: executed more than %d AI funcs\n",
                  MAX_AIFUNCS);
...
void AICast_DBG_RouteTable_f(vec3_t org, char *param) {
  ...
  trap_AAS_SetCurrentWorld(
      0); // use the default world, which should have a routetable
...
#ifdef _WIN32
#undef MAX_PATH		// this is an ugly hack, to temporarily ignore the
current definition, since it's also defined in windows.h #include <windows.h>
#undef MAX_PATH
