...
qboolean G_BounceMissile(gentity_t *ent, trace_t *trace) {
  ...
  if (trace->plane.normal[2] >
      0.2 /*&& VectorLength( ent->s.pos.trDelta ) < 40*/) {
...
void G_MissileImpact(gentity_t *ent, trace_t *trace, int impactDamage,
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    AICast_ProcessBullet(&g_entities[ent->r.ownerNum],
                         g_entities[ent->r.ownerNum].s.pos.trBase,
                         trace->endpos);
  }
  ...
  if (!other->takedamage && (ent->s.eFlags & (EF_BOUNCE | EF_BOUNCE_HALF))) {
    if (G_BounceMissile(ent, trace) &&
        !trace->startsolid) { // no bounce, no bounce sound
      int flags = 0;
      if (!Q_stricmp(ent->classname, "flamebarrel")) {
        ...
        flags =
            (flags >>
             12); // shift right 12 so I can send in parm	(#define SURF_METAL
                  // 0x1000.  metal is lowest flag i need for sound)
        G_AddEvent(ent, EV_GRENADE_BOUNCE,
                   flags); //----(SA)	send surfaceflags for sound selection
      }
    }
    return;
  }
  ...
  if (other->takedamage && other->client) {
    ...
    //); 		vec3_t dir; 		BG_EvaluateTrajectoryDelta( &ent->s.pos, level.time, dir );
...
void G_ExplodeMissile(gentity_t *ent) {
  ...
  if (!zombiespit) {
    ...
    if (ent->parent &&
        !Q_stricmp(ent->parent->classname, "props_flamebarrel")) {
      Msmoke->health = 10;
    } else {
      Msmoke->health = 5;
    }
...
void G_RunMissile(gentity_t *ent) {
  ...
  // DHM - Nerve :: Only in single player
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    AICast_CheckDangerousEntity(ent, DANGER_MISSILE, ent->splashRadius, 0.1,
                                1.0, qtrue);
  }
  ...
  if (tr.fraction != 1) {
    ...
    if (ent->s.weapon == WP_PANZERFAUST) {
      ...
      impactDamage =
          6; // "grenade"/"dynamite"		// probably adjust this based on
             // velocity //----(SA)	adjusted to not break through so much
             // stuff.  try it to see if this is good enough
    }
...
void G_RunSpit(gentity_t *ent) {
  ...
  if (rand() % 100 > 60) {
    ...
    trap_Trace(&tr, ent->r.currentOrigin, NULL, NULL, end, ent->r.ownerNum,
               MASK_SHOT);
...
gentity_t *fire_grenade(gentity_t *self, vec3_t start, vec3_t dir,
  ...
  if (self->client && self->client->ps.grenadeTimeLeft) {
    if (grenadeWPID ==
        WP_DYNAMITE) { // remove any fraction of a 5 second 'click'
      self->client->ps.grenadeTimeLeft *= 5;
      self->client->ps.grenadeTimeLeft -=
          (self->client->ps.grenadeTimeLeft % 5000);
      self->client->ps.grenadeTimeLeft += 5000;
      //			if(self->client->ps.grenadeTimeLeft < 5000)
      //// allow dropping of dynamite that won't explode (for shooting)
      //				noExplode = qtrue;
    }
  ...
  case WP_DYNAMITE:
    ...
    trap_SendServerCommand(self - g_entities,
                           va("dp %d", (bolt->nextthink - level.time) / 1000));
    // JPW NERVE
    if (g_gametype.integer != GT_SINGLE_PLAYER) {
      // check if player is in trigger objective field -- swiped from
      // G_TouchTriggers()
        ...
        if (!strcmp(hit->classname, "trigger_objective_info")) {
          if (hit->track) {
            trap_SendServerCommand(
                -1,
                va("cp \"%s\"", va("Det charge planted near %s!", hit->track)));
          } else {
            trap_SendServerCommand(
                -1, va("cp \"%s\"", va("Det charge planted near objective %d!",
                                       hit->count)));
          }
          i = num;
        }
    ...
    if (g_gametype.integer == GT_SINGLE_PLAYER) {
      //				bolt->health = 5; 				bolt->takedamage
      //= qtrue;
      //
      //				// small target cube
      //				VectorSet(bolt->r.mins, -4, -4, 0);
      //				VectorCopy(bolt->r.mins,
      //bolt->r.absmin); 				VectorSet(bolt->r.maxs, 4, 4, 8);
      //				VectorCopy(bolt->r.maxs,
      //bolt->r.absmax);
    } else {
      bolt->health = 5;
    ...
    //			bolt->r.ownerNum			= ENTITYNUM_WORLD;	// (SA) make the world the
    //owner of the dynamite, so the player can shoot it without modifying the
    //bullet code to ignore players id for hits
...
gentity_t *fire_zombiespirit(gentity_t *self, gentity_t *bolt, vec3_t start,
  ...
  bolt->s.pos.trType = TR_INTERPOLATE; // we'll move it manually, since it needs
                                       // to track it's enemy
...
void fire_lead(gentity_t *self, vec3_t start, vec3_t dir, int damage) {
  ...
  if (traceEnt->takedamage) {
    ...
    if (self->s.weapon == WP_SNIPER && traceEnt->s.eType == ET_MOVER &&
        traceEnt->aiName[0]) {
