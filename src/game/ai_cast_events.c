...
void AICast_Sight(gentity_t *ent, gentity_t *other, int lastSight) {
  ...
  //
  // call the sightfunc for this cast, so we can play associated sounds, or do
  // any character-specific things
  //
  if (cs->sightfunc) {
  ...
  if (other->aiName && other->health <= 0) {
    ...
    if (ocs->deathTime > lastSight) {
      ...
      } else if (!(cs->castScriptStatus.scriptFlags &
                   SFL_FRIENDLYSIGHTCORPSE_TRIGGERED)) {
...
void AICast_Pain(gentity_t *targ, gentity_t *attacker, int damage,
  ...
  // process the event (turn to face the attacking direction? go into
  // hide/retreat state?) need to weigh up the situation, but foremost, an
  // inactive AI cast should always react in some way to being hurt
  cs->lastPain = level.time;

  // record the sighting (FIXME: silent weapons shouldn't do this, but the AI
  ...
  AICast_ScriptEvent(cs, "pain",
                     va("%d %d", targ->health, targ->health + damage));
  ...
  //
  // call the painfunc for this cast, so we can play associated sounds, or do
  // any character-specific things
  //
  if (cs->painfunc) {
...
void AICast_Die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
  ...
  if (self->aiCharacter == AICHAR_ZOMBIE && cs->secondDeadTime) {
    if (cs->secondDeadTime > 1) {
      ...
      self->health +=
          damage; // don't drop below gib_health if we weren't already below it
      return;
    }
    /*
                    if (!cs->rebirthTime)
                    {
                            ...
                            // while waiting for rebirth, we only "die" if we
       drop below gib health return;
                    }
    */
    // always gib
    self->health = -999;
  ...
  if (self->aiCharacter == AICHAR_ZOMBIE && damage > 20 &&
      inflictor != attacker) {
    self->health = -999;
    damage = 999;
  }
  ...
  if (self->client->ps.pm_type == PM_DEAD) {
    ...
    if (self->health < GIB_HEALTH) {
      if (self->aiCharacter == AICHAR_ZOMBIE) {
        ...
        nogib = qfalse;
        /*
                                        // Zombie has special exploding cloud
           effect if (attacker != inflictor || attacker->s.weapon == WP_VENOM)
                                        {
                                                GibEntity( self, killer );
                                                nogib = qfalse;
                                        } else {
                                                // Zombie will decompose upon
           dying self->client->ps.eFlags |= EF_MONSTER_EFFECT2;
                                                self->s.effect2Time =
           level.time+200; self->health = -1;
                                        }
        */
    ...
    // make sure the client doesn't forget about this entity until it's set to
    // "dead" frame otherwise it might replay it's death animation if it goes
    // out and into client view
    ...
    // if we are a zombie, and lying down during our first death, then we should
    // just die
    if (!(self->aiCharacter == AICHAR_ZOMBIE && cs->secondDeadTime &&
          cs->rebirthTime)) {

      // set enemy weapon
      BG_UpdateConditionValue(self->s.number, ANIM_COND_ENEMY_WEAPON, 0,
                              qfalse);
      if (attacker->client) {
        BG_UpdateConditionValue(self->s.number, ANIM_COND_ENEMY_WEAPON,
                                inflictor->s.weapon, qtrue);
      } else {
        BG_UpdateConditionValue(self->s.number, ANIM_COND_ENEMY_WEAPON, 0,
                                qfalse);
      }
      ...
      // set enemy location
      BG_UpdateConditionValue(self->s.number, ANIM_COND_ENEMY_POSITION, 0,
                              qfalse);
      if (infront(self, inflictor)) {
        BG_UpdateConditionValue(self->s.number, ANIM_COND_ENEMY_POSITION,
      ...
      // set gib delay
      if (cs->aiCharacter == AICHAR_HEINRICH ||
          cs->aiCharacter == AICHAR_HELGA) {
    ...
    if (cs->aiCharacter == AICHAR_WARZOMBIE) {
      ...
      if (!Q_strncmp(mapname, "end",
                     3)) { // !! FIXME: post beta2, make this a spawnflag!
  ...
  if (!cs->rebirthTime) {
    ...
    // call the deathfunc for this cast, so we can play associated sounds, or do
    // any character-specific things
    if (!(cs->aiFlags & AIFL_DENYACTION) && cs->deathfunc) {
      cs->deathfunc(self, attacker, damage,
                    meansOfDeath); //----(SA)	added mod
    }
  } else {
    // really dead now, so call the script
...
===============
*/
void AICast_AIDoor_Touch(gentity_t *ent, gentity_t *aidoor_trigger,
                         gentity_t *door) {
  cast_state_t *cs, *ocs;
  gentity_t *trav;
  int i;
  ...
  for (trav = NULL;
       (trav = G_Find(trav, FOFS(target), aidoor_trigger->targetname));) {
    // make sure the marker is vacant
    trap_Trace(&tr, trav->r.currentOrigin, ent->r.mins, ent->r.maxs,
               trav->r.currentOrigin, ent->s.number, ent->clipmask);
    if (tr.startsolid) {
      continue;
    }
    // search all other AI's, to see if they are heading for this marker
    for (i = 0, ocs = AICast_GetCastState(0); i < aicast_maxclients;
         i++, ocs++) {
      if (!ocs->bs) {
        continue;
      }
    ...
    trap_Trace(&tr, ent->r.currentOrigin, mins, ent->r.maxs,
               trav->r.currentOrigin, ent->s.number, ent->clipmask);
    if (tr.fraction < 1.0) {
      continue;
    }
...
void AICast_ProcessActivate(int entNum, int activatorNum) {
  ...
  if (cs->castScriptStatus.castScriptEventIndex >= 0) {
    if (ent->eventTime != level.time) {
      G_AddEvent(
          &g_entities[entNum], EV_GENERAL_SOUND,
          G_SoundIndex(
              aiDefaults[cs->aiCharacter].soundScripts[ORDERSDENYSOUNDSCRIPT]));
    }
    return;
  }
  ...
  if (cs->leaderNum == activatorNum) {
    if (ent->eventTime != level.time) {
      G_AddEvent(
          &g_entities[entNum], EV_GENERAL_SOUND,
          G_SoundIndex(
              aiDefaults[cs->aiCharacter].soundScripts[STAYSOUNDSCRIPT]));
    }
    ...
    for (count = 0, i = 0, tcs = caststates; i < level.maxclients; i++, tcs++) {
      if (tcs->bs && tcs != cs && tcs->entityNum != activatorNum &&
          g_entities[tcs->entityNum].health > 0 &&
          tcs->leaderNum == activatorNum) {
        count++;
      }
    }
    if (count >= 3) {
      if (ent->eventTime != level.time) {
        G_AddEvent(&g_entities[entNum], EV_GENERAL_SOUND,
    ...
    if (ent->eventTime != level.time) {
      G_AddEvent(
          &g_entities[entNum], EV_GENERAL_SOUND,
          G_SoundIndex(
              aiDefaults[cs->aiCharacter].soundScripts[FOLLOWSOUNDSCRIPT]));
    }

    // if they have a wait goal, free it
    if (cs->followEntity >= MAX_CLIENTS &&
        g_entities[cs->followEntity].classname &&
        !strcmp(g_entities[cs->followEntity].classname, "AI_wait_goal")) {
