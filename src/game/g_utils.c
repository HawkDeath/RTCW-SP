  ...
  while ((t = G_Find(t, FOFS(targetname), ent->target)) != NULL) {
    if (t == ent) {
      ...
      if (t->use) {
        ...
        t->flags |=
            (ent->flags & FL_SOFTACTIVATE); // (SA) likewise for soft activation
...
qboolean infront(gentity_t *self, gentity_t *other) {
  ...
  if (self->activateArc) {
    // move the origin of the 'other' up/down so that it matches the 'self' so
    // the check is along a horizontal plane
  ...
  if (!other->aiCharacter &&
      self->activateArc) { //----(SA)	make sure ai's aren't constrained to the
                           //grabarc of an mg42
    float angle;
    angle = RAD2DEG(M_PI - acos(dot));
    if (angle <
        (self->activateArc * 2.0)) { // arc is 'half arc' since that's the way
                                     // the other angles in the mg42 were done
      return qfalse;
    } else {
      return qtrue;
    }
