...
void AICast_ChangeViewAngles(cast_state_t *cs, float thinktime) {
  ...
  if (cs->aiFlags & AIFL_VIEWLOCKED) {
    ...
    if (cs->castScriptStatus.playAnimViewlockTime >= level.time) {
      ...
      if (!g_entities[cs->entityNum].client->ps.legsTimer) {
        ...
        VectorCopy(cs->castScriptStatus.playanim_viewangles,
                   cs->ideal_viewangles);
      }
    }
  }
  ...
  if (cs->lockViewAnglesTime < level.time) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      cs->viewangles[i] = BotChangeViewAngle(
          cs->viewangles[i], cs->ideal_viewangles[i], anglespeed);
      // BotAI_Print(PRT_MESSAGE, "ideal_angles %f %f\n",
      // cs->ideal_viewangles[0], cs->ideal_viewangles[1],
      // cs->ideal_viewangles[2]);` cs->viewangles[i] = cs->ideal_viewangles[i];
    }
  }
...
void AICast_InputToUserCommand(cast_state_t *cs, bot_input_t *bi,
  ...
  // crouch/movedown
  if (aiDefaults[cs->aiCharacter]
          .attributes[ATTACK_CROUCH]) { // only crouch if this character is
                                        // physically able to
  ...
  if (bi->actionflags & ACTION_ATTACK) {
    ...
    if ((cs->triggerReleaseTime < level.time) &&
        ((cs->lockViewAnglesTime >= level.time) ||
         (fabs(AngleDifference(cs->ideal_viewangles[YAW],
                               cs->viewangles[YAW])) < 20)) &&
        // check for radid luger firing by skilled users (release fire between
        // shots)
        (((level.time + cs->entityNum * 500) / 2000) % 2 ||
         !(rand() % (1 + g_gameskill.integer)) ||
         (cs->attributes[ATTACK_SKILL] < 0.5) || (cs->weaponNum != WP_LUGER) ||
         (cs->bs->cur_ps.weaponTime == 0) || (cs->bs->cur_ps.releasedFire))) {
  ...
  if (cs->aiState <= AISTATE_QUERY) {
    if (WEAPS_ONE_HANDED &
        (1 << ucmd->weapon)) { // one-handed wepons don't draw, others do
      ucmd->weapon = WP_NONE;
    }
  }
  //----(SA)	end
  ...
  if (bi->speed) {
    if ((!(cs->aiFlags & AIFL_WALKFORWARD) && cs->enemyNum >= 0) ||
        ((ucmd->forwardmove >= 0) &&
         fabs(AngleNormalize180(AngleDifference(cs->ideal_viewangles[YAW],
                                                cs->viewangles[YAW]))) < 60)) {
      ...
      if (!ucmd->upmove) { // only change it if we don't already have an upmove
                           // set
        ucmd->upmove = DotProduct(up, bi->dir) * bi->speed;
      }
      // if (!ucmd->upmove)	// only change it if we don't already have an
      // upmove set 	ucmd->upmove = abs(forward[2]) * bi->dir[2] * bi->speed;
    }
  }
  //
  ...
  if (!(cs->aiFlags & AIFL_WALKFORWARD) ||
      (!cs->bs->cur_ps.groundEntityNum ||
       cs->bs->cur_ps.groundEntityNum ==
           ENTITYNUM_NONE)) { // only do other movements if we are allowed to
...
void AICast_UpdateInput(cast_state_t *cs, int time) {
  ...
  for (j = 0; j < 3; j++) {
    cs->viewangles[j] =
        AngleMod(cs->viewangles[j] + SHORT2ANGLE(bs->cur_ps.delta_angles[j]));
  }
  //
  ...
  // (cs->attributes[WALKING_SPEED] + 50 < cs->attributes[RUNNING_SPEED] ? 50 :
  // -1)))	// do a fast shuffle if slightly over walking speed
  if (speed <= cs->attributes[WALKING_SPEED]) {
    cs->actionFlags |= CASTACTION_WALK;
  }
  //
  // we use 300 here, because the default player speed is 300, so Cast AI's
  // can't move faster than that
  ...
  if (bi.speed <= (400.0 / 300.0) * (cs->attributes[WALKING_SPEED] +
                                     (cs->attributes[WALKING_SPEED] + 50 <
                                              cs->attributes[RUNNING_SPEED]
                                          ? 50
                                          : -1))) {
    cs->actionFlags |= CASTACTION_WALK;
  }
  ...
  //
  // make sure the respawn flag is disabled (causes problems after multiple "map
  // xxx" commands)
  g_entities[cs->bs->entitynum].client->ps.pm_flags &= ~PMF_RESPAWNED;
  // set the aiState
...
void AICast_Think(int client, float thinktime) {
  ...
  //
  // make sure we are using the right AAS data for this entity (one's that don't
  // get set will default to the player's AAS data)
  ...
  ent->client->ps.eFlags &= ~(EF_NOSWINGANGLES | EF_MONSTER_EFFECT |
                              EF_MONSTER_EFFECT2 | EF_MONSTER_EFFECT3);
  // conditional flags
  ...
  if (ent->health <= 0 || cs->revivingTime || cs->rebirthTime) {
    ...
    if (cs->rebirthTime && cs->rebirthTime < level.time) {
      ...
      if (numTouch) {
        for (i = 0; i < numTouch; i++) {
          // if (!g_entities[touch[i]].client || g_entities[touch[i]].r.contents
          // == CONTENTS_BODY)
      ...
      if (numTouch == 0) { // ok to spawn
        ...
        ent->health = ent->client->ps.stats[STAT_HEALTH] =
            ent->client->ps.stats[STAT_MAX_HEALTH] =
                ((cs->attributes[STARTING_HEALTH] - 50) > 30
    ...
    //
    if (ent->health > GIB_HEALTH && cs->deathTime &&
        cs->deathTime < (level.time - 1000)) {
      // ent->r.svFlags &= ~SVF_BROADCAST;
  ...
  if (cs->secondDeadTime) {
    ...
    BG_UpdateConditionValue(cs->entityNum, ANIM_COND_SECONDLIFE, qfalse,
                            qfalse);
  }
  // set health value
  ...
  if (cs->enemyNum >= 0) {
    if (infront(ent, &g_entities[cs->enemyNum])) {
      BG_UpdateConditionValue(ent->s.number, ANIM_COND_ENEMY_POSITION,
                              POSITION_INFRONT, qtrue);
    } else {
      BG_UpdateConditionValue(ent->s.number, ANIM_COND_ENEMY_POSITION,
                              POSITION_BEHIND, qtrue);
    }
  } else {
    BG_UpdateConditionValue(ent->s.number, ANIM_COND_ENEMY_POSITION,
                            POSITION_UNUSED, qtrue);
  }
  // set defense pose
  ...
  // walking
  animIndex = BG_GetAnimScriptAnimation(cs->entityNum, ent->client->ps.aiState,
                                        ANIM_MT_WALK);
  if (animIndex >= 0) {
  ...
  for (i = 0; i < 3; i++) {
    cs->viewangles[i] = AngleMod(cs->viewangles[i] +
                                 SHORT2ANGLE(cs->bs->cur_ps.delta_angles[i]));
  }
  //
  ...
  if (    ( cs->attackcrouch_time >= level.time ) /*&&
    ...
    // if we are not moving, and we are firing, always stand, unless we are
    // allowed to crouch + fire
    if ((cs->lastucmd.forwardmove || cs->lastucmd.rightmove) ||
        (cs->lastWeaponFired < level.time - 2000) ||
  ...
  if (!(COM_BitCheck(cs->bs->cur_ps.weapons, cs->weaponNum)) ||
      !AICast_GotEnoughAmmoForWeapon(cs, cs->weaponNum)) {
    // select a weapon
  ...
  //
  // in query mode, we do special handling (pause scripting, check for
  // transition to alert/combat, etc)
  if (cs->aiState == AISTATE_QUERY) {
  ...
  //
  // any anim playing on legs should prevent turn anims being played in the
  // cgame
...
void AICast_StartFrame(int time) {
  ...
  // don't let the SIGHTING framerate drop below 10 (too much sighting to
  // process at once)
  if (elapsed > 100) {
    elapsed = 100;
  }
  ...
  for (i = lastthink, ent = &g_entities[lastthink];
       clCount < level.numPlayingClients && count < aicast_maxthink;
       i++, ent++) {
    ...
    if (cs->bs) {
      if (ent->aiInactive == qfalse) {
        ...
        if ((elapsed && cs->scriptAnimTime && cs->scriptAnimTime >= (level.time - 1000)) || ((elapsed >=
                                                                                              50) &&
                                                                                             ((((!VectorCompare(
                                                                                                    ent->client
                                                                                                        ->ps
          ...
          cs->lastThink =
              time + rand() % 20; // randomize this slightly to spread out
                                  // thinks during high framerates
          //
...
void AICast_StartServerFrame(int time) {
  ...
  for (i = 0, ent = g_entities;
       i < level.maxclients /*&& clCount < level.numPlayingClients*/;
       i++, ent++) {
    ...
    if (cs->bs) {
      if (ent->aiInactive == qfalse && ent->inuse) {
        ...
        if (cs->lastThink && elapsed > 0) {
          ...
          //
          // optimization, if they're not in the player's PVS, and they aren't
          // trying to move, then don't bother thinking
          if ((highPriority && (elapsed > 300)) ||
              (g_entities[0].client && g_entities[0].client->cameraPortal) ||
              (highPriority && (cs->vislist[0].visible_timestamp ==
                                cs->vislist[0].lastcheck_timestamp)) ||
              (highPriority &&
               (pcs->vislist[cs->entityNum].visible_timestamp ==
                pcs->vislist[cs->entityNum].lastcheck_timestamp)) ||
              (VectorLength(ent->client->ps.velocity) > 0) ||
              (highPriority &&
               (cs->lastucmd.forwardmove || cs->lastucmd.rightmove ||
                cs->lastucmd.upmove > 0 || cs->lastucmd.buttons ||
                cs->lastucmd.wbuttons))
              // !!NOTE: always allow thinking if in PVS, otherwise bosses won't
              // gib, and dead guys might not push away from clipped walls
              ||
              (trap_InPVS(cs->bs->origin,
                          g_entities[0].s.pos.trBase))) { // do pvs check last,
                                                          // since it's the most
                                                          // expensive to call
            ...
            if (oldLegsTimer < ent->client->ps.legsTimer &&
              ...
              if (cs->castScriptStatus.scriptNoMoveTime <
                  level.time + ent->client->ps.legsTimer) {
                cs->castScriptStatus.scriptNoMoveTime =
                    level.time + ent->client->ps.legsTimer;
              }
            }
          }
...
==============
*/
void AICast_PredictMovement(cast_state_t *cs, int numframes, float frametime,
                            aicast_predictmove_t *move, usercmd_t *ucmd,
                            int checkHitEnt) {
  int frame, i;
  playerState_t ps;
  pmove_t pm;
  ...
  if (checkHitEnt >= 0 &&
      !Q_stricmp(g_entities[checkHitEnt].classname, "ai_marker")) {
    checkReachMarker = qtrue;
    VectorSubtract(g_entities[checkHitEnt].r.currentOrigin, ps.origin,
                   startHitVec);
  ...
  for (frame = 0; frame < numframes; frame++) {
    ...
    if (checkHitEnt >= 0) {
      ...
      if (checkReachMarker) {
        VectorSubtract(g_entities[checkHitEnt].r.currentOrigin, pm.ps->origin,
                       thisHitVec);
        if (DotProduct(startHitVec, thisHitVec) < 0) {
          // project the marker onto the movement vec, and check distance
          ProjectPointOntoVector(g_entities[checkHitEnt].r.currentOrigin,
                                 lastOrg, pm.ps->origin, projPoint);
          if (VectorDistance(g_entities[checkHitEnt].r.currentOrigin,
                             projPoint) < 8) {
      ...
      for (i = 0; i < pm.numtouch; i++) {
        ...
        if (pm.touchents[i] == checkHitEnt) {
          ...
          goto done;
                   ...
                   (pm.touchents[i] != ENTITYNUM_WORLD &&
                    (g_entities[pm.touchents[i]].s.eType != ET_MOVER ||
                     g_entities[pm.touchents[i]].moverState != MOVER_POS1))) {
          // we have hit another entity, so abort
          move->stopevent = PREDICTSTOP_HITCLIENT;
          goto done;
        } else if (!Q_stricmp(g_entities[pm.touchents[i]].classname,
                              "script_mover")) {
      ...
      // if we are trying to get to something, we should keep adjusting our
      // input to head towards them
  ...
  // hack, if we are above ground, chances are it's because we only did one
  // frame, and gravity isn't applied until after the frame, so try and drop us
  // down some
  if (move->groundEntityNum == ENTITYNUM_NONE) {
    VectorCopy(move->endpos, end);
    end[2] -= 32;
    trap_Trace(&tr, move->endpos, pm.mins, pm.maxs, end, pm.ps->clientNum,
               pm.tracemask);
...
============
*/
qboolean AICast_GetAvoid(cast_state_t *cs, bot_goal_t *goal, vec3_t outpos,
                         qboolean reverse, int blockEnt) {
  ...
  // if they have an enemy, and can currently see them, don't move out of their
  // view
  enemyVisible =
      (cs->enemyNum >= 0) && (AICast_CheckAttack(cs, cs->enemyNum, qfalse));
  //
  ...
  if (goal) {
    starttraveltime = trap_AAS_AreaTravelTimeToGoalArea(
        cs->bs->areanum, cs->bs->origin, goal->areanum, cs->travelflags);
  }
  ...
  if (cs->dangerEntity >= 0 && cs->dangerEntityValidTime >= level.time) {
    ...
    averting = qtrue; // not heading for a goal, so we must be getting out of
                      // someone's way
  }
  //
  maxYaw = 0;
  ...
  for (yaw = -angleDiff * invert; yaw * invert <= maxYaw; yaw += inc * invert) {
    ...
    if (cs->dangerEntity >= 0 && cs->dangerEntityValidTime >= level.time) {
      ...
      // distmoved = 99999 - trap_AAS_AreaTravelTimeToGoalArea(
      // BotPointAreaNum(castmove.endpos), castmove.endpos, goal->areanum,
      // cs->travelflags );
    ...
    if ((distmoved > bestmoved)
        //&&	((cs->bs->origin[2] - castmove.endpos[2]) < 64)	// allow up, but
        //not down (falling)
        && (castmove.groundEntityNum != ENTITYNUM_NONE)) {
      // they all passed, check any other stuff
      if (!enemyVisible ||
          AICast_CheckAttackAtPos(cs->entityNum, cs->enemyNum, castmove.endpos,
                                  qfalse, qfalse)) {
        if (!goal ||
            (traveltime = trap_AAS_AreaTravelTimeToGoalArea(
                 BotPointAreaNum(castmove.endpos), castmove.endpos,
                 goal->areanum, cs->travelflags)) < (starttraveltime + 200)) {
          bestyaw = yaw;
          bestmoved = distmoved;
...
============
*/
void AICast_Blocked(cast_state_t *cs, bot_moveresult_t *moveresult,
  ...
  if (cs->blockedAvoidTime < level.time) {
    if (cs->blockedAvoidTime < level.time - 300) {
      ...
      AICast_PredictMovement(cs, 1, 0.6, &move, &ucmd,
                             (goal && goal->entitynum > -1) ? goal->entitynum
                                                            : cs->entityNum);

      // blocked if we hit a client (or non-stationary mover) other than our
      // enemy or goal
      ...
      for (i = 0; i < move.numtouch; i++) {
        if (move.touchents[i] >= MAX_CLIENTS) {
          if (!Q_stricmp(g_entities[move.touchents[i]].classname,
                         "script_mover")) {
            // avoid script_mover's
          ...
          // if we are close to the impact point, then avoid this entity
            ...
            // g_entities[move.touchents[i]].classname );
            blockEnt = move.touchents[i];
          }
          continue;
        }
        ...
        // reject this blocker if we are following or going to them
          ...
          //
          // if they recently were asked to avoid us, then they're probably not
          // listening
          ...
          if (ocs->leaderNum >= 0) {
            ...
            VectorCopy(g_entities[ocs->leaderNum].r.currentOrigin,
                       ogoal.origin);
            if (AICast_GetAvoid(ocs, &ogoal, ocs->obstructingPos, qfalse,
                                cs->entityNum)) {
              // give them time to move somewhere else
              ocs->obstructingTime = level.time + 1000;
            } else {
              // make sure they don't call GetAvoid() for another few frames to
              // let others avoid also
              ocs->obstructingTime = level.time - 1;
              blockEnt = move.touchents[i];
            }
          } else {
            if (AICast_GetAvoid(ocs, NULL, ocs->obstructingPos, qfalse,
                                cs->entityNum)) {
              // give them time to move somewhere else
              ocs->obstructingTime = level.time + 1000;
            } else {
    ...
    if (g_entities[blockEnt].aiName && g_entities[blockEnt].client && cs->bs &&
        VectorDistance(cs->bs->origin, g_entities[blockEnt].r.currentOrigin) <
            128) {
      ...
      if ((oldId != cs->castScriptStatus.scriptId) ||
        ...
        // the script has changed, so assume the scripting is handling the
        // avoidance
        return;
      }
    }
    ...
    if (/*blockEnt > MAX_CLIENTS &&*/ AICast_GetAvoid(cs, goal, pos, qfalse,
                                                      blockEnt)) {
      ...
      cs->blockedAvoidTime =
          level.time - 1; // don't look again for another few frames
      return;
    }
  }
...
AICast_EvaluatePmove
  ...
  Avoidance after the event (leaders instruct AI's to get out the way, AI's
instruct other non-moving AI's to get out the way)
================
*/
void AICast_EvaluatePmove(int clientnum, pmove_t *pm) {
  ...
  // NOTE: this is only enabled for real clients, so their followers get out of
  // their way
  // if (cs->bs)
  //	return;

  // look through the touchent's to see if we've bumped into something we should
  // avoid, or react to
  for (i = 0; i < pm->numtouch; i++) {
    // mark the time, so they can deal with the obstruction in their own think
    // functions
    ...
    // if they are an AI Cast, inform them of our disposition, and hope that
    // they are reasonable enough to assist us in our desire to move beyond our
    // current position
    if (pm->touchents[i] < aicast_maxclients) {
      ...
      AICast_UpdateVisibility(&g_entities[pm->touchents[i]],
                              &g_entities[cs->entityNum], qfalse, qtrue);

      ocs = AICast_GetCastState(pm->touchents[i]);
      if ((ocs->bs) && (AICast_SameTeam(cs, ocs->entityNum)) &&
          (!(ocs->aiFlags & AIFL_NOAVOID)) &&
          ((ocs->leaderNum == cs->entityNum) ||
           (VectorLength(ocs->bs->velocity) < 5)) &&
          (ocs->obstructingTime < (level.time + 100))) {
        // if they are moving away from us already, let them go
        if (VectorLength(ocs->bs->cur_ps.velocity) > 10) {
          vec3_t v1, v2;

          VectorSubtract(ocs->bs->origin,
                         g_entities[clientnum].client->ps.velocity, v2);
          VectorNormalize(v2);
          VectorNormalize2(ocs->bs->cur_ps.velocity, v1);
        ...
        if (ocs->leaderNum >= 0) {
          ...
          if (ocs->bs &&
              AICast_GetAvoid(
                  ocs, &ogoal, ocs->obstructingPos, qfalse,
                  cs->entityNum)) { // give them time to move somewhere else
            ocs->obstructingTime = level.time + 1000;
          }
        } else {
          if (ocs->bs &&
              AICast_GetAvoid(
                  ocs, NULL, ocs->obstructingPos, qfalse,
                  cs->entityNum)) { // give them time to move somewhere else
            ocs->obstructingTime = level.time + 1000;
          }
        }
      ...
      ent = pm->touchents[i];
      if (g_entities[ent].s.modelindex > 0 &&
          g_entities[ent].s.eType == ET_MOVER) {
...
qboolean AICast_RequestCrouchAttack(cast_state_t *cs, vec3_t org, float time) {
  if (cs->attributes[ATTACK_CROUCH] > 0 &&
      AICast_CheckAttackAtPos(cs->entityNum, cs->enemyNum, org, qtrue,
                              qfalse)) {
    if (time) {
...
void AICast_QueryThink(cast_state_t *cs) {
  ...
  visible = AICast_VisibleFromPos(cs->bs->origin, cs->entityNum,
                                  g_entities[cs->enemyNum].r.currentOrigin,
                                  cs->enemyNum, qfalse);

  // make sure we dont process the sighting of this enemy by going into query
  // mode again, without them being visible again after we leave here
  ...
  if ((cs->queryAlertSightTime < 0) ||
      ((cs->queryAlertSightTime < level.time) && visible)) {
    if (!cs->queryAlertSightTime) {
      // set the "short reaction" condition
      BG_UpdateConditionValue(cs->entityNum, ANIM_COND_SHORT_REACTION, qtrue,
                              qfalse);
    }
    AICast_StateChange(cs, AISTATE_COMBAT);
    BG_UpdateConditionValue(cs->entityNum, ANIM_COND_SHORT_REACTION, qfalse,
                            qfalse);
...
void AICast_DeadClipWalls(cast_state_t *cs) {
  /*
          ...
          //anim = BG_GetAnimationForIndex( cs->entityNum,
     (cs->bs->cur_ps.torsoAnim & ~ANIM_TOGGLEBIT) );
          ...
          trap_Trace( &tr, src, vec3_origin, vec3_origin, or.origin,
     cs->entityNum, MASK_SOLID );
          ...
          if (!tr.startsolid && !tr.allsolid && tr.fraction < 1.0) {
                  ...
                  vel[2] = 0;
                  VectorAdd( g_entities[cs->entityNum].client->ps.velocity, vel,
     g_entities[cs->entityNum].client->ps.velocity );
          }
  */
}
...
void AICast_IdleReload(cast_state_t *cs) {
  ...
  if (!((cs->bs->cur_ps.ammoclip[BG_FindClipForWeapon(cs->bs->cur_ps.weapon)] <
         (int)(0.75 * ammoTable[cs->bs->cur_ps.weapon].maxclip)) &&
        cs->bs->cur_ps.ammo[BG_FindAmmoForWeapon(cs->bs->cur_ps.weapon)])) {
    return;
  }
  //
