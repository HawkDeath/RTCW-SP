...
void InitTramcar(gentity_t *ent) {
  ...
  if (!Q_stricmp(ent->classname, "truck_cam")) {
    ent->s.modelindex2 =
        G_ModelIndex("models/mapobjects/vehicles/truck_base.md3");
  }
...
void Calc_Roll(gentity_t *ent) {
  ...
  VectorSubtract(target->nextTrain->nextTrain->s.origin, ent->r.currentOrigin,
                 vec);
  VectorNormalize(vec);
...
void Reached_Tramcar(gentity_t *ent) {
  ...
  if (!Q_stricmp(ent->classname, "props_me109")) {
    ...
    } else if ((next->spawnflags & 1) && !(next->count) &&
               ent->health > 0) { // SCRIPT flag
      GetNextTrack(ent);
      Think_SetupAirplaneWaypoints(ent);
    } else if ((next->spawnflags & 2) && (ent->spawnflags & 8) &&
               ent->health <= 0 && ent->takedamage) { // death path
    ...
    VectorSubtract(ent->nextTrain->nextTrain->s.origin, ent->r.currentOrigin,
                   vec);
    ...
    if (next->spawnflags & 2) { // END
      ...
      if (next->spawnflags & 4) { // reverse
        ...
        VectorSubtract(ent->r.currentOrigin,
                       ent->nextTrain->nextTrain->s.origin, vec);
      } else {
        ent->props_frame_state = truck_moving;
        VectorSubtract(ent->nextTrain->nextTrain->s.origin,
                       ent->r.currentOrigin, vec);
      }
...
If you are going to have enemies ride the tramcar it must be placed in its
ending position when you bsp the map you can the start it by targeting the
desired path_corner
...
per 25 of mass (up to 16).  So 800 gives the most.
*/
...
/*QUAKED plane_waypoint (.5 .3 0) (-8 -8 -8) (8 8 8) SCRIPTS DIE EXPLODE LAPS
ATTACK "count" number of times this waypoint needs to be triggered by an
AIScript "alertentity" call before the aircraft changes tracks "track"	tells it
what track to branch off to there can be several track with the same track name
the program will pick one randomly there can be a maximum of eight tracks at any
branch
...
/*QUAKED props_me109 (.7 .3 .1) (-128 -128 0) (128 128 64) START_ON TOGGLE
SPINNINGPROP FIXED_DIE default health = 1000
*/
...
void props_me109_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                     int damage, int mod) {
  G_Printf("dead\n");
...
void props_me109_pain(gentity_t *self, gentity_t *attacker, int damage,
                      vec3_t point) {
  vec3_t temp;
...
void SP_truck_cam(gentity_t *self) {
  ...
  truck_sound =
      G_SoundIndex("sound/vehicles/start_drive_grind_gears_01_11k.wav");
...
"track" is the targetname of the entity providing the starting direction use an
info_notnull
*/
...
void screen_fade_use(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  if (ent->spawnflags & 1) {
    ...
    trap_SetConfigstring(CS_SCREENFADE,
                         va("1 %i %i", level.time + 100, (int)ent->wait));
    ent->spawnflags &= ~1;
  } else {
    // fade in
    trap_SetConfigstring(CS_SCREENFADE,
                         va("0 %i %i", level.time + 100, (int)ent->delay));
    ent->spawnflags |= 1;
  }
}
