...
#define TFL_DEFAULT                                                            \
  ((TFL_WALK | TFL_CROUCH | TFL_BARRIERJUMP | TFL_JUMP | TFL_LADDER |          \
    ...
    TFL_ELEVATOR | TFL_AIR | TFL_WATER | TFL_SLIME | TFL_JUMPPAD |             \
    TFL_FUNCBOB) &                                                             \
   ~(TFL_JUMPPAD | TFL_ROCKETJUMP | TFL_BFGJUMP | TFL_GRAPPLEHOOK |            \
     TFL_DOUBLEJUMP | TFL_RAMPJUMP | TFL_STRAFEJUMP | TFL_LAVA))
...
was in a solid area
        float			fraction;	// time completed, 1.0 = didn't
hit anything vec3_t		endpos;		// final position cplane_t
plane;		// surface normal at impact
        float			exp_dist;	// expanded plane distance
        int			sidenum;		// number of the brush
side hit bsp_surface_t surface;	// hit surface
        int			contents;	// contents on other side of
surface hit int			ent;			// number of entity hit
} bsp_trace_t;
//
*/
