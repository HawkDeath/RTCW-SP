...
void target_relay_use(gentity_t *self, gentity_t *other, gentity_t *activator) {
  ...
  if (activator) { // activator can be NULL if called from script
    if (self->key) {
      ...
      //			if(self->key == -1)	// relay permanently
      //locked
...
void SP_target_relay(gentity_t *self) {
  ...
  if (G_SpawnInt("key", "", &key)) { // if door has a key entered, set it
    ...
    } else if (self->key > KEY_NUM_KEYS ||
               self->key < KEY_NONE) { // if the key is invalid, set the key in
...
// Ridah, note to everyone: static functions can cause problems with the
// savegame code, so avoid using them unless necessary. if that function is then
// assigned to an entity, client or cast field, the game will not save.
...
==============
*/
void Use_Target_Autosave(gentity_t *ent, gentity_t *other,
                         gentity_t *activator) {
...
void Use_target_fog(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  trap_SetConfigstring(
      CS_FOGVARS, va("%f %f %f %f %f %f %i", ent->accuracy, ent->random, 1.0f,
...
(and use the fog values specified in the sky shader if present) distance value
sets the type of fog.  values > 1 are distance fog (ex. 2048), values < 1 are
density fog (ex. .0002) "near" is fog start distance when using distance fog
...
*/
void SP_target_autosave(gentity_t *ent) { ent->use = Use_Target_Autosave; }
...
/*QUAKED target_smoke (1 0 0) (-4 -4 -4) (4 4 4) Black White SmokeON Gravity
STEAM 1 second = 1000 1 FRAME = 100 delay = 100 = one millisecond default this
is the maximum smoke that will show up time = 5000 default before the smoke
disipates duration = 2000 before the smoke starts to alpha start_size = 24
default end_size = 96 default
...
void target_rumble_think(gentity_t *ent) {
  ...
  if (validrumble) {
    ...
    ent->s.angles[2] =
        ratio * dapitch /
        24.0f; // CGAME uses this for shake effects, this is the scale
  }
