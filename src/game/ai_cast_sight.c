...
qboolean AICast_VisibleFromPos(vec3_t srcpos, int srcnum, vec3_t destpos,
  ...
  //
  for (i = 0; i < 5; i++) {
    if (cs && updateVisPos) { // if it's a grenade or something, PVS checks
    ...
    //
    contents_mask =
        MASK_AISIGHT; //(MASK_SHOT | CONTENTS_AI_NOSIGHT) & ~(CONTENTS_BODY);
                      //// we can see anything that a bullet can pass through
    passent = srcnum;
    hitent = destnum;
    VectorCopy(eye, start);
    VectorCopy(middle, end);
    // if the entity is in water, lava or slime
    if (trap_PointContents(middle, destnum) &
        (CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER)) {
      contents_mask |= (CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER);
    } // end if
      // if eye is in water, lava or slime
    if (trap_PointContents(eye, srcnum) &
        (CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER)) {
      if (!(contents_mask &
            (CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER))) {
        passent = destnum;
    ...
    trap_Trace(&trace, start, NULL, NULL, end, ENTITYNUM_NONE /*passent*/,
               contents_mask);
    // if water was hit
    if (trace.contents & (CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER)) {
      ...
      //(SURF_TRANS33|SURF_TRANS66))
      {
        ...
        contents_mask &= ~(CONTENTS_LAVA | CONTENTS_SLIME | CONTENTS_WATER);
        trap_Trace(&trace, trace.endpos, NULL, NULL, end, passent,
                   contents_mask);
      } // end if
    }   // end if
...
qboolean AICast_CheckVisibility(gentity_t *srcent, gentity_t *destent) {
  ...
  vis = &cs->vislist[ent];
  //
  // if the destent is the client, and they have just loaded a savegame, ignore
  // them temporarily
  if (!destent->aiCharacter && level.lastLoadTime &&
      (level.lastLoadTime > level.time - 2000) && !vis->visible_timestamp) {
    return qfalse;
  }
  // if we heard them
  /*
  ...
  Distance( srcent->r.currentOrigin, ocs->lastWeaponFiredPos ))) { return qtrue;
  }
  */
  //
  // set the FOV
  ...
  if (cs->aiFlags & AIFL_ZOOMING) {
    ...
    if (cs->lastEnemy >=
        0) { // they've already been in a fight, so give them a very large fov
      if (fov < 270) {
        fov = 270;
      }
    }
  }
  // RF, if they were visible last check, then give us a full FOV, since we are
  // aware of them
  if (cs->aiState >= AISTATE_ALERT &&
      vis->visible_timestamp == vis->lastcheck_timestamp) {
    fov = 360;
  }
  // calculate middle of bounding box
  ...
  // calculate eye position
  if ((level.lastLoadTime < level.time - 4000) &&
      (srcent->r.svFlags & SVF_CASTAI)) {
    if (clientHeadTagTimes[srcent->s.number] == level.time) {
      ...
      // save orientation data (so we dont keep checking for a tag when it
      // doesn't exist)
  ...
  //
  if (!AICast_VisibleFromPos(srcent->client->ps.origin, srcent->s.number,
                             destent->client->ps.origin, destent->s.number,
                             qtrue)) {
    return qfalse;
  }
  //
...
void AICast_UpdateVisibility(gentity_t *srcent, gentity_t *destent,
  ...
  shareRange = (VectorDistance(srcent->client->ps.origin,
                               destent->client->ps.origin) < AIVIS_SHARE_RANGE);
  ...
  // we may need to process this visibility at some point, even after they
  // become not visible again
  ...
  if (cs->attributes[AGGRESSION] < 1.0 && srcent->s.onFireEnd > level.time &&
      (!destent->s.number || cs->dangerEntityValidTime < level.time + 2000) &&
  ...
  if (!(vis->flags & AIVIS_ENEMY) && !AICast_SameTeam(cs, destent->s.number)) {
    ...
    if (!(vis->flags & AIVIS_ENEMY) &&
        VectorDistance(vis->visible_pos,
                       g_entities[cs->entityNum].r.currentOrigin) < idr) {
  ...
  if (AICast_SameTeam(cs, destent->s.number) &&
      (srcent->aiTeam == AITEAM_ALLIES || srcent->aiTeam == AITEAM_NAZI)) {
    ...
    } else if (cs->aiState < AISTATE_COMBAT && ocs->aiState >= AISTATE_COMBAT &&
               ocs->bs && (ocs->enemyNum >= 0)) {
      // if we haven't already checked them out
      if (!(vis->flags & AIVIS_INSPECTED)) {
        vis->flags |= AIVIS_INSPECT;
      }
      // if they are alert, we should also go alert
    } else if (cs->aiState < AISTATE_ALERT && ocs->aiState == AISTATE_ALERT &&
               ocs->bs) {
      AICast_StateChange(cs, AISTATE_ALERT);
    }
  }

  // if this is a friendly, then check them for hostile's that we currently
  // haven't upgraded so

  if ((destent->health > 0) &&
      (srcent->aiTeam ==
       destent->aiTeam) && // only share with exact same team, and non-neutrals
    ...
    for (i = 0; i < aicast_maxclients && cnt < level.numPlayingClients; i++) {
      ...
      svis = &cs->vislist[i];
      //
      // if we are close to the friendly, then we should share their visibility
      // info
      if (destent->health > 0 && shareRange) {
        ...
        if ((ovis->visible_timestamp > svis->visible_timestamp) ||
            ((ovis->visible_timestamp > level.time - 5000) &&
             (ovis->flags & AIVIS_ENEMY) && !(svis->flags & AIVIS_ENEMY))) {
          // trigger an EVENT

          // trigger the sight event
          AICast_Sight(srcent, destent, ovis->visible_timestamp);

          // we may need to process this visibility at some point, even after
          // they become not visible again
          svis->flags |= AIVIS_PROCESS_SIGHTING;

          // if we are sharing information about an enemy, then trigger a
          // scripted event
          if (!svis->real_visible_timestamp && ovis->real_visible_timestamp &&
              (ovis->flags & AIVIS_ENEMY)) {
            // setup conditions
            BG_UpdateConditionValue(ocs->entityNum, ANIM_COND_ENEMY_TEAM,
                                    g_entities[i].aiTeam, qfalse);
            // call the event
            BG_AnimScriptEvent(&g_entities[ocs->entityNum].client->ps,
                               ANIM_ET_INFORM_FRIENDLY_OF_ENEMY, qfalse,
                               qfalse);
          }
          ...
          *svis = *ovis;
          ...
          if (/*(cs->aiState == AISTATE_COMBAT) &&*/ (ovis->flags &
                                                      AIVIS_ENEMY) &&
              !(oldvis.flags & AIVIS_ENEMY)) {
            ...
            if (!(cs->vislist[i].flags & AIVIS_SIGHT_SCRIPT_CALLED)) {
              ...
              if (!(cs->aiFlags & AIFL_DENYACTION)) {
                G_AddEvent(srcent, EV_GENERAL_SOUND,
                           G_SoundIndex(aiDefaults[cs->aiCharacter]
                                            .soundScripts[SIGHTSOUNDSCRIPT]));
              }
            }
          }
        }
      ...
      if ((cs->aiState == AISTATE_COMBAT) && AICast_HostileEnemy(ocs, i) &&
          !AICast_HostileEnemy(cs, i)) {
        if (!(cs->vislist[i].flags & AIVIS_SIGHT_SCRIPT_CALLED)) {
          AICast_ScriptEvent(cs, "enemysight", g_entities[i].aiName);
          cs->vislist[i].flags |= AIVIS_SIGHT_SCRIPT_CALLED;
          if (!(cs->aiFlags & AIFL_DENYACTION)) {
...
==============
*/
void AICast_UpdateNonVisibility(gentity_t *srcent, gentity_t *destent,
                                qboolean directview) {
  cast_visibility_t *vis;
  cast_state_t *cs;
  ...
  if (vis->chase_marker_count < MAX_CHASE_MARKERS) {
    if ((level.time - vis->visible_timestamp) >
        (vis->chase_marker_count + 1) * CHASE_MARKER_INTERVAL) {
      VectorCopy(destent->client->ps.origin,
                 vis->chase_marker[vis->chase_marker_count]);
...
#define SIGHT_MIN_DELAY 200
  ...
  // First, check all REAL clients, so sighting player is only effected by
  // reaction_time, not effected by framerate also
  for (srccount = 0, src = 0, srcent = &g_entities[0];
    ...
    // make sure we are using the right AAS data for this entity (one's that
    // don't get set will default to the player's AAS data)
    ...
    for (destcount = 0, dest = 0, destent = g_entities;
      ...
      // if (vis->real_visible_timestamp && cs->aiState > AISTATE_QUERY &&
      // AICast_HostileEnemy(cs, destent->s.number)) 	continue;

      // if we saw them last frame, skip this test, so we only check initial
      // sightings each frame
