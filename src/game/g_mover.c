...
qboolean G_MoverPush(gentity_t *pusher, vec3_t move, vec3_t amove,
  ...
  *obstacle = NULL;
  ...
  for (e = 0; e < listedEntities; e++) {
    ...
    if (check->s.eType == ET_ITEM && check->item->giType == IT_CLIPBOARD &&
        (check->spawnflags & 1)) {
      continue;
    }
...
void G_MoverTeam(gentity_t *ent) {
  ...
  for (part = ent; part; part = part->teamchain) {
    ...
    if (part->s.eType == ET_BAT && part->model &&
        !G_MoverPush(part, move, amove, &obstacle)) {
      break;
...
*/
void MatchTeamReverseAngleOnSlaves(gentity_t *teamLeader, int moverState,
                                   int time) {
  gentity_t *slave;
...
void Reached_BinaryMover(gentity_t *ent) {
  ...
  //	ent->flags &= ~(FL_KICKACTIVATE|FL_SOFTACTIVATE);	// (SA) it was not
  //opened normally.  Clear this so it thinks it's closed normally
...
================
*/
qboolean IsBinaryMoverBlocked(gentity_t *ent, gentity_t *other,
                              gentity_t *activator) {
...
void Use_BinaryMover(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  if (level.time <=
      4000) { // hack.  don't play door sounds if in the first /four/ seconds of
              // game (FIXME: TODO: THIS IS STILL A HACK)
    nosound = qtrue;
  }
  ...
  if (isblocked) {
    ...
    if (!nosound) {
      if (kicked) { // kicked
        if (activator) {
          AICast_AudibleEvent(activator->s.number, ent->s.origin,
...
void finishSpawningKeyedMover(gentity_t *ent) {
  ...
  if (ent->key == KEY_UNLOCKED_ENT) { // the key was not set in the spawn
    if (ent->targetname && findNonAIBrushTargeter(ent)) {
      ent->key = KEY_LOCKED_TARGET; // something is targeting this (other than a
...
void DoorSetSounds(gentity_t *ent, int doortype, qboolean isRotating) {
  ...
  ent->soundSoftopen =
      G_SoundIndex(va("door%i_openq", doortype)); // opening quietly
  ent->soundSoftendo =
      G_SoundIndex(va("door%i_endoq", doortype)); // open quietly
  ent->soundSoftclose =
      G_SoundIndex(va("door%i_closeq", doortype)); // closing quietly
  ent->soundSoftendc =
      G_SoundIndex(va("door%i_endcq", doortype)); // closed quietly
  ...
  if ((ent->s.apos.trType == TR_STATIONARY &&
    ...
    if (ent->active == qfalse) {
      if (ent->key >= KEY_LOCKED_ENT) { // door force locked
        ...
        // door locked because it was targeted, check if the 'other' ent is
        // targeting this door
      ...
      if (locked) {
        if (!walking &&
            activator) { // only send audible event if not trying to open slowly
          AICast_AudibleEvent(
              activator->s.clientNum, ent->s.origin,
              HEAR_RANGE_DOOR_LOCKED); // "someone tried locked door near me!"
        }
        G_AddEvent(ent, EV_GENERAL_SOUND, ent->soundPos3);
        return;
      }
      ...
      if (activator) {
        if (ent->key > KEY_NONE &&
            ent->key < KEY_NUM_KEYS) { // door requires key
          gitem_t *item = BG_FindItemForKey(ent->key, 0);
          if (!(activator->client->ps.stats[STAT_KEYS] & (1 << item->giTag))) {
            if (!walking) { // only send audible event if not trying to open
                            // slowly
              AICast_AudibleEvent(
                  activator->s.clientNum, ent->s.origin,
                  HEAR_RANGE_DOOR_LOCKED); // "someone tried locked door near
                                           // me!"
            }
      ...
      if (ent->flags & FL_DOORNOISE) { // this door always plays the 'regular'
                                       // open sound event
        soundrange = HEAR_RANGE_DOOR_OPEN;
      }
      ...
      //			if(soundrange)
      //				AICast_AudibleEvent(
      //activator->s.clientNum, ent->s.origin, soundrange );
    }
  }
}
//----(SA)	end
...
SHOOT-THRU	Bullets don't stop when they hit the door.  Set
"shoot_thru_scale" with bullet damage scale (see below)
...
syncronicity "noisescale"multiplier for how far the noise from the door will
travel to alert AI "type"		use sounds based on construction of
door: 0 - nosound (default) 1 - metal 2 - stone 3 - lab 4 - wood 5 - iron/jail
         6 - portcullis
         7 - wood (quiet)
...
void SP_func_door(gentity_t *ent) {
  ...
  if (doortype) { // /*why on earthy did this check for <=8?*/ && doortype <= 8)
                  // // no doortype = silent
  ...
  if (G_SpawnInt("key", "", &key)) { // if door has a key entered, set it
    ...
    } else if (ent->key > KEY_NUM_KEYS ||
               ent->key < KEY_NONE) { // if the key is invalid, set the key in
                                      // the finishSpawning routine
      G_Error("invalid key (%d) set for func_door_rotating\n", ent->key);
      ent->key = KEY_UNLOCKED_ENT;
    }
  } else {
    ent->key = KEY_UNLOCKED_ENT; // otherwise, set the key when this ent
                                 // finishes spawning
  }
...
void Think_SetupTrainTargets(gentity_t *ent) {
  ...
  if (ent->s.eType == ET_BAT) { // NOTE: this is odd.  it will never get hit.
                                // remove?  what did it do?
  ...
  if (!Q_stricmp(ent->classname, "func_train") &&
    ...
    trap_LinkEntity(ent);
  } else if (!Q_stricmp(ent->classname, "func_train_particles") &&
             ent->spawnflags & 2) { // TOGGLE
    VectorCopy(ent->nextTrain->s.origin, ent->s.pos.trBase);
    VectorCopy(ent->nextTrain->s.origin, ent->r.currentOrigin);
    trap_LinkEntity(ent);
  } else if (!Q_stricmp(ent->classname, "func_tramcar") &&
             ent->spawnflags & 2) { // TOGGLE
    VectorCopy(ent->nextTrain->s.origin, ent->s.pos.trBase);
    VectorCopy(ent->nextTrain->s.origin, ent->r.currentOrigin);
    trap_LinkEntity(ent);
...
END_OUTER count = default 10 bats radius = maximum distance from center of
entity to place each bat (default=32) speed = speed to travel to next waypoint
(default=300) wait = (used for end map) wait seconds in between spawning target
= (used for end map) distance check from this entity to enable spawning if
player is more than "radius" distance from the target delay = (end map) wait in
seconds this long after player steps outside, before spawning spirits
*/
  ...
  if (owner->active == qtrue && owner->inuse) { // move towards the owner
    ...
    /*
                    ...
                    if (bat->damage < level.time) {
                            trap_Trace( &tr, bat->r.currentOrigin, NULL, NULL,
       bat->r.currentOrigin, bat->s.number, CONTENTS_BODY ); if (tr.startsolid
       && tr.entityNum < MAX_CLIENTS && !g_entities[tr.entityNum].aiCharacter) {
                                    G_Damage( &g_entities[tr.entityNum], bat,
       bat, vec3_origin, bat->r.currentOrigin, 1+rand()%3, DAMAGE_NO_KNOCKBACK,
       MOD_BAT );

                                    // !! TODO: bat biting sound and view
       feedback

                                    // don't keep hurting them each time we
       think bat->damage = level.time + 1000;
                            }
                    }
    */
...
void BatDie(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
...
void FuncEndSpiritsThink(gentity_t *self) {
  ...
  //
  self->nextthink =
      level.time + (int)((1.5 + 2.0 * random()) * (self->wait * 1000));
  //
  if (!self->active) {
  ...
  if (VectorDistance(enemyPos, cEnt->s.origin) > self->radius) {
    ...
    if (VectorDistance(self->s.origin, enemyPos) <
        VectorDistance(self->s.origin, cEnt->s.origin)) {
...
void Use_Static(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  if (ent->r.linked) {
    trap_UnlinkEntity(ent);
    // DISABLED since func_static will carve up AAS anyway, so blocking makes no
    // sense RF, AAS areas are now free
    // if (ent->model)
    //	G_SetAASBlockingEntity( ent, qfalse );
  } else {
    trap_LinkEntity(ent);
    // DISABLED since func_static will carve up AAS anyway, so blocking makes no
    // sense RF, AAS areas are now occupied
    // if (ent->model)
    //	G_SetAASBlockingEntity( ent, qtrue );
  }
}
...
"leakpressure" - force the particles come out with.  '0' would cause them to
fall straight down "leaktime" - how long it leaks before it quits "leakcount" -
how many holes in the entity before it no longer leaks.
...
and models. "model2"	.md3 model to also draw "color" constantLight color
"light"		constantLight radius
...
STAYOPEN You need to have an origin brush as part of this entity. The center of
that brush will be the point around which it is rotated. It will rotate around
the Z axis by default.  You can check either the X_AXIS or Y_AXIS box to change
that (only one axis allowed. If both X and Y are checked, the default of Z will
be used). FORCE		door opens even if blocked
...
void SP_func_door_rotating(gentity_t *ent) {
  ...
  if (VectorLength(ent->rotate) >
      1) { // check that rotation is only set for one axis
    G_Error("Too many axis marked in func_door_rotating entity.  Only choose "
            "one axis of rotation. (defaulting to standard door rotation)");
    VectorClear(ent->rotate);
    ent->rotate[1] = 1;
  }
...
==============
*/
void use_target_effect(gentity_t *self, gentity_t *other,
                       gentity_t *activator) {
  gentity_t *tent;
  ...
  if (self->damage) {
    G_RadiusDamage(self->s.pos.trBase, self, self->damage, self->damage, self,
                   MOD_EXPLOSIVE);
  }
...
/*QUAKED target_effect (0 .5 .8) (-6 -6 -6) (6 6 6) TNT explode smoke rubble
gore lowgrav debris player_damage "mass" defaults to 15.  This determines how
much debris is emitted when it explodes.  (number of pieces) "dmg" defaults to
0.  damage radius blast when triggered "type" - if 'rubble' is specified, this
is the model type ("glass", "wood", "metal", "gibs", "brick", "rock", "fabric")
default is "wood"
*/
...
==============
*/
void func_explosive_explode(gentity_t *self, gentity_t *inflictor,
  ...
  if (self->wait >= 0) {
    ...
    self->nextthink =
        level.time +
        timeToDeath; // delay removal until the animation has played out and
                     // leave as long as the user requested
    self->s.time =
        timeToDeath < 3000
            ? timeToDeath
            : self->nextthink -
                  3000; // 3 sec fade at end, unless the life time is less than
                        // 2 seconds, then fade from now to death
  ...
  VectorCopy(
      origin,
      self->s.origin2); // (SA) changed so i can have the model remain for a
                        // bit, but still have the explosion at the 'center'
  ...
  if (self->damage) {
    G_RadiusDamage(origin, self, self->damage, self->damage + 40, self,
                   MOD_EXPLOSIVE);
  }
  ...
  if (!tent && !self->model2) { // if there's a 'model2', the dir is for that,
                                // not the explosion
    if (self->s.angles[1]) {
      // up
...
==============
*/
void func_explosive_use(gentity_t *self, gentity_t *other,
                        gentity_t *activator) {
  //	func_explosive_explode (self, self, other, self->health, 0);
  func_explosive_explode(self, self, other, self->damage, 0);
}
...
==============
*/
void func_explosive_spawn(gentity_t *self, gentity_t *other,
                          gentity_t *activator) {
  trap_LinkEntity(self);
...
NOBLOCKAAS EXPLO DYNOMITE EXPLO only explosives can damage it rockets grendades
etc DYNOMITE only can be damaged by DY-NO-MITE! Any brush that you want to
explode or break apart.  If you want an explosion, set dmg and it will do a
radius explosion of that amount at the center of the bursh. TOUCHABLE means
automatic use on player contact. USESHADER will apply the shader used on the
brush model to the debris. LOWGRAV specifies that the debris will /always/ fall
slowly "model2" optional md3 to draw over the solid clip brush "numLivingFrames"
- how many frames to loop while the ent is 'alive' (model should have two
animations: living->dead. living always starts at frame '0') "numDeadFrames" -
how many frames to play when the ent 'dies' ('nnnFrames' parameters must go
together) "wait" - how long (in seconds) to leave the model after it's 'dead'.
'-1' leaves forever. NOTE: if you use model2, you must have an origin brush in
the explosive with the center of the origin at the origin of the model "item" -
when it explodes, pop this item out with the debirs (use QUAKED name. ex:
"item_health_small") "dmg" - how much radius damage should be done, defaults to
0 "health" - defaults to 100.  If health is set to '0' the brush will not be
shootable. "targetname" - if set, no touch field will be spawned and a remote
button or trigger field triggers the explosion. "type" - type of debris
...
explodes.  You get one large chunk per 100 of mass (up to 8) and one small chunk
per 25 of mass (up to 16).  So 800 gives the most. "noise" - sound to play when
triggered.  The explosive will default to a sound that matches it's 'type'.  Use
the sound name "nosound" (case in-sensitive) if you want it silent. "leaktype" -
leaks particles of this type "leakpressure" - force the particles come out with.
'0' would cause them to fall straight down "leaktime" - how long it leaks before
it quits "leakcount" - how many holes in the entity before it no longer leaks.
  ...
  if (!(other->client)) {
    ...
    if (ent->spawnflags & 2 && !(ent->spawnflags & 1)) {
      if (ent->aiName) {
        player = AICast_FindEntityForName("player");
        if (player) {
          AICast_ScriptEvent(AICast_GetCastState(player->s.number), "trigger",
                             ent->target);
        }
      }
