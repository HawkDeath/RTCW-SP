...
static animStringItem_t animMoveTypesStr[] = {
    {"** UNUSED **", -1}, {"IDLE", -1},      {"IDLECR", -1},
    {"WALK", -1},         {"WALKBK", -1},    {"WALKCR", -1},
    {"WALKCRBK", -1},     {"RUN", -1},       {"RUNBK", -1},
    {"SWIM", -1},         {"SWIMBK", -1},    {"STRAFERIGHT", -1},
    {"STRAFELEFT", -1},   {"TURNRIGHT", -1}, {"TURNLEFT", -1},
    {"CLIMBUP", -1},      {"CLIMBDOWN", -1},
...
static animStringItem_t animConditionSpecialConditionStr[] = {
    {"** UNUSED **", -1}, {"ESCAPE1_CUTSCENE_1", -1},
    {"VILL1_KARL", -1},   {"VILL1_KESSLER", -1},
...
animModelInfo_t *BG_ModelInfoForClient(int client) {
  ...
  //
  return globalScriptData
      ->modelInfo[globalScriptData->clientModels[client] - 1];
}
...
int BG_AnimationIndexForString(char *string, int client) {
  ...
  for (i = 0, anim = modelInfo->animations; i < modelInfo->numAnimations;
       i++, anim++) {
  ...
  // no match found
  BG_AnimParseError(
      "BG_AnimationIndexForString: unknown index '%s' for model '%s'", string,
      modelInfo->modelname);
...
animation_t *BG_AnimationForString(char *string, animModelInfo_t *modelInfo) {
  ...
  // no match found
  Com_Error(ERR_DROP,
            "BG_AnimationForString: unknown animation '%s' for model '%s'",
...
===============
*/
char *BG_CopyStringIntoBuffer(char *string, char *buffer, int bufSize,
                              int *offset) {
  char *pch;
...
qboolean BG_ParseConditions(char **text_pp, animScriptItem_t *scriptItem) {
  ...
  while (1) {
    ...
    case ANIM_CONDTYPE_BITFLAGS:
      BG_ParseConditionBits(text_pp, animConditionsTable[conditionIndex].values,
    ...
    case ANIM_CONDTYPE_VALUE:
      if (animConditionsTable[conditionIndex].values) {
        token = COM_ParseExt(text_pp, qfalse);
        if (!token || !token[0]) {
          BG_AnimParseError("BG_AnimParseAnimScript: expected condition value, "
                            "found end of line"); // RF modification
        }
...
=================
*/
void BG_ParseCommands(char **input, animScriptItem_t *scriptItem,
                      animModelInfo_t *modelInfo,
  ...
  while (1) {
    ...
    if (command->bodyPart[partIndex] > 0) {
      ...
      command->animIndex[partIndex] =
          BG_AnimationIndexForString(token, parseClient);
      command->animDuration[partIndex] =
          modelInfo->animations[command->animIndex[partIndex]].duration;
      ...
      if (parseMovetype != ANIM_MT_UNUSED &&
        ...
        modelInfo->animations[command->animIndex[partIndex]].movetype |=
            (1 << parseMovetype);
      }
      // if this is a fireweapon event, then this is a firing animation
      if (parseEvent == ANIM_ET_FIREWEAPON) {
        modelInfo->animations[command->animIndex[partIndex]].flags |=
            ANIMFL_FIRINGANIM;
    ...
    while (1) {
      ...
      if (!Q_stricmp(token, "sound")) {
        ...
        if (atoi(token) > 7) {
          BG_AnimParseError(
              va("BG_ParseCommands: showpart number '%d' is too big! (max 8)",
                 atoi(token)));
        }
        ...
        if (atoi(token) > 7) {
          BG_AnimParseError(
              va("BG_ParseCommands: hidepart number '%d' is too big! (max 8)",
                 atoi(token)));
        }
...
#define MAX_INDENT_LEVELS 3
  ...
  // Duffy
  // This is done in each of the calling routines, and assumes all sorts of
  // badness doing it this way anyway
  // scriptData->clientModels[client] = 1 + (int)(modelInfo -
  // *scriptData->modelInfo);
  ...
  while (1) {
    ...
    case PARSEMODE_DEFINES:
      ...
      if (!Q_stricmp(token, "set")) {
        ...
        if (!token || !token[0]) {
          BG_AnimParseError(
              "BG_AnimParseAnimScript: expected condition type string"); // RF
                                                                         // mod
        }
        ...
        token = COM_ParseExt(&text_p, qfalse);
        if (!token || !token[0]) {
          BG_AnimParseError("BG_AnimParseAnimScript: expected condition define "
                            "string"); // RF mod
        }

        // copy the define to the strings list
        defineStr[defineType][numDefines[defineType]].string =
            BG_CopyStringIntoBuffer(token, defineStrings, sizeof(defineStrings),
                                    &defineStringsOffset);
        defineStr[defineType][numDefines[defineType]].hash = BG_StringHashValue(
            defineStr[defineType][numDefines[defineType]].string);
        // expecting an =
        token = COM_ParseExt(&text_p, qfalse);
        if (!token) {
          BG_AnimParseError(
              "BG_AnimParseAnimScript: expected '=', found end of line"); // RF
                                                                          // mod
        }
        if (Q_stricmp(token, "=")) {
          BG_AnimParseError("BG_AnimParseAnimScript: expected '=', found '%s'",
                            token); // RF mod
        }
        ...
        memcpy(&defineBits[ANIM_COND_ENEMY_WEAPON][0],
               &defineBits[ANIM_COND_WEAPON][0],
               sizeof(defineBits[ANIM_COND_ENEMY_WEAPON][0]) *
                   MAX_ANIM_DEFINES);
    ...
    case PARSEMODE_CANNED_ANIMATIONS:
      ...
      if (!Q_stricmp(token, "{")) {
        ...
        if (Q_strncmp(text_p, token, strlen(token))) {
          // this should never happen, just here to check that this operation is
          // correct before code goes live
        ...
        currentScript->items[currentScript->numItems] =
            &modelInfo->scriptItems[modelInfo->numScriptItems++];
    ...
    case PARSEMODE_EVENTS:
      ...
      if (!Q_stricmp(token, "{")) {
        ...
        if (parseMode == PARSEMODE_STATECHANGES) {
          ...
          currentScript =
              &modelInfo->scriptStateChange[oldState][indexes[indentLevel]];
...
===============
*/
int BG_PlayAnim(playerState_t *ps, int animNum, animBodyPart_t bodyPart,
                int forceDuration, qboolean setTimer, qboolean isContinue,
                qboolean force) {
  int duration;
...
#define TIMER_PADDING 150
  ...
  if (forceDuration) {
    ...
    duration = modelInfo->animations[animNum]
                   .duration; // account for lerping between anims
  }
  ...
  case ANIM_BP_LEGS:
    ...
    if ((ps->legsTimer < 50) ||
        (force && (newAnim->priority >= oldAnim->priority))) {
...
int BG_PlayAnimName(playerState_t *ps, char *animName, animBodyPart_t bodyPart,
  ...
  return BG_PlayAnim(ps, BG_AnimationIndexForString(animName, ps->clientNum),
...
int BG_ExecuteCommand(playerState_t *ps, animScriptCommand_t *scriptCommand,
  ...
  if (scriptCommand->bodyPart[0]) {
    ...
    if (scriptCommand->bodyPart[0] == ANIM_BP_BOTH ||
      ...
      playedLegsAnim = (BG_PlayAnim(ps, scriptCommand->animIndex[0],
                                    scriptCommand->bodyPart[0], duration,
  ...
  if (scriptCommand->bodyPart[1]) {
    ...
    if (scriptCommand->bodyPart[1] == ANIM_BP_BOTH ||
      ...
      playedLegsAnim = (BG_PlayAnim(ps, scriptCommand->animIndex[1],
                                    scriptCommand->bodyPart[1], duration,
                                    setTimer, isContinue, force) > -1);
    } else {
...
================
*/
int BG_AnimScriptAnimation(playerState_t *ps, aistateEnum_t estate,
  ...
  if (scriptCommand->bodyPart[0]) {
    if (ps->clientNum) {
      Com_Printf("anim0 (%s): %s",
                 ...
                 modelInfo->animations[scriptCommand->animIndex[0]].name);
    }
  }
  if (scriptCommand->bodyPart[1]) {
    if (ps->clientNum) {
      Com_Printf("anim1 (%s): %s",
                 ...
                 modelInfo->animations[scriptCommand->animIndex[1]].name);
    }
  }
  if (ps->clientNum) {
    Com_Printf("\n");
  ...
  // run it
  return (BG_ExecuteCommand(ps, scriptCommand, qfalse, isContinue, qfalse) !=
          -1);
}
...
qboolean BG_ValidAnimScript(int clientNum) {
  ...
  //
  if (!globalScriptData->modelInfo[globalScriptData->clientModels[clientNum]]
           ->numScriptItems) {
    return qfalse;
  }
  //
...
void BG_UpdateConditionValueStrings(int client, char *conditionStr,
  ...
  value = BG_IndexForString(valueStr, animConditionsTable[condition].values,
                            qfalse);
  //
  globalScriptData->clientConditions[client][condition][0] = value;
}
...
================
*/
int BG_GetAnimScriptAnimation(int client, aistateEnum_t estate,
                              scriptAnimMoveTypes_t movetype) {
  animModelInfo_t *modelInfo;
  animScript_t *script;
  animScriptItem_t *scriptItem = NULL;
...
void BG_AnimUpdatePlayerStateConditions(pmove_t *pmove) {
  ...
  // LEANING
  if (ps->leanf > 0) {
    BG_UpdateConditionValue(ps->clientNum, ANIM_COND_LEANING, LEANING_RIGHT,
                            qtrue);
  } else if (ps->leanf < 0) {
    BG_UpdateConditionValue(ps->clientNum, ANIM_COND_LEANING, LEANING_LEFT,
                            qtrue);
  } else {
    BG_UpdateConditionValue(ps->clientNum, ANIM_COND_LEANING, LEANING_UNUSED,
                            qtrue);
  }
...
#define MAX_ANIM_SCALE 1.1
  ...
  // if they are travelling faster than the moveSpeed, then scale up the gap to
  // counter, since the animation can't play faster than MAX_ANIM_SCALE speed
