...
//
#define MAX_AIFUNCS                                                            \
  15 // if we go over this per frame, likely to have an infinite loop
//
...
//#define AICAST_TFL_DEFAULT	TFL_DEFAULT &
...
#define AIFL_NOAVOID                                                           \
  0x80 // if set, this AI will ignore requests for us to move out the way
#define AIFL_NOPAIN 0x100      // don't stop for pain anims
#define AIFL_WALKFORWARD 0x200 // only walk forward
#define AIFL_DENYACTION                                                        \
  0x400 // used by scripting to prevent dynamic code from executing certain
        // behaviour
#define AIFL_VIEWLOCKED                                                        \
  0x800 // prevent anything outside movement routines from changing view
#define AIFL_CORPSESIGHTING 0x1000 // share information through friendly corpses
#define AIFL_WAITINGTOSPAWN                                                    \
  0x2000 // waiting until space is clear to spawn in to the game
...
#define AIFL_EXPLICIT_ROUTING                                                  \
  0x2000000 // direct routing towards ai_markers, rather than using AAS
#define AIFL_DISMOUNTING 0x4000000
#define AIFL_SPECIAL_FUNC                                                      \
  0x8000000 // prevent external interuption of current think func
...
//
typedef enum { BBOX_SMALL, BBOX_LARGE } BBoxType_t;
//
// attributes
// !!! NOTE: any changes to this must be reflected in the attributeStrings in
// ai_cast.c
typedef enum {
  ...
  ATTACK_SKILL,    // max = 1.0	(ability to attack and do other things, like
                   // retreat)
  REACTION_TIME,   // max = 1.0	(upon seeing enemy, wait this long before
                   // reaction)
  ...
  TACTICAL,  // max = 1.0	(ability to use strategy to their advantage, also
             // behaviour whilst hunting enemy, more likely to creep around)
  CAMPER,    // max = 1.0	(set this to make them stay in the spot they are
             // spawned)
  ...
  HEARING_SCALE_NOT_PVS, // max = 999 (multiply hearing range by this if outside
                         // PVS)
  INNER_DETECTION_RADIUS, // default = 512	(enemies within this range trigger
                          // immediate combat mode
...
typedef struct {
  ...
  char *(*aifuncAttack1)(
      struct cast_state_s *cs); // use this battle aifunc for monster_attack1
  char *(*aifuncAttack2)(
      struct cast_state_s *cs); // use this battle aifunc for monster_attack2
  char *(*aifuncAttack3)(
      struct cast_state_s *cs); // use this battle aifunc for monster_attack2
...
#define AIVIS_PROCESS_SIGHTING                                                 \
  8 // so we know if we have or haven't processed the sighting since they were
    // last seen
#define AIVIS_SIGHT_SCRIPT_CALLED                                              \
  0x10 // set once sight script has been called.. only call once
//
// share range
#define AIVIS_SHARE_RANGE                                                      \
  384 // if we are within this range of a friendly, share their vis info
//
...
typedef struct {
  ...
  vec3_t visible_pos;       // position we last knew of them being at (could be
                            // hearing, etc)
  vec3_t real_visible_pos;  // position we last physically saw them
  vec3_t visible_vel;       // velocity during last sighting
  int notvisible_timestamp; // last time we didn't see the entity (used for
                            // reaction delay)
...
typedef struct {
  ...
  int startingAmmo[MAX_WEAPONS]; // starting ammo values for each weapon (set to
                                 // 999 for unlimited)
} cast_weapon_info_t;
//
// scripting
...
typedef struct {
  ...
  int castScriptEventIndex; // current event containing stack of actions to
                            // perform
  // scripting system AI variables (set by scripting system, used directly by
  // AI)
...
//
typedef enum { MS_DEFAULT, MS_WALK, MS_RUN, MS_CROUCH } movestate_t;
//
typedef enum {
  MSTYPE_NONE,
  MSTYPE_TEMPORARY,
...
typedef struct cast_state_s {
  ...
  // Cast specific information follows. Add to this as needed, this way the
  ...
  cast_script_event_t *castScriptEvents; // contains a list of actions to
                                         // perform for each event type
  cast_script_status_t castScriptStatus; // current status of scripting
  cast_script_status_t
      castScriptStatusCurrent; // scripting status to use for backups
  cast_script_status_t
      castScriptStatusBackup; // perm backup of status of scripting, only used
                              // by backup and restore commands
  int scriptCallIndex;        // inc'd each time a script is called
  int scriptAnimTime,
      scriptAnimNum; // last time an anim was played using scripting
  // the accumulation buffer
  int scriptAccumBuffer[MAX_SCRIPT_ACCUM_BUFFERS];

  //
  cast_weapon_info_t *
      weaponInfo; // FIXME: make this a list, so they can have multiple weapons?
  cast_visibility_t vislist[MAX_CLIENTS]; // array of all other client entities,
                                          // allocated at level start-up
  int weaponFireTimes[MAX_WEAPONS];

  char *(*aifunc)(struct cast_state_s *cs);    // current AI function
  char *(*oldAifunc)(struct cast_state_s *cs); // just so we can restore the
                                               // last aiFunc if required
  ...
  void (*painfunc)(gentity_t *ent, gentity_t *attacker, int damage,
                   vec3_t point);
  void (*deathfunc)(gentity_t *ent, gentity_t *attacker, int damage,
                    int mod); //----(SA)	added mod
  void (*sightfunc)(gentity_t *ent, gentity_t *other, int lastSight);

  // int		(*getDeathAnim)(gentity_t *ent, gentity_t *attacker, int
  // damage);
  ...
  int followEntity;
  float followDist;
  qboolean followIsGoto; // we are really just going to the entity, but should
                         // wait until scripting tells us we can stop
  int followTime;        // if this runs out, the scripting has probably been
                         // interupted
  ...
  int startAttackCount; // incremented each time we start a standing attack
                        // used to make sure we only find a combat spot once per
                        // attack
  ...
  int obstructingTime; // time that we should move so we are not obstructing
                       // someone else
  ...
  int lastWeaponFired; // set each time a weapon is fired. used to detect when a
                       // weapon has been fired from within scripting
  ...
  qboolean grenadeGrabFlag; // if this is set, we need to play the anim before
                            // we can grab it

  vec3_t lastMoveToPosGoalOrg; // if this changes, we should reset the Bot Avoid
                               // Reach
  ...
  int animHitCount; // for stepping through the frames on which to inflict
                    // damage
  ...
  vec3_t loperLeapVel;
  // -------------------------------------------------------------------------------------------
  // if working on a post release patch, new variables should ONLY be inserted
  // after this point
  // -------------------------------------------------------------------------------------------
...
gentity_t *AICast_CreateCharacter(gentity_t *ent, float *attributes,
                                  cast_weapon_info_t *weaponInfo,
                                  char *castname, char *model, char *head,
                                  char *sex, char *color, char *handicap);
void AICast_Init(void);
void AICast_DelayedSpawnCast(gentity_t *ent, int castType);
qboolean AICast_SolidsInBBox(vec3_t pos, vec3_t mins, vec3_t maxs, int entnum,
                             int mask);
void AICast_CheckLevelAttributes(cast_state_t *cs, gentity_t *ent,
                                 char **ppStr);
//
// ai_cast_sight.c
void AICast_SightUpdate(int numchecks);
qboolean AICast_VisibleFromPos(vec3_t srcpos, int srcnum, vec3_t destpos,
                               int destnum, qboolean updateVisPos);
void AICast_UpdateVisibility(gentity_t *srcent, gentity_t *destent,
...
//
// ai_cast_events.c
void AICast_Pain(gentity_t *targ, gentity_t *attacker, int damage,
                 vec3_t point);
void AICast_Die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
