...
void Enable_Trigger_Touch(gentity_t *ent) {
  ...
  if (!targ) {
    ...
    trap_Trace(&tr, targ->client->ps.origin, targ->r.mins, targ->r.maxs,
               targ->client->ps.origin, targ->s.number, mask);
...
/*QUAKED trigger_hurt (.5 .5 .5) ? START_OFF PLAYER_ONLY SILENT NO_PROTECTION
SLOW ONCE Any entity that touches this will be hurt. It does dmg points of
damage each server frame Targeting the trigger will toggle its on / off state.
...
void trigger_aidoor_stayopen(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if (other->client && other->health > 0) {
    ...
    if (door->moverState == MOVER_POS2ROTATE) { // door is in open state waiting
    ...
    // Ridah, door isn't ready, find a free ai_marker, and wait there until it's
    // open
    if (other->r.svFlags & SVF_CASTAI) {

      // we dont have keys, so assume we are not trying to get through this door
      if (door->key >
          KEY_NONE /*&& door->key < KEY_NUM_KEYS*/) { // door requires key
        return;
      }
      ...
      // Ridah, had to change this, since it was causing AI to wait at door when
      // the door is open, and won't close because they are sitting on the
      // aidoor brush TTimo: gcc: suggest parentheses around && within ||
      //   woa this test gets a nasty look
      if ((door->grenadeFired > level.time) ||
          (!((door->activator == other) && (door->moverState != MOVER_POS1) &&
             (door->moverState != MOVER_POS1ROTATE)) &&
           (door->moverState != MOVER_POS2ROTATE) &&
           (door->moverState != MOVER_POS2))) {
        // if we aren't already heading for an ai_marker, look for one we can go
        // to
...
void SP_trigger_aidoor(gentity_t *ent) {
  if (!ent->targetname) {
    G_Printf("trigger_aidoor at loc %s does not have a targetname for "
             "ai_marker assignments\n",
             vtos(ent->s.origin));
  }
...
void gas_touch(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  trap_Trace(&tr, ent->r.currentOrigin, NULL, NULL, other->r.currentOrigin,
  ...
  if (traceEnt->takedamage) {
    ...
    G_Damage(traceEnt, ent, ent, dir, tr.endpos, damage, 0, MOD_POISONGAS);
  }
}
...
You specify which objective it is with a number in "count"
  ...
  "track"		If this is specified, it will override the default
message
*/
  ...
  if (ent->track) {
    if (ent->spawnflags & AXIS_OBJECTIVE) {
      trap_SendServerCommand(
          other - g_entities,
          va("oid 0 \"" S_COLOR_RED "You are near %s\n\"", ent->track));
    } else if (ent->spawnflags & ALLIED_OBJECTIVE) {
      trap_SendServerCommand(
          other - g_entities,
          va("oid 1 \"" S_COLOR_BLUE "You are near %s\n\"", ent->track));
    } else {
      trap_SendServerCommand(other - g_entities,
                             va("oid -1 \"You are near %s\n\"", ent->track));
    }
  } else {
    if (ent->spawnflags & AXIS_OBJECTIVE) {
      trap_SendServerCommand(other - g_entities,
                             va("oid 0 \"" S_COLOR_RED
                                "You are near objective #%i\n\"",
                                ent->count));
    } else if (ent->spawnflags & ALLIED_OBJECTIVE) {
      trap_SendServerCommand(other - g_entities,
                             va("oid 1 \"" S_COLOR_BLUE
                                "You are near objective #%i\n\"",
                                ent->count));
    } else {
      trap_SendServerCommand(
          other - g_entities,
          va("oid -1 \"You are near objective #%i\n\"", ent->count));
    }
  }
}
