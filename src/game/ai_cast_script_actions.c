...
AICast_ScriptAction_GotoMarker
  ...
  syntax: gotomarker <targetname> [firetarget [noattack]] [nostop] OR
...
#define SCRIPT_REACHGOAL_DIST 8
  ...
  if (cs->castScriptStatus.scriptGotoId < 0 &&
      cs->dangerEntityValidTime > level.time) {
    return qfalse;
  }
  ...
  // if we already are going to the marker, just use that, and check if we're in
  // range
  if (cs->castScriptStatus.scriptGotoEnt >= 0 &&
    ...
    if (ent->targetname && !Q_strcasecmp(ent->targetname, token)) {
      // if we're not slowing down, then check for passing the marker, otherwise
      // check distance only
      ...
      if (cs->followSlowApproach && VectorLength(vec) < cs->followDist) {
        ...
        return qtrue;
      } else if (!cs->followSlowApproach &&
                 VectorLength(vec) <
                     64 /*&& DotProduct(cs->bs->cur_ps.velocity, vec) < 0*/) {
        cs->followTime = 0;
        AIFunc_IdleStart(cs); // resume normal AI
        return qtrue;
      } else {
        ...
        if (!token[0] || !Q_stricmp(token, "nostop")) {
          ...
          if (!ent) {
            ...
            if (!ent) {
              G_Error(
                  "AI Scripting: gotomarker cannot find targetname \"%s\"\n",
                  token);
            }
          }
          ...
          if (!token[0] || Q_stricmp(token, "noattack")) {
            ...
            if (ent->r.svFlags & SVF_CASTAI) {
              if (cs->vislist[ent->s.number].real_visible_timestamp !=
                  cs->vislist[ent->s.number].lastcheck_timestamp) {
                fire = qfalse;
              }
            }
            if (fire) {
              for (i = 0; i < 2; i++) {
                diff = abs(AngleDifference(cs->viewangles[i],
  ...
  //
  AIFunc_ChaseGoalStart(cs, ent->s.number,
                        (slowApproach ? SCRIPT_REACHGOAL_DIST : 32),
                        slowApproach);
...
qboolean AICast_ScriptAction_WalkToMarker(cast_state_t *cs, char *params) {
  ...
  if (!AICast_ScriptAction_GotoMarker(cs, params) ||
      (!strstr(params, " nostop") && VectorLength(cs->bs->cur_ps.velocity))) {
...
AICast_ScriptAction_GotoCast
  ...
  syntax: gotocast <ainame> [firetarget [noattack]] OR runtocast <ainame>
[firetarget [noattack]]
===============
*/
...
qboolean AICast_ScriptAction_AbortIfLoadgame(cast_state_t *cs, char *params) {
  ...
  if (strlen(loading) > 0 && atoi(loading) != 0) {
    ...
    cs->castScriptStatus.castScriptStackHead =
        cs->castScriptEvents[cs->castScriptStatus.castScriptEventIndex]
            .stack.numItems;
  }
...
qboolean AICast_ScriptAction_Wait(cast_state_t *cs, char *params) {
  ...
  // EXPERIMENTAL: if they are on fire, or avoiding danger, let them loose until
  // it passes (or they die)
  ...
  if (!cs->bs) {
    return (cs->castScriptStatus.castScriptStackChangeTime + duration <
            level.time);
  }

  token = COM_ParseExt(&pString, qfalse);
  // if this token is a number, then assume it is the moverange, otherwise we
  // have a default moverange with a facetarget
  moverange = -999;
  facetarget = NULL;
  if (token[0]) {
  ...
  if (moverange != 0) { // default to 200 if no range given
    if (moverange > 0) {
      ...
      if ((cs->castScriptStatus.scriptWaitMovetime < level.time) &&
        ...
        if (AICast_CheckAttack(cs, cs->enemyNum, qfalse) ||
            (!AICast_EntityVisible(AICast_GetCastState(cs->enemyNum),
                                   cs->entityNum, qfalse) &&
      ...
      if ((!cs->bs || !cs->bs->cur_ps.legsTimer) &&
          (((cs->castScriptStatus.scriptWaitMovetime > level.time) &&
            (dist > 32)) ||
           (dist > moverange))) {
        cs->castScriptStatus.scriptNoMoveTime = 0;
    ...
    cs->castScriptStatus.scriptNoMoveTime =
        cs->castScriptStatus.castScriptStackChangeTime + duration;
  }
...
qboolean AICast_ScriptAction_PlaySound(cast_state_t *cs, char *params) {
  if (!params) {
    G_Error(
        "AI Scripting: syntax error\n\nplaysound <soundname OR scriptname>\n");
  }

  G_AddEvent(&g_entities[cs->bs->entitynum], EV_GENERAL_SOUND,
             G_SoundIndex(params));
...
qboolean AICast_ScriptAction_Attack(cast_state_t *cs, char *params) {
  ...
  if (params) {
    ent = AICast_FindEntityForName(params);
    if (!ent) {
      G_Error("AI Scripting: \"attack\" command unable to find aiName \"%s\"",
              params);
    }
...
AICast_ScriptAction_PlayAnim
  ...
  syntax: playanim <animation> [legs/torso/both] [HOLDFRAME] [numLoops/FOREVER]
[target]

  NOTE: any new animations that are needed by the scripting system, will need to
be added here
=================
*/
qboolean AICast_ScriptAction_PlayAnim(cast_state_t *cs, char *params) {
  ...
  if (level.animScriptData
          .modelInfo[level.animScriptData.clientModels[cs->entityNum] - 1]
          ->version > 1) { // new (scripted) model
    ...
    if (token && token[0]) {
      ...
      if (token && token[0]) {
        ...
        VectorNormalize(vec);
        vectoangles(vec, cs->ideal_viewangles);
        VectorCopy(cs->ideal_viewangles,
                   cs->castScriptStatus.playanim_viewangles);
        setAngles = qtrue;
      }
    ...
    if (cs->castScriptStatus.scriptFlags & SFL_FIRST_CALL) {
      ...
      duration = BG_PlayAnimName(
          &(client->ps), tokens[0],
          BG_IndexForString(tokens[1], animBodyPartsStr, qfalse), qtrue, qfalse,
          qtrue);
      if (numLoops == -1) {
      ...
      if (!strcmp(tokens[1], "torso")) {
        ...
        // lock the viewangles
        if (!cs->castScriptStatus.playAnimViewlockTime ||
            cs->castScriptStatus.playAnimViewlockTime < level.time) {
          VectorCopy(cs->ideal_viewangles,
                     cs->castScriptStatus.playanim_viewangles);
        }
    ...
    duration =
        endtime - level.time +
        200; // so animations don't run out before starting a next animation
    if (duration > 2000) {
      duration = 2000;
    }
      ...
      if (!Q_strcasecmp(tokens[0], animStrings[i])) {
        if (!Q_strcasecmp(tokens[2], "torso")) {
          if ((client->ps.torsoAnim & ~ANIM_TOGGLEBIT) != i) {
            client->ps.torsoAnim =
                ((client->ps.torsoAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | i;
          }
          client->ps.torsoTimer = duration;
        } else if (!Q_strcasecmp(tokens[2], "legs")) {
          if ((client->ps.legsAnim & ~ANIM_TOGGLEBIT) != i) {
            client->ps.legsAnim =
                ((client->ps.legsAnim & ANIM_TOGGLEBIT) ^ ANIM_TOGGLEBIT) | i;
          }
    ...
    if (i == MAX_ANIMATIONS) {
      G_Printf(
          "AI Scripting: playanim has unknown or invalid animation \"%s\"\n",
          tokens[0]);
    }
    //}
...
qboolean AICast_ScriptAction_SetAmmo(cast_state_t *cs, char *params) {
  ...
  if (weapon != WP_NONE) {
    ...
    if (atoi(token)) {
      ...
      g_entities[cs->entityNum].client->ps.ammo[BG_FindAmmoForWeapon(weapon)] =
          0;
      g_entities[cs->entityNum]
    ...
    if (g_cheats.integer) {
      G_Printf(
          "--SCRIPTER WARNING-- AI Scripting: setammo: unknown ammo \"%s\"",
          params);
    }
...
qboolean AICast_ScriptAction_SetClip(cast_state_t *cs, char *params) {
  ...
  if (weapon != WP_NONE) {
    ...
    //		G_Printf( "--SCRIPTER WARNING-- AI Scripting: setclip: unknown weapon
    //\"%s\"", params );
    return qfalse; // (SA) temp as scripts transition to new names
...
qboolean AICast_ScriptAction_SelectWeapon(cast_state_t *cs, char *params) {
  ...
  if (weapon != WP_NONE) {
    ...
    if (!cs->aiCharacter) { // only do this for player
      // vmCvar_t cvar;
      //
      g_entities[cs->entityNum].client->ps.weaponTime =
          750; // (SA) HACK: FIXME: TODO: delay to catch initial weapon reload
    ...
    //		G_Printf( "--SCRIPTER WARNING-- AI Scripting: selectweapon: unknown
    //weapon \"%s\"", params ); 		return qfalse;	// (SA) temp as scripts
...
qboolean AICast_ScriptAction_GiveWeapon(cast_state_t *cs, char *params) {
  ...
  if (weapon != WP_NONE) {
    ...
    //----(SA)	some weapons always go together (and they share a clip, so this
    //is okay)
    if (weapon == WP_GARAND) {
    ...
    if (!Q_strncasecmp(params, "monsterattack", 13) || weapon == WP_KNIFE) {
      g_entities[cs->entityNum].client->ps.ammo[BG_FindAmmoForWeapon(weapon)] =
          999;
      Fill_Clip(&g_entities[cs->entityNum].client->ps,
                weapon); //----(SA)	added
    }
    // conditional flags
...
qboolean AICast_ScriptAction_TakeWeapon(cast_state_t *cs, char *params) {
  ...
  if (!Q_stricmp(params, "all")) {
    ...
    memset(g_entities[cs->entityNum].client->ps.weapons, 0,
           sizeof(g_entities[cs->entityNum].client->ps.weapons));
    memset(g_entities[cs->entityNum].client->ps.ammo, 0,
           sizeof(g_entities[cs->entityNum].client->ps.ammo));
    memset(g_entities[cs->entityNum].client->ps.ammoclip, 0,
           sizeof(g_entities[cs->entityNum].client->ps.ammoclip));
    cs->weaponNum = WP_NONE;

  } else {

    for (i = 1; bg_itemlist[i].classname; i++) {
      //----(SA)	first try the name they see in the editor, then the
      //pickup name
      if (!Q_strcasecmp(params, bg_itemlist[i].classname)) {
        weapon = bg_itemlist[i].giTag;
        break;
    ...
    if (weapon != WP_NONE) {
      ...
      if (weapon == WP_AKIMBO) {
        // take both the colt /and/ the akimbo weapons when 'akimbo' is
        // specified
        ...
        if (COM_BitCheck(g_entities[cs->entityNum].client->ps.weapons,
                         WP_AKIMBO)) {
          weapon = WP_AKIMBO;
        }
      }
      ...
      // but first make sure we dont have any other weapons that use the same
      // ammo
      clear = qtrue;
...
AICast_ScriptAction_Movetype
  ...
  Sets this character's movement type, will exist until another movetype command
is called
=================
*/
...
qboolean AICast_ScriptAction_AlertEntity(cast_state_t *cs, char *params) {
  ...
  if (!ent->AIScript_AlertEntity) {
    ...
    if (aicast_debug.integer) {
      G_Printf("AI Scripting: alertentity \"%s\" (classname = %s) doesn't have "
               "an \"AIScript_AlertEntity\" function\n",
               params, ent->classname);
    }
    // G_Error( "AI Scripting: alertentity \"%s\" (classname = %s) doesn't have
    // an \"AIScript_AlertEntity\" function\n", params, ent->classname );
    return qtrue;
  }
...
qboolean AICast_ScriptAction_FireAtTarget(cast_state_t *cs, char *params) {
  ...
  for (i = 0; i < 2; i++) {
    diff = abs(
        AngleDifference(cs->bs->cur_ps.viewangles[i], cs->ideal_viewangles[i]));
  ...
  return ((cs->castScriptStatus.castScriptStackChangeTime + atoi(token)) <
          level.time);
}
...
AICast_ScriptAction_SpawnCast
  ...
  <targetname> is the entity marker which has the position and angles of where
we want the new cast AI to spawn
=================
*/
qboolean AICast_ScriptAction_SpawnCast(cast_state_t *cs, char *params) {
  ...
  G_Error("AI Scripting: spawncast is no longer functional. Use trigger_spawn "
          "instead.\n");
  return qfalse;
  /*
          if (!params[0]) {
     ...
     token );
          ...
          if (!G_CallSpawn( newCast )) {
                  G_Error( "AI Scripting: spawncast for unknown entity
     \"%s\"\n", newCast->classname );
          }
...
qboolean AICast_ScriptAction_MissionFailed(cast_state_t *cs, char *params) {
  ...
  trap_SetConfigstring(CS_SCREENFADE,
                       va("1 %i %i", level.time + 250, time * 1000));
...
qboolean AICast_ScriptAction_ObjectivesNeeded(cast_state_t *cs, char *params) {
  ...
  token = COM_ParseExt(&pString, qfalse);
  if (!token[0]) {
    G_Error("AI Scripting: objectivesneeded requires a num_objectives "
            "identifier\n");
  }
...
qboolean AICast_ScriptAction_ObjectiveMet(cast_state_t *cs, char *params) {
  ...
  if (!token[0]) {
    G_Error(
        "AI Scripting: missionsuccess requires a num_objective identifier\n");
  }
...
qboolean AICast_ScriptAction_ResetScript(cast_state_t *cs, char *params) {
  ...
  if (client->ps.torsoTimer &&
      (client->ps.torsoTimer > (level.time - cs->scriptAnimTime))) {
    if ((client->ps.torsoAnim & ~ANIM_TOGGLEBIT) == cs->scriptAnimNum) {
      client->ps.torsoTimer = 0;
    }
  }
  if (client->ps.legsTimer &&
      (client->ps.legsTimer > (level.time - cs->scriptAnimTime))) {
...
AICast_ScriptAction_ChangeLevel
  ...
  Optionally add
        "persistent" if you want the player to keep their inventory through the
transition. "silent" if you want it to not play the mission success music
...
qboolean AICast_ScriptAction_ChangeLevel(cast_state_t *cs, char *params) {
  ...
  if (!Q_stricmp(
          newstr,
          "gamefinished")) { // 'gamefinished' is keyword for 'exit to credits'
    endgame = qtrue;
  }
  ...
  if (!silent && !endgame) {
    trap_SendServerCommand(
        -1, "mu_play sound/music/l_complete_1.wav 0\n"); // play mission success
                                                         // music
  }
  trap_SetConfigstring(CS_MUSIC_QUEUE,
                       ""); // don't try to start anything.  no level load music

  trap_SetConfigstring(CS_SCREENFADE, va("1 %i %i", level.time + 250,
                                         750 + exitTime)); // fade out screen

  trap_SendServerCommand(
      -1, va("snd_fade 0 %d", 1000 + exitTime)); //----(SA)	added
...
qboolean AICast_ScriptAction_LightningDamage(cast_state_t *cs, char *params) {
  ...
  if (!Q_stricmp(params, "on")) {
    cs->aiFlags |=
        AIFL_ROLL_ANIM; // hijacking this since the player doesn't use it
  } else {
...
AICast_ScriptAction_BackupScript
  ...
  backs up the current state of the scripting, so we can restore it later and
resume were we left off (useful if player gets in our way)
=================
*/
...
qboolean ScriptStartCam(cast_state_t *cs, char *params, qboolean black) {
  ...
  trap_SendServerCommand(cs->entityNum,
                         va("startCam %s %d", token, (int)black));
...
qboolean AICast_ScriptAction_Cvar(cast_state_t *cs, char *params) {
  ...
  if (!strcmp(cvarName, "objective")) {
    G_Printf("WARNING: 'objective' cvar set from script.  Do not set directly. "
             " Use 'missionsuccess <num>'\n");
    return qtrue;
  }
...
qboolean AICast_ScriptAction_MusicStart(cast_state_t *cs, char *params) {
  ...
  trap_SendServerCommand(cs->entityNum,
                         va("mu_start %s %d", cvarName, fadeupTime));
...
qboolean AICast_ScriptAction_MusicPlay(cast_state_t *cs, char *params) {
  ...
  trap_SendServerCommand(cs->entityNum,
                         va("mu_play %s %d", cvarName, fadeupTime));
...
qboolean AICast_ScriptAction_MusicFade(cast_state_t *cs, char *params) {
  ...
  trap_SendServerCommand(cs->entityNum,
                         va("mu_fade %f %i", targetvol, fadetime));
...
qboolean AICast_ScriptAction_CatchFire(cast_state_t *cs, char *params) {
  ...
  //
  ent->s.onFireEnd =
      level.time + 99999; // make sure it goes for longer than they need to die
