...
void DeathmatchScoreboardMessage(gentity_t *ent) {
  ...
  trap_SendServerCommand(ent - g_entities,
                         va("scores %i %i %i%s", i, level.teamScores[TEAM_RED],
...
==============
*/
void G_setfog(char *fogstring) { trap_SetConfigstring(CS_FOGVARS, fogstring); }
...
==============
*/
void Cmd_Fogswitch_f(void) { G_setfog(ConcatArgs(1)); }
...
void Cmd_Give_f(gentity_t *ent) {
  ...
  if (give_all || Q_stricmp(name, "weapons") == 0) {
    // ent->client->ps.weapons[0] = (1 << (WP_MONSTER_ATTACK1)) - 1 -
    // (1<<WP_NONE);	//----(SA)	gives the cross now as well
  ...
  if (give_all || Q_stricmpn(name, "armor", 5) == 0) {
    if (g_gametype.integer ==
        GT_SINGLE_PLAYER) { // JPW NERVE -- no armor in multiplayer
      //----(SA)	modified
      if (amount) {
...
=================
*/
void SetWolfData(gentity_t *ent, char *ptype, char *weap, char *pistol,
                 char *grenade, char *skinnum) { // DHM - Nerve
  gclient_t *client;
...
void StopFollowing(gentity_t *ent) {
  ...
  if (g_gametype.integer !=
      GT_WOLF) { // NERVE - SMF - don't forcibly set this for multiplayer
...
void G_SayTo(gentity_t *ent, gentity_t *other, int mode, int color,
             const char *name,
             const char *message) { // removed static so it would link
  if (!other) {
    return;
  }
  if (!other->inuse) {
  ...
  if (mode == SAY_LIMBO) {
    trap_SendServerCommand(
        other - g_entities,
        va("%s \"%s%c%c%s\"", "lchat", name, Q_COLOR_ESCAPE, color, message));
  }
  // -NERVE - SMF
  else {
  ...
  case SAY_TEAM:
    ...
    if (Team_GetLocationMsg(ent, location, sizeof(location))) {
      Com_sprintf(name, sizeof(name),
                  "(%s%c%c) (%s): ", ent->client->pers.netname, Q_COLOR_ESCAPE,
...
void Cmd_StopCamera_f(gentity_t *ent) {
  ...
  if (ent->client->cameraPortal) {
    ...
    while (
        (sp = G_Find(sp, FOFS(classname),
                     "info_player_deathmatch"))) { // info_player_start becomes
                                                   // info_player_deathmatch in
                                                   // it's spawn functon
      if (Distance(ent->s.pos.trBase, sp->s.origin) < 256 &&
...
void Cmd_InterruptCamera_f(gentity_t *ent) {
  AICast_ScriptEvent(AICast_GetCastState(ent->s.number), "trigger",
                     "cameraInterrupt");
}
...
qboolean G_ThrowChair(gentity_t *ent, vec3_t dir, qboolean force) {
  ...
  trap_Trace(&trace, start, mins, maxs, end, ent->s.number,
             MASK_SOLID | MASK_MISSILESHOT);
  ...
  if (!isthrown && force) { // was not successfully thrown, but you /need/ to
                            // drop it.  break it.
...
void Cmd_Activate_f(gentity_t *ent) {
  ...
  if (traceEnt->classname) {
    traceEnt->flags &=
        ~FL_SOFTACTIVATE; // FL_SOFTACTIVATE will be set if the user is holding
                          // his 'walk' key down when activating things

    if (((Q_stricmp(traceEnt->classname, "func_door") == 0) ||
      ...
      if (traceEnt->touch) {
        if (ent->client->pers.autoActivate == PICKUP_ACTIVATE) {
          ent->client->pers.autoActivate =
              PICKUP_FORCE; //----(SA) force the pickup of a normally
                            //autoactivate only item
        }
        traceEnt->active = qtrue;
        traceEnt->touch(traceEnt, ent, &trace);
                ...
                0) /*&& activatetime > oldactivatetime + 1000*/
               && traceEnt->active == qfalse) {
      if (!ent->active && traceEnt->takedamage) { // not a dead gun
        ...
        if (!(ent->client->ps.pm_flags & PMF_DUCKED) &&
          ...
          if (!(cl->ps.grenadeTimeLeft)) { // make sure the client isn't holding
                                           // a hot potato
            traceEnt->active = qtrue;
            ent->active = qtrue;
            ...
            if (!(ent->r.svFlags & SVF_CASTAI)) {
              G_UseTargets(traceEnt,
                           ent); //----(SA)	added for Mike so mounting an
                                 //MG42 can be a trigger event (let me know if
                                 //there's any issues with this)
            }
            return; // avoid dropping down to below, where we get thrown
                    // straight off again (AI)
          }
        }
      }
    } else if ((Q_stricmp(traceEnt->classname, "misc_flak") ==
                0) /*&& activatetime > oldactivatetime + 1000*/
               && traceEnt->active == qfalse) {
      if (!infront(traceEnt,
                   ent)) { // make sure the client isn't holding a hot potato
        gclient_t *cl;
        cl = &level.clients[ent->s.clientNum];
    ...
    // chairs
    else if (traceEnt->isProp && traceEnt->takedamage &&
             traceEnt->s.pos.trType == TR_STATIONARY && !traceEnt->nopickup) {
      if (!ent->active) {
        if (traceEnt->active) {
          // ?
          traceEnt->active = qfalse;
...
int Cmd_WolfKick_f(gentity_t *ent) {
  ...
  qboolean solidKick = qfalse; // don't play "hit" sound on a trigger unless
                               // it's an func_invisible_user
  ...
  if (!ent->melee) { // because we dont want you to open a door with a prop
    if ((Q_stricmp(traceEnt->classname, "func_door_rotating") == 0) &&
      ...
      if (traceEnt->key >= KEY_LOCKED_TARGET) { // door force locked
        ...
        tent->s.otherEntityNum = ent->s.number; //----(SA)	end

        AICast_AudibleEvent(
            ent->s.clientNum, tr.endpos,
            HEAR_RANGE_DOOR_KICKLOCKED); // "someone kicked a locked door near
                                         // me!"
      ...
      if (traceEnt->teammaster && traceEnt->team &&
          traceEnt != traceEnt->teammaster) {
      ...
      traceEnt->flags |=
          FL_KICKACTIVATE; // so cell doors know they were kicked
                           // It doesn't hurt to pass this along since only ent
                           // use() funcs who care about it will check. However,
                           // it may become handy to put a "KICKABLE" or
                           // "NOTKICKABLE" flag on the invisible_user
      traceEnt->use(traceEnt, ent, ent);
      traceEnt->flags &= ~FL_KICKACTIVATE; // reset

      solidKick = qtrue; //----(SA)
    } else if (!Q_stricmp(traceEnt->classname, "props_flippy_table") &&
               traceEnt->use) {
      traceEnt->use(traceEnt, ent, ent);
    } else if (!Q_stricmp(traceEnt->classname, "misc_mg42")) {
  ...
  if (traceEnt->takedamage && traceEnt->client) {
    ...
    if (traceEnt->r.contents >= 0 &&
        (traceEnt->r.contents & CONTENTS_TRIGGER) && !solidKick) {
      tent = G_TempEntity(tr.endpos,
                          EV_WOLFKICK_MISS); // (SA) don't play the "hit" sound
                                             // if you kick most triggers
    } else {
      tent = G_TempEntity(tr.endpos, EV_WOLFKICK_HIT_WALL);
    }
...
void ClientDamage(gentity_t *clent, int entnum, int enemynum, int id) {
  ...
  // sanity check (also protect from cheaters)
  //	if (g_gametype.integer != GT_SINGLE_PLAYER && entnum != clent->s.number)
  //{ 		trap_DropClient( clent->s.number, "Dropped due to illegal ClientDamage
  //command\n" ); 		return;
  //	}
  // -NERVE - SMF
  ...
  case CLDMG_DEBRIS:
    if (g_gametype.integer == GT_SINGLE_PLAYER) {
      G_Damage(ent, enemy, enemy, vec3_origin, vec3_origin, 3 + rand() % 3,
               DAMAGE_NO_KNOCKBACK, MOD_EXPLOSIVE);
    }
    break;
  case CLDMG_SPIRIT:
    if (g_gametype.integer == GT_SINGLE_PLAYER) {
      if (enemy->aiCharacter == AICHAR_ZOMBIE) {
        G_Damage(ent, enemy, enemy, vec3_origin, vec3_origin, 6,
                 DAMAGE_NO_KNOCKBACK, MOD_ZOMBIESPIRIT);
      } else {
        G_Damage(ent, enemy, enemy, vec3_origin, vec3_origin, 8 + rand() % 4,
                 DAMAGE_NO_KNOCKBACK, MOD_ZOMBIESPIRIT);
      }
    }
    break;
  case CLDMG_BOSS1LIGHTNING:
    ...
    if (ent->takedamage) {
      ...
      VectorNormalize(vec);
      G_Damage(ent, enemy, enemy, vec, ent->r.currentOrigin, 6 + rand() % 3, 0,
               MOD_LIGHTNING);
    }
    break;
  case CLDMG_TESLA:
    ...
    if (g_gametype.integer != GT_SINGLE_PLAYER) {
      ...
      // if ( AICast_GetCastState( enemy->s.number )->lastWeaponFiredWeaponNum
      // != WP_TESLA ) 	break; if ( AICast_GetCastState( enemy->s.number
      // )->lastWeaponFired < level.time - 400 ) 	break;
    }

    if ((ent->aiCharacter == AICHAR_PROTOSOLDIER) ||
        (ent->aiCharacter == AICHAR_SUPERSOLDIER) ||
        (ent->aiCharacter == AICHAR_LOPER) ||
        (ent->aiCharacter >= AICHAR_STIMSOLDIER1 &&
         ent->aiCharacter <= AICHAR_STIMSOLDIER3)) {
      break;
    }
    ...
    if (ent->takedamage /*&& !AICast_NoFlameDamage(ent->s.number)*/) {
      ...
      if (!(enemy->r.svFlags & SVF_CASTAI)) {
        G_Damage(ent, enemy, enemy, vec, ent->r.currentOrigin, 8, 0,
                 MOD_LIGHTNING);
      } else {
        G_Damage(ent, enemy, enemy, vec, ent->r.currentOrigin, 4, 0,
                 MOD_LIGHTNING);
      }
    }
    break;
  case CLDMG_FLAMETHROWER:
    ...
    if (g_gametype.integer != GT_SINGLE_PLAYER) {
      ...
      //			if ( !(enemy->client->buttons & BUTTON_ATTACK) )
      //// JPW NERVE flames should be able to damage while puffs are active
      //				break;
    } else {
      // this is required for Zombie flame attack
      // if ((enemy->aiCharacter == AICHAR_ZOMBIE) && !AICast_VisibleFromPos(
      // enemy->r.currentOrigin, enemy->s.number, ent->r.currentOrigin,
      // ent->s.number, qfalse )) 	break;
    }
...
#define FLAME_THRESHOLD 50
      ...
      //	G_Damage( traceEnt, ent, ent, forward, tr.endpos, 1, 0,
      //MOD_LIGHTNING);
      ...
      if (ent->flameQuotaTime && ent->flameQuota > 0) {
        ent->flameQuota -=
            (int)(((float)(level.time - ent->flameQuotaTime) / 1000) *
                  (float)damage / 2.0);
        if (ent->flameQuota < 0) {
          ent->flameQuota = 0;
        }
      ...
      // Ridah, make em burn
      if (ent->client &&
          (/*g_gametype.integer != GT_SINGLE_PLAYER ||*/ !(ent->r.svFlags &
                                                           SVF_CASTAI) ||
           ent->health <= 0 || ent->flameQuota > FLAME_THRESHOLD)) {
...
void Cmd_EntityCount_f(gentity_t *ent) {
...

  {
    ...
    G_Printf("kills %i/%i nazis %i/%i monsters %i/%i \n", kills[0], kills[1],
             nazis[0], nazis[1], monsters[0], monsters[1]);
  }
}
...
void ClientCommand(int clientNum) {
  ...
  //	else if (Q_stricmp (cmd, "callvote") == 0)	//----(SA)	id requests
  //these gone in sp 		Cmd_CallVote_f (ent); 	else if (Q_stricmp (cmd, "vote") ==
  //0)		//----(SA)	id requests these gone in sp 		Cmd_Vote_f (ent);
