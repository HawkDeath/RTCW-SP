...
void limbo(gentity_t *ent) {
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    G_Printf(
        "FIXME: limbo called from single player game.  Shouldn't see this\n");
    return;
  }
...
void reinforce(gentity_t *ent) {
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    G_Printf("FIXME: reinforce called from single player game.  Shouldn't see "
             "this\n");
    return;
  }
  if (!(ent->client->ps.pm_flags & PMF_LIMBO)) {
  ...
  while (!finished) {
    ...
    for (i = 1; i < numDeployable; i++)
      if (level.clients[deployable[i]].deployQueueNumber <
          level.clients[deployable[i - 1]].deployQueueNumber) {
        G_Printf("swapping %d and %d (priority %d and %d)\n", deployable[i],
                 deployable[i - 1],
                 level.clients[deployable[i]].deployQueueNumber,
                 level.clients[deployable[i - 1]].deployQueueNumber);
        j = deployable[i - 1];
...
void SetWolfSpawnWeapons(gclient_t *client) {
  ...
  client->ps.powerups[PW_INVULNERABLE] =
      level.time + 5000; // JPW NERVE some time to find cover
  ...
  if (pc == PC_LT) {
    ...
    /* no grenades for balance JPW NERVE
                    ...
                    client->ps.ammo[BG_FindAmmoForWeapon(WP_GRENADE_LAUNCHER)] =
       2; client->ps.ammoclip[BG_FindClipForWeapon(WP_GRENADE_LAUNCHER)] = 1;
    */
  }
  ...
  if (pc == PC_SOLDIER || pc == PC_LT) {
    ...
    // JPW NERVE -- if LT is selected but illegal weapon, set to team-specific
    // SMG
  ...
  } else { // medic or engineer gets assigned MP40 or Thompson with one magazine
           // ammo
  ...
  // JPW NERVE -- medics on each team make cumulative health bonus -- this gets
  // overridden for "revived" players count up # of medics on team
  for (i = 0; i < level.maxclients; i++) {
    if (level.clients[i].pers.connected != CON_CONNECTED) {
      continue;
    }
...
==================
*/
qboolean G_CheckForExistingModelInfo(gclient_t *cl, char *modelName,
...
qboolean G_GetModelInfo(int clientNum, char *modelName,
  ...
  if (!G_CheckForExistingModelInfo(&level.clients[clientNum], modelName,
                                   modelInfo)) {
...
qboolean G_ParseAnimationFiles(char *modelname, gclient_t *cl) {
  ...
  Q_strncpyz(cl->modelInfo->modelname, modelname,
             sizeof(cl->modelInfo->modelname));
  ...
  Com_sprintf(filename, sizeof(filename), "models/players/%s/wolfanim.script",
              modelname);
  len = trap_FS_FOpenFile(filename, &f, FS_READ);
  if (len <= 0) {
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER && g_entities[0].client &&
      g_entities[0].client->pers.connected == CON_CONNECTED) {
...
void ClientUserinfoChanged(int clientNum) {
  ...
  if (ent->r.svFlags & SVF_BOT) {
    ...
    s = va("n\\%s\\t\\%i\\model\\%s\\head\\%s\\c1\\%s\\hc\\%i\\w\\%i\\l\\%"
           "i\\skill\\%s",
...
void ClientSpawn(gentity_t *ent) {
  ...
  if (g_gametype.integer == GT_WOLF) {
    SetWolfSpawnWeapons(
        client); // JPW NERVE -- increases stats[STAT_MAX_HEALTH] based on # of
                 // medics in game
  }
  // dhm - end

  // Note to Ryan:
  // had to add this because key word giveweapon to player is causing a fatal
  // crash This is only a quick fix for the beach map
  /*
          if (!(ent->r.svFlags & SVF_CASTAI) && level.scriptAI && strstr
     (level.scriptAI, "beach assault"))
          {
                  COM_BitSet( client->ps.weapons, WP_THOMPSON );
                  client->ps.ammo[BG_FindAmmoForWeapon(WP_THOMPSON)] = 100;

                  COM_BitSet( client->ps.weapons, WP_GRENADE_PINEAPPLE );
                  client->ps.ammo[BG_FindAmmoForWeapon(WP_GRENADE_PINEAPPLE)] =
     5;
  ...
  */
  ...
  // *FOLLOW* SetWolfSpawnWeapons() in multiplayer SetWolfSpawnWeapons() now
...
void G_RetrieveMoveSpeedsFromClient(int entnum, char *text) {
  ...
  while (1) {
    ...
    if (anim->moveSpeed == 0) {
      G_Error("G_RetrieveMoveSpeedsFromClient: trying to set movespeed for "
              "non-moving animation");
    }
