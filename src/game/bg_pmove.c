...
static float PM_CmdScale(usercmd_t *cmd) {
  ...
  if (pm->ps->aiChar && !(pm->ps->eFlags & EF_DUMMY_PMOVE)) {
    // restrict AI character movements (don't strafe or run backwards as fast as
    // they can run forwards)
...
// this is the counterstrike way of doing it -- ie you can switch to a non-heavy
// weapon and move at full speed.  not completely realistic (well, sure, you can
// run faster with the weapon strapped to your back than in carry position) but
// more fun to play.  If it doesn't play well this way we'll bog down the player
// if the own the weapon at all.
//
// added #ifdef for game/cgame to project so we can get correct g_gametype
// variable and only do this in multiplayer if necessary
#ifdef CGAMEDLL
  if (cg_gameType.integer != GT_SINGLE_PLAYER)
#endif
#ifdef GAMEDLL
...
static qboolean PM_CheckJump(void) {
  ...
  if (pm->cmd.serverTime - pm->ps->jumpTime <
      500) { // (SA) trying shorter time.  I find this effect annoying ;)
    return qfalse;
  }
...
// Ridah, removed this code since we don't have a grapple, and the grapplePoint
was consuming valuable msg space #if 0 vec3_t vel, v; float vlen;
...
// JPW NERVE
...
#endif
      {
        ...
        if (pm->ps->powerups[PW_NOFATIGUE]) {
          if (pm->ps->powerups[PW_NOFATIGUE] > stamtake) {
            ...
            // don't have that much bonus.  clear what you've got and take the
            // remainder from regular stamina
  ...
  } else if ((pm->ps->stats[STAT_HEALTH] <= 0) && pm->ps->aiChar &&
  ...
  // pm->ps->velocity[1], pm->ps->velocity[2]); Com_Printf("velocity1 = %1.1f\n",
...
static void PM_CrashLand(void) {
  ...
  if (!(pml.groundTrace.surfaceFlags & SURF_NODAMAGE)) {
    ...
    if (delta > 77) {
      ...
      if (!(pm->ps->pm_flags & PMF_DUCKED) &&
          !(pm->cmd.buttons &
            BUTTON_WALKING)) { // quiet if crouching or walking
...
#define AI_STEPTEST_FALLDIST_PER_SEC                                           \
  (STEPSIZE + pm->ps->gravity *                                                \
                  0.35) // always allow them to fall down the minimum stepsize
  ...
  if (pm->ps->groundEntityNum != ENTITYNUM_NONE) {
    ...
    if (pm->ps->aiChar) {
      if (trace.fraction < 1.0) {
        ...
        if (xyspeed > 120 &&
            falldist * pml.frametime < (xyspeed * pml.frametime + STEPSIZE)) {
...
static void PM_Footsteps(void) {
  ...
  if (pm->ps->pm_flags & PMF_DUCKED) {
    ...
    if (!(pm->cmd.buttons & BUTTON_WALKING)) {
      ...
      if (pm->cmd.rightmove && !pm->cmd.forwardmove) {
        if (pm->cmd.rightmove > 0) {
          animResult = BG_AnimScriptAnimation(pm->ps, pm->ps->aiState,
                                              ANIM_MT_STRAFERIGHT, qtrue);
        } else {
          animResult = BG_AnimScriptAnimation(pm->ps, pm->ps->aiState,
                                              ANIM_MT_STRAFELEFT, qtrue);
        }
      }
    ...
    if (!(pm->cmd.buttons & BUTTON_WALKING)) {
      ...
      if (animResult < 0) { // if we havent found an anim yet, play the run
        animResult =
            BG_AnimScriptAnimation(pm->ps, pm->ps->aiState, ANIM_MT_RUN, qtrue);
      }
      if (animResult < 0) { // if we havent found an anim yet, play the run
        animResult = BG_AnimScriptAnimation(pm->ps, pm->ps->aiState,
                                            ANIM_MT_WALK, qtrue);
  ...
  if (animGap > 0) {
    ...
    if (pm->ps->aiChar == AICHAR_SUPERSOLDIER ||
        pm->ps->aiChar == AICHAR_PROTOSOLDIER) {
      // iswalking = qfalse;
    ...
    if (iswalking /*|| (pm->ps->aiChar == AICHAR_HEINRICH)*/) {
      ...
      if (pm->waterlevel == 0) {
        ...
        if (footstep && !pm->noFootsteps) {
          //				if (pm->ps->aiChar == AICHAR_HEINRICH) {	// <--
          //this stuff now handled in PM_footstepforsurf 					PM_AddEvent(
          //EV_FOOTSTEP ); 				} else {
          PM_AddEvent(PM_FootstepForSurface());
          //				}
        }
...
static void PM_BeginWeaponReload(int weapon) {
  ...
  default:
    // DHM - Nerve :: override current animation (so reloading after firing will
    // work)
...
*/
static void
PM_BeginWeaponChange(int oldweapon, int newweapon,
                     qboolean reload) { //----(SA)	modified to play 1st
                                        //person alt-mode transition animations.
  int switchtime;
  ...
  if (!pm->ps->aiChar && !(pm->ps->eFlags & EF_DEAD) &&
      (newweapon == WP_NONE)) { // RF, dont allow changing to null weapon
    return;
  }
  ...
  if (pm->ps->weaponstate == WEAPON_DROPPING ||
      pm->ps->weaponstate == WEAPON_DROPPING_TORELOAD) { //----(SA)	added
    return;
  }

  if (pm->ps->grenadeTimeLeft >
      0) { // don't allow switch if you're holding a hot potato or dynamite
    return;
  }
  ...
  if (oldweapon == WP_GRENADE_LAUNCHER || oldweapon == WP_GRENADE_PINEAPPLE ||
      oldweapon == WP_DYNAMITE || oldweapon == WP_PANZERFAUST) {
  ...
  case WP_GRENADE_PINEAPPLE:
    pm->ps->grenadeTimeLeft =
        0; // initialize the timer on the potato you're switching to
...
void PM_CheckForReload(int weapon) {
  ...
  // don't allow reloading for these weapons.  when the player hits 'reload' or
  // 'fire' when the weapon is empty, it will switch away to the primary.  (so
  // player can see results of last shot through scope without it auto-switching
  // away for the reload)
  if (!pm->ps->aiChar) { // RF, added this check since this confuses the AI
    ...
    case WP_FG42SCOPE:
      if (reloadRequested) {
        doReload = qtrue;
        if (!(pm->ps->ammo[ammoWeap])) { // no ammo left. when you switch out,
                                         // don't try to reload
          doReload = qfalse;
        }
  ...
  if (reloadRequested) {
    // don't allow a force reload if it won't have any effect (no more ammo
    // reserves or full clip)
    if (pm->ps->ammo[ammoWeap]) {
      ...
      if (weapon == WP_AKIMBO) {
        ...
        if (pm->ps->ammoclip[BG_FindClipForWeapon(WP_COLT)] <
            ammoTable[BG_FindClipForWeapon(WP_COLT)].maxclip) {
          doReload = qtrue;
        }
      }
    }
  ...
  // clip is empty, but you have reserves.  (auto reload)
    ...
    if (pm->ps->ammo[ammoWeap]) {           // and you have reserves
      if (weapon == WP_AKIMBO) { // if colt's got ammo, don't force reload yet
                                 // (you know you've got it 'out' since you've
                                 // got the akimbo selected
        if (!(pm->ps->ammoclip[WP_COLT])) {
          doReload = qtrue;
        }
        // likewise.  however, you need to check if you've got the akimbo
        // selected, since you could have the colt alone
...
void PM_WeaponUseAmmo(int wp, int amount) {
  ...
  if (pm->noWeapClips) {
    ...
    if (wp == WP_AKIMBO) {
      if (!BG_AkimboFireSequence(wp, pm->ps->ammoclip[WP_AKIMBO],
                                 pm->ps->ammoclip[WP_COLT])) {
        takeweapon = WP_COLT;
      }
    }
...
int PM_WeaponAmmoAvailable(int wp) {
  ...
  if (pm->noWeapClips) {
    ...
    takeweapon = BG_FindClipForWeapon(wp);
    if (wp == WP_AKIMBO) {
      if (!BG_AkimboFireSequence(pm->ps->weapon, pm->ps->ammoclip[WP_AKIMBO],
                                 pm->ps->ammoclip[WP_COLT])) {
        takeweapon = WP_COLT;
      }
    }
...
void PM_CoolWeapons(void) {
  ...
  if (pm->ps->weapon) {
    pm->ps->curWeapHeat = (((float)pm->ps->weapHeat[pm->ps->weapon] /
                            (float)ammoTable[pm->ps->weapon].maxHeat)) *
                          255.0f;
...
#define AIMSPREAD_VIEWRATE_RANGE 120.0f // degrees per second
  ...
  case WP_MP40:
    wpnScale =
        0.5f; // 2 handed, but not as long as mauser, so harder to keep aim
    break;
  case WP_FG42:
    wpnScale = 0.9f;
    break;
  ...
  if (wpnScale) {
    ...
    if ((pm->ps->eFlags & EF_CROUCHING) &&
        (pm->ps->groundEntityNum !=
         ENTITYNUM_NONE)) { //----(SA)	modified so you can't do this in the
                            //air.  cool?
      wpnScale *= 0.5;
    }
    // jpw
...
static void PM_Weapon(void) {
  ...
  akimboFire = BG_AkimboFireSequence(
      pm->ps->weapon, pm->ps->ammoclip[WP_AKIMBO], pm->ps->ammoclip[WP_COLT]);
  ...
  if (pm->cmd.buttons & BUTTON_USE_HOLDABLE) {
    if (!(pm->ps->pm_flags & PMF_USE_ITEM_HELD)) {
      ...
      if (pm->cmd.holdable) {
        ...
        if (item && (pm->ps->holdable[pm->cmd.holdable] >=
                     item->quantity)) { // ->quantity being how much 'ammo' is
                                        // taken per use
  ...
  if (pm->ps->weapon == WP_GRENADE_LAUNCHER ||
    ...
    // (SA) AI's don't set grenadeTimeLeft on +attack, so I don't check for
    // (pm->ps->aiChar) here
    if (pm->ps->grenadeTimeLeft > 0) {
      if (pm->ps->weapon == WP_DYNAMITE) {
        pm->ps->grenadeTimeLeft += pml.msec;
// JPW NERVE -- in multiplayer, dynamite becomes strategic, so start timer @ 30
// seconds
#ifdef CGAMEDLL
        if (cg_gameType.integer != GT_SINGLE_PLAYER) {
#endif
          ...
          if (pm->ps->grenadeTimeLeft <=
              0) { // give two frames advance notice so there's time to launch
                   // and detonate
    ...
    if (pm->ps->weaponDelay > 0) {
      ...
      if (pm->ps->weaponDelay <= 0) {
        ...
        // double check the player is still holding the fire button down for
        // these weapons so you don't get a delayed "non-fire" (fire hit and
        // released, then shot fires)
        switch (pm->ps->weapon) {
        case WP_VENOM:
...
/*
                // RF, testing special case Pistol, which fires faster if you
   tap the fire button if ( pm->ps->weapon == WP_LUGER ) { if (
   pm->ps->releasedFire ) { if (pm->ps->weaponTime <= 250 && (pm->cmd.buttons &
   BUTTON_ATTACK)) { pm->ps->weaponTime = 0;
                                }
   ...
   (pm->cmd.buttons & BUTTON_ATTACK)) { pm->ps->weaponTime = 0;
                                }
                        } else if (!(pm->cmd.buttons & BUTTON_ATTACK)) {
                                pm->ps->releasedFire = qtrue;
                        }
...
// JPW NERVE -- added back for multiplayer pistol balancing
      ...
      if (pm->ps->weaponstate == WEAPON_RAISING) {
        ...
        pm->ps->weaponstate =
            WEAPON_READY; // need to switch to READY so the reload will work
...
// charge bar isn't full
        ...
        if (pm->ps->weapon == WP_DYNAMITE) {
          if (pm->cmd.serverTime - pm->ps->classWeaponTime <
              g_engineerChargeTime
                  .integer) { // had to use multiplier because chargetime is
                              // used elsewhere as bomb diffuse time FIXME not
                              // really but NOTE changing bomb diffuse time will
                              // require changing this time as well (intended
                              // function: new charge ready when old one
                              // explodes, ie every 30 seconds or so)
      ...
      if (cg_gameType.integer == GT_WOLF) {
        if (pm->ps->weapon == WP_PANZERFAUST) {
          if (pm->cmd.serverTime - pm->ps->classWeaponTime <
              cg_soldierChargeTime.integer) {
            return;
          }
        }
        if (pm->ps->weapon == WP_DYNAMITE) {
          if (pm->cmd.serverTime - pm->ps->classWeaponTime <
              cg_engineerChargeTime.integer) {
            return;
          }
        }
      }
      ...
      if (!(pm->cmd.buttons & (BUTTON_ATTACK | WBUTTON_ATTACK2)) &&
          !delayedFire) { // if not on fire button and there's not a delayed
                          // shot this frame...
        pm->ps->weaponTime = 0;
        pm->ps->weaponDelay = 0;
      ...
      if (pm->ps->leanf != 0 && pm->ps->weapon != WP_GRENADE_LAUNCHER &&
      ...
      default:
        if (!weaponstateFiring) {
          pm->ps->weaponDelay =
              ammoTable[pm->ps->weapon]
                  .fireDelayTime; // delay so the weapon can get up into
                                  // position before firing (and showing the
                                  // flash)
        } else {
          BG_AnimScriptEvent(pm->ps, ANIM_ET_FIREWEAPON, qfalse, qtrue);
        }
      ...
      case WP_GRENADE_PINEAPPLE:
        if (!delayedFire) {
          if (pm->ps->aiChar) { // ai characters go into their regular animation
                                // setup
            BG_AnimScriptEvent(pm->ps, ANIM_ET_FIREWEAPON, qtrue, qtrue);
      ...
      if (pm->ps->weapon) {
        ...
        if (ammoNeeded > ammoAvailable) {
          ...
          reloadingW = (qboolean)(
              ammoNeeded <= pm->ps->ammo[BG_FindAmmoForWeapon(
                                pm->ps->weapon)]); // you have ammo for this,
                                                   // just not in the clip

          if (pm->ps->eFlags & EF_MELEE_ACTIVE) { // not going to be allowed to
                                                  // reload if holding a chair
            reloadingW = qfalse;
          }
      ...
      if (pm->ps->weaponDelay > 0) {
        // if it hits here, the 'fire' has just been hit and the weapon dictated
        // a delay. animations have been started, weaponstate has been set, but
        // no weapon events yet. (except possibly EV_NOAMMO) checks for delayed
        // weapons that have already been fired are return'ed above.
        return;
      }
      ...
      if (ammoTable[pm->ps->weapon].maxHeat) {
        pm->ps->weapHeat[pm->ps->weapon] +=
            ammoTable[pm->ps->weapon].nextShotTime;
      }
      ...
      case WP_AKIMBO:
        ...
        // (SA) (added check for last shot in both guns so there's no delay for
        // the last shot)
        if (!pm->ps->ammoclip[WP_AKIMBO] || !pm->ps->ammoclip[WP_COLT]) {
          if ((!pm->ps->ammoclip[WP_AKIMBO] && !akimboFire) ||
              (!pm->ps->ammoclip[WP_COLT] && akimboFire)) {
      ...
      case WP_SNIPERRIFLE: // (SA) not so much added per shot.  these weapons
                           // mostly uses player movement to get out of whack
        addTime = ammoTable[pm->ps->weapon].nextShotTime;
// JPW NERVE crippling the rifle a bit in multiplayer; it's way too strong so
// make it go completely out every time you fire
#ifdef CGAMEDLL
        if (cg_gameType.integer != GT_SINGLE_PLAYER) {
#endif
#ifdef GAMEDLL
          if (g_gametype.integer != GT_SINGLE_PLAYER) {
#endif
            //			addTime *= 2; // pulled this and reduced rifle
            //damage
        ...
        case WP_SNOOPERSCOPE:
          ...
          // strong so make it go completely out every time you fire snooper
          // doesn't do one-shot body kills, so give it a little less bounce
          addTime = ammoTable[pm->ps->weapon].nextShotTime;
#ifdef CGAMEDLL
          if (cg_gameType.integer != GT_SINGLE_PLAYER) {
#endif
          ...
          case WP_CLASS_SPECIAL:
            // can't reliably get cvars for restart time in here, so set it
            // small and check it in fireweapon routine
            addTime = 50;
            break;
          // jpw
          case WP_MONSTER_ATTACK1:
          ...
          if ((pm->ps->aiChar != AICHAR_PROTOSOLDIER) &&
              ...
              (ammoTable[pm->ps->weapon].maxHeat &&
               pm->ps->weapHeat[pm->ps->weapon])) {
            // it is overheating
            if (pm->ps->weapHeat[pm->ps->weapon] >=
                ammoTable[pm->ps->weapon].maxHeat) {
              pm->ps->weapHeat[pm->ps->weapon] =
                  ammoTable[pm->ps->weapon].maxHeat; // cap heat to max
              PM_AddEvent(EV_WEAP_OVERHEAT);
              //			PM_StartWeaponAnim(WEAP_IDLE1); //
              //removed.  client handles anim in overheat event
              addTime =
          ...
          /*
                  if ( pm->cmd.buttons & BUTTON_GESTURE ) {
                          if ( pm->ps->torsoTimer == 0) {
                                  ...
                                  if (!pm->ps->aiChar)	// Ridah, we'll play a
             custom sound upon calling the Taunt PM_AddEvent( EV_TAUNT );
...
#define TRACE_LADDER_DIST 48.0
          ...
          /*
          if (pm->ps->eFlags & EF_DEAD) {	// dead bodies should fall down
          ladders return;
          }
          ...
          */
          ...
          /*
                  if (!pml.ladder && DotProduct(pm->ps->velocity, pml.forward) <
             0) {
                          // trace along the negative velocity, so we grab onto
             a ladder if we are trying to reverse onto it from above the ladder
                          flatforward[0] = -pm->ps->velocity[0];
                          flatforward[1] = -pm->ps->velocity[1];
                          flatforward[2] = 0;
                          VectorNormalize (flatforward);

                          VectorMA (pm->ps->origin, tracedist, flatforward,
             spot); pm->trace (&trace, pm->ps->origin, pm->mins, pm->maxs, spot,
          ...
          */
          ...
          if (pml.ladder && !pml.walking &&
              (trace.fraction * tracedist > 1.0)) {
            vec3_t mins;
            // if we are only just on the ladder, don't do this yet, or it may
            // throw us back off the ladder
            pml.ladder = qfalse;
            VectorCopy(pm->mins, mins);
            mins[2] = -1;
            VectorMA(pm->ps->origin, -tracedist, laddervec, spot);
            pm->trace(&trace, pm->ps->origin, mins, pm->maxs, spot,
                      pm->ps->clientNum, pm->tracemask);
            if ((trace.fraction < 1) && (trace.surfaceFlags & SURF_LADDER)) {
              // if AI, then be more stringent on their viewangles
              if (pm->ps->aiChar &&
                  (DotProduct(trace.plane.normal, pml.forward) > -0.9)) {
                pml.ladder = qfalse;
              } else {
                ladderforward = qtrue;
                pml.ladder = qtrue;
          ...
          if (pml.ladder) {
            if (pml.walking) {
              // we are currently on the ground, only go up and prevent X/Y if
        ...
        */
        ...
        void PM_Sprint(void) {
          if ((pm->cmd.buttons & BUTTON_SPRINT) &&
              ...
              !(pm->ps->pm_flags & PMF_DUCKED) && (!pm->waterlevel)) {

            if (pm->ps
                    ->powerups[PW_NOFATIGUE]) { // take time from powerup before
                                                // taking it from sprintTime
              pm->ps->powerups[PW_NOFATIGUE] -= 50;

              pm->ps->sprintTime +=
                  10; // (SA) go ahead and continue to recharge stamina at
                      // double rate with stamina powerup even when exerting
              ...
              if (VectorLength(pm->ps->velocity) >
                  128) { // (SA) check for a bit more movement
                pm->ps->sprintTime -= 50;
              }
            }
            ...
            // additional (2x) recharge if in top 75% of sprint bar, or with
            // stamina powerup
        ...
        */
        ...
        void PmoveSingle(pmove_t * pmove) {
          ...
          if (pm->cmd.wbuttons & WBUTTON_ZOOM) {
            if (pm->ps->stats[STAT_KEYS] &
                (1 << INV_BINOCS)) { // (SA) binoculars are an inventory item
                                     // (inventory==keys)
              if (pm->ps->weapon != WP_SNIPERRIFLE &&
                  pm->ps->weapon != WP_SNOOPERSCOPE &&
                  pm->ps->weapon !=
                      WP_FG42SCOPE) { // don't allow binocs if using scope
                if (!(pm->ps->eFlags &
                      EF_MG42_ACTIVE)) { // or if mounted on a weapon
          ...
          if (pmove->cmd.buttons & BUTTON_TALK) {
            // keep the talk button set tho for when the cmd.serverTime > 66
            // msec and the same cmd is used multiple times in Pmove
          ...
          if (pm->ps->pm_type >= PM_DEAD ||
              pm->ps->pm_flags & PMF_LIMBO) { // DHM - Nerve
            pm->cmd.forwardmove = 0;
            pm->cmd.rightmove = 0;
            pm->cmd.upmove = 0;
          }
          ...
          if ((pm->ps->pm_type == PM_SPECTATOR) ||
              (pm->ps->pm_flags &
               PMF_LIMBO)) { // JPW NERVE for limbo mode chase
            PM_CheckDuck();
            PM_FlyMove();
            PM_DropTimers();
            return;
