...
void SP_ai_marker(gentity_t *ent) {
  ...
  if (!(ent->spawnflags & 1)) {
    ...
    trap_Trace(&tr, ent->r.currentOrigin, checkMins, checkMaxs, dest,
               ent->s.number, MASK_PLAYERSOLID | CONTENTS_MONSTERCLIP);
...
void ai_effect_think(gentity_t *ent) {
  ...
  if (!targ) {
    // keep waiting until they enter, if they never do, then we have no purpose,
    // therefore no harm can be done
  ...
  trap_LinkEntity(ent);
  ent->r.svFlags |=
      SVF_BROADCAST; // make sure all clients are aware of this entity
}
...
void AICast_trigger_wait(gentity_t *ent) { ent->nextthink = 0; }
...
void AICast_trigger_trigger(gentity_t *ent, gentity_t *activator) {
  ...
  if (ent->activator) { // they might be dead
    ...
    AICast_ScriptEvent(AICast_GetCastState(ent->activator->s.number), "trigger",
                       ent->target);
  }
