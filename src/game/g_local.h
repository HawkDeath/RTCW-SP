...
#define FL_KICKACTIVATE                                                        \
  0x00040000 //----(SA)	ent has been activated by a kick (doors use this too for
             //ex.)
#define FL_SOFTACTIVATE                                                        \
  0x00000040 //----(SA)	ent has been activated while 'walking' (doors use this
             //too for ex.)
...
struct gentity_s {
  ...
  qboolean aiInactive;
  int aiCharacter; // the index of the type of character we are (from
                   // aicast_soldier.c)
  // done.
  ...
  //----(SA)	added
  float activateArc; // right now just for mg42, but available for setting what
                     // angle this ent can be touched/killed from

  int props_frame_state;

  // Ridah
  int missionLevel;      // highest mission level completed (for previous level
                         // de-briefings)
  ...
  int grenadeExplodeTime; // we've caught a grenade, which was due to explode at
                          // this time
  ...
  int numScriptEvents;
  g_script_event_t *
      scriptEvents; // contains a list of actions to perform for each event type
  ...
  g_script_status_t
      scriptStatusCurrent; // had to go down here to keep savegames compatible
...
typedef struct {
  ...
  int autoActivate; // based on cg_autoactivate userinfo		(uses the PICKUP_
                    // values above)
  int emptySwitch;  // based on cg_emptyswitch userinfo (means "switch my weapon
                    // for me when ammo reaches '0' rather than -1)
...
struct gclient_s {
  ...
  int sniperRifleFiredTime; // JPW NERVE last time a sniper rifle was fired (for
                            // muzzle flip effects)
  float sniperRifleMuzzleYaw;   // JPW NERVE for time-dependent muzzle flip in
                                // multiplayer
...
typedef struct {
  ...
  // RF, record last time we loaded, so we can hack around sighting issues on
  // reload
  int lastLoadTime;

} level_locals_t;

// extern    qboolean	reloading;				// loading up a
// savegame
...
#define DAMAGE_PASSTHRU                                                        \
  0x00000040 // damage came through an explosive, or other player, or has in
             // some way already given damage to something
...
gentity_t *fire_zombiespirit(gentity_t *self, gentity_t *bolt, vec3_t start,
                             vec3_t dir);
...
qboolean G_GetModelInfo(int clientNum, char *modelName,
                        animModelInfo_t **modelInfo);
...
void G_SayTo(gentity_t *ent, gentity_t *other, int mode, int color,
             const char *name,
             const char *message); // JPW NERVE removed static declaration so it
                                   // would link
...
extern vmCvar_t ai_scriptName; // name of AI script file to run (instead of
                               // default for that map)
...
qboolean trap_AAS_RT_GetHidePos(vec3_t srcpos, int srcnum, int srcarea,
                                ...
                                vec3_t returnPos);
int trap_AAS_FindAttackSpotWithinRange(int srcnum, int rangenum, int enemynum,
                                       float rangedist, int travelflags,
                                       float *outpos);
qboolean trap_AAS_GetRouteFirstVisPos(vec3_t srcpos, vec3_t destpos,
                                      int travelflags, vec3_t retpos);
void trap_AAS_SetAASBlockingEntity(vec3_t absmin, vec3_t absmax,
                                   qboolean blocking);
// done.
