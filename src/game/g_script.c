...
g_script_stack_action_t gScriptActions[] = {
    ...
    // DHM - Nerve :: multiplayer scripting commands start with "wm_" (Wolf
    // Multiplayer)
    ...
    {"mu_start", G_ScriptAction_MusicStart}, // (char *new_music, int time)
                                             // // time to fadeup
    {"mu_play", G_ScriptAction_MusicPlay},   // (char *music)
    {"mu_stop", G_ScriptAction_MusicStop},   // (int time)						// time to
                                             // fadeout
    {"mu_fade", G_ScriptAction_MusicFade},   // (float target_volume, int time)
                                             // // time to fade to target
    {"mu_queue",
     G_ScriptAction_MusicQueue}, // (char *new_music)				// music
                                 // that will start when previous fades to 0
...
#define MAX_SCRIPT_EVENTS 64
  ...
  while (1) {
    ...
    if (!token[0]) {
      if (!wantName) {
        G_Error("G_Script_ScriptParse(), Error (line %d): '}' expected, end of "
                "script found.\n",
    ...
    if (token[0] == '}') {
      ...
      if (wantName) {
        G_Error("G_Script_ScriptParse(), Error (line %d): '}' found, but not "
                "expected.\n",
                COM_GetCurrentParseLine());
      }
      wantName = qtrue;
      ...
      if (wantName) {
        G_Error("G_Script_ScriptParse(), Error (line %d): '{' found, NAME "
                "expected.\n",
                COM_GetCurrentParseLine());
      }
    } else if (wantName) {
      ...
      if (numEventItems >= MAX_SCRIPT_EVENTS) {
        G_Error("G_Script_ScriptParse(), Error (line %d): MAX_SCRIPT_EVENTS "
                "reached (%d)\n",
      ...
      while ((token = COM_Parse(&pScript)) && (token[0] != '{')) {
        if (!token[0]) {
          G_Error("G_Script_ScriptParse(), Error (line %d): '}' expected, end "
                  "of script found.\n",
                  COM_GetCurrentParseLine());
        }
      ...
      while ((token = COM_Parse(&pScript)) && (token[0] != '}')) {
        ...
        if (curEvent->stack.numItems >= G_MAX_SCRIPT_STACK_ITEMS) {
          G_Error("G_Script_ScriptParse(): script exceeded MAX_SCRIPT_ITEMS "
                  "(%d), line %d\n",
...
void script_mover_die(gentity_t *self, gentity_t *inflictor,
...
void script_mover_pain(gentity_t *self, gentity_t *attacker, int damage,
                       vec3_t point) {
  G_Script_ScriptEvent(self, "pain",
                       va("%d %d", self->health, self->health + damage));
}
...
scripting) "health" optionally make this entity damagable
*/
...
void script_model_med_use(gentity_t *ent, gentity_t *other,
                          gentity_t *activator) {
