...
vmCvar_t ai_scriptName; // name of AI script file to run (instead of default for
                        // that map)
...
cvarTable_t gameCvarTable[] = {
    ...
    // Rafael gameskill
    {&g_gameskill, "g_gameskill", "2", CVAR_SERVERINFO | CVAR_LATCH, 0,
     qfalse}, // (SA) new default '2' (was '1')
    // done
    ...
    {&g_medicChargeTime, "g_medicChargeTime", "45000",
     CVAR_SERVERINFO | CVAR_LATCH, 0, qfalse},
    {&g_engineerChargeTime, "g_engineerChargeTime", "30000",
     CVAR_SERVERINFO | CVAR_LATCH, 0, qfalse},
    {&g_LTChargeTime, "g_LTChargeTime", "20000", CVAR_SERVERINFO | CVAR_LATCH,
     0, qfalse},
    ...
    {&g_syncronousClients, "g_syncronousClients", "0", CVAR_SYSTEMINFO, 0,
     qfalse},
    ...
    {&g_debugBullets, "g_debugBullets", "0", CVAR_CHEAT, 0,
     qfalse}, //----(SA)	added
    {&g_debugAudibleEvents, "g_debugAudibleEvents", "0", CVAR_CHEAT, 0,
     qfalse}, //----(SA)	added

    {&g_headshotMaxDist, "g_headshotMaxDist", "1024", CVAR_CHEAT, 0,
     qfalse}, //----(SA)	added
...
#pragma export off
  ...
  case AICAST_VISIBLEFROMPOS:
    return AICast_VisibleFromPos((float *)arg0, arg1, (float *)arg2, arg3,
                                 arg4);
...
==============
*/
void G_EndGame(void) { trap_Endgame(); }
...
#define CH_FRIENDLY_DIST                                                       \
  1024 // distance at which you can identify if someone is a friend
...
void G_CheckForCursorHints(gentity_t *ent) {
  ...
  nextCheckTime =
      level.time + 100; // wait a little before checking again	(10hz)
  ...
  trace_contents =
      (CONTENTS_TRIGGER | CONTENTS_SOLID | CONTENTS_PLAYERCLIP | CONTENTS_BODY |
       CONTENTS_CORPSE); // SP fine checking corpses
  trap_Trace(tr, offset, NULL, NULL, end, ps->clientNum, trace_contents);

  oldHintType = ps->serverCursorHint; // store the old one so we know when
  ...
  //
    ...
    // check for friendly.
    if (traceEnt->aiTeam == AITEAM_ALLIES ||
        traceEnt->aiTeam == AITEAM_NEUTRAL) {
    ...
    if (g_gametype.integer == GT_WOLF) {
      if (ps->stats[STAT_PLAYER_CLASS] == PC_MEDIC) {
        if (traceEnt->client && traceEnt->client->ps.pm_type == PM_DEAD &&
          ...
          hintType = HINT_BUILD;
          hintVal = ((float)traceEnt->client->medicHealAmt / 80.f) *
                    255.f; // also send health to client for visualization
          if (hintVal > 255) {
            hintVal = 255;
          }
        }
  ...
  //
    ...
    if (traceEnt->s.eType == ET_GENERAL) {
      ...
      if (!Q_stricmp(traceEnt->classname, "func_invisible_user")) {
        ...
        if (g_gametype.integer == GT_WOLF &&
            traceEnt->s.dmgFlags) { // hint icon specified in entity
    ...
    if (checkEnt) {
      if (checkEnt->s.eType == ET_GENERAL) {
        ...
        if (!Q_stricmp(traceEnt->classname, "ai_trigger")) {
          if ((!Q_stricmp(traceEnt->aiName, "player")) &&
            ...
            if (dist <= 255) {
              hintVal = (int)dist; // range for this hint is 256, so it happens
                                   // to translate nicely
            } else {
              hintVal = 255;
            }
        ...
        if (checkEnt->takedamage &&
            checkEnt->health > 0) { // 0 health explosives are not breakable
          hintDist = CH_BREAKABLE_DIST;
          hintType = HINT_BREAKABLE;
          hintVal =
              checkEnt->health; // also send health to client for visualization
        }
        ...
        if (!Q_stricmp(checkEnt->classname, "func_door_rotating")) {
          if (checkEnt->moverState == MOVER_POS1ROTATE || // stationary/closed
              (checkEnt->moverState == MOVER_POS2ROTATE &&
               checkEnt->flags & FL_TOGGLE)) { // toggle door that's open
          ...
          if (checkEnt->moverState == MOVER_POS1 || // stationary/closed
              (checkEnt->moverState == MOVER_POS2 &&
               checkEnt->flags & FL_TOGGLE)) { // toggle door that's open
      ...
      // hint icon specified in check entity (possibly an entity targeted by an
      // invis_user) and appropriate contact was made, so hintType was set first
      // try the checkent...
  ...
  for (i = 1, e = g_entities + i; i < level.num_entities; i++, e++) {
    ...
    for (j = i + 1, e2 = e + 1; j < level.num_entities; j++, e2++) {
      ...
      if (!strcmp(e->team, e2->team)) {
        ...
        e->teamchain = e2;
        e2->teammaster = e;
        //				e2->key = e->key;	// (SA) I can't set the
        //key here since the master door hasn't finished spawning yet and
      ...
      if (cv->modificationCount != cv->vmCvar->modificationCount) {
        ...
        if (!Q_stricmp(cv->cvarName, "g_playerStart")) {
          ...
          if (player && cv->vmCvar->integer) {
            ...
            // RF, we need to save the game after the "playestart" is called,
            // but before the camera is actually started (since the camera needs
            // to be triggered on loading the game) So we should call the
            // script, but not let it actually run. Therefore upon loading, the
            // script should run, and the camera start.

            saveGamePending = qtrue; // set this temporarily so we dont actually
                                     // run the script just yet
            AICast_ScriptEvent(AICast_GetCastState(player->s.number),
                               "playerstart", "");
            saveGamePending = qfalse; // set it back

            // save the "autosave\\<mapname>" savegame, which is taken before
            // any cameras have been played
            ...
            trap_Cvar_Set("cg_norender",
                          "0"); // camera has started, render 'on'
            trap_Cvar_Set("g_playerstart",
                          "0"); // reset calling of "playerstart" from script
          }
        }
      }
    }
...
==============
G_SpawnScriptCamera
        create the game entity that's used for camera<->script communication and
portal location for camera view
==============
*/
...
int G_SendMissionStats() {
  ...
  player = AICast_FindEntityForName("player");
  if (player) {
    attempts = AICast_NumAttempts(player->s.number) +
               1; // attempts tracks '0' as attempt 1
    AICast_AgePlayTime(player->s.number);
    playtime = AICast_PlayTime(player->s.number);

    for (i = 0; i < 8;
         i++) { // max objectives is '8'.  FIXME: use #define somewhere
  ...
  Q_strcat(cmd, sizeof(cmd),
           va(",%i,%i,%i", ((playtime / 1000) / 60) / 60, minutes,
              (playtime / 1000) % 60));
...
void G_InitGame(int levelTime, int randomSeed, int restart) {
  ...
  // don't need it.
  //	level.snd_fry = G_SoundIndex("sound/player/fry.wav");	// FIXME
  ...
  if (g_gametype.integer == GT_SINGLE_PLAYER) {
    ...
    // DHM - Nerve :: Moved this down so that it only happens in SinglePlayer
    // games
    AICast_Init();
    // done.
    ...
    for (i = 0; i < 8;
         i++) { // max objective cvars: 8 (FIXME: use #define somewhere)
      trap_Cvar_Set(va("g_objective%i", i + 1),
                    "0"); // clear the objective ROM cvars
    }
...
void G_ShutdownGame(int restart) {
  ...
  // Ridah, shutdown the Botlib, so weapons and things get reset upon doing a
  // "map xxx" command
  if (trap_Cvar_VariableIntegerValue("bot_enable")) {
    int i;
...
void CheckExitRules(void) {
  ...
  if (g_timelimit.integer && !level.warmupTime) {
    if (level.time - level.startTime >= g_timelimit.integer * 60000) {
      ...
      if (g_gametype.integer != GT_WOLF && g_gametype.integer != GT_CTF &&
          ScoreIsTied()) {
      ...
      if (level.reloadDelayTime < level.time) {
        ...
        if (g_reloading.integer == RELOAD_NEXTMAP_WAITING) {
          trap_Cvar_Set("g_reloading",
                        va("%d", RELOAD_NEXTMAP)); // set so sv_map_f will know
                                                   // it's okay to start a map
          if (g_cheats.integer) {
          ...
          G_EndGame(); // kick out to the menu and start the "endgame" menu
                       // (credits, etc)
...
void G_RunFrame(int levelTime) {
  ...
  for (i = 0; i < level.num_entities; i++, ent++) {
    ...
    // RF, if this entity is attached to a parent, move it around with it, so
    // the server thinks it's at least close to where the client will view it
    if (ent->tagParent) {
      vec3_t org;
    ...
    if (ent->eventTime && level.time - ent->eventTime > EVENT_VALID_MSEC) {
      ...
      // RF, clear all listed events (fixes hearing lots of sounds and events
      // after vid_restart)
      ...
      if (ent->client) {
        ...
        memset(ent->client->ps.eventParms, 0,
               sizeof(ent->client->ps.eventParms));
