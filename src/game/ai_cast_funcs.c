...
#define AICAST_LEADERDIST_MAX                                                  \
  240 // try and stay at least this close to them when nothing else to do
#define AICAST_LEADERDIST_MIN                                                  \
  64 // get this close if we have a clear line of sight to them
...
char *AIFunc_Restore(cast_state_t *cs) {
  // if the old aifunc was BattleChase, set it back to Battle, in case we have
...
float AICast_GetRandomViewAngle(cast_state_t *cs, float tracedist) {
  ...
  contents_mask =
      CONTENTS_SOLID | CONTENTS_PLAYERCLIP | CONTENTS_WATER | CONTENTS_SLIME;
...
AICast_MoveToPos()
  ...
  returns a pointer to the moveresult it used to make the move, so we can
investigate it outside of this function
============
*/
bot_moveresult_t *AICast_MoveToPos(cast_state_t *cs, vec3_t pos, int entnum) {
  ...
  if (entnum >= 0 && entnum < level.maxclients &&
      caststates[entnum].lastValidAreaTime[cs->aasWorldIndex] >
          level.time - 100) {
  ...
  if (aicast_debug.integer == 2 &&
      (g_entities[cs->entityNum].aiName &&
       !strcmp(aicast_debugname.string, g_entities[cs->entityNum].aiName))) {
  ...
  if (!(cs->aiFlags & AIFL_EXPLICIT_ROUTING) || (entnum < 0) ||
      Q_strcasecmp(g_entities[entnum].classname, "ai_marker")) {
    // use AAS routing
    ...
    if (lmoveresult.failure) {
      ...
      if (entnum > 0 && goal.areanum && entnum >= 0 &&
          entnum < level.maxclients) { // NOTE: dont do this for the player
      ...
      if (lmoveresult.flags & (MOVERESULT_MOVEMENTVIEW | MOVERESULT_SWIMVIEW)) {
        ...
        cs->aiFlags |= AIFL_VIEWLOCKED;
                   ...
                   BFL_ATTACKED)) { // if we are attacking, don't change angles
        bot_input_t bi;
        ...
        // look towards our future direction (like looking around a corner as we
        // approach it)
        if (!(cs->aiFlags & AIFL_WALKFORWARD) &&
            (lmoveresult.flags & MOVERESULT_FUTUREVIEW)) {
          if (AngleDifference(cs->ideal_viewangles[1],
                              lmoveresult.ideal_viewangles[1]) > 45) {
            cs->ideal_viewangles[1] -= 45;
          } else if (AngleDifference(cs->ideal_viewangles[1],
                                     lmoveresult.ideal_viewangles[1]) < -45) {
            cs->ideal_viewangles[1] += 45;
          } else {
          ...
          cs->ideal_viewangles[0] =
              0.5 * AngleNormalize180(cs->ideal_viewangles[0]);
        }
      }
    }
...
============
*/
float AICast_SpeedScaleForDistance(cast_state_t *cs, float startdist,
                                   float idealDist) {
...
void AICast_SpecialFunc(cast_state_t *cs) {
  ...
  case AICHAR_WARZOMBIE:
    ...
    // if we are pursuing the player from a distance, use our "crouch moving
    // defense"
    if ((enemy) &&
  ...
  case AICHAR_HELGA:
    ...
    if ((cs->weaponFireTimes[WP_MONSTER_ATTACK2] &&
         (cs->weaponFireTimes[WP_MONSTER_ATTACK2] > level.time - 12000)) ||
        (cs->weaponFireTimes[WP_MONSTER_ATTACK1] &&
         (cs->weaponFireTimes[WP_MONSTER_ATTACK1] > level.time - 6000))) {
  ...
  case AICHAR_HEINRICH:
    if ((ent->health <= 0.25 * cs->attributes[STARTING_HEALTH]) ||
        (cs->weaponFireTimes[WP_MONSTER_ATTACK1] >
         level.time - 6000) // walk for period after attack
        || (cs->weaponFireTimes[WP_MONSTER_ATTACK1] % 8000 <
            3000)) { // dont run constantly
      cs->actionFlags |= CASTACTION_WALK;
    } else { // charging
      cs->actionFlags &= ~CASTACTION_WALK;
    }
    ...
    if (ent->health <= 0 && ent->takedamage) {
      ...
      if (!ent->takedamage ||
          (ent->count2 < level.time && ent->client->ps.torsoTimer < 4000)) {
        int i;
        gentity_t *trav;
    ...
    break;
  case AICHAR_ZOMBIE:
    if (COM_BitCheck(cs->bs->cur_ps.weapons,
                     WP_MONSTER_ATTACK1)) { // flaming zombie, run
      BG_UpdateConditionValue(cs->entityNum, ANIM_COND_CHARGING, 1, qfalse);
    }
    break;
  }
...
char *AIFunc_Idle(cast_state_t *cs) {
  ...
  if (cs->dangerEntityValidTime >= level.time) {
    if (!AICast_GetTakeCoverPos(cs, cs->dangerEntity, cs->dangerEntityPos,
                                cs->takeCoverPos)) {
      // shit??
    }
  ...
  if (cs->leaderNum >= 0 &&
      Distance(cs->bs->origin, g_entities[cs->leaderNum].r.currentOrigin) >
          MAX_LEADER_DIST) {
    return AIFunc_ChaseGoalStart(cs, cs->leaderNum, AICAST_LEADERDIST_MAX,
                                 qtrue);
  }
  //
  ...
  if (numEnemies == -1) { // query mode
    ...
    for (i = 0; i < numEnemies; i++) {
      if (Distance(cs->bs->origin, cs->vislist[enemies[i]].visible_pos) >
          16) { // if we are really close to the last place we saw them, no
                // point trying to attack, since we'll just end up back here
        if (cs->enemyNum < 0) {
    ...
    if (cs->enemyNum >= 0) {
      if (((cs->leaderNum < 0) ||
           (cs->thinkFuncChangeTime < level.time - 3000)) &&
          AICast_WantsToChase(
              cs)) { // don't leave our leader as soon as we get to them
        return AIFunc_BattleStart(cs);
      } else if (AICast_EntityVisible(AICast_GetCastState(cs->enemyNum),
                                      cs->entityNum, qtrue) ||
                 AICast_CheckAttack(AICast_GetCastState(cs->enemyNum),
                                    cs->entityNum, qfalse)) {
        // if we are tactical enough, look for a hiding spot
        if (!(cs->leaderNum >= 0) && cs->attributes[TACTICAL] > 0.4 &&
          ...
          if (AICast_GetTakeCoverPos(cs, cs->enemyNum,
                                     ...
                                     cs->takeCoverPos)) {
            // go to a position that cannot be seen from the last place we saw
            // the enemy, and wait there for some time
        ...
        return AIFunc_BattleStart(cs);
      } else if (cs->leaderNum <
                 0) { // we should pursue if no leader, and not wanting to hide
        return AIFunc_BattleStart(cs);
      } else {
        // they can't see us anyway, so ignore them
        cs->lastEnemy =
            cs->enemyNum; // at least face them if they come to get us
        cs->enemyNum = -1;
        // crouching makes us look like we are hiding, which is what we are
        // doing
  ...
  //
  // if we couldn't find anything, see if our previous enemy is still around, if
  // so, go find them this is an attempt to prevent guys from running away to
  // hide from something, never to be seen again. They shouldn't really "forget"
  // that they are indeed soldiers.
  if (!(cs->leaderNum >= 0) && cs->lastEnemy >= 0 &&
  ...
  //
  // if we've recently been in a fight, keep looking around, so we don't look
  // stupid
  if (cs->lastEnemy >= 0) {
    // we have been in a battle, so face our enemy in anticipation of their
    // return
    if (ent->aiTeam != AITEAM_ALLIES) {
      vec3_t dir;
      //
      // VectorSubtract( cs->vislist[cs->lastEnemy].visible_pos, cs->bs->origin,
      // dir );
      // hack, use their real angles, prevent them from looking dumb when the
      // player returns
      VectorSubtract(g_entities[cs->lastEnemy].s.pos.trBase, cs->bs->origin,
                     dir);
      if (VectorLength(dir) < 1) {
        ...
        VectorNormalize(dir);
        vectoangles(dir, cs->ideal_viewangles);
        cs->ideal_viewangles[PITCH] =
            AngleNormalize180(cs->ideal_viewangles[PITCH]) * 0.5;
      }
    } else if (cs->attributes[TACTICAL] &&
      ...
      if ((cs->nextIdleAngleChange + 3000) < level.time) {
        ...
        //
        //	Given a src area, pick a random dest area which is visible from
        //that area 	and return it's position, which we'd then use to set the
        //next view angles
        //
        //	This would result in more efficient, more realistic behaviour,
        //since they'd 	also use PITCH angles to look at areas above/below them
      ...
      cs->idleYawChange = AngleDelta(cs->idleYaw, cs->ideal_viewangles[YAW]);
      cs->ideal_viewangles[YAW] = AngleMod(
          cs->ideal_viewangles[YAW] + (cs->idleYawChange * cs->bs->thinktime));
    }
  }
  ...
  if (cs->enemyNum < 0 && cs->attributes[TACTICAL] >= 0.5 &&
...
char *AIFunc_IdleStart(cast_state_t *cs) {
  ...
  if (cs->enemyNum >= 0) {
    if (g_entities[cs->entityNum].aiTeam == AITEAM_NAZI &&
        g_entities[cs->enemyNum].health <= 0) {
  ...
  //----(SA)	try always using the 'casual' stand on spawn and change to
  //crouching one when 'alerted'
  if (cs->aiFlags & AIFL_STAND_IDLE2) {
    //		if (rand()%2 || (cs->lastEnemy < 0 && cs->aiFlags &
    //AIFL_TALKING))
    g_entities[cs->entityNum].client->ps.eFlags |= EF_STAND_IDLE2;
    //		else
    //			g_entities[cs->entityNum].client->ps.eFlags &=
    //~EF_STAND_IDLE2;
  }
...
char *AIFunc_InspectFriendly(cast_state_t *cs) {
  ...
  cs->scriptPauseTime = level.time + 4000; // wait for at least this long before
                                           // resuming any scripted walking, etc
  ...
  if (!followent->inuse) {
    ...
    if (cs->followEntity < MAX_CLIENTS &&
        ((followent->client &&
          followent->client->pers.connected == CON_CONNECTING) ||
         (level.time < 3000))) {
  ...
  if (!(dist < cs->followDist &&
    ...
    if (!moved && (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE ||
                   g_entities[cs->entityNum].waterlevel > 1)) {
      ...
      if (!simTest) {
        ...
        trap_Trace(&tr, cs->bs->origin,
                   NULL /*g_entities[cs->entityNum].r.mins*/,
                   NULL /*g_entities[cs->entityNum].r.maxs*/,
                   followent->r.currentOrigin, cs->entityNum,
    ...
    if (!moved) {
      // use AAS routing
      moveresult =
          AICast_MoveToPos(cs, followent->r.currentOrigin, cs->followEntity);
      ...
      if (!moveresult || moveresult->failure) {
        ...
        if (!(cs->aiFlags & AIFL_MISCFLAG2)) {
          if (trap_AAS_GetRouteFirstVisPos(followent->r.currentOrigin,
                                           cs->bs->origin, cs->travelflags,
                                           cs->takeCoverEnemyPos)) {
    ...
    */
    ...
    if (g_entities[cs->inspectNum].health <= 0) {
      ...
      AICast_ForceScriptEvent(cs, "inspectbodyend",
                              g_entities[cs->inspectNum].aiName);
...

  {
    ...
    if (numEnemies == -1) { // query mode
      ...
      // only override current objective if we are inspecting a dead guy, and
      // the new inspect target is fighting someone
      if ((g_entities[cs->inspectNum].health <= 0) &&
          (g_entities[enemies[0]].health > 0)) {
        return AIFunc_InspectFriendlyStart(cs, enemies[0]);
      }
    }
    // RF, disabled this, if we are interrupted, scripts might not work right,
    // and anyway, this is only a bullet, not as if it's a dead guy or anything
    // else if (numEnemies == -3)	// bullet impact
    //{
    //	if (cs->aiState < AISTATE_COMBAT) {
    //		return AIFunc_InspectBulletImpactStart( cs );
  ...
  if (cs->nextIdleAngleChange < level.time) {
    ...
    if ((cs->nextIdleAngleChange + 3000) < level.time) {
      ...
      //
      //	Given a src area, pick a random dest area which is visible from
      //that area 	and return it's position, which we'd then use to set the next
      //view angles
      //
      //	This would result in more efficient, more realistic behaviour,
      //since they'd 	also use PITCH angles to look at areas above/below them
...
char *AIFunc_InspectFriendlyStart(cast_state_t *cs, int entnum) {
  ...
  AICast_ForceScriptEvent(cs, "inspectfriendlycombatstart",
                          g_entities[entnum].aiName);
  if (cs->aiFlags & AIFL_DENYACTION) {
  ...
  if (cs->aiFlags & AIFL_MISCFLAG2) {
    ...
    if (cs->aiState < AISTATE_ALERT) {
      ...
      if (cs->aifunc != AIFunc_InspectBulletImpact) {
        // G_Error( "scripting passed control out of
        // AIFunc_InspectBulletImpact(), this is bad" );
        return NULL;
      }
    }
    // pause any scripting
...
char *AIFunc_InspectBulletImpactStart(cast_state_t *cs) {
  ...
  if (oldScriptIndex == cs->scriptCallIndex) {
    ...
    BG_AnimScriptEvent(&g_entities[cs->entityNum].client->ps,
                       ANIM_ET_BULLETIMPACT, qfalse, qtrue);
  }
  //
  // if the origin is not visible, set the bullet origin to the closest visible
  // area from the src
  if (!trap_InPVS(cs->bulletImpactStart, cs->bs->origin)) {
    // if it fails, then just look at the source
    trap_AAS_GetRouteFirstVisPos(
        g_entities[cs->bulletImpactEntity].s.pos.trBase, cs->bs->origin,
        cs->travelflags, cs->bulletImpactStart);
  }
  //
  ...
  if (!(dist < cs->followDist &&
    ...
    if (!moved && (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE ||
      ...
      if (!simTest) {
        ...
        trap_Trace(&tr, cs->bs->origin,
                   NULL /*g_entities[cs->entityNum].r.mins*/,
                   NULL /*g_entities[cs->entityNum].r.maxs*/, destorg,
                   cs->entityNum, g_entities[cs->entityNum].clipmask);
        if (tr.fraction == 1) {
          simTest = qtrue;
        }
      }
    ...
    if (!moved) {
      ...
      if (moveresult && moveresult->failure) {
        ...
        // if we can get a visible target, then face it
        if (trap_AAS_GetRouteFirstVisPos(cs->audibleEventOrg, cs->bs->origin,
                                         cs->travelflags, destorg)) {
          cs->aiFlags |= AIFL_MISCFLAG2;
    ...
    */
    ...
    // call a script event
    cs->aiFlags &= ~AIFL_DENYACTION;
    AICast_ForceScriptEvent(cs, "inspectsoundend",
                            g_entities[cs->audibleEventEnt].aiName);
    if (cs->aiFlags & AIFL_DENYACTION) {
      // relinguish control back to scripting
      return AIFunc_DefaultStart(cs);
    }
    ...
    if (cs->battleHuntViewTime < level.time) {
      ...
      cs->ideal_viewangles[YAW] =
          AngleMod(cs->ideal_viewangles[YAW] +
                   (45.0 + random() * 45.0) * (2 * (rand() % 2) - 1));
    }
    //
    if (cs->scriptPauseTime < level.time) {
...
char *AIFunc_InspectAudibleEventStart(cast_state_t *cs, int entnum) {
  ...
  oldScriptIndex = cs->scriptCallIndex;
  AICast_ForceScriptEvent(cs, "inspectsoundstart",
                          g_entities[cs->audibleEventEnt].aiName);
  if (cs->aiFlags & AIFL_DENYACTION) {
    return NULL;
  }
...
char *AIFunc_ChaseGoalIdle(cast_state_t *cs) {
  ...
  if (cs->obstructingTime > level.time) {
    AICast_MoveToPos(cs, cs->obstructingPos, -1);
    cs->speedScale =
        cs->attributes[WALKING_SPEED] / cs->attributes[RUNNING_SPEED];
  }
...
============
*/
char *AIFunc_ChaseGoalIdleStart(cast_state_t *cs, int entitynum,
                                float reachdist) {
  // make sure we don't avoid any areas when we start again
  trap_BotInitAvoidReach(cs->bs->ms);
...
char *AIFunc_ChaseGoal(cast_state_t *cs) {
  ...
  // they are ready, are they inside range? FIXME: make configurable
  dist = Distance(destorg, cs->bs->origin);
  if (cs->followSlowApproach && dist < cs->followDist &&
    ...
    // if we have reached our leader
      ...
      if (cs->followEntity == cs->leaderNum) {
        if (dist < AICAST_LEADERDIST_MIN) {
          ...
          trace_t tr;
          // if we have a clear line to our leader, move closer, since there may
          // be others following also
          trap_Trace(&tr, cs->bs->origin, cs->bs->cur_ps.mins,
                     cs->bs->cur_ps.maxs,
          ...
          // if we have crouching ability, then use it while we are just moving
          // closer
          if (cs->attributes[ATTACK_CROUCH] > 0.1) {
            cs->attackcrouch_time = level.time + 1000;
          }
  ...
  if (!moved && (cs->bs->cur_ps.groundEntityNum != ENTITYNUM_NONE ||
    ...
    if (!simTest) {
      // trace will eliminate most unsuccessful paths
      trap_Trace(&tr, cs->bs->origin, NULL /*g_entities[cs->entityNum].r.mins*/,
                 NULL /*g_entities[cs->entityNum].r.maxs*/,
                 followent->r.currentOrigin, cs->entityNum,
                 g_entities[cs->entityNum].clipmask);
      if (tr.entityNum == cs->followEntity || tr.fraction == 1) {
    ...
    if (simTest) {
      ...
      if (move.stopevent == PREDICTSTOP_HITENT) { // success!
        ...
        if ((move.frames * frameTime) <
            (1.0 +
             (goaldist / (bs->cur_ps.speed * bs->cur_ps.runSpeedScale)))) {
          trap_EA_Move(cs->entityNum, dir, 400);
          vectoangles(dir, cs->ideal_viewangles);
          cs->ideal_viewangles[2] *= 0.5;
          moved = qtrue;
...
============
*/
char *AIFunc_ChaseGoalStart(cast_state_t *cs, int entitynum, float reachdist,
...
char *AIFunc_DoorMarker(cast_state_t *cs) {
  ...
  if ((!door->key) && (door->s.apos.trType == TR_STATIONARY &&
  ...
  //
  moveresult =
      AICast_MoveToPos(cs, followent->r.currentOrigin, followent->s.number);
...
char *AIFunc_BattleRoll(cast_state_t *cs) {
  ...
  if (cs->thinkFuncChangeTime > level.time - 1000) {
    ...
    //
    // if we are taking Cover, use this position, if it's bad, we'll just look
    // for a better spot once we're done here
...
char *AIFunc_BattleRollStart(cast_state_t *cs, vec3_t vec) {
  ...
  vectoangles(vec, cs->ideal_viewangles);
  // do the roll
  duration = BG_AnimScriptEvent(&g_entities[cs->entityNum].client->ps,
                                ANIM_ET_ROLL, qfalse, qtrue);
  //
...
char *AIFunc_BattleHunt(cast_state_t *cs) {
  ...
  AICast_ChooseWeapon(
      cs, qtrue); // enable special weapons, if we cant get them, change back
  if (AICast_EntityVisible(cs, cs->enemyNum,
                           qtrue) // take into account reaction time
    ...
    if (numEnemies == -1) { // query mode
      ...
      if (numEnemies > 0) {
        ...
        for (i = 0; i < numEnemies; i++) {
          if (AICast_CheckAttack(cs, enemies[i], qfalse) ||
              AICast_CheckAttack(AICast_GetCastState(enemies[i]), cs->entityNum,
                                 qfalse)) {
            cs->enemyNum = enemies[i];
            shouldAttack = qtrue;
            break;
        ...
        // note: next frame we'll process this new enemy an begin an attack if
        // necessary
      }
    }
    AICast_ChooseWeapon(cs, qfalse);
  ...
  if (cs->battleHuntPauseTime) {
    ...
    if ((dist = Distance(destorg, cs->bs->origin)) < chaseDist) {
      if (cs->battleChaseMarker ==
          (cs->vislist[cs->enemyNum].chase_marker_count - 1)) {
    ...
    if (cs->battleHuntPauseTime < level.time) {
      ...
      if (!moved && cs->leaderNum < 0) {
        ...
        if (moveresult &&
            moveresult->failure) { // no path, so go back to idle behaviour
...
char *AIFunc_BattleAmbush(cast_state_t *cs) {
  ...
  if (cs->dangerEntityValidTime >= level.time) {
    ...
    // if not found, then keep trying, hopefully a spot will free up soon so we
    // can run the hidepos function
  }
  //
  // are we waiting for a door?
  if (cs->doorMarkerTime > level.time - 100) {
  ...
  if (cs->vislist[cs->enemyNum].chase_marker_count > 0) {
    VectorCopy(
        cs->vislist[cs->enemyNum]
            .chase_marker[cs->vislist[cs->enemyNum].chase_marker_count - 1],
        cs->combatGoalOrigin);
  }
  //
  // look for things we should attack
  // if we are out of ammo, we shouldn't bother trying to attack (and we should
  // keep hiding)
  ammo = cs->bs->cur_ps.ammo;
  shouldAttack = qfalse;
  numEnemies = AICast_ScanForEnemies(cs, enemies);

  // we shouldnt be interrupted from BattleAmbush mode, so try to handle these
  ...
  if (numEnemies == -1) {        // query mode
    ...
    if (AICast_GotEnoughAmmoForWeapon(cs, cs->weaponNum)) {
      ...
      //
      for (i = 0; i < numEnemies; i++) {
        // if (we can get them from here) or (they can get us, AND we have
        // stopped going to our ambush spot)
        if ((AICast_EntityVisible(cs, enemies[i], qfalse) &&
             AICast_CheckAttack(cs, enemies[i], qfalse)) ||
            ((VectorLength(cs->takeCoverPos) < 1 || dist <= 8) &&
             (AICast_EntityVisible(AICast_GetCastState(enemies[i]),
                                   cs->entityNum, qfalse) ||
              AICast_CheckAttack(AICast_GetCastState(enemies[i]), cs->entityNum,
                                 qfalse) ||
              AICast_EntityVisible(AICast_GetCastState(enemies[i]),
          ...
          if (AICast_MoveToPos(cs, g_entities[enemies[i]].client->ps.origin,
                               enemies[i])) {
      ...
      if (!AICast_GotEnoughAmmoForWeapon(cs, cs->weaponNum)) {
        ...
        if (AICast_GetTakeCoverPos(cs, cs->enemyNum,
                                   cs->vislist[cs->enemyNum].visible_pos,
                                   cs->takeCoverPos)) {
          // go to a position that cannot be seen from the last place we saw the
          // enemy, and wait there for some time
          cs->takeCoverTime = level.time + 2000 + rand() % 3000;
          return AIFunc_BattleTakeCoverStart(cs);
        }
      }
  ...
  if (VectorLength(cs->takeCoverPos) > 1 && dist > 8 &&
      (cs->obstructingTime < level.time) /*&& !shouldAttack*/) {
    const float simTime = 0.8;
    float enemyDist;
    //
    // we haven't reached it yet, make sure we at least wait there for a few
    ...
    //
    if ((move.numtouch &&
         move.touchents[0] < aicast_maxclients) // hit something
    ...
    // if we have some enemies that we can attack immediately (without going
    // anywhere to chase them)
    if (shouldAttack) {
      return AIFunc_BattleStart(cs);
    }
    // do we need to go to our leader?
    else if (cs->leaderNum >= 0 &&
             Distance(cs->bs->origin,
                      g_entities[cs->leaderNum].r.currentOrigin) >
                 MAX_LEADER_DIST) {
  ...
  //
  if (!(cs->aiFlags & AIFL_WALKFORWARD) ||
      !VectorLength(cs->bs->cur_ps.velocity)) {
    idleYaw = qtrue;
    // if we know who we are hiding from, face them (hack, so they dont face
    // stupid directions)
    if (cs->enemyNum >= 0) {
      VectorSubtract(g_entities[cs->enemyNum].s.pos.trBase, cs->bs->origin,
                     dir);
      vectoangles(dir, cs->ideal_viewangles);
      idleYaw = qfalse;
    } else if (cs->lastEnemy >= 0) {
    ...
    if (idleYaw) { // look around randomly (but not straight into walls)
      ...
      if (cs->nextIdleAngleChange < level.time) {
        ...
        if ((cs->nextIdleAngleChange + 3000) < level.time) {
          ...
          // adjust with time
          cs->idleYawChange =
              AngleDelta(cs->idleYaw, cs->ideal_viewangles[YAW]);
          /// ((float)(cs->nextIdleAngleChange - level.time) / 1000.0);
  ...
  if (!cs->crouchHideFlag &&
      cs->enemyNum < 0) { // no enemy, and no need to crouch, so stop crouching
...
char *AIFunc_BattleAmbushStart(cast_state_t *cs) {
  ...
  // miscflag1 used to set predicted point as our goal, so we dont keep setting
  // this over and over
  cs->aiFlags &= ~AIFL_MISCFLAG1;
...
char *AIFunc_BattleChase(cast_state_t *cs) {
  ...
  if (AICast_EntityVisible(cs, cs->enemyNum,
    ...
    /*
    if (!cs->vislist[cs->enemyNum].visible_timestamp || Distance( bs->origin,
    cs->vislist[cs->enemyNum].visible_pos ) < 16)
    {
            // we're done attacking, go back to default state, which in turn
    will recall previous state
            //
            return AIFunc_DefaultStart( cs );
    }
    */
  ...
  if (followent->client) {
    ...
    if ((dist = Distance(destorg, cs->bs->origin)) < chaseDist) {
      ...
      if (AICast_GetTakeCoverPos(cs, cs->enemyNum,
                                 ...
                                 cs->takeCoverPos)) {
        VectorCopy(cs->vislist[cs->enemyNum].real_visible_pos,
                   cs->combatGoalOrigin);
        return AIFunc_BattleAmbushStart(cs);
      }
      // couldn't find a spot, so just stay here?
  ...
  //
  // if the enemy is inside a CONTENTS_DONOTENTER brush, and we are close
  // enough, stop chasing them
  if (AICast_EntityVisible(cs, cs->enemyNum, qtrue) &&
    ...
    if (trap_PointContents(destorg, cs->enemyNum) &
      ...
      // just stay here, and hope they move out of the brush without finding a
      // spot where they can hit us but we can't hit them
      return NULL;
    }
  }
  //
  ...
  backwards and look wierd if (cs->vislist[cs->enemyNum].visible_timestamp >
  level.time - 3000) { AICast_AimAtEnemy( cs );	// be ready for an attack if
  ...
  */
  ...
  if ((lastGrenadeFlush > level.time || lastGrenadeFlush < level.time - 5000) &&
      ...
      (cs->weaponFireTimes[WP_GRENADE_LAUNCHER] <
       level.time - (int)(1000 + aicast_skillscale * 1000)) &&
      ((cs->weaponNum == WP_GRENADE_LAUNCHER) ||
       !(cs->castScriptStatus.scriptFlags & SFL_NOCHANGEWEAPON)) &&
      (Distance(cs->bs->origin, cs->vislist[cs->enemyNum].visible_pos) > 100) &&
      (Distance(cs->bs->origin, cs->vislist[cs->enemyNum].visible_pos) <
       1400)) {
    // try and flush them out with a grenade
    // G_Printf("pineapple?\n");
    return AIFunc_GrenadeFlushStart(cs);
  } else if ((lastGrenadeFlush > level.time ||
              lastGrenadeFlush < level.time - 5000) &&
             (cs->aiState >= AISTATE_COMBAT) &&
             (cs->castScriptStatus.castScriptEventIndex < 0) &&
             (cs->startGrenadeFlushTime < level.time - 3000) &&
             (COM_BitCheck(cs->bs->cur_ps.weapons, WP_GRENADE_PINEAPPLE)) &&
             (AICast_GotEnoughAmmoForWeapon(cs, WP_GRENADE_PINEAPPLE)) &&
             (cs->weaponFireTimes[WP_GRENADE_PINEAPPLE] <
              level.time - (int)(1000 + aicast_skillscale * 1000)) &&
             ((cs->weaponNum == WP_GRENADE_PINEAPPLE) ||
              !(cs->castScriptStatus.scriptFlags & SFL_NOCHANGEWEAPON)) &&
             (Distance(cs->bs->origin, cs->vislist[cs->enemyNum].visible_pos) >
              100) &&
             (Distance(cs->bs->origin, cs->vislist[cs->enemyNum].visible_pos) <
  ...
  if ((cs->aiCharacter == AICHAR_ZOMBIE) && (IS_FLAMING_ZOMBIE(ent->s)) &&
  ...
  if (!moved && cs->leaderNum < 0 &&
    ...
    // trace will eliminate most unsuccessful paths
    trap_Trace(&tr, cs->bs->origin, NULL, NULL, followent->r.currentOrigin,
               cs->entityNum, g_entities[cs->entityNum].clipmask);
    if (tr.entityNum == followent->s.number) {
      ...
      if (move.stopevent == PREDICTSTOP_HITENT) { // success!
        trap_EA_Move(cs->entityNum, dir, 400);
        // RF, if we are really close, we might be stuck on a corner, so
        // randomly move sideways
        if ((VectorLength(followent->client->ps.velocity) < 50) &&
            (dist <
             10 + (sqrt(cs->bs->cur_ps.maxs[0] * cs->bs->cur_ps.maxs[0] * 8.0) /
                       2.0 +
                   sqrt(followent->client->ps.maxs[0] *
                        followent->client->ps.maxs[0] * 8.0) /
                       2.0))) {
          // if the box trace is unsuccessful
          trap_Trace(&tr, cs->bs->origin, cs->bs->cur_ps.mins,
                     cs->bs->cur_ps.maxs, followent->r.currentOrigin,
                     cs->entityNum, g_entities[cs->entityNum].clipmask);
          if (tr.entityNum != followent->s.number) {
  ...
  //
  // if they are visible, but not attackable, look for a spot where we can
  // attack them, and head for there. This should prevent AI's getting stuck in
  // a bunch.
  if (!moved && cs->weaponNum >= WP_LUGER && cs->weaponNum <= WP_AKIMBO &&
    ...
    if (cs->leaderNum >= 0) {
      if (cs->combatGoalTime < level.time) {
        if (cs->attackSpotTime < level.time) {
          ...
          if (trap_AAS_FindAttackSpotWithinRange(
                  cs->entityNum, cs->leaderNum, cs->enemyNum, MAX_LEADER_DIST,
      ...
      if (cs->combatGoalTime > level.time) {
        if (Distance(cs->combatGoalOrigin,
                     g_entities[cs->leaderNum].r.currentOrigin) >
            MAX_LEADER_DIST) {
        ...
        // we can't find a way to get to our enemy, so go back to our leader if
        // outside range do we need to go to our leader?
        if (Distance(cs->bs->origin,
                     g_entities[cs->leaderNum].r.currentOrigin) >
            MAX_LEADER_DIST) {
          return AIFunc_ChaseGoalStart(cs, cs->leaderNum, AICAST_LEADERDIST_MAX,
                                       qtrue);
      ...
      if (cs->combatGoalTime < level.time) {
        if (cs->attackSpotTime < level.time) {
          cs->attackSpotTime = level.time + 500 + rand() % 500;
          if (trap_AAS_FindAttackSpotWithinRange(
                  cs->entityNum, cs->entityNum, cs->enemyNum, 512,
                  AICAST_TFL_DEFAULT, cs->combatGoalOrigin)) {
            cs->combatGoalTime = level.time + 2000;
          }
        }
      ...
      if (cs->combatGoalTime > level.time) {
        ...
        if (moveresult &&
          ...
          if (Distance(cs->bs->origin, cs->combatGoalOrigin) < 32) {
            cs->combatGoalTime = 0;
            cs->attackSpotTime =
                level.time + 12000; // dont go to another combatspot for some
                                    // time, prevent repetitive behaviour
          }
        }
      }
    }
  ...
  if (!moved && cs->leaderNum < 0) {
    moveresult = AICast_MoveToPos(cs, destorg, cs->enemyNum);
    if (moveresult &&
        moveresult->failure) { // no path, so try and hude from them
      // pausetime has expired, so go into ambush mode
      if (AICast_GetTakeCoverPos(cs, cs->enemyNum,
                                 cs->vislist[cs->enemyNum].real_visible_pos,
                                 cs->takeCoverPos)) {
...
#define BATTLE_CHASE_ACCEL_TIME 300
  if ((cs->attributes[RUNNING_SPEED] > 170) && (cs->weaponNum != WP_GAUNTLET) &&
      (level.time < (cs->startBattleChaseTime + BATTLE_CHASE_ACCEL_TIME))) {
    float ideal;

    ideal = 0.5 + 0.5 * (1.0 - ((float)((cs->startBattleChaseTime +
  ...
  // if we are going to reach them soon, predict the attack
  {
    ...
    else if (((cs->attributes[TACTICAL] > 0.85) &&
              (cs->aiFlags & AIFL_ROLL_ANIM) && !client->ps.torsoTimer &&
              !client->ps.legsTimer && (cs->lastRollMove < level.time - 800) &&
              (move.numtouch == 0) &&
    ...
    //
             ...
             cs->lastRollMove < level.time - 800 && !client->ps.torsoTimer &&
             cs->castScriptStatus.castScriptEventIndex < 0 &&
             move.numtouch == 0 &&
      ...
      starttravel = trap_AAS_AreaTravelTimeToGoalArea(
          cs->bs->areanum, cs->bs->origin, destarea, cs->travelflags);
      simtravel = trap_AAS_AreaTravelTimeToGoalArea(simarea, move.endpos,
                                                    destarea, cs->travelflags);
      if (simtravel < starttravel) {
        return AIFunc_FlipMoveStart(cs, vec);
      }
    }
    // slow down? so we don't go too far from behind the obstruction which is
    // protecting us
    else if (!(cs->aiFlags & AIFL_WALKFORWARD) &&
             (VectorDistance(cs->bs->origin,
                             g_entities[cs->enemyNum].s.pos.trBase) <
              AICast_WeaponRange(cs, cs->weaponNum)) &&
             (cs->obstructingTime < level.time) &&
             (cs->attributes[TACTICAL] > 0.1) &&
             (AICast_VisibleFromPos(cs->vislist[cs->enemyNum].visible_pos,
                                    cs->enemyNum, move.endpos, cs->entityNum,
                                    qfalse))) {
      // start a crouch attack?
      // if (cs->attributes[ATTACK_CROUCH] > 0.1) {
      //	cs->attackcrouch_time = level.time + 3000;
      ...
      /* disabled, causes them to use up ammo in the clip before they get
      visible if ((cs->castScriptStatus.scriptNoAttackTime < level.time) &&
      (cs->noAttackTime < level.time)) {
...
char *AIFunc_AvoidDanger(cast_state_t *cs) {
  ...
  if (ent->s.onFireEnd > level.time) {
    // set the animation, and a short timer, but long enough to last until the
    // next frame
  ...
  //
  // special case: if it's a grenade, and it's going to land near us with some
  // time left before it explodes, try and kick it back
  //
  danger = &g_entities[cs->dangerEntity];
  if (ent->s.onFireEnd < level.time) {
    if ((danger->s.weapon == WP_GRENADE_LAUNCHER ||
         danger->s.weapon == WP_GRENADE_PINEAPPLE) &&
      ...
      if (!AICast_SameTeam(cs, danger->r.ownerNum)) {
        if (G_PredictMissile(danger, danger->nextthink - level.time,
                             cs->takeCoverPos, qfalse)) {
          ...
          trap_Trace(&tr, cs->takeCoverPos, ent->r.mins, ent->r.maxs, end,
                     cs->entityNum, MASK_SOLID);
          VectorCopy(tr.endpos, cs->takeCoverPos);
          if (!tr.startsolid && (tr.fraction < 1.0) &&
              VectorDistance(cs->bs->origin, cs->takeCoverPos) <
                  cs->attributes[RUNNING_SPEED] * 0.0004 *
                      (danger->nextthink - level.time - 2000)) {

            // check for a clear path to the grenade
            trap_Trace(&tr, cs->bs->origin, ent->r.mins, ent->r.maxs,
                       cs->takeCoverPos, cs->entityNum, MASK_SOLID);

            if (VectorDistance(tr.endpos, cs->takeCoverPos) < 8) {
              danger->flags |= FL_AI_GRENADE_KICK;
              ent->flags |= FL_AI_GRENADE_KICK;
              level.lastGrenadeKick = level.time;
              return AIFunc_GrenadeKickStart(
                  cs); // we should decide our course of action within this
                       // start function (dive or return grenade)
            }
          }
        }
      }
      // if it's really close to us, and we're heading for it, may as well pick
      // it up
      if (VectorLength(danger->s.pos.trDelta) < 10 &&
          VectorDistance(danger->r.currentOrigin, cs->bs->origin) < 128 &&
          (level.lastGrenadeKick < level.time - 3000) &&
          (cs->aiFlags & AIFL_CATCH_GRENADE)) {
        vec3_t vec;
        ...
        if (DotProduct(vec, cs->bs->velocity) > 0) {
          ...
          level.lastGrenadeKick = level.time;
          return AIFunc_GrenadeKickStart(
              cs); // we should decide our course of action within this start
                   // function (dive or return grenade)
        }
      }
    }
  }
  ...
  if (g_entities[cs->dangerEntity].inuse) {
    ...
    if (Distance(cs->dangerEntityPos, cs->takeCoverPos) < cs->dangerDist &&
        AICast_VisibleFromPos(cs->dangerEntityPos, cs->dangerEntity,
                              cs->takeCoverPos, cs->entityNum, qfalse)) {
      // G_Printf("current coverPos is dangerous, looking for a better
      // place..\n" );
      if (!AICast_GetTakeCoverPos(cs, cs->dangerEntity, cs->dangerEntityPos,
                                  cs->takeCoverPos)) {
  ...
  if (cs->dangerEntity >= MAX_CLIENTS &&
      Distance(cs->dangerEntityPos, cs->bs->origin) > cs->dangerDist * 1.5) {
    ...
    if (moveresult) {
      ...
      if (moveresult->failure || moveresult->blocked) {
        ...
        if (g_entities[cs->dangerEntity].inuse) {
          // find a better spot?
          AICast_GetTakeCoverPos(cs, cs->dangerEntity, cs->dangerEntityPos,
                                 cs->takeCoverPos);
        } else {
          VectorCopy(cs->bs->origin, cs->takeCoverPos);
        }
    ...
    //
    // pretend we can still see them while we run to our hide pos, this way they
    // are less likely to forget about their enemy once they get there
    if (ent->s.onFireEnd < level.time && cs->enemyNum >= 0 &&
        cs->vislist[cs->enemyNum].real_visible_timestamp &&
        (cs->vislist[cs->enemyNum].real_visible_timestamp >
         level.time - 10000)) {
      AICast_UpdateVisibility(
          &g_entities[cs->entityNum], &g_entities[cs->enemyNum], qfalse,
          cs->vislist[cs->enemyNum].real_visible_timestamp ==
              cs->vislist[cs->enemyNum].lastcheck_timestamp);
    }

  } else {
    // set our origin as the hidepos, that way if we are still in danger, we
...
char *AIFunc_AvoidDangerStart(cast_state_t *cs) {
  ...
  cs->attackcrouch_time = 0;
  //}
  // make sure we move if we are allowed (scripting will overwrite this if
  // necessary)
...
char *AIFunc_BattleTakeCover(cast_state_t *cs) {
    ...
    if (AICast_GotEnoughAmmoForWeapon(cs, cs->weaponNum)) {
      ...
      for (i = 0; i < numEnemies; i++) {
        if (AICast_CheckAttack(cs, enemies[i], qfalse) ||
            AICast_CheckAttack(AICast_GetCastState(enemies[i]), cs->entityNum,
                               qfalse) ||
            AICast_EntityVisible(AICast_GetCastState(enemies[i]), cs->entityNum,
                                 qtrue)) {
  ...
  // always do this check, if our destination sucks, abort it
  {
    ...
    if (AICast_VisibleFromPos(cs->vislist[cs->enemyNum].visible_pos,
                              cs->enemyNum, cs->takeCoverPos, bs->entitynum,
                              qfalse)) {
      if (!AICast_GetTakeCoverPos(cs, cs->enemyNum,
                                  cs->vislist[cs->enemyNum].visible_pos,
                                  cs->takeCoverPos)) {
      ...
      if (AICast_EntityVisible(AICast_GetCastState(cs->enemyNum), cs->entityNum,
                               qtrue) ||
          AICast_CheckAttack(AICast_GetCastState(cs->enemyNum), cs->entityNum,
                             qfalse)) {
        if (!AICast_GetTakeCoverPos(cs, cs->enemyNum,
                                    cs->vislist[cs->enemyNum].visible_pos,
                                    cs->takeCoverPos)) {
  ...
  // if (cs->enemyNum >= 0 && cs->vislist[cs->enemyNum].real_visible_timestamp
  // && (cs->vislist[cs->enemyNum].real_visible_timestamp > level.time - 2000)) {
  //	AICast_UpdateVisibility( &g_entities[cs->entityNum],
  //&g_entities[cs->enemyNum], qfalse,
  //cs->vislist[cs->enemyNum].real_visible_timestamp ==
  //cs->vislist[cs->enemyNum].lastcheck_timestamp );
  //}
  //
  ...
  if (VectorLength(cs->takeCoverPos) > 1 && dist > 8) {
    ...
    else if ((cs->aiFlags & AIFL_DIVE_ANIM && !client->ps.torsoTimer &&
              cs->castScriptStatus.castScriptEventIndex < 0 &&
              cs->lastRollMove < level.time - 800 && move.numtouch == 0 &&
              (moveDist > simTime * cs->attributes[RUNNING_SPEED] * 0.98) &&
              move.groundEntityNum == ENTITYNUM_WORLD) &&
             (shouldAttack &&
    ...
    if (!(cs->aiFlags & AIFL_MISCFLAG1) &&
        !AICast_VisibleFromPos(cs->vislist[cs->enemyNum].real_visible_pos,
                               ...
                               qfalse) &&
        !AICast_VisibleFromPos(cs->vislist[cs->enemyNum].real_visible_pos,
                               cs->enemyNum, cs->bs->origin, cs->entityNum,
                               qfalse) &&
        trap_AAS_PointAreaNum(
            move.endpos)) { // make sure the endpos is in a valid area
      VectorCopy(move.endpos, cs->takeCoverPos);
      dist = 0;
      cs->aiFlags |= AIFL_MISCFLAG1; // dont do this again
    }
    //
    if (cs->aiFlags & AIFL_FLIP_ANIM && cs->lastRollMove < level.time - 800 &&
        !client->ps.torsoTimer &&
        cs->castScriptStatus.castScriptEventIndex < 0 && move.numtouch == 0 &&
        moveDist > simTime * cs->attributes[RUNNING_SPEED] * 0.9 &&
        move.groundEntityNum == ENTITYNUM_WORLD &&
        cs->attackcrouch_time < level.time) {
      int destarea, simarea, starttravel, simtravel;
    ...
    // if (dist && (cs->thinkFuncChangeTime < level.time - 2000) &&
    // (cs->crouchHideFlag || cs->aiFlags & AIFL_ATTACK_CROUCH)) {
    if (cs->crouchHideFlag || ((cs->thinkFuncChangeTime < level.time - 2000) &&
                               (cs->aiFlags & AIFL_ATTACK_CROUCH))) {
      cs->attackcrouch_time = level.time + 1000;
    }
    ...
    // if we have some enemies in sight, but they can't attack us, flee if
    // possible, otherwise if we are not afraid, go attack them
    else if (numEnemies) {
      ...
      // if (!AICast_GetTakeCoverPos( cs, cs->enemyNum,
      // cs->vislist[cs->enemyNum].visible_pos, cs->takeCoverPos )) { if
      // (!AICast_WantsToTakeCover(cs, qfalse)) return AIFunc_BattleStart( cs );
      //}
    }
    // do we need to go to our leader?
    else if (cs->leaderNum >= 0 &&
  ...
  if (shouldAttack) {
    ...
    float dist;
    //
    // if they are close, and we're heading for them, we should abort this
    // manouver
    ...
    if (!move.numtouch && (cs->thinkFuncChangeTime < level.time - 2000) &&
        (dist > 128) && cs->attributes[TACTICAL] > 0.4 &&
    ...
    } else if (!cs->crouchHideFlag) { // no enemy, and no need to crouch, so
                                      // stop crouching
      // if (cs->attackcrouch_time > level.time + 1000) {
      //	cs->attackcrouch_time = level.time + 1000;
      //}
...
char *AIFunc_GrenadeFlush(cast_state_t *cs) {
  ...
  if (cs->grenadeFlushFiring) {
    ...
    if (cs->weaponFireTimes[cs->grenadeFlushFiring] < cs->thinkFuncChangeTime) {
      ...
      if (cs->weaponNum != cs->grenadeFlushFiring) {
        ...
        hitclient = AICast_SafeMissileFire(
            grenade, grenade->nextthink - level.time, cs->enemyNum, destorg,
            cs->entityNum, endPos);
        // kill the grenade
    ...
    cs->startGrenadeFlushTime =
        level.time + 2000 + rand() % 2000; // dont throw one again for a bit
    return AIFunc_DefaultStart(cs);
  }
  //
  // do we need to avoid a danger?
  ...
  if (AICast_CheckAttack(cs, cs->enemyNum, qfalse) &&
      cs->obstructingTime <
          level.time) { // give us some time to throw the grenade, otherwise go
                        // back to attack state
    ...
    // not visible, go to their previously visible position
    if (!cs->vislist[cs->enemyNum].visible_timestamp ||
        Distance(bs->origin, cs->vislist[cs->enemyNum].real_visible_pos) < 16) {
      // we're done attacking, go back to default state, which in turn will
      // recall previous state
      //
      // face the direction they currently are from this position (bit of a
      // hack, but it looks best)
      VectorSubtract(g_entities[cs->enemyNum].client->ps.origin,
                     cs->vislist[cs->enemyNum].visible_pos, dir);
      vectoangles(dir, cs->ideal_viewangles);
      //
  ...
  //
  if (cs->vislist[cs->enemyNum].lastcheck_timestamp >
          cs->vislist[cs->enemyNum].real_visible_timestamp ||
      dist > 128) {
    //
    // go to them
    ...
    if ((cs->leaderNum < 0) &&
      ...
      // trace will eliminate most unsuccessful paths
      trap_Trace(&tr, cs->bs->origin, g_entities[cs->entityNum].r.mins,
                 g_entities[cs->entityNum].r.maxs, followent->r.currentOrigin,
                 cs->entityNum, g_entities[cs->entityNum].clipmask);
      if (tr.entityNum == followent->s.number) {
        // try walking straight to them
        VectorSubtract(followent->r.currentOrigin, cs->bs->origin, dir);
  ...
  // check to see what will happen
  hitclient =
      AICast_SafeMissileFire(grenade, grenade->nextthink - level.time,
                             cs->enemyNum, destorg, cs->entityNum, endPos);
  // kill the grenade
  G_FreeEntity(grenade);
  // if (!attacked)
  ...
  AICast_AimAtEnemy(cs);
  // if we can't see them, keep facing our movement dir, but use the pitch
  // information
  if (!AICast_EntityVisible(cs, cs->enemyNum, qtrue)) {
  ...
  if (hitclient == 1) {
    ...
    // hit a friendly
    cs->startGrenadeFlushTime =
        level.time + 3000; // don't try again for a while
    ...
    if (cs->thinkFuncChangeTime < level.time - 200) {
      ...
      if (Distance(endPos, cs->bs->origin) >
          100 + Distance(endPos, g_entities[cs->enemyNum].r.currentOrigin)) {
        trap_EA_Attack(bs->client);
        cs->bFlags |= BFL_ATTACKED;
        cs->weaponNum = grenadeType; // select grenade launcher
        cs->grenadeFlushFiring = cs->weaponNum;
...
char *AIFunc_BattleMG42(cast_state_t *cs) {
  ...
  */
  ...
  if (cs->enemyNum >= 0) {
    if (cs->vislist[cs->enemyNum].real_visible_timestamp &&
        cs->vislist[cs->enemyNum].real_visible_timestamp >
            (level.time - 5000)) {
      VectorSubtract(cs->vislist[cs->enemyNum].real_visible_pos,
                     mg42->r.currentOrigin, vec);
    } else if (cs->vislist[cs->enemyNum].visible_timestamp &&
               cs->vislist[cs->enemyNum].visible_timestamp >
                   (level.time - 5000)) {
      VectorSubtract(cs->vislist[cs->enemyNum].visible_pos,
                     mg42->r.currentOrigin, vec);
  ...
  if (cs->enemyNum < 0 || !AICast_CheckAttack(cs, cs->enemyNum, qfalse) ||
    ...
    if (numEnemies > 0) {
      ...
      for (i = 0; i < numEnemies; i++) {
        ...
        if (!cs->vislist[enemies[i]].real_visible_timestamp ||
                (cs->vislist[enemies[i]].real_visible_timestamp < level.time -
        2000)) {
        ...
        */
        ...
        if (cs->vislist[enemies[i]].real_visible_timestamp >
            (level.time - 5000)) {
          VectorSubtract(cs->vislist[enemies[i]].real_visible_pos,
                         mg42->r.currentOrigin, vec);
        } else {
          VectorSubtract(cs->vislist[enemies[i]].visible_pos,
                         mg42->r.currentOrigin, vec);
        }
        VectorNormalize(vec);
        vectoangles(vec, angles);
        angles[PITCH] = AngleNormalize180(angles[PITCH]);
        if (!((fabs(AngleDifference(angles[YAW], mg42->s.angles[YAW])) >
               mg42->harc) ||
          ...
          if (AICast_CheckAttack(cs, enemies[i], qfalse)) {
            ...
            // keep firing at anything behind solids, in case they find a
            // position where they can shoot us, but our checkattack() doesn't
            // find a clear shot
            VectorCopy(angles, bestangles);
            cs->enemyNum = enemies[i];
            shouldAttack = qtrue;
    ...
    if (!shouldAttack) {
      // keep firing at anything behind solids, in case they find a position
      // where they can shoot us, but our checkattack() doesn't find a clear
      // shot
    ...
    // if we had possible enemies, but couldnt find one to attack, then dismount
    // now
    if (unmount) {
...
char *AIFunc_InspectBody(cast_state_t *cs) {
  ...
  if ((cs->inspectBodyTime < 0) && (Distance(cs->bs->origin, enemyOrg) > 64)) {
    ...
    if (g_entities[cs->enemyNum].health < GIB_HEALTH &&
      ...
      G_AddEvent(
          &g_entities[cs->entityNum], EV_GENERAL_SOUND,
          G_SoundIndex(
              aiDefaults[cs->aiCharacter].soundScripts[ORDERSSOUNDSCRIPT]));
    }
    // walk to them
    if (cs->movestate != MS_CROUCH) {
      cs->movestate = MS_WALK;
...
char *AIFunc_GrenadeKick(cast_state_t *cs) {
  ...
  // !!! NOTE: the only way control should pass out of here, is by calling
  // AIFunc_DefaultStart()
  ...
  if (cs->grenadeFlushFiring) {
    ...
    if (g_entities[ent->grenadeFired].nextthink < level.time + 200) {	// cut
    them some slack g_entities[ent->grenadeFired].nextthink = level.time + 200 +
    rand()%500;
    }
    ...
    cs->grenadeFlushFiring = qfalse;
    cs->lockViewAnglesTime = -1;
    cs->startGrenadeFlushTime = level.time + 2000 + rand()%2000;	// dont
    throw one again for a bit level.lastGrenadeKick = level.time; return
  ...
  */
  ...
  if (level.time > danger->nextthink -
                       (int)(2.0 * VectorDistance(cs->bs->origin,
  ...
  if (danger->s.pos.trDelta[2] < 40 && VectorDistance( danger->r.currentOrigin,
  cs->bs->origin ) < 48 && (danger->r.currentOrigin[2] < cs->bs->origin[2]) &&
  ...
  */
  ...
  if (/*cs->grenadeGrabFlag <= 0 || */
      (danger->s.pos.trDelta[2] < 20 &&
       VectorDistance(danger->r.currentOrigin, cs->bs->origin) < 48 &&
       (danger->r.currentOrigin[2] < cs->bs->origin[2]) &&
       VectorLength(danger->s.pos.trDelta) < 50)) {
    //
    // we have a choice here, either pick up and return, or just kick it
    //		if ((cs->grenadeGrabFlag == -1) || (cs->grenadeGrabFlag == qtrue &&
    //level.time > danger->nextthink - 2000)) {	// kick
    ...
    if (cs->grenadeGrabFlag == qtrue) {
      ...
      speed = 400;
      if (cs->enemyNum >= 0) {
        speed = 1.5 * VectorDistance(danger->r.currentOrigin,
      ...
      danger->r.ownerNum =
          ent->s.number; // we are now the owner, let it pass through us
      danger->s.pos.trTime =
    ...
    /*
       ...
       ANIM_ET_PICKUPGRENADE, qfalse, qtrue ); cs->grenadeGrabFlag = qfalse;
                                    // stop the grenade from moving away
                                    danger->s.pos.trDelta[0] = 0;
                                    danger->s.pos.trDelta[1] = 0;
                                    if (danger->s.pos.trDelta[2] > 0) {
                                    ...
                                    // send the grenade on its way
                                    cs->grenadeFlushFiring = qtrue;
                                    AngleVectors( cs->viewangles, dir, NULL,
       NULL ); dir[2] = 0.4; VectorNormalize( dir ); speed = 500; if
       (cs->enemyNum >= 0) { speed = 2*VectorDistance(danger->r.currentOrigin,
       g_entities[cs->enemyNum].r.currentOrigin); if (speed > 650) speed = 650;
                                    }
                                    VectorScale( dir, speed,
       danger->s.pos.trDelta ); trap_LinkEntity( danger ); } else if
       (ent->client->ps.legsTimer < 800) {
  ...
  if (G_PredictMissile(danger, danger->nextthink - level.time, cs->takeCoverPos,
                       qfalse)) {
    // make sure it's a valid position, and drop it down to the ground
  ...
  /*	//
          ...
          if (VectorDistance( danger->r.currentOrigin, cs->bs->origin ) < 128) {
                  ...
                  cs->weaponNum = weapon;	// select grenade launcher
                  shouldAttack = qfalse;	// don't attack until we've caught
     it
          }
  */
  // if we should be attacking something on our way
  ...
  if (danger->s.pos.trDelta[2] < 30) {
          ...
          for (i=0, trav=g_entities; i<aicast_maxclients; i++, trav++) {
                  ...
                  if (!AICast_SameTeam( cs, i ))
                          continue;
                  if (VectorDistance( cs->takeCoverPos, trav->r.currentOrigin )
  > 200) continue; numFriends++;
          }
          // if there are enough friends around, and we have a clear path to the
  position, sacrifice ourself! if (numFriends > 2) { trace_t tr; trap_Trace(
  &tr, cs->bs->origin, ent->r.mins, ent->r.maxs, cs->takeCoverPos,
  cs->entityNum, MASK_SOLID ); if (tr.fraction == 1.0 && !tr.startsolid) {
...
char *AIFunc_Battle(cast_state_t *cs) {
  ...
  if ((cs->bs->cur_ps.weaponTime < 100) // if reloading, don't chase until ready
    ...
    if (!cs->bs->areanum) {
      ...
      } else if (AICast_GetAvoid(cs, NULL, cs->takeCoverPos, qtrue,
                                 cs->enemyNum)) {
      ...
      return AIFunc_BattleChaseStart(cs);
        ...
        // if we weren't moving, it is likely they have dodged back behind
        // something, ready to duck out and take another shot. so, we could fool
        // them by hiding from the position we last saw them from, in the hope
        // that when they return to fire at us, we won't be in their sight.
        if (cs->attributes[TACTICAL] > 0.3 &&
            cs->attributes[AGGRESSION] < 1.0 &&
            cs->attributes[AGGRESSION] <
                (random() + 0.5 * cs->attributes[TACTICAL]) &&
      ...
      // start taking cover
      cs->takeCoverTime =
          level.time + 2000 + rand() % 4000; // only move a little bit
      // cs->attackcrouch_time = 0;		// get out of here real quick
      return AIFunc_BattleTakeCoverStart(cs);
    } else
        // if we haven't thrown a grenade in a bit, go into "grenade flush mode"
        if ((lastGrenadeFlush > level.time ||
             lastGrenadeFlush < level.time - 7000) &&
            (cs->aiState >= AISTATE_COMBAT) &&
            (cs->castScriptStatus.castScriptEventIndex < 0) &&
            (((COM_BitCheck(cs->bs->cur_ps.weapons, WP_GRENADE_LAUNCHER)) &&
              (AICast_GotEnoughAmmoForWeapon(cs, WP_GRENADE_LAUNCHER)) &&
              (cs->weaponFireTimes[WP_GRENADE_LAUNCHER] <
               level.time - (int)(aicast_skillscale * 3000))) ||
             ((COM_BitCheck(cs->bs->cur_ps.weapons, WP_GRENADE_PINEAPPLE)) &&
              (AICast_GotEnoughAmmoForWeapon(cs, WP_GRENADE_PINEAPPLE)) &&
              (cs->weaponFireTimes[WP_GRENADE_PINEAPPLE] <
               level.time - (int)(aicast_skillscale * 3000)))) &&
            !(cs->weaponNum &&
              (cs->castScriptStatus.scriptFlags & SFL_NOCHANGEWEAPON)) &&
            (Distance(cs->bs->origin,
                      cs->vislist[cs->enemyNum].real_visible_pos) > 100) &&
            (Distance(cs->bs->origin,
                      cs->vislist[cs->enemyNum].real_visible_pos) < 1200) &&
  ...
  VectorLength(cs->bs->cur_ps.velocity)) {	// crouch if moving? if
  (cs->attributes[ATTACK_CROUCH] > 0.1) { AICast_RequestCrouchAttack( cs,
  cs->bs->origin, 0.5 );
          }
  }
  */
  ...
  if ((lastGrenadeFlush > level.time || lastGrenadeFlush < level.time - 7000) &&
      ...
      ((cs->weaponNum == WP_GRENADE_LAUNCHER) ||
       !(cs->castScriptStatus.scriptFlags & SFL_NOCHANGEWEAPON)) &&
      (Distance(cs->bs->origin, cs->vislist[cs->enemyNum].real_visible_pos) >
       100) &&
      (Distance(cs->bs->origin, cs->vislist[cs->enemyNum].real_visible_pos) <
       2000)) {
    // try and flush them out with a grenade
  ...
  //
  // Dodge enemy aim?
  if ((cs->attributes[AGGRESSION] < 1.0) && (ent->client->ps.weapon) &&
    ...
    if (DotProduct(aim, enemyVec) > 0.97) {
      ...
      if (move.groundEntityNum == ENTITYNUM_WORLD &&
        ...
        if (AICast_CheckAttackAtPos(cs->entityNum, cs->enemyNum, move.endpos,
                                    cs->bs->cur_ps.viewheight ==
                                        cs->bs->cur_ps.crouchViewHeight,
                                    qfalse)) {
          cs->takeCoverTime = 0;
...
char *AIFunc_BattleStart(cast_state_t *cs) {
  ...
  cs->takeCoverTime =
      level.time + 300 +
      rand() % (2000 + (int)(2000.0 * cs->attributes[AGGRESSION]));
...
recheck:
  ...
  if (fabs(AngleDifference(cs->ideal_viewangles[YAW], cs->viewangles[YAW])) <
      10) {
    // select a weapon
    ...
    //
    if (!rval && cs->weaponNum >= WP_MONSTER_ATTACK1 &&
        cs->weaponNum <= WP_MONSTER_ATTACK3) {
      ...
      AICast_ChooseWeapon(cs,
                          qfalse); // qfalse so we don't choose a special weapon
  ...
  if (!rval) { // use the generic battle routine for all "normal" weapons
    if (cs->weaponNum >= WP_MONSTER_ATTACK1 &&
        cs->weaponNum <= WP_MONSTER_ATTACK3) {
...
char *AIFunc_DefaultStart(cast_state_t *cs) {
  ...
  if (cs->enemyNum >= 0 &&
      (cs->aifunc !=
       AIFunc_Battle)) { // make sure we haven't just come from there
