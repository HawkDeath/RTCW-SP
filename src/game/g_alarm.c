...
void alarmExplosion(gentity_t *ent) {
  ...
  G_RadiusDamage(ent->s.origin, ent, ent->damage, ent->damage, ent,
                 MOD_EXPLOSIVE);
  // return
  ...
  while ((t = G_Find(t, FOFS(targetname), ent->target)) != NULL) {
    if (t == ent) {
      ...
      // (but don't trigger if dying, only activation)
        ...
        if (ent->active &&
            matestoo) { // not dead (and this is the box that was used)
          t->use(t, ent, 0);
        }
      }
    }
...
==============
*/
void alarmbox_die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker,
                  int damage, int mod) {
  gentity_t *t;
