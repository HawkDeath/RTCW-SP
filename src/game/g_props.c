...
void props_bench_die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker,
                     int damage, int mod) {
...
void props_radio_die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker,
                     int damage, int mod) {
...
void props_radio_dieSEVEN(gentity_t *ent, gentity_t *inflictor,
...
void props_locker_tall_die(gentity_t *ent, gentity_t *inflictor,
                           gentity_t *attacker, int damage, int mod) {
...
model - specify a different model for the chair.  must have same frames as
default for this ent
...
void Props_Chair_Die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker,
                     int damage, int mod);
  ...
  if (self->s.groundEntityNum == -1) {
...

    {
      ...
      if (trace.startsolid) {
        ...
        if (player->s.weapon &&
            !(WEAPS_ONE_HANDED & (1 << (player->s.weapon)))) {
          reGrab = qfalse;
        }
...
void Use_DamageInflictor(gentity_t *ent, gentity_t *other,
                         gentity_t *activator) {
  gentity_t *daent;
...
void Use_Props_Shard_Generator(gentity_t *ent, gentity_t *other,
                               gentity_t *activator) {
  int quantity;
  int type;
...
void Props_Barrel_Pain(gentity_t *ent, gentity_t *attacker, int damage,
                       vec3_t point) {
...
void Props_Barrel_Die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker,
                      int damage, int mod) {
  int quantity;
  int type;
  vec3_t dir;
  ...
  if (player) {
    ...
    if (len < 64 && player->s.groundEntityNum != -1) {
      ...
      if (len && !(player->client->ps.pm_time)) {
        ...
        VectorAdd(player->client->ps.velocity, kvel,
...
void crate_die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker,
               int damage, int mod) {
  int quantity;
  int type;
...
void props_crate32x64_die(gentity_t *ent, gentity_t *inflictor,
                          gentity_t *attacker, int damage, int mod) {
...
void props_flippy_table_die(gentity_t *ent, gentity_t *inflictor,
                            gentity_t *attacker, int damage, int mod) {
...
void props_58x112tablew_die(gentity_t *ent, gentity_t *inflictor,
                            gentity_t *attacker, int damage, int mod) {
...
void props_castlebed_die(gentity_t *ent, gentity_t *inflictor,
                         gentity_t *attacker, int damage, int mod) {
...
void props_snowGenerator_use(gentity_t *ent, gentity_t *other,
                             gentity_t *activator) {
  if (!(ent->spawnflags & 1)) {
    ent->spawnflags |= 1;
...
void propsFireColumnUse(gentity_t *ent, gentity_t *other,
                        gentity_t *activator) {
  gentity_t *tent;
  vec3_t start, dir;
...
void props_ExploPartUse(gentity_t *ent, gentity_t *other,
                        gentity_t *activator) {
  //	int i, numpieces;
  gentity_t *part;
  vec3_t start, dir;
...
"type" type of debris ("glass", "wood", "metal", "ceramic", "rubble") default is
"wood" "count" how much debris ei. default 4 pieces
...
void props_decoration_death(gentity_t *ent, gentity_t *inflictor,
                            gentity_t *attacker, int damage, int mod) {
  if (!(ent->spawnflags & 8)) {
    ent->clipmask = 0;
    ent->r.contents = 0;
...
void Use_props_decoration(gentity_t *ent, gentity_t *self,
                          gentity_t *activator) {
  if (ent->spawnflags & 1) {
    trap_LinkEntity(ent);
    ent->spawnflags &= ~1;
...
void SP_skyportal(gentity_t *ent) {
  ...
  trap_SetConfigstring(CS_SKYBOXORG,
                       va("%.2f %.2f %.2f %.1f %i %.2f %.2f %.2f %i %i",
                          ent->s.origin[0], ent->s.origin[1], ent->s.origin[2],
                          fov_x, (int)isfog, fogv[0], fogv[1], fogv[2], fogn,
                          fogf));
  //----(SA)	end
}
...
void props_statue_death(gentity_t *ent, gentity_t *inflictor,
                        gentity_t *attacker, int damage, int mod) {
...
void props_locker_death(gentity_t *ent, gentity_t *inflictor,
                        gentity_t *attacker, int damage, int mod);
//////////////////////////////////////////////////
...
void props_locker_pain(gentity_t *ent, gentity_t *attacker, int damage,
                       vec3_t point) {
...
extern qhandle_t trap_R_GetShaderFromModel(qhandle_t modelid, int surfnum,
                                           int withlightmap);
...
void props_locker_death(gentity_t *ent, gentity_t *inflictor,
  ...
  trap_UnlinkEntity(ent);
  ent->r.maxs[2] = 11; // (SA) make the dead bb half height so the item can look
                       // like it's sitting inside
...
void props_flamethrower_use(gentity_t *ent, gentity_t *other,
                            gentity_t *activator) {
  int rval;
  int rnd;
...
void props_flamethrower_init(gentity_t *ent) {
  ...
  if (!target) {
    ...
    VectorCopy(angles, ent->s.apos.trBase);
    VectorCopy(angles,
               ent->s.angles); // RF, added to fix wierd release build issues
  }
