 ...
 * name:		g_items.c
 *
 * desc:		Items are any object that a player can touch to gain some
 *effect. Pickup will return the number of seconds until they should respawn.
 *				all items should pop when dropped in lava or
 *slime. Respawnable items don't actually go away when picked up, they are just
 *made invisible and untouchable.  This allows them to ride movers and respawn
 *apropriately.
 *
 */
...
void UseHoldableItem(gentity_t *ent, int item) {
  ...
  case HI_STAMINA: // restores fatigue bar and sets "nofatigue" for a time
    ...
    //----(SA)	NOTE:	currently only gives free nofatigue time, doesn't reset
    //fatigue bar. 					(this is because I'd like the restore to be visually gradual
    //(on the HUD item representing 					current status of your fatigue) rather than
...
void Fill_Clip(playerState_t *ps, int weapon) {
  ...
  if (ammomove) {
    if (!ps->aiChar ||
        ps->ammo[ammoweap] <
            999) { // RF, dont take ammo away if they need unlimited supplies
...
void Add_Ammo(gentity_t *ent, int weapon, int count, qboolean fillClip) {
  ...
  switch (ammoweap) {
    // some weaps load straight into the 'clip' since they have no storage
    // outside the clip

  case WP_GRENADE_LAUNCHER: // make sure if he picks up a grenade that he get's
                            // the "launcher" too
...
int Pickup_Weapon(gentity_t *ent, gentity_t *other) {
  ...
  if (ent->count < 0) {
    ...
    if (ent->count) {
      ...
      //----(SA) modified
      //			quantity = ent->item->quantity;
      //			quantity = (random() * (ent->item->quantity -
      //1)) + 1;	// giving 1-<item default count>
      quantity = (random() * (ammoTable[weapon].maxclip - 4)) +
                 4; // giving 4-<item default count>
    }
    ...
    //) {
    //			// respawning rules
    //			// drop the quantity if the already have over the
    //minimum 			if ( other->client->ps.ammo[ BG_FindAmmoForWeapon(weapon)] <
    //quantity ) { 				quantity = quantity - other->client->ps.ammo[
    //BG_FindAmmoForWeapon(weapon)]; 			} else {
    //				quantity = 1;		// only add a single
    //shot
    //			}
    //		}
    //----(SA) end
  ...
  //----(SA)	added
  // check for special colt->akimbo add (if you've got a colt already, add the
  // second now)
  if (weapon == WP_COLT) {
  ...
  //		other->client->ps.ammo[BG_FindAmmoForWeapon(weapon)] = -1; //
  //unlimited ammo
...
void RespawnItem(gentity_t *ent) {
  ...
  /*
          if ( ent->item->giType == IT_POWERUP && g_gametype.integer !=
     GT_SINGLE_PLAYER) {
     ...
     "sound/items/poweruprespawn.wav" ); te->r.svFlags |= SVF_BROADCAST;
          }
  */
...
void Touch_Item(gentity_t *ent, gentity_t *other, trace_t *trace) {
  ...
  if (respawn == RESPAWN_PARTIAL) { // multi-stage health
    ent->s.density--;
    if (ent->s.density) {  // still not completely used up ( (SA) this will
                           // change to == 0 and stage 1 will be a destroyable
                           // item (plate/etc.) )
      ent->active = qtrue; // re-activate
      trap_LinkEntity(ent);
      return;
...
gentity_t *LaunchItem(gitem_t *item, vec3_t origin, vec3_t velocity) {
  ...
  ////----(SA)	commented out since I'm using modelindex2 for model indexing now
  dropped->s.otherEntityNum2 = 1; // DHM - Nerve :: this is taking modelindex2's
                                  // place for signaling a dropped item
  ...
  //		dropped->s.eFlags |= EF_SPINNING;	// spin the weapon as it flies
  //from the dead player.  it will stop when it hits the ground
...
void FinishSpawningItem(gentity_t *ent) {
  ...
  ent->s.otherEntityNum2 =
      0; // DHM - Nerve :: takes modelindex2's place in signaling a dropped item
  ...
  if (ent->item->giType == IT_CLIPBOARD) {
    if (!ent->message) {
      ent->s.density = G_FindConfigstringIndex(
          "clip_test", CS_CLIPBOARDS, MAX_CLIPBOARD_CONFIGSTRINGS, qtrue);
    } else {
      ent->s.density = G_FindConfigstringIndex(
          ent->message, CS_CLIPBOARDS, MAX_CLIPBOARD_CONFIGSTRINGS, qtrue);
    }
    ...
    if (ent->item->giTag >= HI_BOOK1 && ent->item->giTag <= HI_BOOK3) {
      G_FindConfigstringIndex(va("hbook%d", ent->item->giTag - HI_BOOK1),
                              CS_CLIPBOARDS, MAX_CLIPBOARD_CONFIGSTRINGS,
                              qtrue);
    }
    //		ent->touch = Touch_Item;	// no auto-pickup, only activate
  }
  ...
  if (ent->item->giType == IT_HEALTH || ent->item->giType == IT_AMMO ||
    ...
    // has no effect was: for(i=0;i<4,ent->item->world_model[i];i++) {}
    i = 0;
...
void ClearRegisteredItems(void) {
  ...
  //----(SA)	this will be determined by the level or starting position, or
  //the savegame 			but for now, re-register the MP40 automatically 	RegisterItem(
  //BG_FindItemForWeapon( WP_MP40 ) );
  RegisterItem(BG_FindItem(
      "Med Health")); // NERVE - SMF - this is so med packs properly display
}
