...
static void G_LoadArenas( void ) {
...
); if( *arenasFile.string ) { filename = arenasFile.string;
        }
        else {
        ...
        if ( len >= MAX_ARENAS_TEXT ) {
                trap_Printf( va( S_COLOR_RED "file too large: %s is %i, max
...
static void G_SpawnBots( char *botList, int baseDelay ) {
        ...
        while( *p ) {
                ...
                // we must add the bot this way, calling G_AddBot directly at
this stage
                // does "Bad Things"
                trap_SendConsoleCommand( EXEC_INSERT, va("addbot %s %i free
%i\n", bot, skill, delay) );
...
static void G_LoadBots( void ) {
        ...
        if ( len >= MAX_BOTS_TEXT ) {
                trap_Printf( va( S_COLOR_RED "file too large: %s is %i, max
allowed is %i", filename, len, MAX_BOTS_TEXT ) ); trap_FS_FCloseFile( f );
                return;
        }
...
CVAR_SERVERINFO );
        ...
        if( g_gametype.integer == GT_SINGLE_PLAYER ) {
                ...
                Q_strncpyz( map, Info_ValueForKey( serverinfo, "mapname" ),
sizeof(map) ); arenainfo = G_GetArenaInfoByMap( map ); if ( !arenainfo ) {
                        return;
                }
                ...
                if( !restart ) {
                        G_SpawnBots( Info_ValueForKey( arenainfo, "bots" ),
basedelay );
                }
        }
}
