...
*/
void SP_info_notnull_big(gentity_t *self) { G_SetOrigin(self, self->s.origin); }
...
*/
void SP_lightJunior(gentity_t *self) { G_FreeEntity(self); }
...
void grabber_think_hit(gentity_t *ent) {
  G_RadiusDamage(ent->s.pos.trBase, ent, ent->damage, ent->duration, ent,
                 MOD_GRABBER);
...
void grabber_die(gentity_t *ent, gentity_t *inflictor, gentity_t *attacker,
                 int damage, int mod) {
...
==============
*/
void grabber_pain(gentity_t *ent, gentity_t *attacker, int damage,
                  vec3_t point) {
  G_AddEvent(ent, EV_GENERAL_SOUND,
             ent->sound2to1); // sound2to1 is the 'pain' sound
}
...
void spotlight_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
                   int damage, int mod) {

  AICast_AudibleEvent(attacker->s.number, self->r.currentOrigin,
                      1024); // loud audible event
...
"target" - .camera (spline) file for light to track.  do not specify file
extension.
...
//"models/mapobjects/light/searchlight_pivot.md3"
...
"modelscale_vec"	scale multiplier (defaults to 1 1 1, scales each axis as
requested) "trunk"			diameter of solid core (used for trace
visibility and collision (not ai pathing)) "trunkheight"	height of trunk
...
*/
void SP_misc_light_surface(gentity_t *ent) { G_FreeEntity(ent); }
...
void Use_Shooter(gentity_t *ent, gentity_t *other, gentity_t *activator) {
  ...
  case WP_GRENADE_LAUNCHER:
    VectorScale(dir, 700, dir); //----(SA)	had to add this as fire_grenade
...
*/
void SP_shooter_rocket(gentity_t *ent) { InitShooter(ent, WP_PANZERFAUST); }
...
START_ON means it starts out active, the default is to start off and fire when
triggered DLIGHT will have a built-in dlight flashing too (use color picker to
set color of dlight. (def: blue) )
...
void SP_shooter_tesla(gentity_t *ent) {
  ...
  //	game, so this will create an linked-entity on the client that acts as
  //dictated 	and, if necessary, passes back damage info like the weapon.  this
  //will 	keep the server->client messages to a minimum (start firing/stop
  //firing) 	rather than sending an event for each bolt.
  ...
  // finish up after everything has spawned in so we know all potential targets
  // are ready
...
Fires at either the target or the current direction.
"random" is the number of degrees of deviance from the taget. (1.0 default)
"damage" the amount of damage sniper will cause when he hits his target default
is 10 "radius" is the dist the target would need to travel before sniper lost
his beat default 256 "delay"	 is the rate of fire defaults to 1 sec
*/
...
this should be a volume that will encompase the area where the sniper target
assigned to the brush would fire at the player
*/
...
user has r_dynamiclight set to 0. STARTOFF	- means the dlight doesn't spawn
in until ent is triggered
ONETIME		- when the dlight is triggered, it will play through it's cycle
once, then shut down until triggered again "shader" name of shader to apply
...
void SP_dlight(gentity_t *ent) {
  ...
  if (ent->dl_stylestring &&
      strlen(ent->dl_stylestring)) { // if they're specified in a string, use em
  } else if (style) {
    ...
    ent->dl_stylestring =
        "mmmaaa"; // default to a strobe to call attention to this not being set
  }
...
void snow_think(gentity_t *ent) {
  ...
  if (!Q_stricmp(ent->classname, "misc_snow256")) {
    G_FindConfigstringIndex(
        va("%i %.2f %.2f %.2f %.2f %.2f %.2f %i %i %i", PARTICLE_SNOW256,
           ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], tr.endpos[0],
           tr.endpos[1], tr.endpos[2], ent->health, turb, ent->s.number),
        CS_PARTICLES, MAX_PARTICLES_AREAS, qtrue);
  } else if (!Q_stricmp(ent->classname, "misc_snow128")) {
    G_FindConfigstringIndex(
        va("%i %.2f %.2f %.2f %.2f %.2f %.2f %i %i %i", PARTICLE_SNOW128,
           ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], tr.endpos[0],
           tr.endpos[1], tr.endpos[2], ent->health, turb, ent->s.number),
        CS_PARTICLES, MAX_PARTICLES_AREAS, qtrue);
  } else if (!Q_stricmp(ent->classname, "misc_snow64")) {
    G_FindConfigstringIndex(
        va("%i %.2f %.2f %.2f %.2f %.2f %.2f %i %i %i", PARTICLE_SNOW64,
           ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], tr.endpos[0],
           tr.endpos[1], tr.endpos[2], ent->health, turb, ent->s.number),
        CS_PARTICLES, MAX_PARTICLES_AREAS, qtrue);
  } else if (!Q_stricmp(ent->classname, "misc_snow32")) {
    G_FindConfigstringIndex(
        va("%i %.2f %.2f %.2f %.2f %.2f %.2f %i %i %i", PARTICLE_SNOW32,
           ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], tr.endpos[0],
           tr.endpos[1], tr.endpos[2], ent->health, turb, ent->s.number),
        CS_PARTICLES, MAX_PARTICLES_AREAS, qtrue);
  } else if (!Q_stricmp(ent->classname, "misc_bubbles8")) {
    G_FindConfigstringIndex(
        va("%i %.2f %.2f %.2f %.2f %.2f %.2f %i %i %i", PARTICLE_BUBBLE8,
           ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], tr.endpos[0],
           tr.endpos[1], tr.endpos[2], ent->health, turb, ent->s.number),
        CS_PARTICLES, MAX_PARTICLES_AREAS, qtrue);
  } else if (!Q_stricmp(ent->classname, "misc_bubbles16")) {
    G_FindConfigstringIndex(
        va("%i %.2f %.2f %.2f %.2f %.2f %.2f %i %i %i", PARTICLE_BUBBLE16,
           ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], tr.endpos[0],
           tr.endpos[1], tr.endpos[2], ent->health, turb, ent->s.number),
        CS_PARTICLES, MAX_PARTICLES_AREAS, qtrue);
  } else if (!Q_stricmp(ent->classname, "misc_bubbles32")) {
    G_FindConfigstringIndex(
        va("%i %.2f %.2f %.2f %.2f %.2f %.2f %i %i %i", PARTICLE_BUBBLE32,
           ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], tr.endpos[0],
           tr.endpos[1], tr.endpos[2], ent->health, turb, ent->s.number),
        CS_PARTICLES, MAX_PARTICLES_AREAS, qtrue);
  } else if (!Q_stricmp(ent->classname, "misc_bubbles64")) {
    G_FindConfigstringIndex(
        va("%i %.2f %.2f %.2f %.2f %.2f %.2f %i %i %i", PARTICLE_BUBBLE64,
           ent->s.origin[0], ent->s.origin[1], ent->s.origin[2], tr.endpos[0],
           tr.endpos[1], tr.endpos[2], ent->health, turb, ent->s.number),
        CS_PARTICLES, MAX_PARTICLES_AREAS, qtrue);
  }
...
void mg42_muzzleflash(
    gentity_t *ent,
    vec3_t muzzlepos) { // cheezy, but lets me use this routine for finding the
                        // muzzle point for firing the actual bullet
...
void Fire_Lead(gentity_t *ent, gentity_t *activator, float spread, int damage) {
  ...
  if (!Q_stricmp(ent->classname, "misc_mg42")) {
    mg42_muzzleflash(ent, mg42_muzzle); // get current position for mg42 muzzle
                                        // flash/bullet origin
...
#define MG42_IDLEYAWSPEED 80.0 // degrees per second (while returning to base)
  ...
  if (!Q_stricmp(self->classname, "misc_mg42") || !(self->active)) {
    ...
    for (i = 0; i < 3; i++) {
      ...
      if (fabs(diff) > (yawspeed * ((float)FRAMETIME / 1000.0))) {
        clamped = qtrue;
        if (diff > 0) {
          dang[i] = AngleMod(self->r.currentAngles[i] +
                             (yawspeed * ((float)FRAMETIME / 1000.0)));
        } else {
          dang[i] = AngleMod(self->r.currentAngles[i] -
                             (yawspeed * ((float)FRAMETIME / 1000.0)));
        }
  ...
  // whilst trying to get to the other side diff = AngleDifference( dang[YAW],
  // self->s.angles[YAW] );
  ...
  if (fabs(diff) > self->harc && other && other->r.svFlags & SVF_CASTAI) {
    ...
    //self->s.angles[YAW], diff);
  }
  //	else
  //		G_Printf ("not clamped cang %5.2f\n", self->TargetAngles[YAW]);

  if (other && clamped) {
    // we only do this to keep the input angles close to the weapon, this
    // doesn't actually effect the view
      ...
      if (self->s.frame && !is_flak) {
        ...
        if (validshot) {
          if (!(other->r.svFlags & SVF_CASTAI)) {
            ...
            if (self->damage) {
              Fire_Lead(self, other, MG42_SPREAD / self->accuracy,
                        self->damage);
            } else {
              Fire_Lead(self, other, MG42_SPREAD / self->accuracy,
                        MG42_DAMAGE_AI);
          ...
          // play character anim
          BG_AnimScriptEvent(&other->client->ps, ANIM_ET_FIREWEAPON, qfalse,
                             qtrue);

          other->client->ps.viewlocked =
              2; // this enable screen jitter when firing
        }
      }
    } else {
      other->client->ps.viewlocked = 1;
...
void mg42_think(gentity_t *self) {
  ...
  if (owner->client) {
    ...
    if (owner->r.svFlags & SVF_CASTAI) {
      ...
      // kinda dumb since the player had to be close enough to activate it to
      // get this far and the start point for the difference is calculated
      // differently each place anyway
      //			usedist = 96;
    ...
    if (len < usedist && (owner->active == 1) && owner->health > 0) {
      ...
      /*
                              if (owner->r.svFlags & SVF_CASTAI)
                              {
                                      ...
                                      player = AICast_FindEntityForName
         ("player");

                                      if (player)
                                      {
                                              VectorCopy (self->s.angles, temp);
                                              VectorCopy (self->s.angles2,
         self->s.angles); if (!(infront (self, player)))
                                              {
         ...
         NULL, NULL);
                                                              // G_Printf
         ("force use dismount cause not infront\n");
                                                      }
  ...
  if (owner->client) {
    if (fabs(AngleNormalize180(self->s.angles[YAW] -
                               self->s.apos.trBase[YAW])) > 10) {
      BG_EvaluateTrajectory(&self->s.apos, self->nextthink,
...
void mg42_die(gentity_t *self, gentity_t *inflictor, gentity_t *attacker,
              int damage, int mod) {
  gentity_t *gun;
  gentity_t *owner;
...
void mg42_spawn(gentity_t *ent) {
  ...
  ent->soundPos3 =
      G_SoundIndex("sound/weapons/mg42/mg42_death.wav"); // die sound
  ...
  // if (!(ent->spawnflags & 2)) // no tripod
  {
    ...
    base->target =
        ent->target; //----(SA)	added so mounting mg42 can trigger targets
  ...
  gun->target =
      ent->target; //----(SA)	added so mounting mg42 can trigger targets
  gun->use = mg42_use;
  gun->die = mg42_die; // JPW NERVE we want it to be called for non-tripod
                       // machineguns too (for mp_beach etc)
...
grabarc - activatable arc behind the gun.  if not specified, it uses the old
default grabbing dynamics health - how much damage can it take. Default is 50
...
void SP_mg42(gentity_t *self) {
  ...
  G_SpawnFloat("grabarc", "0",
               &grabarc); // half arc, so actually activatable over 120 deg
...
void firetrail_die(gentity_t *ent) { G_FreeEntity(ent); }
...
void tagemitter_die(gentity_t *ent) { G_FreeEntity(ent); }
...
void misc_tagemitter_finishspawning(gentity_t *ent) {
  ...
  if (!parent) {
    G_Error("misc_tagemitter: can't find parent script mover with targetname "
            "\"%s\"\n",
            ent->target);
  }
