...
static saveField_t gentityFields_18[] = {{FOFS(targetdeath), F_STRING}, {0, 0}};

static saveField_t gclientFields[] = {{CFOFS(hook), F_ENTITY},

                                      {0, 0}};

static saveField_t castStateFields[] = {{CSFOFS(aifunc), F_FUNCTION},
...
static ignoreField_t castStateIgnoreFields[] = {
    ...
    {CSFOFS(castScriptEvents),
     sizeof(cast_script_event_t *)}, // gets created upon parsing the script
                                     // file, this is static while playing
...
static persField_t castStatePersFields[] = {
    ...
    // FIXME: if so, we can't save strings in here, so how are we going to
    // create the new AI
...
// TTimo
#ifdef __linux__
  G_Error("Unable to save game.\n\nPlease check that you have at least 5mb "
          "free of disk space in your home directory.");
#else
  G_Error("Insufficient free disk space.\n\nPlease free at least 5mb of free "
          "space on game drive.");
#endif
}
...
void WriteField1(saveField_t *field, byte *base) {
  ...
  case F_ITEM:
    ...
    *(int *)p = index;
    ...
    //	match this with a function address in the function list, which is built
    //using the 	"extractfuncs.bat" in the utils folder. We then save the string
    //equivalent 	of the function. This effectively gives us cross-version save
    //games.
  case F_FUNCTION:
    if (*(byte **)p == NULL) {
      len = 0;
...
void ReadClient(fileHandle_t f, gclient_t *client, int size) {
  ...
  // (SA) I think this should never be hit at all, but as a precaution I'm
  // commenting it out anyway
  /*
          if (!(ent->r.svFlags & SVF_CASTAI) && ent->missionObjectives > 0) {
                  gentity_t *trav;

                  for (trav=NULL; trav = G_Find(trav, FOFS(classname),
     "info_player_checkpoint"); ) { if (trav->missionObjectives ==
     ent->missionObjectives && Distance(trav->s.origin, ent->r.currentOrigin) <
     800) { G_SetOrigin( ent, trav->s.origin ); VectorCopy( trav->s.origin,
     ent->client->ps.origin );

                                  trap_GetUsercmd( ent->client - level.clients,
     &ent->client->pers.cmd ); SetClientViewAngle( ent, trav->s.angles ); break;
                          }
                  }

                  if (!trav) {
                          trap_GetUsercmd( ent->client - level.clients,
     &ent->client->pers.cmd ); SetClientViewAngle( ent,
...
void ReadEntity(fileHandle_t f, gentity_t *ent, int size) {
  ...
  // if this is a camera, then make it the current global camera (silly global
  // variables..)
  ...
  if (ent->s.number == 0) {
    ...
    for (i = 0; i < sizeof(ent->missionObjectives) * 8; i++) {
      if (ent->missionObjectives & (1 << i)) {
        trap_Cvar_Register(&cvar, va("g_objective%i", i + 1), "1",
                           CVAR_ROM); // set g_objective<n> cvar
...
void WriteCastState(fileHandle_t f, cast_state_t *cs) {
  ...
  // write the block
  // if (!G_SaveWrite (&temp, sizeof(temp), f)) G_SaveWriteError();
  length = G_Save_Encode((byte *)&temp, castStateBuf, sizeof(temp),
                         sizeof(castStateBuf));
  if (!G_SaveWrite(&length, sizeof(length), f)) {
    G_SaveWriteError();
  }
...
void ReadCastState(fileHandle_t f, cast_state_t *cs, int size) {
  ...
  if (ver == 10) {
    ...
    trap_FS_Read(castStateBuf, decodedSize, f); // decode it
  ...
  for (ifield = castStateIgnoreFields; ifield->len; ifield++) {
    memcpy(((byte *)&temp) + ifield->ofs, ((byte *)cs) + ifield->ofs,
           ifield->len);
  }
  ...
  if (cs->bs && !cs->deathTime) {
    ...
    memset(g_entities[cs->entityNum].client->ps.delta_angles, 0,
    ...
    // copy the ps
    memcpy(&cs->bs->cur_ps, &g_entities[cs->entityNum].client->ps,
...
qboolean G_SaveGame(char *username) {
  ...
  for (i = 0; i < strlen(username); i++) {
    if (!Q_isforfilename(username[i]) &&
      ...
      G_Printf("G_SaveGame: '%s'.  Invalid character (%c) in filename. Must "
  ...
  // TTimo
  // show_bug.cgi?id=434
  // make sure we keep the global version number consistent with what we are
  // doing
  ver = SAVE_VERSION;
  ...
  // this is only set in the map scripts, and was previously only handled in the
  // menu's
...
void G_LoadGame(char *filename) {
  ...
  if (i != SAVE_VERSION && i != 17 && i != 13 && i != 14 &&
      i != 15) { // 13 is beta7, 14 is pre "SA_MOVEDSTUFF"
    trap_FS_FCloseFile(f);
  ...
  if (ver == 17) {
    ...
    G_Printf(
        "WARNING: backward compatibility, loading a version 17 saved game.\n"
  ...
  if (ver >= SA_MOVEDSTUFF) {
    if (ver > SA_ADDEDMUSIC) {
      ...
      if (strlen(musicString)) {
        trap_Cvar_Register(&musicCvar, "s_currentMusic", "",
                           CVAR_ROM); // get current music
        if (Q_stricmp(musicString,
                      musicCvar.string)) { // it's different than what's
                                           // playing, so fade out and queue up
          //					trap_SendServerCommand(-1, "mu_fade 0
  ...
  /*
     ...
     "autosave" )) {
                  ...
                  if (strstr(mapname, ".svg")) mapname[strstr(mapname, ".svg") -
...
void PersWriteCastState(fileHandle_t f, cast_state_t *cs) {
  ...
  // save the fields
  for (field = castStatePersFields; field->len; field++) { // write the block
...
void PersReadCastState(fileHandle_t f, cast_state_t *cs) {
  ...
  // read the fields
  for (field = castStatePersFields; field->len; field++) { // read the block
