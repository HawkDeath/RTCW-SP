...
qboolean G_ScriptAction_GotoMarker(gentity_t *ent, char *params) {
  ...
  if (!params || ent->scriptStatus.scriptStackChangeTime <
...
qboolean G_ScriptAction_Trigger(gentity_t *ent, char *params) {
  ...
  if (trent) { // we are triggering an AI
    ...
    AICast_ScriptEvent(AICast_GetCastState(trent->s.number), "trigger",
                       trigger);
    return qtrue;
  }
...
qboolean G_ScriptAction_PlayAnim(gentity_t *ent, char *params) {
  ...
  if (looping) {
    ...
    if (idealframe > endframe) {
      ...
      ent->s.eFlags &= ~EF_MOVER_ANIMATE; // stop interpolation, since we have
                                          // gone passed the endframe
    } else {
...
qboolean G_ScriptAction_AlertEntity(gentity_t *ent, char *params) {
  ...
  if (alertent->client) {
    ...
    if (!alertent->AIScript_AlertEntity) {
      G_Error("G_Scripting: alertentity \"%s\" (classname = %s) doesn't have "
    ...
    if (!alertent->use) {
      G_Error("G_Scripting: alertentity \"%s\" (classname = %s) doesn't have a "
              "\"use\" function\n",
              params, alertent->classname);
    }
...
qboolean G_ScriptAction_MissionFailed(gentity_t *ent, char *params) {
  ...
  trap_SendServerCommand(-1,
                         va("snd_fade 0 %d", time * 1000)); //----(SA)	added
...
qboolean G_ScriptAction_MissionSuccess(gentity_t *ent, char *params) {
  ...
  if (!token[0]) {
    G_Error(
        "AI Scripting: missionsuccess requires a mission_level identifier\n");
  }
...
qboolean G_ScriptAction_FaceAngles(gentity_t *ent, char *params) {
  ...
  if (!params || !params[0]) {
    G_Error("G_Scripting: syntax: faceangles <pitch> <yaw> <roll> "
            "<duration/GOTOTIME>\n");
  }
  ...
  if (ent->scriptStatus.scriptStackChangeTime == level.time) {
    ...
    token = COM_Parse(&pString);
    if (!token || !token[0]) {
      G_Error("G_Scripting: faceangles requires a <pitch> <yaw> <roll> "
              "<duration/GOTOTIME>\n");
    }
...
qboolean G_ScriptAction_TagConnect(gentity_t *ent, char *params) {
  ...
  token = COM_Parse(&pString);
  if (!token[0]) {
    G_Error("G_ScriptAction_TagConnect: syntax: attachtotag <targetname> "
            "<tagname>\n");
  }
  ...
  if (!parent) {
    ...
    if (!parent) {
      G_Error("G_ScriptAction_TagConnect: unable to find entity with "
              "targetname \"%s\"",
              token);
    }
  }
...
qboolean G_ScriptAction_StartCam(gentity_t *ent, char *params) {
  ...
  if (!player) {
    G_Error(
        "player not found, perhaps you should give them more time to spawn in");
  }
...
qboolean G_ScriptAction_ObjectiveAxisDesc(gentity_t *ent, char *params) {
  ...
  if (!token[0]) {
    G_Error(
        "G_ScriptAction_ObjectiveAxisDesc: description parameter required\n");
  }
...
qboolean G_ScriptAction_ObjectiveAlliedDesc(gentity_t *ent, char *params) {
  ...
  if (!token[0]) {
    G_Error(
        "G_ScriptAction_ObjectiveAlliedDesc: description parameter required\n");
  }
...
qboolean G_ScriptAction_BackupScript(gentity_t *ent, char *params) {
  ...
  // if we're not at the top of an event, then something is _probably_ wrong
  // with the script
  //	if (ent->scriptStatus.scriptStackHead > 0) {
  //		G_Printf( "ENTITY SCRIPT: WARNING: backupscript not at start of
  //event, possibly harmful.\n");
  //	}
