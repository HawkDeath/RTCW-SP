...
char *AIFunc_Helga_SpiritAttack(cast_state_t *cs) {
  ...
  if ((ent->client->ps.torsoAnim & ~ANIM_TOGGLEBIT) -
  ...
  if (cs->thinkFuncChangeTime < level.time - HELGA_SPIRIT_BUILDUP_TIME) {
    ...
    if (level.time > cs->thinkFuncChangeTime + HELGA_SPIRIT_BUILDUP_TIME +
                         HELGA_SPIRIT_FADEOUT_TIME) {
      ent->client->ps.torsoTimer = 0;
      ent->client->ps.legsTimer = 0;
      return AIFunc_DefaultStart(cs);
    }
...
char *AIFunc_Helga_SpiritAttack_Start(cast_state_t *cs) {
  ...
  // play an anim
  BG_UpdateConditionValue(cs->entityNum, ANIM_COND_WEAPON, WP_MONSTER_ATTACK2,
                          qtrue);
...
char *AIFunc_Helga_Melee(cast_state_t *cs) {
  ...
  anim = (ent->client->ps.torsoAnim & ~ANIM_TOGGLEBIT) -
         BG_AnimationIndexForString("attack3", cs->entityNum);
  if (anim < 0 || anim >= NUM_HELGA_ANIMS) {
    // animation interupted
    cs->aiFlags &= ~AIFL_SPECIAL_FUNC;
    return AIFunc_DefaultStart(cs);
    // G_Error( "AIFunc_HelgaZombieMelee: helgaBoss using invalid or unknown
    // attack anim" );
  }
  if (cs->animHitCount < MAX_HELGA_IMPACTS &&
      helgaHitTimes[anim][cs->animHitCount] >= 0) {
    ...
    cs->ideal_viewangles[PITCH] =
        AngleNormalize180(cs->ideal_viewangles[PITCH]);
    ...
    if (!cs->animHitCount) {
      ...
      hitDelay = helgaHitTimes[anim][cs->animHitCount] -
                 helgaHitTimes[anim][cs->animHitCount - 1];
    }
    ...
    if (level.time - cs->weaponFireTimes[cs->weaponNum] > hitDelay) {
      ...
      if (enemyDist < 10 + AICast_WeaponRange(cs, cs->weaponNum)) {
        trap_Trace(&tr, ent->r.currentOrigin, NULL, NULL,
                   enemy->r.currentOrigin, ent->s.number, MASK_SHOT);
  ...
  AICast_PredictMovement(ecs, 2, 0.3, &move,
...
char *AIFunc_FlameZombie_Portal(cast_state_t *cs) {
  ...
  if (cs->thinkFuncChangeTime < level.time - PORTAL_ZOMBIE_SPAWNTIME) {
    ...
    AICast_UpdateVisibility(&g_entities[cs->entityNum],
                            AICast_FindEntityForName("player"), qfalse, qtrue);
...
char *heinrichSounds[MAX_HEINRICH_SOUNDS] = {
    "heinrichSwordImpact",          "heinrichSwordLungeStart",
    "heinrichSwordKnockbackStart",  "heinrichSwordKnockbackWeapon",
    "heinrichSwordSideSlashStart",  "heinrichSwordSideSlashWeapon",
    "heinrichSwordEarthquakeStart", "heinrichRaiseWarriorStart",
    "heinrichTauntGoodHealth",      "heinrichTauntLowHealth",
};
...
void AICast_Heinrich_Taunt(cast_state_t *cs) {
  ...
  if (ent->health > cs->attributes[STARTING_HEALTH] * 0.25) {
    if (lastTaunt > level.time || lastTaunt < level.time - 20000) {
      G_AddEvent(ent, EV_GENERAL_SOUND,
                 heinrichSoundIndex[HEINRICH_TAUNT_GOODHEALTH]);
      lastTaunt = level.time;
    }
  } else {
    if (lastTaunt > level.time || lastTaunt < level.time - 40000) {
      G_AddEvent(ent, EV_GENERAL_SOUND,
                 heinrichSoundIndex[HEINRICH_TAUNT_LOWHEALTH]);
      lastTaunt = level.time;
    }
  }
}
  ...
  if (cs->enemyNum >= 0 && !(cs->aiFlags & AIFL_MISCFLAG1)) {
    ...
    */                                                                                                                                                                                                           // ready for damage?
    if (cs->thinkFuncChangeTime < level.time - HEINRICH_LUNGE_DELAY) {
      ...
      // do melee damage
      if (tr && (tr->entityNum == cs->enemyNum)) {
        G_Damage(&g_entities[tr->entityNum], ent, ent, left, tr->endpos,
                 HEINRICH_LUNGE_DAMAGE, 0, MOD_GAUNTLET);
        // sound
        G_AddEvent(ent, EV_GENERAL_SOUND,
                   heinrichSoundIndex[HEINRICH_SWORDIMPACT]);
      }
    }
  }
...
char *AIFunc_Heinrich_SwordLungeStart(cast_state_t *cs) {
  ...
  // sound
  G_AddEvent(ent, EV_GENERAL_SOUND,
             heinrichSoundIndex[HEINRICH_SWORDLUNGE_START]);
  // face them
  AICast_AimAtEnemy(cs);
  // clear flags
  cs->aiFlags &= ~(AIFL_MISCFLAG1 | AIFL_MISCFLAG2);
  // play the anim
  BG_PlayAnimName(&ent->client->ps, "attack9", ANIM_BP_BOTH, qtrue, qfalse,
                  qtrue);
  // start the func
...
char *AIFunc_Heinrich_SwordKnockback(cast_state_t *cs) {
    ...
    */                                                                                                                                                                                                           // ready for damage?
    if (cs->thinkFuncChangeTime < level.time - HEINRICH_KNOCKBACK_DELAY) {
      ...
      if (tr && (tr->entityNum == cs->enemyNum)) {
        ...
        G_Damage(&g_entities[tr->entityNum], ent, ent, left, tr->endpos,
                 HEINRICH_KNOCKBACK_DAMAGE, 0, MOD_GAUNTLET);
        // sound
        G_AddEvent(ent, EV_GENERAL_SOUND,
                   heinrichSoundIndex[HEINRICH_SWORDIMPACT]);
        // throw them in direction of impact
        if ((ent->client->ps.torsoAnim & ~ANIM_TOGGLEBIT) ==
            BG_AnimationIndexForString("attack2", cs->entityNum)) {
          // right
          right[2] = 0.5;
          VectorMA(g_entities[cs->enemyNum].client->ps.velocity, 400, right,
                   g_entities[cs->enemyNum].client->ps.velocity);
        } else {
          // left
          left[2] = 0.5;
          VectorMA(g_entities[cs->enemyNum].client->ps.velocity, 400, left,
                   g_entities[cs->enemyNum].client->ps.velocity);
        }
      }
    }
  ...
  /*	DISABLED FOR SWORDKNOCKBACK..looks bad
          // if they are outside range, move forward
          AICast_PredictMovement( ecs, 2, 0.3, &move,
     &g_entities[cs->enemyNum].client->pers.cmd, -1 ); VectorSubtract(
     move.endpos, cs->bs->origin, vec ); vec[2] = 0; enemyDist = VectorLength(
     vec ); enemyDist -= g_entities[cs->enemyNum].r.maxs[0]; enemyDist -=
     ent->r.maxs[0]; if (enemyDist > 30) {	// we can get closer if
     (ent->client->ps.legsTimer) { cs->castScriptStatus.scriptNoMoveTime =
     level.time + 100;
...
char *AIFunc_Heinrich_SwordKnockbackStart(cast_state_t *cs) {
  ...
  // sound
  G_AddEvent(ent, EV_GENERAL_SOUND,
             heinrichSoundIndex[HEINRICH_SWORDKNOCKBACK_START]);
  // weapon sound
  G_AddEvent(ent, EV_GENERAL_SOUND,
             heinrichSoundIndex[HEINRICH_SWORDKNOCKBACK_WEAPON]);
  // face them
  AICast_AimAtEnemy(cs);
  // clear flags
  cs->aiFlags &= ~(AIFL_MISCFLAG1 | AIFL_MISCFLAG2);
  // play the anim
  if (rand() % 2) {
    BG_PlayAnimName(&ent->client->ps, "attack2", ANIM_BP_BOTH, qtrue, qfalse,
                    qtrue);
  } else {
    BG_PlayAnimName(&ent->client->ps, "attack3", ANIM_BP_BOTH, qtrue, qfalse,
                    qtrue);
  }
  // start the func
  ...
  if (cs->enemyNum >= 0 && !(cs->aiFlags & AIFL_MISCFLAG1)) {
    ...
    if (cs->thinkFuncChangeTime < level.time - HEINRICH_SLASH_DELAY) {
      ...
      if (tr && (tr->entityNum == cs->enemyNum)) {
        ...
        VectorNegate(right, left);
        G_Damage(&g_entities[tr->entityNum], ent, ent, left, tr->endpos,
                 HEINRICH_SLASH_DAMAGE, 0, MOD_GAUNTLET);
        // sound
        G_AddEvent(ent, EV_GENERAL_SOUND,
                   heinrichSoundIndex[HEINRICH_SWORDIMPACT]);
        // throw them in direction of impact
...
char *AIFunc_Heinrich_SwordSideSlashStart(cast_state_t *cs) {
  ...
  // sound
  G_AddEvent(ent, EV_GENERAL_SOUND,
             heinrichSoundIndex[HEINRICH_SWORDSIDESLASH_START]);
  // weapon sound
  G_AddEvent(ent, EV_GENERAL_SOUND,
             heinrichSoundIndex[HEINRICH_SWORDSIDESLASH_WEAPON]);
  // face them
  AICast_AimAtEnemy(cs);
  // clear flags
  cs->aiFlags &= ~(AIFL_MISCFLAG1 | AIFL_MISCFLAG2);
  // play the anim
  BG_PlayAnimName(&ent->client->ps, "attack8", ANIM_BP_BOTH, qtrue, qfalse,
                  qtrue);
  // start the func
...
char *AIFunc_Heinrich_Earthquake(cast_state_t *cs) {
  ...
  if (!(cs->aiFlags & AIFL_MISCFLAG1)) {
    ...
    if (cs->thinkFuncChangeTime < level.time - HEINRICH_STOMP_DELAY) {
      ...
      if (tr && (tr->entityNum == cs->enemyNum)) {
        G_Damage(&g_entities[tr->entityNum], ent, ent, vec3_origin, tr->endpos,
                 HEINRICH_STOMP_DAMAGE, 0, MOD_GAUNTLET);
      }
  ...
  if (cs->thinkFuncChangeTime < level.time - HEINRICH_STOMP_DELAY) {
    ...
    if ((enemy->s.groundEntityNum != ENTITYNUM_NONE) &&
      ...
      VectorScale(enemyVec,
                  2.0 * (0.6 + 0.5 * random()) * scale *
                      (0.6 + 0.6 * (1.0 - (enemyDist / HEINRICH_STOMP_RANGE))),
                  enemyVec);
      enemyVec[2] = scale * HEINRICH_STOMP_VELOCITY_Z *
                    (1.0 - 0.5 * (enemyDist / HEINRICH_STOMP_RANGE));
      // bounce the player using this velocity
      VectorAdd(enemy->client->ps.velocity, enemyVec,
...
char *AIFunc_Heinrich_MeleeStart(cast_state_t *cs) {
  ...
  } else if (VectorDistance(ent->r.currentOrigin, enemy->r.currentOrigin) >=
             HEINRICH_SLASH_RANGE) {
  ...
  case 1:
    ...
    if (lastStomp >
        level.time - 12000) { // plenty of time to let debris disappear
      return NULL;
    }
    ...
    // sound
    G_AddEvent(ent, EV_GENERAL_SOUND,
               heinrichSoundIndex[HEINRICH_EARTHQUAKE_START]);
    // play the anim
    BG_PlayAnimName(&ent->client->ps, "attack7", ANIM_BP_BOTH, qtrue, qfalse,
                    qtrue);
    // start the func
...
char *AIFunc_Heinrich_RaiseDeadStart(cast_state_t *cs) {
  ...
  if (cnt < HEINRICH_RAISEDEAD_COUNT && free) { // need a new one
    ...
    cs->aiFlags |= AIFL_SPECIAL_FUNC;
    // start the animation
    BG_PlayAnimName(&ent->client->ps, "attack4", ANIM_BP_BOTH, qtrue, qfalse,
                    qtrue);
    // play the sound
    G_AddEvent(ent, EV_GENERAL_SOUND,
               heinrichSoundIndex[HEINRICH_RAISEDEAD_START]);
    // start the func
  ...
  while ((trav = G_Find(trav, FOFS(classname), "func_bats"))) {
    if (trav->spawnflags & 4) {
      ...
      if (trav) {
        if (VectorDistance(g_entities[0].s.pos.trBase, trav->s.origin) >
            circleDist) {
          cs->aiFlags &= ~AIFL_MISCFLAG1;
          ent->count2 = 0;
          cs->aiFlags |= AIFL_SPECIAL_FUNC;
          // start the animation
          BG_PlayAnimName(&ent->client->ps, "attack4", ANIM_BP_BOTH, qtrue,
                          qfalse, qtrue);
          // play the sound
          G_AddEvent(ent, EV_GENERAL_SOUND,
                     heinrichSoundIndex[HEINRICH_RAISEDEAD_START]);
      ...
      if (trav) {
        if (VectorDistance(g_entities[0].s.pos.trBase, trav->s.origin) >
          ...
          G_AddEvent(ent, EV_GENERAL_SOUND,
                     heinrichSoundIndex[HEINRICH_RAISEDEAD_START]);
          // start the func
          cs->aifunc = AIFunc_Heinrich_RaiseDead; // just do raise dead, without
                                                  // raising any warriors
          return "AIFunc_Heinrich_RaiseDead";
        }
      }
      break;
