...
typedef enum {
  ...
  ERR_ENDGAME // not an error.  just clean up properly, exit to the menu, and
              // start up the "endgame" menu  //----(SA)	added
} errorParm_t;
...
#define REF_FORCE_DLIGHT                                                       \
  (1 << 31) // RF, passed in through overdraw parameter, force this dlight under
            // all conditions
...
//#define	MAX_EVENTS				2	// max events per frame
//before we drop events
...
typedef struct playerState_s {
  ...
  vec3_t velocity;
  int weaponTime;
  int weaponDelay; // for weapons that don't fire immediately when 'fire' is hit
                   // (grenades, venom, ...)
  int grenadeTimeLeft; // for delayed grenade throwing.  this is set to a
                       ...
                       // amount of time left before the grenade goes off (or if
                       // it gets to 0 while in players hand, it explodes)
  ...
  trace_t serverCursorHintTrace; // not communicated over net, but used to store
                                 // the current server-side cursorhint trace

  // ----------------------------------------------------------------------
  // not communicated over the net at all
  // FIXME: this doesn't get saved between predicted frames on the clients-side
  // (cg.predictedPlayerState) So to use persistent variables here, which don't
  // need to come from the server, we could use a marker variable, and use that
  // to store everything after it before we read in the new values for the
  // predictedPlayerState, then restore them after copying the structure
  // recieved from the server.

  // (SA) yeah.  this is causing me a little bit of trouble too.  can we go
  // ahead with the above suggestion or find an alternative?
  ...
  float aimSpreadScaleFloat; // (SA) the server-side aimspreadscale that lets it
                             // track finer changes but still only transmit the
                             // 8bit int to the client
...
typedef struct usercmd_s {
  ...
  signed char wolfkick; // RF, we should move this over to a wbutton, this is a
                        // huge waste of bandwidth

  unsigned short cld; // NERVE - SMF - send client damage in usercmd instead of
                      // as a server command
} usercmd_t;
...
typedef enum {
  ...
  TR_GRAVITY_PAUSED, //----(SA)	has stopped, but will still do a short trace to
...
// RF, put this here so we have a central means of defining a Zombie (kind of a
// hack, but this is to minimize bandwidth usage)
...
typedef struct entityState_s {
  ...
  int legsAnim;  // mask off ANIM_TOGGLEBIT
  int torsoAnim; // mask off ANIM_TOGGLEBIT
                 //	int		weapAnim;		// mask off ANIM_TOGGLEBIT	//----(SA)
                 //removed (weap anims will be client-side only)
