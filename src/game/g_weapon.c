...
void Bullet_Fire_Extended(gentity_t *source, gentity_t *attacker, vec3_t start,
                          vec3_t end, float spread, int damage, int recursion);
...
void Weapon_Knife(gentity_t *ent) {
  ...
  if (traceEnt->client) {
    if (ent->client->ps.serverCursorHint == HINT_KNIFE) {
      //		AngleVectors (ent->client->ps.viewangles,
      //pforward, NULL, NULL); 		AngleVectors (traceEnt->client->ps.viewangles,
      //eforward, NULL, NULL);

      // (SA) TODO: neutralize pitch (so only yaw is considered)
      //		if(DotProduct( eforward, pforward ) > 0.9f)	{
      //// from behind

      // if relaxed, the strike is almost assured a kill
      // if not relaxed, but still from behind, it does 10x damage (50)

      // (SA) commented out right now as the ai's state always checks here as
      // 'combat'
  ...
  G_Damage(traceEnt, ent, ent, vec3_origin, tr.endpos,
           (damage + rand() % 5) * s_quadFactor, 0, mod);
}
      ...
      if ((traceEnt->client->ps.pm_type == PM_DEAD) &&
        ...
        if (level.time - ent->client->ps.classWeaponTime >
          ...
          ent->client->ps.classWeaponTime =
              level.time - g_medicChargeTime.integer;
        }
        ...
        if (ent->client->ps.classWeaponTime > level.time) { // heal the dude
                                                            /*
                                                                                                    ...
                                                                                                    traceEnt->r.maxs[2] = 0;
                                                                                                    traceEnt->client->ps.pm_type =
                                                               PM_NORMAL;                                                   traceEnt->client->ps.torsoAnim =
                                                               traceEnt->client->oldAnimTorso;                                                   traceEnt->client->ps.legsAnim =
                                                               traceEnt->client->oldAnimLegs;                                                   traceEnt->die = player_die;
                                                                                                    traceEnt->client->respawnTime
                                                               = level.time + 1700;
                                                            */
          ...
          VectorCopy(org, traceEnt->client->ps.origin);
           ...
           //g_medicChargeTime.integer) {
    if (level.time - ent->client->ps.classWeaponTime >=
        g_medicChargeTime.integer * 0.25f) {
      if (level.time - ent->client->ps.classWeaponTime >
          g_medicChargeTime.integer) {
        ent->client->ps.classWeaponTime =
            level.time - g_medicChargeTime.integer;
      ...
      //level.time) 				ent->client->ps.classWeaponTime = level.time;
...
void Weapon_Engineer(gentity_t *ent) {
  ...
  trap_Trace(&tr, muzzleTrace, NULL, NULL, end, ent->s.number,
  ...
  if (traceEnt->methodOfDeath == MOD_DYNAMITE) {
    ...
    if (traceEnt->health >= 248) {
      ...
      if (ent->client->sess.sessionTeam == TEAM_RED) {
        trap_SendServerCommand(-1,
                               "cp \"Axis engineer disarmed a det charge!\n\"");
      } else {
        trap_SendServerCommand(
            -1, "cp \"Allied engineer disarmed a det charge!\n\"");
      }
      // jpw
    }
    ...
    if (ent->client->ps.classWeaponTime + g_engineerChargeTime.integer <
        level.time) {
      ent->client->ps.classWeaponTime =
          level.time - g_engineerChargeTime.integer;
    }
...
void weapon_callAirStrike(gentity_t *ent) {
  ...
  ent->nextthink = level.time + 1000 + NUMBOMBS * 100 +
                   crandom() * 50; // 3000 offset is for aircraft flyby
  ...
  if ((tr.fraction < 1.0) && (!(tr.surfaceFlags & SURF_SKY))) {
    G_SayTo(ent->parent, ent->parent, 2, COLOR_YELLOW,
            "Pilot: ", "Can't see target, aborting bomb run");
    return;
  }
  ...
  RotatePointAroundVector(bombaxis, pos, lookaxis,
                          90 +
                              crandom() * 30); // munge the axis line a bit so
  ...
  // add an aircraft (looks suspiciously like a rocket right now) (but doesn't
  // work)
  /*
          bomb = G_Spawn();
     ...
     SVF_BROADCAST;
          bomb->s.weapon		= WP_GRENADE_LAUNCHER; // might wanna change
     this bomb->r.ownerNum	= ent->s.number; bomb->parent		=
     ent->parent; bomb->damage		= 400; // maybe should un-hard-code
     these? bomb->splashDamage  = 400; bomb->classname		=
     "fighterbomber"; bomb->splashRadius			= 400;
          bomb->methodOfDeath			= MOD_DYNAMITE; // FIXME add MOD
     for air strike bomb->splashMethodOfDeath	= MOD_DYNAMITE_SPLASH;
          ...
          bomb->s.modelindex = G_ModelIndex(
     "models/mapobjects/vehicles/m109.md3" );
  */
  for (i = 0; i < NUMBOMBS; i++) {
    bomb = G_Spawn();
    bomb->nextthink =
        level.time + i * 100 + crandom() * 50 +
        1000; // 1000 for aircraft flyby, other term for tumble stagger
    bomb->think = G_ExplodeMissile;
    bomb->s.eType = ET_MISSILE;
...
// problems)
//			(this was causing some problems with bullet marks appearing
//since snapping 			too far off the target surface causes the the distance between
//the transmitted impact 			point and the actual hit surface larger than the mark
//radius.  (so nothing shows) )
...
#define	MACHINEGUN_TEAM_DAMAGE	G_GetWeaponDamage(WP_MACHINEGUN) // JPW
// wimpier MG in teamplay
*/
// jpw

// RF, wrote this so we can dynamically switch between old and new values while
// testing g_userAim
float G_GetWeaponSpread(int weapon) {
  if (g_gametype.integer ==
      GT_SINGLE_PLAYER) { // JPW NERVE -- don't affect SP game
  ...
  } else { // JPW NERVE but in multiplayer...  new spreads and don't look at
           // g_userAim
    switch (weapon) {
    case WP_LUGER:
      return 600;
...
void Tesla_Fire(gentity_t *ent) {
  // TODO: Find all targets in the client's view frame, and lock onto them all,
  // applying damage and telling all clients to draw the appropriate effects.
...
void EmitterCheck(gentity_t *ent, gentity_t *attacker, trace_t *tr) {
  ...
  SnapVectorTowards(tr->endpos,
                    attacker->s.origin); // make sure it's out of the wall
...
void Bullet_Endpos(gentity_t *ent, float spread, vec3_t *end) {
  ...
  if (ent->r.svFlags & SVF_CASTAI) {
    ...
    if (ent->s.weapon == WP_SNOOPERSCOPE || ent->s.weapon == WP_SNIPERRIFLE ||
      ...
      //		if(ent->s.weapon == WP_SNOOPERSCOPE || ent->s.weapon ==
      //WP_SNIPERRIFLE) {
...
void Bullet_Fire_Extended(gentity_t *source, gentity_t *attacker, vec3_t start,
  ...
  gentity_t *traceEnt;
  int dflags =
      0; // flag if source==attacker, meaning it wasn't shot directly, but was
         // reflected went through an entity that allows bullets to pass through
  qboolean reflectBullet = qfalse;

  // RF, abort if too many recursions.. there must be a real solution for this,
  // but for now this is the safest fix I can find
  if (recursion > 12) {
    return;
  }
  ...
  if (traceEnt->takedamage) {
    ...
    if (reflectBool) {
      ...
      //----(SA)	modified to use extended version so attacker would pass
      //through 			Bullet_Fire( traceEnt, 1000, damage );
      Bullet_Endpos(traceEnt, 2800, &reflect_end); // make it inaccurate
      Bullet_Fire_Extended(traceEnt, attacker, muzzleTrace, reflect_end, spread,
                           damage, recursion + 1);
      //----(SA)	end
      ...
      // DHM - Nerve :: Only in single player
      if (attacker->client && traceEnt->client &&
          g_gametype.integer == GT_SINGLE_PLAYER &&
          (traceEnt->r.svFlags & SVF_CASTAI) &&
          (attacker->r.svFlags & SVF_CASTAI) &&
          AICast_SameTeam(AICast_GetCastState(attacker->s.number),
                          traceEnt->s.number)) {
      ...
      G_Damage(traceEnt, attacker, attacker, forward, tr.endpos, damage, dflags,
               ammoTable[attacker->s.weapon].mod);

      // allow bullets to "pass through" func_explosives if they break by taking
      // another simultanious shot start new bullet at position this hit and
      // continue to the end position (ignoring shot-through ent in next trace)
      // spread = 0 as this is an extension of an already spread shot (so just
      // go straight through)
      if (Q_stricmp(traceEnt->classname, "func_explosive") == 0) {
        ...
        if (traceEnt->health <= 0) {
          Bullet_Fire_Extended(
              traceEnt, attacker, tr.endpos, end, 0, damage / 2,
              recursion + 1); // halve the damage each player it goes through
        }
      }
    }
  }
...
gentity_t *weapon_grenadelauncher_fire(gentity_t *ent, int grenType) {
  ...
  if ((ent->s.apos.trBase[0] > 0) &&
      (grenType !=
       WP_GRENADE_SMOKE)) { // JPW NERVE -- smoke grenades always overhand
    underhand = qtrue;
  }
  ...
  upangle = max(upangle, -50); //	now clamped to			-50 / 50
  ...
  // JPW NERVE
           ...
           WP_GRENADE_SMOKE) { // smoke grenades *really* get chucked
    upangle *= 800;
  }
  // jpw
  else {            // WP_DYNAMITE
  ...
  if(ent->aiCharacter)
  {
          VectorScale(forward, 700, forward);				//----(SA)
  700 is the default grenade throw they are already used to m = fire_grenade
  (ent, muzzleTrace, forward);	//----(SA)	temp to make AI's throw grenades
  at their actual target
  }
  else
  */

  {
    ...
    if (underhand) {
      VectorMA(muzzleEffect, 15, forward,
               tosspos); // move a little bit more away from the player (so
                         // underhand tosses don't get caught on nearby lips)
      tosspos[2] -= 24;  // lower origin for the underhand throw
      upangle *= 1.3;    // a little more force to counter the lower position /
                         // lack of additional lift
    }

    VectorScale(forward, upangle, forward);

    {
      // check for valid start spot (so you don't throw through or get stuck in
      // a wall)
      trace_t tr;
      vec3_t viewpos;
  ...
  if (grenType == WP_GRENADE_SMOKE) {
    if (ent->client->sess.sessionTeam ==
        TEAM_RED) { // store team so we can generate red or blue smoke
  ...
  //later, but only for forward/back not strafing
...
==============
*/
void weapon_venom_fire(gentity_t *ent, qboolean fullmode,
...
void Weapon_RocketLauncher_Fire(gentity_t *ent, float aimSpreadScale) {
  ...
  if (!ent->aiCharacter) {
    ...
    //		VectorCopy( ent->s.pos.trBase, viewpos );
    //		viewpos[2] += ent->client->ps.viewheight;
    //		trap_Trace (&tr, viewpos, NULL, NULL, muzzleEffect, ent->s.number,
    //MASK_SHOT); 		if(tr.fraction < 1) {	// oops, bad launch spot
...
#define FLAME_THRESHOLD 50
    ...
    if (traceEnt->flameQuotaTime && traceEnt->flameQuota > 0) {
      traceEnt->flameQuota -=
          (int)(((float)(level.time - traceEnt->flameQuotaTime) / 1000) *
                (float)damage / 2.0);
    ...
    // Ridah, make em burn
    if (traceEnt->client &&
        (traceEnt->health <= 0 || traceEnt->flameQuota > FLAME_THRESHOLD)) {
      ...
      if (traceEnt->health <= 0 || !(traceEnt->r.svFlags & SVF_CASTAI) ||
        ...
        traceEnt->s.onFireEnd =
            level.time +
            99999; // make sure it goes for longer than they need to die
      }
...
void CalcMuzzlePoint(gentity_t *ent, int weapon, vec3_t forward, vec3_t right,
  ...
  {
  case WP_PANZERFAUST:
    //			VectorMA( muzzlePoint, 14, right, muzzlePoint );	//----(SA)	new
    //first person rl position
    VectorMA(muzzlePoint, 10, right,
             muzzlePoint); //----(SA)	new first person rl position
    VectorMA(muzzlePoint, -10, up, muzzlePoint);
    break;
    //		case WP_ROCKET_LAUNCHER:
    //			VectorMA( muzzlePoint, 14, right, muzzlePoint );	//----(SA)	new
    //first person rl position 			break;
  case WP_DYNAMITE:
  case WP_GRENADE_PINEAPPLE:
  case WP_GRENADE_LAUNCHER:
...
void CalcMuzzlePointForActivate(gentity_t *ent, vec3_t forward, vec3_t right,
  ...
  // snap to integer coordinates for more efficient network bandwidth usage
  //	SnapVector( muzzlePoint );
  // (SA) /\ only used server-side, so leaving the accuracy in is fine (and
  // means things that show a cursorhint will be hit when activated)
  //			(there were differing views of activatable stuff between
  //cursorhint and activatable)
}
// done.
...
void CalcMuzzlePoints(gentity_t *ent, int weapon) {
  ...
  if (!(ent->r.svFlags &
        SVF_CASTAI)) { // non ai's take into account scoped weapon 'sway' (just
                       // another way aimspread is visualized/utilized)
    ...
    if (weapon == WP_SNIPERRIFLE || weapon == WP_SNOOPERSCOPE ||
      ...
      viewang[PITCH] += ZOOM_PITCH_AMPLITUDE * sin(phase) *
                        (spreadfrac + ZOOM_PITCH_MIN_AMPLITUDE);

      phase = level.time / 1000.0 * ZOOM_YAW_FREQUENCY * M_PI * 2;
      viewang[YAW] += ZOOM_YAW_AMPLITUDE * sin(phase) *
                      (spreadfrac + ZOOM_YAW_MIN_AMPLITUDE);
    }
  }
...
void FireWeapon(gentity_t *ent) {
  ...
  if (g_userAim.integer) {
    ...
    if (ent->aiCharacter) {
      ...
      case WP_PANZERFAUST:
        aimSpreadScale +=
            0.3f; // it's calculated a different way, so this keeps the accuracy
                  // never perfect, but never rediculously wild either
        break;
  ...
  case WP_SNIPERRIFLE:
    ...
    if (!ent->aiCharacter) {
      //		if (g_gametype.integer != GT_SINGLE_PLAYER) {
      VectorCopy(ent->client->ps.viewangles, viewang);
      //			viewang[PITCH] -= 6; // handled in clientthink
      //instead
      ent->client->sniperRifleMuzzleYaw =
          crandom() * 0.5; // used in clientthink
  ...
  case WP_FG42SCOPE:
    if (!ent->aiCharacter) {
      ...
      VectorCopy(ent->client->ps.viewangles, viewang);
      //			ent->client->sniperRifleMuzzleYaw =
      //crandom()*0.04; // used in clientthink
  ...
  case WP_DYNAMITE:
    ...
    // if (ent->aiName && !Q_strncmp(ent->aiName, "mechanic", 8) &&
    // !AICast_HasFiredWeapon(ent->s.number, ent->s.weapon)) 	weapon_crowbar_throw
    //(ent); else
  ...
  case WP_TESLA:
    ...
    if (!ent->aiCharacter) {
      ...
      if (ent->s.groundEntityNum == ENTITYNUM_NONE) {
        VectorMA(ent->client->ps.velocity, -32, forward,
                 ent->client->ps.velocity);
      } else {
        VectorMA(ent->client->ps.velocity, -100, forward,
                 ent->client->ps.velocity);
      }
    }
    break;
