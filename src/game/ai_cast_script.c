...
qboolean AICast_ScriptAction_AbortIfLoadgame(cast_state_t *cs,
                                             char *params); //----(SA)	added
...
qboolean AICast_ScriptAction_SetClip(cast_state_t *cs,
                                     char *params); //----(SA)	added
qboolean AICast_ScriptAction_SelectWeapon(cast_state_t *cs, char *params);
qboolean AICast_ScriptAction_GiveArmor(cast_state_t *cs,
                                       char *params); //----(SA)	added
qboolean AICast_ScriptAction_SetArmor(cast_state_t *cs,
                                      char *params); //----(SA)	added
qboolean AICast_ScriptAction_SuggestWeapon(cast_state_t *cs,
                                           char *params); //----(SA)	added
...
qboolean AICast_ScriptAction_EndGame(cast_state_t *cs,
                                     char *params); //----(SA)	added
qboolean AICast_ScriptAction_Teleport(cast_state_t *cs,
                                      char *params); //----(SA)	added
...
qboolean AICast_ScriptAction_Cigarette(cast_state_t *cs,
                                       char *params); //----(SA)	added
qboolean AICast_ScriptAction_StartCam(cast_state_t *cs, char *params);
qboolean AICast_ScriptAction_StopCam(cast_state_t *cs,
                                     char *params); //----(SA)	added
...
qboolean AICast_ScriptAction_MusicStart(cast_state_t *cs,
                                        char *params); //----(SA)
qboolean AICast_ScriptAction_MusicPlay(cast_state_t *cs,
                                       char *params); //----(SA)
qboolean AICast_ScriptAction_MusicStop(cast_state_t *cs,
                                       char *params); //----(SA)
qboolean AICast_ScriptAction_MusicFade(cast_state_t *cs,
                                       char *params); //----(SA)
qboolean AICast_ScriptAction_MusicQueue(cast_state_t *cs,
                                        char *params); //----(SA)
...
cast_script_stack_action_t scriptActions[] = {
    ...
    {"abort_if_loadgame",
     AICast_ScriptAction_AbortIfLoadgame}, //----(SA)	added
    ...
    {"objectivemet",
     AICast_ScriptAction_ObjectiveMet}, // dupe of missionsuccess so scripts can
                                        // changeover to a more logical name
    ...
    {"mu_start", AICast_ScriptAction_MusicStart}, // (char *new_music, int time)
                                                  // // time to fade in
    {"mu_play", AICast_ScriptAction_MusicPlay},   // (char *new_music)
    {"mu_stop",
     AICast_ScriptAction_MusicStop}, // (int time)	// time to fadeout
    {"mu_fade",
     AICast_ScriptAction_MusicFade}, // (float target_volume, int time) // time
                                     // to fade to target
    {"mu_queue",
     AICast_ScriptAction_MusicQueue}, // (char *new_music)	// music that
                                      // will start when previous fades to 0
                                      //----(SA)	end
...
qboolean AICast_EventMatch_StringEqual(cast_script_event_t *event,
                                       char *eventParm);
qboolean AICast_EventMatch_IntInRange(cast_script_event_t *event,
                                      char *eventParm);
...
cast_script_event_define_t scriptEvents[] = {
    ...
    {"enemysight", AICast_EventMatch_StringEqual}, // enemy has been sighted for
                                                   // the first time (once only)
    {"sight", AICast_EventMatch_StringEqual}, // non-enemy has been sighted for
                                              // the first time (once only)
    {"enemydead", AICast_EventMatch_StringEqual}, // our enemy is now dead
    {"trigger",
     AICast_EventMatch_StringEqual}, // something has triggered us (always
                                     // followed by an identifier)
    {"pain", AICast_EventMatch_IntInRange},   // we've been hurt
    {"death", AICast_EventMatch_StringEqual}, // RIP
    {"activate",
     AICast_EventMatch_StringEqual}, // "param" has just activated us
    {"enemysightcorpse",
     AICast_EventMatch_StringEqual}, // sighted the given enemy as a corpse, for
                                     // the first time
    {"friendlysightcorpse",
     NULL}, // sighted a friendly as a corpse for the first time
    {"avoiddanger",
     AICast_EventMatch_StringEqual}, // we are avoiding something dangerous
    {"blocked", AICast_EventMatch_StringEqual},     // blocked by someone else
    {"statechange", AICast_EventMatch_StringEqual}, // changing aistates
    {"bulletimpact", NULL},
    {"inspectbodystart",
     AICast_EventMatch_StringEqual}, // starting to travel to body for
                                     // inspection
    {"inspectbodyend",
     AICast_EventMatch_StringEqual}, // reached body for inspection
    {"inspectsoundstart",
     AICast_EventMatch_StringEqual}, // reached sound for inspection
    {"inspectsoundend",
     AICast_EventMatch_StringEqual}, // reached sound for inspection
    {"attacksound",
     AICast_EventMatch_StringEqual}, // play a custom attack sound, and/or deny
...
qboolean AICast_EventMatch_StringEqual(cast_script_event_t *event,
  ...
  if (!event->params || !event->params[0] ||
      (eventParm && !Q_strcasecmp(event->params, eventParm))) {
    return qtrue;
  } else {
    return qfalse;
  }
...
===============
*/
cast_script_stack_action_t *AICast_ActionForString(cast_state_t *cs,
                                                   char *string) {
  int i;
...
#define MAX_SCRIPT_EVENTS 64
  ...
  while (1) {
    ...
    if (!token[0]) {
      if (!wantName) {
        G_Error("AICast_ScriptParse(), Error (line %d): '}' expected, end of "
                "script found.\n",
    ...
    if (token[0] == '}') {
      ...
      if (wantName) {
        G_Error("AICast_ScriptParse(), Error (line %d): '}' found, but not "
                "expected.\n",
                COM_GetCurrentParseLine());
      }
      wantName = qtrue;
      ...
      if (wantName) {
        G_Error("AICast_ScriptParse(), Error (line %d): '{' found, NAME "
                "expected.\n",
                COM_GetCurrentParseLine());
      }
    } else if (wantName) {
      ...
      if (numEventItems >= MAX_SCRIPT_EVENTS) {
        G_Error("AICast_ScriptParse(), Error (line %d): MAX_SCRIPT_EVENTS "
                "reached (%d)\n",
                COM_GetCurrentParseLine(), MAX_SCRIPT_EVENTS);
      }

      // if this is a "friendlysightcorpse" event, then disable corpse vis
      // sharing
        ...
        if (eventNum == 13) { // statechange event, check params
          if (strlen(token) > 1) {
            if (BG_IndexForString(token, animStateStr, qtrue) < 0) {
              G_Error("AICast_ScriptParse(), Error (line %d): unknown state "
                      "type '%s'.\n",
                      COM_GetCurrentParseLine(), token);
            }
          }
      ...
      while ((token = COM_Parse(&pScript)) && (token[0] != '}')) {
        ...
        if (!action) {
          G_Error(
              "AICast_ScriptParse(), Error (line %d): unknown action: %s.\n",
              COM_GetCurrentParseLine(), token);
        }
        ...
        for (i = 0; token[0]; i++) {
          ...
          if (i == 0) {
            ...
            if (!Q_stricmp(
                    action->actionString,
                    "giveweapon")) { // register weapon for client pre-loading
              gitem_t *weap = BG_FindItem2(
                  token); // (SA) FIXME: rats, need to fix this for weapon names
                          // with spaces: 'mauser rifle'
                          //							if(weap)
              RegisterItem(weap); // don't be nice, just do it.  if it can't
                                  // find it, you'll bomb out to the error menu
            }
            //----(SA)	end
          }
        ...
        if (curEvent->stack.numItems >= AICAST_MAX_SCRIPT_STACK_ITEMS) {
          G_Error("AICast_ScriptParse(): script exceeded MAX_SCRIPT_ITEMS "
                  "(%d), line %d\n",
  ...
  if (numEventItems > 0) {
    ...
    memcpy(cs->castScriptEvents, events,
...
void AICast_ScriptChange(cast_state_t *cs, int newScriptNum) {
  ...
  if (AICast_ScriptRun(cs, qtrue)) {
    ...
    cs->castScriptStatus.castScriptStackHead =
        scriptStatusBackup.castScriptStackHead;
    cs->castScriptStatus.castScriptStackChangeTime =
        scriptStatusBackup.castScriptStackChangeTime;
    cs->castScriptStatus.castScriptEventIndex =
        scriptStatusBackup.castScriptEventIndex;
...
void AICast_ScriptEvent(struct cast_state_s *cs, char *eventStr, char *params) {
  ...
  if (((aicast_debug.integer == 1) ||
       ((aicast_debug.integer == 2) &&
        ((strlen(aicast_debugname.string) < 1) ||
         (g_entities[cs->entityNum].aiName &&
          !strcmp(aicast_debugname.string,
                  g_entities[cs->entityNum].aiName)))))) {
    G_Printf("(%s) AIScript event: %s %s ", g_entities[cs->entityNum].aiName,
             eventStr, params);
  }
    ...
    if (cs->castScriptEvents[i].eventNum == eventNum) {
      if ((!cs->castScriptEvents[i].params) ||
          (!scriptEvents[eventNum].eventMatch ||
           scriptEvents[eventNum].eventMatch(&cs->castScriptEvents[i],
                                             params))) {
...
================
*/
void AICast_ForceScriptEvent(struct cast_state_s *cs, char *eventStr,
                             char *params) {
  int oldPauseTime;
...
qboolean AICast_ScriptRun(cast_state_t *cs, qboolean force) {
  ...
  // only allow the PLAYER'S spawn function through if we're NOT still waiting
  // on everything to finish loading in
  if (!cs->entityNum && saveGamePending &&
      Q_stricmp("spawn",
                scriptEvents[cs->castScriptEvents[cs->castScriptStatus
                                                      .castScriptEventIndex]
                                 .eventNum]
                    .eventStr)) {
    // char loading[4];
    // trap_Cvar_VariableStringBuffer( "savegame_loading", loading,
    // sizeof(loading) ); if (strlen( loading ) > 0 && atoi(loading) != 0)
    // // we're loading a savegame
    return qfalse;
  }
  ...
  stack =
      &cs->castScriptEvents[cs->castScriptStatus.castScriptEventIndex].stack;
          ...
          ((strlen(aicast_debugname.string) < 1) ||
           (g_entities[cs->entityNum].aiName &&
            !strcmp(aicast_debugname.string,
                    g_entities[cs->entityNum].aiName)))))) {
      G_Printf(
          "(%s) AIScript command: %s %s\n", g_entities[cs->entityNum].aiName,
          stack->items[cs->castScriptStatus.castScriptStackHead]
              .action->actionString,
          (stack->items[cs->castScriptStatus.castScriptStackHead].params
               ? stack->items[cs->castScriptStatus.castScriptStackHead].params
               : ""));
    }
    //
    if (!stack->items[cs->castScriptStatus.castScriptStackHead]
             .action->actionFunc(
                 cs, stack->items[cs->castScriptStatus.castScriptStackHead]
                         .params)) {
      // check that we are still running the same script that we were when we
      // call the action
      if (cs->castScriptStatus.castScriptEventIndex >= 0 &&
          stack ==
