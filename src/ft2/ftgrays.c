...
/*                                                                       */
static int render_scanline(RAS_ARG_ TScan ey, TPos x1, TScan y1, TPos x2,
                           TScan y2) {
...
static int render_cubic(RAS_ARG_ FT_Vector *control1, FT_Vector *control2,
                        FT_Vector *to) {
  TPos dx, dy, da, db;
  int top, level;
  int *levels;
...
static void grays_render_span(int y, int count, FT_Span *spans,
                              PRaster raster) {
  unsigned char *p;
    ...
    if (ras.span_y != y || count >= FT_MAX_GRAY_SPANS) {
      ...
      if (ras.span_y >= 0) {
        ...
        for (n = 0; n < count; n++, span++)
          fprintf(stderr, "[%d..%d]:%02x ", span->x, span->x + span->len - 1,
                  span->coverage);
        fprintf(stderr, "\n");
      }
  ...
  for (;;) {
    ...
    if (cur < limit && start->y == cur->y) {
      ...
      if (cover && x < ras.max_ex - ras.min_ex) {
        grays_hline(RAS_VAR_ x, y, cover * (ONE_PIXEL * 2),
...
static int FT_Outline_Decompose(FT_Outline *outline,
                                FT_Outline_Funcs *interface, void *user) {
#undef SCALED
...
static int grays_convert_glyph(RAS_ARG_ FT_Outline *outline) {
  static FT_Outline_Funcs interface = {(FT_Outline_MoveTo_Func)Move_To,
  ...
  if (ras.max_ex <= 0 || ras.min_ex >= ras.target.width || ras.max_ey <= 0 ||
      ras.min_ey >= ras.target.rows) {
    return 0;
  }
...
static void grays_raster_reset(FT_Raster raster, const char *pool_base,
                               long pool_size) {
