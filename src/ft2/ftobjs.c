...
/*                                                                       */
BASE_FUNC(FT_Error) FT_Alloc(FT_Memory memory, FT_Long size, void **P) {
  FT_Assert(P != 0);
...
/*    drivers.                                                           */
/*                                                                       */
BASE_FUNC(void) FT_Free(FT_Memory memory, void **P) {
  FT_TRACE7(("FT_Free:"));
...
/*                                                                       */
static FT_Error ft_new_input_stream(FT_Library library, FT_Open_Args *args,
                                    FT_Stream *astream) {
  FT_Error error;
  FT_Memory memory;
  FT_Stream stream;
  ...
  *astream = 0;
  ...
  if (args->flags & ft_open_memory) {
    error = 0;
    FT_New_Memory_Stream(library, args->memory_base, args->memory_size, stream);
...
static void ft_done_stream(FT_Stream *astream) {
...
BASE_FUNC(FT_Error) FT_GlyphLoader_Create_Extra(FT_GlyphLoader *loader) {
  ...
  if (!ALLOC_ARRAY(loader->base.extra_points, loader->max_points, FT_Vector)) {
    loader->use_extra = 1;
...
/*                                                                       */
BASE_FUNC(FT_Error)
FT_GlyphLoader_Check_Points(FT_GlyphLoader *loader, FT_UInt n_points,
                            FT_UInt n_contours) {
  FT_Memory memory = loader->memory;
...
/*                                                                  */
BASE_FUNC(FT_Error)
FT_GlyphLoader_Check_Subglyphs(FT_GlyphLoader *loader, FT_UInt n_subs) {
  FT_Memory memory = loader->memory;
  FT_Error error = FT_Err_Ok;
  FT_UInt new_max;
  ...
  if (new_max > loader->max_subglyphs) {
    new_max = (new_max + 1) & -2;
    if (REALLOC_ARRAY(base->subglyphs, base->num_subglyphs, new_max,
                      FT_SubGlyph)) {
      goto Exit;
    }
...
BASE_FUNC(FT_Error)
FT_GlyphLoader_Copy_Points(FT_GlyphLoader *target, FT_GlyphLoader *source) {
  FT_Error error;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_EXPORT_FUNC(FT_Error) FT_New_GlyphSlot(FT_Face face, FT_GlyphSlot *aslot) {
  FT_Error error;
  FT_Driver driver;
...
FT_Load_Glyph(FT_Face face, FT_UInt glyph_index, FT_Int load_flags) {
  ...
  autohint = hinter && !(load_flags & (FT_LOAD_NO_SCALE | FT_LOAD_NO_HINTING));
  if (autohint) {
  ...
  if (face->transform_flags) {
    ...
    if (renderer) {
      error = renderer->clazz->transform_glyph(
          renderer, slot, &face->transform_matrix, &face->transform_delta);
    }
    /* transform advance */
  ...
  if (!error && slot->format != ft_glyph_format_bitmap &&
    ...
    error = FT_Render_Glyph(slot, (load_flags & FT_LOAD_MONOCHROME)
                                      ? ft_render_mode_mono
...
/*                                                                       */
FT_EXPORT_FUNC(FT_Error)
FT_Load_Char(FT_Face face, FT_ULong char_code, FT_Int load_flags) {
  FT_UInt glyph_index;
...
static void destroy_face(FT_Memory memory, FT_Face face, FT_Driver driver) {
  ...
  FT_List_Finalize(&face->sizes_list, (FT_List_Destructor)destroy_size, memory,
                   driver);
  face->size = 0;
...
/*                                                                       */
FT_EXPORT_FUNC(FT_Error)
FT_New_Memory_Face(FT_Library library, FT_Byte *file_base, FT_Long file_size,
...
/*                                                                       */
FT_EXPORT_FUNC(FT_Error)
FT_Open_Face(FT_Library library, FT_Open_Args *args, FT_Long face_index,
             FT_Face *aface) {
  FT_Error error;
  FT_Driver driver;
  FT_Memory memory;
  ...
  if (args->flags & ft_open_driver && args->driver) {
    ...
    for (; cur < limit; cur++) {
      ...
      if (FT_MODULE_IS_DRIVER(cur[0])) {
        ...
        error =
            open_face(driver, stream, face_index, num_params, params, &face);
        if (!error) {
          goto Success;
        }
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_EXPORT_FUNC(FT_Error) FT_New_Size(FT_Face face, FT_Size *asize) {
  FT_Error error;
  FT_Memory memory;
  FT_Driver driver;
  FT_Driver_Class *clazz;
...
static void ft_recompute_scaled_metrics(FT_Face face,
  ...
  metrics->descender =
      (FT_MulFix(face->descender, metrics->y_scale) + 32) & -64;

  metrics->height = (FT_MulFix(face->height, metrics->y_scale) + 32) & -64;

  metrics->max_advance =
      (FT_MulFix(face->max_advance_width, metrics->x_scale) + 32) & -64;
}
...
FT_Set_Char_Size(FT_Face face, FT_F26Dot6 char_width, FT_F26Dot6 char_height,
  ...
  if (clazz->set_char_sizes) {
    error = clazz->set_char_sizes(face->size, char_width, char_height,
...
FT_Get_Kerning(FT_Face face, FT_UInt left_glyph, FT_UInt right_glyph,
               FT_UInt kern_mode, FT_Vector *kerning) {
  FT_Error error = FT_Err_Ok;
  FT_Driver driver;
  FT_Memory memory;
  ...
  if (driver->clazz->get_kerning) {
    error = driver->clazz->get_kerning(face, left_glyph, right_glyph, kerning);
    if (!error) {
...
/*                                                                       */
FT_EXPORT_FUNC(FT_Error) FT_Select_Charmap(FT_Face face, FT_Encoding encoding) {
  FT_CharMap *cur;
  FT_CharMap *limit;
...
/*    table).                                                            */
/*                                                                       */
FT_EXPORT_FUNC(FT_Error) FT_Set_Charmap(FT_Face face, FT_CharMap charmap) {
  FT_CharMap *cur;
  FT_CharMap *limit;
...
/*                                                                       */
FT_EXPORT_FUNC(FT_UInt) FT_Get_Char_Index(FT_Face face, FT_ULong charcode) {
  FT_UInt result;
  FT_Driver driver;
...
/*                                                                       */
FT_EXPORT_FUNC(void *) FT_Get_Sfnt_Table(FT_Face face, FT_Sfnt_Tag tag) {
  void *table = 0;
...
LOCAL_FUNC
FT_Error FT_Render_Glyph_Internal(FT_Library library, FT_GlyphSlot slot,
                                  FT_UInt render_mode) {
  FT_Error error = FT_Err_Ok;
  FT_Renderer renderer;
...
/*                                                                       */
FT_EXPORT_FUNC(FT_Error)
FT_Render_Glyph(FT_GlyphSlot slot, FT_UInt render_mode) {
  FT_Library library;
