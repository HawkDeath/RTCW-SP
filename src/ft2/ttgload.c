...
/*                                                                       */
LOCAL_FUNC
void TT_Get_Metrics(TT_HoriHeader *header, FT_UInt index, FT_Short *bearing,
                    FT_UShort *advance) {
...
/*                                                                       */
static void Get_HMetrics(TT_Face face, FT_UInt index, FT_Bool check,
                         FT_Short *lsb, FT_UShort *aw) {
...
/*                                                                       */
static FT_Byte *Get_Advance_Widths(TT_Face face, FT_UShort ppem) {
  FT_UShort n;
...
static FT_Error TT_Access_Glyph_Frame(TT_Loader *loader, FT_UInt glyph_index,
                                      FT_ULong offset, FT_UInt byte_count) {
  FT_Error error;
...
static void TT_Forget_Glyph_Frame(TT_Loader *loader) {
  FT_Stream stream = loader->stream;

  FORGET_Frame();
}

static FT_Error TT_Load_Glyph_Header(TT_Loader *loader) {
  FT_Stream stream = loader->stream;
...
static FT_Error TT_Load_Simple_Glyph(TT_Loader *load) {
  FT_Error error;
...
static FT_Error TT_Load_Composite_Glyph(TT_Loader *loader) {
  FT_Error error;
  FT_Stream stream = loader->stream;
...
static FT_Error TT_Process_Simple_Glyph(TT_Loader *load, FT_Bool debug) {
  ...
  if (IS_HINTED(load->load_flags)) {
    ...
    if (n_ins > 0) {
      ...
      load->exec->pedantic_hinting =
          (FT_Bool)(load->load_flags & FT_LOAD_PEDANTIC);
      load->exec->pts = *zone;
  ...
  /* otherwise, load a composite! */
    ...
    /* Now, read each subglyph independently. */
    {
      ...
      if (num_subglyphs > 0 && loader->exec && ins_pos > 0 &&
        ...
        if (FILE_Seek(ins_pos) || READ_UShort(n_ins)) {
          goto Fail;
        }
        FT_TRACE5(("  Instructions size = %d\n", n_ins));
...
static void compute_glyph_metrics(TT_Loader *loader, FT_UInt glyph_index) {
  FT_BBox bbox;
  TT_Face face = (TT_Face)loader->face;
  FT_Fixed x_scale, y_scale;
...

  {
    ...
    if (face->vertical_info && face->vertical.number_Of_VMetrics > 0) {
      ...
      TT_Get_Metrics((TT_HoriHeader *)&face->vertical, glyph_index,
      ...
      if (face->os2.version != 0xFFFF) {
        ...
        advance_height =
            (FT_UShort)(face->os2.sTypoAscender - face->os2.sTypoDescender +
                        face->os2.sTypoLineGap);
      } else {
        top_bearing = face->horizontal.Line_Gap / 2;
        advance_height =
            (FT_UShort)(face->horizontal.Ascender + face->horizontal.Descender +
  ...
  if (!face->postscript.isFixedPitch && size && IS_HINTED(loader->load_flags)) {
    FT_Byte *widths = Get_Advance_Widths(face, size->root.metrics.x_ppem);
...
/*                                                                       */
LOCAL_FUNC
FT_Error TT_Load_Glyph(TT_Size size, TT_GlyphSlot glyph, FT_UShort glyph_index,
                       FT_UInt load_flags) {
  SFNT_Interface *sfnt;
  TT_Face face;
  FT_Stream stream;
  ...
  if (size && (load_flags & FT_LOAD_NO_BITMAP) == 0 && sfnt->load_sbits) {
    ...
    error = sfnt->load_sbit_image(face, size->root.metrics.x_ppem,
                                  size->root.metrics.y_ppem, glyph_index,
                                  load_flags, stream, &glyph->bitmap, &metrics);
    if (!error) {
