...
/*                                                                       */
LOCAL_FUNC
FT_Error TT_New_GlyphZone(FT_Memory memory, FT_UShort maxPoints,
                          FT_Short maxContours, TT_GlyphZone *zone) {
  FT_Error error;
...
/*                                                                       */
LOCAL_DEF
FT_Error TT_Init_Face(FT_Stream stream, TT_Face face, FT_Int face_index,
  ...
  error = TT_Load_Locations(face, stream) || TT_Load_CVT(face, stream) ||
...
FT_Error TT_Init_Size(TT_Size size) {
  ...
  if (ALLOC_ARRAY(size->function_defs, size->max_function_defs, TT_DefRecord) ||

      ALLOC_ARRAY(size->instruction_defs, size->max_instruction_defs,
                  TT_DefRecord) ||

      ALLOC_ARRAY(size->cvt, size->cvt_size, FT_Long) ||

      ALLOC_ARRAY(size->storage, size->storage_size, FT_Long)) {
...
FT_Error TT_Reset_Size(TT_Size size) {
  ...
  if (metrics->x_ppem >= metrics->y_ppem) {
    ...
    size->ttmetrics.y_ratio =
        FT_MulDiv(metrics->y_ppem, 0x10000L, metrics->x_ppem);
  } else {
    size->ttmetrics.scale = metrics->y_scale;
    size->ttmetrics.ppem = metrics->y_ppem;
    size->ttmetrics.x_ratio =
        FT_MulDiv(metrics->x_ppem, 0x10000L, metrics->y_ppem);
    size->ttmetrics.y_ratio = 0x10000L;
  }

  /* Compute root ascender, descender, test height, and max_advance */
  metrics->ascender =
      (FT_MulFix(face->root.ascender, metrics->y_scale) + 32) & -64;
  metrics->descender =
      (FT_MulFix(face->root.descender, metrics->y_scale) + 32) & -64;
  metrics->height = (FT_MulFix(face->root.height, metrics->y_scale) + 32) & -64;
  metrics->max_advance =
      (FT_MulFix(face->root.max_advance_width, metrics->x_scale) + 32) & -64;
