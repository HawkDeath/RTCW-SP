...
FT_Error ah_outline_load(AH_Outline *outline, FT_Face face) {
  ...
  if (num_contours > outline->max_contours) {
    ...
    if (REALLOC_ARRAY(outline->contours, outline->max_contours, new_contours,
                      AH_Point *)) {
      goto Exit;
    }
...
void ah_outline_compute_segments(AH_Outline *outline) {
  ...
  for (dimension = 1; dimension >= 0; dimension--) {
    ...
    for (; contour < contour_limit; contour++) {
      ...
      if (ABS(last->out_dir) == major_dir && ABS(point->out_dir) == major_dir) {
...
void ah_dump_segments(AH_Outline *outline) {
  ...
  for (dimension = 1; dimension >= 0; dimension--) {
    ...
    for (seg = segments; seg < limit; seg++) {
      printf(
          "  [ %5d | %4d | %5s | %4d | %5d | %4d | %5d | %5d ]\n",
          seg - segments, (int)seg->pos,
          seg->dir == ah_dir_up
              ? "up"
              : (seg->dir == ah_dir_down
                     ? "down"
          ...
          seg->serif ? (seg->serif - segments) : -1, (int)seg->num_linked,
          seg->first - points, seg->last - points);
    }
...
static void ah_outline_compute_edges(AH_Outline *outline) {
  ...
  for (dimension = 1; dimension >= 0; dimension--) {
    ...
    edge_distance_threshold =
        FT_MulFix(outline->edge_distance_threshold, scale);
...
void ah_dump_edges(AH_Outline *outline) {
  ...
  for (dimension = 1; dimension >= 0; dimension--) {
    ...
    for (edge = edges; edge < limit; edge++) {
      printf(
          "  [ %5d | %4d | %5s | %4d | %5d |  %c  | %5.2f | %5.2f ]\n",
          edge - edges, (int)edge->fpos,
          edge->dir == ah_dir_up
              ? "up"
          ...
          edge->serif ? (edge->serif - edges) : -1, edge->blue_edge ? 'y' : 'n',
          edge->opos / 64.0, edge->pos / 64.0);
    }
