...
static FT_Error Load_SBit_Range_Codes(TT_SBit_Range *range, FT_Stream stream,
  ...
  if (ALLOC_ARRAY(range->glyph_codes, count, FT_UShort) || ACCESS_Frame(size)) {
    goto Exit;
  }
  ...
  for (n = 0; n < count; n++) {
    ...
    if (load_offsets) {
      range->glyph_offsets[n] = (FT_ULong)range->image_offset + GET_UShort();
    }
  }
...
static FT_Error Load_SBit_Range(TT_SBit_Range *range, FT_Stream stream) {
  ...
  case 3: /* variable metrics with 2-byte offsets */
  {
    ...
    if (ALLOC_ARRAY(range->glyph_offsets, num_glyphs, FT_ULong) ||
        ACCESS_Frame(num_glyphs * size_elem)) {
      goto Exit;
    }

    for (n = 0; n < num_glyphs; n++)
      range->glyph_offsets[n] = (FT_ULong)(
          range->image_offset + (large ? GET_ULong() : GET_UShort()));
    FORGET_Frame();
  } break;
...
FT_Error TT_Load_SBit_Strikes(TT_Face face, FT_Stream stream) {
  ...
  const FT_Frame_Field strike_start_fields[] = {
      /* no FT_FRAME_START */
      FT_FRAME_ULONG(TT_SBit_Strike, ranges_offset), FT_FRAME_SKIP_LONG,
      FT_FRAME_ULONG(TT_SBit_Strike, num_ranges),
      FT_FRAME_ULONG(TT_SBit_Strike, color_ref), FT_FRAME_END};
  ...
  /* allocate the index ranges for each strike table */
  {
    ...
    while (count > 0) {
      ...
      if (ALLOC_ARRAY(strike->sbit_ranges, strike->num_ranges, TT_SBit_Range)) {
        goto Exit;
      }
      ...
      while (count2 > 0) {
        ...
        range->table_offset = table_base + strike->ranges_offset + GET_ULong();
        count2--;
        range++;
      }
      ...
      while (count2 > 0) {
        /* Read the header */
        if (FILE_Seek(range->table_offset) || ACCESS_Frame(8L)) {
          goto Exit;
        }
...
/*                                                                       */
static FT_Error Find_SBit_Image(TT_Face face, FT_UInt glyph_index,
                                FT_Int x_ppem, FT_Int y_ppem,
...
/*    function exit.                                                     */
/*                                                                       */
static FT_Error Load_SBit_Metrics(FT_Stream stream, TT_SBit_Range *range,
...
static FT_Error Load_SBit_Image(TT_SBit_Strike *strike, TT_SBit_Range *range,
  ...
  /* the array of elements.                                    */
  {
    ...
    for (; count > 0; count--, comp++) {
      ...
      error =
          Find_SBit_Range(comp->glyph_code, strike, &elem_range, &elem_offset);
      if (error) {
        goto Fail_Memory;
      }
      ...
      error = Load_SBit_Image(strike, elem_range, ebdt_pos, elem_offset, map,
                              ...
                              y_offset + comp->y_offset, stream, &elem_metrics);
      if (error) {
        goto Fail_Memory;
      }
    }
...
/*                                                                       */
LOCAL_FUNC
FT_Error TT_Load_SBit_Image(TT_Face face, FT_Int x_ppem, FT_Int y_ppem,
  ...
  error = Find_SBit_Image(face, glyph_index, x_ppem, y_ppem, &range, &strike,
                          &glyph_offset);
  if (error) {
    goto Exit;
  }
  ...
  error = Load_SBit_Image(strike, range, ebdt_pos, glyph_offset, map, 0, 0,
                          stream, metrics);
  if (error) {
    goto Exit;
  }
