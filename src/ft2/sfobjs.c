...
/*                                                                       */
static FT_String *Get_Name(TT_Face face, FT_UShort nameid) {
...
static FT_Encoding find_encoding(int platform_id, int encoding_id) {
...
FT_Error SFNT_Init_Face(FT_Stream stream, TT_Face face, FT_Int face_index,
  ...
  if (!face->psnames) {
    face->psnames =
        (PSNames_Interface *)FT_Get_Module_Interface(library, "psnames");
  }
...
FT_Error SFNT_Load_Face(FT_Stream stream, TT_Face face, FT_Int face_index,
  ...
  /* Load tables */
  if (LOAD_(header) || LOAD_(max_profile) ||
      ...
      LOAD_(charmaps) || LOAD_(names) || LOAD_(os2) || LOAD_(psnames)) {
    goto Exit;
  }
  ...
  if (LOAD_(hdmx) || LOAD_(gasp) || LOAD_(kerning) || LOAD_(pclt)) {
    goto Exit;
  }
  ...
  /* now set up root fields */
  {
    ...
    if (face->num_sbit_strikes) {
      ...
      if (ALLOC_ARRAY(root->available_sizes, face->num_sbit_strikes,
                      FT_Bitmap_Size)) {
        return error;
      }

      for (n = 0; n < face->num_sbit_strikes; n++) {
        root->available_sizes[n].width = face->sbit_strikes[n].x_ppem;
        root->available_sizes[n].height = face->sbit_strikes[n].y_ppem;
      }
    } else {
    ...
    root->max_advance_height =
        face->vertical_info ? face->vertical.advance_Height_Max : root->height;
