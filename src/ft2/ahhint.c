...
static void ah_align_linked_edge(AH_Hinter *hinter, AH_Edge *base_edge,
                                 AH_Edge *stem_edge, int vertical) {
...
static void ah_align_serif_edge(AH_Hinter *hinter, AH_Edge *base,
                                AH_Edge *serif) {
  FT_Pos dist;
  FT_Pos sign = 1;
  ...
  for (dimension = 1; dimension >= 0; dimension--) {
    ...
    for (edge = edges; edge < edge_limit; edge++) {
...

      {
        ...
        delta = 0;
        if (edge2 + 1 < edge_limit && edge2[1].flags & ah_edge_done) {
...
LOCAL_FUNC
void ah_hinter_hint_edges(AH_Hinter *hinter, int no_horz_edges,
                          int no_vert_edges) {
...
static void ah_iup_interp(AH_Point *p1, AH_Point *p2, AH_Point *ref1,
                          AH_Point *ref2) {
  AH_Point *p;
  FT_Pos u;
  FT_Pos v1 = ref1->v;
      ...
      if (point <= end_point) {
        ...
        if (cur_touched == first_touched) {
          ...
          if (first_touched > points) {
            ah_iup_interp(first_point, first_touched - 1, cur_touched,
                          first_touched);
          }
        }
      }
...
static void ah_hinter_scale_globals(AH_Hinter *hinter, FT_Fixed x_scale,
                                    FT_Fixed y_scale) {
  FT_Int n;
...
FT_Error ah_hinter_new_face_globals(AH_Hinter *hinter, FT_Face face,
                                    AH_Globals *globals) {
  FT_Error error;
  FT_Memory memory = hinter->memory;
...
static FT_Error ah_hinter_load(AH_Hinter *hinter, FT_UInt glyph_index,
                               FT_UInt load_flags, FT_UInt depth) {
  ...
  case ft_glyph_format_outline:
    ...
    /* width/positioning that occured during the hinting process  */
    {
      ...
      AH_Edge *edge2 = edge1 + outline->num_vedges - 1; /* rightmost edge */
...
FT_Error ah_hinter_load_glyph(AH_Hinter *hinter, FT_GlyphSlot slot,
