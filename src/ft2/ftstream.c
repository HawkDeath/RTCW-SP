...
BASE_FUNC(FT_Error) FT_Seek_Stream(FT_Stream stream, FT_ULong pos) {
  FT_Error error;
...
BASE_FUNC(FT_Error) FT_Skip_Stream(FT_Stream stream, FT_Long distance) {
  return FT_Seek_Stream(stream, (FT_ULong)(stream->pos + distance));
}

BASE_FUNC(FT_Long) FT_Stream_Pos(FT_Stream stream) { return stream->pos; }
...
BASE_FUNC(void) FT_Release_Frame(FT_Stream stream, FT_Byte **pbytes) {
  if (stream->read) {
...
BASE_FUNC(FT_Char) FT_Read_Char(FT_Stream stream, FT_Error *error) {
  FT_Byte result = 0;
...
BASE_FUNC(FT_Short) FT_Read_Short(FT_Stream stream, FT_Error *error) {
  FT_Byte reads[2];
  FT_Byte *p = 0;
  FT_Short result = 0;
...
BASE_FUNC(FT_Short) FT_Read_ShortLE(FT_Stream stream, FT_Error *error) {
  FT_Byte reads[2];
  FT_Byte *p = 0;
  FT_Short result = 0;
...
BASE_FUNC(FT_Long) FT_Read_Offset(FT_Stream stream, FT_Error *error) {
  FT_Byte reads[3];
  FT_Byte *p = 0;
  FT_Long result = 0;
...
BASE_FUNC(FT_Long) FT_Read_Long(FT_Stream stream, FT_Error *error) {
  FT_Byte reads[4];
  FT_Byte *p = 0;
  FT_Long result = 0;
...
BASE_FUNC(FT_Long) FT_Read_LongLE(FT_Stream stream, FT_Error *error) {
  FT_Byte reads[4];
  FT_Byte *p = 0;
  FT_Long result = 0;
...
BASE_FUNC(FT_Error)
FT_Read_Fields(FT_Stream stream, const FT_Frame_Field *fields,
               void *structure) {
  FT_Error error;
  ...
  do {
    ...
    case ft_frame_ulong_le: /* read a 4-byte little-endian long */
    {
      ...
      if (p + 3 < stream->limit) {
        value = (FT_ULong)p[0] | ((FT_ULong)p[1] << 8) |
                ((FT_ULong)p[2] << 16) | ((FT_ULong)p[3] << 24);
        stream->cursor += 4;
      }
      sign_shift = 0;
      break;
    ...
    case ft_frame_uoff3_le: /* read a 3-byte little-endian long */
    {
      ...
      if (p + 2 < stream->limit) {
        value = (FT_ULong)p[0] | ((FT_ULong)p[1] << 8) | ((FT_ULong)p[2] << 16);
        stream->cursor += 3;
      }
      sign_shift = 8;
      break;
