...
#define FT_FRAME_FIELD(frame_op, struct_type, field)                           \
  {                                                                            \
    frame_op, sizeof(FT_FIELD_REF(struct_type, field)),                        \
...
#define NEXT_Short(buffer)                                                     \
  (buffer += 2,                                                                \
   ((short)((signed char)buffer[-2] << 8) | (unsigned char)buffer[-1]))
...
#define NEXT_Offset(buffer)                                                    \
  (buffer += 3,                                                                \
   (((long)(signed char)buffer[-3] << 16) |                                    \
    ((long)(unsigned char)buffer[-2] << 8) | (long)(unsigned char)buffer[-1]))
...
#define NEXT_ShortLE(buffer)                                                   \
  (buffer += 2,                                                                \
   ((short)((signed char)buffer[-1] << 8) | (unsigned char)buffer[-2]))
...
#define NEXT_OffsetLE(buffer)                                                  \
  (buffer += 3,                                                                \
   (((long)(signed char)buffer[-1] << 16) |                                    \
    ((long)(unsigned char)buffer[-2] << 8) | (long)(unsigned char)buffer[-3]))
...
#define NEXT_LongLE(buffer)                                                    \
  (buffer += 4,                                                                \
   (((long)(signed char)buffer[-1] << 24) |                                    \
    ...
    ((long)(unsigned char)buffer[-3] << 8) | (long)(unsigned char)buffer[-4]))
...
BASE_DEF(FT_Error) FT_Seek_Stream(FT_Stream stream, FT_ULong pos);

BASE_DEF(FT_Error) FT_Skip_Stream(FT_Stream stream, FT_Long distance);
...
BASE_DEF(void) FT_Release_Frame(FT_Stream stream, FT_Byte **pbytes);
...
BASE_DEF(FT_Char) FT_Read_Char(FT_Stream stream, FT_Error *error);

BASE_DEF(FT_Short) FT_Read_Short(FT_Stream stream, FT_Error *error);

BASE_DEF(FT_Long) FT_Read_Offset(FT_Stream stream, FT_Error *error);

BASE_DEF(FT_Long) FT_Read_Long(FT_Stream stream, FT_Error *error);

BASE_DEF(FT_Short) FT_Read_ShortLE(FT_Stream stream, FT_Error *error);

BASE_DEF(FT_Long) FT_Read_LongLE(FT_Stream stream, FT_Error *error);

BASE_DEF(FT_Error)
FT_Read_Fields(FT_Stream stream, const FT_Frame_Field *fields, void *structure);

#define USE_Stream(resource, stream)                                           \
  FT_SET_ERROR(FT_Open_Stream(resource, stream))

#define DONE_Stream(stream) FT_Done_Stream(stream)

#define ACCESS_Frame(size) FT_SET_ERROR(FT_Access_Frame(stream, size))

#define FORGET_Frame() FT_Forget_Frame(stream)
...
#define FILE_Seek(position) FT_SET_ERROR(FT_Seek_Stream(stream, position))

#define FILE_Skip(distance) FT_SET_ERROR(FT_Skip_Stream(stream, distance))

#define FILE_Pos() FT_Stream_Pos(stream)

#define FILE_Read(buffer, count)                                               \
  FT_SET_ERROR(FT_Read_Stream(stream, (FT_Byte *)buffer, count))

#define FILE_Read_At(position, buffer, count)                                  \
  FT_SET_ERROR(FT_Read_Stream_At(stream, position, (FT_Byte *)buffer, count))
