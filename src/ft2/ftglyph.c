...
/*                                                                       */
FT_EXPORT_FUNC(void) FT_Matrix_Multiply(FT_Matrix *a, FT_Matrix *b) {
...
static FT_Error ft_bitmap_copy(FT_Memory memory, FT_Bitmap *source,
                               FT_Bitmap *target) {
  FT_Error error;
...
static FT_Error ft_bitmap_glyph_init(FT_BitmapGlyph glyph, FT_GlyphSlot slot) {
...
static FT_Error ft_bitmap_glyph_copy(FT_BitmapGlyph source,
...
static void ft_bitmap_glyph_done(FT_BitmapGlyph glyph) {
  FT_Memory memory = FT_GLYPH(glyph)->library->memory;

  FREE(glyph->bitmap.buffer);
}

static void ft_bitmap_glyph_bbox(FT_BitmapGlyph glyph, FT_BBox *cbox) {
...
static FT_Error ft_outline_glyph_init(FT_OutlineGlyph glyph,
                                      FT_GlyphSlot slot) {
  FT_Error error = FT_Err_Ok;
  FT_Library library = FT_GLYPH(glyph)->library;
  ...
  MEM_Copy(target->tags, source->tags, source->n_points * sizeof(FT_Byte));
...
static void ft_outline_glyph_done(FT_OutlineGlyph glyph) {
  FT_Outline_Done(FT_GLYPH(glyph)->library, &glyph->outline);
}

static FT_Error ft_outline_glyph_copy(FT_OutlineGlyph source,
...
static void ft_outline_glyph_transform(FT_OutlineGlyph glyph, FT_Matrix *matrix,
                                       FT_Vector *delta) {
  if (matrix) {
...
static void ft_outline_glyph_bbox(FT_OutlineGlyph glyph, FT_BBox *bbox) {
  FT_Outline_Get_CBox(&glyph->outline, bbox);
}

static FT_Error ft_outline_glyph_prepare(FT_OutlineGlyph glyph,
                                         FT_GlyphSlot slot) {
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_EXPORT_FUNC(FT_Error) FT_Glyph_Copy(FT_Glyph source, FT_Glyph *target) {
  FT_Glyph copy;
  FT_Error error;
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
FT_EXPORT_FUNC(FT_Error) FT_Get_Glyph(FT_GlyphSlot slot, FT_Glyph *aglyph) {
...
FT_Glyph_To_Bitmap(FT_Glyph *the_glyph, FT_ULong render_mode, FT_Vector *origin,
  ...
  error =
      ft_new_glyph(glyph->library, &ft_bitmap_glyph_class, (FT_Glyph *)&bitmap);
  if (error) {
    goto Exit;
  }
