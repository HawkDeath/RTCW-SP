...
static void optim_log(const char *fmt, ...) {
  va_list ap;
...
static void AH_Dump_Stems(AH_Optimizer *optimizer) {
  int n;
  AH_Stem *stem;
  ...
  for (n = 0; n < optimizer->num_stems; n++, stem++) {
    LOG((" %c%2d [%.1f:%.1f]={%.1f:%.1f}="
         ...
         optimizer->vertical ? 'V' : 'H', n, FLOAT(stem->edge1->opos),
         FLOAT(stem->edge2->opos), FLOAT(stem->edge1->pos),
         FLOAT(stem->edge2->pos), FLOAT(stem->min_pos), FLOAT(stem->max_pos),
         FLOAT(stem->force), FLOAT(stem->velocity)));
  }
}

static void AH_Dump_Stems2(AH_Optimizer *optimizer) {
  int n;
  AH_Stem *stem;
  ...
  for (n = 0; n < optimizer->num_stems; n++, stem++) {
    LOG((" %c%2d [%.1f]=<%1.f..%1.f> force=%.1f speed=%.1f\n",
         optimizer->vertical ? 'V' : 'H', n, FLOAT(stem->pos),
         FLOAT(stem->min_pos), FLOAT(stem->max_pos), FLOAT(stem->force),
         FLOAT(stem->velocity)));
  }
}

static void AH_Dump_Springs(AH_Optimizer *optimizer) {
  int n;
  AH_Spring *spring;
  AH_Stem *stems;
  ...
  for (n = 0; n < optimizer->num_springs; n++, spring++) {
    LOG((
        " [%d-%d:%.1f:%1.f:%.1f]", spring->stem1 - stems, spring->stem2 - stems,
        FLOAT(spring->owidth),
...
static int valid_stem_segments(AH_Segment *seg1, AH_Segment *seg2) {
  return seg1->serif == 0 && seg2 && seg2->link == seg1 &&
         seg1->pos < seg2->pos && seg1->min_coord <= seg2->max_coord &&
         seg2->min_coord <= seg1->max_coord;
}

/* compute all stems in an outline */
static int optim_compute_stems(AH_Optimizer *optimizer) {
...
static FT_Pos stem_spring_area(AH_Stem *stem1, AH_Stem *stem2) {
...
static int optim_compute_springs(AH_Optimizer *optimizer) {
...
static void optim_compute_tensions(AH_Optimizer *optimizer) {
...
static int optim_compute_stem_movements(AH_Optimizer *optimizer) {
...
static FT_Pos optim_compute_distortion(AH_Optimizer *optimizer) {
  AH_Spring *spring = optimizer->springs;
  AH_Spring *limit = spring + optimizer->num_springs;
  FT_Pos distortion = 0;
...
static void optim_record_configuration(AH_Optimizer *optimizer) {
  FT_Pos distortion;
...
static void optim_compute(AH_Optimizer *optimizer) {
  int n;
  FT_Bool moved;
...
int AH_Optimizer_Init(AH_Optimizer *optimizer, AH_Outline *outline,
                      FT_Memory memory) {
  FT_Error error;
  ...
  error = optim_compute_stems(optimizer) || optim_compute_springs(optimizer);
  if (error) {
    goto Fail;
  }
