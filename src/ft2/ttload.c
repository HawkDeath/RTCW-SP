...
/*                                                                       */
LOCAL_FUNC
FT_Error TT_Goto_Table(TT_Face face, FT_ULong tag, FT_Stream stream,
                       FT_ULong *length) {
  TT_Table *table;
  FT_Error error;
...
/*                                                                       */
LOCAL_FUNC
FT_Error TT_Load_SFNT_Header(TT_Face face, FT_Stream stream, FT_Long face_index,
                             SFNT_Header *sfnt) {
  FT_Error error;
  FT_ULong format_tag;
  ...
  const FT_Frame_Field ttc_header_fields[] = {
      FT_FRAME_START(8), FT_FRAME_LONG(TTC_Header, version),
      FT_FRAME_LONG(TTC_Header, count), FT_FRAME_END};
  ...
  if (format_tag == TTAG_ttcf) {
    ...
    if (ALLOC_ARRAY(face->ttc_header.offsets, face->ttc_header.count,
                    FT_ULong) ||
  ...
  /* now, check the values of `num_tables', `seach_range', etc. */
  {
    ...
    /*                                                                  */
    if (num_tables == 0 || entry_selector > num_tables ||
...
FT_Error TT_Load_Directory(TT_Face face, FT_Stream stream, SFNT_Header *sfnt) {
  ...
  if (ALLOC_ARRAY(face->dir_tables, face->num_tables, TT_Table)) {
    goto Exit;
  }
  ...
  for (; entry < limit;
       entry++) { /* loop through the tables and get all entries */
    entry->Tag = GET_Tag4();
    ...
    FT_TRACE2(("  %c%c%c%c  -  %08lx  -  %08lx\n", (FT_Char)(entry->Tag >> 24),
               (FT_Char)(entry->Tag >> 16), (FT_Char)(entry->Tag >> 8),
               (FT_Char)(entry->Tag), entry->Offset, entry->Length));
  }
...
/*                                                                       */
LOCAL_FUNC
FT_Error TT_Load_Any(TT_Face face, FT_ULong tag, FT_Long offset,
...
/*    FreeType error code.  0 means success.                             */
/*                                                                       */
static FT_Error TT_Load_Metrics(TT_Face face, FT_Stream stream,
                                FT_Bool vertical) {
  FT_Error error;
  FT_Memory memory = stream->memory;
  ...
  if (num_shorts < 0) {
    ...
    error =
        vertical ? TT_Err_Invalid_Vert_Metrics : TT_Err_Invalid_Horiz_Metrics;
    goto Exit;
  }
...
/*                                                                       */
LOCAL_FUNC
FT_Error TT_Load_Metrics_Header(TT_Face face, FT_Stream stream,
                                FT_Bool vertical) {
  FT_Error error;
...
FT_Error TT_Load_Names(TT_Face face, FT_Stream stream) {
  ...
  const FT_Frame_Field name_table_fields[] = {
      FT_FRAME_START(6), FT_FRAME_USHORT(TT_NameTable, format),
      FT_FRAME_USHORT(TT_NameTable, numNameRecords),
      FT_FRAME_USHORT(TT_NameTable, storageOffset), FT_FRAME_END};
  ...
  if (ALLOC_ARRAY(names->names, names->numNameRecords, TT_NameRec) ||
  ...
  if (storageSize > 0) {
    ...
    if (ALLOC(names->storage, storageSize) ||
        FILE_Read_At(table_pos + names->storageOffset, names->storage,
                     storageSize)) {
      goto Exit;
    }
    ...
    /* Print Name Record Table in case of debugging */
    {
      ...
      for (; cur < limit; cur++) {
        ...
        FT_TRACE3(("%d %d %x %d\n  ", cur->platformID, cur->encodingID,
                   cur->languageID, cur->nameID));
...
FT_Error TT_Load_CMap(TT_Face face, FT_Stream stream) {
  ...
  const FT_Frame_Field cmap_fields[] = {
      FT_FRAME_START(4), FT_FRAME_USHORT(TT_CMapDir, tableVersionNumber),
      FT_FRAME_USHORT(TT_CMapDir, numCMaps), FT_FRAME_END};

  const FT_Frame_Field cmap_rec_fields[] = {
      FT_FRAME_START(6), FT_FRAME_USHORT(TT_CMapTable, format),
      FT_FRAME_USHORT(TT_CMapTable, length),
      FT_FRAME_USHORT(TT_CMapTable, version), FT_FRAME_END};
  ...
  if (ALLOC_ARRAY(face->charmaps, cmap_dir.numCMaps, TT_CharMapRec)) {
    goto Exit;
  }
...
FT_Error TT_Load_OS2(TT_Face face, FT_Stream stream) {
  ...
  const FT_Frame_Field os2_fields_extra[] = {
      FT_FRAME_START(8), FT_FRAME_ULONG(TT_OS2, ulCodePageRange1),
      FT_FRAME_ULONG(TT_OS2, ulCodePageRange2), FT_FRAME_END};
...
FT_Error TT_Load_Gasp(TT_Face face, FT_Stream stream) {
  ...
  for (j = 0; j < num_ranges; j++) {
    ...
    FT_TRACE3(
        (" [max:%d flag:%d]", gaspranges[j].maxPPEM, gaspranges[j].gaspFlag));
  }
  FT_TRACE3(("\n"));
...
FT_Error TT_Load_Hdmx(TT_Face face, FT_Stream stream) {
...

  {
    ...
    for (; cur < limit; cur++) {
      /* read record */
      if (READ_Byte(cur->ppem) || READ_Byte(cur->max_width)) {
        goto Exit;
      }
