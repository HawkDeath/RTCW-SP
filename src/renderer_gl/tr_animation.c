...
static int R_CullModel(mdsHeader_t *header, trRefEntity_t *ent) {
  ...
  if (ent->e.nonNormalizedAxes) {
    cullSphere = qfalse; // by defalut, cull bounding sphere ONLY if this is not
                         // an upscaled entity
...
=================
*/
float R_CalcMDSLod(refEntity_t *refent, vec3_t origin, float radius,
                   float modelBias, float modelScale) {
  float flod, lodScale;
  ...
  if (projectedRadius != 0) {
    ...
    lodScale = r_lodscale->value; // fudge factor since MDS uses a much smoother
                                  // method of LOD
...
void R_AddAnimSurfaces(trRefEntity_t *ent) {
  ...
  for (i = 0; i < header->numSurfaces; i++) {
    ...
    //----(SA)	blink will change to be an overlay rather than replacing the
    //head texture. 		think of it like batman's mask.  the polygons that have eye
    //texture are duplicated 		and the 'lids' rendered with polygonoffset over the
    //top of the open eyes.  this gives 		minimal overdraw/alpha blending/texture
    //use without breaking the model and causing seams
...
==============
*/
void R_CalcBoneLerp(mdsHeader_t *header, const refEntity_t *refent,
                    int boneNum) {
  int j;
...
==============
*/
void R_CalcBones(mdsHeader_t *header, const refEntity_t *refent, int *boneList,
                 int numBones) {
...
===============
*/
void R_RecursiveBoneListAdd(int bi, int *boneList, int *numBones,
