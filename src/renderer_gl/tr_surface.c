...
void RB_AddQuadStampFadingCornersExt(vec3_t origin, vec3_t left, vec3_t up,
  ...
  tess.texCoords[ndx + 4][0][0] = tess.texCoords[ndx + 4][1][0] =
      (s1 + s2) / 2.0;
  tess.texCoords[ndx + 4][0][1] = tess.texCoords[ndx + 4][1][1] =
      (t1 + t2) / 2.0;

  // center uses full alpha
  *(unsigned int *)&tess.vertexColors[ndx + 4] = *(unsigned int *)color;
...
static void VectorArrayNormalize(vec4_t *normals, unsigned int count) {
...
#if idppc
  {
    ...
    // instruction, runs *much* faster than calling sqrt().  We'll use a single
    // Newton-Raphson refinement step to get a little more precision.  This
    // seems to yeild results that are correct to 3 decimal places and usually
    // correct to at least 4 (sometimes 5). (That is, for the given input range
    // of about 0.6 to 2.0).
    do {
      float x, y, z;
      float B, y0, y1;
