...
void R_DlightBmodel(bmodel_t *bmodel) {
  ...
  // int)
  //		meaning, shouldn't ->needDlights be changed to an int rather than a
  //qbool?
...
void R_SetupEntityLighting(const trRefdef_t *refdef, trRefEntity_t *ent) {
  ...
  for (i = 0; i < refdef->num_dlights; i++) {
    ...
    if (dl->dlshader) { //----(SA)	if the dlight has a diff shader
                        //specified, you don't know what it does, so don't let
                        //it affect entities lighting
      continue;
    }
