...
void RB_BeginDrawingView(void) {
  ...
  if (r_uiFullScreen->integer) {
    ...
    if (backEnd.refdef.rdflags &
      ...
      if (r_fastsky->integer ||
        ...
        if (glfogsettings[FOG_PORTALVIEW].registered) {
          ...
          //clear for testing portal sky clear
      ...
      if (glfogNum > FOG_NONE && glfogsettings[FOG_CURRENT].registered) {
        ...
        } else if (!(r_portalsky->integer)) { // portal skies have been manually
    ...
    // (SA) well, this is silly then
                 ...
                 ->integer) { //  || backEnd.refdef.rdflags & RDF_NOWORLDMODEL
    ...
    } else { // world scene, no portal sky, not fastsky, clear color if fog says
             // to, otherwise, just set the clearcolor
      if (glfogsettings[FOG_CURRENT]
              .registered) { // try to clear fastsky with current fog color
        qglClearColor(glfogsettings[FOG_CURRENT].color[0],
                      glfogsettings[FOG_CURRENT].color[1],
...
#define ZOMBIEFX_HIT_MAXRANGE_SQR                                              \
  36 // each bullet that strikes the bounding box, will effect verts inside this
     // range (allowing for projections onto the mesh)
...
typedef struct {
  ...
  unsigned short vertHits[ZOMBIEFX_MAX_HITS]; // bit flags to represent those
                                              // verts that have been hit
  int numNewHits;
  vec3_t newHitPos[ZOMBIEFX_MAX_NEWHITS];
  vec3_t newHitDir[ZOMBIEFX_MAX_NEWHITS];
} trZombieFleshHitverts_t;
//
trZombieFleshHitverts_t zombieFleshHitVerts[MAX_SP_CLIENTS]
                                           [2]; // one for upper, one for lower

void RB_ZombieFXInit(void) {
  memset(zombieFleshHitVerts, 0, sizeof(zombieFleshHitVerts));
}

void RB_ZombieFXAddNewHit(int entityNum, const vec3_t hitPos,
                          const vec3_t hitDir) {
  int part = 0;
  ...
  if (entityNum >= MAX_SP_CLIENTS) {
    Com_Printf(
        "RB_ZombieFXAddNewHit: entityNum (%i) outside allowable range (%i)\n",
        entityNum, MAX_SP_CLIENTS);
    return;
  }
  if (zombieFleshHitVerts[entityNum][part].numHits +
  ...
  // add it to the list
  VectorCopy(hitPos,
             zombieFleshHitVerts[entityNum][part]
                 .newHitPos[zombieFleshHitVerts[entityNum][part].numNewHits]);
  VectorCopy(hitDir,
             zombieFleshHitVerts[entityNum][part]
                 .newHitDir[zombieFleshHitVerts[entityNum][part].numNewHits]);
  zombieFleshHitVerts[entityNum][part].numNewHits++;
}

void RB_ZombieFXProcessNewHits(trZombieFleshHitverts_t *fleshHitVerts,
                               int oldNumVerts, int numSurfVerts) {
  ...
  byte hitCounts[ZOMBIEFX_MAX_VERTS]; // so we can quickly tell if a particular
                                      // vert has been hit enough times already
  ...
  for (i = 0, hitTrav = fleshHitVerts->vertHits; i < fleshHitVerts->numHits;
       i++, hitTrav++) {
    hitCounts[*hitTrav]++;
  }
  ...
  for (i = 0; i < fleshHitVerts->numNewHits; i++) {
    ...
    // for each vertex
    for (j = 0, bestHitDist = -1, xyzTrav = tess.xyz[oldNumVerts],
        normTrav = tess.normal[oldNumVerts];
         j < numSurfVerts; j++, xyzTrav += 4, normTrav += 4) {
      ...
      // if we are still struggling to find a hit, then pick the closest outside
      // the OK range
      if (!foundHit) {
        if (thisDist < ZOMBIEFX_HIT_MAXRANGE_SQR &&
            (bestHitDist < 0 || thisDist < bestHitDist)) {
    ...
    if (!foundHit && bestHitDist >= 0) {
      // set the appropriate bit-flag
      fleshHitVerts->isHit = qtrue;
      fleshHitVerts->vertHits[fleshHitVerts->numHits++] =
          (unsigned short)bestHit;
      if (fleshHitVerts->numHits == ZOMBIEFX_MAX_HITS) {
        break;
      }
    }
...
void RB_ZombieFXShowFleshHits(trZombieFleshHitverts_t *fleshHitVerts,
                              int oldNumVerts, int numSurfVerts) {
  byte *vertColors;
...
void RB_ZombieFXDecompose(int oldNumVerts, int numSurfVerts,
...
void RB_ZombieFX(int part, drawSurf_t *drawSurf, int oldNumVerts,
                 int oldNumIndex) {
  int numSurfVerts;
  float deltaTime;
  char *surfName;
  ...
  // Central point for Zombie post-tess processing. Various effects can be added
  // from this point
  ...
  fleshHitVerts =
      &zombieFleshHitVerts[backEnd.currentEntity->e.entityNum][part];
  ...
  if (!Q_stricmp(surfName, zombieFxFleshHitSurfaceNames[part])) {
    ...
    // check for any new bullet impacts that need to be scanned for triangle
    // collisions
  ...
  // decompose?
  if (deltaTime) {
    RB_ZombieFXDecompose(oldNumVerts, numSurfVerts,
                         deltaTime / ZOMBIEFX_FADEOUT_TIME_SEC);
  }
}
...
void RB_RenderDrawSurfList(drawSurf_t *drawSurfs, int numDrawSurfs) {
  ...
  for (i = 0, drawSurf = drawSurfs; i < numDrawSurfs; i++, drawSurf++) {
    if (drawSurf->sort == oldSort) {
      ...
      /*
                              // RF, convert the newly created vertexes into
         dust particles, and overwrite if (backEnd.currentEntity->e.reFlags &
         REFLAG_ZOMBIEFX) { RB_ZombieFX( 0, drawSurf, oldNumVerts, oldNumIndex
         );
                              }
                              else if (backEnd.currentEntity->e.reFlags &
         REFLAG_ZOMBIEFX2) { RB_ZombieFX( 1, drawSurf, oldNumVerts, oldNumIndex
         );
                              }
      */
      continue;
...
const void *RB_StretchPicGradient(const void *data) {
  ...
  //		*(int *)tess.vertexColors[ numVerts + 1 ] =
  //		*(int *)tess.vertexColors[ numVerts + 2 ] =
  //		*(int *)tess.vertexColors[ numVerts + 3 ] = *(int
  //*)backEnd.color2D;
