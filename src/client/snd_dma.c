...
cut off only by the same sound on this channel
        SND_OKTOCUT			0x001	- allow sound to be cut off by
any following sounds on this channel
...
====================
*/
void S_ThreadStartSoundEx(vec3_t origin, int entityNum, int entchannel,
                          sfxHandle_t sfxHandle, int flags);

void S_StartSoundEx(vec3_t origin, int entityNum, int entchannel,
                    sfxHandle_t sfxHandle, int flags) {
  ...
  if (!sfxHandle &&
      entchannel !=
          CHAN_WEAPON) { // let null weapon sounds try to play.  they kill any
                         // weapon sounds playing when a guy dies
    return;
  }
...
void S_ThreadStartSoundEx(vec3_t origin, int entityNum, int entchannel,
  ...
  for (i = 0; i < MAX_CHANNELS; i++) {
    if (s_channels[i].entnum == entityNum && s_channels[i].thesfx &&
      ...
      if (entityNum < MAX_CLIENTS && s_channels[i].entchannel != CHAN_AUTO &&
          s_channels[i].entchannel != CHAN_WEAPON) {
        S_ChannelFree(&s_channels[i]);
        continue;
      }
...
==============
*/
void S_StartSound(vec3_t origin, int entityNum, int entchannel,
...
void S_StopAllSounds(void) {
  ...
  for (i = 0; i < MAX_STREAMING_SOUNDS;
...
==================
*/
void S_ClearLoopingSounds(void) { snd.numLoopSounds = 0; }
...
void S_AddLoopingSound(int entityNum, const vec3_t origin,
                       const vec3_t velocity, const int range,
...
void S_AddLoopSounds(void) {
  ...
  for (i = 0; i < snd.numLoopSounds; i++) {
    ...
    ch->thesfx = loop->sfx;
    // RF, disabled doppler for looping sounds for now, since we are reverting
    // to the old looping sound code
    ch->doppler = qfalse;
...
============
*/
portable_samplepair_t *S_GetRawSamplePointer() { return s_rawsamples[0]; }
...
void S_Update_Mix(void) {
  ...
  /*
     ...
     snd.pushPop[i].flags ); } else { S_ThreadStartSoundEx(NULL,
     snd.pushPop[i].entityNum, snd.pushPop[i].entityChannel, snd.pushPop[i].sfx,
     snd.pushPop[i].flags );
                  }
          }
...
void S_StartBackgroundTrack(const char *intro, const char *loop,
  ...
  if (fadeupTime) {
    ...
    //		ss->fadeEnd			= s_paintedtime + (((float)(ss->info.rate)/1000.0f )
    //* fadeupTime);
...
======================
*/
void S_StartStreamingSound(const char *intro, const char *loop, int entnum,
...
======================
*/
void S_StopBackgroundTrack(void) { S_StopStreamingSound(0); }
...
void S_UpdateStreamingSounds(void) {
  ...
  soundMixAheadTime = s_soundtime; // + (int)(0.35 * dma.speed);	// allow for
                                   // talking animations
      ...
      if (!ss->samples) { // at the end of the sound
        ...
        if (i == 0 && snd.nextMusicTrackType) { // queued music is queued
          if (ss->file) {
            ...
            numStreamingSounds--;
            //						memset( &s_rawsamples[i], 0,
            //MAX_RAW_SAMPLES*sizeof(portable_samplepair_t) );	// really clear
            //it
          ...
          /*
             ...
             {
                                                          S_StartBackgroundTrack(
             nextMusicVA, ss->name, 0);		// play once, then go back to
             looping what's currently playing } else {	// QUEUED_PLAY_LOOPED
                                                          S_StartBackgroundTrack(
          ...
          */
          ...
          if (ss->loop && ss->loop[0]) {
            ...
            ss->kill = 1;
            if (i == 0) {
              Cvar_Set("s_currentMusic", ""); //----(SA)	so the savegame
                                              //know's it's supposed to be quiet
