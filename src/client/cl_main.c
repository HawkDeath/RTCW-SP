...
Called by Com_Error when a game has ended and is dropping out to main menu in
the "endgame" menu ('credits' right now)
==============
*/
...
void CL_PlayDemo_f(void) {
  ...
  //	CL_FlushMemory();	//----(SA)	MEM NOTE: in missionpack, this is
  //moved to CL_DownloadsComplete
...
void CL_MapLoading(void) {
  ...
  //	S_StartBackgroundTrack( "sound/music/l_briefing_1.wav", "", -2);	//
  //'-2' for 'queue looping track' (QUEUED_PLAY_LOOPED)
...
void CL_RequestMotd(void) {
 ...
 // Info_SetValueForKey(info, "renderer", cls.renderConfig.renderer_string);
...
=================
*/
void CL_ResetPureClientAtServer(void) { CL_AddReliableCommand(va("vdr")); }
...
static void CL_Cache_MapChange_f(void) { cacheIndex++; }
...
void CL_InitRenderer(void) {
  ...
  re.BeginRegistration(&cls.renderConfig);
  ...
  g_console_field_width = cls.renderConfig.vidWidth / SMALLCHAR_WIDTH - 2;
...
TTimo
  show_bug.cgi?id=447
  only supporting "open" syntax for URL openings, others are not portable or
need to be added on a case-by-case basis the shellExecute syntax as been kept to
remain compatible with win32 SP demo pk3, but this thing only does open URL
...
void CL_ShellExecute_URL_f(void) {
  ...
  if (Q_stricmp(Cmd_Argv(1), "open")) {
    Com_DPrintf("invalid CL_ShellExecute_URL_f syntax (shellExecute \"open\" "
                "<url> <doExit>)\n");
    return;
  }
...
void CL_Init(void) {
  ...
  Cvar_Get("model", "bj2",
           CVAR_USERINFO |
               CVAR_ARCHIVE); // temp until we have an skeletal american model
  ...
  // Cmd_AddCommand ( "shellExecute", CL_ShellExecute_f );	//----(SA) added
  // (mainly for opening web pages from the menu)
