...
void Con_DrawSolidConsole(float frac) {
  ...
  lines = cls.renderConfig.vidHeight * frac;
  if (lines <= 0) {
    return;
  }

  if (lines > cls.renderConfig.vidHeight) {
    lines = cls.renderConfig.vidHeight;
  }
  ...
  if (y < 1) {
    ...
    if (frac >=
        0.5f) { // only draw when the console is down all the way (for now)
  ...
  for (x = 0; x < i; x++) {
    ...
    SCR_DrawSmallChar(cls.renderConfig.vidWidth - (i - x) * SMALLCHAR_WIDTH,
...
void Con_DrawConsole(void) {
  ...
  case CA_LOADING: // only during cgame initialization, never during main loop
    if (!con_debug->integer) { // these are all 'no console at all' when
                               // con_debug is not set
      return;
    }
  ...
  case CA_CINEMATIC: // playing a cinematic or a static pic, not connected to a
                     // server
  default:
    break;
  }
