...
typedef struct {
  ...
  int cgameUserHoldableValue; // current holdable item to add to usercmd_t
                              // //----(SA)	added
  ...
  qboolean cameraMode; //----(SA)	added for control of input while
                       //watching cinematics
...
typedef struct {
  ...
  int lastExecutedServerCommand; // last server command grabbed or executed with
                                 // CL_GetServerCommand
  ...
  qboolean downloadRestart; // if true, we need to do another FS_Restart because
                            // we downloaded a pak
