...
extern qboolean getCameraInfo(int camNum, int time, vec3_t *origin,
...
==============
*/
void CL_SetUserCmdValue(int userCmdValue, int holdableValue,
                        float sensitivityScale, int cld) {
...
==============
*/
void CL_CgameError(const char *string) { Com_Error(ERR_DROP, "%s", string); }
...
int CL_CgameSystemCalls(int *args) {
  ...
  case CG_UPDATESCREEN:
    ...
    //		Com_EventLoop();	// FIXME: if a server restarts here, BAD THINGS
    //HAPPEN!
  ...
  case CG_SETUSERCMDVALUE:
    CL_SetUserCmdValue(args[1], args[2], VMF(3),
                       args[4]); //----(SA)	modified	// NERVE - SMF -
                                 //added fourth arg [cld]
    return 0;
...
void CL_InitCGame(void) {
  ...
  //	cgvm = VM_Create( "cgame", CL_CgameSystemCalls, Cvar_VariableValue(
  //"vm_cgame" ) );
  if (!cgvm) {
