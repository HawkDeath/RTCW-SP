...
void S_AdpcmDecode(const char indata[], short *outdata, int len,
                   struct adpcm_state *state) {
  // LordHavoc: removed 4-clause BSD code for Intel ADPCM codec
}
...
void S_AdpcmGetSamples(sndBuffer *chunk, short *to) {
  ...
  // get samples
  S_AdpcmDecode((const char *)out, to, SND_CHUNK_SIZE_BYTE * 2,
                &state); // DAJ added (const char*)
}
...
void S_AdpcmEncodeSound(sfx_t *sfx, short *samples) {
  ...
  while (count) {
    ...
    // encode the samples
    S_AdpcmEncode(samples + inOffset, (char *)out, n,
                  &state); // DAJ added (char*)
