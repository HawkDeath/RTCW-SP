...
typedef struct {
  ...
  qboolean looping, holdAtEnd, dirty, alterGameState, silent, shader, letterBox,
      sound;
  fileHandle_t iFile;
  e_status status;
...
//				size =	number of bytes of input (= # of shorts of
//output) 				signedOutput = 0 for unsigned output, non-zero for signed output 				flag
//= flags from asset header
//
...
// Parameters:	from -> buffer holding encoded data
//				to ->	buffer to hold decoded data
//				size =	number of bytes of input (= 1/4 # of bytes
...
// Parameters:	from -> buffer holding encoded data
//				to ->	buffer to hold decoded data
//				size =	number of bytes of input (= 1/2 # of bytes
//of output) 				signedOutput = 0 for unsigned output, non-zero for signed output
//				flag = flags from asset header
//
// Returns:		Number of samples placed in output buffer
...
// Parameters:	from -> buffer holding encoded data
//				to ->	buffer to hold decoded data
//				size =	number of bytes of input (= # of bytes of
//output) 				signedOutput = 0 for unsigned output, non-zero for signed output 				flag
//= flags from asset header
//
// Returns:		Number of samples placed in output buffer
...
e_status CIN_RunCinematic(int handle) {
  ...
  cinTable[currentHandle].tfps =
      (((CL_ScaledMilliseconds() - cinTable[currentHandle].startTime) *
        cinTable[currentHandle].roqFPS) /
       1000);
  ...
  if (played && cinTable[currentHandle].sound) {
    if (s_rawend[CIN_STREAM] < s_soundtime &&
        (s_soundtime - s_rawend[CIN_STREAM]) < 100) {
...
void CIN_DrawCinematic(int handle) {
  ...
  if (cinTable[handle].letterBox) {
    ...
    //		re.DrawStretchPic( 0, 0, SCREEN_WIDTH, LETTERBOX_OFFSET, 0, 0, 0, 0,
    //cls.whiteShader ); 		re.DrawStretchPic( 0, SCREEN_HEIGHT-LETTERBOX_OFFSET,
    //SCREEN_WIDTH, LETTERBOX_OFFSET, 0, 0, 0, 0, cls.whiteShader );
    //----(SA)	adjust for 640x480
    re.DrawStretchPic(0, 0, w, barheight, 0, 0, 0, 0, cls.whiteShader);
    re.DrawStretchPic(0, vh - barheight - 1, w, barheight + 1, 0, 0, 0, 0,
                      cls.whiteShader);
  }
