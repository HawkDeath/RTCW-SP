...
#define BOTAREA_JIGGLE_DIST 32
  ...
  // Ridah, jiggle them around to look for a fuzzy area, helps LARGE characters
  // reach destinations that are against walls
  ofs[2] = 10;
  for (ofs[0] = -BOTAREA_JIGGLE_DIST; ofs[0] <= BOTAREA_JIGGLE_DIST;
       ofs[0] += BOTAREA_JIGGLE_DIST * 2) {
    for (ofs[1] = -BOTAREA_JIGGLE_DIST; ofs[1] <= BOTAREA_JIGGLE_DIST;
         ofs[1] += BOTAREA_JIGGLE_DIST * 2) {
...
void BotUpdateInventory(bot_state_t *bs) {
  ...
  bs->inventory[INVENTORY_MAUSER] =
      COM_BitCheck(bs->cur_ps.weapons, (WP_MAUSER));
  bs->inventory[INVENTORY_MP40] = COM_BitCheck(bs->cur_ps.weapons, (WP_MP40));
  bs->inventory[INVENTORY_GRENADELAUNCHER] =
      COM_BitCheck(bs->cur_ps.weapons, (WP_GRENADE_LAUNCHER));
  bs->inventory[INVENTORY_VENOM] = COM_BitCheck(bs->cur_ps.weapons, (WP_VENOM));
  bs->inventory[INVENTORY_FLAMETHROWER] =
      COM_BitCheck(bs->cur_ps.weapons, (WP_FLAMETHROWER));
  bs->inventory[INVENTORY_GAUNTLET] =
      COM_BitCheck(bs->cur_ps.weapons, (WP_GAUNTLET));

  // ammo
  bs->inventory[INVENTORY_9MM] = bs->cur_ps.ammo[BG_FindAmmoForWeapon(WP_MP40)];
  bs->inventory[INVENTORY_792MM] =
      bs->cur_ps.ammo[BG_FindAmmoForWeapon(WP_MAUSER)];
  bs->inventory[INVENTORY_GRENADES] =
      bs->cur_ps.ammo[BG_FindAmmoForWeapon(WP_GRENADE_LAUNCHER)];
  bs->inventory[INVENTORY_127MM] =
      bs->cur_ps.ammo[BG_FindAmmoForWeapon(WP_VENOM)];
  bs->inventory[INVENTORY_FUEL] =
      bs->cur_ps.ammo[BG_FindAmmoForWeapon(WP_FLAMETHROWER)];
...
==================
*/
int TeamPlayIsOn(void) { return (gametype == GT_TEAM || gametype == GT_CTF); }
  ...
  for (n = 0; n < 10; n++) {
    ...
    if (len > 200) {
      ...
      if (!trace.startsolid) {
        ...
        if (!(pc & CONTENTS_LAVA)) { //----(SA)	modified since slime is no
                                     //longer deadly
...
int BotFindEnemy(bot_state_t *bs, int curenemy) {
  ...
  for (i = 0; i < MAX_CLIENTS; i++) {
    ...
    if (curenemy < 0 && dist > 200 && !healthdecrease &&
...
void BotAimAtEnemy(bot_state_t *bs) {
  ...
  //----(SA) if we're not using this routine at all and my changes are
  //irrelivant, please let me know. 	if (wi.number == WP_MACHINEGUN) {
  //		aim_accuracy = trap_Characteristic_BFloat(bs->character,
  ...
  if (wi.number == WP_SILENCER) {
    aim_accuracy = trap_Characteristic_BFloat(
        bs->character, CHARACTERISTIC_AIM_ACCURACY_SP5, 0, 1);
    aim_skill = trap_Characteristic_BFloat(bs->character,
                                           CHARACTERISTIC_AIM_SKILL_SP5, 0, 1);
  }
  //	if (wi.number == WP_BFG) {
  //		aim_accuracy = trap_Characteristic_BFloat(bs->character,
  //CHARACTERISTIC_AIM_ACCURACY_BFG10K, 0, 1); 		aim_skill =
  //trap_Characteristic_BFloat(bs->character, CHARACTERISTIC_AIM_SKILL_BFG10K,
  //0, 1);
  //	}
  //
  if (aim_accuracy <= 0) {
  ...
  //
  if (wi.number == WP_FLAMETHROWER) {
    //	if (wi.number == WP_MACHINEGUN ||	//----(SA)	removing old weapon
    //references 		wi.number == WP_SHOTGUN || 		wi.number == WP_RAILGUN) { distance
    // towards the enemy
    dist = VectorLength(dir);
    if (dist > 150) {
      dist = 150;
...
void BotCheckAttack(bot_state_t *bs) {
  ...
  BotAI_Trace(&trace, start, mins, maxs, end, bs->entitynum,
              MASK_SHOT); //----(SA) should this maybe check the weapon type and
                          //adjust the clipflag?  it seems like this is probably
                          //fine as-is, but I thought I'd note it.
    ...
    if (wi.proj.damagetype & DAMAGETYPE_RADIAL) {
      if (trace.fraction * 1000 < wi.proj.radius) {
        ...
        if (points > 0) {
          ...
          //CHARACTERISTIC_SELFPRESERVATION, 0, 1); 					if (random() <
          //selfpreservation) return;
          return;
        }
      }
...
int BotEntityToActivate(int entitynum) {
  ...
  BotAI_Print(PRT_ERROR,
              "BotEntityToActivate: unknown activator with classname \"%s\"\n",
              classname);
  return 0;
}
...
//activating entities\n", netname);
    ...
    if (!strcmp(classname, "func_door"))
    {
            ...
            VectorClear(angles);
            AAS_BSPModelMinsMaxsOrigin(modelindex - 1, angles, mins, maxs,
    NULL);
    ...
    if (!strcmp(classname, "func_button")) {
      ...
      if (health) {
        ...
        //
        // AAS_ClearShownDebugLines();
        // AAS_DrawArrow(bs->origin, goalorigin, LINECOLOR_BLUE,
        // LINECOLOR_YELLOW);
        //
        VectorSubtract(goalorigin, bs->origin, movedir);
        vectoangles(movedir, moveresult->ideal_viewangles);
        ...
        if (i < numareas) {
          ...
          if (bs->activatemessage_time < trap_AAS_Time()) {
            Com_sprintf(
                buf, sizeof(buf),
                "I have to activate a button at %1.1f %1.1f %1.1f in area %d\n",
    ...
    if (!strcmp(classname, "trigger_multiple"))
    {
            ...
            model = AAS_ValueForBSPEpairKey(ent, "model");
            modelindex = AAS_IndexFromModel(model);
            //if the model is not precached (bad thing but happens) assume model
is "*X" if (!modelindex) modelindex = atoi(model+1); VectorClear(angles);
            AAS_BSPModelMinsMaxsOrigin(modelindex - 1, angles, mins, maxs,
NULL); VectorAdd(mins, maxs, mid); VectorScale(mid, 0.5, mid); VectorCopy(mid,
start); start[2] = maxs[2] + 24; VectorSet(end, start[0], start[1], start[2] -
100); trace = AAS_TraceClientBBox(start, end, PRESENCE_CROUCH, -1); if
(trace.startsolid) return;
            ...
            {
                    Com_sprintf(buf, sizeof(buf), "I have to activate a trigger
at %1.1f %1.1f %1.1f in area %d\n", goalorigin[0], goalorigin[1], goalorigin[2],
AAS_PointAreaNum(goalorigin)); EA_Say(bs->client, buf); bs->activatemessage_time
= AAS_Time() + 5; } //end if* / #endif //OBSTACLEDEBUG
            //
            ...
            {
#ifdef OBSTACLEDEBUG
                    botimport.Print(PRT_MESSAGE, "trigger area has no
reachabilities\n"); #endif //OBSTACLEDEBUG if (bs->ainode == AINode_Seek_NBG)
bs->nbg_time = 0; else if (bs->ainode == AINode_Seek_LTG) bs->ltg_time = 0; }
//end if else
            {
                    AIEnter_Seek_ActivateEntity(bs);
            } //end else
