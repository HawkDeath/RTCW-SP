...
void SV_InitReliableCommandsForClient(client_t *cl, int commands) {
  ...
  cl->reliableCommands.commandLengths =
      Z_Malloc(commands * sizeof(*cl->reliableCommands.commandLengths));
  cl->reliableCommands.commands =
      Z_Malloc(commands * sizeof(*cl->reliableCommands.commands));
  //
...
void SV_InitReliableCommands(client_t *clients) {
  ...
  if (sv_gametype->integer == GT_SINGLE_PLAYER) {
    ...
    for (i = 1, cl = &clients[1]; i < sv_maxclients->integer; i++, cl++) {
      SV_InitReliableCommandsForClient(cl,
                                       MAX_RELIABLE_COMMANDS); // TODO, make 0's
    }
  } else {
    // multiplayer
...
qboolean SV_AddReliableCommand(client_t *cl, int index, const char *cmd) {
  ...
  length = strlen(cmd);
  //
  if ((cl->reliableCommands.rover - cl->reliableCommands.buf) + length + 1 >=
      cl->reliableCommands.bufSize) {
  ...
  if (i) {
    ...
    for (i = 0, ch = cl->reliableCommands.buf; i < cl->reliableCommands.bufSize;
         i++, ch++) {
      if (!*ch && (!i || !*(ch - 1))) { // make sure we dont start at the
                                        // terminator of another string
        // see if this is the start of a valid segment
        for (ch2 = ch, j = 0;
             i < cl->reliableCommands.bufSize - 1 && j < length + 1 && !*ch2;
             i++, ch2++, j++) {
          // loop
        }
        //
...
void SV_FreeAcknowledgedReliableCommands(client_t *cl) {
  ...
  while (cl->reliableCommands.commands[ack]) {
    // clear the string
    memset(cl->reliableCommands.commands[ack], 0,
           cl->reliableCommands.commandLengths[ack]);
    // clear the pointer
...
void SV_ChangeMaxClients(void) {
  ...
  Z_Free(svs.clients);
#else
  free(svs.clients); // RF, avoid trying to allocate large chunk on a fragmented
                     // zone
#endif
  ...
  if (oldMaxClients < sv_maxclients->integer) {
    if (sv_gametype->integer == GT_SINGLE_PLAYER) {
      ...
      for (i = oldMaxClients; i < sv_maxclients->integer; i++) {
        SV_InitReliableCommandsForClient(&svs.clients[i],
...
void SV_SpawnServer(char *server, qboolean killBots) {
  ...
  // Ridah, enforce maxclients in single player, so there is enough room for AI
  // characters
  {
    ...
    if (!g_gameskill) {
      g_gameskill =
          Cvar_Get("g_gameskill", "2",
                   CVAR_SERVERINFO | CVAR_LATCH |
                       CVAR_ARCHIVE); // (SA) new default '2' (was '1')
    }
    // done

    if (!g_gametype) {
      g_gametype = Cvar_Get("g_gametype", "0",
  ...
  if (sv_gametype->integer == GT_SINGLE_PLAYER) {
    ...
    // just set it to a negative number,so the cgame knows not to draw the
    // percent bar
    Cvar_Set("com_expectedhunkusage", "-1");
  }
      ...
      if (svs.clients[i].netchan.remoteAddress.type == NA_BOT) {
        if (killBots || Cvar_VariableValue("g_gametype") == GT_SINGLE_PLAYER) {
          SV_DropClient(&svs.clients[i],
                        " gametype is Single Player"); // DAJ added message
          continue;
        }
        isBot = qtrue;
      } else {
...
void SV_Init(void) {
  ...
  //----(SA)	heh, whoops.  we've been talking to id masters since we got a
  //connection... 	sv_master[0] = Cvar_Get ("sv_master1",
  //"master3.idsoftware.com", 0 );
