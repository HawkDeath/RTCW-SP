...
static void SV_UserMove(client_t *cl, msg_t *msg, qboolean delta) {
  ...
  key ^= Com_HashKey(SV_GetReliableCommand(cl, cl->reliableAcknowledge &
  ...
  for (i = 0; i < cmdCount; i++) {
    ...
    if (sv_gametype->integer !=
        GT_SINGLE_PLAYER) { // RF, we need to allow this in single player, where
                            // loadgame's can cause the player to freeze after
                            // reloading if we do this check
