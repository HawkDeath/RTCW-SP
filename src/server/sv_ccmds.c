...
#ifdef __linux__
        Com_Error(ERR_DROP,
                  "Unable to save game.\n\nPlease check that you have at least "
                  "5mb free of disk space in your home directory.");
#else
        Com_Error(ERR_DROP, "Insufficient free disk space.\n\nPlease free at "
                            "least 5mb of free space on game drive.");
#endif
        return;
      }
    }
...
static void SV_MapRestart_f(void) {
  ...
  if (Cmd_Argc() > 1) {
    ...
    if (sv_gametype->integer ==
        GT_SINGLE_PLAYER) { // (SA) no pause by default in sp
      delay = 0;
    } else {
      delay = 5;
    }
...
void SV_LoadGame_f(void) {
  ...
  if (!strstr(filename, ".") ||
      Q_strncmp(strstr(filename, ".") + 1, "svg", 3)) {
