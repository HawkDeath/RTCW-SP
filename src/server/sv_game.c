...
qboolean SV_EntityContact(const vec3_t mins, const vec3_t maxs,
                          const sharedEntity_t *gEnt, const int capsule) {
...
int SV_GameSystemCalls(int *args) {
  ...
  case BOTLIB_AAS_RT_GETHIDEPOS:
    return botlib_export->aas.AAS_RT_GetHidePos(
        VMA(1), args[2], args[3], VMA(4), args[5], args[6], VMA(7));
  ...
  case BOTLIB_AAS_GETROUTEFIRSTVISPOS:
    return botlib_export->aas.AAS_GetRouteFirstVisPos(VMA(1), VMA(2), args[3],
                                                      VMA(4));
...
===================
*/
qboolean SV_GetModelInfo(int clientNum, char *modelName,
