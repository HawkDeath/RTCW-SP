...
void SV_AddServerCommand(client_t *client, const char *cmd) {
  ...
  if (client->reliableSequence - client->reliableAcknowledge ==
    ...
    for (i = client->reliableAcknowledge + 1; i <= client->reliableSequence;
         i++) {
      // Com_Printf( "cmd %5d: %s\n", i, client->reliableCommands[ i &
      // (MAX_RELIABLE_COMMANDS-1) ] );
      Com_Printf(
          "cmd %5d: %s\n", i,
          SV_GetReliableCommand(client, i & (MAX_RELIABLE_COMMANDS - 1)));
    }
    Com_Printf("cmd %5d: %s\n", i, cmd);
  ...
  // Q_strncpyz( client->reliableCommands[ index ], cmd, sizeof(
  // client->reliableCommands[ index ] ) );
