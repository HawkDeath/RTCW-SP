...
int SV_BotAllocateClient(void) {
  ...
  for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++) {
    // Wolfenstein, never use the first slot, otherwise if a bot connects before
    // the first client on a listen server, game won't start
    if (i < 1) {
      continue;
    }
    // done.
...
void BotDrawDebugPolygons(void (*drawPoly)(int color, int numPoints,
  ...
  if (bot_debug->integer) {
    ...
    botlib_export->BotLibVarSet("bot_testroutevispos",
                                bot_testroutevispos->string);
...
==================
*/
void BotImport_Trace(bsp_trace_t *bsptrace, vec3_t start, vec3_t mins,
...
==================
*/
void BotImport_EntityTrace(bsp_trace_t *bsptrace, vec3_t start, vec3_t mins,
...
==================
*/
int BotImport_inPVS(vec3_t p1, vec3_t p2) { return SV_inPVS(p1, p2); }
...
==================
*/
void BotImport_BSPModelMinsMaxsOrigin(int modelnum, vec3_t angles,
...
==================
*/
void *BotImport_GetMemory(int size) { return malloc(size); }
...
==================
*/
void BotImport_FreeMemory(void *ptr) { free(ptr); }
...
==================
*/
void BotImport_DebugPolygonShow(int id, int color, int numPoints,
                                vec3_t *points) {
  bot_debugpoly_t *poly;
...
==================
*/
void BotImport_DebugPolygonDelete(int id) { debugpolygons[id].inuse = qfalse; }
...
==================
*/
void BotImport_DebugLineDelete(int line) { BotImport_DebugPolygonDelete(line); }
...
void SV_BotInitCvars(void) {
  ...
  Cvar_Get("bot_reachability", "0", 0); // show all reachabilities to other
                                        // areas
...
qboolean BotImport_AICast_VisibleFromPos(vec3_t srcpos, int srcnum,
  ...
  return VM_Call(gvm, AICAST_VISIBLEFROMPOS, (int)srcpos, srcnum, (int)destpos,
                 destnum, updateVisPos);
}
...
===============
*/
qboolean BotImport_AICast_CheckAttackAtPos(int entnum, int enemy, vec3_t pos,
                                           qboolean ducking,
                                           qboolean allowHitWorld) {
  return VM_Call(gvm, AICAST_CHECKATTACKATPOS, entnum, enemy, (int)pos, ducking,
                 allowHitWorld);
}
// done.
