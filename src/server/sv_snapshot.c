...
static void SV_AddEntitiesVisibleFromPoint(
    ...
    //clientSnapshot_t *oldframe, qboolean localClient ) {
    //									snapshotEntityNumbers_t *eNums, qboolean portal )
    //{
  ...
  for (e = 0; e < sv.num_entities; e++) {
    ...
    if ((playerEnt->s.eFlags & EF_VIEWING_CAMERA) && !portal) {
      if (ent->r.svFlags & SVF_PORTAL) {
        ...
        //				SV_AddEntitiesVisibleFromPoint(
        //ent->s.origin2, frame, eNums, qtrue, oldframe, localClient );
  ...
  notVisible:
    ...
    // whether we can see it or not DHM - Nerve :: not in multiplayer please
...
static void SV_BuildClientSnapshot(client_t *client) {
  ...
  //	// try to use a previous frame as the source for delta compressing the
  //snapshot 	if ( client->deltaMessage <= 0 || client->state != CS_ACTIVE ) {
  //		// client is asking for a retransmit
  //		oldframe = NULL;
  //	} else if ( client->netchan.outgoingSequence - client->deltaMessage
  //		>= (PACKET_BACKUP - 3) ) {
  //		// client hasn't gotten a good message through in a long time
  //		Com_DPrintf ("%s: Delta request from out of date packet.\n",
  //client->name); 		oldframe = NULL; 	} else {
  //		// we have a valid snapshot to delta from
  //		oldframe = &client->frames[ client->deltaMessage & PACKET_MASK
  //];
  //
  //		// the snapshot's entities may still have rolled off the buffer,
  //though 		if ( oldframe->first_entity <= svs.nextSnapshotEntities -
  //svs.numSnapshotEntities ) { 			Com_DPrintf ("%s: Delta request from out of date
  //entities.\n", client->name); 			oldframe = NULL;
  //		}
  //	}
  ...
  SV_AddEntitiesVisibleFromPoint(org, frame, &entityNumbers, qfalse,
                                 client->netchan.remoteAddress.type ==
                                     NA_LOOPBACK);
  //	SV_AddEntitiesVisibleFromPoint( org, frame, &entityNumbers, qfalse,
  //oldframe, client->netchan.remoteAddress.type == NA_LOOPBACK );
