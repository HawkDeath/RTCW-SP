...
typedef struct flare_s {
  ...
  int flags;
  // for coronas, the client determines current visibility, but it's still
  // inserted so it will fade out properly
...
*/
void RB_AddFlare(
    void *surface, int fogNum, vec3_t point, vec3_t color, float scale,
    vec3_t normal, int id,
    int flags) { //----(SA)	added scale. added id.  added visible
  int i;
  flare_t *f, *oldest;
  vec3_t local;
  float d;
...
void RB_RenderFlare(flare_t *f) {
  ...
  //			additive blends and have them fog correctly with our distance
  //fog. 		/when/ we fix the blend problems with distance fog, this should 		be
  //changed back to additive since there's nearly no hit for that 		but the alpha
  //blend is noticably slower.
  ...
  tess.vertexColors[tess.numVertexes][3] =
      f->drawIntensity *
      255; //----(SA)	mod for alpha blend rather than additive
           //	tess.vertexColors[tess.numVertexes][3] = 255;		//----(SA)	mod
           //for alpha blend rather than additive
  tess.numVertexes++;
