...
typedef struct corona_s {
  ...
  int flags; // '1' is 'visible'
             // still send the corona request, even if not visible, for proper
             // fading
} corona_t;
...
extern cvar_t *r_picmip2; // controls picmip values for designated (character
                          // skin) textures
...
image_t *R_CreateImageExt(const char *name, const byte *pic, int width,
                          ...
                          qboolean characterMip, int wrapClampMode);
//----(SA)	end
...
void RB_AddQuadStampFadingCornersExt(vec3_t origin, vec3_t left, vec3_t up,
                                     byte *color, float s1, float t1, float s2,
                                     float t2);
...
void RB_AddFlare(void *surface, int fogNum, vec3_t point, vec3_t color,
                 float scale, vec3_t normal, int id,
                 int flags); // TTimo updated prototype
...
int R_MarkFragments(int orientation, const vec3_t *points,
...
void RE_AddPolyToScene(qhandle_t hShader, int numVerts,
                       const polyVert_t *verts);
// Ridah
void RE_AddPolysToScene(qhandle_t hShader, int numVerts,
...
void RE_StretchPicGradient(float x, float y, float w, float h, float s1,
...
extern void R_SetFog(int fogvar, int var1, int var2, float r, float g, float b,
                     float density);
