...
void RB_CalcDeformVertexes(deformStage_t *ds) {
  ...
  if (ds->deformationWave.frequency < 0) {
    ...
    VectorScale(worldUp,
                0.4 + 0.6 * fabs(backEnd.currentEntity->e.fireRiseDir[2]),
                worldUp);
...
void RB_CalcFogTexCoords(float *st) {
  ...
  if (fog->hasSurface) {
    ...
    eyeT =
        DotProduct(backEnd.or.viewOrigin, fogDepthVector) + fogDepthVector[3];
  } else {
  ...
  for (i = 0, v = tess.xyz[0]; i < tess.numVertexes; i++, v += 4) {
    ...
    if (eyeOutside) {
      if (t < 1.0) {
        ...
        t = 1.0 / 32 +
            30.0 / 32 * t / (t - eyeT); // cut the distance at the fog plane
      }
    } else {
      if (t < 0) {
        t = 1.0 / 32; // point is outside, so no fogging
...
void RB_CalcTurbulentTexCoords(const waveForm_t *wf, float *st) {
  ...
  for (i = 0; i < tess.numVertexes; i++, st += 2) {
    ...
    st[0] = s + tr.sinTable[((int)(((tess.xyz[i][0] + tess.xyz[i][2]) * 1.0 /
                                        128 * 0.125 +
                                    now) *
                                   FUNCTABLE_SIZE)) &
                            (FUNCTABLE_MASK)] *
                    wf->amplitude;
    st[1] = t + tr.sinTable[((int)((tess.xyz[i][1] * 1.0 / 128 * 0.125 + now) *
                                   FUNCTABLE_SIZE)) &
                            (FUNCTABLE_MASK)] *
                    wf->amplitude;
  }
