...
static void ProjectDlightTexture(void) {
  ...
  for (l = 0; l < backEnd.refdef.num_dlights; l++) {
    ...
    for (i = 0; i < tess.numVertexes; i++, texCoords += 2, colors += 4) {
      ...
      //				if( DotProduct( tess.normal[i], dir) <
      //0) { 					clipBits[i] = 255;	// not lighted (backface) 					continue;
      //				}
      //			}
    ...
    for (i = 0; i < tess.numIndexes; i += 3) {
      ...
      //); 				VectorNormalize(dir); 				if( DotProduct( tess.normal[i], dir) < 0) {
      //					continue;	// not lighted
      //(backface)
      //				}
      //			}
    ...
    //----(SA) creating dlight shader to allow for special blends or alternate
    //dlight texture
    {
      shader_t *dls = dl->dlshader;
      if (dls) {
        //				if (!qglActiveTextureARB ||
        //dls->numUnfoggedPasses < 2) {
        ...
        /*
           ...
           | GLS_DEPTHFUNC_EQUAL);
                                                        ...
                                                        // setup each TMU
                                                        for (tmu=0;
           tmu<glConfig.maxActiveTextures && i<dls->numUnfoggedPasses; tmu++,
           i++) {
           ...
           GL_TEXTURE_2D );
                                                                ...
                                                                R_BindAnimatedImage(
           &dls->stages[i]->bundle[0] );
                                                        }

                                                        // draw the elements
                                                        R_DrawElements(
           numIndexes, hitIndexes ); backEnd.pc.c_totalIndexes += numIndexes;
                                                        backEnd.pc.c_dlightIndexes
           += numIndexes;
                                                }

                                                // turn off unused TMU's
                                                for (tmu=1;
           tmu<glConfig.maxActiveTextures; tmu++) {
                                                        // set back to default
           state GL_SelectTexture( tmu ); qglDisable( GL_TEXTURE_2D );
                                                }
        ...
        */
        ...
        /*
           ...
           multitexture
                                                ...
                                                GL_State( GLS_SRCBLEND_DST_COLOR
           | GLS_DSTBLEND_ONE | GLS_DEPTHFUNC_EQUAL );

                                                // setup each TMU (use all
           available TMU's) for (tmu=0; tmu<glConfig.maxActiveTextures &&
           tmu<(dl->overdraw+1); tmu++) { GL_SelectTexture( tmu ); if (tmu) {
                                                                qglEnable(
           GL_TEXTURE_2D ); GL_TexEnv( GL_ADD ); GL_State(
           GLS_SRCBLEND_DST_COLOR | GLS_DSTBLEND_ONE | GLS_DEPTHFUNC_EQUAL );
                                                                qglEnableClientState(
           GL_TEXTURE_COORD_ARRAY ); qglTexCoordPointer( 2, GL_FLOAT, 0,
           texCoordsArray[0] ); qglEnableClientState( GL_COLOR_ARRAY );
                                                                qglColorPointer(
           4, GL_UNSIGNED_BYTE, 0, colorArray );
                                                ...
                                                // draw each bundle
                                                for(i=0; i<(dl->overdraw+1);
           i+=glConfig.maxActiveTextures)
                                                {
                                                        // make sure we dont
           draw with too many TMU's if
           (i+glConfig.maxActiveTextures>(dl->overdraw+1)) { for (tmu=0;
           tmu<glConfig.maxActiveTextures; tmu++) { if (tmu+i>=(dl->overdraw+1))
           { GL_SelectTexture( tmu ); qglDisable( GL_TEXTURE_2D );
                                                                        }
                                                                }
                                                        }
                                                        // draw the elements
...
void RB_StageIteratorGeneric(void) {
  ...
  // This must be done first, since we can't change the arrays once they have
  // been locked
  if (qglPNTrianglesiATI && tess.ATI_tess) {
...
#endif
    qglEnableClientState(
        GL_NORMAL_ARRAY); // RF< so we can send the normals as an array
    qglNormalPointer(GL_FLOAT, 16, input->normal);
  }
