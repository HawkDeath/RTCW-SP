...
=================
*/
int R_OldMarkFragments(int numPoints, const vec3_t *points,
...
int R_MarkFragments(int orientation, const vec3_t *points,
  ...
  // vec3_t			bestCenter;	// center point projected onto the closest
  // surface
  float texCoordScale;
  // float			dot;
  ...
  if (!oldMapping) {
    ...
    surf->plane.dist; if (!bestdist) { if (dot < 0) bestdist = fabs(dot) + 1000;
    // avoid this surface, since the point is behind it else bestdist = dot;
                                            VectorCopy( surf->plane.normal,
    bestnormal ); VectorMA( center, -dot, surf->plane.normal, bestCenter ); }
    else if (dot >= 0 && dot < bestdist) { bestdist = dot; VectorCopy(
    surf->plane.normal, bestnormal ); VectorMA( center, -dot,
    surf->plane.normal, bestCenter );
                                    }
                            }
                    }
                    // bestCenter is now the real center
                    VectorCopy( bestCenter, center );
    Com_Printf("bestnormal: %1.1f %1.1f %1.1f \n", bestnormal[0], bestnormal[1],
    bestnormal[2] );
    */
  ...
  for (i = 0; i < numsurfaces; i++) {
    ...
    if (*surfaces[i] == SF_GRID) {
      ...
      if (!oldMapping) {
        ...
        // check the normal of this face
        if (dot < -epsilon &&
            DotProduct(surf->plane.normal, projectionDir) >= 0.01) {
          continue;
