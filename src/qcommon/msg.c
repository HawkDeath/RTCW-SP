...
void MSG_WriteDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from,
  ...
  if (from->angles[0] == to->angles[0] && from->angles[1] == to->angles[1] &&
      ...
      from->weapon == to->weapon && from->holdable == to->holdable &&
      from->wolfkick == to->wolfkick && from->cld == to->cld) { // NERVE - SMF
  ...
  MSG_WriteDeltaKey(msg, key, from->cld, to->cld,
                    16); // NERVE - SMF - for multiplayer clientDamage
}
...
void MSG_ReadDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from,
  ...
  if (MSG_ReadBits(msg, 1)) {
    ...
    to->cld = MSG_ReadDeltaKey(
        msg, key, from->cld, 16); // NERVE - SMF - for multiplayer clientDamage
  } else {
...
changeVectorLog_t changeVectorLog[MAX_CHANGE_VECTOR_LOGS] = {
    {0,
     {0x08, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00}}, // 723 uses in test
    {0,
     {0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0x00, 0x00, 0x00,
      0x00}}, // 285 uses in test
    {0,
     {0xe1, 0x00, 0xc0, 0x01, 0x80, 0x00, 0x00, 0x10, 0x00,
      0x00}}, // 235 uses in test
    {0,
     {0xe1, 0x00, 0xc0, 0x01, 0x20, 0x40, 0x00, 0x00, 0x00,
      0x00}}, // 162 uses in test
    {0,
     {0x28, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00}}, // 161 uses in test
    {0,
     {0x00, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00,
      0x00}}, // 139 uses in test
    {0,
     {0x01, 0x00, 0x00, 0x00, 0x80, 0x11, 0x00, 0x00, 0x00,
      0x00}}, // 92 uses in test
    {0,
     {0x03, 0x00, 0x00, 0x00, 0x80, 0x11, 0x00, 0x00, 0x00,
      0x00}}, // 78 uses in test
    {0,
     {0xe3, 0x00, 0xf0, 0x8f, 0x03, 0x00, 0x00, 0x10, 0x00,
      0x00}}, // 54 uses in test
    {0,
     {0xe1, 0x00, 0xf0, 0x89, 0x03, 0x00, 0x00, 0x00, 0x00,
      0x00}}, // 49 uses in test
    {0,
     {0xe1, 0x80, 0xc0, 0x21, 0x00, 0x11, 0x00, 0x20, 0x00,
      0x00}}, // 40 uses in test
    {0,
     {0x03, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00,
      0x00}}, // 37 uses in test
    {0,
     {0xe9, 0x30, 0xc0, 0x01, 0x00, 0x11, 0x00, 0x20, 0x00,
      0x00}}, // 35 uses in test
    {0,
     {0xe1, 0x80, 0xc0, 0x21, 0x10, 0x01, 0x00, 0x00, 0x00,
      0x00}}, // 30 uses in test
    {0,
     {0xe1, 0x00, 0xc0, 0x01, 0x10, 0x01, 0x00, 0x00, 0x00,
      0x00}}, // 29 uses in test
    {0,
     {0xe3, 0x00, 0x00, 0x00, 0x00, 0x08, 0x00, 0x00, 0x00,
      0x00}}, // 26 uses in test
    {0,
     {0xe1, 0x00, 0xc0, 0x01, 0x00, 0x40, 0x00, 0x00, 0x00,
      0x00}}, // 20 uses in test
    {0,
     {0xe0, 0x00, 0xc0, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00}}, // 19 uses in test
    {0,
     {0xe1, 0x80, 0xc0, 0xa1, 0x03, 0x01, 0x00, 0x00, 0x00,
      0x00}}, // 19 uses in test
    {0,
     {0x11, 0x00, 0x00, 0x00, 0x00, 0x11, 0x00, 0x00, 0x00,
      0x00}}, // 17 uses in test
    {0,
     {0x00, 0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00,
      0x00}}, // 16 uses in test
    {0,
     {0xe0, 0x00, 0xc0, 0x01, 0x40, 0x00, 0x00, 0x00, 0x00,
      0x00}}, // 15 uses in test
    {0,
     {0x28, 0x00, 0x00, 0x00, 0x00, 0x20, 0x00, 0x00, 0x00,
      0x00}}, // 14 uses in test
    {0,
     {0xe3, 0x00, 0xc0, 0xc1, 0x03, 0x04, 0x00, 0x10, 0x00,
      0x00}}, // 14 uses in test
    {0,
     {0xe1, 0x80, 0xc0, 0x21, 0x00, 0x01, 0x00, 0x00, 0x00,
      0x00}}, // 12 uses in test
    {0,
     {0x19, 0x10, 0x00, 0x00, 0x00, 0x11, 0x00, 0x20, 0x00,
      0x00}}, // 12 uses in test
    {0,
     {0x68, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
      0x00}}, // 11 uses in test
    {0,
     {0xe1, 0x00, 0xc0, 0xc1, 0x03, 0x04, 0x00, 0x10, 0x00,
      0x00}}, // 10 uses in test
    {0,
     {0xe1, 0x80, 0xc0, 0x21, 0x10, 0x05, 0x00, 0x00, 0x00,
      0x00}}, // 9 uses in test
    {0,
     {0xe1, 0x00, 0xc0, 0x01, 0x20, 0x40, 0x00, 0x10, 0x00,
      0x00}}, // 9 uses in test
    {0,
     {0xe1, 0x80, 0xc0, 0x21, 0x00, 0x11, 0x00, 0x00, 0x00,
      0x00}}, // 9 uses in test
    {0,
     {0x28, 0x00, 0x00, 0x00, 0x00, 0xa0, 0x00, 0x00, 0x00,
      0x00}}, // 8 uses in test
};
...
void MSG_ReportChangeVectors_f(void) {
  ...
  Com_Printf("%i%% of vectors compressed\n",
             100 * c_compressedVectors /
                 (c_compressedVectors + c_uncompressedVectors));
#else
  int i, j;
  int total;
  changeVectorLog_t *cv;

  qsort(changeVectorLog, numChangeVectorLogs, sizeof(changeVectorLog_t),
        CompareCV);
  total = 0;
...
netField_t entityStateFields[] = {
    ...
    {NETF(event), 10},
    {NETF(eventParm), 8},
    {NETF(eventSequence), 8}, // warning: need to modify cg_event.c at "// check
                              // the sequencial list" if you change this
    {NETF(events[0]), 8},
    {NETF(events[1]), 8},
    {NETF(events[2]), 8},
    {NETF(events[3]), 8},
