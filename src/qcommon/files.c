...
qboolean FS_PakIsPure(pack_t *pack) {
  ...
  if (fs_numServerPaks) {
    ...
    //   this means you can have the same pk3 as the server under a different
    //   name, you will still get through sv_pure validation (what happens when
    //   two pk3's have the same checkums? is it a likely situation?) also, if
    //   there's a wrong checksumed pk3 and autodownload is enabled, the
    //   checksum will be appended to the downloaded pk3 name
...
=================
*/
int FS_LoadStack() { return fs_loadStack; }
...
===========
*/
static void FS_Remove(const char *osPath) { remove(osPath); }
...
Do a binary check of the two files, return qfalse if they are different,
otherwise qtrue
===========
*/
qboolean FS_FileCompare(const char *s1, const char *s2) {
  ...
  f2 = fopen(s2, "rb");
  if (!f2) { // this file is allowed to not be there, since it might not exist
    ...
    if (search->pack && search->pack->hashTable[hash]) {
      ...
      do {
        ...
        if (!FS_FilenameCompare(pakFile->name, filename)) {
          ...
          if (!(pak->referenced & FS_QAGAME_REF) &&
              FS_ShiftedStrStr(filename, "dTZT`X!di`", 13)) {
      ...
      if (fs_restrict->integer || fs_numServerPaks) {
        ...
        if (Q_stricmp(filename + l - 4, ".cfg") // for config files
            //					&& Q_stricmp( filename + l - 5, ".menu" )	// menu
            //files
  ...
  *listbuf = 0;
  ...
  for (i = 0; i < nPotential; i++) {
    ...
    if (Q_stricmp(name, BASEGAME) && Q_stricmpn(name, ".", 1)) {
      ...
      Sys_FreeFileList(pPaks); // we only use Sys_ListFiles to check wether .pk3
                               // files are present
...
static void FS_AddGameDirectory(const char *path, const char *dir) {
  ...
  for (i = 0; i < numfiles; i++) {
    ...
    // JPW NERVE sp_* to _p_* so "sp_pak*" gets alphabetically sorted before
    // "pak*"
    //----(SA)	SP mod
...
qboolean FS_idPak(char *pak, char *base) {
  ...
  for (i = 0; i < NUM_ID_PAKS; i++) {
    ...
    // downloading/overwriting official files, so exclude both SP and MP files
