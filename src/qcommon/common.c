  ...
  if (com_logfile && com_logfile->integer) {
    ...
    if (!logfile && FS_Initialized() && !opening_qconsole) {
      ...
      logfile = FS_FOpenFileWrite(
          "rtcwconsole.log"); //----(SA)	changed name for Wolf
...
==============================================================================
  ...
  The old zone is gone, mallocs replaced it. To keep the widespread code changes
down to a bare minimum Z_Malloc and Z_Free still work.
*/
...
void Com_InitHunkMemory(void) {
  ...
  if (com_dedicated && com_dedicated->integer) {
    ...
    pMsg = "Minimum com_hunkMegs for a dedicated server is %i, allocating %i "
           "megs.\n";
  } else {
...
#else
  FS_Printf(f, "// id Software and Activision will NOT ask you to send this "
               "file to them.\r\n");
#endif
  FS_FCloseFile(f);
}
...
void Com_Init(char *commandLine) {
  ...
  if (!com_dedicated->integer) {
    ...
    if (!com_introPlayed->integer) {
      ...
      PlayIntroMovies();
#endif
      // Cvar_Set( com_introPlayed->name, "1" );		//----(SA)
      // force this to get played every time (but leave cvar for override)
...
void _copyDWord(unsigned int *dest, const unsigned int constant,
...
qboolean Com_Memcmp(const void *src0, const void *src1,
                    const unsigned int count) {
  unsigned int i;
  // MMX version anyone?
