...
static rserr_t GLW_SetMode(const char *drivername, int mode, int colorbits,
...
*/
static qboolean GLW_StartDriverAndSetMode(const char *drivername, int mode,
                                          int colorbits,
                                          qboolean cdsFullscreen) {
  rserr_t err;
...
static qboolean GLW_CreateWindow(const char *drivername, int width, int height,
  ...
  if (!g_wv.hWnd) {
    ...
    g_wv.hWnd =
        CreateWindowEx(exstyle, WINDOW_CLASS_NAME, "Wolfenstein", stylebits, x,
  ...
  */
  ...
  if (strstr(glConfig.extensions_string, "GL_ATI_pn_triangles")) {
    if (r_ext_ATI_pntriangles->integer) {
      ...
      qglPNTrianglesiATI =
          (PFNGLPNTRIANGLESIATIPROC)qwglGetProcAddress("glPNTrianglesiATI");
      qglPNTrianglesfATI =
          (PFNGLPNTRIANGLESFATIPROC)qwglGetProcAddress("glPNTrianglesfATI");
...
void GLimp_Init(void) {
  ...
  if (strstr(buf, "geforce") || strstr(buf, "ge-force") ||
      strstr(buf, "radeon") || strstr(buf, "nv20") || strstr(buf, "nv30") ||
      strstr(buf, "quadro")) {
