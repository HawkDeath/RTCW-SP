...
static LONG WINAPI ConWndProc(HWND hWnd, UINT uMsg, WPARAM wParam,
  ...
  case WM_CTLCOLORSTATIC:
    if ((HWND)lParam == s_wcd.hwndBuffer) {
      ...
      //			SetBkColor( ( HDC ) wParam, RGB( 0x00, 0x00,
      //0xB0 ) ); 			SetTextColor( ( HDC ) wParam, RGB( 0xff, 0xff, 0x00 ) );
  ...
  case WM_COMMAND:
    ...
    break;
  case WM_CREATE:
    //		s_wcd.hbmLogo = LoadBitmap( g_wv.hInstance, MAKEINTRESOURCE(
    //IDB_BITMAP1 ) ); 		s_wcd.hbmClearBitmap = LoadBitmap( g_wv.hInstance,
...
void Sys_CreateConsole(void) {
  ...
  s_wcd.hWnd = CreateWindowEx(
      0, DEDCLASS, "Wolf Console", DEDSTYLE, (swidth - 600) / 2,
