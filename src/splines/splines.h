...
extern void glLabeledPoint(idVec3 &color, idVec3 &point, float size,
                           const char *label);
...
public:
  ...
  int selectPointByRay(float ox, float oy, float oz, float dx, float dy,
                       float dz, bool single) {
    idVec3 origin(ox, oy, oz);
    idVec3 dir(dx, dy, dz);
    return selectPointByRay(origin, dir, single);
  }

  int selectPointByRay(const idVec3 origin, const idVec3 direction,
                       bool single) {
    int i, besti, count;
    float d, bestd;
    idVec3 temp, temp2;
...
public:
  ...
  void setColors(idVec3 &path, idVec3 &segment, idVec3 &control,
                 idVec3 &active) {
    pathColor = path;
...
public:
  ...
  void getActiveSegmentInfo(int segment, idVec3 &origin, idVec3 &direction,
                            float *fv);
