...
void debugLine(idVec3 &color, float x, float y, float z, float x2, float y2,
               float z2) {
  //	idVec3 from(x, y, z);
  //	idVec3 to(x2, y2, z2);
...
void idCameraDef::getActiveSegmentInfo(int segment, idVec3 &origin,
                                       idVec3 &direction, float *fov) {
#if 0
...
void idCameraDef::buildCamera() {
  ...
  for (i = 0; i < events.Num(); i++) {
    ...
    case idCameraEvent::EVENT_WAIT: {
      ...
      startTime =
          (long int)(events[i]->getTime() + atof(events[i]->getParam()) * 1000);
      long endTime = startTime + 1000;
      speed = 0;
      while (startTime <= endTime) {
        cameraPosition->addVelocity(startTime, loopTime, speed);
...
void idCameraDef::addEvent(idCameraEvent::eventType t, const char *param,
                           long time) {
