...
int Com_AddToGrowList(growList_t *list, void *data) {
  ...
  if (list->maxElements < 0) {
    Com_Error(ERR_FATAL, "Com_AddToGrowList: maxElements = %i",
              list->maxElements);
  }
...
void *Com_GrowListElement(const growList_t *list, int index) {
  if (index < 0 || index >= list->currentElements) {
    Com_Error(ERR_DROP, "Com_GrowListElement: %i out of range of %i", index,
...
===================
*/
void Info_NextPair(const char *(*head), char key[MAX_INFO_KEY],
