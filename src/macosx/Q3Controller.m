...
{
  NS_DURING { [self quakeMain]; }
  NS_HANDLER { Sys_Error("%@", [localException reason]); }
  NS_ENDHANDLER;
  Sys_Quit();
}
    ...
    (NSApplication *)sender;
{
  ...
  choice = NSRunAlertPanel(nil, @"Quit without saving?", @"Don't Quit", @"Quit",
                           nil);
...
- (void)showBanner;
{
  ...
  if (!hasShownBanner) {
    ...
    if (showBanner->integer != 0) {
      ...
      bannerImage = [[NSImage alloc]
          initWithContentsOfFile:[[NSBundle mainBundle]
                                     pathForImageResource:@"banner.jpg"]];
      bannerRect = NSMakeRect(0.0, 0.0, [bannerImage size].width,
                              [bannerImage size].height);

      splashPanel = [[NSPanel alloc] initWithContentRect:bannerRect
...
- (void)connectToServer:(NSPasteboard *)pasteboard
...
- (void)performCommand:(NSPasteboard *)pasteboard
              userData:(NSString *)data
                 error:(NSString **)error;
{
  NSArray *pasteboardTypes;
...
- (void)quakeMain;
{
  ...
  for (argumentIndex = 0; argumentIndex < argumentCount; argumentIndex++) {
    ...
    // Don't pass the Process Serial Number command line arg that the Window
    // Server/Finder invokes us with
  ...
  installationPath =
      [[NSUserDefaults standardUserDefaults] objectForKey:installationPathKey];
  if (!installationPath) {
    // Default to the directory containing the executable (which is where most
    // users will want to put it
    installationPath =
...
#warning TJW: We are hard coding the app name here since the dedicated server is a tool, not a app bundle and does not have access to the Info.plist that the client app does.  Suck.
  ...
  while (YES) {
    ...
    if ([defaultManager fileExistsAtPath:dataPath]) {
      // Check that the data directory contains at least one .pk3 file.  We
      // don't know what it will be named, so don't hard code a name (for
      // example it might be named 'french.pk3' for a French release
      NSArray *files;
...
#warning TJW: We are hard coding the app name and default domain here since the dedicated server is a tool, not a app bundle and does not have access to the Info.plist that the client app does.  Suck.
    NSLog(@"Unable to determine installation directory.  Please move the "
          @"executable into the '%@' installation directory or add a '%@' key "
          @"in the 'Q3DedicatedServer' defaults domain.",
          appName, installationPathKey,
    ...
    result = NSRunAlertPanel(
        demoAppName,
        @"You need to select the installation directory for %@ (not any "
        @"directory inside of it -- the installation directory itself).",
    ...
    if (result == NSOKButton) {
      ...
      if ([filenames count] == 1) {
        ...
        [[NSUserDefaults standardUserDefaults] setObject:installationPath
                                                  forKey:installationPathKey];
  ...
  do {
    ...
    attributes = [NSDictionary
        dictionaryWithObjectsAndKeys:[NSNumber numberWithUnsignedInt:0750],
    ...
    NS_HANDLER {
      ...
      NSRunAlertPanel(
          nil,
          @"Unable to create '%@'.  Please make sure that you have permission "
          @"to write to this folder and re-run the game.",
          @"OK", nil, nil, homePath);
#endif
      Sys_Quit();
    }
  ...
  for (commandLineLength = 1, argumentIndex = 1; argumentIndex < argc;
       argumentIndex++)
...
static void Sys_CreatePathToFile(NSString *path, NSDictionary *attributes) {
  ...
  for (dirIndex = startingIndex; dirIndex < dirCount; dirIndex++) {
    ...
    partialPath = [NSString
        pathWithComponents:[pathComponents
                               subarrayWithRange:NSMakeRange(0, dirIndex + 1)]];

    // Don't use the 'fileExistsAtPath:isDirectory:' version since it doesn't
    // traverse symlinks
    ...
    if (!fileExists) {
      ...
      if (![[attributes objectForKey:NSFileType]
        ...
        [NSException raise:NSGenericException
                    format:@"Unable to write to path \"%@\" because \"%@\" is "
                           @"not a directory",
                           path, partialPath];
      }
    }
