...
print "// This has to be done to avoid infinite recursion between our "
      "glGetError wrapper and QGLCheckError()\n";
...
#print "argName='$argName'\n";
    ...
    if ($ #argTypes >= 0) {
      ...
      for ($i = 0; $i <= $ #argTypes; $i++) {
        ...
        elsif(/ ^GLsizei$ / || / ^GLint$ /) { print "$argName=%ld"; }
        elsif(/ ^GLfloat$ / || / ^GLdouble$ / || / ^GLclampf$ / ||
              / ^GLclampd$ /) {

          print "$argName=%f";
        }
        elsif(/ ^GLbyte$ /) { print "$argName=%d"; }
        elsif(/ ^GLubyte$ /) { print "$argName=%u"; }
        elsif(/ ^GLshort$ /) { print "$argName=%d"; }
        elsif(/ ^GLushort$ /) { print "$argName=%u"; }
        elsif(/ ^GLboolean$ /) { print "$argName=%u"; }
        elsif(/\* $ /) {

#TJW-- Later we should look at the count specified in the function name,       \
    look at the basic type and print out an array.Or we could just             \
        special case them ...
