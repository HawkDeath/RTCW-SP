...
OSStatus audioDeviceIOProc(AudioDeviceID inDevice, const AudioTimeStamp *inNow,
  ...
  //    assert(outOutputData->mBuffers[0].mDataByteSize == (dma.submission_chunk
  //    * sizeof(float)));
  ...
  if (s_chunkCount * submissionChunk > dma.channels * s_paintedtime) {
    ...
    if (outputStreamBasicDescription.mSampleRate == 44100) {
      for (sampleIndex = 0; sampleIndex < dma.submission_chunk; sampleIndex++) {
        // Convert the samples from shorts to floats.  Scale the floats to be
        // [-1..1].
...
qboolean SNDDMA_Init(void) {
  ...
  if (bufferSize->integer % chunkSize->integer) {
    ri.Error(ERR_FATAL,
             "s_buffersize must be an even multiple of s_chunksize\n");
  }
  ...
  if (outputDeviceID == kAudioDeviceUnknown) {
    Com_Printf(
        "AudioHardwareGetProperty: outputDeviceID is kAudioDeviceUnknown\n");
    return qfalse;
  }
  ...
  status = AudioDeviceSetProperty(outputDeviceID, NULL, 0, NO,
                                  kAudioDevicePropertyBufferSize, propertySize,
                                  &bufferByteCount);
  if (status) {
    Com_Printf("AudioDeviceSetProperty: returned %d when setting "
               "kAudioDevicePropertyBufferSize to %d\n",
               status, chunkSize->integer);
    return qfalse;
  }

  propertySize = sizeof(bufferByteCount);
  status = AudioDeviceGetProperty(outputDeviceID, 0, NO,
                                  kAudioDevicePropertyBufferSize, &propertySize,
                                  &bufferByteCount);
  if (status) {
    Com_Printf("AudioDeviceGetProperty: returned %d when setting "
  ...
  if (status) {
    Com_Printf("AudioDeviceGetProperty: returned %d when getting "
               "kAudioDevicePropertyStreamFormat\n",
               status);
    return qfalse;
  }
...
===============
*/
int SNDDMA_GetDMAPos(void) { return s_chunkCount * dma.submission_chunk; }
