...
static /*inline*/ void _SendMsg(MsgPort *port, unsigned int msgCode,
                                void *msgData, const char *functionName,
                                const char *portName, const char *msgName) {
  int rc;

#if USE_MACH_PORTS
  mach_msg_header_t msg;

  // printf("SendMsg: %s %s %s (%d %08lx)\n",functionName, portName, msgName,
  // msgCode, msgData);
  /*
   typedef	struct
   {
...
static /*inline*/ void _WaitMsg(MsgPort *port, unsigned int *msgCode,
                                void **msgData, const char *functionName,
                                const char *portName) {
  int rc;
#if USE_MACH_PORTS
...
void *GLimp_RendererSleep(void) {
  ...
  //_Log("rs0\n");
  /* If we actually had some work to do, then tell the front end we completed
   * it. */
  if (rendererProcessingCommand) {
    SendMsg(&frontEndMsgPort, RenderCompletedMsg, NULL);
    rendererProcessingCommand = NO;
...
#ifndef USE_CGLMACROS
  // We are done waiting for the background thread, take the current context
  // back.
  OSX_GLContextSetCurrent();
#endif
...
void GLimp_WakeRenderer(void *data) {
  ...
  // Don't set flag saying that the renderer is processing something if it's
  // just being told to exit.
  // if(data != NULL)
