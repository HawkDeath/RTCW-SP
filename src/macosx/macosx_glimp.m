...
#define ADD_ATTR(x)                                                            \
  do {                                                                         \
    if (attributeIndex >= attributeSize) {                                     \
      attributeSize *= 2;                                                      \
      pixelAttributes = NSZoneRealloc(                                         \
          NULL, pixelAttributes, sizeof(*pixelAttributes) * attributeSize);    \
    }                                                                          \
...
static NSOpenGLPixelFormatAttribute *GetPixelAttributes() {
  ...
  pixelAttributes =
      NSZoneMalloc(NULL, sizeof(*pixelAttributes) * attributeSize);
  ...
  // Specify the number of color bits.  If we don't have a valid specified value
  // or we are not full screen, use the current display mode's value.
  ...
  if (!r_fullscreen->integer) {
    colorDepth = [[glw_state.desktopMode
        objectForKey:(id)kCGDisplayBitsPerPixel] intValue];
  }
  ADD_ATTR(colorDepth);
...
void Sys_UpdateWindowMouseInputRect(void) {
  ...
  windowRect.origin.y =
      screenRect.size.height - (windowRect.origin.y + windowRect.size.height);
...
// This is needed since CGReleaseAllDisplays() restores the gamma on the
// displays and we want to fade it up rather than just flickering all the
// displays
static void ReleaseAllDisplays() {
  CGDisplayCount displayIndex;

  Com_Printf("Releasing displays\n");
  for (displayIndex = 0; displayIndex < glw_state.displayCount;
       displayIndex++) {
    CGDisplayRelease(
        glw_state.originalDisplayGammaTables[displayIndex].display);
  }
}
...
static qboolean CreateGameWindow(qboolean isSecondTry) {
  ...
  glw_state.desktopMode =
      (NSDictionary *)CGDisplayCurrentMode(glw_state.display);
  ...
  if (!R_GetModeInfo(&glConfig.vidWidth, &glConfig.vidHeight,
                     &glConfig.windowAspect, current_mode)) {
    ri.Printf(PRINT_ALL, " invalid mode\n");
    return qfalse;
  }
  ri.Printf(PRINT_ALL, " %d %d %s\n", glConfig.vidWidth, glConfig.vidHeight,
            windowed[glConfig.isFullscreen]);

  if (glConfig.isFullscreen) {

    // We'll set up the screen resolution first in case that effects the list of
    // pixel formats that are available (for example, a smaller frame buffer
    // might mean more bits for depth/stencil buffers).  Allow stretched video
    // modes if we are in fallback mode.
    ...
    err = CGDisplaySwitchToMode(glw_state.display,
                                (CFDictionaryRef)glw_state.gameMode);
    if (err != CGDisplayNoErr) {
      CGDisplayRestoreColorSyncSettings();
      ReleaseAllDisplays();
  ...
  pixelFormat = [[[NSOpenGLPixelFormat alloc]
      initWithAttributes:pixelAttributes] autorelease];
  NSZoneFree(NULL, pixelAttributes);

  if (!pixelFormat) {
    CGDisplayRestoreColorSyncSettings();
    CGDisplaySwitchToMode(glw_state.display,
                          (CFDictionaryRef)glw_state.desktopMode);
    ReleaseAllDisplays();
  ...
  OSX_SetGLContext([[NSOpenGLContext alloc] initWithFormat:pixelFormat
                                              shareContext:nil]);
...
#warning TJW: might be nice to change the title of the window to have the version number, current server or something like that (maybe even preferences for what to put in the title).
  ...
  [pixelFormat getValues:(long *)&glConfig.colorBits
            forAttribute:NSOpenGLPFAColorSize
        forVirtualScreen:0];
  [pixelFormat getValues:(long *)&glConfig.depthBits
            forAttribute:NSOpenGLPFADepthSize
        forVirtualScreen:0];
  [pixelFormat getValues:(long *)&glConfig.stencilBits
            forAttribute:NSOpenGLPFAStencilSize
        forVirtualScreen:0];

  glConfig.displayFrequency =
      [[glw_state.gameMode objectForKey:(id)kCGDisplayRefreshRate] intValue];
...
// This can be used to temporarily disassociate the GL context from the screen
// so that CoreGraphics can be used to draw to the screen.
void Sys_PauseGL() {
  if (!glw_state.glPauseCount) {
    qglFinish(); // must do this to ensure the queue is complete

    // Have to call both to actually deallocate kernel resources and free the
    // NSSurface
...
void GLimp_Init(void) {
  ...
  if (Sys_QueryVideoMemory() == 0 && !r_allowSoftwareGL->integer) {
    ri.Error(ERR_FATAL,
             "Could not initialize OpenGL.  There does not appear to be an "
             "OpenGL-supported video card in your system.\n");
  }
  ...
  if (r_swapInterval->modified) {
    ...
    if (!glConfig.stereoEnabled) { // why?
      [[NSOpenGLContext currentContext]
             setValues:(long *)&r_swapInterval->integer
...
static void _GLimp_RestoreOriginalVideoSettings() {
  ...
  // CGDisplayCurrentMode lies because we've captured the display and thus we
  // won't get any notifications about what the current display mode really is.
  // For now, we just always force it back to what mode we remember the desktop
  // being in.
  if (glConfig.isFullscreen) {
    err = CGDisplaySwitchToMode(glw_state.display,
                                (CFDictionaryRef)glw_state.desktopMode);
...
void GLimp_SetGamma(unsigned char red[256], unsigned char green[256],
  ...
  err = CGSetDisplayTransferByTable(glw_state.display, 256, redGamma,
  ...
  // Store the gamma table that we ended up using so we can reapply it later
  // when unhiding or to work around the bug where if you leave the game sitting
  // and the monitor sleeps, when it wakes, the gamma isn't reset.
...
static void GLW_InitExtensions(void) {
...
#if 0 // Win32 does this differently than we do -- I'll provide a C function
      // that looks the same that will do the correct ObjC stuff
  ...
  if (r_swapInterval) {
    ri.Printf(
        PRINT_ALL,
        "...using +[NSOpenGLContext setParameter:] for qwglSwapIntervalEXT\n");
    r_swapInterval->modified = qtrue; // force a set next frame
  }
#endif
    ...
    if (r_ext_multitexture->integer) {
      ...
      if (qglActiveTextureARB) {
        qglGetIntegerv(GL_MAX_ACTIVE_TEXTURES_ARB,
                       (GLint *)&glConfig.maxActiveTextures);
  ...
  if (strstr(glConfig.extensions_string, "GL_EXT_compiled_vertex_array") &&
    ...
    if (r_ext_compiled_vertex_array->integer) {
      ...
      qglLockArraysEXT =
          (void(APIENTRY *)(GLint, GLint))qwglGetProcAddress("glLockArraysEXT");
      qglUnlockArraysEXT =
  ...
  if (strstr(glConfig.extensions_string, "GL_APPLE_transform_hint")) {
    r_appleTransformHint =
        ri.Cvar_Get("r_appleTransformHint", "1", CVAR_ARCHIVE);
...
// Returns zero if there are no hardware renderers.  Otherwise, returns the max
// memory across all renderers (on the presumption that the screen that we'll
// use has the most memory).
static unsigned long Sys_QueryVideoMemory() {
  CGLError err;
  ...
  for (rendererInfoIndex = 0; rendererInfoIndex < rendererInfoCount &&
                              totalRenderers < rendererInfoCount;
       rendererInfoIndex++) {
    rendererInfo = rendererInfos[rendererInfoIndex];
    // Com_Printf("rendererInfo: 0x%08x\n", rendererInfo);

    err = CGLDescribeRenderer(rendererInfo, 0, kCGLRPRendererCount,
                              &rendererCount);
    if (err) {
    ...
    for (rendererIndex = 0; rendererIndex < rendererCount; rendererIndex++) {
      ...
      err = CGLDescribeRenderer(rendererInfo, rendererIndex, kCGLRPRendererID,
                                &rendererID);
      if (err) {
        Com_Printf("CGLDescribeRenderer(kCGLRPRendererID) -> %d\n", err);
        continue;
      ...
      accelerated = 0;
      err = CGLDescribeRenderer(rendererInfo, rendererIndex, kCGLRPAccelerated,
                                &accelerated);
      if (err) {
      ...
      vram = 0;
      err = CGLDescribeRenderer(rendererInfo, rendererIndex, kCGLRPVideoMemory,
                                &vram);
      if (err) {
      ...
      // presumably we'll be running on the best card, so we'll take the max of
      // the vrams
      if (vram > maxVRAM) {
        maxVRAM = vram;
      }
...
// We will set the Sys_IsHidden global to cause input to be handle differently
// (we'll just let NSApp handle events in this case).  We also will unbind the
// GL context and restore the video mode.
qboolean Sys_Hide() {
  if (Sys_IsHidden) {
    // Eh?
    return qfalse;
  ...
  // Don't need to store the current gamma since we always keep it around in
  ...
  // Shut down the input system so the mouse and keyboard settings are restore
  // to normal
  Sys_ShutdownInput();

  // Hide the application so that when the user clicks on our app icon, we'll
  // get an unhide notification
  [NSApp hide:nil];
...
qboolean Sys_Unhide() {
  ...
  // Restore the input system (last so if something goes wrong we don't eat the
  // mouse)
  Sys_InitInput();
