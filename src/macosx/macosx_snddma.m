...
// TJW - Different versions of SoundManager have different DMA buffer sizes.  On
// MacOS X DP2, the buffer size is 8K.  On MacOS 9 it is much smaller.  The
// SoundManager guy at Apple says that the size of the buffer will be decreasing
// for final release to help get rid of latency.
...
qboolean SNDDMA_Init(void) {
  ...
  if (bufferSize->integer % chunkSize->integer) {
    ri.Error(ERR_FATAL,
             "snd_bufferSize must be an even multiple of snd_chunkSize\n");
  }

  // create a sound channel
  s_sndChan = NULL;
  err = SndNewChannel(&s_sndChan, sampledSynth, initStereo,
                      NewSndCallBackProc(S_Callback));
  if (err) {
    return false;
  }
  ...
  s_mixedSamples =
      NSZoneMalloc(NULL, sizeof(*s_mixedSamples) * maxMixedSamples);
...
===============
*/
int SNDDMA_GetDMAPos(void) { return s_chunkCount * submissionChunk; }
