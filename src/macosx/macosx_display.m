...
NSDictionary *Sys_GetMatchingDisplayMode(qboolean allowStretchedModes) {
  ...
  if (cMinFreq && cMaxFreq && cMinFreq->integer && cMaxFreq->integer &&
    ...
    ri.Error(ERR_FATAL, "r_minDisplayRefresh must be less than or equal to "
                        "r_maxDisplayRefresh");
  }
  ...
  if (!displayModes) {
    ri.Error(
        ERR_FATAL,
        "CGDisplayAvailableModes returned NULL -- 0x%0x is an invalid display",
        glw_state.display);
  }
  ...
  for (modeIndex = 0; modeIndex < modeCount; ++modeIndex) {
    ...
    if (verbose) {
      ri.Printf(PRINT_ALL, " mode %d -- %s\n", modeIndex,
                [[mode description] cString]);
    }

    // Make sure we get the right size
    object = [mode objectForKey:(id)kCGDisplayWidth];

    if ([[mode objectForKey:(id)kCGDisplayWidth] intValue] !=
            glConfig.vidWidth ||
        [[mode objectForKey:(id)kCGDisplayHeight] intValue] !=
            glConfig.vidHeight) {
      if (verbose) {
    ...
    if (!allowStretchedModes) {
      if ([[mode objectForKey:(id)kCGDisplayIOFlags] intValue] &
    ...
    if ([[mode objectForKey:(id)kCGDisplayBitsPerPixel] intValue] !=
        colorDepth) {
      if (verbose) {
...
void Sys_GetGammaTable(glwgamma_t *table) {
  ...
  // TJW: We _could_ loop here if we get back the same size as our table,
  // increasing the table size.
  err =
      CGGetDisplayTransferByTable(table->display, tableSize, table->red,
                                  table->green, table->blue, &table->tableSize);
...
void Sys_StoreGammaTables() {
  // Store the original gamma for all monitors so that we can fade and unfade
  // them all
  ...
  glw_state.originalDisplayGammaTables = calloc(
      glw_state.displayCount, sizeof(*glw_state.originalDisplayGammaTables));
...
void Sys_SetScreenFade(glwgamma_t *table, float fraction) {
  ...
  CGSetDisplayTransferByTable(table->display, table->tableSize, red, green,
                              blue);
}
