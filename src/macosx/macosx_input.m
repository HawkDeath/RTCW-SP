...
static void Sys_PreventMouseMovement(CGPoint point) {
  ...
  if (err != CGEventNoErr) {
    Sys_Error("Could not disable mouse movement, "
              "CGAssociateMouseAndMouseCursorPosition returned %d\n",
              err);
  }
  ...
  if (err != CGEventNoErr) {
    Sys_Error("Could not disable mouse movement, CGWarpMouseCursorPosition "
              "returned %d\n",
              err);
  }
}
...
static void Sys_ReenableMouseMovement() {
  ...
  if (err != CGEventNoErr) {
    Sys_Error("Could not reenable mouse movement, "
              "CGAssociateMouseAndMouseCursorPosition returned %d\n",
              err);
  }
...
void Sys_InitInput(void) {
  ...
  // For hide support.  If we don't do this, then the command key will get stuck
  // on when we hide (since we won't get the flags changed event when it goes
  // up).
...
#warning TJW: r_fullscreen is not normally set by the time we get here, so we have to initialize it with copy-n-paste from tr_init.c
  ...
  // these defaults were arrived at via emprical testing between a Windows box
  // and a Mac OS X box
...
static void Sys_LockMouseInInputRect(CGRect rect) {
  ...
  // Now, put the mouse in the middle of the input rect (anywhere over it would
  // do) and don't allow it to move.  This means that the user won't be able to
  // accidentally select another application.
...
static void Sys_StartMouseInput() {
  ...
  if (mouseactive) {
    // Com_Printf("**** Attempted to start mouse input while already
    // started\n");
    return;
  }
  ...
  if ((eventStatus = NXOpenEventStatus())) {
    ...
    // Do the effing keyboard too, so we don't get flooded with key repeat
    // events.
...
CGDirectDisplayID Sys_DisplayToUse(void) {
  ...
  if (displayIndex < 0 || displayIndex >= displayCount) {
    // This is documented (in CGDirectDisplay.h) to be the main display.  We
    // want to return this instead of kCGDirectMainDisplay since this will allow
    // us to compare display IDs.
...
void Sys_SetMouseInputRect(CGRect newRect) {
  ...
  // newRect.origin.y, newRect.size.width, newRect.size.height);
...
static void Sys_ProcessMouseMovedEvent(NSEvent *mouseMovedEvent,
                                       int currentTime) {
  CGMouseDelta dx, dy;
  ...
  // The AppKit NSEvent doesn't support delta mouse movement.  We can (and have
  // in the past) kept track of the previous position and on each event computed
  // the delta.  When the mouse approached the edge of the window rect, we'd
  // warp it back to the center.  This was ugly and had a bunch of edge cases.
...
// If we are 'paused' (i.e., in any state that our normal key bindings aren't in
// effect), then interpret cmd-h and cmd-tab as hiding the application.
...
static inline void sendEventForCharacter(NSEvent *event, unichar character,
                                         qboolean keyDownFlag,
                                         int currentTime) {
  if (in_showevents->integer) {
  ...
  if (character == NSF9FunctionKey && !keyDownFlag) {
    // Log and reset the root timer.  We should currently only have the root on
    // the stack.
    OTStackPopRoot();
  ...
  default:
    if (character >= 'a' && character <= 'z') {
      ...
      Sys_QueEvent(currentTime, SE_KEY, 'a' + (character - 'A'), keyDownFlag, 0,
                   NULL);
      if (keyDownFlag) {
...
static inline void processKeyEvent(NSEvent *keyEvent, qboolean keyDownFlag,
                                   int currentTime) {
  ...
  for (characterIndex = 0; characterIndex < characterCount; characterIndex++) {
    sendEventForCharacter(keyEvent,
                          [characters characterAtIndex:characterIndex],
                          keyDownFlag, currentTime);
  }
}

static inline void sendEventForMaskChangeInFlags(int quakeKey,
...
static inline void processFlagsChangedEvent(NSEvent *flagsChangedEvent,
                                            int currentTime) {
  int newModifierFlags;

  newModifierFlags = [flagsChangedEvent modifierFlags];
  sendEventForMaskChangeInFlags(K_COMMAND, NSCommandKeyMask, newModifierFlags,
                                currentTime);
  sendEventForMaskChangeInFlags(K_CAPSLOCK, NSAlphaShiftKeyMask,
                                newModifierFlags, currentTime);
  sendEventForMaskChangeInFlags(K_ALT, NSAlternateKeyMask, newModifierFlags,
                                currentTime);
  sendEventForMaskChangeInFlags(K_CTRL, NSControlKeyMask, newModifierFlags,
                                currentTime);
  sendEventForMaskChangeInFlags(K_SHIFT, NSShiftKeyMask, newModifierFlags,
                                currentTime);
  currentModifierFlags = newModifierFlags;
}

static inline void processSystemDefinedEvent(NSEvent *systemDefinedEvent,
                                             int currentTime) {
...
static inline void processEvent(NSEvent *event, int currentTime) {
  ...
  switch (eventType) {
    // These six event types are ignored since we do all of our mouse down/up
    // process via the uber-mouse system defined event.  We have to accept these
    // events however since they get enqueued and the queue will fill up if we
    // don't.
...
static void Sys_SendKeyEvents(int currentTime) {
  ...
  if (Sys_IsHidden) {
    timeout =
        [NSDate dateWithTimeIntervalSinceNow:0.25 * SNDDMA_GetBufferDuration()];
  }

  // This gets call regardless of whether inputActive is true or not.  This is
  // important since we need to be poking the event queue in order for the
  // unhide event to make its way through the system.  This means that when we
  // hide, we can just shut down the input system and reeanbled it when we
  // unhide.
  while ((event = [NSApp nextEventMatchingMask:NSAnyEventMask
    ...
    if (Sys_IsHidden) {
      ...
      // Mac OS X 10.0.3 has a bug where the if the monitor goes to sleep in
      // fullscreen GL mode, the gamma won't be restored.  We'll restore the
      // gamma if there is a pause while in the game of more than 10 seconds. We
      // don't do this on the 'Sys_IsHidden' branch since unhiding will restore
      // the monitor gamma.
      if ((currentTime - lastEventTime > 1 * 1000) && lastEventTimeValid) {
        // Com_Printf("Restoring monitor gamma after being idle for %f
...
sysEvent_t Sys_GetEvent(void) {
  ...
  // The queue must be empty.  Check all of the event sources.  If the events
  // are already in the queue, we can't imply any real ordering, so we'll avoid
  // extra system calls and give them all the same time.
  ...
  // During debugging it is sometimes usefull to be able to start/stop mouse
  // input. Don't turn on the input when we've disabled it because we're hidden,
  // however.
  if (!com_dedicated->integer) {
  ...
  // Otherwise, return an empty event to indicate that there are no events
  // pending.
