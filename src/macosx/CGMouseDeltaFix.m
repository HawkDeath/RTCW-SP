...
static void CGFix_NotificationCallback(CGSNotificationType note,
...
void CGFix_Initialize() {
  ...
  if (registerNotifyProc) {
    // We've already been called once and have registered our callbacks.  If the
    // original version works, this will be NULL, but we'll end up doing nothing
    // (again, possibly).
    return;
  }
  ...
  version =
      [[cgBundle infoDictionary] objectForKey:@"CFBundleShortVersionString"];
  ...
  if (![[components objectAtIndex:0] isEqualToString:@"1"] ||
      [[components objectAtIndex:1] intValue] > 93) {
  ...
  registerNotifyProc =
      NSAddressOfSymbol(NSLookupAndBindSymbol("_CGSRegisterNotifyProc"));
  // NSLog(@"registerNotifyProc = 0x%08x", registerNotifyProc);

  // Must not work if we got here
  originalVersionShouldWork = NO;

  // We want to catch all the events that could possible indicate mouse movement
  // and sum them up
  registerNotifyProc(CGFix_NotificationCallback,
                     kCGSEventNotificationMouseMoved, NULL);
  registerNotifyProc(CGFix_NotificationCallback,
                     kCGSEventNotificationLeftMouseDragged, NULL);
  registerNotifyProc(CGFix_NotificationCallback,
                     kCGSEventNotificationRightMouseDragged, NULL);
  registerNotifyProc(CGFix_NotificationCallback,
                     kCGSEventNotificationNotificationOtherMouseDragged, NULL);
...
static void CGFix_NotificationCallback(CGSNotificationType note,
  ...
  // fprintf(stderr, "CGFix_NotificationCallback(note=%d, date=0x%08x,
  // dataLength=%d, arg=0x%08x)\n", note, data, dataLength, arg);
  ...
  if ((note != kCGSEventNotificationMouseMoved &&
    ...
    fprintf(stderr,
            "Unexpected arguments to callback function "
            "CGFix_NotificationCallback(note=%d, date=0x%08x, dataLength=%d, "
            "arg=0x%08x)\n",
            note, data, dataLength, arg);
  }
  abort();
...
// fprintf(stderr, "  dx += %d, dy += %d\n", event->data.move.deltaX,
// event->data.move.deltaY);
}
